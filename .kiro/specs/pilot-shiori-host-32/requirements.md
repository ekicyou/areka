...
- **Adjacent expectations（隣接系への前提・非所有）**:
  ...
  - **IPC 方式（設計議題・WM_COPYDATA を強く推奨）**: 先進坑の自前 IPC（要件 2）の具体方式の**最終決定は設計フェーズ（`/kiro-design`）の議題**とし、要件フェーズではハード固定しない。**強く推奨＝WM_COPYDATA**（HWND の受け渡しも含め通信を **Window Message に一本化**＝helper の `wintf-winmsg-executor` メッセージループの WndProc へ自然配送・統合作業ゼロ・single-in-flight 同期 req/resp・伺か idiom）。**named pipe は設計上の強い理由がある場合のみ**受容（全二重/大データ/push が要るとき。legacy=pull 専用・payload 小ゆえ host-32 では通常不要）。WM_COPYDATA を後押しする環境前提（i686 ビルド・`wintf-winmsg-executor` i686・`pasta.dll` cdecl flat-C）は技術調査で実証済（research.md §3.1.1 / §6）。
