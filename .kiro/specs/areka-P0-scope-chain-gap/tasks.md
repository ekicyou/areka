...
**要件 7 と design C6 は承認フローを一度も通っていない。** `spec.json` の `approvals` は 3 フェーズとも `approved: true` だが、これは **2026-08-11 時点の「要件 6 本・C0〜C5」の版に対する承認**である。要件 7 は 2026-08-13 に開発者裁定（差し戻し）で追加され、design C6・タスク 6.1〜6.4・実装・実機確認まで進んだが、`approvals` は更新していない（承認は開発者の権限のため AI 側で立てない）。

同様に `design-validation.md`（2026-08-11・Decision: **GO**）も要件 6 本・C0〜C5 を前提とした判定であり、**要件 7／C6 を含む版での設計検証は走っていない**。

`/kiro-complete` の DoD ゲートは `approvals` を見るため、**この欠落を検出せず通過する**。完了させる前に、要件 7／C6 を承認扱いとするか、`/kiro-validate-design` 等を回し直すかを裁定されたい。`updated_at` のみ実態（2026-08-13）へ更新済み。
