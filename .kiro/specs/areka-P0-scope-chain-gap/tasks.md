...
同様に `design-validation.md`（2026-08-11・Decision: **GO**）も要件 6 本・C0〜C5 を前提とした判定であり、**要件 7／C6 を含む版での設計検証は走っていない**。

`/kiro-complete` の DoD ゲートは `approvals` を見るため、**この欠落を検出せず通過する**。完了させる前に、下記「開発者裁定アジェンダ」を裁定されたい。`updated_at` のみ実態（2026-08-13）へ更新済み。
...
|---|---|---|---|---|
| 1 | 手続き | **要件 7／C6 の追補をどう正規化するか**。(a) この場で追認し tasks.md に記録、または (b) 現行版で `/kiro-validate-design` を回し直し、GO を確認してから追認 | (a) は第三者検証なしの承認（実装・実機証跡はある）。(b) は非対話・低コストで、承認の根拠が現行版の検証報告になる | **(b)** — 08-11 の GO は旧版にしか効いておらず、検証を回す費用は小さい |
