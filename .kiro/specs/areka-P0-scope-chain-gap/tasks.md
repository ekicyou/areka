...
- [ ] 6. 実表示寸での連鎖再解決（要件 7・差し戻しで追加）
- [ ] 6.1 既定位置の保持と「未接触」判定を用意する
...
- **開発者判断が要る点**: 宣言した範囲（初期配置の連鎖規則）で完了とみなすか、定常状態の隣接まで要件へ含めるか。後者を採る場合、連鎖の再評価は `windowposition-limit`・DPI 遷移系と干渉するため、本仕様の差し戻しではなく別仕様への起票が妥当と考える。
- 再現手段: フィクスチャを複製し `ghost/master/dic/{boot,menu}.pasta` の move 指令を除去したプローブで起動する（**フィクスチャ本体は非改変**）。本セッションのプローブは scratchpad に作成済み。

## 承認判断のための実機観察で発見した孤児欠陥（担当仕様なし・要起票）
