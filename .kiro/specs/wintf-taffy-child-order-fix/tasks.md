...
- [x] 1. (P) Layout レイヤーの階層同期修正
- [x] 1.1 (P) sync_taffy_tree_system に Children クエリを追加
...
- [x] 1.2 (P) Children 順序に基づく taffy 子ノード設定処理を実装
...
- [x] 2. (P) Graphics レイヤーの階層同期修正
- [x] 2.1 (P) visual_hierarchy_sync_system に Children クエリを追加
  - システムシグネチャに `children_query: Query<&Children>` パラメータを追加
  - Phase 1 の未同期エンティティ収集後、各未同期エンティティの親を `HashSet<Entity>` に収集（affected_parents）
  - _Requirements: 2.1, 2.2_

- [x] 2.2 (P) Children 順序に基づく Visual 階層再配置処理を実装
  - 各影響親について `children_query.get(parent)` で `Children` を取得
...
- [x] 3. (P) 回帰防止テストの実装
- [x] 3.1 (P) 異なるアーキタイプ兄弟の taffy ツリー順序テスト
