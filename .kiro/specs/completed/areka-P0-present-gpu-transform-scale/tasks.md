...
- [x] 1. 基盤: 表示の記録（原寸 bitmap → 描画命令）を新設する
...
- [x] 2. 表示経路を wintf のコマンドリスト経路へ付け替える
...
- [x] 3. 予算席・段階別計時・判定スクリプトを撤去分に合わせる
...
- [x] 4. drain 相の登録先を同一 tick で描かれる席へ移す
...
- [x] 5. 既存の檻を新しい不変条件へ導き直す（設計 付録 A′ の裁定）
...
- [x] 6. CPU リサンプラ本体とその檻を撤去する
- [x] 6.1 合成クレートからリサンプラを撤去し、寸法権威だけを残す
...
- [x] 8.2 実機 2 水準サインオフを行う
  ...
  - _Depends: 8.1_
  - _Manual: 2026-09-12 に開発者サインオフ済み（`verification/acceptance-record.md` §7・§8）。水準②（200%）は debug 自動走行でログ判定、続けて release ビルドで開発者立ち会いの実測と目視を行い合格。綴りどおりの 125% は未実施で、代わりに 150%（k=3/2）と 100%（k=1）を実施した（要件 2.7 の目的＝水準ごとに異なる物理寸・崩れなしは 3 水準で充足）。未消化＝胸の当たり判定・スクリーンショット・e2e §13.1 行 3 の目視・静かな機械でのティッカー追い付き採り直し_
...
- 8.2（続き・開発者サインオフ 2026-09-12）: release ビルド（`areka.exe` 5,043,712 B ＋ i686 release helper 130,560 B）で立ち会い実測。トーク間隔 15 秒・3 分窓。**200%（k=2）: CPU 0.83%（機械全体・22 論理コア）／外れ中央値 2.65 ms・p90 5.60・最大 10.17／ヒット 0.44 ms／定常確保 0 件／`ERROR` 0**。**100%（k=1）: CPU 1.08%／外れ中央値 5.02 ms**。**DPI 切替 6 回すべて `cache_hit=true`（要件 5.3 の実機証拠・§7.1）**。`[hit_region_client]` 217 行・`region=Some("Head")` 2 件（胸は未観測）。⚠100% 窓が重く出たのは tick 15 段すべてが一様に 2.2〜3.4 倍＝表示スケール切替直後の机上全体の作り直しの余波と判定（k 非依存の段も同率・60 秒再採で 0.95% へ復帰）。⚠ティッカー追い付きは 15 件／103 件で目標 10 件超だが、トーク間隔 15 秒＝目標の前提と条件が違うため合否に使えない。⚠release 走行は操作者停止ゆえ `unload_clean` の記録なし（正規終了は水準②の走行で確認済み）。⚠computer-use 不可でスクリーンショットは開発者の目視のみ。
