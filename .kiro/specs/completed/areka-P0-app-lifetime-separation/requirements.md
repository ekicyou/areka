...
> **要件 6 の裁定 1・2 は 2026-09-23 の要件ディスカッションで、裁定 3（OS の閉鎖要求）は同日の設計ディスカッションで開発者が確定した。** この形のまま設計・実装へ進む。
...
- **Windows 自身が窓を閉じる経路（`WM_CLOSE`＝Alt＋F4・`taskkill`（`/F` なし）・タスクマネージャの「タスクの終了」）は areka を通らない。** wintf の `WM_CLOSE` の処理（`crates/wintf/src/ecs/window_proc/lifecycle.rs` の `fn WM_CLOSE`）は届いた窓 1 枚の entity を消すだけで、areka に知らせる口が無い。今日はキャラ窓に Alt＋F4 を押すとその窓だけ消えて半端なゴーストになり、4 枚全部に押せば窓 0 で終わる（別れの台詞は無い）。本仕様の後にこの経路を放置すると「窓 0 なのにプロセスが残り、終わらせる入口が無い」状態を作る。
...
**areka のアプリは「終了の指示」で終わり、窓の数では終わらない。** 窓が 0 枚の瞬間があってもプロセスは生き続ける。今日の終了操作 6 種は、全窓を閉じたあとに明示の終了の指示を 1 つ出すようになり、利用者から見える終わり方は今までと区別が付かない。加えて 7 種目として **Windows からの閉鎖要求（Alt＋F4・`taskkill`・「タスクの終了」）を正規の終了操作にする**——ゴースト窓ではメニューの「終了」と同じ終了系列（別れの台詞 → 全窓を閉じる → 終了の指示）へ、ダミー窓では閉じて終了の指示へ。wintf には「窓 0 で終了するか」を利用側が選べる口、明示の終了の指示を受ける口、そして「閉鎖要求が来たら窓を消す代わりに呼ぶ関数」を窓に差せる部品を足す。wintf の既定は変えず、example は 1 文字も書き換えない。
...
- **In scope**:
  ...
  - wintf: 窓が OS の閉鎖要求（`WM_CLOSE`）を受けたとき、窓を消す代わりに利用側の関数を呼ぶ部品（付けなければ従来どおり窓を消す）。
  - areka: OS の閉鎖要求を 7 種目の終了操作として結線する（ゴースト窓＝メニューの「終了」と同じ終了要求を kanade へ・ダミー窓＝閉じて終了の指示）。
  - 常設の smoke テストの追随。
...
- **Out of scope**:
  ...
  - Alt＋F4 のキー入力そのものを SHIORI へ `OnKeyPress` として渡すかどうか（キーボード入力を持つ spec の判断。本仕様は「閉鎖要求として届いたら終了系列へ」だけを定める）。
  - Windows のログオフ／シャットダウン（`WM_QUERYENDSESSION`／`WM_ENDSESSION`）への応答。
...
8. The wintf shall 実行中に「窓 0 で終了する／しない」を切り替える口を持たない（裁定 1・要件 6）。
9. The wintf shall 窓が OS の閉鎖要求（`WM_CLOSE`）を受けたときに呼ぶ関数を、利用側が窓ごとに差せる部品を 1 つ持つ。
10. When 窓が OS の閉鎖要求を受ける and その窓に閉鎖要求の関数が差されている, the wintf shall 窓を消さず、その関数を呼ぶ。
11. When 窓が OS の閉鎖要求を受ける and その窓に閉鎖要求の関数が差されていない, the wintf shall 従来どおり窓を消す（example の振る舞いは変わらない）。

### Requirement 3: areka の終了操作 7 種は同じ見え方で終わる

**Objective:** As a areka の利用者, I want 今日ある終了操作のどれを使っても今までと同じように終わり、Windows からの閉鎖要求でも綺麗に終われること, so that 寿命の切り離しが利用者に見えず、標準の道具（Alt＋F4・`taskkill`・「タスクの終了」）で areka を閉じられる
...
6. The areka shall 「終了のために全窓を閉じる」操作と「終了の指示を出す」操作を 1 つの操作として行い、終了経路のどこにも片方だけを呼ぶ形を残さない（裁定 2・要件 6）。
7. When 終了の指示を出す, the areka shall どの終了操作から来たか（終了系列の完了・強制退避・ダミー窓・smoke・OS の閉鎖要求〈ダミー窓、および 3.10 の未結線のゴースト窓〉）を記録に残す（ライフサイクル事象として info）。
8. The areka shall 終了操作 7 種のどれでも、終了の指示のあとの後始末の順序と終了コード（0）を今日と同じに保つ。
9. When ゴースト窓（キャラ窓・バルーン窓）が OS の閉鎖要求を受ける, the areka shall その窓を消さず、メニューの「終了」と同じ終了要求（操作された窓のスコープを載せた `CloseReason::User`）を kanade へ 1 件送る（以後は 3.1 の経路＝別れの台詞 → 全窓を閉じる → 終了の指示）。
10. When ゴースト窓が OS の閉鎖要求を受ける and kanade への送り口（`MouseWiring`）が World に無い, the areka shall その旨を記録に残し、別れの台詞なしで全窓を閉じて終了の指示を出す（窓は出たが kanade との結線に失敗した起動でも、標準の道具で閉じられるアプリのままにする＝裁定 3。09-23 開発者承認で改訂・旧文「終了要求を送らず記録するだけ」ではこの起動が閉じられなかった）。
11. When ダミー窓が OS の閉鎖要求を受ける, the areka shall ダミー窓を閉じ、終了の指示を出す（3.4 と同じ見え方）。
12. When OS の閉鎖要求を受ける, the areka shall 受けたことと窓の種類を記録に残す（ライフサイクル事象として info）。
...
3. The 本仕様 shall 足すテストを上の 2 つの判断分岐（と要件 1.4・1.5 の縁の条件）、および OS の閉鎖要求の判断分岐 2 つ（wintf: 関数が差されていれば窓を消さずに呼ぶ＝2.10／areka: ゴースト窓の関数が操作された窓のスコープで終了要求を送る＝3.9）に限り、既に確かめられている配線（閉じる → 登録表から外れる → 合図が鳴る・終了要求 → kanade の握手）を再テストしない。
...
5. When 実機で確認する, the 開発者 shall 有界の自動終了（`AREKA_APP_SMOKE_EXIT_MS`）とログの絞り込みで終了操作の見え方を確かめ、キャラ窓への Alt＋F4 で別れの台詞のあとに終わることを見て、終了後に areka のプロセスが残っていないことを見る。
6. While 実機で確認する, the 開発者 shall 自分が起こしたと確認できたプロセス以外を止めない。

### Requirement 6: 裁定（2026-09-23 確定）

**Objective:** As a 開発者, I want brief が挙げた裁定候補を要件の段階で確定しておくこと, so that 設計がこの形で進む

#### Acceptance Criteria

1. The 本仕様 shall **裁定 1（口の形・2026-09-23 開発者確定）**として、wintf の選択口を「アプリの構築時の設定値」＋「明示の終了の指示 1 本」の形とし、「実行中に切り替えられる資源」と「終了の仕掛けを利用側が差し替える」の 2 案を採らない（実行中の切替は「戻し忘れ」という新しい状態を作る。開発規律: 状態を増やさず形を変える）。
2. The 本仕様 shall **裁定 2（送り忘れの防止・2026-09-23 開発者確定）**として、areka の終了経路で「終了のための全窓破棄」と「終了の指示」を 1 つの操作にまとめ、片方だけを呼べない形にする。時間で自動終了する安全網は持たない（切替中の「窓 0 で生きている」を壊すため）。
3. The 本仕様 shall **裁定 3（OS の閉鎖要求・2026-09-23 設計ディスカッションで開発者確定）**として、Windows からの閉鎖要求（Alt＋F4・`taskkill`・「タスクの終了」）を 7 種目の正規の終了操作とし、ゴースト窓ではメニューの「終了」と同じ終了系列へ乗せる。「無視する」案（SSP と同じ振る舞い）は採らない——正典（ukadoc）に禁じる定めは無く、無視すると標準の道具では areka を閉じられなくなるため。「閉鎖要求を受けても窓 0 でプロセスが残る」案（受け入れて文書に残す）も採らない。
4. Where 設計・実装の途中で裁定 1〜3 のいずれかを覆す必要が判明する, the 本仕様 shall 開発者へ議題として上げ、確定を待ってから要件・設計の該当箇所を改める。
