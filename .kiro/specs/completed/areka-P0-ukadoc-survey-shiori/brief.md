...
- Ex 168 は群単位で `not-applicable` 候補（外部送信元・受け口の有無だけを note に）。実装済みの証拠は toolkit の規則 7（ソースの ukadoc URL）＝本 spec が `implemented` と判定した項目には、定義箇所へ URL を置く小さな変更が伴う（`ALLOWED_EVENT_IDS` の 11 要素・`ALLOWED_RESOURCE_IDS`・`build_request` のヘッダ）。これは実行時挙動を変えない doc コメントのみで、本 spec の唯一のコード接触。
...
> **📌 2026-09-02 開発者指針「伺からしい価値観を持つ要素の優先度を上げる」（toolkit 規則 6・9 に凍結・裁定は議題 5〜7）**: 分類軸に ⑸ **テーマ付与**（`values[]`・付与規則＝「この項目が無いと利用者はゴーストの何を失うか」に答えられるテーマだけ）を加える。本ドメインの代表例（id は ukadoc 実測）: **気配**＝OnSecondChange／OnMinuteChange／OnHourTimeSignal（見切れ・重なり・放置秒の Reference 込み）／OnBoot／OnFirstBoot／OnClose ⇒ 旧段階 C から **A** へ／**触れ合い**＝OnMouseMove 連続／OnMouseDoubleClick／OnMouseWheel／OnMouseGesture／OnFileDropping／**OnFileDrop2**（正典・旧 OnFileDrop／OnFileDropped／OnFileDropEx は `alias_of`）⇒ D&D は触れ合い＋更新の 2 テーマで **B**／**記憶**＝OnUserInput／OnUserInputCancel／OnTeach 系 3／OnNotifyUserInfo（A）・OnVanish 系 5（OnVanishSelecting／Selected／Cancel／ButtonHold／OnVanished・**B**）／**更新**＝OnUpdate 系 24（OnUpdateProcessExec→OnUpdateBegin→OnUpdateReady→OnUpdate.OnDownloadBegin→OnUpdate.OnMD5Compare{Begin,Complete,Failure}→OnUpdateComplete／Failure→OnUpdateResult{,Ex,Explorer}・OnUpdateCheck 系 4・OnUpdateOther 系 9）＝**新旧両軸で高い唯一の群・B 先頭**／**気配り**＝OnSysResume／OnSysSuspend／OnBatteryNotify／OnBatteryLow／Critical／OnScreenSaverStart／OnFullScreenApp*／OnWindowStateMinimize／Restore／OnDisplayChange（C）／**交わり**＝OnCommunicate／OnGhostCalled／OnGhostChanged／OnOtherGhostTalk／OnOtherGhostBooted／SSTP／FMO（D・基盤の重さはテーマ側で下げない）。**テーマ 0**＝Ex 168・OnBasewareUpdating／Updated（areka 自身の配布＝別軸・note に注記）・`property.get`／`property.set`・HEADLINE。**SAORI**（DLL 共通仕様の 1 行）＝`not-applicable`（実装主体は SHIORI＝里々 `@saori`・YAYA ローダー・蒼空 `Saori.Load()`）・note に成立条件（host-32 の同居環境＝32bit 同一プロセス・作業ディレクトリ `ghost/master`・DLL 検索パス）・段階 A の検証項目として links。ブリーフィング節の並びは「黙って壊れる」→「テーマ付き」の順（toolkit 規則 6 の序列）。
