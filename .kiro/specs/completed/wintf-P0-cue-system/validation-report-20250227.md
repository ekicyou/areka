...
## 11. Post-Implementation Discovery (2026-02-28)

### 🚧 Blocking Issue: DolaRuntime Misplacement

**Status**: ⚠️ **Architecture Issue Discovered**  
**Blocked By**: `wintf-P0-dola-boundary` (new specification)  
**Severity**: Medium — cue-system の機能は正常動作するが、設計上の誤配置が確認された  
**Discovery Date**: 2026-02-28

#### 問題の詳細

cue-system 実装時に DolaRuntime を bevy_ecs の `Resource` としてシングルトン化し、`crates/wintf/src/ecs/cue/runtime.rs` に配置したが、これは設計ミスである：

1. **責務の混在**: cue-system は「演出指令の配送基盤」であり、アニメーション実行エンジン（dola）の管理場所ではない
2. **未使用の実装**: `update_dola_runtime` システムが存在するが、cue パイプラインはこれを一切消費していない（`_result` で破棄）
3. **誤った Resource 化**: DolaRuntime は ECS の Resource でもなく Component でもない — 単なる「タイミングエンジン」（アニメーション変数の補間・再生・購読を行う内部実装の道具）
4. **シングルトン前提の誤り**: 複数の DolaRuntime が同時に動作する可能性があるが、Resource 化により単一インスタンスに制約されている

#### cue パイプラインへの影響

**実害なし** — cue の時刻制御は FrameTime のみで完結しており、DolaRuntime に依存していない：

| モジュール | DolaRuntime 依存 |
|-----------|-----------------|
| CueSheet / Cue | ❌ No |
| dispatch_pending_cue_sheets | ❌ No — EntityRegistry + FrameTime |
| CueQueue / pop_ready | ❌ No — current_time のみ |
| CueSheetTracker | ❌ No — QueueSnapshot のみ |
| update_cue_sheet_trackers | ❌ No — FrameTime のみ |
| **update_dola_runtime** | ✅ Yes — **唯一の利用者だが出力未消費** |

#### リグレッション修正

本 validation 実施時に `taffy_flex_demo` のパニックが発見されたが、これは cue-system とは無関係の潜在バグ（`clip_sync_system` が `Res<DCompGraphicsResource>` を非Optional で要求、ULW モードでは未初期化）であり、修正済み。

#### 今後のアクション

1. **新仕様の策定**: `wintf-P0-dola-boundary` で dola ランタイムの責務境界を定義
   - dola を使うか・使わないか・拡張するか・wintf 側を拡張すべきか
   - DolaRuntime の所有者は誰か（EcsWorld 外部? コンポーネントの内部フィールド? 利用者ごと?）
   - cue-system から DolaRuntime 関連コードを除去すべきか

2. **cue-system の最終判定**: 本実装は **GO（条件付き承認）** を維持
   - cue パイプライン自体は正しく動作（75/75 tests passed）
   - DolaRuntime の誤配置は局所的問題（1ファイル + 1システム）
   - 次仕様で責務境界を明確化後、局所修正で対応可能

3. **ブロック解除条件**: `wintf-P0-dola-boundary` の設計承認 + 必要に応じて cue-system の局所修正

#### 設計指示の振り返り

元の指示「dola の思想を共有する」は達成されたが、「dola ランタイムを利用する」とは指示されていなかった。「今後使うことになる」とは伝えられたが、「どう使うべきか」「誰が所有すべきか」の線引きが不明確だったため、cue モジュール内に誤配置された。
...
Date: 2025-02-27  
Updated: 2026-02-28 (Post-Implementation Discovery added)  
