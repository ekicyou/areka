...
> **📌 2026-09-11 棚卸⑬（分割 ⑴⑵⑶ を実施）**——本 spec は **⑴ 基盤相**（`draw.rs` 分割・`decode_tag` の `"f"` 腕・装飾 `CueCommand`・per-run 3 層配管・`disable.font.*`／`TextEffects` シームの実体化）＋**font 系 10 項目**（`name`・`height`・`color`・`bold`・`italic`・`underline`・`strike`・`sub`・`sup`・`outline`）＋**`default`／`disable`**（リセット意味論＝`balloon-lifecycle-events` 項目 9 と `choice-marker-styling` が消費）を持つ。寄せ 2（`align`／`valign`）＋影 3（`shadowcolor`／`shadowcolor,none`／`shadowstyle`）＋cursor-tag からの追加登記 1〜4（SC8 の裁定・あふれ判定の前提）は **`areka-P0-text-align-shadow-canon`** へ、descript `font.*` 基底 13 キーの解析は **`areka-P0-balloon-font-descript-keys`** へ切り出した。本 spec は `balloon/parse.rs` に**触れない**（既定層は解析済み 5 キーだけを読み、残り 8 キーの配線は後着の方が行う＝相互登記）。**1,000 行番人の追加所見（2026-09-11 実測）**: `draw.rs` 988 のほか `layout.rs` 955・`actor.rs` 952・`region.rs` 951 も射程＝分割は `draw.rs` 1 本で済まない可能性・新規ファイルで足すこと。編成＝**W13**（`sakura-tag-word-boundary` と `areka-parsers/sakura` 同 dir 別ファイル）。規模 XL → **L**。design 段階は Fable。
