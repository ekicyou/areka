...
- 完了時の未解決問題の棚卸（2026-09-25）: その場で解決 **0 件**・起票 **0 件**。見た候補と除いた理由: ① `actor.rs` の `round_trip` の `shiori_reply_timeout` の腕は `recv()` が無期限に待つので届かない防御の腕（写し先は `Ipc`＝テスト済みの送出失敗・応答の切断と同じ結果）＝問題ではない。② 接続失敗の理由の文言が着順で 2 通りに揺れる件は、どちらも対応する `error!` と同じ文言で要件 2.1 を満たす（2.1 の注記）＝裁定済みの形。③ i686 の要求方向の「応答なし」打ち切り（戻り 0・last error 0）が `SendFailed` のままの件は、helper が要求の往復の外では常にメッセージを回しているので、最初の失敗の分類には現れない（先に期限切れの `Timeout` で Fault に倒れた後の Unload でだけ起きうる）＝告知の種類に影響しない。④ 3.3 のテストの `shutdown` に期限が無い件は、kanade が先に止まっているので固まらない（レビューの任意の提案）。
