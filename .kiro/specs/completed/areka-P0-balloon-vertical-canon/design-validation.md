...
## 追記: 討議による解消（2026-08-28）

本報告の条件付き NO-GO は、設計ディスカッションで **3 件すべて解消され、GO の条件を満たした**。

- **重大 3**: カテゴリ A（自明修正）で解消——DD8 の根拠差し替え・双方向登記の完了時再検証・§8 13 項目名の tasks 完了条件化・付録（注）の責務外明記（コミット `fdead4ae`）。
- **重大 1・重大 2**: 討議議題 1 で開発者裁定（推奨案承認）——境界を repo 全域の出荷／テスト資産へ拡張（`emo2-kakukaku/descript.txt`・`emo2-choice/descript-plain.txt` を Modified Files へ・pilot のコードは非接触）・棚卸しを語 grep から意味論（origin 宣言全列挙×マージ後 validrect 突合）へ変更・実ゴースト開始点 (36,46)／(24,40) を逐語固定する檻 `tests/shipped_fixture_region_test.rs` を新設・要件 10.7 の読み（退行＝被覆喪失・期待値更新は退行でない）と 10.9 の一般化を要件本文へ登記（コミット `f43d1e17`）。
