...
- **In scope**（本仕様が観測可能な振る舞いとして所有する範囲）
  ...
  - 骨格 `main`: ロギング初期化・パニックハンドラ・UI ランタイム起動・構成入力（ゴースト／バルーンのルートパス）解決とログ出力・検証用ダミー窓の起動と app（main）によるメッセージループ駆動・ダミー窓 close での正常終了。
  ...
  - 後続仕様が本物のゴースト窓生成へ置き換えるための replace-me シーム（本仕様ではダミー検証窓を開く）。
...
4. When 本番アプリが起動する, the areka アプリ shall UI ランタイムを起動し、そのメッセージループを app（main）自身が駆動する。
5. The areka アプリ骨格 shall 本物のゴースト窓の生成・配置・座標・DPI ロジックを保持しない（それらは window-placement／emo-present の領分）。UI ランタイム起動と正常終了の観測目的に限り、ゴースト内容も配置主張も持たない最小の検証用ダミー窓を開くことを許容し、後続仕様がこれを本物のゴースト窓へ置き換える。
...
1. While エンジンが未結線である, the areka アプリ shall 構成入力の解決とログ出力を経て検証用ダミー窓を開き、app（main）が UI ランタイムのメッセージループを駆動し、そのダミー窓が閉じられたとき正常に終了できる。
2. The areka アプリ shall 後続仕様が本物のゴースト窓の生成へ置き換えるための差し込み関数（「ここを本物に置き換える」replace-me シーム）を提供し、本仕様ではその関数が検証用ダミー窓を開く。
...
2. When 本仕様の変更後にクレートのテストを実行する, the areka クレート shall 既存の緑のテスト（SHIORI 契約チェーンの e2e テスト群を含む＝緑判定の対象）を緑のまま維持する。
3. The areka クレート shall モックデモ example へ移設したモック UI ユニットテストを、テストコード資産として保全すれば足るものとし、`cargo test` での実行継続を厳格には要求しない（テストコードの移設先と実行対象化の可否は設計フェーズで裁定する）。
4. The モックデモ example shall 移行前のモック UI と観測上等価な挙動を保つ。
