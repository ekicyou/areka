...
- **In scope（本ユニットが観測可能に実現する振る舞い）**:
  ...
  - 合成入力（surface id＋bind 集合）→ 合成結果のキャッシュ（直前 1 件のメモ化）と、その無効化の口。
...
**Objective:** As a emo ランタイム, I want 合成入力（surface id＋bind 集合）単位で直前の合成結果を保持し、必要時に無効化できること, so that 合成入力が変わらない再表示で合成を繰り返さず、入力が変われば必ず再合成され、資産再読込時には確実に作り直せる

> **改訂（2026-07-09）**: 初版は「surface id をキーに」としていたが、合成結果は surface id と bind 集合の
> **両方**で定まるため、同一 surface の着せ替え・まばたき（bind 差分）が古い合成結果に衝突する仕様バグで
> あった（まばたきデモで顕在化）。キーは**合成結果を一意に定める入力の全体**を捕捉しなければならない。
> 併せて多エントリ全保持を廃し、直前 1 件のメモ化とする（将来 seriko がアニメ pattern 状態を合成入力へ
> 加えると状態空間が膨張し、全保持はメモリ堆積と低ヒット率の二重苦になるため）。

> **改訂（2026-08-15・開発者裁定。改訂元は `areka-P0-recompose-budget` 要件 7.1／7.3 の裁定ゲート）**:
> 容量を **1 → 3**・置換方式を **LRU**（最近最も使われていないものから追い出す）とする。
>
> 上の 2026-07-09 改訂が「直前 1 件」を選んだ理由のうち**低ヒット率の側が実測で否定された**。
> 実機 emo2 の 25 分走行で、キャラ面（`TargetId(0)`／`surface_id=1000`）は **1066 適用すべてが
> 引き当て外れ**（命中率 0.0%）であり、1 コマ適用の **80%** が毎コマの作り直しだった。走行の適用列を
> LRU で再生した命中率は容量 1→**0.0%**／2→10.5%／**3→56.2%**／4→56.5%／8→57.6%／64→68.5% で、
> **膝は 3 にある**（emo2 のまばたきが 3 コマの繰り返しであるため）。以後は容量を増やしても伸びない。
>
> メモリ堆積の側の懸念は生きているので**上限つき**とし、無制限の全保持は引き続き採らない
> （pattern 状態込みのキー空間は走行全体で 100 個規模へ膨らむ）。代金は 1 対象あたり
> 約 3.44MB → 約 10.3MB。**下の受入基準 2〜4（キー完全一致のみヒット・無効化・所有）は一切変わらない。**
> 実測の正本は `areka-P0-recompose-budget/remeasure-2026-08-15.md` §4、判断の正本は同 spec の
> requirements.md Requirement 7 改訂欄である。

#### Acceptance Criteria

1. The emo-present レイヤ shall 合成入力（surface id と bind 集合）をキーに、直近の合成結果を保持するキャッシュ（**上限 3 件・LRU 置換**のメモ化表。2026-08-15 改訂前は容量 1 のメモ化スロット）を備える。キーは合成結果を一意に定める入力の全体を捕捉しなければならない。上限を超える挿入では**最近最も使われていないエントリ**を追い出す。
2. When 直前の合成入力と完全一致する切替指令を受け取る, the emo-present レイヤ shall 再合成せずキャッシュされた結果を用いて表示を更新する。When 合成入力が 1 要素でも異なる切替指令を受け取る, the emo-present レイヤ shall 必ず再合成して表示を更新する。
