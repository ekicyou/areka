...
- **[完了ゲート検出: 実機実走が共有 emo2 fixture を汚染し spine テストが落ちる]**: `/kiro-complete` の Test Gate で `spine_harness_boots_scripted_ghost_and_reaches_attach_ready` ほか 1 件が失敗。原因は環境 flake ではなく**実機サインオフ（8.7）が実 emo2 fixture へ書き残した起動記録**——spine は `emo2_root()`（実 fixture そのもの）を ghost_root に boot するため、`[boot] count="1"` が残ると `first_boot=false` と判定され **OnFirstBoot が発行されず** scripted boot 系列（OnInitialize → username → OnFirstBoot → …）の期待と食い違う（実際の観測列は OnBoot 直行の 4 件）。検出②（共有 test ghost 汚染）と同型の病理が実 fixture 側でも発生した形。修正: `emo2_root()`（spine.rs）が**呼ばれるたびに ghost スコープの永続ディレクトリを除去**し、テストを実行順・実機実走から独立させる（fixture は gitignore 済みゆえ削除は安全）。なお `cargo test -p areka spine -- --test-threads=1` は GPU テストの制約で STATUS_ACCESS_VIOLATION になるため、spine は**既定の並列実行**で走らせること（単一スレッド化は誤った切り分け手段）。
- **[8.7 実機サインオフ検出④: 原点の不統一＝「キャラの原点は下端中央」へ徹底統一]**: クランプ非対称（検出③）修正後も **むらさきのバルーンだけ**が縦 140px・横 104px ずれる。開発者の問い「キャラの原点はどこ？下の中央でしょ？サーフェスサイズが変わったからと言って原点が動くならそれが問題」が核心を突いた。計測ログで確定した原因: **むらさきは surface0(434×687) → surface1000(382×547) へ切替**わる（エモは 336×400 固定＝無傷。これが「むらさきだけ」の理由）。ところが原点の定義が層ごとに不統一で、寸法が変わると原点が動いていた——(a) `resize_window_to` は左上 x 据え置き＝幅変動で見た目の中心が横へ動く、(b) `follow_balloon` は左上基準 offset ＝高さ変動でバルーンが引きずられる、(c) 永続 `anchor_edge_basis(Bottom)` は `(0, h)`＝x が左上基準、(d) `char_pos_entries` は左上 x をそのまま保存＝保存時 w=382／復元時 w=434 で原点が 26px ずれる。**修正（あるべき姿・下端中央原点へ四者統一）**: (a) `resize_window_to` が Bottom で**下端中央保持**（旧中央→新左上へ付け替え）、(b) resize 時に `BalloonFollow.offset` を原点移動ぶん `+Δ` 変換して**原点相対を不変**に、(c) `anchor_edge_basis(Bottom)` を **`(w/2, h)`**＝足元中央へ、(d) char 位置も **`char_pos_to_origin_x`/`char_pos_from_origin_x`** で原点 x として保存・復元。Free/Top/Left/Right は原点が中央でないため従来どおり（アンカー別に限定）。檻 7 件を新意味論へ更新＋新檻 `resize_window_to_keeps_balloon_relative_to_bottom_center_origin`（原点不動・バルーン非追随を固定）。199 placement テスト緑。
