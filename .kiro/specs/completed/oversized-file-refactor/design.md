...
- グリーン、かつ**テスト名インベントリのゴールデン差分が空**（追加・消失・リネーム・重複ゼロ）であることを毎回確認（R3.5、Behavior-Equivalence Verification 参照）。未解決の失敗を残して完了扱いにしない（R4.4）。
...
### Behavior-Equivalence Verification（R3.3 / R3.5 / R4.2）— 厳格（テスト名差分）

> **背景**: `#[path] mod` 宣言の書き忘れ等でテストファイルがコンパイル対象から外れると、**テストが静かに減ってもグリーン**になる。総数比較だけでは、リネーム・1件入替・重複を見逃す。よってテスト名インベントリの差分照合を必須とする。

- **ゴールデン取得（着手前・1回）**: リファクタ着手前に、対象クレートの**テスト名インベントリ**を取得し基準として保存する（例: `cargo test -p <crate> -- --list`、必要に応じてテストバイナリ別に取得）。
- **毎回照合（各ファイル分割ごと）**: 各 src/tests ファイルの分割・抽出の直後に、当該クレートのテスト名インベントリを再取得し、**ゴールデンとの名前差分が空（追加・消失・リネーム・重複ゼロ）** であることを照合する。差分が出た場合は是正し、解消不能なら当該変更を revert（R4.4）。
- in-source テスト抽出（Pattern A）でも同一インベントリ照合を適用し、移設後に同一テストが収集・実行されることを名前レベルで保証する。
- 死体削除フェーズ（R1）で削除する `#[cfg(test)]` を含まない項目はテスト名に影響しないが、削除前後でインベントリが不変であることを併せて確認する。
