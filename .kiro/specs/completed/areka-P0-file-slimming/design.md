...
**Purpose**: 本 spec は areka ワークスペースの**ファイル構造のみ**を是正する。`#[cfg(test)]` テストモジュール（合計 500 行超）を持つ 49 ファイルからテストモジュール本体を専用テストファイルへ移設し、1,000 行を超えるテストファイルはテーマ単位の複数ファイルへ分割し、本番本体が突出して太い `follow.rs`・`frame.rs` の ~~2 本のみ~~ を責務単位のサブモジュールへ分割する。動かすのは「コードの置き場所」であって「コードの中身」でも「本番のふるまい」でもない。

> **【改訂・2026-08-09 開発者裁定／`verification/notes.md` §39 に記録】上記の「2 本のみ」は取り消す。** 要件 1.7 の宣言——「本 spec の基準は『1 ファイルの行数』であって『本番ファイルの行数』ではない」——が上位であることが確認され、要件 4.5 が改訂（1,000 行を超える全ファイルの本番本体を分割する）、要件 1.4 が明確化（既分離ファイルの除外は**移設**のみに掛かり**分割**には掛からない）された。**現行の運用規則: 出自を問わず 1,000 行を超える全ファイルが本 spec の対象である。** **あわせて移設の外延も 1 本増えた**——本書が随所で「移設対象は必須対象 **49 本**のみ」と書いているのは改訂前の外延であり、タスク 8.8 が `crates/areka-ghost/src/shiori_inproc.rs`（1,018 行・テストコード 446 行＝閾値未満）を要件 1.5 の併せ移設として外へ出したため、**実績の外延は 50 本**である（§移設方式の裁定の末尾・§裁定一覧 #10・§Requirements Traceability 1.5 行の各注記を参照）。本書の §Boundary Commitments・§Non-Goals・§移設方式の裁定・§裁定一覧・§Requirements Traceability も同裁定により改訂している（各所の注記を参照）。実作業はタスク 8.1〜8.12。
...
- ~~`follow.rs`・`frame.rs` 以外の本番本体分割（本番本体 500〜1,000 行の水準は健全と判定済み）~~
  ...
  **【改訂・2026-08-09 開発者裁定／`verification/notes.md` §39 に記録】この Non-Goal は取り消す。** 要件 1.7 の宣言——「本 spec の基準は『1 ファイルの行数』であって『本番ファイルの行数』ではない」——が上位であることが確認され、要件 4.5 が改訂され（`follow.rs`・`frame.rs` 以外の本体分割禁止を取り消し、**1,000 行を超える全ファイル**の本番本体を分割する）、要件 1.4 が明確化された（既分離ファイルの除外は**移設**のみに掛かり**分割**には掛からない）。**現行の運用規則: 出自を問わず 1,000 行を超える全ファイルが分割の対象である。** 括弧書きの根拠「本番本体 500〜1,000 行の水準は健全」は 1,000 行超のファイルには最初から当てはまらず、根拠の届かない範囲まで禁止を及ぼしていた（§39.2 ③）。本項に該当する実作業は **タスク 8.9**（`areka-emo-present/src/presenter.rs` 1,066 行）・**8.10**（`areka-emo-text/examples/emo-text-layer.rs` 1,434 行）・**8.11**（`areka/examples/emo-present.rs` 1,168 行）・**8.12**（`areka/examples/collision-probe.rs` 1,063 行）。**1,000 行以下の本番本体を分割しない点は変わらない。**

- ~~テストモジュールが 500 行以下のファイルの移設（任意移設は本 spec では**行わない**——設計判断 #10、§裁定一覧）~~

  **【改訂・2026-08-09 開発者裁定／`verification/notes.md` §39 に記録】原則は維持するが、例外が 1 件ある。** 上と同じ裁定——要件 1.7 の宣言が上位・要件 4.5 の改訂・要件 1.4 の明確化——により、**出自を問わず 1,000 行を超える全ファイルが本 spec の対象**になった。その帰結として設計判断 #10 の「任意移設 0 件」は成立しなくなり、**タスク 8.8 が `crates/areka-ghost/src/shiori_inproc.rs`（1,018 行）のテストコード 446 行を移設した**。446 行は 500 行の閾値未満で要件 1.1 の必須対象ではない。**授権は要件 1.5 である**——1.5 は当該ファイルの移設を「**必須としない**」と定めるだけで**禁じてはおらず**、「ただし……併せて移設することを許容する」と明示的に許可している。1.5 が許可の例として挙げる動機は「同一ディレクトリ・同一モジュール群の一貫性」だが、**8.8 の動機はそれではなく要件 1.7 の 1,000 行目安を本番本体の側で満たすことである**（§48）。すなわち 1.5 は**併せ移設という手段を許可する条項**として働き、動機は 1.7 が供給した。この読みは下の §裁定一覧 #10 の「一貫性のための任意移設の実績は依然 0 件」と矛盾しない。
  なお群 8 でファイル内テストモジュールが外へ出たファイルは 8.8 と **8.2**（`areka-kanade/tests/kanade/common/mod.rs` の `mod smoke` 256 行）の 2 本だけだが（§53.5 の会計 −702 ＝ 446 ＋ 256）、8.2 は 1 テストモジュールの移設ではなく共有ハーネス本体の D1 分割であり、その一部として `mod smoke` が名前を保存したまま 1 ファイルへ移ったものである（§44.1・§48 冒頭の分類）。**#10 が禁じた「一貫性のための任意移設」に当たるのは 8.8 の 1 件のみ。**
...
- ~~テストモジュールを持たない巨大テストファイル（`choice_test.rs` 1,563 行等）の分割——「同居テストモジュールの外出し」という本 spec の操作が定義できないため対象外（将来 spec の受け皿は steering 追記でカバー）~~
  ...
  **【改訂・2026-08-09 開発者裁定／`verification/notes.md` §39 に記録】この Non-Goal は取り消す。** 上と同じ裁定——要件 1.7 の宣言が上位・要件 4.5 の改訂・要件 1.4 の明確化——により、**出自を問わず 1,000 行を超える全ファイルが分割の対象**になった。「本 spec の操作が定義できない」という理由も成立しない——本 spec の操作は「同居テストモジュールの外出し」だけではなく**テーマ単位の分割**を含み（要件 1.7）、分割は同居テストモジュールの有無に依存しないからである（§39.2 ①: これらのファイルは `tests/` 配下でファイル全体がテストのため `#[cfg(test)]` を書かず、必須対象の絞り込み条件がそもそも発火していなかった）。本項に該当する実作業は **タスク 8.3**（`areka-kanade/tests/kanade/choice_test.rs` 1,563 行＝本項が名指ししていた当のファイル）・**8.4**（`mouse_test.rs` 1,116 行・`close_test.rs` 1,009 行）・**8.7**（`dola/tests/cue/runtime_test.rs` 1,102 行）。加えて既分離ファイルの分割（要件 1.4 の明確化）が **8.1**（`areka/src/emo2_boot/spine.rs` 2,503 行）・**8.5**（`areka-parsers/src/shell/decode_tests.rs` 1,395 行）・**8.6**（`areka-emo-compose/src/golden_tests.rs` 1,356 行）、共有ハーネスの本体分割が **8.2**（`common/mod.rs` 1,657 行）である。

## Boundary Commitments

### This Spec Owns

- **テストモジュールの「位置」**: ~~必須対象 49 ファイル~~ **必須対象 49 ファイル ＋ 要件 1.5 の併せ移設 1 ファイル（`areka-ghost/src/shiori_inproc.rs`・タスク 8.8）＝ 50 ファイル**の `#[cfg(test)] mod` ブロック本体の置き場所と、その接続宣言
- ~~**`follow.rs`・`frame.rs` の本番本体のファイル内訳**（項目の中身は不変・置き場所のみ）~~ → **1,000 行を超える全ファイルの本番本体のファイル内訳**（項目の中身は不変・置き場所のみ）

  **【改訂・2026-08-09 開発者裁定／`verification/notes.md` §39 に記録】上の 2 行の外延を実績に合わせて改める。** 本節の一覧は `kiro-review` の Boundary Audit と `verification/notes.md` §53.12 の `BOUNDARY:` 判定が突き合わせる**基準そのもの**なので、古い外延のまま残すと「はみ出しなし」の判定が空虚になる（`DESIGN: 逸脱なし` と同型の欠陥）。改訂後の所有範囲は次のとおり:

  | 所有 | 内訳 |
  |---|---|
  | テストモジュールの位置 | 必須対象 **49** ＋ 併せ移設 **1**（`shiori_inproc.rs`・8.8）＝ **50** |
  | 本番本体のファイル内訳 | `follow.rs`（6.1）・`frame.rs`（6.2）＋ **群 8 の 13 本**（§39.4 の一覧・タスク 8.1〜8.12） |

  群 8 の 13 本の内訳（すべて 1,000 行超・合計 17,450 行）: `areka/src/emo2_boot/spine.rs` 2,503（8.1）・`areka-kanade/tests/kanade/common/mod.rs` 1,657（8.2）・`areka-kanade/tests/kanade/choice_test.rs` 1,563（8.3）・`areka-emo-text/examples/emo-text-layer.rs` 1,434（8.10）・`areka-parsers/src/shell/decode_tests.rs` 1,395（8.5）・`areka-emo-compose/src/golden_tests.rs` 1,356（8.6）・`areka/examples/emo-present.rs` 1,168（8.11）・`areka-kanade/tests/kanade/mouse_test.rs` 1,116（8.4）・`dola/tests/cue/runtime_test.rs` 1,102（8.7）・`areka-emo-present/src/presenter.rs` 1,066（8.9）・`areka/examples/collision-probe.rs` 1,063（8.12）・`areka-ghost/src/shiori_inproc.rs` 1,018（8.8）・`areka-kanade/tests/kanade/close_test.rs` 1,009（8.4）。**§Out of Boundary は改訂していない**（1 行も影響を受けていない）。
...
> **【改訂・2026-08-09 開発者裁定／`verification/notes.md` §39 に記録】括弧内の外延「必須対象 49 本のみ」は 50 本に改める。** タスク 8.8 が `crates/areka-ghost/src/shiori_inproc.rs`（1,018 行・テストコード 446 行＝500 行の閾値未満）を要件 1.5 の併せ移設として外へ出したため、**移設対象の実績の外延は 49 ＋ 1 ＝ 50 本**である。動機は一貫性ではなく要件 1.7 の 1,000 行目安であり、要件 1.5 は「必須としない／ただし併せ移設を許容する」条項として手段を授権している（§Non-Goals の第 2 改訂注記・§裁定一覧 #10・§Requirements Traceability 1.5 行に同旨）。**「既存 55 宣言を揃え直さない」という本文の裁定そのものは有効である**——8.8 は既存宣言の揃え直しではなく新規の外出しだからである。research R-1 の裁定は設計時点の記録として残す。
...
| 10 | 任意移設（500 行以下） | ~~**行わない（0 件）**~~ → **例外 1 件（タスク 8.8）**。差分最小化を優先し、混在ディレクトリの一貫性は steering の新規運用規律で将来収束させる方針は維持する。**【改訂・2026-08-09 開発者裁定／§39】** 要件 4.5 の改訂・要件 1.4 の明確化により 1,000 行超の全ファイルが対象になった結果、`areka-ghost/src/shiori_inproc.rs`（1,018 行）のテストコード 446 行（閾値未満）を要件 1.5 の併せ移設として外出しした。**「一貫性のための任意移設」の実績は依然 0 件** | §裁定一覧（本行）・§Non-Goals の改訂注記 |
...
> **【追記・2026-08-09 開発者裁定／`verification/notes.md` §39 に記録】上の一覧は設計時点の計画であり、実績はこれに 13 ファイルを加える。** 要件 4.5 の改訂・要件 1.4 の明確化により 1,000 行超の全ファイルが対象になった結果、群 8（タスク 8.1〜8.12）が §39.4 の 13 本を追加で変更した（全数は §This Spec Owns の改訂注記の表を参照）。うち `areka-emo-present/src/presenter.rs` は必須対象 49 と 13 本の**両方**に属する唯一のファイルである（タスク 4.3 のテスト分離と 8.9 の本体分割）。ブランチ全体の実績は `crates/**/*.rs` が **A=253 / M=61 / D=0 / R=0**（§53.10）。本節は計画の記録として原文を残す。
...
| 1.5 | 500 行以下は必須としない | §裁定一覧 #10（**改訂 2026-08-09**: 任意移設 ~~0 件~~ → 例外 1 件＝タスク 8.8 の `shiori_inproc.rs` 446 行・要件 1.7 のための併せ移設）・§Non-Goals の改訂注記 |
...
| 4.5 | ~~2 本以外の本体分割禁止~~ → **（改訂後）1,000 行超の全ファイルを分割** | ~~§Non-Goals（presenter.rs 1,042 行等は分割しない）~~ **【改訂・2026-08-09 開発者裁定／`verification/notes.md` §39 に記録】** 要件 1.7 の宣言（基準は「1 ファイルの行数」であって「本番ファイルの行数」ではない）が上位であることが確認され、要件 4.5 は改訂、要件 1.4 は明確化された（既分離ファイルの除外は**移設**のみ・**分割**は対象）。**運用規則: 出自を問わず 1,000 行を超える全ファイルが分割の対象。** 該当 Non-Goal 3 項は取り消し済（§Non-Goals の改訂注記）。実作業はタスク 8.1〜8.12（13 本・17,450 行）、うち本項が直接掛かるのは 8.9〜8.12 の 4 本 |
...
| `cargo test -p areka --examples` の既存 E0433（テストモジュール内 `crate::` 参照） | `crates/areka/src/placement/spawn.rs:879`（先行裁定注記 `:871`）。**【設計時点の記録・アンカーは移動済】**本 spec のタスク 5.2（`f64fac1`）の純テスト移設により、HEAD での出所は `crates/areka/examples/../src/placement/spawn_assembly_tests.rs:183` である（`verification/notes.md` §53.9 で HEAD 再確認・§37.1 #2 と `tasks.md` の Implementation Notes は訂正後のアンカーを持つ）。欠陥は同一で、落ちるのは `window-placement` と `collision-probe` の 2 本 | `test-cage-determinism` へ所見送付（証跡手順は `--exclude areka` で運用） |
