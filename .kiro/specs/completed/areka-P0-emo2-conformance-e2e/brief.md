...
>   - **※2026-08-27 補正（`areka-P0-balloon-offset-dpi` 要件ディスカッションからの申し送り）**: ⒝ の「追従 offset が変わらない」は atom 時点の契約の写しであり、`areka-P0-balloon-offset-dpi` の着地後は**陳腐化する**——拡大率遷移では追従 offset は**拡大率倍へ更新される**のが正（それにより見た目の相対位置＝作者空間の距離が保たれる）。適合表を書くときは同 spec の Requirement 3 を正として読み替えること。なお**作業領域の再スナップで offset に触れない**期待（`frame_work_area_resnap_tests.rs`）はそのまま不変である——変わるのは拡大率遷移の側だけ。
...
> **✅ 消化済み（2026-09-10 の走行 A・受入記録 §8.1 項目 19）。** A9 で拡大率を変えて描き直された直後（A10）も、バルーンがキャラの手前に居続けたことを開発者が目視で確かめた（「問題なし」）。**この確認は実機でしか取れない**という上の 3 点の理由は変わらないが、確認そのものは済んだので、以後この行を未確認の残件として読まない。登記はロードマップ `.kiro/steering/roadmap.md` の「主な申し送りの生存先」の行にも置いた（送り元は完了置き場に在り改変しないため）。
...
> **✅ 消化済み（2026-09-10 の走行 A・受入記録 §8.1 項目 20／内訳は同 §8.2）。** 引数・環境変数・作業ディレクトリの 3 経路が同時に成立したことを、3 つの証跡が同じ走行で揃ったことで確かめた——`event=connect_failed` **0 行**・実 SHIORI 由来の起動挨拶が画面に出た（`id=OnFirstBoot` 1 行）・`event=unload_clean` **1 行**。3 つのどれが欠けても脳の読み込みは成立しないので、経路を選り分けて確かめる必要は無い。登記はロードマップの「主な申し送りの生存先」の行にも置いた。
