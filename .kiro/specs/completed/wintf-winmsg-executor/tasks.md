...
- **【解決済 gap】factory の Window.parent 転送＋フレーム反映（4.3 レビュー発見→検証後対応 2026-06-30）**: 旧 create_windows は `Window.parent` を `CreateWindowExW` の hWndParent へ渡し、style も生成時適用でフレーム（caption）を描いていたが、新 EcsWindowFactory（3.4）は `WindowType::TopLevel` 固定で parent を無視し、style を後付け `SetWindowLongPtrW(GWL_STYLE)` で適用していた。ライブラリ `new_ex` の `WindowType` は TopLevel/MessageOnly のみで生成時 hWndParent を露出しないことを crate ソースで確認。**対応**: (1) `window.parent.is_some()` のとき生成後 `SetParent(hwnd, parent)` で反映（標準の親設定 API・非子窓のオーナー意味とは近似・doc 明記）、(2) `apply_initial_state` で `SetWindowLongPtrW(GWL_STYLE)` 後に `SetWindowPos(SWP_FRAMECHANGED|NOMOVE|NOSIZE|NOZORDER|NOACTIVATE)` を追加しフレーム再計算を効かせる。いずれも現 consumer（全て parent:None・WS_POPUP フレームレス）には実質 no-op ゆえリグレッションなし——areka 実機で表示/ドラッグ/ダブルクリック終了 exit0 を再確認（開発者立会）。これで factory が旧 create_windows と parent/フレームのパリティを回復。
