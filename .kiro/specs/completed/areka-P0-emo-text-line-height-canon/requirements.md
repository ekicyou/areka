...
### Requirement 11: 裁定 2026-09-06 の追補——スクロール描画量の削減と、観測用 example の前提の再導出

**Objective:** 運用者として、行送りの確定に伴って表面化した 2 件（スクロール 1 段あたりの描画呼び出しが「ダーティ矩形の枚数 × 描画対象行数」の積で膨らむこと／観測用 example の検査 3 件が落ちること）を本仕様の中で決着させ、製品の不変条件（確定した行のレイアウトを作り直さない・確定行を蓄積して再描画しない）が実測で示せる状態にしたい。

**裁定（開発者・2026-09-06・タスク 3.4 の残件 3 件に対して）**: 決定 3＝スクロール描画量の削減を**本仕様で今行う**（ダーティ矩形ごとに、その矩形と交差する行だけを描く）。決定 4＝「確定した行のレイアウトを作り直さない」の検査が落ちる件は**本仕様で直す**（引受先を探さない・新 spec は起こさない）。

**深掘りの結果（2026-09-06・実走で確認）**: 決定 4 の件は製品の不変条件の破れではなかった。観測用 example（`examples/emo-text-layer/`）の「完成プラトー」の選び方が、改行を次の文字まで遅らせる現行の意味論（`areka-P0-newline-defer`・2026-07-18 着地）より前の前提のままで、選ばれた時点では新しい行が 1 文字だけの途中だった（可視グリフ数 37・39・41＝いずれも奇数・短行は 2 文字）。1 段の間に「途中の行が伸びる」と「次の行が入る」が重なり、生成 2 回と数えていたにすぎない。この example は 2026-07-17 から止まっていた（タスク 3.4 で復旧）ので、改行遅延の着地後に一度も走っておらず、前提の陳腐化が現れなかった。全容は design §13 と `verification/derivation-ledger.md` §3.5.3。

#### Acceptance Criteria

1. The emo テキスト層 shall スクロールの 1 フレームで、ダーティ矩形ごとに**その矩形と行送り軸で交差する行だけ**を描き（描画呼び出しの回数＝各矩形の交差行数の**和**）、ダーティ矩形の枚数と描画対象行数の積では描かない。
2. The emo テキスト層 shall 上記の変更の前後で、参照描画（全域再描画）との画素単位の等価比較（`viewbox_draw_live_diff_tests.rs`・`viewbox_draw_oracle_regression_tests.rs`）が両側とも緑のままであり、注入した差分を検出する負の対照が赤のままであること（描く行を減らしても画素が 1 つも変わらない）。
3. The 本仕様 shall 「どの矩形にどの行を描くか」の割当を純粋層（`viewbox.rs`）で導出し、決定論テストで固定する（2 枚以上のダーティ矩形に別々の行が交差する入力で、矩形ごとの行の並びと、その和集合が描画対象行に一致すること）。実描画側（`viewbox_draw.rs`）の決定論テストは、描画呼び出しの増分が積ではなく和であることを固定する。
4. The 本仕様 shall ハイライト帯の拡張（`expand_overhang_for_band`）・帯の式・面内 blit・露出帯の導出・残滓の導出（タスク 3.4）には手を入れない。
5. The 観測用 example shall 「完成プラトー」を、改行遅延の意味論に従って「次の行が開く直前の、末尾の行が完成しているプラトー」として選び（次のプラトーで先頭可視行が 1 進むもの）、その選び方で「確定行は行レイアウトを再生成しない（生成増分は流入 1 行分以下）」が横書き・縦書きの両方で通ること。選び方を旧来のまま（先頭可視行が 1 進んだ直後）にすると同じ検査が落ちることを、本仕様の記録に残す。
6. The 観測用 example shall スクロール描画量の一定性（連続 2 段で描画増分が等しい）を、**送り出される行が同じ短行どうし**の 2 段で比べる（統制された比較）。短行の本数（`OVERFLOW_LINES`）と時刻表（`T_CHECK`・Clear の注入時刻・ゲートの項目数・プラトー走査の窓）はそれに合わせて式で導き直し、縦書きの「先頭バンドの行内インク範囲が縮む」（C5）も同じ導き直しで通ること。短行は 2 文字のまま（1 文字にすると「完成」と「開く」が同じプラトーになり、選び方の是正が緑のまま意味を失う）。
7. The 本仕様 shall `EXPOSURE_BAND_DRAW_BOUND` を削減後の実走で横書き・縦書きとも採り直し、その内訳を「矩形ごとの交差行数の和」として記述に残す（積の記述を残さない）。
8. The 観測用 example shall 横書き・縦書きの両方で、`__COMPAT_LAYER=DPIUNAWARE`（k=1.0）のもと全検査を通過して `PASS` を出力する（自動テストの DoD とは別の手動実走・Requirement 10.4 の「実機走行」には含めない）。
9. The 本仕様 shall 採らなかった案（可視窓の先頭行を毎スクロールフレーム一律にダーティへ入れる／残滓の幅をフォント既定値で固定して一定にする／example の検査を上限だけへ改める／新 spec を起こす）とその理由を設計に記録する。
10. The 本仕様 shall 「確定した行のレイアウトを作り直さない」が製品側で保たれていること（`LineLayoutStore` は内容不変の行を再生成しない）を、`tests/viewbox_scroll_test.rs` の既存検査が緑のままであることと example の生成増分が流入 1 行分以下であること（実測はタスク 7.2 で **0**——C8 の観測は全行を提示した後に完成プラトーだけを再提示するため、各行の最終内容が既にキャッシュ済み。機序は台帳 §3.5.3）で示し、タスク 3.4 の「先行不具合」の疑いが誤報であった旨を再導出台帳へ記録する。
