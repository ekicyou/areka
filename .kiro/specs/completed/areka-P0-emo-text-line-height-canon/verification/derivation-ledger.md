...
**実測**（すべて `__COMPAT_LAYER=DPIUNAWARE`＝`scale_k=1.0` での example 実走・2026-09-06）:

| 段階 | 横書き `draw1/draw2` | 縦書き `draw1/draw2` | `create1/create2` |
|---|---|---|---|
| 3.4 着地時（積・旧選び方） | 16 / 16 | 9 / 16 | 2 / 2 |
| 7.1 削減後・旧選び方（参考） | 8 / 8 | 6 / 7 | 2 / 2 |
| 7.2 選び直し後（最終） | **5 / 5** | **3 / 3** | **0 / 0** |

7.2 の実走ログ（それぞれ 27 検査すべて緑・`PASS`・終了コード 0）:

- 横書き: `p=9 long_lines=3 fvl1=10 fvl2=11 visible_line_count=4 draw1=5 draw2=5 create1=0 create2=0 blit1=1 blit2=1`
- 縦書き: `p=7 long_lines=7 fvl1=8 fvl2=9 visible_line_count=10 draw1=3 draw2=3 create1=0 create2=0 blit1=1 blit2=1`

`long_lines` は「短行が 1 本も流入していない時刻（`t = 1.95`）の配置行数」の実測で、design §13.4 の見込み（横 3 行・縦 7 列）と一致した。比べる 2 段の始点の先頭可視行はどちらも `long_lines` 以上（横 9 ≥ 3・縦 7 ≥ 7）＝送り出される行は 2 段とも短行「ほな」であり、統制された比較になっている。

**`EXPOSURE_BAND_DRAW_BOUND` の内訳（矩形ごとの交差行数の和）**: 上限は両モードの最大値 **5**。内訳は一時的な診断ログ（ダーティ矩形と各矩形の交差行 index を出力し、採取後に取り除いた）で採った。

| モード | 露出帯の矩形 | 送り出された行の残滓 | 流入した変化行の矩形 | 和 |
|---|---|---|---|---|
| 横書き（`y 91..122`／`y 0..3`／`y 89..122`） | ∩ 2 行 | ∩ 1 行 | ∩ 2 行 | **5** |
| 縦書き（`x 0..31`／—／`x 20..51`） | ∩ 1 列 | 矩形なし | ∩ 2 列 | **3** |

横書きで露出帯と変化行の矩形がそれぞれ 2 行に交差するのは、直前行の下端はみ出し（実測 3px）＋ガード 1px が 2px の行間を越えて隣の矩形へ食い込むため。縦書きで残滓の矩形が立たないのは、送り出される列が短行「ほな」の列でブロック軸（左右）のはみ出しが 0 だから（漢字を含む列は 1px 超）——これが「送り出される行を短行に揃える」統制が要る理由そのものである。

**負の対照（要件 11.5）**: 旧来の選び方（先頭可視行が 1 進んだ**直後**のプラトー）を 7.1 の削減後にそのまま走らせると、横書き・縦書きとも `create1/create2 = 2 / 2` で「確定行は行レイアウトを再生成しない（生成増分は流入 1 行分以下）」が落ちる（上表 2 行目・横書きはこの 1 件、縦書きはこれに C5 と `draw1 == draw2` を加えた 3 件が赤）。選び直した後は両モードとも 0 / 2 段とも 0 で緑。

**「製品の不変条件は破れていなかった（誤報）」の確認（要件 11.10）**: 最終の生成増分は 0 / 0 である（設計時の見込みは 1 / 1 だった）。0 になる機序は `LineLayoutStore::line_layout` のキャッシュ（行 index をキーに、内容文字列が一致すれば作り直さない・`draw.rs`）にある——C8 の測定は stage 4 のチェックポイント（`T_CHECK[4] = 3.4`＝全 16 行が出そろった時刻）の後に走るので、各行 index の**最終内容**の TextLayout はすでにキャッシュ済みで、そこへ時刻を戻して同じ内容を描き直しても生成は起きない。旧来の選び方で 2 になっていたのは、選ばれた点の末尾行が 1 文字だけの途中（内容が「ほ」）でキャッシュの「ほな」と一致せず作り直し、次の段でまた「ほな」へ戻って作り直していたためで、**確定行（内容不変の行）の作り直しではない**。製品側の不変条件が保たれていることは `tests/viewbox_scroll_test.rs::real_pump_scroll_redraws_only_dirty_and_unchanged_frames_touch_nothing` が緑のまま（`cargo test -p areka-emo-text --no-fail-fast` 全緑・終了コード 0）であることが併せて示す。3.4 の記録の「先行不具合」の疑いは誤報として取り下げる。
...
| 10 | 「`BalloonModel::name()`（`crates/areka-parsers/src/balloon/model.rs` の名前アクセサ）でバルーン名を警告に載せられる」（Components「`TextRegion.inline_limit` と警告」の擬似コード・Risks・依存表の `BalloonModel::name`（P1）・DD-9） | design が指した `pub fn name` は `impl Font` の**フォント名**で、`BalloonModel` にバルーン名の取得口は無い。名前は `descript.txt` の `name,` に実在する（`emo2-kakukaku` は `name,kakukaku for emo-gs`）が、`crates/areka-parsers/src/balloon/parse.rs` の `map_merged` が写像していない（タスク 4.1 の実走で判明・2026-09-06） | `balloon` 欄は定数 `BALLOON_NAME_PLACEHOLDER = "(名前なし)"` で埋めた（タスク本文が許す縮退・欄は落とさない）。要件 6.7 の「バルーン名」は形の上でしか満たせていない。**引受先＝`areka-P0-balloon-canon-residue` の項目 14**（バルーン定義の粗さの残件台帳・活きた spec）。**⚠再訂正 2026-09-06**: 当初は `areka-P0-ukadoc-survey-assets` の brief へ登記したが、同 spec は同日 PR #141 で**完了・アーカイブ**され、完了 spec は先送りを消化できない。同 spec の調査結果自身も `doc/ukadoc-coverage/ledger/assets.toml` の `ukadoc:descript_balloon:name…` を `status = "absent"`／`owner = ""`／`C45` として「担当は決まっていない」と記録しており、残るのは実装側の引き受けだけだった。アーカイブは非改変ゆえ当初の登記は取り下げ済み。design §Components と research の該当箇所は訂正済み |
| 11 | 「塊内で hard が発火すると `SegmentPlan` の『塊は途中分割されない』不変条件に例外ができる——soft > hard の粗いバルーン定義でだけ起きる縮退」（Components「`LayoutEngine` ゲート③」Risks） | 塊先頭の容量を `limit = soft.min(hard)` で先決する以上、送り幅の積み上がりだけでは塊内 hard は**構造的に到達不能**。到達するのは塊の途中で `\_l`（`TextItem::CursorMove`）が行内位置を進めた場合だけで、それは soft ≤ hard の通常バルーンでも起きうる（タスク 4.2 の実走・新設テスト `hard_limit_fires_inside_a_segment_and_leaves_a_readable_record` はこの経路で発火させている） | 実装は design の規則（hard を配置直前に必ず評価）に適合・是正なし。Risks の記述を本行で訂正 |
| 12 | 「soft ≤ hard の入力では本仕様前後で出力がビット一致する（6.4）」（同ブロックの事後条件・要件 6.4 の字義） | `\_l` が描画範囲の遠辺付近へ跳んだ直後のグリフは、従来は描画範囲の外へ置かれていたものが折り返されるようになる＝その一点で字義上は変わる。要件 6.2（描画範囲の外に文字を置かない・無条件）が裁定本文であり 6.4 の「折返しの位置」は本文の通常折返し（soft が決める位置）を指すと読み、**6.2 を優先**（タスク 4.2 のレビューで裁定）。既存テストへの影響は 0 本（`layout_test_support` の領域はすべて soft == hard に解決） | 事後条件を「`\_l` による行内位置の跳躍を伴わない入力では」と限定して読む。本行で記録 |
| 13 | 要件 5.2「おしゃべり頻度: 4 項目 **3 行**」・5.3「2 項目・2 行目は空」 | 配置層の「行」（`PositionedLine`）で数えると、おしゃべり頻度は**4 本**（上端 40／70／100／**100**——`\_l` が「たまーに」と「もどる」を同じ段の別の行矩形に分ける）、位置調整は **2 本**（文字の無い段は行を作らない）。要件の「行」は**視覚上の段**を指す（タスク 5.1 の実走・`tests/kero_menu_capacity_test.rs` が「段」と書き分けて固定） | 語彙の食い違いとして記録。要件文は改訂しない。行数を数える下流（ダーティ行など）は「行矩形の本数 ≠ 段数」を前提にすること |

以上 13 件は、いずれも「設計を黙って写さず、実測との食い違いを記録する」方針での追記・訂正です（#8／#9 はタスク 2.1 の実走で、#10 はタスク 4.1 の実走で、#11／#12 はタスク 4.2 の実走とレビューで、#13 はタスク 5.1 の実走で判明した分で、いずれも 2026-09-06 に追記しました）。
