...
- emo-text（`Custom` は既に良性 skip・無改変）／emo-compose（**D12 の pattern0 厳密選択是正=plan.rs `flatten_surface` の1点のみ本仕様が所有**・他は非改変・2026-07-23 改定）／emo-present 本体（test-only）
...
| `char*.bindoption*.group,カテゴリ名,mustselect/multiple` | カテゴリの排他/複数選択オプション | **mustselect=実導出／multiple=語彙保持** | **（2026-07-23 実機サインオフ改定・R4.5）** mustselect（腕/口/眉/目）は実導出＝排他選択（同カテゴリ新パーツ on で他を自動 off・D11）。当初「語彙保持・実挙動なし／スクリプトが自前排他するため実挙動なしでも成立」と実測を根拠に仮定していたが、実 emo2 pasta が mustselect カテゴリで off を送らず on のみ連打しベースウェア排他に依存することが実機で反証され、実導出へ昇格。multiple（紅等・非宣言＝既定）はスクリプト明示 on/off で従来どおり成立ゆえ語彙保持のまま |
...
| **D12**（実機サインオフ第2欠陥改定・2026-07-23） | **emo-compose の静的合成 pattern 選択を厳密 pattern0（index==0）へ是正する**（R9）。`crates/areka-emo-compose/src/plan.rs` の `flatten_surface`（:307）の `anim.patterns.iter().min_by_key(\|p\| p.index)` を `anim.patterns.iter().find(\|p\| p.index == 0)` へ変更し、pattern0 を持たない bind animation（まばたき等の `interval,bind+random` 再生アニメ・pattern1 以降のみ）は静的合成へ寄与させず `debug!`＋skip する（既存センチネル skip と同水準の良性経路）。**キャンバス外形算出（:529 の `min_by_key`）は無改変**（bind オン/オフ・pattern0 有無でサイズ不変契約＝将来の seriko-loop 再生時のはみ出し防止に有利・emo2 外形の golden 波及回避）。「まばたきは default OFF のまま触れない」（3分類表）の仮定は実機で反証（実 pasta が `\![bind,まばたき,…]` を on 連打）→「BindSet には載るが静的寄与なし・再生は seriko-loop（M-life）」へ改定。emo-present（R6.3）・seriko・mayuna・parsers は無改変 | 実機サインオフ第2欠陥（常時閉じ目）。emo2 まばたき 1400（`bind+random`・pattern1..3）/1403（`bind`・pattern2）は pattern0 非保持ゆえ `min_by_key` が閉じ目フレーム（1412/1414）を静的土台に誤採用しベース目を覆う。正典＝pattern0 が静的土台・pattern1 以降は seriko-loop 再生フレーム。plan.rs コメント（:306「疎 index 許容ゆえ最小 index を pattern0 とする」）が誤りの自白。上流 emo-compose 起因だが本仕様が所有（実機サインオフの合格条件ゆえ） |
| **D11**（実機サインオフ改定・2026-07-23） | **mustselect カテゴリの排他選択を実導出する**（R4.5）。(1) parsers が `sakura/kero.bindoption*.group,カテゴリ,mustselect` を取り込み `BindGroupDefaults` へ mustselect カテゴリ名集合を転記（`multiple`／非宣言は既定＝非排他で無視）。(2) seriko `BindResolver` へ mustselect カテゴリ集合＋カテゴリ→ID 索引（`category_ids`）を additive 追加し `is_mustselect(ns,category)`／`category_ids(ns,category)` を提供（parsers 非依存・素データ構築は不変）。(3) `ScopeStates` へ `apply_bind_exclusive(scope, category_ids, target_id) -> BindApplyOutcome`（新集合＝`(現集合 − category_ids) ∪ {target_id}`・Changed/StateOnly/Unchanged は既存と同型）を追加。(4) actor の bind 分岐: `Apply{category,part,on:true}` かつ `is_mustselect` なら `apply_bind_exclusive`、それ以外（on:false・非 mustselect）は既存 `apply_bind`。冪等・Show 再発行・解決不能 skip の規律は不変。`BindResolver::new` 署名変更は seriko tests/e2e/areka 呼出元をコンパイル結合ゆえ atomic 追随（D4 と同型） | 実 emo2 が mustselect（腕/口/眉/目）で off なし on 連打＝ベースウェア排他に依存することが実機で判明（R4.5・当初 M1 先送りの仮定を反証）。修正シームは task 1.1 の `category_ids` が既に用意済み＝差替シーム消費。SSP 正典の排他選択挙動に忠実 |
...
| `crates/areka-emo-compose/src/plan.rs` | **（2026-07-23 D12 改定）** `flatten_surface`（:307）の静的合成 pattern 選択を `min_by_key(index)` → `find(index==0)` へ（pattern0 厳密・無ければ `debug!`+skip）。**外形算出 :529 は無改変**。in-source/golden テストへ pattern0 非保持 skip 檻＋emo2 まばたき不活性檻を追加 | 9.1/9.2/9.3/9.5 |
