...
原本 `fixtures/emo2/emo2-kakukaku` の複製。**原本との差分は次の 3 点だけ**で、
既定層（`descript.txt`）とすべての画像は原本と 1 バイトも違わない
（Paint.NET の作業ファイル `online.pdn` だけは実行に不要なので落とした）。

1. **面別上書き層**（`balloons0s.txt`／`balloonk0s.txt`）の `windowposition` 各 3 行——下表。
2. `install.txt` の `name`／`directory`（`kakukaku for emo2 (windowposition-limit verification)`
   ／`emo2-kakukaku-wplimit`）。複製であることを名前で判るようにしたもので、判定には効かない。
3. `readme.txt`（**新規追加**）。この節と同じ内容を fixture 側にも置いてある。
...
> **時刻の書き方（本節の規約）**: areka のログ行のタイムスタンプは **UTC**（末尾 `Z`）である。
> 一方、保全ディレクトリ名の `-HHMMSS` は `Get-Date`（§2.1）が付けるので **ローカル時刻＝UTC+9**
> であり、しかも**走行の終了後**に採られるため走行の**終わり**に近い。両者は 9 時間ずれるうえ
> 指す瞬間も違うので、**本節で時刻を書くときは必ず `Z`（UTC）か「ローカル」かを明記すること。**
> 例: `wplimit-signoff-visual-20260814-191549` は **10:12:07Z 開始・10:15:48Z 終了**であり、
> ディレクトリ名の `191549` は**終了時刻のローカル表記**（＝10:15:48Z + 9h）である。
...
>
> > **弁別に効いているのは「キャラ寸の記録」ではなく「その寸で組まれた基本位置」である。**
> > `merge_scope restore` が採寸値（ここでは `868x1374`＝`434x687`×2）を書くこと自体は
> > 是正の前後で変わらない（§7.3 の 2 つめの開示の表）。本走行を是正前と確定させるのは
> > 下記 J2 の `from=(2046,-376)`——これは `2012 + (868−800)/2` であり、是正後なら
> > `2012 + (764−800)/2 = 1994` になっていたはずの値である。走行順序でも裏が取れる:
> > 本走行は **07:03:03Z**、`Rederive` が現れる最初の走行は **11:22:35Z** で、4 時間以上早い。
>
...
> 基本位置について是正後に採れているのは §7.2（k=1.0・100%）だけである。これは本仕様の
> **未解決の残件**として ここに登記する（是正後の 200% 再走を行えば埋まる）。
>
> **是正後・k=2.0 の走行そのものは存在する**（§7.4 の `wplimit-200cycle-20260814-220900`）が、
> 6 scope すべてが保存された相対位置を持って起動しているため**キーワード基本位置を
> 一度も踏んでおらず**、この残件は埋まらない（理由は §7.4 末尾）。

| 項目 | 値 |
|---|---|
| 実行日時 | 2026-08-14 **07:03:03Z 〜 07:05:04Z**（ローカル 16:03:03 〜 16:05:04・121.0 秒・`AREKA_APP_SMOKE_EXIT_MS=120000`） |
...
**J3 補正ログ ⇄ 位置ログ: PASS** — `Clamp` は `boot-gate` 1 件・`runtime-gate` 1 件。
**J3 の突合対象は `runtime-gate` の 1 件だけ**であり（§4 の J3 の但し書き＝起動時関門は
窓書込より前のデータ変換なので対になる位置ログを持たない）、その 1 件は
`from=(2166,1752) → to=(2166,1549)` に対し直後の
`[diag.window_move] route=BalloonFollow entity=5v0 kind=balloon scope=1 x=2166 y=1549` と
逐語一致している。

> **`boot-gate` の 1 件については「直後の位置ログと一致」を主張してはならない。**
> 本走行の `[diag.window_move]` は全部で **4 行**（`kind=char` 2・`kind=balloon` 2）で、
> `boot-gate` の `to=(2134,1549)` はそのどれとも一致しない——`x=2134` を持つ行が 1 行も無い。
> これは欠陥ではなく設計どおりで、起動時関門が直したのは**配置結果のデータ**であり、
> その値がそのまま窓へ書かれるとは限らないからである。本走行で差が出た内訳は同じログで辿れる:
> 起動時関門が見たのは `default_char_x=2110` から組んだ `2110 + (336−288)/2 = 2134`、
> その後 `route=MoveCue` がキャラ窓を `x=2142` へ動かし、追従が
> `2142 + balloon_off_x(24) = 2166` を組み直して窓へ書いた。`y` は両者とも `1549` で一致している。

**この 2 件はどちらも scope 1（中央下）が作業領域の下辺で止められたもの**
（`work_area` 下辺 `1752`）で、下記 J4 が「不要になった」と述べている scope 0 の
**右端**クランプとは別件である。

**J4 作業領域への内包: PASS** — **scope 0（中央上）について**、是正後は水平中央位置が最初から
領域内に収まり、**右端**クランプが不要になった。

**「是正前は `x=2489` で 9px はみ出していた」の出所は別走行
`wplimit-dpi96-keyword-20260814-193311`**（`wplimit-signoff-dpi96-keyword.log`・
10:26:36Z 開始 〜 10:32:49Z 終了・`primary_dpi=96`・**是正前**）である。その `runtime-gate` の 1 行が
`scope=0 from=(2489,852) → to=(2480,852) balloon_size=400x224 work_area=0,0,2880,1752` を
記録している（`2489 + 400 − 2880 = 9`）。本走行（是正後）の `Clamp` 行に `2489` は 1 件も無い。

> **この 2 走行は水平方向について直接比較できる。** どちらもキャラ窓が同じ位置・同じ実表示寸
> （`kind=char scope=0 x=2472 y=1205 w=382 h=547`）で観測されており、`adjust_dx` は両方 `0`、
> `balloon 400x224` も同じである。違うのは `balloon_off_x` だけ:
>
> | 走行 | `balloon_off_x` | 導出元のキャラ幅 | バルーン `x` | 右端 | クランプ |
> |---|---|---|---|---|---|
> | `…-193311`（是正前） | **+17** | 採寸値 `434` → `(434−400)/2` | `2472+17 = 2489` | `2889 > 2880` | **必要**（→ 2480） |
> | `…-203002`（是正後） | **−9** | 実表示寸 `382` → `(382−400)/2` | `2472−9 = 2463` | `2863 ≤ 2880` | **不要** |
>
> 後者の `−9` は本節の `[balloon-keyword] Rederive` が記録した `new_offset` そのものである。
> なお 2 走行は fixture の `windowposition.y` が違う（`…-193311` は原本の `-129`・
> `…-203002` は `y,0`）が、`y` は水平位置に効かないのでこの比較は成立する。

**scope 1（中央下）の下辺クランプは是正後も必要で、実際に 2 件出ている**
...
- **DD6 が実機で確認できた**（拡大率 200% 側・出所は §7.3 の水準①目視実走
  `wplimit-signoff-visual-20260814-191549`・**是正前**）: バルーンを画面外 `y=-107` で解放したとき、
  同時刻の保存は生値 `persist=(-143,-717)`、`release-gate` は表示だけを
  `from=(1983,-107) → to=(1983,0)` へ補正した。補正は相対位置へ焼き付いていない。
  **同型のペアは是正後の 200% 走行でも採れている**（§7.4・`balloon_y=-139` に対し
  `persist=(-1923,-749)`）——DD6 は是正の前後どちらでも成立している。
...
>
> 下表の水準①欄は、直下の目視実走
> （ログ `wplimit-signoff-visual-20260814-191549`・**10:12:07Z 開始 〜 10:15:48Z 終了**
> ＝ローカル 19:12:07 〜 19:15:48。ディレクトリ名の `191549` は**終了**時刻のローカル表記であって
> 開始時刻ではない）に基づく。この走行は
...
> 基本位置について是正後に採れているのは水準②（k=1.0・§7.2）のみ。200% で再走すれば埋まる。
> 是正後・k=2.0 の走行は §7.4 に登記してあるが、キーワード基本位置を踏んでいないので
> この残件は埋まらない。
...
>   `wplimit-signoff-dpi96-20260814-192617`（**10:18:58Z 開始 〜 10:25:54Z 終了**
>   ＝ローカル 19:18:58 〜 19:25:54・約 7 分・`primary_dpi=96`・
>   `work_area=0,0,2880,1752`）で観測した。この走行は task 4.5（`d6cb7a52`）**より前**＝
>   **是正前**である。
>
>   > **判定の根拠は走行順序**であって、走行の中身にビルド世代を弁別する印があるわけではない。
>   > **使ってはいけない 2 つの「印」**——どちらも是正後の走行にも同じように現れるので
>   > 弁別に使えない:
>   >
>   > - 「`merge_scope restore` のキャラ寸が採寸値 `434x687` のまま」は**印にならない**。
>   >   是正後の `wplimit-dpi96-fixed-20260814-203002` も同じ行に `char_w=434 char_h=687` を
>   >   書いている。再導出は**この行より後**に走るので、復元合流の行は是正の前後で同じ値になる。
>   > - 「`Rederive` 0 件」も**印にならない**。§7.2 が自ら記しているとおり、再導出は
>   >   **保存された相対位置が無いとき**にしか発火しない。本走行は
>   >   `saved_off_x=Some(-265) saved_off_y=Some(-243)`（scope 0）を読み戻して起動しており、
>   >   是正後のビルドであっても `Rederive` は 0 件になる。
>   >
>   > **実際に是正前を確定させるのは、保全ログのタイムスタンプで測った走行順序である:**
>   >
>   > | 走行 | UTC | 効く事実 |
>   > |---|---|---|
>   > | `wplimit-signoff-dpi96-20260814-192617`（本走行） | 10:18:58 開始・**10:25:54 終了** | — |
>   > | `wplimit-dpi96-keyword-20260814-193311` | **10:26:36 開始** | `saved_off_x`／`saved_off_y` とも **`None`** で起動し、それでも `Rederive` **0 件**。是正後のビルドなら**必ず発火していた**ので、この走行は**決定的に是正前**である |
>   > | `wplimit-dpi96-fixed-20260814-203002` | **11:22:35 開始** | 保全ログ全体で `Rederive` を出した**最も早い**走行（1 件） |
>   >
>   > 本走行は「決定的に是正前」と判っている `…-193311` が始まる **42 秒前に終わっている**。
>   > ゆえに本走行も是正前である。是正後の実行体が現れるのは 11:22Z 以降で、
>   > 10:26Z 台の走行より**約 1 時間後**にあたる。
...
| 実行時間 | 2026-08-14 **10:18:58Z 〜 10:25:54Z**（ローカル 19:18:58 〜 19:25:54・約 7 分・`ghost shutdown sequence completed` で正常終了） |
...
| ビルド世代 | **是正前**（task 4.5 `d6cb7a52` より前）。**根拠は走行順序**——本走行の終了 10:25:54Z の 42 秒後（10:26:36Z）に始まった `wplimit-dpi96-keyword-20260814-193311` が「保存値 `None` で起動して `Rederive` 0 件」＝決定的に是正前であり、`Rederive` を出す最初の走行は 11:22:35Z である。**「キャラ寸 `434x687`」も「`Rederive` 0 件」も弁別に使えない**（§7.3 の 2 つめの開示の表） |
...
### 7.4 200% の再起動サイクル実走（**是正後**・k=2.0）— 参考証跡

> **この走行は §8 の未解決残件を埋めない。** 理由は本節末尾に書いた。**§6 の完了判定表には
> 影響しない**（この走行は §5 の目視も §4 の全項目の突合も行っていない）。それでも登記するのは、
> **是正後のビルド × k≠1** で採れている唯一の保全ログであり、DD6（補正を相対位置へ焼き付けない）
> と `release-gate` の 200% 側証跡を持つからである。

| 項目 | 値 |
|---|---|
| 保全先 | `%LOCALAPPDATA%\areka-diag\wplimit-200cycle-20260814-220900\` |
| ログ | `wplimit-200cycle1.log`（166,976 bytes・md5 `14F69623AB446CC8292D3076811E1009`・768 行）／`wplimit-200cycle2.log`（21,310 bytes・md5 `4746CA403F406521B0101224EF253BEC`・95 行）／`wplimit-200cycle3.log`（21,046 bytes・md5 `E573D41DB9D2891B2EBD58EA3B404BEF`・94 行）＝**1 本 1 起動で計 3 サイクル** |
| 実行時間 | 2026-08-14 **13:08:23Z 〜 13:09:18Z**（ローカル 22:08:23 〜 22:09:18・3 起動を連続） |
| ビルド世代 | **是正後**。`Rederive` を出した最初の走行 `wplimit-dpi96-fixed-20260814-203002`（11:22:35Z）より**約 1 時間 46 分後**に始まっている |
| areka 自身が申告した実効 DPI | `primary_dpi=192`・`k_shell=2.0`・`k_balloon=2.0`（＝**k≠1**） |
| モニタ | index0 `work_area=0,0,2880,1704`（タスクバー 96px）／index1 `work_area=-2560,195,0,1795` `dpi=144` |
| 解決値 | scope0 `limit=true x_mode=CenterTop`／scope1 `limit=true x_mode=CenterBottom`（`windowposition_y=Some(0)`・`adjust_dy=0`＝現行 fixture の `y,0` どおり） |

**補正の実測**

| 関門 | 1 サイクル目 | 2 サイクル目 | 3 サイクル目 |
|---|---|---|---|
| `boot-gate` | 0 | 0 | 0 |
| `runtime-gate` | **61** | 0 | 0 |
| **`release-gate`** | **1** | 0 | 0 |
...
- **DD6（補正を相対位置へ焼き付けない）の 200%・是正後の逐語ペア**——1 サイクル目、バルーンを
  画面外 `y=-139` で解放したとき:

  ```
  balloon DragEnd 保存 scope=0 balloon_x=235 balloon_y=-139 char_x=2158 char_y=610
      offset_tl_x=-1923 offset_tl_y=-749 persist_x=-1923 persist_y=-749 char_w=764 char_h=1094
  [balloon-limit] Clamp 解放時補正 … scope=0 context="release-gate" route=BalloonLimitRelease
      from=PointPx { x: 235, y: -139 } to=PointPx { x: 235, y: 0 } balloon_size=SizePx { w: 800, h: 448 }
  ```

  保存されたのは**生値** `(-1923,-749)`、補正されたのは**表示位置だけ**。
  （§7.2 が引く `wplimit-signoff-visual-20260814-191549` の DD6 ペアは**是正前**の同型で、
  こちらは是正後の同型である。）
- 保存された相対位置の読み戻しは **3 サイクルとも完全恒等**（2・3 サイクル目の
  `merge_scope restore` は scope0 `saved_off=(-303,-259)`・scope1 `saved_off=(291,-140)` で同値）。

**なぜこの走行が §8 の残件を埋めないか**

§8 の残件は「**キーワード基本位置**について、k≠1 での**是正後**の実機証跡が無い」である。
本走行は是正後・k=2.0 の条件を満たすが、**キーワード基本位置を一度も踏んでいない**:

- 3 起動 6 scope すべてが**保存された相対位置を持った状態**で起動している
  （1 サイクル目でさえ scope0 `saved_off=(-101,-158)`・scope1 `saved_off=(154,-95)` を
  読み戻している——直前の `wplimit-final-20260814-215251` のドラッグが残した値）。
- 要件 4.7 により、保存値がある間はキーワードの素材が装填されない。実際
  **`[balloon-keyword] Rederive` は 3 本とも 0 件**である。
- ゆえに本走行が示したのは「保存値の往復」と「関門の封じ込め」であって、
  **「キーワードが導く基本位置が実表示寸に対して中央へ揃っているか」ではない**。

残件を埋めるには、**永続状態を消してから**（§8 の `sylphya.toml` の項）200% で起動し、
`Rederive` の `new_offset` と直後のバルーン位置ログが実表示寸に対して中央であることを
突合する必要がある。
...
- **未解決の残件（記録の欠落）: キーワード基本位置について、k≠1 での是正後の実機証跡が無い。**
  ...
  **是正後・k=2.0 の走行が無いわけではない**——§7.4 の `wplimit-200cycle-20260814-220900` が
  それだが、6 scope すべてが保存された相対位置を持って起動したため
  `[balloon-keyword] Rederive` が 3 本とも 0 件で、キーワード基本位置を一度も踏んでいない。
  **再走するときは、まず下の `sylphya.toml` の項に従って永続状態を消すこと**——消さずに
  起動すると同じ理由で空振りする。
