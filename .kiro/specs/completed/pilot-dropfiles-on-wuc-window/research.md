...
- **選択肢**: (i) 窓手続きの中で幾何を定数で計算／(ii) 記録を待ち行列に積み ECS の system が次の tick で判定／(iii) 受け口が World への弱い参照（`Weak<RefCell<EcsWorld>>`＝wintf が `run()` の冒頭で入れる `EcsWorldSelfRef` の複製）を文脈として持ち、到着の場で `upgrade()` → `try_borrow()` して判定。
- **選択**: (iii)。
- **理由**: 透過機構と同じ判定器・同じ座標系（物理 px のクライアント座標＝`DragQueryPoint` の値をそのまま渡せる）で、要件 4 の「クリックの当たり判定と一致するか」を直接測れる。(i) は 200% で論理 px と物理 px を混ぜる罠（§5 の落とし穴 (1)）。(ii) は行が 1 tick 遅れ、待ち行列と system が 1 つずつ増える。窓手続きが World を `try_borrow` で借りるのは wintf 自身の `dispatch_window_message` と同じ作法。取っ手は自前で作らず `EcsWorldSelfRef` を使う（設計レビュー指摘 1: `Rc` を World 自身へ入れると自分を強く指す輪ができ、終了時に解放されず R5 の観測を濁らせる）。
- **代償**: 借りられない瞬間（tick の途中の同期配送・終了処理の途中）は `opaque=unknown`。R1 の観測になるのは `[dropfiles] 終了` の行より前に出たときだけ（設計レビュー指摘 3: `crates/wintf/src/com/wuc.rs` の `pump_current_thread_messages` は World を借りたまま待ち行列を配送する）。
- **型名の注意**（設計レビュー指摘 2）: `hit_test_in_window` の引数は `wintf::ecs::PointF`。`wintf::ecs::PhysicalPoint` は `pointer` 側の明示の再輸出が勝って整数の `Point` になるので使わない。
...
#### 付随の決定
- **終了コードは 0（上限時間で終了＝正常）／2（初期化の失敗）／3（窓が外から閉じられた）**。手本の 1（打ち切り）・「較正不合格」はこの example に該当が無い。3 は設計レビューの観察で足した（Alt＋F4 の走行を初期化の失敗と読み違えない）。
...
- `try_borrow` の失敗が頻発する（`終了` の行より前で） → 投函でなく同期配送の経路がある証拠。行に `opaque=unknown` が残るので見落とさない。必要なら (ii) の待ち行列へ切り替える。`終了` の行の直後の `unknown` は終了処理の配送によるもので、証拠にしない。
