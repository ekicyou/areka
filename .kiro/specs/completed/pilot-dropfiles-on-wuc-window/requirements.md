...
- **直す**: 届くが条件つき（例: 透過の付け外しの時機・受け入れの宣言の付け方）で、本坑の中で直せる。直し方を README に書く。絵の上では届くが絵の外でも窓が受け取ってしまう（背後の窓へ抜けない）場合もここに入れる（本坑は絵の外の落とし物を捨て、背後へ抜けないことを既知の制限とする）。
...
2. The pilot shall 検証をふつうの権限（管理者でない）で起動した example で行い、起動時に管理者として動いているかをログへ出す。管理者として動いていた走行は判定に使わず、ふつうの権限で走り直す（areka を管理者で動かすことは普通は無いため、管理者で起動したときの手当ては本仕様でも本坑でも扱わない）。
3. When ⓐとⓒで落とし物のメッセージが届いてパスが取れ、ⓑで届かず背後の窓に渡ったとき, the pilot shall 結果を **go** の見立てとする。
4. If ⓐ〜ⓒのいずれかで届かない、または条件によって届き方が変わったとき, the pilot shall 「違う」と結論する前に、条件つきで届く可能性のある次の手当てを順に試して各結果をログと README に記録する——受け入れの宣言の付け方（窓を建てた後に付け直す・`DragAcceptFiles` で宣言する）・透過の付け外しの時機との関係。
5. When 前項の手当てのいずれかで届くようになったとき, the pilot shall 結果を **直す** の見立てとし、効いた手当てを README に書く。
6. When 前項の手当てをすべて試しても ⓐ か ⓒ で届かないとき, the pilot shall 結果を **違う** の見立てとし、`IDropTarget` へ倒す場合の STA の置き場所の見立てを学びに書く。
7. When 前項の手当てをすべて試した後、ⓐとⓒでは届くが ⓑ で窓が受け取る（背後の窓へ抜けない）とき, the pilot shall 結果を **違う** でなく **直す** の見立てとし、本坑での扱い（落ちた位置を当たり判定にかけ、絵の外なら捨てる・背後へ抜けないことを既知の制限として書く）を README に書く。
8. The pilot shall 見立てを README に書くが、go／違う／直す の判定は開発者（人間）に委ね、Claude Code 単独で go を宣言して本坑へ進まない。
...
4. The README shall 検証結果の幕で、第 5 要件の各項ⓐ〜ⓒの結果（届いたか・取れたパス・その時点の透過の状態・受け入れの宣言が残っていたか）・ふつうの権限で起動したこと（起動時のログ）・試した手当てとその結果・学び・日付を書く。
