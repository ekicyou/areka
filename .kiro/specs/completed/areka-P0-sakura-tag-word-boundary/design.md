...
- 根拠（要件 7.2 の 4 点）: ① 正典はタグ語の終端規則を明文化していない。② 正典のタグ名は 3 形しか無いことを `doc/ukadoc-coverage/ledger/sakura-script.toml` の `\` 始まり項目で全数え確認した（1 文字 31・`\_` ＋ 1 文字 14・`\__` ＋ 1 文字 5）。例外は角括弧付きの旧仕様の綴り `q*` 1 つで、`doc/ukadoc-coverage/catalog.toml` の全見出しの機械検索で確かめた（2026-09-17 追記・`research.md` §11）。③ `[` はエスケープ対象でなく（エスケープはバックスラッシュの二重化のみ）台詞中の素の `[` は正当な入力である。④ 完了済み spec `areka-P0-sakura-parse` の受入基準 13.1 に `[` 後続形のテストが無かった。
