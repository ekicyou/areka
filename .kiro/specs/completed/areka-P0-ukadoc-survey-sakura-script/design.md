...
**変わること**: 着地済みの台帳 1 本が 342 項目とも埋まり、ブリーフィング 1 本が増え、担当ドメインの報告 1 本が作り直され、実装済みと判定した項目の定義箇所に正典 URL の 1 行コメントが 23 行加わる。実行時の判断は 1 つも変わらない。
...
`doc/COMPAT_ARCHITECTURE.md` §8 の見出しは「正典沈黙箇所の areka 裁量記録」であり、行には 2 種類ある。⑴ 正典が黙っている所を areka が決めた行（`%username` の既定値・`%selfname2` 未定義時など）。⑵ 正典に定めがあるのに別の応答を返すと明記した行（`\![move]` の名前付き `--key=value` 形・非スコープ基準・`time>0` の 3 行。`\_l` の縦書きの行は**かつてここに入っていたが、2026-09-04 の `areka-P0-cursor-tag-canon` の着地で既知非互換が取り下げられ ⑵ から外れた**）。**`degraded` の根拠になるのは ⑵ だけ**である。要件 2.6 は「正典が定める応答に対して別の応答を返す」と書いており、正典が黙っている所には当たらない。⑴ の行は `implemented` の判断を覆さず、裁量の内容を備考に写す。

「M1 非実装／非受理／非実導出で語彙（と意味論）の記録だけがある」と明記した行（時間指令の allowlist＝「語彙保持」・`\![set,balloontimeout]` と `\x`／`\x[noclear]`＝「完全な語彙と意味論の記録のみ」・`\f[align]`／`\f[valign]`＝「未実装（語彙記録）」）は `vocabulary-only` の根拠であって `degraded` ではない（要件 2.5 の 2 つ目の登記）。`\_l` の行は**かつて**「未実装（語彙記録＋既知非互換の登記）」で ⑵ に当たっていたが、**2026-09-06 の rebase が持ち込んだ現 HEAD ではその行の表題自身が既知非互換の取り下げを宣言し、状態欄は「実装済み（2026-09-04・`areka-P0-cursor-tag-canon` の着地。解決の唯一の定義点＝`crates/areka-emo-text/src/cursor_tag.rs::resolve_cursor_axis`）」である**。したがって ⑴ でも ⑵ でもなく、規則 2 の順 4 で `implemented` になる（DD-9 の Q5）。
...
| 5 | `\_l[x,y]` の縦書き | **`implemented`**（**2026-09-06 の最終検証で `degraded` から改めた**）。**当初の判定は rebase 前の §8 を読んで下したものだった**——現 HEAD の当該行は表題自身が「既知非互換は下欄のとおり取り下げた」と書き、状態欄は「実装済み（2026-09-04・`areka-P0-cursor-tag-canon` の着地）」である。コードも同じで、`crates/areka-emo-text/src/cursor_tag.rs` の `resolve_cursor_axis` が絶対（負値を含む）・`@` 相対・`px`／`em`／`lh`／`%`・`centerx`／`centery` をすべて実導出する。`Err` に落ちるのは `CenterAxisMismatch`（軸を取り違えて書いたとき）と `Unparsable`（解釈不能）の 2 つだけで、いずれも**正典の正常形に対する縮退ではなく書き手の誤りに対する反応**だから要件 2.6 の「正典が定める応答に対して別の応答」に当たらない。疑義 SC8（`\_l` の副作用の文が横書き前提のまま）は**正典側が未規定**なので、これも根拠にならない。よって順 3 は当たらず、順 4 ⒜（`decode_tag` の `"_l"` の腕 → `Instruction::Cursor` → `compile` の cue → `resolve_cursor_axis`）で `implemented`。§8 を全文検索しても `\_l` の縮退を今も主張する行は 1 行も無い |
...
| 3 | 2 本以上が主張するが**分担が合意済み**（互いを名指ししている、または §8 に住み分けが登記されている。**2026-09-05 のタスク 3.1 で実測を取り直した**——主張の型を `ownership`／`example`／`lexical-boundary` の 3 つに割って行番号ごとに逐語で裏取りすると、二重所有は **6 件**で、うち順 3 に落ちるのは **4 件**＝`\![set,balloontimeout,時間]`・`\_l[x,y]`〔根拠は名指しでなく §8 の `\_l` の行の「実装は areka-P0-cursor-tag-canon」の登記。**この 1 件だけは 2026-09-06 の是正で `implemented` になったため、担当は「作業が残る側」ではなく要件 8.8 の「誰が実装したか」の記録として読む**〕・`\x`・`\x[noclear]`。`\![set,choicetimeout,時間]` は**主張する現役の brief が 1 本だけ**〔相方 `areka-P0-choice-select-events` は完了済み〕なので順 3 ではなく要件 8.7 の側の話であり、この列から外す） | まだ作業が残っている側の spec 名。相方と分担の中身を備考に書く（二重所有として扱わない） |
...
| 同 `decode_tag` の `"_w"`・`"n"`・`"p"`・`"s"`・`"b"`・`"_l"`・`"q"` の腕 | `\_w[時間]`・`\n[パーセント]`・`\n[half]`・`\p[ID番号]`・`\s[ID番号]`・`\b[ID番号]`・`\_l[x,y]`・`\q` の現行形のうち `implemented` と判定したもの | 9 |
...
`move_cue.rs` は `degraded` なので置かない（`decode_tag` の `"_l"` の腕は 2026-09-06 の是正で `implemented` になったので置く）。`lexer.rs` は DD-5 により 1 行も置かない。`crates/areka-sylphya/src/vocab/flat.rs` の語彙表にも置かない（要件 9.6）。触る 5 ファイルの現在の行数は 351／521／159／156／**389** で、1 ファイル 1,000 行の上限から遠い（要件 9.7。2026-09-06 の rebase 後の実測。sylphya_wiring.rs は areka-P0-ukadoc-survey-property が置いた URL コメント 2 行の分だけ 387 → 389 に増えた）。
...
| `implemented` は 30 件・URL コメントは約 30 行・7 ファイル・5 クレート | **23 件・23 行・5 ファイル・5 クレート**。減った内訳は `\![move]`・`\q[タイトル,script:実行内容]`・`\q[タイトル,ID1,ID2,ID3...]` が `degraded`（−3。`\_l[x,y]` も一度はここに数えたが、2026-09-06 の最終検証で `implemented` へ戻した）、`\bID番号`・`\pID番号`・`\q[タイトル,ID]` が `alias`（−3）、単独 `\![*]` が `absent`（−1） | コメントを置く場所の表。`lexer.rs` と `move_cue.rs` に触らなくなる |
