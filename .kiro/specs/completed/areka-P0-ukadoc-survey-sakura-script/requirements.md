...
- `doc/ukadoc-coverage/` は 2026-09-04 の実測時点では存在しない。**2026-09-05 に更新**: 上流の道具 `crates/ukadoc-survey` が `origin/main` へ着地し（PR#136）本ワークツリーへ取り込んだので、現在は `doc/ukadoc-coverage/` 一式（カタログ 1,749 件・4 ドメインの台帳・報告 4 本・テーマ定義・手順書）が repo にある。担当の台帳 `ledger/sakura-script.toml` は 342 項目すべてが `unclassified` で並んでおり、前置き・欄の順・id のバイト順は設計の規則 0 と一致する。これにより要件 1.10 の「道具が未着地の場合」の枝は使わず、要件 11.1（報告の作り直し）が本 spec の担当に入り、要件 11.2 の枝は使わない。要件そのものの改訂は要らない（1.10 と 11.1／11.2 はいずれも条件付きの条項であり、条件が満たされた側へ倒れただけである）。
...
5. Where 2.4 に当たらず、かつ areka の登記に名前が載っている, the さくらスクリプト調査 shall 状態を `vocabulary-only` とする。「登記」は 2 つに限る: `crates/areka-sylphya/src/vocab/flat.rs` の `%` 語彙表（26 名と 2 つの書式記録）と、`doc/COMPAT_ARCHITECTURE.md` §8 が「M1 非実装／非受理／非実導出で、語彙（と意味論）の記録だけがある」と明記している行（実文は行ごとに「完全な語彙と意味論の記録のみ」「未実装（語彙記録）」「語彙保持」と違う。「語彙・意味論のみ記録」という逐語は §8 に無い）。**2.4 と 2.5 の双方に当たる項目は `implemented` とする**（`%` 語彙表の 26 名には値の源が実在する `username`・`selfname`・`selfname2`・`keroname` が含まれるため、表に名前があることだけで `vocabulary-only` としない）。
6. Where 正典が定める応答に対して areka が別の応答（記録付きの読み飛ばし・即時反映・座標を返さないなど）を返すことが既に登記されている, the さくらスクリプト調査 shall 状態を `degraded` とし、転記元（`doc/COMPAT_ARCHITECTURE.md` の該当行）を備考に書く。**この判定は 2.4 より先に当てる**——正典の主要な用法が動いていても（`\![move]` の基本形・`\q[タイトル,script:実行内容]` の選択肢表示など。**`\_l[x,y]` の横書きも 2026-09-05 まではこの例に挙げていたが、`areka-P0-cursor-tag-canon` の着地で `doc/COMPAT_ARCHITECTURE.md` の既知非互換が取り下げられたため、2026-09-06 の最終検証の是正で例から外した**）、登記された縮退や既知の非互換が 1 つでもあれば `degraded` とし、動く用法と動かない用法の両方を備考に書く。`degraded` の項目にはソースの URL コメントを置かない（並走 2 本の調査と同じ扱い）。
...
2. The さくらスクリプト調査 shall 本文の注記で向きが決まる 7 件（本文に `[旧仕様]` を含む項目）を先に確定させ、そのうち置き換え先が本文に明示されているもの（`\![reloadsurface]` は `\![reload,shell]`・`\z` は本文が「選択肢を含むスクリプトにおける `\e`」と書くので `\e`・旧書式 `\q[ID][タイトル]または\q*[ID][タイトル]` は本文が「上記のものとは違い」と現行の `\q` の書式群を参照で特定しているので現行 `\q` の正典）を `alias_of` で結ぶ。旧書式 `\q` は areka の `fold_legacy_q` がわざと選択肢にせず素通しにするので、別名の行でもその綴りを受けないことを備考に書く（3.4）。`\a` は旧仕様の札が付くが本文が現行の振る舞い（`OnAITalk` を発生させる）を明記しているので、別名にせず自分の状態を持ち、`triggers` で `OnAITalk` を指す。
...
5. Where 同じ命令で引数の数だけが違う組がある（実測 9 群。名寄せの規則しだいで 2 件以上の id を持つ名前は 20〜24 群・99〜107 件になるが、そのうち別名の候補になるのはこの 9 群だけ）, the さくらスクリプト調査 shall 引数の空間が広い側を正典・狭い側を別名とし、狭さ（`\sID番号` の「0〜9 のみ」など）を備考に書く。**引数の値が違うだけの兄弟**（`\![set,windowstate,minimize]` と `stayontop`・`\![open,dialog,color]` と `folder`・`\n[パーセント]` と `\n[half]`・`\_a[ID]` と `\_a[OnID,...]`・`\i[ID番号]` と `\i[ID,wait]`・`\__w[時間]` と `\__w[animation,ID]`・`\_b[...,inline]` と `\_b[...,x,y]` など 11 群）は同じ機能の別の書式ではないので、別名にせず、それぞれが自分の状態を持つ。ただし別名の判定は群ではなく 2 つの id の対に当てるので、兄弟の群の中に引数の数だけが違う対が混じっていれば（`\_b[...,x,y]` と `\_b[...,inline]` が別々の鎖を持つ・`\_a[ID]` は `\_a[ID,r2,r3...]` の引数を減らした形）その対だけは別名になる。名寄せの規則（どこまでを名前とみなすか）と対の判定は設計で 1 つの文に書き切り、ブリーフィングの一覧はその規則で再現できる数を載せる。
...
10. The さくらスクリプト調査 shall 版番号が無い 277 件を最も古いものと決めつけない。そのうち本文が 2 桁の版だけを書いている項目（**2026-09-05 のタスク 2.4 の裁定で訂正**。当初の実測は「5 件」としたが、それは見出しの行の数であって id の数ではなく、また `\![move]` の `2.5` は版ではなかった。id で数え直した実測は **6 件**: `\![sound,play,...]`・`\![sound,load,...]`・`\![sound,option,...]` の各 `2.4`〔正典では別々の 3 id〕・`\__v[オプション]` の `2.4`・`\![biff(,アカウント名)]` の `2.5`・`\![execute,resetwindowpos]` の `2.7 RC1`。`\![move]` の本文に版の区画は無く、`2.5` は記述例の「2.5 秒かけて移動する」という所要時間であって版番号ではない——本条自身が警告する「秒数や例文の数字を誤って拾う」形にちょうど当たっていた）は、カタログの抽出規則（3 桁）では拾えないので `introduced` を空文字のままとし、備考に「本文は `2.4` と書くがカタログの規則で拾えない」の形で本文の版を写して、単なる世代不明と区別する。2 桁を拾う規則を上流へ提案しない（`\w時間` の `0.45` など秒数や例文の数字を誤って拾うため）。
...
8. The さくらスクリプト調査 shall 使用頻度（影響する既存資産の広さ）の参照値として、里々／YAYA コミュニティ wiki の作例に綴りが現れる文書数（ukadoc MCP の検索結果の総数・里々と YAYA を別々に数える）を用い、それが裁定（roadmap 追記(89)⑧⑷）の言う「標準テンプレート辞書が使うタグ」の弱い代理であることをブリーフィングと備考に明記する（開発者裁定 2026-09-04: この環境の ukadoc MCP に標準テンプレート辞書そのものは入っておらず、あるのは wiki の作例だけであるため）。
8a. Where 里々／YAYA の標準テンプレート辞書（新規ゴースト作成の雛形）の辞書ファイルが、開発者から場所を指定されて手に入る, the さくらスクリプト調査 shall その辞書ファイルだけを走査した綴りの出現を参照値に置き換え、8 の代理を補助に下げる。実ゴースト資産の走査は行わない。
...
4. If 2 本以上の brief が同じ id を主張する, then the さくらスクリプト調査 shall どちらか一方を憶測で選ばず、`owner` を空文字のままにして両者の主張をブリーフィングに並べ、裁定案を 1 つ添えたうえでそれが案であって決定ではないことを明記する。ただし両者が既に分担を合意している場合（互いに相手を名指ししている、または `doc/COMPAT_ARCHITECTURE.md` §8 に住み分けが登記されている。実測: `\![set,balloontimeout,時間]` の compile 側＝`sakura-time-directives`／実導出側＝`balloon-canon-residue`・`\![set,choicetimeout,時間]` の compile 側＝`sakura-time-directives`／実行時側＝完了済み `choice-select-events`。**2026-09-05 のタスク 3.1 で実測を取り直した**——主張の型を逐語で割ると二重所有は 6 件で、この条の順 3 に当たるのは `\![set,balloontimeout,時間]`・`\_l[x,y]`〔根拠は §8 の登記〕・`\x`・`\x[noclear]` の 4 件。`\![set,choicetimeout,時間]` は主張する現役の brief が 1 本だけなので本条ではなく 8.7 の側の話である）は二重所有として扱わず、まだ作業が残っている側を `owner` に書き、相方と分担の中身を備考に書く。互いに相手を知らない主張（実測 2 件: `\![embed,...]` を `sakura-time-directives` と `property-query-channels` が、`\![move]` を `surfaces-basepos` と `sakura-time-directives` が、それぞれ別々に主張。**2026-09-05 のタスク 3.1 の訂正**——`\![move]` は実際に両者が互いを 0 回も名指ししていないが、`\![embed,...]` は `property-query-channels` の brief が相手を **Adjacent として一方向に名指ししている**。その名指しは「層違いで非衝突」と述べるだけで分担を決めておらず相方も相手を認識していないので、結論は本条のとおり `owner` を空にする側で変わらないが、備考の理由は「相手を知らない」ではなく「名指しは一方向で分担が未確定」と書き分ける。`\![move]` は `doc/COMPAT_ARCHITECTURE.md` §8 の 5 行が完了済み `sakura-dialogue-tags` を担当に書くだけで、両者の住み分けは登記されていない）だけがこの条の対象になる。
...
1. The さくらスクリプト調査 shall areka の実行時の振る舞いを変えない（追加するのは要件 9 のコメント行だけ）。字句・意味写像・compile・消費側のいずれの判断も変えない。**2026-09-06 の開発者裁定で 1 つだけ例外を設けた**——上流の道具の空振り台帳（`crates/ukadoc-survey/tests/consistency/checks.rs` の `census` が要件 6.5・6.6・6.7 の 3 面・6.8・6.11 を「今日の実データでは対象 0 件」と固定している行）は、本 spec が台帳を埋めたことで**初めて対象が生まれた**。テスト自身の説明文が「対象が 1 件でも生まれたら `Subjects::NonEmpty` へ移し、非空の主張へ書き換えること」と指示しており、上流 toolkit spec は `completed/` で新しい作業を引き受けられないため、**本 spec がその行を移す**。テスト専用の変更であり実行時に評価される記述は 1 行も足さない（実行時の振る舞いは 1 ビットも変わらない）。**2026-09-06 の 2 度目の適用**——同じ裁定は「上流の常設テストが『今日の実データ』を前提に置いており、本 spec が台帳を埋めたことでその前提が崩れた」という**同じ型の案件すべて**に及ぶ。実例 2 件目は `crates/ukadoc-survey/tests/consistency/perturb.rs` の `ANCHOR_ID`（摂動の的）で、本 spec の台帳がその id へ `triggers` を張ったため「所見がちょうど 1 件」の前提が崩れた。いずれもテスト専用の変更で実行時の振る舞いは変わらない。
...
| 「COMPAT §8 登記」 | §8 のうちさくらスクリプトのタグを扱う行は **17 行**、`%` を主題にする行が **4 行**（`:128`・`:137`・`:138`・`:151`）、合わせて **21 行**。タグの行は（`:129`・`:130`〜`:134`・`:143`・`:154`・`:158`・`:161`・`:164`・`:181`・`:183`・`:198`・`:203`・`:206`・`:208`）。うち複数の行が「M1 非実装／非受理／非実導出」で語彙（と意味論）の記録だけがあると明記しており（実文は「完全な語彙と意味論の記録のみ」「未実装（語彙記録）」「語彙保持」）、要件 2.5 の「登記」の 2 つ目の源になる。§8 の本文中の file:line には既に古いものがある（例: `:164` が指す `decode.rs:180` は現在 `:186`） | 実測（要件 8.10） |
