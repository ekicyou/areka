...
- [x] 1. ループオフセット型の定義
- [x] 1.1 `LoopOffset` enum と `LoopOffsetRange` struct の実装
...
- [x] 2. ストーリーボード定義への統合
- [x] 2.1 `Storyboard` と `CompiledStoryboard` への `loop_offset` フィールド追加
...
- [x] 3.3 `StoryboardInstance` への遅延パラメータフィールド追加
...
- [x] 3.4 `advance_loop()` への遅延生成ロジック統合
...
- [x] 4. Pause/Resume/Cancel との整合性確保
- [x] 4.1 既存の Pause/Resume メカニズムとの動作確認
...
- [x] 5.2 (P) `validate.rs` への V14-V17 ルール追加
