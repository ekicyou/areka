...
| 段 | 中身 | 並走できる境界 |
|---|---|---|
| 0 | 要件 1 の 3 条件の確認と数え直し。道具の追加分（`documents`・`patch`・`priority-apply`・`render_summary` の純関数化・`paths`）を在中テスト付きで実装。骨組みの形は本設計 D-1 で確定しているので、文書の下書きと並走できる | 道具（crate）と文書（doc）は別ディレクトリ |
