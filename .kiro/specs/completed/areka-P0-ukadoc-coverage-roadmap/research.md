...
| # | 処分 | 反映先 |
|---|---|---|
| 1 | **開発者裁定（議題 1・2026-09-11）: (a) `linkage.md` が帰属の正本**。機械の束を核にし、関連 0 本の 1,127 件は `links` を足さずに人手で束へ入れる。`links` は跨ぐ骨格の不足分だけ | 要件 3.6・4.1 ⑵⑶・4.2 |
...
| 5 | 要件で確定: 判定 ⑹ は台帳＋カタログ由来の本文だけ。**開発者裁定（設計議題 1・2026-09-12）: 証拠の表は `summary.md` から外し `evidence` 副手続きに一本化・⑹ は全文一致**（案 c） | 要件 11.1 ⑹・11.2・設計 D-5 |
| 6 | 要件で確定: 段階内の束の順位を 1 から通し。README の欄の定義は変えない（10 刻み提案は処分台帳で却下） | 要件 7.1 |
| 7 | **設計**——3 文書の機械可読な形と、判定 ⑴ が拾う id の範囲 | 設計 |
| 8 | **開発者裁定（議題 2・2026-09-11）: (a) テンプレート辞書の実物を静的に読む**（里々「ポストと狛犬」・YAYA「はろーYAYAわーるど」／「SimpleYAYA」の 2〜3 本に限る・実走しない・配布物はリポジトリに入れない）。取得できなければ wiki の語彙へ退路（6.8） | 要件 6.3・6.8・9.5・Boundary |
