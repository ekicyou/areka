...
- 行に「分割の由来」を記録する拡張（6.4 は**開発者裁定 2026-09-04 で取り下げ**＝`PositionedLine` にフィールドは足さず、配置層は内容の無い行を出さない。`\c[line]` の「行」は台本の構造＝DD-9。`\c[line]` 自体は**開発者裁定 2026-09-05 で実装しない**）。
...
- `\c[line]`／`\c[char]` 系の実装（**開発者裁定 2026-09-05: 実装しない**。SSP はバルーンの文字領域をビットマップとして保持し（書いた内容は画像として維持）、areka はグリフをそのまま保持する＝哲学が異なり同様にはなりえない。台帳 `areka-P0-ukadoc-survey-sakura-script` へは「意図的非実装」として転記する）。
...
- `PositionedLine` に分割の由来を持たせる変更——本書「行構造」の拡張の口を消費する（`\c[line]` は 2026-09-05 裁定で実装しないので、この口を消費する予定は無い）。
...
| 6.3 | `\c[line]` が成立しうる行境界 | 配線・登記 | 内容のある行の境界は `Vec<PositionedLine>`。内容の無い区間と折返し除外は台本構造の数え方（DD-9・参考記録。`\c[line]` は 2026-09-05 裁定で実装しない） | H6・H6b |
...
| 7.3 | `\c[line]` 所有者不在の登記 | 登記 | 同上 |同上（2026-09-05 裁定で「意図的非実装」へ更新） | — |
| 7.4 | SC9 解消の登記 | 登記 | 同上 | — |
...
| 項目 | 登記 | 追跡先 | Req |
|---|---|---|---|
| `\_l` 直後の左寄せリセット | 正典逐語（付録 A＝`requirements.md:187`・`:207`）: 「`\_l`実行直後には、トラブル防止のため行揃えが左揃えにリセットされる」「`\_l`タグが来た場合は左寄せに戻る」。**2026-09-04 に ukadoc 本文と 1 文字ずつ突き合わせて一致を確認**（`ukadoc:list_sakura_script:_5c_l_5bx_2cy_5d:1`・`ukadoc:list_sakura_script:_5cf_5balign_2c_5bc4_305b_308b_5074_5d:1`）。**戻るのは `align` だけ**——正典 `\f[valign,寄せる側]` は「alignと異なり、`\n`や`\_l`ではリセットされない」と定める（`ukadoc:list_sakura_script:_5cf_5bvalign_2c_5bc4_305b_308b_5074_5d:1`・同日確認） | `areka-P0-text-decoration-canon`。**双方向の実在を 2026-09-04 に確認**——送り＝`requirements.md:138`（Requirement 7.1）・本行・同 brief `:68-75`（本仕様からの追加登記の節）・`:72`（追加登記 1）、受け＝同 brief `:33`（In に「`\_l` 相互作用含む」）・`:34`（「`\_l` 側の実装（`cursor-tag-canon`）」と名指し、左寄せリセット・インデント相互作用は自らが「align 実装 spec」）・`:47`（Upstream に本仕様・「相互参照必須」）。**同 `:34` の指名は正確**——本仕様の `brief.md:60` が「実装は align 実装 spec が所有」と、`:34` が引く語そのもので指名している（spec 名は書かれておらず役割名での指名）。指名の実体は `brief.md:60`・`requirements.md:138`・`design.md:69` と本表。上の追加登記（td-canon brief の新節）で spec 名を明示して補った | 7.1 |
| `\_l` 後の中央揃えインデント | 正典逐語（付録 A＝`requirements.md:207`）: 「`\_l`タグで移動後に`\f[align]`タグで中央揃えを設定した場合、`\_l`タグのX座標分インデント処理されたと仮定して中央揃え処理される」。**2026-09-04 に ukadoc 本文と一致を確認**。渡すべき X 座標は `cursor_tag.rs` が解決済みで、寄せの計算へ渡す口だけが無い | 同上（同 brief `:73`＝追加登記 2） | 7.1 |
| SC8（縦書きでのインデント軸） | 正典は X のまま。縦書きではインデント量は行内軸＝Y から来るはず。**未解決・本仕様は裁定しない**（`requirements.md:139`＝Requirement 7.2・付録 B の SC8 行 `:217`） | 同上。**追跡先の実在と引受を 2026-09-04 に確認**——同 brief `:74`（追加登記 3）が SC8 を名指しで引き受け、`:33` の In が「align/valign/underline の縦書き写像」を所有範囲に含む | 7.2 |
| `\c[line]`／`\c[line,数値,開始位置]`（`\c[char]` 系も同じ） | **実装しない（開発者裁定 2026-09-05）**: SSP はバルーンの文字領域をビットマップとして保持し（書いた内容は画像として維持）、areka はグリフをそのまま保持する＝哲学が異なり同様にはなりえない。現状は `decode.rs:219` に `c` の腕が無く `Raw` へ落ち、`compile.rs:203` の catch-all が無視する＝無害な no-op（挙動変更なし）。実装済みは無引数 `\c` のみ（`state.rs:374`／`actor.rs:427`）。以下は当初の登記（参考記録）——実装所有者**不在**（2026-09-02 実測・**2026-09-04 再確認**＝`.kiro/`〔`completed/` を除く〕と `doc/` の Markdown・TOML 全域で `\c[line` の一致が本仕様の文書以外 0 件）。「行」＝`\_l[x,y]`・`\n`・`\n[half]`・`\n[パーセント]` による分割、`\_l` 以外で分割された内容の無いものは行と見なさない（正典逐語＝`requirements.md:205`・2026-09-04 に ukadoc 本文と一致を確認）。**配置層の行数を使ってはならない**（折返しを含み・内容の無い区間を持たない）。行は台本構造＝item 列（`TextItem::LineBreak`／`CursorMove`）を分割点で区切って数え、`\c` 自身を保留の確定点（文字と同じ発行）として扱う（DD-9・開発者裁定 2026-09-04） | 台帳 `areka-P0-ukadoc-survey-sakura-script` へ「意図的非実装」として転記する（同 spec は別ブランチで進行中・本ブランチからは触れない）。**同 spec の実在を 2026-09-04 に確認**——`brief.md:5`（所有範囲＝さくらスクリプト全 342 項目）・`:27`（全項目に「担当 spec〔既存 brief 名または『所有者なし』〕」を付ける）・`:43`（In に無所有一覧） | 6.3・7.3 |
| SC9（`centerx`／`centery` の基準） | **解消**。正典文「これだけは文字描画開始点ではなくバルーン画像そのものが基準」（付録 A＝`requirements.md:191`）。本仕様が実装する。**注**: この一文は 2.8.83 で加わったもので、ukadoc の MCP スナップショット（2.8.80）には `centerx`／`centery` の行自体が無い（`requirements.md:209` の注・2026-09-04 に MCP 側で再確認済み）。逐語の正本はライブ ukadoc から採録した付録 A | — | 7.4 |
| `\_l` の 3 個目以降の引数 | `crates/areka-parsers/src/sakura/decode.rs:224-229`（`fn decode_cursor`）が先頭 2 つだけを取り出し、残りを持つ反復子をそのまま捨てる（記録も警告もしない）。**行位置を 2026-09-04 に引き直して同一であることを確認**。正典は 2 引数のみ＝実害なし。「記録なく捨てる」の是正は所有者なし。本仕様は `areka-parsers` に触れない | なし | 7.6 |
| `\f[align]`／`\f[valign]`／`\c[line]`／`\_q` | 本仕様は実装しない。**2026-09-04 に検索で確認**——`crates/areka-emo-text/src/` の本番ファイル 16 本に、これらを扱う分岐が 1 つも無い（一致するのは `canvas.rs:31`・`draw.rs:144-145` の説明文だけ。`surface.rs` の 3 件は COM API 名 `create_dispatcher_queue_controller` の部分一致で無関係） | 上記各行 | 7.5 |
| あふれ判定から後戻り行が漏れる（タスク 5.3 の所見） | `LayoutEngine::visible_window`（`layout.rs:634`）は可視範囲を**最新行の遠端だけ**で判定する（`:653-654`）ため、`\_l` で行送り方向へ戻ると境界の外の行が置き去りになる（横書き・境界 36 で 4 行目の下端 49 が外に残るのに、最新行の下端 23 であふれ非発火）。最小スキップの探索（`:665`）も行が行送り方向へ単調に並ぶことを暗黙の前提にする。要件 2.8 が式の変更を禁じるため、本仕様は**今日の値の固定だけ**を行った（`crates/areka-emo-text/src/layout_cursor_overflow_tests.rs:113-166`＝値が変わると赤になる） | `areka-P0-text-decoration-canon`（同 brief `:75`＝追加登記 4 へ 2026-09-04 に登記）。根拠＝同 brief `:33` が `\f[align]`／`\f[valign]` の寄せ（＝行の置き場所）と縦書き写像を In に持ち、`:65` が「cursor-tag と `layout.rs`／`state.rs` を共有」と宣言していること。寄せは行矩形を行送り軸の方向へも動かす（正典により縦書きの `valign` は左寄せ／右寄せ＝行送り軸）ので同じ前提に触れる。**同 spec が要件フェーズで引き受けない場合は `areka-P0-ukadoc-survey-sakura-script` の台帳へ差し戻す** | 2.8 |
| `actor.rs` に残る旧正典番号（タスク 4.1 が 6.3 へ回した裁定依頼） | `crates/areka-emo-text/src/actor.rs:238`・`:762` の説明文が「`\_l` カーソル**換算**縮退（**6.5**）」と旧正典を称している。`6.5` は退役した完了仕様 `areka-P0-choice-render` の番号（本仕様では 5.2／5.3）で、「換算」も旧語（現在は解決＝`cursor_tag::resolve_cursor_axis`）。**2026-09-04 に 2 行とも実在を確認** | **本仕様の Requirement 8.6 が所有している**（ソース内で正典を称する説明の一致）。欠けているのは spec ではなくタスク側の担当で、書き換えを持つタスク（1.2・1.3・2.1・4.1）の射程がいずれも `actor.rs` を含まなかった。**タスク 7.1（一括着地）で拾うか追加タスクを起こすかの裁定が要る**（`tasks.md:209`） | 8.6 |

### 旧正典を称する説明の残存確認（Requirement 8.6・2026-09-04）

`crates/` 全域（本番・テストとも）を検索し、**旧換算関数名と旧原点定義を「現在の挙動」として語る説明が 0 件**であることを確認した。書き換え自体はタスク 1.2・1.3・2.1・4.1 が所有しており、本項は確認のみを行う。

- **旧換算関数名 `cursor_to_image_px`**: 一致 1 件（`crates/areka-emo-text/src/layout_cursor_center_origin_tests.rs:609`）。「**本仕様より前の着地（現行値）**…旧実装は…（タスク 1.2 以前の `layout.rs`）」と明示的に過去のこととして書かれた履歴の記述であり、現在の挙動の説明ではない。関数の定義そのものは残っていない。
- **旧原点定義**（「validrect の左上」「文字描画範囲左上」を areka の基点として語るもの）: 一致 10 件。内訳＝⑴ **履歴の記述 3 件**（同ファイル `:608`・`:610`・`:636`。いずれも「旧実装」「ukadoc **2.8.80**」「旧正典」と明示）⑵ **取り違え候補の名札 2 件**（同 `:271`・`layout_cursor_wiring_tests.rs:183`。「そうであってはならない基点」を列挙する側）⑶ **現在も真の記述 5 件**（同 `:647`・`layout_cursor_wiring_tests.rs:57`・`:116`・`:123`・`:136`）＝未宣言バルーンでは新正典の `TextRegion::start()` が validrect の左上と一致するという事実で、`:136` は「縮退した結果 `TextRegion::start()` が validrect の左上と一致するからであって」と理由まで書いている。
- **検索機構の対照**: 同じ検索で、実在する語は `resolve_cursor_axis` 55 件・`region.start()` 28 件と一致し、存在しない語では 0 件になる。上の 0 件は検索の故障によるものではない。
