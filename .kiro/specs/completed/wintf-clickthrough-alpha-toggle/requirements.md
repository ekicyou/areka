...
### Requirement 9: リリースビルド互換・依存最小

**Objective:** システム保守者として、本機能が既存のリリース最適化・依存方針を崩さずに組み込まれてほしい。それにより出荷構成とビルドの健全性を保てる。

> **スコープ注記（32bit 検証の除外・開発者指示 2026-07-02）**: 本機構は areka／wintf 本体（**x64＋arm64 ネイティブ**）の UI スレッド上で動作する。i686（32bit）ターゲットは x86 SHIORI 駆動の helper のみに隔離されるトラックであり、本機構は helper に含まれない。したがって旧 AC「32bit ターゲットの可搬性を崩さない」（i686 ビルド完走を検証条件とする記述）は本坑スコープ外として削除する。steering の全体制約「32bit 可搬性を崩さない」は host-32／shiori-abi トラックに対して有効であり、本除外はそれを変更しない。

#### Acceptance Criteria

1. The click-through 機構 shall 既存のリリース最適化設定（`opt-level='z'`, `lto=true`）でビルド可能かつ動作する。
2. The click-through 機構 shall 新規の大型クレートを追加せず、依存追加は最小限に留める（追加が必要な場合は提案として依頼者へ確認し独断追加しない）。
