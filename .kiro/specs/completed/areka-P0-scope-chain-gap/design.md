...
- 位置の追従・保存・復元の実装変更（`follow.rs`・`persist.rs` は無改変。persist の restore ログは**観測にのみ**用いる）。
  - **要件 7 による改訂（2026-08-13）**: `spawn.rs` は**改変対象へ移した**——連鎖の再解決には「spawn 時の既定位置」が要り、その保持先を `ScopeWindows` へ足す必要があるため（requirements.md の Boundary Context も同日に「初期配置の確定点」を In scope へ追加済み）。`follow` は無改変のまま（`move_window_to` を**呼ぶ**だけで実装には触れない）。7.3⑶（復元位置の除外）は `main.rs` の**復元合成シーム**（`restore_merged_placements` の呼び手側）を変更した——`persist.rs` 本体（merge 純関数・保存・restore ログ）が無改変であることは変わらない。
...
- **確定経路は P4 クランプを経由しない** → `windowposition-limit`（W6.5）。`finalize_chain_once` が `move_window_to` でキャラ窓 X を直接書くため、「キャラ窓は必ず work area 内」という構造的保証が確定後配置では失われる。limit のクランプをどの書き込み口へ掛けるかは同 spec の設計判断。roadmap 干渉台帳の「resolver 同一関数内 30 行差」も不完全になった。
  - **受容の記録**（設計検証 Round 2 指摘 3・2026-08-13）: 本仕様では緩和せず**受容**する。ここに独自の左端見送りを足すと第 3 の半端なクランプ実装が生まれ、wpl の設計判断を先取りするため。発生条件＝確定時に先頭スコープが配置時より**広い面**へ差し替わって左へ寄る、または後続の実表示幅の合計が先頭の X を超える（多スコープ連鎖・狭い work area）。emo2（2 スコープ・work area 2880px）では発生しない。
...
| 7.4 | 確定後のサーフェス切替では再解決しない（横滑り防止） | C6: 確定済みフラグで二度目以降を no-op。**確定が一度も起きない場合の可観測性**はタスク 6.5 の一発診断（`ChainFinalizeStall`＋`ChainDeferReason`）が担う——「走らなかった」と「走って移動 0 だった」をログから判別できる |
