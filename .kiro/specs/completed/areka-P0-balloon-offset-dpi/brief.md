...
> **📌 2026-08-27 棚卸⑪（W6.9 完走後の申し送り・本ブロックと下の棚卸⑩ブロックが以下の全追記より優先）**:
> - **着手ゲート全開**——W6.95（pwc ∥ bod ∥ zsp ∥ bvc の 4 本並走）として着手可。cage 完了（PR#119・08-27）により**檻は最初から一本化済み共有ハーネスで書ける**: ログ捕捉は `log-capture-kit`（`placement/test_support.rs` は委譲 shim 化済み・2026-08-27 実測確認）・一時パスは `temp-path-kit`。旧台帳 bod⇄cage の import 追随リスクは消滅。
> - **アンカー実測再確認（2026-08-27・ドリフトなし）**: U4 doc＝`follow/window_move.rs:24`・`windowposition.rs` 単位混在 doc＝:191（本文 :215）。
> - 同居 4 本との干渉は roadmap 台帳（pwc⇄bod・zsp⇄bod）参照——**zsp が `enqueue_window_set_pos` funnel を触らないことを design の不変条件とする**ため、本 spec が同関数署名を変える場合は zsp と design 時に相互確認のこと。

> **📌 2026-08-22 棚卸⑩（W6.75 完走後の申し送り・本ブロックが以下の全追記より優先）**:
> - **着手ゲート全開**: atom は 2026-08-22 に完走・`/kiro-complete` 済み（PR#114・`.kiro/specs/completed/areka-P0-dpi-transition-atomicity/`）。van・atom の両ゲートが充足。**【同日 追記(81) で改訂】旧「次ウェーブ＝本 spec 単独（W6.8）」は開発者指示（優先度低・並行数最大化）で撤回**——配置は **W6.95・pwc と 2 本並走**（cage の**後**＝一本化済み共有ハーネスで檻を書ける利得・pwc とはファイル素、pwc が B-4 を採る場合のみ `mount.rs` 配置契約で意味論近接＝design 時に相互照合）。**e2e より後ろへは送れない**（適合 #1「DPI 検証は追従込み」の前提）。cage 着地後は capture ヘルパの import 先を共有 crate の実形へ合わせること。
