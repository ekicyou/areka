...
>
> **📌 2026-08-27・設計フェーズで 13 件すべて決着済み。回答の一覧は下の §10、根拠は `design.md` の Key Decisions（D1〜D17）にある。本節は選択肢の記録として残す。**
...
14. ~~**保存形式の「版」という語の意味**（Requirement 5.1）~~ ——**要件ディスカッションで解決済み（2026-08-27・カテゴリ A）**: Requirement 5.1 の文言を「キー・値ごとの版を導入しない（ファイル全体の既存 `format-version` は対象外）」へ精密化した。設計フェーズへの持ち越しなし。
...
# 設計フェーズの調査記録（2026-08-27）

> 種別＝**Extension（既存システムへの結合中心）**。ギャップ分析が静的読み取りを完了させていたため、設計フェーズの調査は⑴ 上の分析が「不明」「衝突」と記した箇所の逐語確認と、⑵ 分析が扱っていなかった制約の掘り出し、の 2 点に絞った。外部依存の新規追加はゼロゆえ Web 調査は行っていない。

## 9. 設計フェーズが新たに確定させた事実（すべて 2026-08-27 実測）

### 9.1 遷移比は既存の公開面だけで組める——`ScaleRatio` の判断見直しは**不要**

- `ScaleRatio::new(num: u32, den: u32) -> Option<ScaleRatio>`（`crates/areka-emo-compose/src/scale.rs:76`）は**公開**であり、gcd 約分済みの正準形を返す（`new(120, 96) == new(5, 4)`・doc `:73-75`）。
- ゆえに**表示 DPI の整数 2 つから遷移比を直接組める**。2 つの `ScaleRatio` から比を作る必要が最初から無い。
- **先例が既にある**: `crates/areka/src/emo2_boot/frame_dpi_reproject_tests.rs:443` が `ScaleRatio::new(to_dpi.into(), from_dpi.into())` で再射影比を組んでいる——「比は生の DPI 整数から組む」は本プロジェクトの既存の書き方である。
- したがって §7 判断 2（`ScaleRatio` へ比／逆数の API を足すか）は **「足さない・足す必要が無い」** で決着する。`scale.rs:256-260` の記述を覆す必要も、`scale-exact-rational` の裁定領域へ触れる必要も無い。
- なお同記述の逐語は「本 spec が `scale.rs` へ追加する公開面は本メソッドのみである（`num`／`den` アクセサは**新設しない**——W6.5 が計画する `ratio()` 等との名前二重化を避けるため）」であり、**恒久の禁止ではなく当該 spec の範囲宣言**だった。上のギャップ分析が「明文の拒否」と読んだのはやや強い読みである（結論は変わらない——どちらにせよ触らない）。

### 9.2 遷移比では作者基準 DPI が**約分で消える**——要件 4.4 の残差の見積りは訂正が要る

- 拡大率の定義は `k_axis(d) = app_scale × (d ÷ author_dpi_axis)`（`areka-emo-present/src/scale.rs:146-149` の `policy.app_scale.mul(ScaleRatio::new(dpi_x, author_dpi))`）。
- ゆえに遷移の比は `k_axis(d₁) ÷ k_axis(d₀) = d₁ ÷ d₀` となり、**シェル軸とバルーン軸で同一**になる。
- **上の §2.5 が示した残差の見積り `((k_shell比 − k_balloon比) × balloon_w) ÷ 2` は成立しない**——2 本の比は遷移では常に等しいためである。差が出るのは拡大率の**絶対値**であって、遷移の**比**ではない。
- したがって要件 4.4 の「どちらの拡大率を用いるか」は**どちらでもない（表示 DPI 比）**が答えであり、揃えに残るのは丸め残差だけになる。残差の出所は⑴ `char_w`／`balloon_w` が作者寸から `scale_len` で個別に丸められること（各 ≤0.5px）⑵ 中点の整数除算（<1px）⑶ 追随の `scale_signed` の丸め（≤0.5px）の 3 つで、表示 DPI 比 2 倍でも合計 3px を超えない。

### 9.3 `BalloonFollow.offset` への非テスト書込は **3 か所**（4 か所ではない）

| # | 位置 | 種別 |
|---|---|---|
| 1 | `placement/spawn.rs:482-485` | 構造体リテラル構築 |
| 2 | `placement/follow/drag_follow.rs:534-537` | `follow.offset = ...`（バルーン単独ドラッグ） |
| 3 | `placement/follow/keyword_base.rs:142-145` | `f.offset = new_offset;`（キーワード再導出） |

- 上の §2.6 が 4 番目に数えた**保存値の復元は独立の書込口ではない**——`persist::merge_scope` は `ScopePlacement.balloon_offset` を差し替えるだけで、実際の Component 生成は 1 番の構築を通る。設計はこれを「構築口の入力違い」として扱える。
- 1 は欄が増えれば必ずコンパイルエラーになる。2 と 3 は**ならない**——ここが基準対方式の唯一の実装上の危険であり、設計は `offset` の私有化で型の問題へ変換した（design D14）。

### 9.4 現行資産の実測——軸の選択は今日は無影響／欠陥 B には実在の運び手がいる

- **`seriko.dpi` の宣言はリポジトリ内の `*.txt` に 1 件も無い**。バルーン側の `dpi,` 宣言も 0 件。よって現行のすべてのフィクスチャで shell 軸＝balloon 軸であり、**`balloon.offsetx` の軸をどちらに決めても今日の出力は変わらない**（＝軸の決定は契約の決定であって挙動の変更ではない）。要件 7.7 の「作者基準 DPI が異なる場合」はフィクスチャでは踏めず、純関数の檻でしか踏めない。
- **`balloon.offsetx` を実際に宣言している資産が木の中にある**: `vendors/pasta/crates/pasta_sample_ghost/ghosts/hello-pasta/shell/master/descript.txt:7-10`（および `vendors/pasta/release/hello-pasta/...` の同内容）が `sakura.balloon.offsetx,64` ／ `sakura.balloon.offsety,0` ／ `kero.balloon.offsetx,64` ／ `kero.balloon.offsety,0` を持つ。**欠陥 B は「宣言するゴーストが来れば顕在化する」潜在ではなく、既に運び手が同じ木にいる**。要件 2 の実機検証には emo2 ではなくこの資産を使う。
- 宣言位置が `shell/master/descript.txt` であることは、語彙の出所がシェル作者の空間であるという読み（design D2＝シェル軸）を実物で裏づける。

### 9.5 分量規律が設計の形を縛る（ギャップ分析が扱っていなかった制約）

- 行数の番人は `crates/log-capture-kit/tests/file_length_guard_test.rs` にあり、**実測した超過ファイルの集合が例外表 `OVER_LIMIT_ALLOWED`（11 件）と完全に一致すること**を要求する（`OVER_LIMIT_ALLOWED_COUNT` との逐語一致も別に検査する）。
- `crates/areka/src/placement/follow/window_move.rs`（1,228 行）は既に例外表に載っている。一方、**新たに 1,000 行を超えるファイルを 1 つでも作ると番人が赤になり、例外表の編集が要る**——要件 9.6 がそれを禁じている。
- ゆえに⑴ 新規コードは新規ファイルへ置く、⑵ `window_move.rs` への変更は**行数を増やさない呼び替えに留める**、⑶ 判定器を `transition_judge.rs`（929 行）／`transition_judge_verdict.rs`（863 行）へ足さず新設する、が設計の制約になる（design D17）。

### 9.6 適用相の実形——追随は待ち札の関門に自動的に従う

- `run_dpi_phase`（`emo2_boot/frame/dpi.rs:474`）は Bevy system ではなく `&mut World` の排他関数で、`Emo2Wiring` から `presenter` と `dpi_state` を取り出して `dpi_phase_with` へ委ねる。**`EmoPresenter` は到達可能**（`frame/wiring.rs:49`）だが、design D5 の結論により本仕様は使わない。
- `dpi_phase_with` は 2 巡構成で、**第 1 巡が全窓の待ち札の関門（`apply_dpi_phase_gate`）を先に通し**、通過した窓だけを第 2 巡が処理する（`:284-311`／`:313-372`）。第 2 巡の先頭に追随を置けば、追随も自動的に同じ関門に従う（見送り中の窓を追い越さない）。
- 見送られた窓は次回以降 `With<DpiSyncHold>` の合流（`:274-282`）で対象へ戻るため、追随が失われない。
- `refresh_scale_report` の呼出は `:335`。バルーンの扱い（`Some` 腕の `resize_window_keep_position`＝`:159`／`None` 腕の no-op＝`:369`）はすべてその**後**にある。
- `None` 腕でもキャラ窓は `reproject_char_window_at_current_size`（`:414`）→ `resize_window_to` を通るため、**両腕とも手順 6 の `follow_balloon` に到達する**。
- `resize_window_to` の手順 4（べき等 skip・`window_move.rs:337-345`）は **`return false`** で抜ける。手順 6 に到達しないこの腕の存在と、戻り値で判別できることが design D16（収束の保証）の実装可能性を担保する。

### 9.7 `follow_balloon` は私有再束縛——公開面の格上げが要る

- `placement/follow.rs` は `use self::drag_follow::{BalloonFollowTrigger, follow_balloon};` と**私有**で再束縛している（`pub use` ではない）。frame 層から呼ぶには `pub(crate) use` への格上げが要る。crate 内に留めるため外部公開面は増えない。

### 9.8 `COMPAT:172` は陳腐化しない——相互確認は不要

- 同行（`windowposition-limit` 所有）の主張は「キーワードが定める基本位置に対して、調整量の加算を従来どおり継続する（`windowposition.y` の数値と `balloon.offsetx`／`offsety` を基本位置へ加算する。数値指定時とまったく同じ扱い）」である。
- 本仕様は**加算をやめない**。`windowposition` 数値と同じく拡大率を掛けてから加算するので、「数値指定時とまったく同じ扱い」はむしろ強まる。
- ゆえに §6 申し送り 5（自分の行で上書きするか／所有者へ相互確認するか）は **「どちらも不要」** で決着する。本仕様は自らの単位空間行で軸の割り当てを明示的に補うのみ。

---

## 10. §7 の設計判断項目への回答（13 件すべて確定）

| §7 | 項目 | 決定 | design の対応 |
|---|---|---|---|
| 1 | 単位空間の選択 | 「現在の表示 DPI における物理 px」（表現据置き）＋基準対を Component へ持たせる | D1 |
| 2 | `balloon.offsetx`／`offsety` の軸 | **シェル軸**（語彙の出所＝シェル `descript.txt`・既存裁定 `mod.rs:395-397` と同じ論法の延長） | D2 |
| 3 | 換算の適用点 | **供給層**（`apply_scope_windowpositions` の直前）。`config.rs` の純粋転記契約に触れない | D3 |
| 4 | 遷移の変換規則 | **基準対から毎回引き直す**（前回の結果を入力にしない）＝往復無誤差が構造的に成立 | D4 |
| 5 | 旧拡大率の入手 | **入手しない**。基準 DPI と現在 DPI から比を組む。`applied_ratio` は使わない | D5 |
| 6 | 追随の挿入位置 | frame 層・`refresh_scale_report` の**前**。`resize_window_to`／`enqueue_window_set_pos` の署名は 1 つも変えない | D6 |
| 7 | キーワード再導出との排他 | **素材の有無で分岐**（`keyword_base.rs:71-78` の明文の設計判断を反転させない） | D7 |
| 8 | 残差の許容量 | 軸不一致の残差は生じない（§9.2）。丸め残差のみで **1 軸 ≤3px**（比 1/2〜2）。実値も檻で固定 | D8 |
| 9 | 縮退時の警告水準 | 本仕様の経路だけが警告。`applied_ratio` を使わないため既存 donor と接点が無く**非対称が生じない** | D9 |
| 10 | 記録の出し方 | `transition_diag` へ種別 `offset` を 1 つ追加（既定 OFF）。常時 `info!` は足さない | D10 |
| 11 | 互換記録の行数 | **3 行**。他仕様の行は書き換えない。`COMPAT:172` は陳腐化しない（§9.8） | D11 |
| 12 | `\![move]` 経路の扱い | 変更なし（表現据置きの利得）。`.offset` → `.offset()` の呼び替えのみ | D12 |
| 13 | 既存の遷移テスト 2 本 | **主張を書き換える**。書込前読みの構造と空振り防止の証人 3 つは保つ | D13 |
| 14 | ~~保存形式の「版」の語~~ | 要件ディスカッションで解決済み（持ち越しなし） | — |

§6 の Research Needed への対応: 1（SSP オラクル）＝実装フェーズの実機セッションへ・両腕の判断木を design が確定 ／ 2（`ScaleRatio` の API）＝**不要**（§9.1） ／ 3（軸と残差）＝§9.2・§9.4 で確定 ／ 4（拡大率の渡し方）＝D5・D6 で「渡さない」に決着 ／ 5（`COMPAT:172`）＝§9.8 で不要 ／ 6（既存テストの反転範囲）＝D13 と Testing Strategy の全数区分。

---

## 11. 統合（Synthesis）の結果

**一般化**: 要件 2（供給時の換算）と要件 3（遷移時の追随）は「作者が書いた量を、いま表示している拡大率の物理 px へ写す」という**同一の問題の 2 つの時点**である。設計はこれを 1 つのモジュール（`follow/offset_space.rs`）に集約し、供給時は `scale_author_offset`・遷移時は `rescale_follow_offset` という 2 つの入口を与えた。インタフェースは一般化したが、実装は現在の要件が求める範囲に留めている（第 3 の入口を用意しない）。

**Build vs. Adopt**: 新規に作ったものは基準対の概念と 2 本の純関数だけである。丸め（`ScaleRatio::scale_len`）・符号保存と飽和（`scale_signed`）・比の構築（`ScaleRatio::new`）・観測チャネル（`transition_diag`）・判定器の形・テストハーネス（`log-capture-kit`／`FrameHarness`）はすべて既存資産を採用した。**新しい丸め規約も新しい公開 API も 1 つも作らない**ことが、この spec の最大の設計目標である。

**単純化**: 検討して**捨てた**もの——⑴ `ScaleRatio` への比／逆数 API（§9.1 により不要）⑵ 旧拡大率を保持する新 Component／Resource（基準 DPI が基準対の中にあるため不要）⑶ 作者空間で offset を保持する案（下流の確定済み契約への波及が最大・保存値との単位食い違い）⑷ 追随のための新しい `PlacementRoute` 語（追随は窓書込ではなく Component 書込なので経路語を持たない）⑸ 保存形式への基準 DPI の記録（要件 5.1 が禁じ、D15 の未係留がその代替として機能する）。

---

## 12. 本書が保証しないこと（設計フェーズ時点）

- SSP の拡大率跨ぎ挙動（要件 6.1〜6.3）と実機サインオフ（要件 8）は未実施。design が手順と両腕の判断木を確定させただけである。
- design D8 の残差上限 3px は**式からの見積り**であって実測ではない。決定論テストが DPI 行列で実値を固定した時点で、上限が妥当かを再評価すること（実値が上限に迫るなら、上限ではなく設計の側を疑う）。
- 上の §2.8 の (a)〜(d) 区分は 2026-08-27 の実測だが、同居 3 本（pwc・zsp・bvc）が同じ木を触るため、**タスク実装の直前に `origin/main` へ再突合すること**。

---

# 実装フェーズの調査記録

## 13. SSP 拡大率跨ぎ観測の結果（2026-08-27 実測・task 1.1・要件 6.1〜6.3）

§12 が「未実施」と記していた観測を実施した。**採った腕は B**（参照挙動が存在しない ⇒ areka 裁量）。

**観測条件**: SSP `C:\wintools\ssp\ssp.exe`（profile 空＝初回起動）・ゴースト えも2DEBUG・
DISPLAY1 primary dpi=192 と DISPLAY2 dpi=144 の 2 台。採取は先行仕様の
`completed/areka-P0-scope-chain-gap/tools/measure-ssp-rects.ps1`（Per-Monitor v2 aware・読み取りのみ）。
キャラ窓を合成マウス入力でモニタ間へドラッグ往復させ、各段でバルーンを開かせて窓矩形を実測した。
逐語の証跡は `ssp-dpi-crossing-observation-2026-08-27.log`。

**実測値**（追従オフセット ＝ balloon(L,T) − char(L,T)・むらさき scope）:

| 段 | 表示 DPI | char (L,T,W,H) | balloon (L,T,W,H) | 追従オフセット |
|---|---|---|---|---|
| 1 起動直後 | 192 | 2181, 610, 764x1094 | 1901, 366, 800x448 | **(−280, −244)** |
| 3 DISPLAY2 へ移送後 | 144 | −1582, 701, **764x1094** | −1862, 457, **800x448** | **(−280, −244)** |
| 4 DISPLAY1 へ戻した後 | 192 | 2018, 610, 764x1094 | 1738, 366, 800x448 | **(−280, −244)** |

**確定した 2 事実**:

1. **SSP は拡大率遷移で追従オフセットを再スケールしない**。192 → 144 → 192 の往復で 3 段とも bit 同一。
2. **SSP は拡大率遷移で窓の寸法も変えない**。起動時の主モニタ DPI で焼いた物理寸
   （キャラ 764x1094 ＝ 素材 382x547 の k=2・バルーン 800x448）を別 DPI のモニタへ移しても据え置く。
   ＝ SSP には「拡大率遷移」という状態遷移そのものが存在しない。

**腕の選択の根拠**: 2 により、SSP には移植できる「拡大率跨ぎの追従規則」が無い。1 の「何もしない」は
規則ではなく 2 の帰結である。areka は表示経路が拡大率に追従する設計（`areka-dpi-following`）を採っており、
追従しない実装の非規則をそのまま採ると設計原則と両立しない——要件 6.3 の但し書きに該当する。
よって design.md の D1〜D5 をそのまま採り、task 9 で互換記録へ 4 欄で登記する。
**design.md の「腕 B が濃厚である根拠」および Open Questions の危険「SSP 観測が腕 A になる」は、
本観測で解消した（変換規則の見直しは不要・行列テストの期待値は design どおりでよい）。**

**既登記 2 例との整合（要件 6.4）**: 本件は SSP が作者空間オフセットをスケールしないことを示す 3 例目であり、
`COMPAT:146`（`windowposition.x` ＝生値 × k・実機実測）・`COMPAT:154`（`\![move]` の dx/dy ＝作者基準 px × k・
意図的な SSP 非互換）と同じ系列に並ぶ。矛盾は無い。

**付随して観測された SSP 挙動（本仕様の対象外・記録のみ）**: ドラッグした scope だけがモニタを移り、
相方 scope（エモ）は元のモニタに残った。areka の scope 連鎖とは別の挙動だが、本仕様は追従オフセットのみを扱う。
