...
- [x] 6.2 未実装メソッド命令のwarn+skipを実装する
...
- [x] 7. Integration: Composer facade の結線
...
- 7 (lib.rs/plan.rs/blit.rs): `Composer{ops:Vec<BlitOp>, visited:Vec<u32>}`（**scratchのみ・ComposedSurface結果フィールドなし＝no-cache 9.4**）。`compose_into(&mut self,out,world,atlas,sid,binds)->Result<(),ComposeError>`＝毎フレーム経路（`build_plan(&mut self.ops,&mut self.visited,...)`→`execute(out,extent,&self.ops,atlas)`）、`compose(...)->Result<ComposedSurface,_>`＝fresh割当便宜（compose_intoへ委譲）。**build_plan/derive_ops/compute_extent が visited を `&mut Vec<u32>` scratch 引数化**（各traversal entryで clear・祖先スタックpop-on-exitで空復帰・extent→ops逐次実行ゆえ共有安全）＝定常ゼロアロケーション（ptr+ops.capacity安定をテスト実証）。**ホットパス resolve-free**（compose/build_plan/execute は `atlas.entry` O(1)のみ・`.resolve(` は atlas_bind.rs=構築時のみ・静的テスト`hot_path_sources_do_not_call_atlas_resolve`で担保）。thread/async/channel/tokio 皆無・同期値返し。build_plan/execute から `#[allow(dead_code)]` 除去（Composerが消費）。composer_tests.rs=11本（emo2的multilayer end-to-end含む）。**残clippy**: flatten_surface/flatten_extent の8引数 too_many_arguments（private walker・build_plan facade化後も残存＝将来scratch構造体化余地）。
- coverage-log-firing (test-only addendum・開発者「決定論的テスト網羅」原則): 要件1.4/3.3/7.3/8.4/10.5 が**ログ記録を第一級契約**として要求するのに、production の warn!/error! 発火18箇所は観測挙動(None/skip/Err)のみ検証＝発火自体が未被覆だった。**thread-local 捕捉ハーネス** `src/log_capture.rs`（`tracing::subscriber::with_default`＋自前 `Layer`・**global禁止**＝並行テスト安全・crates/areka/shiori_demo.rs 踏襲）を新設し、`src/log_firing_tests.rs` で全要件必須ログを檻化: 7.3 ops循環(flatten_surface)/7.3 extent循環(flatten_extent・"外形算出"前置で**両path判別**＝ops檻がextent warnで空虚合格しない罠を回避)/8.4 未実装method/1.4 頁欠落/3.3 未解決alias/10.5 SurfaceNotFound&EmptyComposition error!/1.4+2.1 重複id/1.4+2.2 非存在append target/4.3 未束縛element。各テストは **LEVEL(WARN/ERROR)＋弁別field＋観測副作用** を pin。Part3: ComposeError Display厳密一致・blit uv_rect.x/y≠0（6.1レビュー指摘の直接被覆漏れを解消）。from_name別名は既存test被覆済。`tracing-subscriber` は `[dev-dependencies]{workspace=true}`（既存workspace基盤dep・出荷closure不変＝12.2遵守）。production .rs 無変更。**135 tests green**（119+16）・parallel/single-thread両方で非flake・各production log行コメントアウトで対応テストがRED（独立再現済）。
