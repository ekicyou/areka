...
> - **W4 emo-dpi-scaling は本 spec の席を意図的に保全して着地済み**: `measure_scope_sizes` は2段化（`measure_native_scope_sizes` measure.rs:153 native 採寸＋`apply_scaling` k 適用）され、**measure.rs:127-128／:227-229 に「`balloon_size` が scope 別値になり得る席を潰さない（要件 7.8・kero-balloon（W5）申し送り）」の明示コメント実在**＝per-scope 採寸化に構造変更は不要。さらに frame.rs:545 に `wiring.balloon_models.insert(scope, balloon_model.clone())` の **per-scope モデル記憶マップが導入済み**（足場半分完成——現状は全 scope 同一 model を挿しているだけ）。
> - **新配線面（brief 未記載・本 spec が引き受ける義務）**: W4 が `refresh_actor_binding`（emo-text actor.rs:348）＋文字層 k 再追従 `run_text_scale_phase`（frame.rs:928）を新設し、**attach と再追従が同一 ActorKey 写像を要求**（frame.rs:552-554 に警告コメント）。per-scope model 化の際は再追従経路（`balloon_models` マップ消費側）も同時に scope 別 model へ差し替えること。追記㊾の「`refresh_actor_binding` の binding 比較の穴」も本 spec 担当。
> - **行アンカー再補正（2026-07-31 実測）**: `measure_scope_sizes` :62-131 → **:136-147**・balloons0 ハードコード＝`measure_balloon_surface0` **:371-409**（固定 :402/:409・「全スコープ共通」採寸 :179-180）・テスト定数 :353/:382 → **:534/:543/:564**＋k 端到端 **:1147**。frame.rs 無条件 ShowSurface :444-453 → **:531-540**・`connect_balloon_text` 呼出 :456-462 → **:549-556**・fn 定義 :483-502 → **:577-596**。assets.rs balloon ループ :258-262 → **:278-284**・`build_balloon_model` :263 → **:285**・LoopTables :269-278 → **:291-300**。emo-present balloon.rs `FRAME_PREFIX` :39・`frame_id` :88・balloonk 除外テスト :264・`BALLOON_FACE0_TXT` assets.rs:79 は**不変**。
> - **W6 への申し送り2本**: `balloon-visibility`（frame.rs 同一関数域の後着再突合・㊹どおり）に加え、**`bindoption-exclusivity`（追記(52)で W6 編成）が assets.rs:196-210（BindResolver 構築）を触る＝本 spec の assets.rs 先行着地後に相手が rebase**（異ハンク :278-300 vs :196-210）。
> - rebase 条項（gpu-test-crash・spine.rs 新規 GPU world テスト時のみ owner-thread 委譲）・原点=下端中央基準は roadmap W5 行が正本。
