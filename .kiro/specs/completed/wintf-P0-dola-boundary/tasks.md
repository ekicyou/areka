...
- [x] 1. wintf から DolaRuntime 誤配置コードを除去する

- [x] 1.1 (P) cue モジュールの DolaRuntime 関連コードとシステムを削除する
...
- [x] 1.2 (P) 既存の DolaRuntime 統合テストファイルを分割・整理する
...
- [x] 2. (P) dola に演出コマンド・ドメイン型を実装する

- [x] 2.1 (P) バリア種別・ルーティングコマンド・演出コマンド・統合ペイロード型を実装する
...
- [x] 3. (P) DolaRuntime に tick() と last_result() の 2 フェーズ API を追加する
...
- [x] 4. dola に TimedSchedule&lt;T&gt; を実装する

- [x] 4.1 エントリの型定義と挿入 API を実装する
...
- [x] 4.2 tick() と ready() の 2 フェーズ API を実装する
...
- [x] 4.3 バリア管理とルーティング収集 API を実装する
...
- [x] 5. dola に CueSheet と compile_sheet を実装する

- [x] 5.1 CueSheet 型と Actor フィルタリング API を実装する
...
- [x] 6. dola cue モジュールを統合・公開してユニットテストを実装する

- [x] 6.1 lib.rs に cue モジュールを公開して主要型を再エクスポートする
...
- [x] 6.2 (P) TimedSchedule のユニットテストを実装する
...
- [x] 6.3 (P) CueSheet と compile_sheet のテストを実装する
...
- [x] 6.4 (P) DolaRuntime tick/last_result のテストを実装する
...
- [x] 7. wintf cue の型定義を dola 再エクスポートに置き換える

- [x] 7.1 cue/command.rs を再エクスポートのみのファイルに置き換える
  - wintf cue/command.rs の全型定義を削除し、dola cue から一括再エクスポートする形に書き換える
  - cue/mod.rs から不要なインポート・宣言が残っていないことを確認する
  - _Requirements: 3.5_

- [x] 7.2 コンパイルエラーを解消して後方互換を確認する
...
- [x] 8. CueQueue を TimedSchedule&lt;CueCommand&gt; 委譲設計に再設計する

- [x] 8.1 CueQueue に TimedSchedule&lt;CueCommand&gt; を内包した push/pop API を再設計する
...
- [x] 8.2 新 CueSheet 投入時の全破棄・再構築フローを実装する
...
- [x] 8.3 エンティティ参照の変換を push/pop 境界に実装する
...
- [x] 8.4 CueQueue + TimedSchedule の統合フローをテストする
...
- [x] 9. wintf に DolaAnimator ECS Component を実装する

- [x] 9.1 DolaAnimator Component と tick_dola_animators System を実装する
...
- [x] 9.2 DolaAnimator の ECS 統合テストを実装する
