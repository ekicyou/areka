...
- [x] 1. find_owner_window ユーティリティ実装
- [x] 1.1 ChildOf 逆走査ロジックの実装
...
- [x] 1.2 find_owner_window のユニットテスト
...
- [x] 2. (P) build_bubble_path の Window 境界停止修正
- [x] 2.1 (P) Window コンポーネント検出ロジック追加
...
- [x] 2.2* (P) build_bubble_path テストケース追加
...
- [x] 3. (P) WM_MOUSELEAVE のウィンドウスコーピング修正
- [x] 3.1 (P) PointerState クリアのスコープフィルタ追加
...
- [x] 3.2 (P) thread_local バッファのスコープ付きクリア
...
- [x] 4. (P) WM_MOUSEMOVE の leave 処理スコーピング修正
- [x] 4.1 (P) collect_entities_to_leave ヘルパー関数実装
...
- [x] 4.2 (P) WM_MOUSEMOVE ハンドラの2箇所修正
...
- [x] 5. (P) ドラッグ終了の HWND ガード追加
- [x] 5.1 (P) handle_button_message の HWND 検証ロジック追加
...
- [x] 6. (P) taffy_flex_demo のマルチウィンドウ化
- [x] 6.1 (P) create_flexbox_window のパラメータ化
...
- [x] 7. (P) マルチウィンドウ統合テストの実装
- [x] 7.1 (P) test_find_owner_window ユニットテスト
  - Task 1.2 で作成済みの場合はスキップ
  - Pure ECS テストで find_owner_window の基本動作を検証
  - _Requirements: 5.1_

- [x] 7.2 (P) test_mouseleave_scoped_pointer_clear 統合テスト
...
- [x] 7.3 (P) test_build_bubble_path_stops_at_window 統合テスト
  - Task 2.2 で作成済みの場合はスキップ
  - build_bubble_path が Window で停止することを検証
  - _Requirements: 5.1_

- [x] 7.4 (P) test_drag_state_hwnd_guard 統合テスト
  - Task 5.2 で作成済みの場合はスキップ
  - DragState HWND ガードの動作を検証
  - _Requirements: 5.3_

- [x] 7.5 (P) tests/multiwindow_event_test.rs ファイル作成
  - 上記テストを1つのファイルにまとめる
