...
- **跨ビットネス HWND/メッセージ規約（documented・枯れた領域）**: SendMessage/PostMessage は win32k のサービスで HWND はシステム全域有効＝x64↔x86 成立。USER/GDI ハンドルは **32bit に収まる値**であることが MS 保証（"Interprocess Communication Between 32-bit and 64-bit Applications"）→ **HWND は 32bit 表現で受け渡す**（x86=32bit／x64 は上位 0 の 64bit 値として扱い、x64→x86 は truncate 安全）。WM_COPYDATA は OS が `COPYDATASTRUCT`＋`lpData` を跨ビットネス marshal（各側 native レイアウト・手動 struct 越境しない）。`dwData`(ULONG_PTR) は 32bit 受信側で低 32bit のみ保証→タグは ≤32bit・実データはバイトバッファへ。**WM_COPYDATA は SendMessage 専用**（PostMessage 不可）。ポインタは載せない。少数の精密ルールを守るのみ＝低リスク。先進坑で 1 往復を実走観測すること自体が確証（go 基準(1) の一部）。
