...
## 10. 設計フェーズの記録（2026-08-27・`design.md` 生成）

> 上の §1〜§9 は要件ディスカッション**前**のギャップ分析である。§9 の 10 項目は要件討議で全て消化された（裁定の記録 4・5＝**見送り＋登記**）。本節は設計フェーズが行った探索・合成・裁定の記録であり、**§1〜§9 の結論を上書きしない**。

### 10.1 探索の範囲（Discovery: minimal）

裁定が「見送り＋登記」で確定済みであり、設計は実装アーキテクチャを持たない。したがって全面探索も統合探索も行わず、**登記の裏取りに必要な実測のみ**を行った（外部調査・subagent 展開は不要と判断）。

| # | 確認したこと | 結果（2026-08-27 実測・HEAD `a6d27c73`） |
|---|---|---|
| ⒜ | 可視化の段のアンカー（Requirement 9.5） | `apply_show` `show.rs:46`／`set_visible` `:375`／`set_bounds` `:381`／`Visualize` 発行 `:392`＝**現存・ドリフトなし** |
| ⒝ | 判定量の飽和（§5-②）の現物 | `crates/areka/src/placement/transition_judge.rs:817` の `saturating_sub`＝現存。合否束は `transition_judge_verdict.rs:169` の `Bounds::signoff()`＝2 量とも armed のまま |
| ⒞ | `t_us` の起点（§0-2） | `crates/wintf/src/ecs/window/transition_diag.rs:692` `since_tick_start_us`／`:703` `stamp`＝現存 |
| ⒟ | steering の追随（Requirement 8.4） | `roadmap.md` ゴール表 `:67`・W6.95 行 `:82`・干渉台帳 `:89`（pwc⇄bod の要ウォッチは B-4 却下で**解消**と登記）＝**追随済み** |
| ⒠ | 本 spec 名のソース参照 | `crates/` `doc/` 全域で **0 件**＝完了時のアーカイブ移動でコードからの参照が壊れる経路は無い（記憶 `kiro-complete-archive-breaks-code-to-spec-doc-reads` の型に該当しない） |
| ⒡ | 完了ゲートのコマンド | `cargo test --workspace`（steering `workflow.md:35`） |

> §6 の末尾で参考記録した doc コメント内の陳腐化 spec パス 2 件（`transition_signoff_procedure_tests.rs:3`・`transition_judge_reobservation_tests.rs:13`）は**現存**を再確認した。実行に影響せず（実パス定数 `:41` は是正済み）、本 spec はコード非接触を証跡とするため触らない——`design.md` の Out of Boundary に観測事実として登記した。

### 10.2 合成（3 観点）

- **一般化**: 本 spec の受入基準のうち実装を要するもの（要件 2.1-2.5・5.2-5.6・7.1-7.3）は**すべて採用時条項**であり、見送りの裁定により一斉に不適用となる。共通化すべき能力は残らない。一般化の対象は「未達をどう見えるようにするか」の側にあり、それは先行 `draw-load-parity` の登記形式（要件文書の改訂欄＋引受先の明示）を**そのまま踏襲**することで足りる。新しい様式を作らない。
- **作るか採るか**: 作るものが無い。登記・検証・手渡しはいずれも既存の仕組み（要件文書の改訂欄・`cargo test --workspace`・`research.md`）で足り、新規の道具・依存・観測点をいずれも導入しない。
- **簡約**: 設計から落としたもの——⑴ B-3／B-3′ の実装アーキテクチャ（保留の器・commit の入口・wintf の flush 後フック）⑵ B-4 の一時オフセットと解除点の設計 ⑶ 提示側観測点の設計 ⑷ 合否量の分離（§5-③）の機械的な表し方——**⑷ は合否判定そのものを行わないため問題が実体化しない**。残したのは登記・検証・手渡しの 3 つだけである。

### 10.3 設計判断

#### Decision: 設計を「縮小設計」に限定する

- **Context**: 要件 1.5 の裁定が見送りで確定しており、Requirement 8.4 が「設計文書まで同じ裁定を追随させる」ことを求める。
- **代替案**: ⑴ 通常の設計を書き B-4 を「将来の実装案」として設計しておく ⑵ 設計を書かずに要件のみで閉じる ⑶ 縮小設計（採用）。
- **選択と理由**: ⑴ は却下された候補の実装設計を残すことになり、後から読む者が「設計はある＝あとは実装だけ」と誤読する危険がある（B-4 は合否量に効かないので、その誤読は 3 つの壊し方を無償で持ち込む）。⑵ は 8.4 の追随義務に反する。⑶ は裁定・却下理由・不適用要件・残余検証・手渡しに絞り、**作らないものを設計しない**。
- **Trade-offs**: 将来の再着手時に設計を一から起こす必要があるが、その素材は §1〜§8 が保持している。むしろ「却下された案の設計」を残さないことが誤読の予防になる。

#### Decision: 完了条件を要件 7.7 の 1 本に絞る

- **Context**: 是正が無いため合否判定（要件 3）も新規決定論テスト（要件 7.1-7.6）も発生しない。
- **選択**: 完了条件＝`cargo test --workspace` 全通過（7.7）＋コード非接触の証跡＋アンカー再確認（9.5）。実機サインオフは課さない。
- **理由**: 実行時挙動を変更しないため実機で再確認すべき差分が無い（記憶 `areka-evidence-classes-static-equals-real-machine`）。起点実測 `atom-73-signoff-1` が未達の証拠として既に保全されている。

#### Decision: 要件 4.6 の読みを設計側で明示する

- **Context**: 4.6 は完了宣言に実機採取と決定論テストの双方を要求するが、要件 3 は見送りにより不適用である。文言をそのまま読むと両者が矛盾する。
- **選択**: 4.6 を「**是正の合格を宣言する場合の**要求」と読む。本 spec は合格を宣言せず「未達 40 件の登記」として閉じるため、片方の結果で合格と読み替える行為が起きない。
- **理由**: `requirements.md` の「見送りにより不適用となる要件」が要件 3 を不適用と定めた以上、4.6 を文言どおり適用すると完了が原理的に不可能になる。要件 8.5（未達を残す完了は合格と読める書き方をしない）と整合する読みはこの 1 つだけである。**この読みは `design.md`「完了報告の規律」に明記した**（papering over ではなく明示的な解釈として登記）。

### 10.4 危険と対処

| 危険 | 対処 |
|---|---|
| 「テスト全緑」が未達の解消と読まれる | 完了報告で 7.7 の充足と 2.1 の未達を同じ段落に並べない（`design.md` 完了報告の規律） |
| 既存のサインオフ判定が違反を報告し続けることを「退行」と読まれる | 正常な状態であることを設計・要件の双方に登記（要件 8.3） |
| 将来の再着手が判定量の飽和を知らずに着手し、直っていないのに PASS を得る | `design.md`「将来仕様への手渡し」の**罠①**として明示（§5-② が根拠） |
| 将来の再着手が `t_us` をプロセス起点と誤読し規模を 1 桁誤る | 同**罠②**（§0-2 が根拠） |
| 並走 3 spec の着地でアンカーがずれ、登記が実体と食い違う | 完了時に V3（アンカー再確認）を再実行。ずれが Revalidation Triggers の第 1・2 行に当たるなら見送りの根拠が失効するので開発者へ差し戻す |
