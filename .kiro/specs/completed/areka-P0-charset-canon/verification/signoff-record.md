...
里々側の目視は**同日の開発者裁定で要件 11.1 から外した**——今日の areka ではこの検体で立ち絵も
バルーンも出ず、見えるものが無いからである（障害は 8 節、裁定も 8 節）。**未実施のまま残る欄は無い。**
...
- emo2 は本物のゴースト窓が 4 枚生え、バルーンへ文字が流れた（装着の記録あり）。
  **emo2 の目視は開発者が実施し、合格した**（2026-09-13・気になる点なし・7 節）。
- **里々標準テンプレートは窓が生えない。** 本仕様とは無関係の別の欠落
  （シェル定義ファイルが基準画像を明示せず、ファイル名の慣習に頼っている）で採寸が失敗し、
  検証用ダミー窓へ退避する。**この検体では挨拶の目視ができない**（8 節）。**要件 11.1 の
  目視部分は 2026-09-13 の開発者裁定で外した**。絵を出す仕事は `areka-P0-shell-implicit-surface` が引き受ける（8 節の裁定）。
...
| 検体 | 目視で見るもの | 結果 |
|---|---|---|
| ⑴ 里々標準テンプレート | ——（**目視は求めない**。2026-09-13 の開発者裁定で要件 11.1 から外した） | **対象外**。今日の areka では立ち絵もバルーンも出ず、見えるものが無い（8 節）。絵を出す仕事は `areka-P0-shell-implicit-surface` が引き受ける |
...
当初はここに 3 択（⑴ 暗黙の基準画像を実装する spec を起こして目視を預ける／⑵ 検体を
替える／⑶ 目視の対象を emo2 だけにする）を並べ、完了ゲートで決めることにしていた。
**開発者は ⑶ ＋ ⑴ を採った。**

- **要件 11.1 の目視部分を外した。** この検体に求めるのは「文字コードの経路が最後まで
  正常に働いたことをログで示す」ことまでとする。要件・設計・タスクの本文を同日に
  訂正済み。
- **本仕様の目視は emo2 で行う**（7 節 ⑵・合格）。
- **絵が出ない件は独立した spec として起票した**——`areka-P0-shell-implicit-surface`
  （`.kiro/specs/areka-P0-shell-implicit-surface/brief.md`・roadmap の W14 ⑧）。
  検体を替える案 ⑵ は採らない。検体は 2026-09-11 の要件ディスカッションで開発者が
  確定したものであり、替えれば「里々の標準テンプレートが動く」という目標そのものが
  遠のくからである。

**残る宿題の在処**: 新しい spec は「絵」を、本仕様は「文字」を受け持つ。里々の標準
テンプレートが人の目に正しく映るのは、**両方が着地したとき**である。新しい spec は
その実機目視を自分の完了条件として持つ。
