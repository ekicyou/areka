...
ここでの「崩れが 0」は**反映待ちを除いて**数える（開発者裁定・2026-09-24）。反映待ちとは、アプリが当たり判定を新しくしてから OS がその絵を画面に出すまでの画面更新 1 回ぶんのフレーム（絵は古く当たり判定だけが新しい）で、差し替え方に依らず、本番のバルーン番号の切り替えでも同じように出る。反映待ちは混在の内訳として別に数え、本番のバルーン番号の切り替えで出た数を床とする。「崩れが 0」は「反映待ち以外の崩れが 0、かつ反映待ちの数が床と同数以下」を指す。

バルーンの切り替えは、資産の差し替えに限らず、さくらスクリプトのバルーン番号の切り替え（`\b[ID]`）で日常的に起こり、どちらもシームレスに動く必要がある（開発者裁定・2026-09-24）。窓の大きさを絵と揃える処理は両者に共通であり、案 A を選んでも要る。
...
- **In scope**:
  ...
  - バルーン番号の切り替え（`\b[ID]` 相当・同じバルーンの中の大きさの違う面への切り替え）を同じ観測で数えること。
  - 有界な自動終了。
...
6. The pilot example shall バルーン資産の差し替えとは別に、同じバルーンの中で大きさの違う面への切り替え（さくらスクリプトのバルーン番号の切り替え `\b[ID]` に当たる、同じ表示先への面の表示の指令。`StayseeBalloon` の `balloons0.png` 335×205 と `balloons2.png` 335×395 の間）を少なくとも 1 往復行い、差し替えと同じ観測で数える。
7. The README shall バルーン番号の切り替えの数を、資産の差し替えの版とは分けて示し、1 以上の崩れが出たときは本番で既に通っている経路の既存の欠陥として学びに記録する（先進坑の中では直さない）。
...
#### Acceptance Criteria
1. When 差し替えを要求したとき, the pilot example shall 要求の直前のフレームから、新しい資産の絵と当たり判定が揃った最初のフレームの後さらに一定数のフレーム（数は設計で決め、README に書く）まで、途切れなく各フレームを観測する。ここで「1 フレーム」は OS が画面を 1 回更新することを指す。
2. The pilot example shall 各フレームで画面に出ている絵を、入力として渡した値やアプリの中の状態からではなく、OS が更新した実際の画面を取り込んで判別する。当たり判定は、その画面の更新の直前にアプリが済ませたフレームの時点のものと突き合わせる。
3. The pilot example shall 各フレームで、表示されている絵がどちらのバルーンのもの（一方・他方・両方・どちらでもない）か、および効いている当たり判定がどちらのバルーンのもの（一方・他方・両方・どちらでもない。「両方」は 2 つのバルーンのマスクが同時に効いている状態）かを判別し、当たり判定も渡した値からではなく実際に効いている判定に問い合わせて判別する。ここで「実際に効いている当たり判定」は、クリック透過を決めるのと同じ wintf の判定（`wintf::ecs::hit_test_in_window`）に、2 つのバルーンの α の形が食い違う標本点を問い合わせた結果を指す。OS の窓のクリック透過の付け外し（カーソルの動きで動き、フレームと揃わない）と当たり判定領域の名前（バルーンは持たない）は対象に含めない。
4. The pilot example shall 観測したフレームを次の 4 種の崩れで数え、1 つのフレームが複数の種類に当たるときはそれぞれに数える——「混在」（絵と当たり判定が別のバルーンのもの、絵と当たり判定の片方だけがどちらでもない、絵に両方のバルーンが同時に見えている、または当たり判定に両方のバルーンのマスクが同時に効いている）・「空」（窓が消えている、または何も描かれていない）・「古い絵の残り」（新しい資産の絵と当たり判定が揃った最初のフレームより後に、古いバルーンの絵が少しでも見えている）・「大きさの食い違い」（窓の大きさと表示中の絵の大きさが合わず、絵が切れて見える、または窓の中でずれて見える）。
5. When 1 回の差し替えの観測が終わったとき, the pilot example shall その差し替えについて観測したフレームの数と、4 種の崩れそれぞれの数を、崩れが無い種類も 0 と明示してログへ出す。混在については、絵と当たり判定が揃う前のフレームで絵が古く当たり判定だけが新しいもの（反映待ち）の数を内訳として併記する。
6. If 2 つのバルーンの絵または当たり判定を観測の方法で見分けられないとき、画面の更新を取りこぼしたとき、または窓が他の物に覆われていたとき, the pilot example shall 該当する数を 0 とせず「測れない」と報告する。
...
5. When わざと「大きさの食い違い」にしたフレーム（窓の大きさの合わせ直しを 1 回飛ばしたもの）を数えたとき, the pilot example shall それを「大きさの食い違い」として 1 以上数える。
6. When 差し替えを行わず同じ資産を表示し続けたフレームの並びを数えたとき, the pilot example shall 4 種の崩れをいずれも 0 と数える。
7. If 較正のいずれかの項が期待と異なる数を出したとき, the pilot example shall 較正の失敗をログへ出し、その実行で得た本番の崩れの数を有効な結果として扱わない。
8. The pilot example shall 較正の結果を、本番の崩れの数と並べてログへ出す。
...
4. The README shall 検証結果の幕で、較正の結果・版ごとと差し替えごとの 4 種の崩れの数（0 も明示する）・観測で分かることと分からないこと（読み戻した描画結果が画面の見た目のどこまでを表すか）・学び・日付を書く。
5. The README shall 検証結果の幕に、測った数が合否基準（go／直す／違う）のどれに当たるかの見立てを添えるが、判定の欄は開発者が下すまで確定させない。見立てでは、バルーン番号の切り替えで出た反映待ちの数を床として示し、各版の反映待ち以外の崩れと、床と比べた反映待ちの数の両方で読む。
