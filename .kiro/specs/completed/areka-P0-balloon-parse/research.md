...
3. **【D3】「未指定」の表現方式**: `Option<T>` 直持ちか、NewType でラップした上での `Option` か。R2.6「既定値で埋めない」を満たす最小形は各モデル化キーを `Option` にすること。座標成分（x/y や t/b/l/r）を個別 `Option` にするか、幾何単位でまとめて `Option` にするか（fixture では validrect は 4 成分同時に現れる）。
   - **要件ディスカッション #1 決着（要件側制約）**: 未指定は **0 やゼロ値と区別可能**でなければならず, parser は既定値・0 で代替してはならない（R2.6/R3.4 改訂）。根拠は ukadoc が origin/wordwrappoint/validrect の省略時値を「不明」と明記＝権威ある既定が存在せず, 真の既定は画像サイズ依存の消費側計算であり得るため。→ 型形状（`Option` 直持ち vs NewType 内包）は design 判断だが, **「None を 0 と区別して保持」という性質は要件で確定済**。design はこの制約を満たす型を選ぶこと。
