...
- **Adjacent expectations**:
  ...
  - トークの上書き・中断規律（SSP 準拠・ukadoc 確認済み）: 既定は後出し優先（last-writer-wins）＝再生中の talk に新 talk が来ると現行を中断・上書きする。中断のトリガは複数あり得る——(1) 新 talk の scheduling、(2) ユーザーのバルーン中断（skip・ダブルクリック等・emo(UI) 由来）、(3) ゴースト切替、(4) 将来 areka が対応する場合の外部 SSTP——が、これらは全て **kanade（運行表）へ集約**され、sakura が受け取る中断入力は **kanade からの `Close` 単一**である（R7）。sakura は中断のトリガ種別を区別せず Close に従うのみ。どのトリガを honor/拒否するかの調停——上書き拒否ガード、ユーザー中断の無効化 `\![enter,nouserbreakmode]`（**通常 SSTP 不可・Owned/Auth.SSTP のみ可という source 特権付き**＝SSP には出所の特権ヒエラルキーがあり後出し優先は同格ピア間の既定にすぎない）、SSTP 送信元特権——は **kanade の scheduling 判断＋後続 M の拡張点**であり、対応する `\![...]` 群は **M-boot 外タグ＝R8 の無視＋シーム**。M-boot の talk 出所はゴースト自身の SHIORI（kanade 経由）ただ 1 つ＝単一 owned source ゆえ source 特権も user-break 調停も moot となり、全 talk が Close で上書きされ「同時に有効なトークは 1 本」＝単一 current スロットで整合する。ガード状態を sakura から kanade へ露出する結線は後続 M の拡張点とする。
