...
- **無効側で赤になるテストは自ら名乗る**。要件 3.2 の対照イベントにより、硬化なしで取りこぼした窓は黙って空を返さず失敗を宣告するので、赤の集合は決定論的で特定可能である。この性質を利用して赤の集合を確定し、**両側から同じフィルタで除外して比較する**（要件 13.2）。除外後は**両側とも全緑**でなければ比較値として採らない。
  - **2026-08-27 訂正（タスク 11.1 の実測で後半が偽と判明）**: 前半——**取りこぼした窓が黙って空を返さず失敗を宣告する**——は成立する。観測した赤はすべて「このログが 1 行あるはず」という肯定側の表明で、捕捉列が空であることを診断つきで宣告していた。**しかし「赤の集合が決定論的」は偽である。** 実測: 無効側を 8 回走らせて不変に赤なのは較正テスト 1 本だけ、他は 1/8〜5/8 で出入りし、5 本を除外して走らせたら 6 本目が新たに出た。親が独立に 3 回走らせた結果は **0 件 / 2 件 / 0 件**で、2 件のうち 1 本は 8 回の走行に一度も現れなかったテストだった。`--test-threads=1` に落とすと無効側でも赤は 0 件になる。
  - **なぜ偽だったのかは、この設計が見落としていた当たり前のこと**——無効側の赤は**毒化の競合**が起こす。それは本仕様が消そうとしている病そのものであり、**並列の巡り合わせに依存するのが病の定義**である。「病の症状を観測して除外する」という手順は、病の性質と噛み合っていなかった。
  - **したがって除外の集合は観測ではなく静的な列挙で決める**（要件 13.2 の「特定し、除外して比較する」は満たす。列挙の手段を観測から静的走査へ替えるだけである）。**共有捕捉窓を使うテストを走査で列挙し、両側から同じ集合を除外する**——`tests/with_default_guard_test.rs` の走査器が捕捉サイトを既に列挙できる。除外後は**両側とも全緑**でなければ比較値として採らない、という規律は変わらない。
  - **除外しても測定は意味を保つ。** 常駐の代償は捕捉テストの中だけでなく**ワークスペース全体に及ぶ**——probe が 2 個居ると `has_just_one` が偽になり、あらゆる `tracing` の発火点が毎回の判定を通る。捕捉テストを除いた残りにもその代償は乗っているので、差が出るならそこに出る。
