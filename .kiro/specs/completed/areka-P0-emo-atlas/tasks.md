...
- [x] 1. 基盤整備: 新規クレート雛形・WIC デコード経路の拡張・共有契約型の定義
...
- [x] 3. 統合: bake パイプラインの結線と fixture 検証
- [x] 3.1 bake エントリポイントの結線
...
- [x] 3.2 emo2 fixture を用いた統合テスト（shell・balloon 横断）
...
- 4.1 cross-env 堅牢化（最終検証で確定）: **デコード決定性は構造保証**（WIC は `Initialize(_,PBGRA,DitherNone,None,0.0,MedianCut)` で色管理/ICC/ディザ非適用＝PNG 可逆でビット一致・下流 trim/pack/bake/normalize は float/rand/時刻ゼロの整数のみ）。再現性の実残点は2つとも解消: ① `rectangle-pack = "=0.4.2"` へ厳密ピン（Cargo.lock は `.gitignore` 除外ゆえマニフェストで版固定＝別クローンで packing 版が揺れない・前例 `wintf-winmsg-executor "=0.0.5"`）。② golden 比較は `include_str!` を `.replace("\r\n","\n")` で LF 正規化（この repo は autocrlf で text を CRLF 化＝新規クローン Windows で改行差のみで誤検出する潜在バグを是正）。配置値は環境非依存で一致。
- 完了ゲートで発見・是正（COM テスト規律）: `probe_pna_false_for_emo2`（2.2）が**実行順・スレッド数依存でフレーキー**だった。原因は暗黙 MTA 依存＋壊れた fallback（未初期化のまま `new()`→失敗時に空 `with_com_initialized(||{})` で init+uninit＝差引ゼロ→未初期化で再 `new()`）。並列では他テストが MTA を保持し暗黙 MTA で通るが、単一スレッド/workspace 一括では `CO_E_NOTINITIALIZED`。**教訓: WIC/COM テストは必ず `with_com_initialized` で factory 生成〜使用まで包む**（暗黙 MTA に依存しない）。`cargo test -p <crate>` 単体では緑でも `cargo test --workspace`/`--test-threads=1` で露呈しうる＝完了ゲートで両方を確認すべし。
