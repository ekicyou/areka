...
5. **解決一致（7.3(a)・目視が証拠力の中核）**: 点は**実表示窓の client 座標経路からのみ取得する**——`GetCursorPos`（実カーソル位置・screen）→ `ScreenToClient` → 得られた client 点を `resolve_hit_region(presenter, 0, x, y)` へ渡し、結果を live にログする。操作者（人間または agent）は**画面に見えているゴーストの頭／胸／背景を目視で狙って**カーソルを動かし、「視覚上その部位に載っていること」と解決結果（`"Head"`／`"Bust"`／`None`）の一致を記録する。実測 client 座標と解決結果を物理 px で表に残す（`window-placement` acceptance-record の「実測証跡（物理 px）」列と同形）。
   ...
   - **マウス経路照合（設計討議#3・validation Issue 3 の決着）**: probe 窓へ `OnPointerMoved` ハンドラを1行装着し（`OnPointerPressed(on_shell_pressed)` の donor 前例＝`emo-present.rs:523`）、記録行ごとに**本番マウス経路**の `PointerState.client_point` と `ScreenToClient(GetCursorPos())` を**ペア列（client_x/y・s2c_x/y・Δx/Δy）**で実測表へ併記する。操作者は各記録行でカーソルを **500ms 以上静止**させる（`dispatch_pointer_events` は `PointerState` 存続中は毎 tick 発火＝静止サンプルが必ず取れる・`dispatch/mod.rs:221-229`）。記録行の要求は **Δ=(0,0) の厳密一致**（両者とも同一 HWND の client 物理 px 整数＝丸め誤差の正当源が無く許容誤差は設けない）。静止で Δ≠0 の場合は 1px 動かして再静止を1回行い（クリック透過 poll 12ms の過渡 race 対策）、なお持続すれば FAIL として記録する。**ペア列は Head/Bust（不透明画素）行のみ**——背景 None 行はクリック透過（`WS_EX_TRANSPARENT`＋`HTTRANSPARENT`）によりイベント自体が窓へ届かず、ペア欠測が正しい挙動である（C-3 整合・その旨を表の脚注に明記）。この照合はクエリ系 API 経路と WM_MOUSEMOVE lparam 経路が **OS 内部でのみ合流する非トートロジー検査**であり、per-monitor v2 混在 DPI 環境での awareness 経路間不一致・wintf ポインタ配管の座標歪み（f32→i32 往復）を捕捉する。狙点は引き続き目視由来（反トートロジー条件は不変）。
...
- Risks — probe の証明範囲（設計討議#2/#3で確定）: **証明する**——(a) 表示側恒等契約と、それを本番で維持する反映経路（`resize_window_to`→SetWindowPos→`GetClientRect`）の単位保存性（実 DPI 2水準・数値 assert）、(b) マウス経路の**空間**一致（`client_point` ≡ `ScreenToClient`・静止記録行）。**証明しない**——(1) `frame.rs` の resnap 検知ループ（scope 走査・寸法差分判定）＝completed `areka-P0-surface-resize-resnap` が所有（決定論部は headless 檻・実機は 2026-07-14 目視サインオフ〔単一 DPI 水準・数値記録なし〕。**本番 resize 経路の数値 k=1.0 実測は本 probe が初**）。(2) 窓**生成**経路（spawn 初期寸）の実 DPI 数値正しさ＝completed `areka-P0-window-placement` acceptance-record が所有。(3) イベント配送〜SHIORI 一周＝撫でクラスタ合流サインオフ（7.4）が所有。
...
- **C-4（`input-events` Req8.3 へ・設計討議#3で範囲精緻化）**: 撫で一周の統合実機サインオフは合流サインオフが1回で実施（7.4）。マウス経路の**空間**一致（`PointerState.client_point` ≡ `ScreenToClient(GetCursorPos)`・窓 client 物理 px）は**本 spec のプロトコル 5. 静止記録行が確認する**ため、合流サインオフに残るのは**イベント配送〜SHIORI 一周**（`OnPointerMoved`→resolver→Reference4→応答 talk）と balloon スコープの検証である。申し送り: `dispatch_pointer_events` は `PointerState` 存続中、カーソル静止でも**毎 tick** `OnPointerMoved` を発火する（ホバー状態配信・`dispatch/mod.rs:221-229`）——W2 の間引き設計はこの発火特性を前提にすること。
...
1. **含端規則（C2）が正典の裏付けを持たない**: 閉区間はリポジトリ内前例（wintf）と「囲まれた範囲」の読みに基づく決定であり、SSP 実挙動との突合はしていない。**机上での突合は不能と実査で確定**（2026-07-17 設計討議#4: SSP ソースは非公開——作者 GitHub・ukatech org・公式サイト・コミュニティ一覧・GitHub 全域コード検索の5系統が独立に一致。「SSP はオープンソース化された」はフォークロア。互換クローン ninix-kagari は crossing-number 法＝閉/半開いずれとも不一致で先例にならない）。将来の唯一の実証経路は**実 SSP バイナリへの境界1pxブラックボックス突合**（境界プローブ用ゴーストで OnMouseMove Reference4 を観測）で、実施するなら撫でクラスタ合流サインオフ（7.4）と同席が自然。影響範囲は境界1px＝撫で体験に実害はほぼ無く、変更点を純関数の比較式1箇所に閉じ、境界檻で固定することで是正コストを最小化する（Revalidation Trigger 3）。
2. **画家則の SSP 逸脱が e2e で検出されない**（C-5）: 検出手段が無いこと自体を記録として残す以上の緩和策は無い（意図的逸脱ゆえ）。
3. **probe の証跡範囲は設計討議#2/#3で拡張・精緻化済み**（C-4）: 表示側の単位保存性（本番 resize 経路・数値 assert）とマウス経路の空間一致（静止記録行）は本 spec が検証し、合流サインオフに残るのはイベント配送〜SHIORI 一周と balloon スコープのみ。分担を要件本文（7.3/7.4）と本節で二重に明示し、「双方が相手に任せて誰も検証しない」最悪形を封じる。
