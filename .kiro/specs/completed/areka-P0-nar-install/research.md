...
- 38 ファイルの内訳（付録 A-1 を再実行・一致）: `areka` 13・`areka-emo-text` 7・`pilot` 4（`shiori-host-32/{helper,helper_window,main,shiori_proxy}.rs`＝i686 の helper を含む）・`areka-emo-atlas` 3・`areka-emo-compose` 3・`areka-parsers` 3・`areka-seriko` 3・`areka-emo-present` 1・`areka-ghost` 1。`emo2-kakukaku` を実行行で綴る 27 のうち 38 に無いのは 4 で、パスを組むのは `areka/examples/emo-present/setup.rs:147`（`emo2("emo2-kakukaku")`）だけ（同じ形は 38 の内側の `areka-emo-atlas/src/emo2_e2e.rs:215` にもあり、見張りの 4 形目 `emo2("emo2-kakukaku")` で両方を拾う＝設計検証の指摘で追加）。残り 3（`balloon_model_tests.rs:10`・`balloon_target_tests.rs:434`・`state_cue_apply_tests.rs:608`）は説明文の中の綴り。
