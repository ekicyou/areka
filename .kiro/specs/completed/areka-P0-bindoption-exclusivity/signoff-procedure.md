...
- **条件A**: `|Changed 回数(まばたき) − Changed 回数(目)| <= 4`
- **条件B**: 最後のまばたき Changed が最後の目 Changed の **120 秒以内**

> **条件A の閾値は 2026-08-11 の開発者裁定で 2 → 4 へ改訂済み**（design.md §Real Machine Sign-off の J2 に経緯を記録）。初版の `2` は**是正が正しく効いても赤になる**——本節 §7 の申し送りのとおり、保全ログから排他置換を再現した想定形が まばたき 22 / 目 25＝**差 3** になるため。想定値 3 に余裕 1 を足して 4 とした。是正前の実測は**差 22**なので、緩和後も飽和との桁差は保たれる。
...
== J2（要件 5.4・飽和パターンの不在）==
  ...
  末尾時刻差: 316.848 秒
  条件A（回数差 <= 4）: FAIL / 条件B（末尾 120 秒以内）: FAIL
  → J2: FAIL
```

（上記は**閾値改訂後**（条件A `<= 4`）に再実行した実測。差 22 は改訂後も余裕で FAIL するため、
既知ケース較正は緩和後も維持されている。改訂前の `<= 2` でも同じ FAIL だった。）
...
（発火 1400×97 / 1402×22 / 1403×50 が全て直近痕跡と一致）。J2 は まばたき Changed 22 /
目 Changed 25＝**差 3** で、**閾値改訂後（`<= 4`）は PASS**（改訂前の `<= 2` では FAIL——
これが 2026-08-11 の閾値裁定を招いた観測。§7 参照）。
...
- **【解決済み・2026-08-11 開発者裁定】J2 条件A の閾値は 2 → 4 へ改訂した。** 以下は改訂に至った観測の記録である（判定値そのものは §4.3 が正）。保全ログから合成した想定形では
  ...
  実走の並びによって数回起こり得る。**裁定は「想定値 3 に余裕 1 を足して 4」**。
  是正前の実測は差 22・末尾時刻差 316.8 秒なので、緩和後も飽和との桁差（22 vs 4）は
  保たれ検出力は落ちない。**なお改訂後の 4 を超えて FAIL した場合**は、
  **飽和（片側が恒久沈黙）とは別物**の可能性が残るため、条件B（末尾時刻差）と J1 の結果、
  およびまばたき Changed が実走末尾まで継続しているかを併せて見て、改めて開発者裁定に
  かけること。
