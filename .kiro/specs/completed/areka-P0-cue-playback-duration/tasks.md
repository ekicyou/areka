...
- [x] 2. Foundation: cue envelope の拡張（duration・Wait・ClearAll・配送エンベロープ）
...
- [x] 3. Foundation: 自己完結した絶対時刻台本と占有終了判定
...
- [x] 6. Core: 演者が duration honor 契約と出力契約を実装する
...
- [x] 7. Integration: さくらスクリプト前段を front-end と talk アクター glue へ縮小する
...
- [x] 8. Integration: 実配線（kanade→dispatcher→ランタイム→演者）と旧世代撤去
...
- [x] 10. Validation: 実機受け入れ（人間サインオフ）
...
- **Task 10 人間サインオフ成立（2026-07-17・開発者承認）**: 実 emo2 ゴースト（`crates/pilot/examples/shiori-host-32/fixtures/emo2`）＋実 pasta.dll（i386）＋実 DPI＋**絶対パス**起動（R8.5）で実走。`target/debug/areka.exe <abs ghost_root> <abs balloon_root>`＋`RUST_LOG=info`・自動 close 無し。**落とし穴**: `cargo build/test --workspace` が **x64 版 `shiori-host32-helper.exe` を `target/debug/` へ置く**ため、実走前に i686 版（`target/i686-pc-windows-msvc/debug/`）を areka.exe 隣へ**上書きコピー必須**（x64 helper では 32bit pasta.dll を LoadLibrary 不可）。観測記録: wire 成立＋attach planned=2/attached=2（両スコープ実表示装着）、**6 talk 実行**（起動1＋ランダム5＝#6 の talk 遷移 5 回）、各 steady talk が `steady_talk`→**4〜5 秒後**に `steady_talk_done`（＝TalkDone が占有 horizon まで待つ＝早期終了しない傍証）、窓 close→`unload_clean`（helper exit(0)）→ghost shutdown→**exit 0**。**開発者判定: 「タイミング関係は治っているように見える」＝#3・#4・#6・`\s` 同期の解消を実機で確認しサインオフ**（R8.1–8.5 充足）。**開発者所見（本 spec 対象外・申し送り）**: 「タイミング以外の改善が別途必要」——本 spec は時間の権威のみを担い、描画/レイアウト・bind/mayuna 合成（#2）・実行時サーフェスリサイズ（#1）等は Non-Goals ゆえ隣接 spec（`mayuna-compose`／`surface-resize-resnap` 等）の領分。roadmap 側で継続。**既知の非致命 ERROR（本 spec 起因でない）**: 起動グリーティングで `kanade: 未知 talk_id の再生完了通知（unknown_talk_done talk_id=1）`——kanade が boot greeting を撃ちっぱなしで 0.65ms 後に boot 系列完了を宣言し talk_id を追跡しないため、TalkDone 到達時に slot 不一致となる既存挙動（horizon 化で TalkDone が遅くなっても結末は同じ・「現 Phase 維持で継続」で非致命）。kanade 領分。
