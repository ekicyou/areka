...
## 2026-09-18 追記（A0 へ前倒し・第 1 スライス）

- 開発者「メニューは最初に実施できないか」→ 実測: 本 spec が触る `crates/areka/src/input_events/`・wintf のポインタ経路・新規 `menu.rs` には検体パスの参照が 0＝`nar-install` の書き換え対象（検体参照のある既存テスト・example 38 ファイル）と共有 0。**A0 で `nar-install` と並走**。
- **A0 で着地させる第 1 スライス**: 右クリック → Win32 標準メニュー・「説明書」（`readme` を既定アプリで開く）・「終了」（既存の `OnClose` 握手へ）・`*button.caption` リソース・`popupmenu.visible`・登記式 `MenuRegistry`。
- ゴースト／シェル／バルーンのサブメニュー（列挙は `baseware-root-layout`＝A1-②・切替は `ghost-shell-balloon-switch`＝A2）と「インストール…」「ネットワーク更新」（A4）は、それぞれの spec が `MenuRegistry` へ登記する。本 spec は登記の口（ラベル・有効／無効・サブメニュー・選択時の閉包）を要件で確定し、決定論テストは「登記した項目が正しい位置に現れる」で口を検証する。
- 実機確認は現行の argv 起動（emo2）で足りる（根・列挙に依存しない）。
