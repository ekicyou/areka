...
3. **DD-3 移行手段の位置づけ — 裁定済み（要件ディスカッション #2）**: 機械変換（B-1）＋同値検証を**使い捨ての一回限りスクリプト（Python 等で可・恒久資産化しない）として本仕様スコープ内**で実施すると確定。恒久的な下流ツーリング（Rust codegen 等）はスコープ外のまま。無損失の証拠は当該プログラムによる変換前後の同値検証に一本化（requirements.md Requirement 9.5・In/Out scope 改訂済み）。削除（R7.2）は検証合格後。
4. **DD-4 Q1（残置 vs 削除）— 裁定済み（要件ディスカッション #1）**: **C-2（削除・オンデマンド結合）に確定**。現 consumer ゼロ＋将来も単一ファイル入力を使わない旨を開発者が確認したため、C-1 残置（banner・再生成運用 R-N3 を含む）は不採用。requirements.md Requirement 7 が削除を規定済み。
