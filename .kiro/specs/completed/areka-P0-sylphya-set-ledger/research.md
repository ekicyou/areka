...
## 10. 設計ディスカッションでの処理（2026-09-13）

設計検証（`design-validation.md`・GO 条件付き）の重大 2 件と非重大 5 件を精査した結果、**開発者に諮る議題（カテゴリ B）は 0 件**。すべて字面の矛盾・規約の取り落とし・検査の形の不備であり、答えで作業が変わるものは無かった。カテゴリ A として次を修正した。

| # | 元の指摘 | 修正 |
|---|---|---|
| A1 | 要件 2.4「ファイルを変更しない」が 3.6／9.1／9.2 と矛盾（要件ディスカッションで 9.2 だけ改訂し 2.4 が取り残された） | 要件 2.4 を 9.2 に揃える（名簿・写し・較正行に限り判定 3 本の本文を変えない）。設計の整合節と対応表に 2.4 を併記 |
| A2 | 要件 7.2「例外なく変えない」が 1.3（4 名の分類変化）と字面で矛盾 | 要件 7.2 を「登記 4 名以外のあらゆる名前」へ改め、4 名の変化は 1.3 の唯一の変化として完了報告に開示すると明記。設計 Impact にも追随 |
| A3 | 「21 の写し 0 件」の grep が設計自身の「既存 21」と T7 の名前で必ず赤／「5 本」の写しは 8 箇所あるのに 1 箇所しか挙げていない | 探し語を「現在の件数を表す形」5 形に絞り較正（25 版 ≥ 5 件）を添える。T7 は名前から数を外しスライスで書く。「5 本」8 箇所＋「8 本」1 箇所を列挙し全部 6／9 へ（doc は判定本文でないので 9.2 に触れない） |
| A4 | 兄弟テストのモジュール名が `structure.md` 規約（`mod <モジュール名>;`）に不一致 | `mod set_ledger_tests;`（ファイル名 `dotted_set_ledger_tests.rs` は据え置き） |
| A5 | T5 の `include_str!` が `actor.rs` だけで兄弟テスト 3 本を列挙していない（`structure.md` の規律） | `actor_tests.rs`・`actor_actor_integration_tests.rs`・`actor_actor_criteria_cage.rs` も読み 4 本で参照 0 |
| A6 | 要件 5.3 が `/// ukadoc:` を例示するが配列要素の直上では `//` しか置けない | 要件 5.3 を「`ukadoc: <URL>` 1 語の単独行・行頭記号は置き場所に応じて `///`／`//`」へ改める（先例 `placement/config.rs`） |
| A7 | 担当欄の実数え `'^owner = ""$'` は CRLF で道具により 0（恒真）になり得る | `\r\?$` の形と使うシェル（Git Bash）を設計に明記 |

要件 2.4／5.3／7.2 の改訂は既存の裁定（名簿登記は t_zpd12 の指示どおりの正規対応・4 名の分類変化は本 spec の目的そのもの）の字面への追随であり、steering に追随すべき新しい裁定は無い。
