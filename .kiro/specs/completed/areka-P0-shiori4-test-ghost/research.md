...
- **D-1（DLL パス解決）[討議#3 で方針確定・機構は設計]**: **決着＝手動プリビルド段を要さない完全自己完結**（要件1.2/5.4）。x64 テストDLL は**同一ネイティブ target のワークスペースメンバ**ゆえ `cargo test --workspace` が自動ビルドする（i686 testdll のクロスビルド手動プリビルドとは非対称——[[workspace-test-needs-i686-host32-artifacts]] の摩擦を持ち込まない）。**残る設計詳細**＝(1) e2e が build 済み cdylib（`target/<profile>/`）を locate する手段（`build.rs` で OUT_DIR 起点に target パス解決／実行時探索）、(2) `InProc` に渡すのは明示 `dll_path` か `mount.shiori.dir.join(file)`＋コピーか。※`cargo test --workspace` はメンバのテストビルド時にそのメンバの lib(cdylib) をビルドするため native cdylib は自動生成されるが、`build.rs` 依存関係で locate 確実性を担保する設計が望ましい。
