...
| # | 決定 | 代替案 | 採った理由 | 追認点 |
|---|---|---|---|---|
| DD-1 | 1 周を相単位の状態機械にし台帳を正本にする。**1 ターンでは背景コマンドを起動する相まで続けて進める**（2026-08-22 議題 3 裁定・節約の対象は LLM 推論トークン・測定マシンの忙しさは制約にしない） | 1 ターンで 1 周を完走／1 ターン 1 相 | `/goal` は背景作業でターンを切る。要約・再開に強い | 相の遷移表を決定論テストで固定 |
| DD-2 | 判定材料を STATUS 行 1 本に集約し、条件文はその字面で書く | 判定役にファイルを読ませる | 判定役はファイルを読まない（公式） | 条件文テンプレートの字面とスクリプトの出力を同じ定数から出す |
| DD-3 | 旗方式の tick 門＋心拍＋起動直後の全走。**部品は無条件に実装し、周 1 の A/B で既定 ON/OFF を決める**（2026-08-22 議題 2 裁定＝設計どおり作って ON/OFF で様子を見る） | スケジュール単位の省略（案 B）／起床間引き（案 D） | 上限が最大・順序不変を保ちやすい・3.2 と両立 | 旗を立てる生産者の一覧を字面検査で固定 |
| DD-4 | スレッド別 CPU は実行体の内側で採る——**自前のスレッド名簿（生成点で役割名を登録）＋`GetThreadTimes`**。名簿外（bevy `TaskPool`）は `unregistered_rest` 1 行。昇格時は段③のサンプリングから TID 別を併記（**2026-08-22 設計ディスカッション議題 1 裁定＝(a)**・ToolHelp は feature 無し＝`Cargo.toml` 非接触と両立しないため不採用） | ETW の CSWITCH 集計／ToolHelp 列挙（feature 追加）／段③からのみ導出 | 名前付きで決定論的に出る・道具の導入無し・`Cargo.toml` 非接触 | `GetThreadTimes` の粒度（約 15.6ms）を README に注記 |
