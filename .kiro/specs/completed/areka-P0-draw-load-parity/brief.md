...
> **📌 2026-08-22 棚卸⑩（⚠ 本 brief の全性能数値は Bevy 0.19/Taffy 更新より前の実測）**: main に **Bevy 0.19 への更新（`bf2d7950`・2026-08-19）が spec 外の直接コミットとして着地**し、実行器そのものが改稿された（`ExecutorKind` API は撤去済み——atom が檻 5 本を移行した際「`get_executor_kind` に後継が無い」ことを確認済み）。本 brief の中心数値（`try_tick_world` 1 tick 578µs・壁時計 6.85%・13 スケジュール・FrameFinalize 182µs/Draw 143µs 等）は**すべて更新前（2026-08-15 以前）の実測**である。要件段階の再計測は元々必達だったが、**実行器改稿を跨ぐため傾向すら持ち越せない前提**で臨むこと——スケジュール数・tick コストの内訳・「120回/秒 全走」という構造自体を再確認してから要件を立てる。SSP 比較（10.97% 対 3.05%）の areka 側も再採取が要る。**【同日 追記(81)＝棚卸⑩補】本 spec は W8 から W6.9 へ前倒しされ cage と 2 本並走**（開発者指示＝並行数最大化。「M1 完成を妨げない」位置づけと「大改造不要なら」の裁定は不変）。**同居裁定: `command.rs` は丸ごと本 spec 所有・cage は非接触**——`SELF_INITIATED_DEPTH` の `Cell<i32>` 化（:49・追記(74)⑹「flush へ手を入れるついでに片づくなら安い」）も本 spec が実施し、着地形を cage へ申し送る（見送る場合はその旨を cage へ即報告＝差し戻し先は cage）。cage の共有 leaf crate が wintf の Cargo.toml dev-deps へ 1 行足す見込み＝本 spec は Cargo.toml 非接触を保つと衝突ゼロ。
