...
- [x] 1. handlers.rs をファイル別に分割してウィンドウプロシージャモジュールを整理する
- [x] 1.1 ライフサイクルとディスプレイ変更ハンドラを lifecycle.rs に切り出す
...
- [x] 1.2 マウス移動・ヒットテスト・離脱ハンドラを mouse_move.rs に切り出す
...
- [x] 1.3 ボタン・ホイールハンドラを mouse_button.rs に切り出す
...
- [x] 1.4 キーボード・ドラッグ制御ハンドラを keyboard.rs に切り出す
...
- [x] 1.5 残った位置・DPI ハンドラをもとに window_pos.rs を完成させ handlers.rs を削除する
...
- [x] 2. DPI 変更時の中心保持補正ロジックを window_pos.rs に実装する

- [x] 2.1 (P) BoxStyle 論理サイズから新 DPI スケールの物理サイズを算出するロジックを実装する
...
- [x] 2.2 (P) 旧物理サイズと新物理サイズから中心保持補正量を算出するロジックを実装する
  - `window_pos.rs` 内に純粋関数として実装する
...
- [x] 2.3 DPI 変更時の補正エントリポイントを実装して WM_WINDOWPOSCHANGED ハンドラに統合する
...
- [x] 3. 補正ロジックのユニットテストを実装する

- [x] 3.1 (P) 中心保持補正量算出のユニットテストを実装する
...
- [x] 3.2 (P) BoxStyle 物理サイズ変換のユニットテストを実装する
  - `window_pos.rs` の `#[cfg(test)]` ブロックに追加する
...
- [x] 3.3 SetWindowPosGuard カウンタ方式の動作を確認する（先行実装済み）
