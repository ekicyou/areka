...
|---|---|---|
| `FRAME_INTERVAL_MAX_WAIT_MS` | 150（emo2 `animation1400` のコマ待ち 0→150→22 の**最大値**）。判定式⑴の上限の土台 | 第 1 段実測・§A6 の裁定（`baseline-2026-08-14.md` §5）。**旧 `FRAME_INTERVAL_EXPECTED_MS = 172` はこの行に置き換わった**——172 はコマ間隔ではなくサイクル全長であり、上限に使うと 197.8ms までの鈍化を合格にした |
| `FRAME_INTERVAL_CYCLE_SPAN_MS` | 172（同アニメの**サイクル全長**＝コマ待ちの和）。判定窓の条件 2 に使い、**上限には使わない** | 同上（§A6） |
| `FRAME_INTERVAL_TOLERANCE` | 許容率（例 15%・第 1 段で較正） | R4.2⑴ |
| `FRAME_INTERVAL_IDLE_MAX_ACTIVE` | 1（判定窓の条件 4＝区間両端の推定活性アニメ本数の上限。**数えるのは判定対象系列に対応する `scope` のアニメだけ**） | §A6-1（remediation 1）＋§A6-2（remediation 2）。系列鍵はアニメ単位ではないので、同一鍵に 2 本のアニメが重なると隣接差はどちらのコマ間隔でもなくなる。**位相しだいで偽の合格へ倒れる**（fixture `P13_same_key_two_animations_pass_side`）。ただし別 `scope` は別の系列鍵なので数えない——合算すると上限超過の区間を別 `scope` が覆っただけで不合格が合格へ反転する（fixture `P14_other_scope_animation_overlap`） |
| `FRAME_INTERVAL_SERIES_SCOPE` | `TargetId(0)/surface_id=1000 → scope="0"`（emo2 固有） | 系列と発火ログを結ぶ唯一の表（§A6） |
| `FRAME_INTERVAL_JUDGED_SERIES` | `TargetId(0) × surface_id=1000` の明示指定（**all-or-nothing**） | §A6。自動選別からの切替 |
| `FRAME_INTERVAL_MIN_SAMPLES` | 20（明示指定では不合格は返すが合格は返さない下限） | §A6。dev short が 22 本まで減るため 30 から引き下げ |
| `FRAME_INTERVAL_WINDOW` | 判定式⑴の測定窓と**判定粒度**の定義: `target_id`／`surface_id` で系列を分離し、発火起点の窓 C（条件 1〜4・§A6 で確定）に限定する。条件 4 が**同一 `scope` 内の**複数アニメ重畳の区間を落とす（`FRAME_INTERVAL_IDLE_MAX_ACTIVE`・別 `scope` は数えない＝§A6-2）。**判定は分離した系列ごとに行い、判定対象の系列が 1 つでも上限を超えたら⑴は不合格とする**。同一 target の複数系列をまとめた p95 は参考表示にとどめ、判定には使わない——速い系列の 5% の裾に遅い系列が丸ごと吸収され、スロー再生が合格に化けるため。どの系列を判定対象とするかと窓の境界条件は第 1 段ベースラインで確定し README に登記 | R4.2⑴/R4.5（validation Issue 2 の取り込み・重畳 apply の混入による偽陽性/偽陰性の排除。validation Issue 2 の「対象 target 限定」は**判定対象の絞り込み**＝`FRAME_INTERVAL_JUDGED_SERIES` の意であり、target 単位への再併合ではない） |
...
- 確定した窓: 「`seriko: loop 抽選発火` を起点とし、⑴起点から `CYCLE_SPAN_MS`（=172）以内に始まり、
  ⑵区間内に次の発火を含まず、⑶区間の**両端**で、**その系列に対応する `scope` の**推定活性アニメ本数が
  `FRAME_INTERVAL_IDLE_MAX_ACTIVE`（=1）以下」
  である間隔のみを判定に使う。上限は**アニメ定義に現れる最大のコマ待ち**（=150）×（1＋許容率）。
- 判定対象系列は `TargetId(0) × surface_id=1000` を明示指定する（ケロ側はアニメ ID が複数のベース
  サーフェスで重複しており、系列とアニメ定義が 1 対 1 に対応しないため判定できない）。
- この窓での実測は release long p95=164.018ms（合格・判定対象 309 本・上限超過 9 本）・
  release short p95=178.615ms（不合格・65 本・上限超過 5 本）・
  dev short p95=821.971ms（不合格・22 本・上限超過 9 本）で、ビルド差を正しく分離できている。
- 既存の Revalidation Trigger（「判定式⑴の判定粒度・系列鍵・判定対象の確定方法・**判定窓の条件**の変更 → `judge-perf.py` と
  自己較正 fixture の再検証」）がそのまま適用される。

#### A6-1. 条件 4（同一鍵の重畳除外）の復帰＝remediation 1 の裁定

窓を発火起点（窓 C）へ作り直したとき、旧・窓 B が持っていた「推定活性アニメ本数 ≦ 1」の絞り込みが
**代わりを置かないまま落ちた**。系列鍵は `(target_id, surface_id)` でありアニメ単位ではないので、
同一鍵の上で 2 本のアニメが同時に走ると両者のコマ適用が 1 本の列に混ざり、隣接差は
**どちらのアニメのコマ間隔でもなくなる**。倒れる向きは混ざり方の位相で決まる。

- 遅いほうのコマがサイクルの外へ落ちる位相 → 長い空きが間隔として残り**偽の不合格**（fixture `P12`）
- 遅いほうのコマがサイクルの内側へ落ちる位相 → 長い間隔が短い間隔 2 本へ割られ、
  **上限の 4 倍で動くアニメが窓から丸ごと消えて合格に化ける**（fixture `P13`。条件 4 が無いと exit 0 を返す）

後者は task 2.3 が三度踏んだ「陽性対照の無い経路が静かに壊れる」形そのものである。
ゆえに条件 4 を窓 C へ組み込み、**偽の合格側の陽性対照** `P13_same_key_two_animations_pass_side` を
corpus へ足した（従来は偽の不合格側 `P12` しか無かった）。

- **オラクルは動かない。** 3 走行の p95 と⑴の合否は条件 4 の前後で完全に同一である
  （164.018 合格／178.615 不合格／821.971 不合格）。判定対象の本数も
  **309 / 65 / 22 のまま動かない**——emo2 の 3 走行では条件 4 が落とす区間が 1 つも無い。
- **残る穴。** 条件 4 の活性本数は発火・停止 info! の収支からの**間接推定**なので、
  発火ログを出さない書き換えが同一鍵へ混ざる形は今も検出できない（`P12` がその記録）。
  塞ぐには perf 行にアニメ識別子を足す必要があり、本 spec の範囲外である。

#### A6-2. 条件 4 の数える範囲を判定対象系列の `scope` へ限定＝remediation 2 の裁定

remediation 1 の実装は活性本数を**全 `scope` 合算**で数えていた。これは偽合格の経路である。

- **条件 4 が要る理由が、そのまま数える範囲を決める。** 条件 4 は「**同一の系列鍵**に 2 本の
  アニメが乗ると隣接差がどちらのコマ間隔でもなくなる」ために要る。別 `scope` は
  **別の系列鍵**（別 `target_id`）へコマを出すので、ケロが動いていることはキャラのコマ送りが
  健全かどうかについて何も語らない。合算は「別アニメによる系列の汚染」と「マシンが忙しい」を
  同一視するが、判定式⑴が測っているのは後者ではない。
- **実測（合算していたときの害）。** 長時間 release で 42 本が落ちて 309→267 本になっていたが、
  その 42 本は**全て「キャラ 1 本＋ケロ 1 本」**であり（区間の始端 a における活性集合が
  39 本で `{("0","1400"), ("1","0")}`・3 本で `{("0","1402"), ("1","0")}`）、
  **`1400`×`1402` の重畳は 1 件も無かった**。
  A6-1 が「落ちた 42 本は `1400` と `1402` の重畳」と記していたのは**誤りである**（本節で訂正）。
  短時間 release の 1 本も `("0","1402")+("1","0")` である。
- **証跡の破壊。** 落ちた 42 本には**上限超過が 1 本（173.997ms）含まれ**、長時間 release の
  上限超過は 9 → 8 本へ減っていた。短時間 release では、上限超過だった 5 本の区間に
  `scope="1"` の発火・停止を挟むだけで p95 が 178.615 → 161.928ms・本数 65 → 60 本となり、
  ⑴が**不合格から合格へ反転する**。⑴が探している当の証跡を丸ごと飲み込める以上、
  これは保守的な見張りではなく偽合格の経路である。
- **corpus の穴。** `P12` も `P13` も 2 本のアニメが**どちらも `scope="0"`** であり、
  cross-scope の陽性対照が 1 つも無かった。だからこの誤りがレビュー 3 回を素通りした
  ——task 2.3 と同型（「その経路に陽性対照がゼロ」）の 4 度目である。
  `P14_other_scope_animation_overlap` を足して固定した（上限超過 10 本の区間だけを
  別 `scope` のアニメが覆う形。合算で数えると exit 0＝合格、`scope` 限定で数えると exit 1＝不合格）。
- **一方通行の但し書き。** 条件 4 は間隔を減らすことしかしないので、相が変われば
  `MIN_SAMPLES=20` を割って**判定不能**へ倒れる（合格ではない）。ただし「減るだけ」は
  「安全側」ではない——落ちた中に上限超過が入れば残りの p95 は下がる。上の反転がその形である。
