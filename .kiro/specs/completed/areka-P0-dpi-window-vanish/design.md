...
- **D15（採用: 位置センチネルは resize 打ち切り・追加 2026-08-01・タスク 6.2 実装レビュー #2 起因＝タスク 6.3 新設・開発者承認「本 spec で塞ぐ」）**: `resize_window_to` の raw 導出（`follow.rs:1030-1046`）は `WindowPos.position` の `Option::None` のみを縮退し、wintf 正典の**もう一つの未確定表現** `CW_USEDEFAULT`（`== i32::MIN`・`WindowPos::default()` が position に持つ）を素通しする。位置センチネル窓が入ると ①手順 3a の `old_rect` が `i32::MIN` 近傍の全 work area 非交差矩形になり `guard_visibility` が「もともと留置」と誤読して `Keep`＝**6.1 が敷いた安全側 clamp の腕が黙って死ぬ**、②手順 3b の中央付替えと射影 T の入力（raw）も同時に汚染される。到達性は現状ゼロだが構造的不可能ではない——`main.rs:480` `spawn_dummy_window` は `WindowPos` を明示挿入せず `on_window_add` フックが `position = CW_USEDEFAULT` を挿すため**センチネル位置の窓は本番プロセスに実在**し、唯一の障壁は手順 1 の `Anchored` ゲート（`follow.rs:1009`）＝component 1 個ぶんの距離（6.2 レビュー ラウンド 2 が独立に確定）。**検討 3 案**: (a) `old_rect` のみセンチネルフィルタ＝6.2 バルーン側（`follow.rs:662-665`）と同型で guard は安全側 clamp へ倒れるが、**raw の汚染（3b 中央付替え・射影 T の X）は残り**、位置権威が確立していない窓を placement が clamp 由来の任意 X へ「配置」する＝位置権威の僭称（未解決: T 入力汚染）／(b) **resize 打ち切り**＝position センチネルを `Option::None` と同じ「窓生成前／位置未確定」状態として既存縮退（`follow.rs:1038-1043` の `warn!`＋`false`）と同じ腕へ合流（解決: `old_rect`・3b・T の汚染を一括で断つ・Req 3.3「位置決定に必要な入力（…現在位置）が取得できない場合は現状維持＋警告」に**そのまま適合**）／(c) 記録のみ＝「安全網の腕が黙って死ぬ」型が残る（6.2 レビューが却下済）。**採用: (b)**。判定は**センチネル一致**（`pos.x == CW_USEDEFAULT || pos.y == CW_USEDEFAULT`）——負座標そのものは正当（合成レイアウトの左モニタは `-1920..0`）ゆえ符号で判定してはならない（wintf 正典 `window_pos.rs:41`／`monitor_systems.rs:408` と同式）。**帰結**: ⑴寸センチネルとの**非対称は意図的**——寸未確定は接地点（位置）が実在し resize に意味がある＝`old_rect` 不明の安全側 clamp（既存手順 3a）、位置未確定は保存すべき接地点が存在しない＝打ち切り、⑵**requirements.md は無改変**（Req 3.3 は入力欠落を一般語で要求済み＝要件 gap ではない・D13 帰結⑶と同判断）、⑶既存檻 `undetermined_old_size_is_treated_as_unknown_rect_and_clamps`（位置実値・寸のみセンチネル）は**無改変で緑のまま**＝⑴の非対称の檻を兼ねる、⑷6.2 バルーン側の `old_rect` フィルタ（`follow.rs:662-665`）は**無改変**——バルーンの提案位置はキャラとの恒等式由来で raw 汚染の径路がなく、フィルタで足りている。
- **D16（採用: 終了時静穏の義務は表示基盤の呼出点まで及ぶ・追加 2026-08-01・タスク 7.3 実装レビュー起因＝タスク 7.5 新設・開発者承認）**: `TEARDOWN-SILENCE: FAIL`（diagnosis-report §2.3.4）の確定原因が**誤り**であったことが独立に二重確定した（同節の訂正ブロックが正本）。真の出所は `crates/wintf/src/ecs/window_proc/lifecycle.rs:70` の `w.world_mut().despawn(entity)` で、`window_handle.rs:279` の**非同期** `PostMessageW(WM_CLOSE)` を経由するため**陳腐化した id** を叩く。`diagnosis-procedure.md` §6.4 の判定は**主体を問わない全件判定**ゆえ、areka 側だけを是正しても 7.4 の実機サインオフは `FAIL` のままである。**検討 3 案**: (a) 判定定義を「areka 由来に限る」へ緩める＝観測装置を被検体に合わせて曲げる行為で、Req 2.10（観測装置の故障を被検体の健全性と読み替えることの禁止）の趣旨に真っ向から反する（未解決: 実機の良性ノイズが恒久的に残り、真の異常を隠し続ける）／(b) `PostMessageW(WM_CLOSE)` を同期 despawn へ変える＝窓破棄の正準シグナル（`clickthrough/controller.rs:90` が明記）を組み替える大改造で、Req 2.7 が禁じる「確定した機構以外の変更」そのもの（未解決: 影響範囲が本 spec を大きく超える）／(c) **`lifecycle.rs:70` の呼出点へ 3.2 と同じ区別（entity 不在＝正常終了系 `debug!`／実在するが規約違反＝真の異常 `warn!`）を敷く**＝欠陥クラスが 3.2・7.3 と同一で、語彙も既存の `DESPAWNED_SKIP_TAG` を再利用できる。**採用: (c)**。**帰結**: ⑴Req 6.2/6.3 の主体は要件文では「areka アプリケーション」だが、**終了時静穏の義務は窓 entity を despawn する呼出点すべてに及ぶ**——3.2（消費側 4 入口）→7.3（areka の呼出点 2 箇所）→7.5（表示基盤の呼出点 1 箇所）は同一義務の網羅であって境界の逸脱ではない、⑵`lifecycle.rs` を変更ファイル表へ登記した（Req 2.7 が求める「確定した機構」の登記＝本裁定と §2.3.4 の訂正がその確定である）、⑶**`WM_CLOSE` の投函契約・窓破棄の正準シグナルは変更しない**——変えるのは受け手が陳腐化 id をどう報告するかだけ、⑷檻は wintf 側 in-source に置く（areka からは当該経路へ到達できない）。
...
| `crates/areka/src/placement/follow.rs` | `PlacementRoute` 引数配管・`guard_visibility`／`work_area_for_window_with_origin`／`VisibilityVerdict` 追加・`enqueue_window_set_pos` 成功時 diag レコード・「entity 不在」と「`Anchored` 欠落」の区別・**`resize_window_to` 手順 3 の位置センチネル打ち切り（D15・タスク 6.3）** | 本 spec 単独所有 |
...
| 3.3 | 入力欠落時は現状維持＋警告 | follow 縮退群（既存）＋非ドラッグ経路の warn 昇格＋**位置センチネル（`CW_USEDEFAULT`）の打ち切り（D15・タスク 6.3）** | route 条件の水準分岐 |
...
| 6.2 | 破棄済み窓へ警告以上のログを出さない | 消費側存在確認（debug skip）＋**despawn 呼出点の存在確認**（areka＝`despawn_smoke_targets`／`enqueue_window_set_pos`・表示基盤＝`lifecycle.rs:70`） | 「不在」と「Anchored 欠落」の区別・D16（呼出点まで及ぶ） |
| 6.3 | 参照先不在は正常系打ち切り・他 scope 継続 | resnap_with／reconcile_reported_sizes の per-scope continue＋同上 | 同上 |
...
3. **Phase C（是正）**: S1（D3: `ExternalAuthority`＋decision 消費）・S2（D7: `None` 経路再射影）・S3（D6: 遷移ガード配線）・**S3 補（D15: 位置センチネル打ち切り・タスク 6.3・6.2 レビューが確定した 6.1 導入穴の是正ゆえ Req 2.7 の「確定した機構」内）**。Phase B が新原因を確定した場合のみ対象を追加（Req 2.7）——**S4 がこれに該当したが、被検体ではなく観測経路の欠陥ゆえ Phase B′ へ置く**。
4. **Phase D（檻・C と交互）**: 各是正の赤→緑（Req 5.4）を是正コミットの直前後で実行記録。最終の実機再サインオフ。
   **Phase D′（診断記載の訂正に伴う追加是正・2026-08-01）**: タスク 7.3 の実装中に `TEARDOWN-SILENCE: FAIL` の**確定原因が誤り**であったことが二重に確定し、§2.3.4 を訂正した（Req 2.8 の静的構造証跡ゆえ実機の採り直しを要しない）。**タスク 7.5（`lifecycle.rs:70` の存在確認・D16）はこの Phase に属し、7.4 の実機再サインオフより前に着地させる**——7.4 は `TEARDOWN-SILENCE` を全件判定で読むため、未着地のままでは必ず `FAIL` する。S4 が Phase B′ を要したのと同型の事象（**観測の前提そのものが壊れていた**）である。
