...
> **📌 2026-09-06 `areka-P0-emo-text-line-height-canon` からの登記（balloon descript の `name` キー）**: `crates/areka-parsers/src/balloon/parse.rs` の `map_merged` は balloon `descript.txt` の `name,`（例: `emo2-kakukaku` の `name,kakukaku for emo-gs`）を写像せず、`BalloonModel` にバルーン名の取得口が無い（同 spec 台帳 §7 #10）。同 spec の警告「折返し基準が描画範囲の外に解決された」（`TextRegion::resolve`・要件 6.7）は `balloon` 欄を定数 `BALLOON_NAME_PLACEHOLDER = "(名前なし)"` で埋めており、名前が通れば `crates/areka-emo-text/src/region.rs` のその定数 1 箇所を差し替えるだけでよい。本 spec の balloon 162 キー台帳では `name` を「読まれているが黙って捨てられている」側へ分類し、写像の要否を裁定へ回すこと。
