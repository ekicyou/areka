...
## 追記（2026-09-11・方針の転換＝「予算を削る」でなく「無駄を無くす」）

開発者の指摘（2026-09-11）: 「D2D なのだから CPU 負荷があっても圧倒的に軽くないとおかしい。立ち絵は高々 10 枚以下の合成、バルーンはテキストエディタ並みで済むはず。最初にバルーンが実現できた時はこんなに重くなかった。後から入った無駄である」。履歴で裏を取った結果、指摘のとおりである。

- **重くなった時点**: 2026-07-29 の `areka-P0-emo-dpi-scaling`（PR #91・`b161fc1c`）。設計 D3 が **Strategy A2＝合成後の native 面を present 段で CPU の整数 bilinear で k 倍にリサンプル**（D5）し、D6 で k 倍後のバイト列からマスクも作り直す形にした。k=1 は恒等バイトコピーなので 100% では無料——「最初は重くなかった」のは k=1 だったからで、200% で初めて 1.4 Mpx × 約 40 ns/px ＝ 50〜60 ms の CPU 仕事が**外れのたびに UI スレッド**へ乗るようになった。08-15 の `recompose-budget` は compose だけを測って 22 ms → 1.2 ms にしたが、リサンプルは測っていない。
- **無駄の正体**: ビットマップはどのみち GPU へ上げる（`t_upload_us` ≈ 0.3 ms）。上げた先の WUC／D2D は変換行列で k 倍を事実上 0 ms で描ける。CPU で 1.4 Mpx を作ってから上げるのは二度手間で、SSP が DWM に任せて無料なのと同じ理由で GPU に任せるのが本来の形。**バルーンは重くない**（文字はベクタ描画・バルーン画像の外れは走行 A で 1 回）。重いのは立ち絵の表情・まばたきの絵ごとの外れ。
- **D3 が GPU を却下した 2 つの理由は、今は成り立たない**: ⑴「マスク不整合が W5 境界侵食」→ W5（`collision-dpi-hittest`・PR #100）は当たり判定を**点 ÷k で native 座標へ縮約**する形で着地しており、k 倍のマスクを当たり判定に使っていない。窓のクリック透過用 α マスクだけが k 倍のバイト列由来で、これは native α を 1 チャンネルで k 倍する（または ÷k 参照）ので済む。⑵「鮮明性欠如」→ D2D の補間モードで bilinear／高品質を選べる。⑶ D5 の「GPU stretch は決定論 readback の檻と不整合」→ k≠1 の golden は present 段の CPU 面を読んでいる。GPU 化では**k=1 の合成 golden を正とし、k 倍の見た目は実機サインオフ**へ移す裁定が要る（既存の k 付き golden の再導出）。
- **推奨の書き換え**: 前節の「ⓐ 高速 CPU 経路 → ⓒ 容量」を取り下げ、**ⓑ GPU 拡大（native 面を上げて WUC／D2D の変換で k 倍）を本命**とする。A2 の CPU リサンプルと k 倍マスク生成は撤去。期待: 外れ 1 回 ＝ compose 1〜7 ms ＋ upload 0.3 ms（native 寸）。設計ディスカッションで D3／D5／D6 を正式に覆す（完了 spec の裁定の上書きは `doc/COMPAT_ARCHITECTURE.md` の上書き表と steering に登記）。
- **規模**: M（presenter の表示経路・WUC visual の変換・マスク生成・k≠1 golden の扱い・実機 2 水準サインオフ）。
