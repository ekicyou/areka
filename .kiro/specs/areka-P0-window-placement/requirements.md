...
12. When 既定（初期）位置を計算するとき, the 窓配置機構 shall プライマリモニタ（`is_primary`）の work area を基準とする（初期表示のみ。どのモニタに居たかの復元は position-persist（M-life）の領分）。
...
5. When ユーザーがキャラ窓・バルーン窓をドラッグするとき, the 窓配置機構 shall 窓を全モニタ（仮想デスクトップ全体）にわたって移動可能とし、ドラッグ制約を単一モニタへ閉じ込めない（初期表示のプライマリモニタ基準はドラッグ範囲を制約しない）。
6. If ドラッグ制約（`DragConstraint`）を適用するとき, then the 窓配置機構 shall その範囲を全モニタ配置の和（仮想デスクトップ）に対して物理 px 一貫で算出し、モニタ境界をまたぐ移動で窓を画面外へ消失させない（2026-07-05 リジェクトの単一モニタ誤釘付け・二重スケールの再発防止）。
7. While `alignmenttodesktop` が `bottom`（下端吸着）のスコープのキャラ窓がドラッグされているとき, the 窓配置機構 shall Y を現在窓が属するモニタの work area 下端（`bottom − h`）へ釘付けし、上下方向のドラッグ移動を不可とする（ukadoc 正典「上または下に吸着した場合、上下方向へのドラッグ移動ができなくなる」）。左右方向は 4.5 のとおり全モニタ移動可とし、モニタを跨いだ場合は跨いだ先のモニタの work area 下端へ再吸着する。`free` は全方向移動可・`Seam` 値は bottom と同挙動とする。（2026-07-11 目視受け入れ検証での開発者指摘により追加・承認）
8. When ユーザーがバルーン窓を単独ドラッグしたとき, the 窓配置機構 shall バルーン窓のみを移動させ（キャラ窓は不動・bottom 吸着の対象外）、ドラッグ後のキャラ窓との相対 offset を記憶して以後のキャラ窓ドラッグ・窓移動 API の追従に用いる（SSP de-facto。記憶はセッション内のみ・永続化は position-persist（M-life）の領分）。（2026-07-11 目視受け入れ検証での開発者指摘により追加・承認。4.4 の「正式な配置規則の非所有」は不変——初期 offset の幾何規則は暫定のまま、ユーザー調整の記憶のみを追加する）
