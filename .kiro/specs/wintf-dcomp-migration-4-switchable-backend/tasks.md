...
- [x] 1. `CompositionMode` enum と `Window` 構造体フィールドの基盤実装
- [x] 1.1 `CompositionMode` enum の定義と公開
...
- [x] 1.2 `Window` 構造体への `composition_mode` プライベートフィールド追加
...
- [x] 2. `DCompGraphicsResource` ECS リソースの実装
- [x] 2.1 (P) `DCompGraphicsResource` の構造体定義とリソース登録
...
- [x] 2.2 (P) `DCompGraphicsResource::new()` と `invalidate()` の実装
...
- [x] 3. (P) `GraphicsCore` から DComp フィールドを除去し共通基盤に純化
...
- [x] 4. (P) `on_visual_add` フックへの DComp コンポーネント条件付き挿入
- [x] 4.1 (P) `find_owner_window_composition_mode` ヘルパー関数の実装
...
- [x] 4.2 (P) `on_visual_add` における DComp コンポーネント挿入ロジックの追加
...
- [x] 5. (P) `compositor_init_system` の ULW モード限定化
...
- [x] 6. `init_window_graphics` の DComp 遅延初期化対応
- [x] 6.1 `init_window_graphics` への `DCompGraphicsResource` 遅延初期化ロジック追加
...
- [x] 6.2 `init_window_graphics` への `CompositionMode::DComp` ランタイムフィルタ追加
...
- [x] 7. (P) `invalidate_dependent_components` への `DCompGraphicsResource` 無効化連動
  - システムパラメータに `Option<ResMut<DCompGraphicsResource>>` を追加
...
- [x] 8. ウィンドウ生成・WndProc の `CompositionMode` 連動
- [x] 8.1 (P) `create_windows` システムの `CompositionMode` 連動ウィンドウスタイル実装
...
- [x] 8.2 (P) `handlers.rs` の WM_PAINT `CompositionMode` 分岐実装
...
- [x] 9. `world.rs` への DComp システムスケジュール再登録
- [x] 9.1 5ステージへの DComp システム追加登録
...
- [x] 9.2 既存 ULW スケジュール順序・依存関係の保持確認
...
- [x] 10. `cargo test` 全パスと既存後方互換性の確認
- [x] 10.1 `Window` 構造体変更に伴うコンパイルエラーの修正
  ...
  - _Requirements: 9.1_
- [x] 10.2 既存 ULW パイプライン後方互換テスト実行
  - `cargo test` を実行し、全テストがパスすることを確認
  - `CompositionMode::ULW`（デフォルト）の動作が変更されていないことを確認
  - _Requirements: 9.1, 9.2_

- [x] 11. DComp パイプライン向けユニットテストの実装
- [x] 11.1 (P) `CompositionMode` と `Window` の単体テスト
...
- [x] 12. DComp パイプライン向けインテグレーションテストの実装
- [x] 12.1 コンポーネント自動挿入の統合テスト
...
- [x] 12.2* `GraphicsCore.invalidate()` → `DCompGraphicsResource` 連動無効化の統合テスト
...
- [x] 14. `multi_backend_demo` サンプルの実装
