...
- [ ] 5.1 S1 是正: OS 提案位置の源断ちとゴースト窓への外部権威宣言
  ...
  - **4.3 の赤 4 件の `#[ignore]` を全て外す**（`window_pos.rs` の `s1_red_*`＝dpi96／dpi120／dpi192／no-write-context）。**dpi96 の 1 件も外すこと**——「96 では緑」は是正後も成立する性質であり、外して初めて 96/120/192 の非対称が回帰檻として保存される。外し忘れると 5.1 の完了状態「4.3 の檻が緑に変わる」がゲートを掛けたまま形式的に満たされてしまう
