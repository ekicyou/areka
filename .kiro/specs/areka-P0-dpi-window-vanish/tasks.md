...
- [x] 5.2 S2 是正: 位置の権威と寸の権威の分離
  ...
  - **7.2 への申し送り（5.1 と合わせて 2 件）**: ⑴`[WM_WINDOWPOSCHANGED] DPI center correction skipped: BoxStyle not found` の literal（5.1 が §6.5 の判定語へ昇格させたが in-source 檻ゼロ）⑵`follow.rs` の `Anchored 未付与` warn の**水準と literal**（上記のとおりバルーン据置き檻の唯一の守り）。**どちらも「判定語に使っているのに檻が無い」型**で、書式や水準を変えれば手順書・檻が静かに嘘になる
  - 完了状態: 4.4 の檻が **`#[ignore]` 無しで**緑に変わる。不可視・未表示・寸不変のいずれの経路でも接地点が保たれ、可視化時に変化後の DPI 相当の寸と規約準拠の位置で表示される
  - _追加（2026-08-01・5.2 レビューが実測）_: **バルーン据置きの檻は「別ファイルの warn 水準 1 点」に依存している**。レビュアが「バルーンを射影経路へ流す」＋「`follow.rs:847` の `Anchored 未付与` warn を `debug!` へ降格」の 2 変異を同時に当てると**檻が完全に緑**になった。この檻が誤配線を捕まえているのは「バルーンが動かないこと」ではなく「`follow.rs` の warn が warn であること」に由来する。バルーンは `Anchored` を持たない設計（`spawn.rs:961-963,1146-1147` が固定）ゆえ「動かない」だけの assert は当該変異に対して**不動点**——[[2.2 の教訓]] が「不変量がログ側にしか現れない」形で再来した 6 例目。ブロッキングとしないのは、将来バルーンが `Anchored` を得れば位置 assert（`frame.rs:3603-3608`）が代替で捕まえる二段構えになっているため。**7.2 で `follow.rs` の当該 warn の水準・literal を固定する檻を足すこと**
  - _Requirements: 2.7, 2.8, 4.1, 4.2, 4.5, 4.6_
  - _Depends: 4.5, 5.1_
  - （現状維持のべき等性が成立する前提が「S1 是正により生位置が areka の確定値であること」ゆえ 5.1 の後でなければならない＝並行不可）

- [ ] 6. Phase C: 可視性の構造保証
- [x] 6.1 S3 是正: キャラ窓経路への遷移ガード配線
...
- [x] 6.3 S3 補: 位置の未確定表現（`CW_USEDEFAULT`）をキャラ窓経路でも旧矩形不明として扱う
  ...
  - _完了（2026-08-01）_: 編集面は `crates/areka/src/placement/follow.rs` **1 ファイル・2 ハンクのみ**（**298 挿入／0 削除**＝D15 帰結⑴⑷の「無改変」が diff で証明される形）。センチネル打ち切りは手順 3 の `Option::None` 縮退の直後（`follow.rs:1045-1068`）に置き、`raw`・`old_rect` の導出より**前**で断つ＝①`old_rect` 汚染と②手順 3b／射影 T の入力汚染を 1 箇所で同時に塞ぐ。檻 3 件を新設（`undetermined_position_aborts_resize_without_writing`・`negative_real_position_is_not_aborted_and_still_resizes`・`single_axis_position_sentinel_also_aborts`）。`cargo test -p areka` **623 passed / 0 ignored**・`cargo test -p wintf` **1,057 passed**（非退行）・`cargo build` / `--release` 双方成功
...
- **6.3（「書込ゼロ」の檻を機械的に非空虚と示す手順＝本 spec で初めて成立）**: 打ち切り系の檻は `!ok`（戻り値）の assert が**先に**発火するため、その後段にある「`WindowPos` がバイト不変」の assert は**一度も評価されずに緑**になり得る——[[5.2 の教訓＝空虚性 6 例目]] の「不動点」型が最も潜みやすい配置である。レビュアは**`!ok` の assert を一時的に無効化したうえで**ガード撤去の変異を当て、`WindowPos == before` の行が**単独で赤くなる**ことを実測した（`enqueue_window_set_pos`・`follow.rs:1359-1368` が `bypass_change_detection` で position・size を書き戻すため、ガードが無ければ実際に書かれる）。**「戻り値」と「副作用不在」を同一テストで主張する檻は、前段の assert を外して後段が単独で赤くなることを確かめること**——さもなくば後段は装飾に過ぎない。
- **6.3（センチネル判定は wintf 正典と「同型」であって「同式」ではない）**: wintf 正典（`crates/wintf/src/ecs/graphics/systems/window_pos.rs:41`／`crates/wintf/src/ecs/layout/systems/monitor_systems.rs:408`）が見るのは `position.x` と `size.width` の 2 つで、6.3 が置いたのは `pos.x || pos.y`（位置の**両軸**）。D15 が明示的にこの式を指定しており、`y == i32::MIN` は到達可能な実座標ではないので偽陽性の risk が無い＝**正典より厳しいのは意図的**。初版のコメントは「同式」と書いており、[[4.6 の教訓＝doc が実装より強い約束をする]] の再来になるためレビュー指摘を容れて「同型（正典が見るのは `position.x` と `size.width`・ここは位置の両軸）」へ是正した。**正典を引用するコメントは、引用元と自分の差分まで書くこと。**
- **6.3 → 7.2 への申し送り（本番到達性はゼロのまま・障壁は `Anchored` 1 個）**: 6.3 は入口を塞いだが、センチネル位置の窓が `resize_window_to` に**到達しない**状況自体は変わっていない（`main.rs:480` `spawn_dummy_window` が生む `position = CW_USEDEFAULT` の窓は手順 1 の `Anchored` ゲート・`follow.rs:1009` で先に返る）。ゆえに 6.3 の 3 檻は**将来の到達に対する保険**であり、7.2 が混在 DPI 回帰檻を書く際に「センチネル窓は打ち切られる」を前提にしてよい。`undetermined_old_size_is_treated_as_unknown_rect_and_clamps`（位置実値・寸のみセンチネル＝clamp 腕）との**非対称は意図的**（D15 帰結⑴）——7.2 が両者を混同して片方の期待値へ寄せないこと。
...
- **6.1 → 6.2 への必須申し送り（`route_applies_visibility_guard` をそのまま流用してはならない）**: 発火条件の述語（`follow.rs:1559`）は**書込自身の route** を入力に取り、`BalloonFollow`／`KeepPositionResize` を**偽**へ倒している（キャラ窓の反映口 `resize_window_to` の判定として正しい）。バルーンの適用可否は「随伴の**引き金**がドラッグだったか配置系だったか」で決まり、それは `follow_balloon` の**呼出元**しか知らない——`follow_balloon` は `resize_window_to`（配置系）と `on_char_drag`（ドラッグ）の双方から呼ばれる（`follow.rs:505`）。同述語を `BalloonFollow` に当てると**バルーンのガードが常に無効**になり、逆に無条件適用するとドラッグ随伴でバルーンが引き戻されて Req 3.1 の「明示操作の尊重」を壊す。**`follow_balloon` へ引き金を配管する**のが 6.2 の設計課題である。ログ判定語（`VISIBILITY_CLAMP_TAG` ほか `follow.rs:1526-1532`）と `apply_visibility_guard`（`:1608`）はそのまま再利用できる。
- **6.1（`NearestFallback` は「射影の入力」ではなく「決めた位置」で判定する）**: 当初 `raw`（射影の入力）の帰属で判定したところ、既存檻 `drain_reconcile_skips_despawned_scope_at_debug_and_processes_surviving_scopes`（`frame.rs`）が赤になった——合成 fixture の**生存** scope 窓が、下端吸着**前**の位置では中心がちょうど work area 下端に乗る（半開判定ゆえ非帰属）ためである。下端吸着では `raw` の中心が work area の外にあることは珍しくなく、`raw` 基準の判定は「射影が正しく接地させて可視域へ収めた窓」まで食い違いとして報告する**偽陽性**になる。Req 3.2 が言うのは「窓位置を**決めた**とき」の食い違いゆえ `proposed` 基準へ改めた（`follow.rs:1643`）。**`clamp_wa` は引き続き射影の入力側（`raw`）から引く**——両者を混ぜると `guard_visibility` の事後条件（clamp 後に `clamp_wa` と交差する）が崩れる。同一純関数を 2 回引く理由は `apply_visibility_guard` の doc に明記した。
- **6.1（旧矩形『不明』は `Option::None` だけではない＝[[4.6 の教訓]] の再来）**: `WindowPos::default()` は寸を `Some(SizeI { CW_USEDEFAULT, CW_USEDEFAULT })`（`i32::MIN` センチネル）で持つ。素の矩形として交差判定へ入れると `saturating_add` で逆転矩形になり、「もともと画面外に留置されていた」と誤判定して**安全側 clamp の腕が丸ごと死ぬ**。`old_rect` 導出（`follow.rs:903-912`）で非正寸を `None` へ倒し、檻 `undetermined_old_size_is_treated_as_unknown_rect_and_clamps` が固定した。**バルーンの旧矩形を組む 6.2 も同じ罠を踏む**（バルーン窓も `WindowPos::default()` 由来のセンチネルを持ち得る）。
- **6.1 → 7.2 への申し送り（新設した判定語 3 語は in-source 檻がリテラル固定済み）**: `[visibility-guard] ClampX`／`NearestFallback`／`WorkAreaUnresolved`（`follow.rs:1526-1532`）は `diagnosis-procedure.md` §3.1 の **O17〜O19** へ昇格させ、同書 §3.3 の「未実装」行は退役させた。檻側にも**独立の literal**（`follow.rs` の `CLAMP_TAG`／`NEAREST_TAG`／`UNRESOLVED_TAG`）を置いてあるので、本体定数を書き換えれば檻が赤になる——5.1／5.2 が起票した「判定語に使っているのに檻が無い」型の穴を本 3 語では作っていない。7.2 が追加する檻は**残り 2 件**（`BoxStyle not found` の literal と `follow.rs` の `Anchored 未付与` warn の水準・literal）だけでよい。
- **6.1 → 7.4 への必須申し送り（+36px＝S5 は本 spec で解消しない・コードで確定）**: 遷移ガードは⑴Y を一切変えない（`guard_visibility` の `ClampX` 腕は `y: proposed_pos.y` を返す・既存檻 `guard_never_modifies_y_in_any_branch` が固定）⑵②-b の +36px 矩形は旧新どちらの work area とも**交差する**ので第 1 分岐で `Keep` へ抜ける⑶中心は旧 work area の内側ゆえ `NearestFallback` としても観測されない。機序も確定した——**areka の `MonitorSnapshot`（placement の全判断が読む権威）は `main.rs:703` ただ 1 箇所でしか書かれず、更新経路が存在しない**（4.6 が直したのは wintf 側 `Monitor` エンティティ表であって本 Resource ではない）。ゆえに射影は起動時の work area 下端を永久に読む。**所有は `areka-P0-dpi-transition-atomicity`**（spec 実在確認済み・roadmap 未着手表に開発者裁定として明記）。7.4 の再サインオフで `ground_y` が新下端に一致しなくても、それは 6.1 の不足ではない。
- **6.1（S3 の赤は `#[ignore]` を要さない）**: 配線（是正）と檻の新設が同一タスクに属するため、赤は「配線前のツリーで新設檻を走らせる」ことで同一セッション内に採れる。実行記録は `diagnosis-report.md` **§3.4**（新設・§3.1〜§3.3 と非重複）。`grep -rn '#\[ignore' crates/areka/src` は引き続き**一致ゼロ**。
- **5.2（檻の空虚性・6 例目は「不動点」型の再来・実測で捕捉）**: 「バルーン窓は `None` 経路で位置据置き」を主張する檻を**書込ゼロだけ**で書くと**空虚**になる。誤実装（`Balloon` 腕をキャラと同じ射影経路へ流す）を当てても、バルーン窓には `Anchored` が付かない設計（`spawn.rs` の檻が固定）ゆえ `resize_window_to` は `warn!`＋`false` で**窓を動かさない**——書込ゼロの assert は緑のまま通る。是正版は**警告以上のログが 1 行も出ないこと**を併せて主張し、当該変異（M2）が**ログ assert だけで**赤になることを実測した。[[2.2 の教訓]]（不変を主張する檻は変異が不動点に落ちないか確かめる）が、今度は「不変量が挙動ではなくログ側にしか現れない」形で再来した。**「動かない／変わらない」を主張する檻は、誤実装が同じ結果を別の理由で作らないかを必ず確かめること。**
- **5.2 → 6.1 への必須申し送り（遷移ガードの配線点が 1 本増えた）**: `resize_window_to` へ入る**非ドラッグ配置系経路は 4 本→5 本**になった。DPI 相が 2 本に分岐している——⑴`frame.rs:870`（寸の再導出結果あり・従来経路）⑵**`frame.rs:937`（再導出結果なし・現寸のまま位置だけ再射影＝5.2 が新設）**。**両者とも route は `DpiReproject`** ゆえ、D13 帰結⑴の「発火 route 集合＝`AnchorChange`/`Resnap`/`DpiReproject`/`ReportedSizeReconcile`」はそのまま有効で**ガードの配線点は 1 箇所のまま**だが、⑵は「寸が変わらないのに位置だけ動く」走行を新たに生む＝ガードが評価する提案矩形の寸が**現寸**であることを前提にすること。診断レポート §1.3 の経路表は 5.2 が再測定して更新済み（S3 未是正ゆえ同節は現ツリー実値を保つ）。
- **5.2 → 6.1 への申し送り（§2.5.3 の S5 候補との関係）**: 実機②-b が観測した「work area の変化に接地点が追随しない（+36px）」は、**5.2 の是正で `DpiReproject` 経路については解消し得る**——`Changed<DPI>` が立てば寸の再導出結果に関わらず射影が走り、Y が変化後の work area 下端から再導出されるため。ただし②-b の 13 件は**すべて寸を伴う書込**（`Some` 経路＝S2 のゲートに掛かっていない）だったので、+36px の機序は「射影が古い work area を読んだ」＝**モニタ表の更新と DPI 相の実行順**の問題である可能性が残る（5.2 は順序を変えていない）。**7.4 の再サインオフで `ground_y` が新下端に一致するかを必ず確認すること**。一致しなければ S5 は別機序として生き残る。
- **5.2 → 7.1 への申し送り（ゲートは全解除済み）**: `s2_red_*` 4 件（dpi96 含む）の無視属性は撤去済みで、**`grep -n '#\[ignore' crates/areka/src` の一致は 0 件**（注記文からも字面を排除したため、4.4 が残した「コメント 3 件の誤爆」も消えている＝`-n` で内訳を見る必要すら無くなった）。7.1 は S1（`window_pos.rs`）・S2（`frame.rs`）の双方で 0 件を再確認するだけでよい。**残る無視属性は `cargo test -p wintf` の lib 2 件（4.6 の `s4_red_`）のみ**——これは 7.1 の完了状態「ゲートされた赤証跡が 1 件も残っていない」が掃く対象である。
- **5.2（Req 4.5 は「べき等 skip」で成立しており、追加の分岐を持たない）**: 「再導出結果が得られないなら現状維持」は**条件分岐で実現していない**。現寸をそのまま `resize_window_to` へ渡すと、同寸・同 work area では導出値が現在値と一致し `follow.rs:909-919` のべき等 skip が書込ゼロで抜ける。ゆえに「現状維持か規約復元か」を判定する述語は**どこにも無く**、書込の有無は射影の結果が現在値と一致するか否かだけで決まる。**この構造を「判定が抜けている」と読んで述語を足さないこと**——足すと 4.1/4.2（規約違反時の復元）と 4.5（正常系の維持）が二重管理になる。
...
- **5.1 → 5.2 への申し送り**: S1 是正により `WindowPos.position` が OS 由来座標で汚染されなくなった＝**design.md:490 の「`None` 経路の再射影は正常系ではべき等」の前提が実際に成立する状態**になった。4.4 が置いた `s2_dpi_phase_writes_nothing_when_the_ground_point_already_holds`（Req 4.5 の書込ゼロ）はこの前提の上で読むこと。~~`grep '#\[ignore' crates/areka/src` の**実属性**は `s2_red_` 4 件のみ（コメント行 3 件が誤爆するので `-n` で内訳を見ること）。~~ **（2026-08-01・5.2 着地で陳腐化）** 5.2 が `s2_red_` 4 件を解除し**コメント側の誤爆字面も書き替えた**ため、現ツリーの `grep -rn '#\[ignore' crates/areka/src` は**一致ゼロ**。areka 側にゲートは 1 件も残っていない（7.1 はこれを前提に読むこと）。
