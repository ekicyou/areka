...
> **本節が本文で扱う 4 件（S1・S2・S3・S3′）は、実機ログを 1 行も参照せずに確定している。** 確定日 2026-07-31・確定タスク 4.2・確定手段＝コード読解（下記 `file:line` を開けば第三者が同じ結論に到達できる）。

> **追記（2026-08-01・台帳の可用性の是正）**: 下の §1.0 一覧表に **S4 の参照行を追加**した。S4 は本節の 4 件と確定の経緯が異なる（**実機セッション②が発見**し、根本原因を**静的構造証跡**で確定した＝§0.1 の 2 クラスの双方を踏んでいる）ため、**記録の本体は §2.7 に置いたまま移動も複製もしない**——一覧表の行は §2.7 への**ポインタ**である。追加の理由は台帳の可用性にある：S4 は確定した欠陥でありながら §1.0 の一覧に載っておらず、「確定した欠陥の台帳」を §1.0 だけで閉じられなかった。**本追記はいかなる既存の確定も取り消さない**（§5 メンテ規約 2）。
...
| **S4**（**参照行**・記録の本体は **§2.7**） | モニタ表の更新要否が**モニタの同一性判定**（識別子の一致）で条件付けられ、識別子が不変のまま値だけ変わる表示構成変更で更新分岐が**構造的に到達不能**になる | **7.1・7.2**（波及: 3.1・3.2・4.1・4.2 の**判定根拠を凍結**させる） | `Monitor` の `PartialEq`（`monitor.rs:103-107`）＋ `detect_display_change_system` の更新分岐（`monitor_systems.rs:229-236`） | 4.6（**着地済み 2026-07-31**・§2.2／§3.3） | §3.3 → 7.1 |

S4 の行は上記「追記（2026-08-01）」のとおり**ポインタであって記録の本体ではない**——欠陥の構造・根本原因・実機での発見経緯・是正の成立確認はすべて **§2.7**（＋§2.2／§2.5.2／§3.3）が持つ。「未充足にする受入基準」欄の **7.1・7.2** は requirements.md:74（Req 2.8 の S4 定義）の明文であり、波及の 4 件も同行が「判定根拠を凍結させる」と明記しているものである。
...
> **本節の採取は完了している（2026-08-01 更新）**——①（2026-07-31 16:35・`PASS`）／②-a（同 21:02・`FAIL 0/6` → S4 発見）／②-b（2026-08-01 08:25・`PASS 6+6/6`・S4 是正後）の 3 走行で **タスク 4.5 は完了**である（§2.2／§2.5.2／§5 の本文と同じ内容）。是正後のビルドでの②の採り直し（②c・2026-08-01 08:37）は **7.4 の担当**であり、結果は **§4** が持つ。
>
> _（2026-08-01 更新・陳腐化した記述の差し替え）_ 本注記は当初「**本節はセッション①のみ記入済み。セッション②は未採取**」「**タスク 4.5 は未完である**——②未採取のまま 4.5 を `[x]` にしてはならない」と記していた。これは 4.5 実施中の進行状態の記述であり、②-a／②-b の採取をもって陳腐化した（§5 メンテ規約に言う**陳腐化した記述の更新**であって、確定の取り消しではない）。**当時の禁止事項そのものは規律としてなお有効**である——推測や代替で埋めない・②未採取のまま完了扱いにしない。
>
> 埋めるのはタスク 4.5（開発者が実機で実行するゲートタスク）であり、**推測や代替で埋めてはならない**。採取は `diagnosis-procedure.md` の手順に厳密に従い、**Phase A 完了・S1/S2 是正未投入のビルド**で行う（是正投入後は消失の実機再現自体が起きなくなり、Q1〜Q4 の確定材料が永久に失われる）。
>
...
#### 2.3.6 参考: 本 spec 範囲外だが実機で確認された事象

> **番号の是正（2026-08-01）**: 本項は当初 `2.3.5` を名乗っていたが、同じ番号が §2.3 内で**二重に使われていた**（もう一方＝「Q3 の補足: ②-b で観測された書込の連鎖」）。**先に置かれていた側を `2.3.5` のまま残し**、後段の本項を **`2.3.6`** へ振り直した。**内容は 1 文字も変えていない。** 他文書・本書の他節が「§2.3.5」として本項（`BoxStyle not found` 84 件の表）を参照している場合は **§2.3.6** と読み替えること。
...
> **⚠ 訂正（2026-08-01・上記一覧に Req 7.1・7.2 が欠落していた）**
>
> 上の 3 行は**取り消さない**（いずれも実測に基づく正しい記述である）。欠けていたのは**主たる未充足 AC** のほうである——requirements.md:74（Req 2.8 の S4 定義）は S4 を「**Requirement 7.1・7.2 を未充足にし**、Requirement 3.1・3.2・4.1・4.2 の判定根拠を凍結させる」と定めている。
>
> - **Req 7.1**（OS が表示構成の変更を通知したとき, 表示基盤は全モニタの境界矩形・work area 矩形・DPI・プライマリ標識を再取得し**モニタ表へ反映する**）——`Updating Monitor entity` 0 件（上記「観測結果」）がその未充足の実機像である。
> - **Req 7.2**（モニタ表の反映要否の判定に**同一性判定を用いず値の変化を用いる**）——S4 の欠陥そのものを名指しした AC である。
>
> **欠落の理由**: 本節（§2.7）を書いた時点で **Requirement 7 はまだ存在しなかった**。Requirement 7（OS 側の表示構成変更への追従）は §2.7 の発見を受けて新設された AC 群であり、上の 3 行は**新設前の要件番号だけで書かれている**。上記の 3 行と本訂正を合わせたものが S4 の完全な未充足 AC 一覧である（§1.0 の S4 参照行はこの完全形＝主 7.1・7.2／波及 3.1・3.2・4.1・4.2 を載せている）。
...
| 消失痕跡 | **`VANISH-TRACE: NONE`（両セッション）**。**いずれも全 work area 非交差の矩形が 1 件も無い**（幾何突合＝§6.3 の正本判定）。突合の内訳は下表——**「判定行数」と「総数」は一致しない**ので、件数を引用する際は必ず区別すること<br><br>／① `[diag.window_move]` **総数 1,893・判定 1,793・判定不能 100**／`[guarded_set_window_pos]` 3,976（全件判定可）<br>／②c `[diag.window_move]` **総数 38・判定 37・判定不能 1**／`[guarded_set_window_pos]` 38（全件判定可）<br><br>**判定不能の内訳**: `w=-`／`h=-`（移動専用書込＝§6.3 手順 2）でありながら**同一 entity の既知寸がまだ 1 件も無い**行、すなわち各窓の**最初の移動専用レコード**である。寸が不明では矩形を組めないため幾何判定の対象外であり、**非交差の可能性を否定できていない残余**として明示的に記録する（0 件と読まないこと）。窓生成直後の 1 窓あたり数行に限られ、直後に寸を伴うレコードが続いて判定対象へ入る |
| 接地点保存の実測 | **`ground_y=1704` が全 DPI 遷移を通じて定数**（scope0）。同一遷移で `h` は 1094↔821・`x` は 2120↔2216（中心保存の付替え）と動くのに接地点だけが不動＝**Req 4.1／4.2 の実機成立** |
| 残余 A（提案矩形） | **`RESIDUE-A-SUGGESTED-RECT: PASS`**（手順書 §7.1・手順 1・2・4。手順 3 は §7.1 の注記どおり是正後は §6.5 の否定側へ反転する）／**`S1-SOURCE-CUT: PASS (external=24/24, boxstyle_warn=0, x_divergence=0)`（両セッション）**——手順書 §6.5 が定める判定形式。**追加報告フィールド `unreachable=0`**（`policy=unreachable` は 1 行も無い＝**手順書 §3.4** の除外規則の適用対象なし）。判定の反転が実測で成立——**是正前のセッション①は `applied=true` が 84/84・`BoxStyle not found` も 84 件**（§2.3.3・§2.3.6）だったのに対し、是正後は**全件 `policy=ExternalAuthority` かつ `applied=false`・`BoxStyle not found` 0 件**。§6.5 が要求する「肯定側⑴と否定側⑷を対で読む」を満たしている。`x_divergence` の測定法と実値は **§4.4** |
| 残余 B（モニタ列挙） | **`RESIDUE-B-MONITOR-ENUM: PASS`**（手順書 §7.2・手順 1〜5 すべて一致）。⑴ `[diag.monitor_snapshot] … count=2` が実台数と一致／⑵ `dpi=192`＝200%・`dpi=144`＝150%（拡大率の実値は同 `$LOGDIR\meta.txt` に記録済み＝手順書 §2.5 の要求）／⑶ `context=monitor_snapshot` 群と `context=prepare_ghost_windows` 群の `[diag.monitor]` 行が同値／⑷ 両側の列挙が一致（`handle=395545`／`461083` が areka `[diag.monitor]` と wintf `[initialize_layout_root]` の双方に同値で出現）／⑸ `work_area=0,0,2880,1704` < `bounds=0,0,2880,1800`（タスクバー 96px）・index=1 の負座標 `-2560,195,0,1795` が忠実転写。`[visibility-guard] WorkAreaUnresolved` は**0 件**＝観測装置側の異常なし |
| **S1** の是正後の実機挙動（AC **4.3**・タスク 5.1） | 上記のとおり源断ちが成立（84/84 陽性 → 24/24 陰性）。OS 提案位置は記録されるが**書かれない** |
| **S2** の是正後の実機挙動（AC **4.1・4.2・4.6**・タスク 5.2） | `ground_y` 定数＝射影の欠落が消えた。DPI 相の書込は全て `route=DpiReproject`。**手順書 O16**（`dpi reproject: WindowPos.size 未確定` の `warn!`・target `areka::emo2_boot::frame`。**§4.1 で言及する「旧称 O16＝現 O20」の再導出行とは別物**）は 0 件＝5.2 が新設した縮退腕へ落ちた窓は無い |
| **S3** の是正後の実機挙動（AC **3.1**・タスク **6.1**＝キャラ窓経路の配線） | `[visibility-guard] ClampX` **0 件**（キャラ窓由来＝`route=` が `AnchorChange`／`Resnap`／`DpiReproject`／`ReportedSizeReconcile` のもの）。**ガードは配線されているが発火する状況が起きなかった**——`VANISH-TRACE: NONE` と整合する。**ガード不発は不足ではない**：非交差への遷移そのものが生じなかったことの裏面であり、手順書 §6.3 が「O17〜O19 は代替ではなく補助」と定めるとおり合否は幾何突合が出している |
| **S3′** の是正後の実機挙動（AC **3.4**・タスク **6.2**＝`follow_balloon` の配線） | `[visibility-guard] ClampX` **0 件**（バルーン窓由来＝`route=BalloonFollow` のもの）。S3 と同じ読み——**配線済み・発火状況なし**。バルーン矩形も全件 work area と交差しており（`VANISH-TRACE: NONE`）、「キャラは見えているのに会話が読めない」状態への遷移は 1 度も起きていない。随伴そのものの成立は **§4.5**（Q4）が実ログで示す |
...
> **識別子の読み替え（2026-08-01 追記）**: 本項および §2.2／§2.2.1／§2.3 が **`O16`** と書いている再導出行（`[detect_display_change_system] Redriving window DPI from updated Monitor`）は、手順書側で **`O20`** へ振り直された。`O16` が手順書 §3.1 の別の観測点（`dpi reproject: WindowPos.size 未確定`・target `areka::emo2_boot::frame`・`warn`）と**二重に割り当てられていた**ためである（先に採番されていた §3.1 側を `O16` のまま残した）。**本書の既存記述は取り消さず、判定語のリテラル（`Redriving window DPI from updated Monitor`）で読むこと**——実機ログの grep はリテラルで行うので実害は無い。手順書 §3.1 の注記が正本。

`WM_DPICHANGED` は届いている（②-a の 0 件とは異なる）が、到達時には **O16（現 `O20`）が既に DPI を更新済み**で `new == old` になっている。§5.1 の改訂が想定した「同一の遷移が両方に現れることはない（等値ガード）」が**逆順（再導出が先・通知が後）でも成立する**ことが実測された。`[detect_display_change_system] Display configuration change applied windows_redriven=4` が 6 回。
...
>
> _（2026-08-01 追記）_ 上記の是正提案は **`diagnosis-procedure.md` へ反映済み**である（§6.2 手順 5＋是正注記／§6.1 手順 7 の「セッション①限定」明記／§2.4 の「値は 15 分である必要はない」注記）。本項の記述は発見の記録として残す。

#### 4.4 `S1-SOURCE-CUT` の `x_divergence` の実測（判定形式の是正・2026-08-01 追記）

> **是正の趣旨**: 上表の残余 A 欄は当初 `S1-SOURCE-CUT: PASS (external=24/24, boxstyle_warn=0, **unreachable=0**)` と記していた。しかし手順書 §6.5 が定める判定形式の第 3 スロットは **`x_divergence`** である（同 §6.5 手順 5＝セッション①で観測された **861px 級の X 差の消滅**＝S1 の指紋そのもの）。`unreachable=0` も事実だが**別のフィールド**であり、`x_divergence` のスロットを占めてはならない。本項は `x_divergence` を実測して補い、`unreachable=0` は**追加報告フィールド**として残す。**既存の記述は取り消さない。**

**測定法**（`entity`／`hwnd` の書式差を避けるため、結合は手順書 §4.2 の規約どおり `entity=` を経由する）:

1. **O8**（`[WM_DPICHANGED] suggested position write decision`）の行から `entity=` → `hwnd=` の対応表を作る。
2. **O4**（`[diag.window_move]`）が記録した「**areka が決めた (x, y)**」の集合を作る。
3. 同一 hwnd への **O9**（`[guarded_set_window_pos] Calling SetWindowPos`）の**実書込 (x, y)** が⑵の集合に含まれるかを全件突合し、含まれない書込＝「**areka が決めていない位置への書込**」を `x_divergence` として数える。

**セッション②c（`session2c-osdpi.log`・ドラッグなし）: `x_divergence = 0`**

- ゴースト窓への `SetWindowPos` は **38 件**（`hwnd="0x900A2"` 13／`"0xC00BA"` 12／`"0x500BC"` 7／`"0x2450B20"` 6）。
- **38 件すべてが areka の決定値と一致**し、**areka が決めていない位置への書込は 0 件**。861px 級どころか 1px の乖離も存在しない。
- これは §6.5 手順 5 が求める「O9 と `[diag.window_move]` の X が一致し、861px 級の X 差が 0 件」の成立そのものである。是正前のセッション①（2026-07-31）では同じ差分が **−861〜+861 の範囲でばらついていた**（§2.3.3・§2.5 の「最大 861px 乖離」）。

**セッション①（`session1-drag.log`・ドラッグあり）では本指標は適用外**

同じ測定を①へ当てると `[guarded_set_window_pos]` **3,976 件中 2,078 件**が不一致になるが、**これはドラッグ由来の書込であって S1 の指紋ではない**。根拠は 3 つある:

- 手順書 **§4** が「**ドラッグ由来の位置は O4 に出ない**」と明記している（ドラッグ経路は `enqueue_window_set_pos(..., route=None)` で呼ぶ＝**設計上レコードを出さない**・手順書 §3.3 末尾行）。ゆえに「areka の決定値の集合」に載らないのは**正常**であって乖離ではない。
- 実測でも裏が取れている——不一致 **2,078 件**のうち **2,064 件**が `[drag] Dragging started` 〜 `Dragging ended` の**区間内**に出ている。
- 同セッションの `wintf::ecs::drag` target の位置更新行は **2,075 件**（同 target 2,170 行から状態遷移・捕捉管理の 4 種＝`[drag]` 31・`[CaptureGuard]` 32・`[start_preparing]` 16・`[cleanup_drag_state]` 16 を除いた数）で、不一致件数 2,078 と**ほぼ一致**する。

> **適用外条件の明記（本項の主目的）**: `x_divergence` は「**areka を通らない位置書込が残っていないか**」を測る指標である。したがって、**そもそも areka を通らないことが正常であるドラッグ経路を含むセッションには当てられない**。①の 2,078 件を `x_divergence` として読み `S1-SOURCE-CUT: FAIL` と結論するのは**誤りである**。`S1-SOURCE-CUT` の `x_divergence` は**ドラッグを含まないセッション②で出す**（手順書 §7.1 が確認手順について「②が本命」と定めるのと同じ理由）。①側で S1 是正の成立を読むときは、`x_divergence` ではなく §6.5 の⑴⑷の組（`policy=ExternalAuthority`＋`applied=false` が受理と同数・`BoxStyle not found` 0 件）を用いること——こちらは①でも成立している（本節冒頭の表）。

#### 4.5 Q4 の実機ログ引用（AC 2.1 の充足）と AC 2.5 の恒等式の実測（2026-08-01 追記）

> **是正の趣旨**: AC 2.1 は Q1〜Q4 の**各問い**について「実機ログの該当行を引用した回答」を求めている。§2.3 の表は **Q4**（バルーン消失はキャラ随伴か独立か）に 1 行の主張を置いたのみで、Q3 のような**専用の小節も引用行も持っていなかった**（Q1／Q2 は §2.3.1／§2.3.2 が導出統計を持つ）。本項がその欠を補う。**§2.3 の表と既存の確定は取り消さない。**
>
> あわせて **AC 2.5 の恒等式**（バルーンとキャラ窓の相対位置が消失の前後で保たれていたこと）も §2 では**実測されていなかった**——AC 2.5 の**前件**（「バルーンの消失がキャラ窓追従の随伴である場合」）が**不成立**（消失ゼロ）だったためである。前件が成立しなくても**恒等式そのものは測れる**ので、本項が実値を与えて 2.5 に実機の数字を持たせる。

**Q4 への回答: バルーンの消失は 0 件。そのうえで随伴は実測で成立しており、キャラ窓との相対位置は DPI 遷移の前後で厳密に保存されている（バルーン独立の不具合ではない）。**

引用はいずれも 2026-08-01 の再サインオフ生ログ（`%LOCALAPPDATA%\areka-diag\20260801-172820\`）。

**⑴ セッション②c（`session2c-osdpi.log`）・scope 0——`DpiReproject`(char) の直後に `BalloonFollow`(balloon) が続く**

```
2026-08-01T08:38:12.456839Z DEBUG actor{actor=emo-text}: areka::placement::diag: [diag.window_move] route=DpiReproject entity=4v0 kind=char scope=0 x=2216 y=883 w=573 h=821 dpi=144
2026-08-01T08:38:12.456953Z DEBUG actor{actor=emo-text}: areka::placement::diag: [diag.window_move] route=BalloonFollow entity=3v0 kind=balloon scope=0 x=1730 y=413 w=- h=- dpi=144
2026-08-01T08:38:15.739494Z DEBUG actor{actor=emo-text}: areka::placement::diag: [diag.window_move] route=DpiReproject entity=4v0 kind=char scope=0 x=2120 y=610 w=764 h=1094 dpi=192
2026-08-01T08:38:15.739522Z DEBUG actor{actor=emo-text}: areka::placement::diag: [diag.window_move] route=BalloonFollow entity=3v0 kind=balloon scope=0 x=1730 y=413 w=- h=- dpi=192
```

- 随伴の成立: char の書込と balloon の書込が **114µs／28µs** しか離れていない＝同一フレームの連鎖である（`route=BalloonFollow` は `resize_window_to` の手順 7 からのみ出る）。**balloon 側が単独で動いた記録は 1 件も無い。**
- **相対位置の実測**（接地点＝下端中央基準。`ground = (x + w/2, y + h)`・整数除算）:

| dpi | char の接地点 | balloon の左上 | **接地点基準の相対** | 左上基準の相対（参考） |
| --- | --- | --- | --- | --- |
| 144 | `(2216+286, 883+821)` = **(2502, 1704)** | (1730, 413) | **(−772, −1291)** | (−486, −470) |
| 192 | `(2120+382, 610+1094)` = **(2502, 1704)** | (1730, 413) | **(−772, −1291)** | (−390, −197) |

**接地点基準の相対位置は (−772, −1291) で完全一致＝保存されている。** 一方**左上基準の相対は一致しない**——キャラ窓の寸が 573×821 ↔ 764×1094 と変わるためである。これは欠陥ではなく設計どおりで、`resize_window_to` の手順 6（`crates/areka/src/placement/follow.rs:1163`・`d_origin_x`／`d_origin_y` による `BalloonFollow.offset` の付け替え・`:1182-1183`）が「原点＝下端中央からの相対」を不変に保つために左上基準 offset を寸差ぶん付け替えているからである（記憶〈キャラ窓の原点は下端中央〉）。

**⑵ 同セッション・scope 1（もう 1 体でも同型）**

```
2026-08-01T08:38:12.494067Z DEBUG actor{actor=emo-text}: areka::placement::diag: [diag.window_move] route=DpiReproject entity=6v0 kind=char scope=1 x=1210 y=1104 w=504 h=600 dpi=144
2026-08-01T08:38:12.494102Z DEBUG actor{actor=emo-text}: areka::placement::diag: [diag.window_move] route=BalloonFollow entity=5v0 kind=balloon scope=1 x=1482 y=787 w=- h=- dpi=144
2026-08-01T08:38:15.793129Z DEBUG actor{actor=emo-text}: areka::placement::diag: [diag.window_move] route=DpiReproject entity=6v0 kind=char scope=1 x=1126 y=904 w=672 h=800 dpi=192
2026-08-01T08:38:15.793179Z DEBUG actor{actor=emo-text}: areka::placement::diag: [diag.window_move] route=BalloonFollow entity=5v0 kind=balloon scope=1 x=1482 y=787 w=- h=- dpi=192
```

接地点は dpi=144 で `(1210+252, 1104+600)` = **(1462, 1704)**、dpi=192 で `(1126+336, 904+800)` = **(1462, 1704)**。balloon は両方とも (1482, 787) ゆえ**接地点基準の相対は (20, −917) で一致**。

**⑶ セッション①（`session1-drag.log`）・ドラッグでモニタを跨いだ場合でも保存される**

```
2026-08-01T08:29:36.448693Z DEBUG actor{actor=emo-text}: areka::placement::diag: [diag.window_move] route=DpiReproject entity=4v0 kind=char scope=0 x=-300 y=974 w=573 h=821 dpi=144
2026-08-01T08:29:36.448776Z DEBUG actor{actor=emo-text}: areka::placement::diag: [diag.window_move] route=BalloonFollow entity=3v0 kind=balloon scope=0 x=-1248 y=421 w=- h=- dpi=144
2026-08-01T08:29:38.975200Z DEBUG actor{actor=emo-text}: areka::placement::diag: [diag.window_move] route=DpiReproject entity=4v0 kind=char scope=0 x=-331 y=610 w=764 h=1094 dpi=192
2026-08-01T08:29:38.975293Z DEBUG actor{actor=emo-text}: areka::placement::diag: [diag.window_move] route=BalloonFollow entity=3v0 kind=balloon scope=0 x=-1183 y=330 w=- h=- dpi=144
```

- dpi=144（monitor1 側・work area 下端 1795）: 接地点 `(-300+286, 974+821)` = **(−14, 1795)**・balloon (−1248, 421) → 相対 **(−1234, −1374)**
- dpi=192（monitor0 側・work area 下端 1704）: 接地点 `(-331+382, 610+1094)` = **(51, 1704)**・balloon (−1183, 330) → 相対 **(−1234, −1374)**

**モニタが変わり接地点の絶対座標が (−14, 1795) → (51, 1704) と動いても、相対位置は (−1234, −1374) のまま 1px も変わらない。** これが **AC 2.5 の恒等式の実機値**である（Req 4.4「窓寸が変わってもバルーンとの相対位置＝追従オフセットを保つ」の実機成立でもある）。

> **引用行の読み方の注意（Req 1.5）**: ⑶の 4 行目 `route=BalloonFollow … dpi=144` の `dpi=` は**バルーン窓自身の `DPI` component の値**であり、直上の char 行の `dpi=192` とは別物である（バルーン窓の DPI 受理がこの時点でまだ届いていない）。**同一行の `dpi=` を「相手窓の DPI」と読まないこと。** 相対位置の算定に `dpi=` は使っていない（使うのは `x`／`y`／`w`／`h` のみ）。

**結論（Q4・AC 2.5）**: バルーンの消失は両セッションとも 0 件（`VANISH-TRACE: NONE`）であり、AC 2.5 の**前件は不成立**である。そのうえで随伴の恒等式は上記のとおり**厳密に保存**されており、「バルーンだけが独立に飛ぶ」機構は実機ログ上に存在しない。なお 2026-07-31 のセッション①で開発者が目視した**バルーンの見えなくなる現象は Z オーダー起因**であって位置ではない（§2.3.2・`areka-P0-ghost-window-zorder` が所有）——本項の相対位置の保存はその判断と矛盾しない。
...
| §2 | **4.5（完了・2026-08-01）** | 実機 2 セッションの採取結果・Q1〜Q4・S1〜S3′ の痕跡。①／②-a／②-b の 3 走行で記入完了（**旧記載「セッション①のみ記入済み・②未採取」は 2026-08-01 に陳腐化**——§2 冒頭の更新注記と同旨） |
| §4 | **7.4（完了・2026-08-01）** | 是正後の実機再サインオフ（②c を含む） |
...
> **節の並びの是正（2026-08-01）**: 本節の小節は当初 **`5.2` → `5.1` の逆順**で並んでいた。番号は他節から参照されている（§1.4／§2.5.3／§3.4 が「§5.2」を、§3.3 が「§5.1」を指す）ため**番号は変えず、並び順のみ昇順へ入れ替えた**。**両小節の本文は 1 文字も変えていない**（§5 メンテ規約 2 の「上書きしない」を満たす純粋な移動である）。
