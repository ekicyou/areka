...
|---------------|------|-----------|------|-----------|
| WindowGraphics | 🔴 RED | Phase 1 → Phase 4 | WindowD3D11Compositor に置換 | Req 6.1 |
| VisualGraphics | 🔴 RED | Phase 4 | 廃止（per-entity Visual不要） | Req 6.3 |
| SurfaceGraphics | 🔴 RED | Phase 4 | 廃止（per-entity Surface不要） | Req 6.3 |
...
| 4 | `create_surface_for_visual` | 🔴 RED | Phase 4 | 廃止 |
| 5 | `draw_recursive` (dead_code) | 🔴 RED | Phase 4 | 削除 |
...
| 10 | `init_window_visual` (deprecated) | 🔴 RED | Phase 4 | 削除 |
| 11 | `sync_surface_from_arrangement` (deprecated) | 🔴 RED | Phase 4 | 削除 |
...
| VM3 | `create_visual_only()` | 🔴 RED | Phase 4 | 削除 |
| VM4 | `visual_resource_management_system` | 🔴 RED | Phase 2 | 削除 |
| VM5 | `window_visual_integration_system` | 🔴 RED | Phase 2 | 削除 |

**要約**: 24関数中 **14関数がRED（廃止/置換）**、3関数がYELLOW（改修）、7関数がGREEN（維持）

### 1.3 ファイル分類

| ファイル | 分類 | 所属子仕様 | 対応 |
|---------|------|-----------|------|
| `com/dcomp.rs` (315行) | 🔴 RED | Phase 4 | ファイル全体削除 |
| `ecs/graphics/visual_manager.rs` (170行) | 🔴 RED | Phase 4 | ファイル全体削除 |
| `ecs/graphics/core.rs` | 🟡 YELLOW | Phase 2 | DComp初期化除去 |
| `ecs/graphics/components.rs` | 🔴 RED / 🟢 GREEN混在 | Phase 1-4 | WindowGraphics→WindowD3D11Compositor置換、VisualGraphics/SurfaceGraphics削除 |
...
| `examples/dcomp_demo.rs` | 🔴 RED | Phase 4 | 削除 |
...
- **Phase 3 → Phase 4**: Phase 4 は ULW パイプラインが完全稼働（Phase 3 完了）を前提に旧コード削除する
...
| DComp コード未使用 | `com/dcomp.rs` は存在するが一切参照されない | `use` 文がゼロ（`com/dcomp.rs` 自体以外） |
...
Phase 4 (クリーンアップ)
└── wintf-dcomp-migration-4-dcomp-removal     ← Phase 3 に依存
```
...
| Phase 4 | `cargo test` 全パス + `cargo build --examples` 全パス + DComp参照ゼロ | CI/grep検証 | Req 10.1 |
...
| 4 | `wintf-dcomp-migration-4-dcomp-removal` | DCompコード削除・クリーンアップ |
...
| Req 5 | GraphicsCore簡素化 | Phase 2 | §3.3 Phase 2契約 |
