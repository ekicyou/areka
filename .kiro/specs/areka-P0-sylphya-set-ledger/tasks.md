...
- [x] 1.5 件数の導出と、正典に根拠のない仕分けの作りを説明文に書き残す
...
- [x] 2. 本機能が足した項目を項目ごとに判定する兄弟テストの新設
- [x] 2.1 兄弟テストを接続し、件数・集合・既存分の順序を判定する
...
- [x] 2.3 仕分けが記録用の表を読まないことと、URL 注記の本数を構造として判定する
...
- [x] 3. 先送りを守る決定論テストの走査対象名簿へ記録用の表を登記する
