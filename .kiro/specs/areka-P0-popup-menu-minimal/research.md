...
4. **`popupmenu.type` を照会するか**: 要件 3.8 は「値で変えない」。照会して捨てる（台帳「縮退」・往復 1〜3 件増）か、照会しない（台帳「語彙のみ」のまま・要件 10.3 の「縮退」表記を「照会しない」へ改める）か。
5. **`char{n}`（n≧2）の運び方**: `EventId` に第 3 出所（文字列＋接尾辞規則）を足すか、α は scope {0,1} 固定で `Static` 2 名だけ持ち n≧2 を語彙のみとするか（`char_scope` の `debug_assert!(scope <= 1)` と整合）。
