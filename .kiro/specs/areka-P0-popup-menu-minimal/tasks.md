...
**最終検証（2026-09-19）で足したこと**: ⑴ 要件 4.4（開けなかった失敗の記録）に決定論テストが無かったので `readme_tests.rs` に 1 本足した（実在しないファイルを渡す＝机では何も開かず、`ShellExecuteW` は符号 2 を返すだけ。「`readme::open` へ届くテストは書かない」の規律は**成功側**に限る）。⑵ wintf の「`JustEnded` は 1 フレームだけ」という説明と実装の食い違い（`reset_to_idle` の呼び手が製品に無い）は wintf 側の欠陥で、本仕様の範囲外。別セッションの作業候補として切り出した（本仕様の是正はどちらの直し方でも動き続ける）。⑶ 返事待ち中に**別の窓**で右ダブルクリックが預けられた場合、`poll_once` は預かりの scope を要求の scope と比べずに送りうる（1 秒未満の返事待ちの間に別の窓で 1 クリック＋押下が要る＝実質届かないので是正しない・記録のみ）。⑷ main が 1 コミット進んでいる（PR #157）。取り込むと `doc/ukadoc-coverage/` の報告 2 本と `roadmap-draft.md` が衝突し、台帳・`briefing.md`・steering `roadmap.md` は黙って重なる＝`/kiro-complete` の取り込み時に報告を作り直し、手書きの数を道具で数え直すこと。
