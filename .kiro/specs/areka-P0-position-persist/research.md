...
8. **起動記録の書込タイミング（軸C 後半）**【要件ディスカッション #2 で決着済み（2026-07-24）——**C3（トーク末尾マーカー方式）採用**】: 「完了」＝**初回挨拶トークの再生完走**と定義（R3.4 に明記済み）。C1（Steady 到達監視の新 Action）は boot 運行全体の完了監視を増設し系の負荷を上げるため不採用、C2（eager）は挨拶完走保証を欠くため不採用。採用形＝**初回挨拶スクリプトの受付処理（OnFirstBoot 応答の CueSheet 化）で、末尾に「初回起動完了」を示す内部キューコマンドを埋め込み、再生時のマーカー消費を契機に `persist_put(BootCount)` を実行**する。既存規律と整合: `\!` 汎用キャリア 1 本・消費側 name 選別（typed 個別新設禁止）／cue 再生制御は dola 層に集約済み（マーカー消費は既存の cue 消費経路に乗る＝新たな監視者不要）。設計で確定する残件: (i) マーカーの name と埋め込み位置（sakura コンパイル後の CueSheet 末尾 vs スクリプト末尾追記）、(ii) マーカー消費→persist_put の消費者の置き場（sink 同型）、(iii) **204 フォールスルー時（初回挨拶トークが存在しない場合）の記録時機**（挨拶が無い＝完走保証の対象も無いため、OnBoot 側トーク末尾 or 204 確定時点の eager のいずれかを設計で裁定）、(iv) BootCount の値意味論（M1 のゲート条件は「値の存在」で足りる・増分は将来語彙＝過剰実装をしない線引き）。
