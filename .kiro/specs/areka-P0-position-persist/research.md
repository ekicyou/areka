...
4. **Free アンカー窓の DragEnd 観測**: 現行 spawn は非 Free のみ `OnDragEnd` を結線（spawn.rs:230-234）。Free 窓の位置保存をどう観測するか——(a) Free にも OnDragEnd を結線（ハンドラ内で Free は wndproc 確定位置を読むだけの保存専用アーム）、(b) M1 の実ゴースト（emo2）は Bottom 吸着ゆえ Free 保存は将来領分として明示スコープ外。emo2 実機で Free が出ない事実と要件 R1.1（「キャラ窓」一般）の整合を確定。
