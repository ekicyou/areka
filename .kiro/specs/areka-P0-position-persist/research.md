...
8. **起動記録の書込タイミング（軸C 後半）**【要件ディスカッション #2 で決着済み（2026-07-24）——**C3（トーク末尾マーカー方式）採用**】: 「完了」＝**初回挨拶トークの再生完走**と定義（R3.4 に明記済み）。C1（Steady 到達監視の新 Action）は boot 運行全体の完了監視を増設し系の負荷を上げるため不採用、C2（eager）は挨拶完走保証を欠くため不採用。採用形（2026-07-24 追補で格上げ）＝**汎用「プロパティ SET」キューコマンドの新設**。本質は「初回起動完了」という**プロパティを作る**ことに尽きるため、キュー語彙は boot 専用マーカーではなく**汎用のプロパティ SET キュー**（key と値を運ぶ・`\!` 汎用キャリア上の name 選別＝typed 個別新設禁止の規律どおり）とし、初回挨拶スクリプトの受付処理（OnFirstBoot 応答の CueSheet 化）で**末尾に BootCount への SET キューを埋め込む**。再生時の SET キュー消費者が統一プロパティシステムへ書込（永続 key なら persist_put へ到達）する。既存規律と整合: 「名前で引ける値は 1 機構」（sylphya）の書込側対応物／cue 再生制御は dola 層に集約済み（消費は既存の cue 消費経路に乗る＝新たな監視者不要）／**将来消費者（M2 `\![vanish]` の vanish 増分・ゴースト切替）が同じ SET 語彙の name 違いでそのまま乗れる**。設計で確定する残件: (i) SET キューの name・引数形式（key/値の運び方・増分 vs 代入の語彙）と埋め込み位置（sakura コンパイル後の CueSheet 末尾 vs スクリプト末尾追記）、(ii) SET キュー消費者の置き場（sink 同型）と書込可能 key の統制（任意 key 書込を許すか・persist 対象 key 族への限定か）、(iii) **204 フォールスルー時（初回挨拶トークが存在しない場合）の記録時機**（挨拶が無い＝完走保証の対象も無いため、OnBoot 側トーク末尾 or 204 確定時点の eager のいずれかを設計で裁定）、(iv) BootCount の値意味論（M1 のゲート条件は「値の存在」で足りる・増分は将来語彙＝過剰実装をしない線引き）。
