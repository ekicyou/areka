...
3. **現象消失の grep（8.1/8.3）**: `あふれ発火` （`layout.rs` `visible_window` の既存 debug marker）が **0 件**。正当なあふれ（長文トーク）が観測された場合の縮退判定: `ratio=1.5` の NewLine 適用後、同一 actor 宛て後続 `Text cue 適用` が無い区間で `あふれ発火` が無いことを窓判定する。
