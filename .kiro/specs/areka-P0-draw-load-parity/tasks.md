...
- [x] 7.3 1 入口の計測サブコマンド（perf-loop.ps1: measure-baseline／rank-run／rank／prepare-ab／measure-ab／compare／followup／final）
...
- [x] 7.5 1 周の手順スキル（perf-loop-iteration）
...
- [ ] 9.2 ベースラインと周 1（tick の門の A/B）
  - _Blocked: 環境起因（2026-08-23 kiro-impl 自律実行で確定）。本セッションでは ⒜ 利用者の VS Code が起動する `rust-analyzer`（pid 32516・遊休）が目標定義 `[quiet].heavy_process_names` に当たり、静寂確認が 4/4 回 NOT_QUIET → `measure-baseline` が exit 2（根拠: `%LOCALAPPDATA%\areka-diag\perf-loop\draw-load-parity\baseline-20260823\release-FAILED\quiet-before.txt`）。利用者のエディタのプロセスは止めない。⒝ 非対話セッションのため `SetCursorPos`/`SendInput` が ACCESS_DENIED → 追随チェックの clickthrough/drag/balloon_follow が INCONCLUSIVE＝設計 C13 により 1 周も採用できない。⒞ 非昇格のため段③（関数別）は UNAVAILABLE。解除条件: **対話デスクトップかつ昇格した PowerShell から起動した Claude Code セッション**で、⑴ セッションを起動する前に環境変数 `CLAUDE_CODE_GOAL_CHECKIN_MINUTES=60` を設定する、⑵ DPI の違うモニタが 2 面ある（1 面しか無い機械では `dpi` が永久に判定不能＝1 周も採用できないので、開発者の判断で目標定義の `[followup] required` から `dpi` を外す＝`tools/perf/README.md` §17）、⑶ 前回の走行が残した `areka.exe` と稼働中の `python` が無い、の 3 点をそろえてから `/goal` に `results/goal-text-2026-08-23.md` の本文（トークン 87696907）を貼る。⒜ の遊休 `rust-analyzer` は 2026-08-23 の是正で解除済み——`check-quiet.ps1` は名前の有無ではなく採取窓の間に使った CPU（`[quiet].heavy_process_cpu_min_pct` 既定 1.0%）で見るので、止めなくてよい。台帳は `WAIT_BASELINE`（pending_run=…\baseline-20260823\release）で待機中＝スキルの再起動規則で `measure-baseline -Build release -Date 20260823 -Resume` から続く。**2026-08-23 再確認（2 回目の kiro-impl 起動時）**: 対話デスクトップのセッションでは `SetCursorPos` が成功（err=0）＝⒝ は解除・モニタ 2 面を検出。残る阻害は ⒜' 機械が静かでない（利用者の `evernote_export` 比較ジョブ 2 本で CPU 10〜12%・台帳 `not_quiet_retries=1` につき次の NOT_QUIET で計測失敗扱い）と ⒞ 非昇格（段③のみ欠く＝exit 5 でループは止まらない）。起動は設計どおり `/goal`（Opus 5・`perf-loop-iteration`）で行い、kiro-impl からは回さない。_
