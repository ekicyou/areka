...
> - **⑻ 正本**: `.kiro/specs/areka-P0-dpi-transition-atomicity/`（要件 10.3／10.4／10.6・design「Components and Interfaces > C2 `command.rs` 合流＋flush 観測」・`mechanism-ledger.md` が file:line の正本）。
...
> - **⑺ 正本**: `.kiro/specs/areka-P0-dpi-transition-atomicity/mechanism-ledger.md` **§10**（§10.3 が名指しの 3 量・§10.5 が候補 4 案の突合と採否・§10.6 が実装への受け渡し）／`design.md` の「C8 原子性の段階裁定」。生ログは `C:\Users\maz-o\AppData\Local\areka-diag\atom-71-recapture-1\atom-signoff.log`（リポジトリ外・複製しない）。
>
