...
- **Out of scope**:
  ...
  - SSP との完全一致（画素数が違えば同じ土俵ではない可能性がある）
...
8. If 更新後のベースラインで負荷の最大項がフレーム駆動（tick）以外にある, the 開発プロセス shall 是正に入らず、最大項の所在と本 spec の境界（In／Out）の再裁定を開発者へ提示する

### Requirement 2: 比較の前提の確定と目標の置き方の裁定

**Objective:** As a 開発者, I want SSP の描画解像度を実測で確定し、その上で「SSP 同等圏」を CPU の絶対値で置くか画素あたりの効率で置くかを裁定したい, so that 目標が暗に品質低下（引き伸ばし表示）を要求していないことを確かめてから是正する

#### Acceptance Criteria

1. When SSP との比較を行う前に, the 計測 shall 同一マシン・同一ゴースト emo2・同一拡大率（200%）で SSP の実描画の画素数（100% で描いて引き伸ばしているか、200% で描いているか）を実測で確定し、推定のままにしない（開発者観察: SSP のバルーン文字がぼやける。事実なら areka 764×1094 に対し SSP 382×547 で画素の仕事量は 4 倍、CPU 3.6 倍は画素あたり 0.9 倍）
2. When SSP の描画解像度が確定した, the 開発プロセス shall 目標の置き方 2 案——(A) CPU の絶対値（release アイドル CPU 3.0% 未満＝先行 spec 4.4 の継承）／(B) 画素あたりの効率（実描画画素数で正規化した CPU を SSP と比べる）——を開発者へ提示し、裁定を得てから Requirement 6 の合格判定式を確定する
3. While 裁定が出ていない, the 判定スクリプト shall 現行の較正値（`IDLE_CPU_MAX_RELEASE_PCT = 3.0`・狭義の未満比較）を既定として保持し、(B) の値は参考として併記する
4. When 裁定が (B) になった, the 判定スクリプト shall 正規化の式・SSP 側の実測値・採取日を較正値として明記し、較正値の変更履歴を残す
5. When SSP と同一手順で比較する, the 計測 shall 同一マシン・同一ゴースト・同一拡大率・同一走行長（25 分）・同一採取間隔で、areka 側を現行ツリーで再採取する（brief の 10.97% は更新前）
6. The 開発プロセス shall 裁定結果（絶対値か画素あたりか・SSP 描画解像度の実測値）を `doc/COMPAT_ARCHITECTURE.md` へ「areka 裁量の性能目標」として登記する（互換台帳にフレームレート／アイドル CPU の SSP 互換規定は現状存在しない）
...
9. When 較正値（`WARMUP_EXCLUDE_SEC` ほか）を見直す, the 判定スクリプト shall 見直しの根拠（容量 3 で暖機が 4 倍に伸びた実測など）を較正値の注記に残し、自己較正 fixture で既知ケースの合否が変わらないことを確かめる
...
1. The 開発プロセス shall SSP 側の配置手順（emo2 を SSP へ配置する場所・拡大率の設定・採取間隔・走行長・測定後の削除）を `tools/perf/README.md` に登記する
2. When SSP 比較の測定が終わる, the 開発プロセス shall SSP 側に配置したゴーストを削除し、削除したことを記録に残す
...
5. If 本 spec の途中で別 spec の担当ファイルへの変更が必要になった, the 開発プロセス shall 担当 spec の実在を確認して即報告し、勝手に境界を広げない
