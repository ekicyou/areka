...
> 本書の件数・定義名はすべてこのワークツリー（`claude/areka-p0-ukadoc-survey-sakura-script` 相当の作業ブランチ・`origin/main` に追従済み）で 2026-09-04 に数え直した。備考に行番号は書かない規律（要件 2.11）に合わせ、本書も判断の根拠は**定義の名前**で示す。表の中の行番号は設計時点の所在を示すためだけのもので、成果物へは写さない。
...
**変わること**: 成果物 2 本が増え、実装済みと判定した項目の定義箇所に正典 URL の 1 行コメントが約 23 行加わる。実行時の判断は 1 つも変わらない。
...
- `doc/ukadoc-coverage/ledger/sakura-script.toml` — `list_sakura_script` の 342 id だけを収める台帳 1 本。編集権は本 spec が単独で持つ（1.1・1.3・12.2）。
...
- 道具の着地後の `doc/ukadoc-coverage/report/sakura-script.md` の再生成（11.1）。着地前は作らない（11.2）。
...
`%` は別系統で、`sysvar.rs` の `resolve_system_var` が値を作り、`crates/areka-sylphya/src/vocab/flat.rs` の `FLAT_VOCAB`（26）と `SYNTAX_RECORDS`（2）が名前だけを並べる（実行時の解決には使われない）。本ドメインで「名前は登記されているが何も起こらない」に当たるのは、この表と `doc/COMPAT_ARCHITECTURE.md` §8 の「語彙・意味論のみ記録」の行だけである（2.5）。

### 生産の流れ

```mermaid
flowchart TD
  SNAP[ukadoc スナップショット index.json] -->|付録 B の手順| SKEL[骨組み 342 項目 status=unclassified]
...
上流 toolkit の検査のうち、本 spec 単独では確かめられないものが 2 つある。ブリーフィングの末尾に注記し、本 spec の完了判定には使わない（要件 1 の付随注記・10.7）。

| 上流の検査 | 内容 | 本 spec での扱い |
|---|---|---|
| 6.4 | カタログの全 id が 4 つの台帳のいずれか 1 つにちょうど 1 回だけ現れる | 他 3 台帳が揃うまで確かめられない。本台帳側の 342 件の網羅は V1・V2 で確かめる |
| 6.7（両端実在） | 関連の両端の id が実在する | 相手がカタログに実在することは V7 でスナップショットに直接当てて確かめる（要件 6.7 が「書く前に確かめる」を求めるため。他台帳の完成を待たない） |

上流の検査 6.6（`implemented` に証拠がある）・6.7 の別名連鎖禁止・6.7 の版番号所属は、本 spec 側で V6・V10・V11 として先取りする。
...
この判定は 105 件へ機械的に当たり、**14 件が `alias`** になる。`\_b` は `x,y` 族と `inline` 族の 2 本の鎖に割れて 4 つの根（正典）と 2 つの別名を出す——要件 4.5 の 2 つの記述が両方とも成り立つ形である。`\![execute,http-get]` のように置き場所の語が違う（`URL,パラメータ` と `URL,オプション,…`）ものも接頭辞にならないので、根が 2 つ残る。根が 2 つ以上ある群は、その事実と兄弟の id を備考に書く。

本文の注記で向きが決まる 7 件（要件 4.2）はこの判定の**前**に当て、置き換え先が本文に明示されている 2 件（`\![reloadsurface]`→`\![reload,shell]`・`\z`→`\e`）を `alias` にする。**`alias_of` の相手は必ず鎖の根（自身が `alias` でない id）にする**（要件 4.7 の連鎖禁止）。旧書式 `\q[ID][タイトル]または\q*[ID][タイトル]` も同じ扱いで、指す先は `\q` の鎖の根であって、`alias` である `\q[タイトル,ID]` ではない（要件 3.5）。
...
「M1 未実装（語彙・意味論のみ記録）」と明記した行（時間指令の allowlist・`\x`／`\x[noclear]`・`\f[align]`／`\f[valign]` など）は `vocabulary-only` の根拠であって `degraded` ではない（要件 2.5 の 2 つ目の登記）。
...
| 2 | DD-2 の判定で別名になった、または本文の注記が旧仕様と述べ置き換え先を明示している | `alias` | **16** |
...
| 5 | `crates/areka-sylphya/src/vocab/flat.rs` の `%` 語彙表に名前がある、または §8 が「語彙・意味論のみ記録」と明記して名指ししている | `vocabulary-only` | **34〜38** |
| 6 | 上のいずれにも当たらない | `absent` | **261〜265** |
...
- 見込みの合計は 1＋16＋3＋23＋（34〜38）＋（261〜265）＝342。確定値は V4・V5 と納品時の集計で置く。

### 規則 3: `introduced`（要件 4.9〜4.11）

1. その項目の本文へ DD-9 の Q6 の抽出規則を当て、拾えた版番号の集合を作る。
2. 集合が空なら `""`（世代不明。最古と決めつけない）。ただし本文が 2 桁の版だけを書いている 5 件（`\![sound,play|load|option]` の `2.4`・`\__v[オプション]` の `2.4`・`\![biff(,アカウント名)]` の `2.5`・`\![execute,resetwindowpos]` の `2.7 RC1`・`\![move]` の `2.5`）は、備考に「本文は `2.4` と書くがカタログの規則で拾えない」の形で本文の版を写し、単なる世代不明と区別する。2 桁を拾う規則を上流へ提案しない。
...
| 順 | 条件 | 結果 |
|---|---|---|
| 1 | 本文が旧仕様と述べ、置き換え先も本文が名指ししている（`\![reloadsurface]`→`\![reload,shell]`・`\z`→`\e`） | `alias`。`alias_of` は相手の鎖の根 |
...
| 順 | 条件 | owner |
|---|---|---|
| 1 | `status = "alias"` または `not-applicable` | `""`（作業が残らない） |
| 2 | 2 本以上が主張し、**互いに相手を知らない**（実測 1 件＝`\![embed,イベント名,r0,r1,r2...]` を `sakura-time-directives` と `property-query-channels` が別々に主張） | `""`（裁定待ち）。両者の主張をブリーフィング ⑷ 節に並べ、裁定案を 1 つ添え、それが案であって決定ではないことを明記する |
| 3 | 2 本以上が主張するが**分担が合意済み**（互いを名指ししている、または §8 に住み分けが登記されている。実測 `\![set,balloontimeout,時間]`・`\![set,choicetimeout,時間]`・`\![move]`） | まだ作業が残っている側の spec 名。相方と分担の中身を備考に書く（二重所有として扱わない） |
...
|---|---|---|---|
| 1 | `status` が `implemented`・`alias`・`not-applicable` | `""` | 作業が残らない |
| 2 | 「書いてあるのに何も起きない」うえに**引数が台詞へ漏れる**（壊れ方 ⑶。`\sID番号` の族・短縮形の語でない 1 文字タグに数字が続く形） | `"C10"` | ⑴ 黙って壊れるうえ、台詞そのものが崩れて利用者の目に見える |
...
| `crates/areka/src/emo2_boot/zorder_cue.rs` の名前・選択子の定数 | `\![set,zorder,スコープID,スコープID,...]`・`\![reset,zorder]` | 2 |
| `crates/areka-sakura/src/sysvar.rs` の `resolve_system_var` の既定値の腕 | `%username` | 1 |
| `crates/areka-ghost/src/sylphya_wiring.rs` の `derive_flat_statics` の 3 分岐 | `%selfname`・`%selfname2`・`%keroname` | 3 |
| 合計 | | **23** |
...
```
doc/
└── ukadoc-coverage/                    # 本 spec が最初に作る（現時点で repo に不在）
    ├── ledger/
    │   └── sakura-script.toml          # 台帳 342 項目（新規・本 spec の単独所有）
    └── briefing-sakura-script.md       # ブリーフィング 7 節＋末尾注記（新規・本 spec の単独所有）
```

`doc/ukadoc-coverage/report/sakura-script.md` は道具が着地するまで**作らない**（要件 11.2）。着地していれば台帳から再生成して同時にコミットし、手で編集しない（要件 11.1・11.3）。`values.md`・`README.md`・`catalog.toml`・`linkage.md`・`report/summary.md`・他 3 台帳は作らず触らない（要件 11.5）。
...
- `crates/areka/src/emo2_boot/zorder_cue.rs` — 名前・選択子の定数の直上に `///` を 2 行。
...
作業用ディレクトリは `crates/` にも `doc/` にも `.kiro/` にも置かない。既定は `%TEMP%\areka-ukadoc-sakura\`。3 本の使い捨ての台本（`gen_skeleton`・`gen_tables`・`check_ledger`）と、段の間で受け渡す中間ファイルを置く。**コミットしない。** 理由は 2 つ——⑴ カタログ生成・初期台帳生成・常設の整合検査は上流 toolkit の持ち物であり、同じものを本 spec が repo に置くと二重になる ⑵ 上流 要件 3.3a により、道具が後から初期台帳を生成しても本 spec が書いた項目は書き換えられない。台本が失われても、生成規則（規則 0）と検査項目（V1〜V15）が本書に全部書いてあるので同じ結果を再現できる。

道具が作業期間中に着地した場合は、上流の初期台帳の上へ書き足す形に切り替えてよい。切り替えても成果物と決定表は変わらない。
...
| ⑷ 所有者なし／裁定待ちの一覧 | `owner` が空の id を理由別（所有者なし／裁定待ち）に並べる。`\![embed,...]` の裁定案を 1 つ添え、案であって決定ではないと明記する | 8.4・8.6 |
...
|---|---|---|
| 1 | 規則 0 で骨組みを生成し、各 id の本文から版番号の集合を拾う | `skeleton.toml`（342 項目・`status = "unclassified"`）・`versions.tsv`（id／版番号の集合） |
...
| 10 | V1〜V15 を回し、結果をブリーフィングの末尾に記録する | `check-report.txt` → ブリーフィング末尾 |

**段 9 を段 8 より前に出さない。** URL コメントは台帳の従属物であり、`status` が動けば置き場所も動く。**段 9 の前に既存テストの基準線を取る**（要件 9.8）。

## 検証計画

上流の道具が無くても走る決定論の検査を、作業用の台本 1 本（`check_ledger`）にまとめる。`crates/` にテストを足さない。すべて入力が同じなら結果が同じで、ネットワークにも実機にも依存しない。**1 度だけ回し、結果をブリーフィングの末尾に書き残す**（0 件で緑になったのか検査が働いて緑になったのかを読者が区別できるよう、検査した件数も並べて書く）。
...
| V9 | `priority` が `^[A-E][0-9]+$`。`implemented`・`alias`・`not-applicable` は `""`。90 より大きい数値が無い | 7.5 |
...
| V14 | `git diff --name-only`（比較元は分岐点の `origin/main`）の変更対象が `doc/ukadoc-coverage/` 配下・上記 5 ファイルの URL コメント・本 spec 自身の `.kiro/specs/areka-P0-ukadoc-survey-sakura-script/` だけ。`doc/COMPAT_ARCHITECTURE.md`・`.kiro/steering/roadmap.md`・他 3 台帳・11 本の brief に差分が無い（**パスの実在を先に確かめてから差分を取る**——実在しないパスへの `git diff` は空を返して「差分なし」と区別が付かない） | 8.9・11.5・12.1〜12.4 |
| V15 | 段 9 の前後で `cargo test --workspace` を 1 度ずつ走らせ、結果が同じであること（i686 の host-32 成果物を先にビルドしておく）。完了直前に `doc/ukadoc-coverage/catalog.toml` の有無を確かめ、着地していれば報告の再生成を足す | 9.8・11.1・11.2 |

**道具そのものを較正する**（緑は台本が壊れていても出る）。V1〜V12 は、それぞれ「わざと 1 か所壊した写し」で赤になることを 1 度は確かめてから本番に当てる。例——id を 1 つ削って V1・V2 が赤になるか、`status` を綴り違いにして V5 が赤になるか、`alias_of` を別名へ向けて V6 が赤になるか、URL の末尾を 1 文字変えて V11 が赤になるか。
