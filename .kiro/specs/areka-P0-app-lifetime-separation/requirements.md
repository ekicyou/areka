...
7. When 終了の指示を出す, the areka shall どの終了操作から来たか（終了系列の完了・強制退避・ダミー窓・smoke・ダミー窓への OS の閉鎖要求）を記録に残す（ライフサイクル事象として info）。
...
10. When ゴースト窓が OS の閉鎖要求を受ける and kanade への送り口（`MouseWiring`）が World に無い, the areka shall 終了要求を送らず、その旨を記録に残す（メニューの「終了」と同じ扱い）。
