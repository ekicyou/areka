...
| **DD8** | 追跡先の双方向登記の検証方法 | **実装時点の実在確認＋項目列挙の突合を design.md／tasks の検査項目として持つ**。spec 文書を `include_str!` で読む檻は**作らない** | `/kiro-complete` のアーカイブ移動が「コードからの spec 文書実ファイル読み」を壊す既知の穴があり（PR#114 で 5 件が main で赤のまま放置された）、追跡先 4 本はいずれも M2 ゲートで brief が requirements へ置き換わる。文書間の登記は文書で検査するのが正しい |
...
**Implementation Notes**
- Integration: §8 は `include_str!` による保護もテストも無い（2026-08-27 実測・0 件）。したがって檻は作らず、レビューで担保する（DD8）。
- Validation: 上表の再検証（file:line の陳腐化はこのリポジトリで通算 8 度踏まれている——**着手時に必ず引き直すこと**）。
...
- 新規テストモジュールは兄弟ファイルへ置き、**`lib.rs` の `PURE_SOURCES` へ追加する**（`structure.md`「include_str! で本文を読む構造テストは兄弟テストファイルも走査対象に列挙する」——列挙しないと被覆が黙って縮む）。
