...
2. **animation-sort（ascend/descend）の扱い**: bind 静的合成の順序は「animation-sort → ID 昇順」の2段が正典（brief）。**ただし現行 areka-parsers::shell::Animation に animation-sort フィールドは無い**（model.rs 確認）。emo2 が同キーを使うか要確認。未使用なら既定 ascend（ID 昇順）で可＝**emo-compose は現行モデルで完結**。使う場合はパーサ拡張が要り**スコープ跨ぎの design 判断**。
