...
- `/kiro-requirements-discussion areka-P0-windowposition-limit` で §6 の設計判断アイテムを裁定（特に 1〜4）。→ **完了（§9）**
- ~~SSP 実測（R3）は設計フェーズ冒頭のコード非接触作業として実施可能~~ → **SSP 実測は廃止（§9 裁定）**
- `/kiro-design areka-P0-windowposition-limit` へ。

## 9. 要件ディスカッション決着（2026-08-14・開発者裁定）

要件ディスカッションで旧 R3（SSP 実測義務）が廃止され、本書の前提が一部更新された：

1. **`windowposition.limit` は SSP 系仕様（SSP 2.5.10 追加）と裁定**——他ベースウェアへの互換追従義務なし。SSP 実測 3.1(a)〜(f) は全廃し、areka 裁定へ差し替え（新 R3.1）：適用は**常時不変量**（全書き込み経路）／バルーンドラッグはドラッグ中自由・**解放時補正**／制限領域は**キャラ窓帰属モニタの作業領域**／補正は**相対位置へ焼き付けない**。
2. **本書への影響**：
   - §3.1 論点 A：「適用時点表」の観測待ちは解消。常時不変量ゆえ **§1.2 の全 7 経路が補正対象**（経路 6 wndproc ドラッグは「解放時」に掛ける）。案 A-1/A-2/A-3 の比較自体は設計判断として有効なまま。
