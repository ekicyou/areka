...
- **doc の行番号アンカーは分割で総崩れになる**（実害を確認）。`main.rs` を 1,005→871 行へ分割した際、本仕様が書いた 3 箇所だけでなく**本仕様と無関係な既存 doc 17 箇所**（`input_events/balloon.rs` 16・`emo2_boot/frame/wiring.rs` 3・`wintf` の `window_pos_tests.rs` 1）も同時に陳腐化させていた。全て関数名参照へ置換済みで、`main.rs` + コロン + 行番号 形式のアンカーはリポジトリからゼロになった。
