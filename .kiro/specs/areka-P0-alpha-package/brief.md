...
- **Out**: 検証項目表と第三者の手順 12 項目の実機一周・README の仕上げ・署名と宣言（#17）／**既定ゴーストの差し替え**（議題 ⑶ で emo2 を入れられないと決まった場合も、`boot_resolve.rs` は #13・#50 と共有するので #17 で行う。本仕様はゴーストの入らない zip も組めるようにしておくだけ）／インストーラ（msi 等）・署名付きの exe・自動更新（本体の更新は α 後の予約）
...
- 第三者の資産の再配布条件を README に正しく載せる（`StayseeBalloon` は CC0、`claudia` は Unlicense、`konnoyayame` のシェルは CC BY-NC-ND＝zip に入れない）。
...
- ⑶ **emo2 を zip に入れてよいか**。`emo2` のシェルの説明書（`shell/master/readme.txt`・コンフィズリー・ゆゆぴか氏）は「フリーシェルとしての再配布」と「商用利用」を禁じ、再配布するならこのテキストを必ず同梱せよと書いている。\1 側のシェル（City-Pop'n・大槻氏）の条件は書庫に無い。入れられない場合は、既定ゴーストを `claudia`（全体 Unlicense）などへ替えるか、ゴースト無しで配って「入れてください」の告知から始めるか（差し替えの実装は #17）。見える差は「初めて起動したときに誰が立つか」。
