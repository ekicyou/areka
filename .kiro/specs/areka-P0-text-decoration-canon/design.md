...
15. 既存行の改訂: 「`\f[align]`／`\f[valign]`／下線の縦書き写像」（:181）の注記を「下線は実装済み（DirectWrite の既定・行 7）・align／valign の追跡先は `areka-P0-text-align-shadow-canon`」へ、追跡先欄の「`\f` 核 17 項目の所有者」を「基盤＋フォント系 10＋一括の戻し 2＝本仕様・寄せ 2＋影 3＋SC8＝align-shadow」へ。「`\_l` の縦書き座標系の正典写像」（:183）の SC8 の追跡先も同様に。
