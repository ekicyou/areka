...
- **In scope**（利用者・ゴースト作者から見える範囲）:
  ...
  - **終了の予約**: `\-` を含む台本を利用者が中断したら、ただちに終了すること。終了イベント（`OnClose`）の別れの台詞が、`\-` の有無にかかわらず必ず終了で終わること（**完了 spec `areka-P0-kanade` の要件 4.5「close 握手中の talk が quit=false で完了したら終了させず定常運転へ復帰する」を上書きする**・2026-09-20 開発者裁定）。
...
7. The areka shall 既に決定論テストで固定されている配線を**もう一度テストしない**（記憶 test-only-decision-branches-not-proven-wiring）。零も明示する＝再テストしないのは次の **1 つ**である: 閉じ指示 → 再生停止 →「中断で終わった」を返す経路（`crates/areka-sakura/src/drive_lifecycle_tests.rs`）。**逆に、書き換える既存テストは 2 本**である: `crates/areka-kanade/src/schedule/close.rs` の `value_then_ended_refuses_close_and_resumes_pump` と `value_then_interrupted_refuses_close_same_as_ended`。どちらも「別れの台詞が `\-` 無しで終わったら終了を拒む」を固定しており、要件 3.6 の上書きで**規則そのものが変わる**ので、新しい規則（終了へ進む）を固定するテストへ改める（陳腐化ではなく更新・記憶 obsolete-vs-broken-test-policy）。
