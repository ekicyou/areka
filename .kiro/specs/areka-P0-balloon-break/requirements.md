...
**中断しても、台本が予約していた終了だけは実行される**（2026-09-20 開発者裁定・議題 2）。`\-` を含む台本を中断したら、ゴーストはただちに終了する。あわせて、**終了イベント（`OnClose`）の応答として再生する別れの台詞は、areka では必ず終了で終わる**——末尾に `\-` が在るのと同じ結果になる。別れの台詞の途中でダブルクリックすれば、台詞を飛ばしてただちに終了する。
...
- **In scope**（利用者・ゴースト作者から見える範囲）:
  ...
  - **終了の予約**: `\-` を含む台本を利用者が中断したら、ただちに終了すること。終了イベント（`OnClose`）の別れの台詞が、`\-` の有無にかかわらず必ず終了で終わること（**完了 spec `areka-P0-kanade` の要件 4.5「close 握手中の talk が quit=false で完了したら終了させず定常運転へ復帰する」と、完了 spec `areka-P0-popup-menu-minimal` の要件 5.4「終了の握手が拒まれたら、メニューからの終了でも終了しない」を上書きする**・2026-09-20 開発者裁定。この後、終了の握手が定常運転へ戻る経路は 0 本になる）。
...
5. The areka shall 中断を受け入れるかどうかを**再生中かどうか**だけで決め、どのスコープのバルーンがダブルクリックされたかでも、**どの場面の再生か（起動の挨拶・定常のトーク・別れの台詞）でも変えない**（2026-09-20 裁定・議題 1）。利用者から見た規則は「喋っていれば止まる。作者が止めさせないと書いた区間だけ止まらない」の 1 つである。起動の挨拶を止めさせたくない作者は `\![enter,nouserbreakmode]` を書く——それがこのタグの用途である。スコープ番号は「どこで起きたか」を伝えるためだけに運ばれる（下流 #35 が `OnBalloonBreak` の Reference1 に使う）。
...
8. The areka shall **「バルーンは隠れたのに台本の再生は続いている」状態を作らない**（2026-09-20 裁定・議題 1）。無効化の区間でないときにバルーンのダブルクリックで隠したなら、そのとき再生中の台本は必ず止まる。バルーンを隠す判断（表示の側・押下と同じ描画のうちに下す＝要件 4.5）と再生を止める判断（会話の運行の側）は別々の場所で下されるので、この対は「無効化の区間かどうかは隠す側だけが見る／止める側は再生中なら場面を問わず止める」という分担で守る。止める指示を渡せなかったとき（要件 1.7）だけがこの対の破れであり、そこは `error!` で記録する。
...
1. When 中断が受け入れられた, the areka shall 再生中のさくらスクリプトを止め、**中断した位置より後ろのタグ・文字・待ちを 1 つも実行しない**（面替え・`\![raise]`・`\!` の各コマンド・文字送り・`\w` の待ちを含む）。**例外は 1 つだけ**: 台本が予約していた終了（要件 3.6・3.8）は実行する。
...
4. When 中断で再生が止まり、終了の予約（要件 3.6・3.8）が無かった, the areka shall 定常へ戻り、次のトーク（ランダムトーク・イベント応答）をそれまでと同じ規則で始める。
5. The areka shall 中断のときに、そのとき出ていた面（サーフェス）を元へ戻さない。止めた時点の面のまま残す（正典に戻す規定は無く、「止める」以上のことをしないため）。
6. The areka shall 終了イベント（`OnClose`）の応答として再生する別れの台詞を、**末尾に `\-` が在るのと同じ結果になるもの**として扱う（**裁定 6・2026-09-20 開発者裁定**）。別れの台詞が最後まで再生されたとき・利用者に中断されたとき・その他の理由で途中で終わったときの**いずれでも、ゴーストは終了へ進む**。台本に `\-` が書かれているかどうかでは変えない。SSP は終了イベントでも `\-` が無ければ終了しないが（開発者の観察）、areka は「終了を指示したら必ず終了する」側を採る。これは完了 spec `areka-P0-kanade` の要件 4.5（`\-` の無い別れの台詞が終わったら終了させず定常運転へ復帰する）を**上書きする**。同じ規則に乗っていた完了 spec `areka-P0-popup-menu-minimal` の要件 5.4（終了の握手が拒まれたら、メニューの「終了」でも終了しない）も上書きになる——拒まれる場面そのものが無くなり、メニューの「終了」も引き止められない。**別れの台詞が選択肢を含むとき**は、終了の握手の最中の選択は既存の規則で受け付けられない（変更 0）ので、台本は選択肢の待ちで止まり、30 秒の上限か利用者の余白のダブルクリックで終了へ進む。「選んだ応答で引き止める」演出は areka では効かない。求めるのは結果であって、台本の文字列の末尾に `\-` を継ぎ足すことではない（SHIORI の返す台本は `\e` で終わるのが通例で、その後ろに継ぎ足した `\-` は実行されない）。
7. When 別れの台詞の最中に中断が受け入れられた, the areka shall 台詞の残りを再生せず、**ただちに終了へ進む**。別れの台詞を最後まで待たせない。
8. When 終了イベント以外のトークで、`\-` を含む台本の再生中に利用者の中断が受け入れられた, the areka shall **ただちに終了へ進む**（`\-` に辿り着いたのと同じ扱い）。台本が `\-` を含むかどうかは再生を始める前に確定しており（`crates/areka-sakura/src/compile.rs` が台本を組み立てる時点で終わり方を `TalkEndReason::Quit` に確定し、再生中の状態がそれを持ち回っている）、中断はその予約を捨てない。終了が効くのは**利用者の中断だけ**であり、それ以外の途中終了——新しいトークによる差し替え・選択肢の時間切れの解除——では、`\-` を含む台本であっても**終了しない**（既存どおり・変更 0）。
9. When 中断が受け入れられた, the areka shall **中断を理由とするイベントを SHIORI へ 1 件も送らない**（`OnBalloonBreak` は裁定 1 により本仕様の範囲外。中断が起きたことは運行側まで届くだけで、そこから先へは進まない）。中断が終了へ進んだとき（要件 3.7・3.8）に終了の手続きが既存どおり行う SHIORI とのやり取りは、この限りでない。
...
5. The areka shall 隠す時刻の決め方を設計に委ねる。ただし**「1 フレーム遅らせる」形の解は採らない**（開発者方針・記憶 no-frame-delay-fixes-change-the-state-shape）。要件が定めるのは「利用者がダブルクリックしてから、次に画面が描かれるときにはもうバルーンが無い」ことである。**2026-09-20 裁定（議題 1）: この文は字義どおりに守る**（「利用者に見えない程度の遅れなら可」へは緩めない）。会話の運行の側へ問い合わせて返事を待つ形は、別スレッドへの往復が挟まって字義を満たせないので採らない。隠す判断は、押下を検出した側がその場で下す。
...
4. When 無効化の区間に入ったまま `\![leave,nouserbreakmode]` が現れずにトークが終わった, the areka shall **遅くとも次のトークが始まる時点までに**無効化を解く（**裁定 7・2026-09-20 議題 1 で改訂・正典は沈黙**）。解かないと、閉じ忘れた台本 1 本のせいでそのゴーストが以後ずっと止められなくなり、利用者が黙らせる手段を永久に失うためである。「トークが終わった瞬間」ではなく「次のトークの始まりまで」とするのは、無効化の旗を持つ表示の側が既に受け取っている合図（トークが始まった）だけで解けるようにし、「トークが終わった」という合図を新しく足さないためである。**その代わりに生じる差を明示する**: 閉じ忘れた台本が終わってから次のトークが始まるまでの間、居残っているバルーンはダブルクリックで消せない（要件 2.2 の例外）。このバルーンは既定 30 秒のタイムアウトで消え、次のトークからは普通に止められる。閉じ忘れが無い台本では、この差は **0** である。ここで言う「次のトーク」は、指示を 1 件でも配るトークである。内容を 1 つも持たない台本（裸の `\e` など）は画面に何も出さず、無効化を解く契機にもならない（設計で確認・閉じ忘れが無ければ差は **0**）。
...
7. The areka shall 無効化の状態を SHIORI の `status` へ見せない（`nouserbreak` トークンを立てるのは `areka-P0-status-execution-states` の仕事）。ただし、本仕様が持つ旗をその spec が読めるようにしておき、読み口を塞がない。旗は表示の側に置かれる（議題 1 の裁定）ので、その spec が `nouserbreak` を立てるときは「旗が変わった」を表示の側から会話の運行の側へ 1 種伝える必要がある（既に在る向きの通知であり、新しい向きの線は要らない）。本仕様はその通知を**作らない**（読む者がまだ居ないため）。
...
1. The areka shall 次の **8 つの判断分岐**を決定論テストで固定する（記憶 test-only-decision-branches-not-proven-wiring・既に証明済みの配線は再テストしない）:
   ⑴ 再生中 → 止まる（要件 2.1・3.1）
   ...
   ⑺ `\-` を含む台本を利用者が中断 → 終了へ進む／`\-` を含まない台本を中断 → 定常へ戻る／`\-` を含む台本が利用者の中断**以外**で途中終了 → 終了しない（要件 3.8・3.4）
   ⑻ 別れの台詞が `\-` 無しで最後まで再生された → 終了へ進む／別れの台詞が中断された → 終了へ進む（要件 3.6・3.7）
2. The areka shall 「中断の後に次のトークが普通に始まる」ことを決定論テストで確かめる（要件 3.4）。
3. The areka shall 無効化の区間の出入り（入る・出る・閉じ忘れたまま次のトークが始まる・区間外の `leave`）を決定論テストで確かめる（要件 5.1・5.2・5.4・5.6）。
...
7. The areka shall 既に決定論テストで固定されている配線を**もう一度テストしない**（記憶 test-only-decision-branches-not-proven-wiring）。零も明示する＝再テストしないのは次の **1 つ**である: 閉じ指示 → 再生停止 →「中断で終わった」を返す経路（`crates/areka-sakura/src/drive_lifecycle_tests.rs`）。**逆に、書き換える既存テストは 3 本**である（4 本目は 0 本・設計検証で確認）: `crates/areka-kanade/src/schedule/close.rs` の `value_then_ended_refuses_close_and_resumes_pump` と `value_then_interrupted_refuses_close_same_as_ended`、および統合テスト `crates/areka-kanade/tests/kanade/close_test_handshake_tests.rs` の `close_refused_resumes_pump_then_terminates_via_resumed_talk`。いずれも「別れの台詞が `\-` 無しで終わったら終了を拒む」を固定しており、要件 3.6 の上書きで**規則そのものが変わる**ので、新しい規則（終了へ進む）を固定するテストへ改める（陳腐化ではなく更新・記憶 obsolete-vs-broken-test-policy）。
...
9. The areka shall 実機サインオフを **1 件**行い、確認項目と結果を tasks の完了記録に残す: emo2（絶対パス起動・記憶 areka-emo2-signoff-needs-absolute-paths）で長い台詞の途中にバルーンを左ダブルクリックし、⑴ 文字送りがその場で止まる ⑵ バルーンが即座に消える（30 秒待たない）⑶ その後に次のランダムトークが普通に出る、の 3 点を確かめる。加えて、別れの台詞を返すゴーストで右クリックメニューの「終了」を選び、⑷ 台詞の途中でバルーンを左ダブルクリックするとただちに終了する、を確かめる（計 4 点）。
...
6. The 本仕様 shall 裁定 6「`OnClose` の別れの台詞を中断したとき」を**ただちに終了する**で進める（**2026-09-20 開発者裁定・議題 2**・要件 3.6〜3.8）。当初の既定（既存規則のまま終了が取り消される）は取り下げた。裁定の中身は 2 つ: ⑴ 終了イベントの別れの台詞は、areka では `\-` の有無にかかわらず必ず終了で終わる（末尾に `\-` を自動で付けたのと同じ結果。完了 spec `areka-P0-kanade` の要件 4.5 を上書き）⑵ 終了イベント以外のトークでも、`\-` を含む台本を利用者が中断したら終了する。開発者の観察では、SSP は別れの台詞の中断でただちに終了し、かつ終了イベントでも `\-` が無ければ終了しない。areka は前者に合わせ、後者には合わせない（終了を指示した利用者を待たせない・引き止めない）。
7. The 本仕様 shall 裁定 7「無効化の区間を閉じずにトークが終わったとき」を**遅くとも次のトークの始まりまでに解く**で進める（要件 5.4・2026-09-20 議題 1 で「トークの終わりで解く」から改訂）。正典は沈黙しており、解かない側を採ると閉じ忘れ 1 本でそのゴーストが永久に止められなくなる。
8. The 本仕様 shall 裁定 8「バルーンを隠す判断と再生を止める判断を誰が下すか」を**隠す判断は押下を検出した表示の側がその場で下し（無効化の旗もそこだけが持つ）、止める判断は会話の運行の側が『再生中なら場面を問わず止める』で下す**で進める（2026-09-20 開発者裁定・議題 1・要件 2.5・2.8・4.5）。理由: ⑴ 既に在る向きの通知だけで組める（会話の運行の側から表示の側へ「隠せ」を運ぶ線も、再生の側から会話の運行の側へ旗を運ぶ線も新設しない）⑵ 旗を後から据える形にならないので、起動の挨拶の `\![enter,nouserbreakmode]` を取りこぼさない ⑶ 押下と同じ描画のうちに消える。「会話の運行の側が全部決める」形の美点（隠れたのに再生が続く、が構造上起きない）は、要件 2.8 の分担で代える。
9. Where 開発者が上の既定を覆した, the 本仕様 shall 該当する要件と設計・境界の節を同時に改訂する（記憶 revise-design-not-just-requirements）。
