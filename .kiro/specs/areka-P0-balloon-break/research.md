...
## 13. 設計ディスカッションの記録（2026-09-20）

設計検証（`design-validation.md`・判定 GO）の指摘 2 件と細目 7 件を、開発者の判断を待たない修正として `design.md` と `requirements.md` へ入れた。開発者の判断が要る議題は **0 件**（どの項目も、答えで作業が変わらないか、2026-09-20 の裁定の帰結だった）。

- **指摘 1（選択の帳簿を消す時点）**: `fn on_user_break` が受理の時点で `clear_choice_ledger(&mut state, "user_break")` を呼ぶ。§12 の R-4 の結論「足す仕組み 0」は「掃除点を 1 つ足す」へ改める。完了通知を待って消す形では、`UserBreak` → 期限後の `Tick` → `TalkDone` の並びで `OnChoiceTimeout` が出る（`fn fire_choice_timeout_if_due` は「中断を出した後か」を見ない）。
- **指摘 2（上書きする完了済み要件）**: `areka-P0-kanade` 要件 4.5 に加えて `areka-P0-popup-menu-minimal` 要件 5.4。終了の握手が `Steady` へ戻る口は `fn on_close_talk_wait` の `Input::TalkDone` の腕だけだったので、改訂後は戻る経路が **0 本**。コードの作業量は変わらない。
- **要件へ反映した実測の差**: 要件 7.7 の書き換えるテスト 2 本 → 3 本／要件 3.6 へ選択肢つきの別れの台詞の帰結と popup-menu 5.4 の上書き／要件 5.4 の「次のトーク」の読み。
- **細目**: `State` の欄の追随（22 行・8 ファイル）と 2 つのテストファイルの行数の見立て／コメントの追随 4 か所／起動記録だけのトークの例外／Risks へ 3 行（守られたトークが始まる瞬間の窓・受理直後の差し替え・切断検出が本番で到達しなくなる）。
- **§11 の覚え書きの訂正（§12.3）**: 旗を表示の合図の線の `TalkStarted` で解く形は、`Input` と `Update` の取り出しの段の違いで `Enter` → `TalkStarted` の逆順になるため採らない。7 本目の受け口がトークの境界を自分で検出し、3 値を 1 本の線に流す。設計検証が実コードで順序の議論を確かめ、妥当と判定した。裁定の中身（旗は UI だけが持つ・次のトークの始まりで解く）は変えていない。
