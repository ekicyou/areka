...
4. The areka ワークスペース shall `region.rs` 冒頭の「描画開始点は宣言どおり」「撤去された規約」の説明を本仕様の規則へ書き直し、撤去→取り下げの経緯（2026-08-27 撤去・2026-09-18 取り下げ・理由）と参照先（本仕様）を残す。同じ旧規則（宣言された `origin` は範囲の内外を問わず宣言どおり）を現在形で語っているほかのソース内コメントも、同じ規則へ言い直す——放置すると修正後に偽の記述になる。
