...
- [x] 5. (P) Core: testdll request fixture 拡張
...
- 陳腐化テスト除去（feature 最終検証で露見・2026-07-05）: `echo_roundtrip.rs`（先行 spec host32-ipc の scaffolding テスト）は helper の `respond()` echo スタブ（REQUEST→同一バイト返送）を検証していたが、本 spec の **R3.1 が echo を意図的に proxy 駆動へ置換**（Task 4.2）したため恒久失敗するようになった。i686 helper 未ビルド時は resolve 段で panic して隠れていたが、Task 7 で helper をビルドした時点で実挙動が露見。echo は仕様判断で退役ゆえ「更新して動かす」は不成立（echo 復活＝仕様違反／`shiori_request_e2e.rs` と重複）。実 helper 越し往復疎通の役目は `shiori_request_e2e.rs`（ダミー testdll で実 request 経路）が引き継ぎ、framing 安全性は凍結 ipc 単体テスト、LOAD 前 REQUEST→500 は helper loopback（4.2）がカバー済み。→ **削除**（開発者承認 2026-07-05「不要なテストは除外・意味あるが動かぬなら更新」原則に基づく）。README.md のテストコマンドと各所の宙ぶらりん参照コメントも整備。
