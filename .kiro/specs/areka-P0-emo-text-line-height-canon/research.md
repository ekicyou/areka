...
- **`BalloonModel::name()`**: `crates/areka-parsers/src/balloon/model.rs:379` に存在 → 折返し基準が描画範囲の外のときの警告（R6.7）にバルーン名を載せられる。**⚠訂正（2026-09-06・タスク 4.1 の実走）**: 当該行の `pub fn name` は `impl Font` のフォント名で、`BalloonModel` にバルーン名の取得口は無い。警告の `balloon` 欄はプレースホルダ定数（台帳 §7 #10・引受先は `balloon-canon-residue` の項目 14——当初の `ukadoc-survey-assets` は2026-09-06 に完了したため差し替え）。
...
> **改訂（2026-09-05・設計ディスカッション議題 1・§12 B-1）**: 以下のうち DD-1（α セル丈）・DD-2 の「実測後に確定」・DD-3（`FontBinding`）・DD-5／DD-6（行ボックス丈と帯の撤去）・DD-7（仮想行間 3）・DD-12（`metrics.rs` 切り出し）・DD-13／DD-14（SSP 実測の段取り）は裁定で撤回または改訂した。現在の判断の正本は `design.md` §9（撤回は「撤回（2026-09-05 議題 1）」と明記）。本節は設計時点の検討記録としてそのまま残す。
...
- B-1: SSP 実測が開発者の SSP 環境に加える変更の可否——(a) profile の退避と既定への初期化、(b) SSTP 受信の有効化、(c) repo fixture の複製バルーン `emo2-kakukaku-lh` の設置、(d) 実測を実装エージェントが本機で自走してよいか。→ **解決（2026-09-05・議題 1）**: 開発者方針「**SSP 実測主義は取らない**。意味論は輸入する」。ukadoc の記述（`font.height`・`\_l` の `lh`・`\f[height]` の CSS サイズ指定）と、開発者提供の SSP 200% 画像＋コントローラが起動した areka の 200% 画像の比較（字の大きさ一致・行送りだけ 30 対 35）から裁定: **決定 1＝`font.height` は em（現行のまま）・決定 2＝行間は定数 2 image px**。候補 α（セル丈）は不採用（画像で字の大きさが一致＝em）。brief の「係数 1.0 はインクが重なる」は行ボックス（37.24）とインク（≈22）の取り違えであった。§4.1 の予測表で「advance＝em」を仮定したのも誤り（Yu Gothic UI はプロポーショナル・実送り ≈ 23）。→ Requirement 1／3／5.5／6.5／8.3／10.4 を改訂・design.md は §4／§5／DD-1／2／5／6／7／12 を改訂（コミット `resolve design discussion #1`）。SSP 実測の道具・手順・`metrics.rs` 切り出しは設計から撤去。
