...
- C-1（§9-7）: Requirement 6 の裁定＝案 3「供給面を折返し閾値まで広げる（上限は画像の辺）」の採否。SSP が同条件で右端を欠かせていても案 3 を採るか。縦書きへの同形適用。
- C-2（§9-9）: 完了 spec `emo-text-layer` の design／research を注記つきで直接改訂する（brief の Desired Outcome 1・R2.1／2.2）か、`doc/COMPAT_ARCHITECTURE.md` §8 の先例（アーカイブ非改変・上書きの事実を §8 と本 spec に記録）に揃えるか。
