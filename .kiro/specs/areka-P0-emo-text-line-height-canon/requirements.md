...
- 関連症状（同根）: `descript.txt:14` の `wordwrappoint.x,-34` は画像幅 288 基準で **254** に解決され（`region.rs:251-258`・明示値はクランプされない）、文字描画範囲の右端 **240** の外に出る。`\_l[5em,…]` 後の「閉じる」「もどる」（x164..248）は折り返されずに置かれるが、供給面が文字描画範囲ちょうどのため右端 8px が欠ける。
...
- **In scope**（利用者・運用者から見える範囲）:
  ...
  - 折返し閾値（`wordwrappoint`）が文字描画範囲の外に解決されるときの areka の挙動の裁定と是正。
...
**Objective:** 利用者として、バルーン定義の折返し位置が文字描画範囲の外に置かれていても、選択肢のラベルの右端が欠けずに読めてほしい。運用者として、その扱いを裁定として記録し、バルーン側の粗さと areka の挙動を切り分けたい。

#### Acceptance Criteria

1. The 本仕様 shall 「折返し閾値が文字描画範囲の当該辺の外に解決される」場合の areka の挙動を裁定として記録し、SSP が同条件（`emo2-kakukaku`・`menu.pasta:15` の「閉じる」）で右端を欠かせるか否かの実測（Requirement 1 と同じ撮影から読む）を裁定の根拠欄に添える。
2. While 折返し閾値が文字描画範囲の当該辺の外（本 fixture では 254 > 240）に解決されている, the emo テキスト層 shall 折返し閾値までに置かれた文字を欠けさせずに表示する（「閉じる」「もどる」の x164..248 が全て見える）。文字を見せる範囲を広げる先はバルーン画像の内側（幅 288）を超えない。
3. While 折返し閾値が文字描画範囲の内（本体側 `emo2` では 351 ≤ 356）にある, the emo テキスト層 shall 本仕様の前後で表示結果を 1 画素も変えない。
4. The emo テキスト層 shall 折返し閾値を文字描画範囲の辺へ丸め込まない（丸め込むと「閉じる」が 4 行目へ折り返されて再びあふれ、Requirement 5.1 を満たせなくなることを裁定の根拠に記す）。
5. When 折返し閾値が文字描画範囲の外に解決されたバルーンを読み込んだ, the emo テキスト層 shall その事実を警告ログ（バルーン名・解決値・辺の値）で 1 回記録し、バルーン側の粗さとして `areka-P0-balloon-canon-residue` の台帳へ登記する（fixture は改変しない）。
6. The 本仕様 shall 本 Requirement の裁定を要件ディスカッションで開発者が確認できるよう、選ばなかった案（現状維持＝8px 欠けたまま／閾値の丸め込み）とその帰結を並べて記録する。
