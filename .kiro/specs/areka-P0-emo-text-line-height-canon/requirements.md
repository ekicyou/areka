...
6. The emo テキスト層 shall 選択肢のハイライト帯・ヒット帯を行ボックス丈とピッチから導く現行の防御式（行ボックス丈を `[font.height, max(font.height, ピッチ)]` に収める）を保ち、帯が隣接行の帯と重ならず、選択肢の文字のインクが帯の外へ出ないことを実フォントの読み戻しで確かめる（Requirement 5.6・8.5）。
