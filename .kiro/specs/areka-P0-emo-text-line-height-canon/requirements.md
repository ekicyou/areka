...
10. The 本仕様 shall 「確定した行のレイアウトを作り直さない」が製品側で保たれていること（`LineLayoutStore` は内容不変の行を再生成しない）を、`tests/viewbox_scroll_test.rs` の既存検査が緑のままであることと example の生成増分 1 で示し、タスク 3.4 の「先行不具合」の疑いが誤報であった旨を再導出台帳へ記録する。
