...
実施日 2026-08-28。対象 HEAD `7d79a8fa`（`feat(areka-P0-present-write-coherence): 裁定・却下理由・未達・手渡しの登記完結を検証 (task 3.2)`）。作業ツリー差分は `git status --porcelain` → ` M vendors/pasta` の 1 行のみ（V2-5・突合⑴・突合⑵ と同じ扱い）。

> **本節が塞ぐ誤読**: 本仕様の閉じ方を「直った」と読むこと。要件文書・設計文書・検証記録がどれだけ丁寧に未達を登記しても、**最後に開発者が読むのは完了報告の 1 枚**である。そこに合格と読める一文が混ざれば、登記は帳消しになる。本節はその 1 枚の文面を確定させ、合格と読める表現が入っていないことを**機械で**確かめた記録である。
> **本節の限界**: 検査しているのは ⑴ 文面が禁止語彙を合格の意味で使っていないか ⑵ 設計「完了報告の規律」3 条を満たすか、の 2 点だけである。**文面に書かれた数値そのものの正しさは V1・V2・突合⑴・突合⑵ が担保しており、本節は再検証しない**（本節は数値を各節から引いて置くだけである）。
> **本節自身の扱い**（V2・突合⑵ と同じ注記）: 本節を書き込む行為そのものが `verification/notes.md` に未コミット差分を 1 件作る。上の 1 行は書き込みより**前**の状態である。許可集合の内側の変更であり、本タスクのコミットで解消する。

### 3.3-1: 完了報告の文面（正本）

以下の `**完了報告（…）**` から次の見出し `### 3.3-2:` の直前までが**報告文の本体**であり、3.3-2 の禁止語彙検査はこの範囲だけを対象とする。この範囲の外（3.3-2・3.3-3）には、検査のために意図的に禁止語彙を並べた行が含まれる。

**完了報告（areka-P0-present-write-coherence・2026-08-28）**

**1. どう閉じたか**

本仕様は 2026-08-27 の要件討議の裁定（見送り＋登記）に従い、**未達 40 件を登記して閉じた**。是正のコードは 1 行も書いていない。閉じ方は「登記」であって、合格の宣言ではない——要件 4.6 は合格を宣言する場合に実機と決定論の双方の結果を要求する条項であり、本仕様は合格を宣言しないため発動しない。

**2. 登記した未達（実機専用系統・計 40 件）**

| 量 | 実測（`atom-73-signoff-1`・release・8 遷移） | 上限 | 違反 |
|---|---|---|---|
| `visualize_to_write_us` | 210,329 … 306,301µs（上限の **12.6〜18.4 倍**） | 16,667µs | **32 窓中 32 件**（上限以下 0 件） |
| `flush_total_us` | 143,231 … 231,910µs | 16,667µs | **8 遷移中 8 件**（もとより本仕様の合否外＝要件 4.2） |

合計 **40 件**（32 ＋ 8）。数量の正本は atom 確定台帳 `.kiro/specs/completed/areka-P0-dpi-transition-atomicity/mechanism-ledger.md` §11（`:549` 以降）であり、要件文書・設計文書・台帳の 3 者でこれらの値が一致することは突合⑵-2 で確かめた。比は算術でも再導出できる（`210329/16667 = 12.62`・`306301/16667 = 18.38`・`143231/16667 = 8.59`・`231910/16667 = 13.91`）。

利用者に見える症状は「画面の拡大率を切り替えた瞬間、絵は旧寸のまま、窓の位置と寸だけ先に新しくなる中間状態が最大 0.3 秒程度見える」ことである。**この症状は本仕様の後も、着手前とまったく同じように残る。** 切り替えの頻度が稀であることから、無視できる優先度と裁定された。

**3. 引受先はない**

この 40 件を引き受ける仕様は存在しない（要件 8.2 の明示）。`tick-gate-adoption` は tick の門の本採用が担当であって本量は担当しない。**将来この症状を直すには、新規仕様の起票が要る。** 本報告は後送先を名指ししない——名指しできる先が実在しないためである。

来歴も残す。atom 確定台帳 `:661` は次の段の引受先として本仕様の名を挙げていた。その本仕様が見送りを裁定した結果、**引き受け手が居なくなった**。これが「引受先なし」の来歴そのものである。

**4. 採らなかった 4 つの案と、その理由（各 1 行）**

- **B-3（文言どおり・可視化を窓書込の直前まで遅らせる）**: 窓書込の刻印は OS 呼出（`EndDeferWindowPos`）が戻った後に出るため、OS 呼出そのものが 143,231µs 以上を占め、上限 16,667µs に**構造的に届かない**（下限でも上限の 8.6 倍）。直しても未達は残る。
- **B-3′（flush 完了後に可視化の反映を駆動する）**: 上限へ届く唯一の形だが、要件 1.2⑵「tick の駆動と窓書込 flush の駆動の関係を変更する」に該当する大改造であり、要件 1.3 により候補から落ちる。
- **B-4（遷移中だけサーフェスを窓内下端中央へ置く補償）**: 判定量は可視化と窓書込の**時刻差**しか見ず、B-4 はどちらの時刻にも触れないため合否量を 1µs も動かせない一方、当たり判定の原点に触れる危険を持ち込む。費用と釣り合わない。
- **観測のみ先行（提示側の観測点を足す）**: いずれ直す前提が無いため採らない。観測点は是正の準備であって、是正しないなら費用だけが残る。

**5. 判らないまま残ること**

**「絵と窓のどちらが先に画面へ出るか」は未特定のままである**（要件 5.1）。アプリ側のログの時刻順は「寸が先・位置が後」、開発者の目視の申告は逆向きで、この食い違いは是正前後の 2 採取で同じ向きに再現している。本仕様は提示側の観測点を追加しないため、この食い違いは**未特定のまま次へ渡る**。そもそも「画面に出た時刻」を読む口はリポジトリに 1 つも無い（突合⑵-1 の R4 で `crates/` 全域を走査して確認した）。

**6. 差分と、実行時の挙動**

本仕様の差分は**仕様文書のみ**である。`origin/main` との差分は 8 ファイルで、内訳は本仕様の spec ディレクトリ配下 7 ファイルと `.kiro/steering/roadmap.md` 1 ファイル。`crates/` 配下・`Cargo.toml`・`vendors/` は 1 行も現れない（V2-1・V2-3・V2-7）。上限の定数・判定の仕組み・観測の語彙も 1 文字も書き換えていない（突合⑵-4）。

したがって **本仕様による実行時の挙動は、着手前とまったく同一である。** 利用者から見て変わったことは何も無い。

**7. ワークスペース全体のテスト（要件 7.7 の記録）**

`cargo test --workspace` を実行し、95 ターゲットすべてが `test result: ok`、通過 **5,894**・失敗 **0**・ignored 36、プロセスの終了コードは 0 だった（V1 に採取条件と空振りでないことの確認を含めて記録してある）。これは**要件 7.7（ワークスペース全体のテストが通る状態で仕様を閉じる）の充足**である。

この緑が言えるのは「本仕様が何も壊していない」ことだけであり、**要件 2.1 の充足ではない**。本仕様は是正のコードを 1 行も書いていないので、絵と窓が同じ提示フレームで揃うかを測る量は、上の第 2 節に登記したまま残っている。本仕様群では全部緑のまま実在の欠陥が素通りした事例が二度あり、緑を十分性の証拠として読んではならない。決定論側の判定がすべて PASS であることも、絵と窓が同じ提示フレームに載った証拠ではない（要件 4.4）——測っている量が違う。

**8. これから先、判定はどう見えるか**

上限・判定の仕組み・観測の語彙をそのまま残したので、実機の採取ログに対する既存のサインオフ判定は、この 2 量の違反を**これからも報告し続ける**。**それが正常な状態である**（要件 8.3）。もし違反が報告されなくなったなら、症状が直ったからではなく、上限か判定の仕組みか語彙のどれかが動いたからだと疑うこと。**報告が出続けることを、未達が片付いた印と読んではならない。**

**9. 再着手する人へ**

再着手の素材と、着手前に裁定すべき 2 点は `design.md`「将来仕様への手渡し（再着手の前提）」（`design.md:274`〜`:282`）にある。

- **素材の所在**: `research.md` §1・§3・§5・§6・§7・§8。**この並びが正であり、出所は `design.md:278`（本体）である。** トレーサビリティ表の C5 行（`design.md:249`）は §1・§3・§5・§10 を挙げており、そちらだけを辿ると工数と危険度（§7）・影響を受ける既存テスト（§6）・未調査事項（§8）に着かない（突合⑵-5 の申し送り 2 件目）。
- **罠①**: 判定量は同じフレームの `write_us - visualize_us` を**飽和減算**で求める（`crates/areka/src/placement/transition_judge.rs:817`）。可視化を窓書込の後ろへ動かすと値は 0 になり、直った実体が無くても上限を下回る。**置き場を決めるより先に、合否の意味をどう保つかを裁定すること。**
- **罠②**: 刻印の `t_us` はプロセス開始からではなく**当該 tick の開始からの経過**である（`crates/wintf/src/ecs/window/transition_diag.rs:692`・`:703`）。この読み替えを持たずに規模を見積もると 1 桁ずれる。
- **未調査で残る 1 点**: `EndDeferWindowPos` 単独の所要は測られていない。その直前・直後に計時点を足せば確定でき、この 1 点が B-3′ の到達可否を決める（`research.md` §8-②）。

**10. 証跡の所在**

すべて `.kiro/specs/areka-P0-present-write-coherence/verification/notes.md` にある——V1（ワークスペース全体のテスト）・V2（コード非接触）・V3／V4（上流アンカー・上限・判定の仕組み・失効条件）・V5（steering 追随）・突合⑴（不適用 23 件と生きている 33 件の確定）・突合⑵（裁定・却下理由・数値・手渡しの登記）。

### 3.3-2: 合格と読める表現が 0 件であることの機械検査

> **目で読んで数えない。** task 3.2 の自己監査は「実行していないコマンドの出力を書いた」1 件と「`grep -n` の出力を実際より少ない行数で載せた」4 件を掘り当てている。本節は**採った出力をそのまま貼り**、省略したときは省略した旨と真の件数を書く。

#### ⒜ 禁止語彙の定義（この 12 語を検査対象とする）

| 語 | 禁止する用法 | 許容する用法（**否定・自己言及**） |
|---|---|---|
| `合格` | 本仕様が合格したと読める書き方 | 「合格の宣言ではない」「合格を宣言しない」のような**否定文** |
| `PASS` | 本仕様の合否量が PASS したと読める書き方 | 「PASS であることも証拠ではない」のような**否定文** |
| `GO` | 判定として GO を出したと読める書き方 | （本文に用例なし） |
| `解決` | 未達が解決したと読める書き方 | （本文に用例なし。※`解決器` のような**語の一部としての偽陽性**は task 2.2 のレビューで実例が出ている——見つかったら語境界を見て判定する） |
| `達成` | ゴールを達成したと読める書き方 | （本文に用例なし） |
| `クリア` | 上限をクリアしたと読める書き方 | （本文に用例なし） |
| `充足` | **合否量**（`visualize_to_write_us`／`flush_total_us`）や要件 2.1 が充足したと読める書き方 | **要件 7.7 の充足**（設計「完了報告の規律」第 2 条が要求する書き方そのもの）／「要件 2.1 の充足ではない」という**否定文** |
| `完了` | **ゴール**（絵と窓が同じ提示フレームで揃うこと）が完了したと読める書き方 | 「**完了報告**」という文書名／「flush **完了**後」のように**別の処理の終了**を指す語（語の一部としての偽陽性） |
| `成功` | 是正が成功したと読める書き方 | （本文に用例なし） |
| `問題なし` | 未達を問題なしと読める書き方 | （本文に用例なし） |
| `是正済` | 是正が済んだと読める書き方 | （本文に用例なし） |
| `全緑` | 全緑をもって閉じたと読める書き方 | （本文に用例なし） |

#### ⒝ 検査対象の切り出し（報告文の本体だけを取る）

3.3-2・3.3-3 には**検査のために意図的に禁止語彙を並べた行**があるので、報告文の本体だけを機械で切り出してから検査する。

```
$ awk '/^### 3\.3-2:/{exit} /^\*\*完了報告（areka-P0-present-write-coherence/{f=1} f' \
      verification/notes.md > $S/report.txt
$ wc -l < $S/report.txt
63
$ grep -cv '^$' $S/report.txt
36
$ head -1 $S/report.txt
**完了報告（areka-P0-present-write-coherence・2026-08-28）**
$ tail -1 $S/report.txt

$ grep -v '^$' $S/report.txt | tail -1
すべて `.kiro/specs/areka-P0-present-write-coherence/verification/notes.md` にある——V1（ワークスペース全体のテスト）・…（以下略）
```

（`$S` はスクラッチパッドのディレクトリ。**仕様ディレクトリの外**に置いた——本仕様の差分集合を汚さないため。切り出した **63 行**のうち末尾 1 行は見出しの直前の空行であり、**非空は 36 行**である。`tail -1` がその空行を返すので、最終行の中身は `grep -v '^$' | tail -1` で採り直した。その出力は横幅の都合で末尾を `…（以下略）` と縮めて貼っている——**出力は 1 行しかなく、縮めたのはその 1 行の後半だけである。**）

> **切り出しの罠（実際に踏んだ・1 件）**: 最初に書いた切り出しは開始条件と終了条件を旗の上げ下げで表す形（`… {f=1} … {f=0} f`）だった。**本節 ⒝ の中に、その `head -1` の出力例として報告文の 1 行目が貼ってある**ため、終了後にその行がもう一度開始条件へ当たり、旗が上がり直す。結果、切り出しは 63 行のはずが **221 行**になり、禁止語彙の一致は 8 件のはずが **117 件**（`grep -c` は 5 のはずが **48**）になった。段落分離検査も `両方を含む段落=4` という偽の赤を返した。**採取コマンドが自分の出力例を文書に含むと、その出力例自身が採取条件に当たる**——上の形（最初の終了条件で `exit` する）へ改めて解消した。以下に貼る数値はすべて改めた後の再採取である。

#### ⒞ 検査の実行（採った出力をそのまま貼る）

```
$ PAT='合格|PASS|GO|解決|達成|クリア|充足|完了|成功|問題な[しい]|是正済|全緑'

$ grep -cE "$PAT" $S/report.txt
5

$ grep -oE "$PAT" $S/report.txt | sort | uniq -c
      1 PASS
      2 充足
      3 合格
      2 完了

$ grep -nE "$PAT" $S/report.txt
1:**完了報告（areka-P0-present-write-coherence・2026-08-28）**
5:本仕様は 2026-08-27 の要件討議の裁定（見送り＋登記）に従い、**未達 40 件を登記して閉じた**。是正のコードは 1 行も書いていない。閉じ方は「登記」であって、合格の宣言ではない——要件 4.6 は合格を宣言する場合に実機と決定論の双方の結果を要求する条項であり、本仕様は合格を宣言しないため発動しない。
27:- **B-3′（flush 完了後に可視化の反映を駆動する）**: 上限へ届く唯一の形だが、要件 1.2⑵「tick の駆動と窓書込 flush の駆動の関係を変更する」に該当する大改造であり、要件 1.3 により候補から落ちる。
43:`cargo test --workspace` を実行し、95 ターゲットすべてが `test result: ok`、通過 **5,894**・失敗 **0**・ignored 36、プロセスの終了コードは 0 だった（V1 に採取条件と空振りでないことの確認を含めて記録してある）。これは**要件 7.7（ワークスペース全体のテストが通る状態で仕様を閉じる）の充足**である。
45:この緑が言えるのは「本仕様が何も壊していない」ことだけであり、**要件 2.1 の充足ではない**。本仕様は是正のコードを 1 行も書いていないので、絵と窓が同じ提示フレームで揃うかを測る量は、上の第 2 節に登記したまま残っている。本仕様群では全部緑のまま実在の欠陥が素通りした事例が二度あり、緑を十分性の証拠として読んではならない。決定論側の判定がすべて PASS であることも、絵と窓が同じ提示フレームに載った証拠ではない（要件 4.4）——測っている量が違う。
```

> **`grep -n` の出力は 5 行**（`grep -c` の **5** と一致）であり、上に貼ったのは**その 5 行すべて**である。行の中身も 1 文字も削っていない。
> **`grep -o` の総出現数は 8 件**（`1+2+3+2`）。1 行に複数の一致があるため、一致した**行数** 5 とは一致しない。以下の逐一判定は**出現 8 件**を対象とする。

#### ⒟ 一致 8 件の逐一判定（数えるだけで済ませない）

| # | 行 | 語 | 現れ方（逐語） | 判定 |
|---|---|---|---|---|
| 1 | `:1` | `完了` | `**完了報告（areka-P0-present-write-coherence・2026-08-28）**` | **許容**——文書名。ゴールが完了したという意味を持たない |
| 2 | `:5` | `合格` | `閉じ方は「登記」であって、合格の宣言ではない` | **許容**——**否定** |
| 3 | `:5` | `合格` | `要件 4.6 は合格を宣言する場合に実機と決定論の双方の結果を要求する条項であり` | **許容**——条項の内容説明。その前件（合格を宣言する場合）が偽であることを直後の句で述べている |
| 4 | `:5` | `合格` | `本仕様は合格を宣言しないため発動しない` | **許容**——**否定**（`design.md:272`「本仕様は合格を宣言しない」と同じ字面） |
| 5 | `:27` | `完了` | `- **B-3′（flush 完了後に可視化の反映を駆動する）**` | **許容**——**語の一部としての偽陽性**。「窓書込 flush という処理が終わった後」の意であり、本仕様の完了とは無関係（却下した案の説明文） |
| 6 | `:43` | `充足` | `これは**要件 7.7（ワークスペース全体のテストが通る状態で仕様を閉じる）の充足**である` | **許容**——**要件 7.7 に対する充足**。設計「完了報告の規律」第 2 条が要求する書き方そのもの。合否量に対する充足ではない |
| 7 | `:45` | `充足` | `**要件 2.1 の充足ではない**` | **許容**——**否定** |
| 8 | `:45` | `PASS` | `決定論側の判定がすべて PASS であることも、絵と窓が同じ提示フレームに載った証拠ではない（要件 4.4）` | **許容**——**否定**（要件 4.4 の趣旨をそのまま述べる文） |

**判定: 肯定用法（合格と読める用法）は 0 件・否定または偽陽性が 8 件。** 12 語のうち `GO`・`解決`・`達成`・`クリア`・`成功`・`問題なし`・`是正済`・`全緑` の **8 語は 1 件も現れない**（上の `grep -o | sort | uniq -c` にその行が立たないことがそれを示す）。

- **語境界の偽陽性は現に 1 件出た**（#5 の `flush 完了後`）。task 2.2 のレビューが `解決` に対して `解決器` という偽陽性を挙げた形と同型である。本文に `解決` は 1 件も無いが、**同じ罠は `完了` の側で起きていた**——件数だけを見ていれば「禁止語彙が 8 件ある」という誤った赤になっていた。

#### ⒠ 陽性対照——**この検査が赤を出せること**

赤を出せると示していない検査は証拠にならない。**意図的に合格と読める 1 文**（**対照であって報告文ではない**・報告文の切り出し範囲の外にある）へ同じコマンドを当てた。

```
$ printf '%s\n' '（対照・本文ではない）本仕様は実機サインオフに合格し、未達 40 件は解決した。ワークスペース全体テストは全緑・全て PASS、決定論テストも GO で問題なし。目標は達成・上限もクリア・要件 2.1 は充足し、是正済につき是正は成功、本仕様は完了とする。' > $S/control.txt

$ grep -cE "$PAT" $S/control.txt
1

$ grep -oE "$PAT" $S/control.txt | sort | uniq -c
      1 GO
      1 PASS
      1 クリア
      1 充足
...
$ grep -oE "$PAT" $S/control.txt | wc -l
12
$ grep -oE "$PAT" $S/control.txt | sort -u | wc -l
12
```

- **12 語すべてが発火した。** ゆえに ⒞ の結果は、検査が働いた上での「肯定用法 0 件」である。
- 上の `uniq -c` 出力のうち `問題なし` の行は、採取した端末では末尾 1 文字が化けて表示された（多バイト文字の**表示上**の問題であって、一致そのものは成立している）。**バイトで確かめ直した**——`grep -c '問題なし' $S/control.txt` → **1**、`grep -c '問題なし' $S/report.txt` → **0**。

### 3.3-3: 設計「完了報告の規律」3 条の遵守

`design.md` の「完了報告の規律（要件 8.5）」（`grep -n "完了報告の規律" design.md` → **2 行**＝`:247`（C3 の所在列）と `:266`（本節の見出し）。条文は `:268`-`:270` の 3 行）の逐語に対して、報告文のどこで満たしているかを対応づけた。

| # | 条文（`design.md` 逐語） | 報告文で満たしている箇所 | 検査 |
|---|---|---|---|
| 1 | 完了報告は「**未達 40 件を登記して閉じた**」ことを明示し、合格・PASS・解決と読める書き方をしない。 | 第 1 節の冒頭文が `**未達 40 件を登記して閉じた**` を**逐語で**持つ。合格・PASS・解決と読める書き方をしないことは 3.3-2 ⒟ の逐一判定（肯定用法 **0 件**）で確かめた | 下の `grep -c` ＋ 3.3-2 |
| 2 | ワークスペース全体テストの緑は**要件 7.7 の充足**であって、要件 2.1 の充足ではない。**両者を同じ段落で並べない。** | 第 7 節・第 1 段落が `要件 7.7 …の充足` と書き、第 2 段落が `要件 2.1 の充足ではない` と書く。合否量そのものは第 2 節に置き、**第 7 節には合否量の数値を 1 つも置いていない**（「上の第 2 節に登記したまま残っている」と参照するだけ） | 下の段落分離検査 |
| 3 | 本仕様の差分が仕様文書のみであることを併記し、「実行時の挙動は着手前と同一」と明言する。 | 第 6 節。`本仕様の差分は**仕様文書のみ**である`（差分 8 ファイルの内訳つき）＋ `**本仕様による実行時の挙動は、着手前とまったく同一である。**` | 下の `grep -c` |

第 1 条・第 3 条の逐語確認:

```
$ grep -c '未達 40 件を登記して閉じた' $S/report.txt
1
$ grep -c '差分は\*\*仕様文書のみ\*\*である' $S/report.txt
1
$ grep -c '実行時の挙動は、着手前とまったく同一である' $S/report.txt
1
```

#### 第 2 条の段落分離を機械で示す

「同じ段落で並べない」は目で読んでも確かめられない。**合否量のトークン**と**テスト結果のトークン**を定義し、報告文を空行で段落へ割って、**両方を含む段落が 0 であること**を数えた。

- 合否量トークン: `visualize_to_write_us`・`flush_total_us`・`16,667`・`210,329`・`306,301`・`143,231`・`231,910`・`12.6`・`18.4`・`40 件`・`32 件`・`8 件`
- テスト結果トークン: `5,894`・`95 ターゲット`・`test result`・`cargo test --workspace`・`ignored`

```
$ awk 'BEGIN{RS=""} {n++;
    a=($0 ~ /visualize_to_write_us|flush_total_us|16,667|210,329|306,301|143,231|231,910|12\.6|18\.4|40 件|32 件|8 件/);
    b=($0 ~ /5,894|95 ターゲット|test result|cargo test --workspace|ignored/);
    if(a&&b) bad++}
  END{printf "段落数=%d 両方を含む段落=%d\n", n, bad+0}' $S/report.txt
段落数=27 両方を含む段落=0
```

段落ごとの内訳（同じ判定式を段落ごとに出力させたもの・**27 段落すべて**を載せる）:

| 段落 | 合否量 | テスト結果 | 段落 | 合否量 | テスト結果 |
|---|---|---|---|---|---|
| P01 | 0 | 0 | P15 | 0 | 0 |
| P02 | 0 | 0 | P16 | 0 | 0 |
| P03 | **1** | 0 | P17 | 0 | 0 |
...
- **合否量を含む段落は 6 つ**（P03〜P06・P09・P12＝報告文の第 1〜4 節）、**テスト結果を含む段落は 1 つだけ**（P19＝第 7 節の第 1 段落）。**重なりは 0** であり、両者のあいだには 6 段落（第 5 節・第 6 節）が挟まっている。

**陽性対照——この段落検査が赤を出せること**（**対照であって報告文ではない**）:

```
$ printf '%s\n' '（対照・本文ではない）`cargo test --workspace` は 95 ターゲットすべて `test result: ok`・通過 5,894・失敗 0 であり、`visualize_to_write_us` の違反 40 件（上限 16,667µs）は残っている。' > $S/para_control.txt
$ （同じ awk を $S/para_control.txt に対して実行）
段落数=1 両方を含む段落=1
```

**両者を 1 段落へ入れると即座に赤になる。** ゆえに本文の `両方を含む段落=0` は、検査が働いた上での 0 である。

### 3.3-4: 判定

- **観察可能な完了は充たされた**——完了報告の文面が 3.3-1 に確定して残り（報告文 **63 行**・うち非空 36 行）、**合格と読める表現（肯定用法）は 0 件**である。禁止語彙 12 語に対する一致は 8 件あるが、**8 件すべてが否定文または語の一部としての偽陽性**であり、1 件ずつ逐語で判定した（3.3-2 ⒟）。検査が赤を出せることは、12 語すべてを発火させる陽性対照で示した。
- **設計「完了報告の規律」3 条はいずれも満たしている。** 第 1 条＝`未達 40 件を登記して閉じた` の逐語 1 件。第 2 条＝合否量とテスト結果は**同じ段落に 1 度も同居しない**（27 段落・重なり 0・対照は 1 段落で赤）。第 3 条＝差分は仕様文書のみ・実行時の挙動は着手前と同一、の 2 文を逐語で持つ。
- **報告文の数値はすべて本記録および要件文書から引いた**——実測域・上限・比・違反件数は突合⑵-2（正本＝要件文書「未達の登記」節と atom 確定台帳 §11）、テスト結果は V1、差分 8 ファイルは V2-1、`file:line` は V3・V4-a・突合⑵-3 である。**記憶から書いた数値は 1 つも無い。**
