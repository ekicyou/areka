...
- Validation: `commit_tests.rs` の注入 ⑴ 宛先を `hold`（読み共有で開いたまま）→ 退避の `rename` が失敗 → `Write`・木がバイト単位で同一（9.4・5.7） ⑵ 2 件目を `hold`・1 件目は無い親フォルダの下 → 1 件目は置かれ親も作られる → 失敗で親フォルダごと消えて同一（5.4） ⑶ **戻せなかった**の固定入力: `commit` を呼ぶ前に、置いた 1 件目の削除を拒む仕掛けを仕込み（実装時に較正して決めた: rustc 1.98.1 では読み取り専用も `remove_file` で消え、削除を拒む ACL は置くための `rename` まで止める。採ったのは、中身を `testkit::pe_image()`（テスト実行ファイルの写し）にして `new/` の写しに `testkit::pin`＝`LoadLibraryExW(LOAD_LIBRARY_AS_IMAGE_RESOURCE)` を掛ける方式で、`rename` は通り、宛先の `remove_file` は os error 5 で失敗する。両方の結果は常設の較正テストが持つ）、2 件目の宛先を `hold` → 1 件目は置かれる → 2 件目で失敗 → `unwind` の `remove_file(宛先 1)` が失敗 → `RollbackFailed { stuck: [1 件目] }`・`work` に作業場所（7.4 の全数対応・5.5） ⑷ 差分の後・確定の前に、2 件目の親フォルダを対象の外を指すジャンクション（`cmd /c mklink /J`）に置き換える（`commit` を呼ぶ前に仕込む）→ `Escapes`・外側には何も作られていない・1 件目は戻されて木が同一（5.6）。`run_tests` 版の ⑶ ⑷ だけ `FakeFetch` の `on_get` で 2 件目の取得時に仕込む。`work_tests` に ⑸ 戻せなかった走行の作業場所（`old/` に中身）が残る状態でもう 1 周 `create` しても消えず残骸に列挙される（5.5）。
