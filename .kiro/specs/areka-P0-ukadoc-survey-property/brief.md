...
> **📌 2026-09-02 開発者指針「伺からしい価値観を持つ要素の優先度を上げる」（toolkit 規則 6・9 に凍結・裁定は議題 5〜7）**: 本ドメインは**原則 `values[]` 空**（照会配管＝テーマ 0）。付ける場合は toolkit 規則 9 の問い「この項目が無いと利用者はゴーストの何を失うか」への答えを `note` に必ず書く（例: `currentghost.seriko.*` の着せ替え状態＝装い・`system.*` は空）。順位は壊れ方⑴（辞書が値を前提にして黙って壊れるか）と基盤共有度⑷で決まる＝段階 C 末尾が既定。
