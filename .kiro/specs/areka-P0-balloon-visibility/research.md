...
> | 項目 | 区分 | 処理 |
> |---|---|---|
> | D1 文字層 hide | **要件ディスカッション** | 議題 1（スコープ裁定） |
> | D2 可視化の分離 | **設計フェーズ** | 選択肢空間は議題 2（Requirement 1.2 の厳格さ）で確定し、⒜⒝⒞の選択は設計 |
> | D3 所有者の一元化 | **要件ディスカッション** | 議題 3（境界の明文化） |
...
> | D11 既存注記の是正範囲 | **要件ディスカッション** | 議題 4（スコープ裁定） |
> | D12 SERIKO ループとの相互作用 | **要件ディスカッション → 設計** | 規約の要否は議題 3 に統合、実装形は設計 |
>
