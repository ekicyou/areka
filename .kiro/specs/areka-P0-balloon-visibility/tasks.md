...
- [ ] 6.5 (P) 可視性の所有権と分離可視化を検証する
  - _Note (6.4 からの申し送り): `presenter_visibility_tests.rs:375`/`:399` の `hide_is_immediate_regardless_of_ownership` は**非表示の直前を presenter の内部フラグ `target_visible` でしか主張していない**（entity の実可視を見ていない）ため、単体では「元から不可視だった」場合と区別できない。同ファイルは 6.5 の所有なので、ついでに entity 実値での前提主張へ強化できる。また `mount_entities` ヘルパが `presenter_hide_contract_tests.rs:49-58` と当該ファイル `:31-41` で重複している——共有ヘルパへ寄せるなら 6.5 か 6.6 が適任。_
