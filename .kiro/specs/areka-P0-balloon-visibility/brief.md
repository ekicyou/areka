...
> **📌 2026-08-13 追記(61) `ghost-window-zorder` 着地——再表示時に呼ぶ口の具体形が確定（相互登記の履行）**:
> - **呼ぶ口は `wintf::ecs::ReassertZOrder`**（コンポーネント）。**再表示したバルーン窓の entity へ 1 つ insert するだけ**でよい。維持系が次の巡で消費し、キャラ窓のすぐ手前へ入れて実測で検証し、要求は自動的に消える。位置・寸法には一切触れない。
>   - 公開経路は `wintf::ecs::{...}` から import すること（`wintf::ecs::window::...` を直接指すと再エクスポート漏れを見逃す）。到達性は `areka/src/placement/spawn_zorder_pair_export_tests.rs:24` が固定している。
>   - **宣言（`KeepDirectlyAbove`）は spawn 時に既に付いている**ので、本 spec が付け直す必要はない。宣言の無い窓へ要求を付けると warn で捨てられる。
> - **⚠ 隣接は「最も近い可視の隣」で測る**（2026-08-13 裁定）。**不可視のバルーン窓は隣として数えられない**。よって**バルーンが隠れている間、キャラ窓は「可視の通常窓のうち最前面」になりやすい**——この状態は zorder 側の「挿入位置が他アプリの常時最前面窓になる」分岐（`InsertSpec::TopOfNormalBand`）を踏む確率を上げる。分岐自体は是正済みで安全だが、**本 spec が hide→show を結線して初めて日常的に踏まれる経路**である点は認識しておくこと。
> - **⚠ zorder 側の決定論的テストは「可視のバルーンへ要求を挿す」形しか通していない**。**まだ不可視のバルーン／可視になった直後のバルーンへ要求が届く順序**は未検証である——本 spec が結線する際、show の適用と要求の挿入の順序を自分で決めて固定すること（可視化が先に効いていないと、測る側が自分を読み飛ばす）。
> - zorder 側は W6 で着地済み（実機サインオフ合格・`plan-a-gate.md`／`signoff.md`）。**「再表示時に手前に出る」保証には乗ってよい**が、上の 2 点は本 spec 側の宿題である。
>
