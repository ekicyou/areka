...
| ⑹ 本仕様が新設した檻が終了指示の消化を待てず期限切れになる | `crates/areka/src/emo2_boot/spine_conformance_support_tests.rs:557`（`kanade_probe_raises_no_shiori_call_and_observes_the_close`） | **本仕様が直す**（隔離せず、外へ渡す引受先も持たない・2026-09-04 の開発者裁定） | **檻自身の競走欠陥であって製品欠陥ではない。** kanade がまだ `BootVersion` に居るあいだに届いた終了指示は `pending_close` へ保留され（`crates/areka-kanade/src/schedule/boot.rs:31` の分岐・実体は `:285-288`）、保留を消化するのは `Steady{None}` に居る kanade の Tick だけである（`crates/areka-kanade/src/schedule/steady.rs:693`）。檻は kanade Tick を 1 本も送らないため保留が永久に残り、`SPIN_WAIT` で期限切れになる。製品は毎秒の Tick で必ず消化するので直す先は檻である。対象は編集集合の内側（`crates/areka/src/emo2_boot/` の兄弟テストファイル）ゆえ R12.1 の例外を要さない |
