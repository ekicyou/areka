...
- 完成判定を脅かす間欠的な赤 3 系統に**隔離か更新かの裁定**を下し、失う被覆と根治の引受先を記録する（根治はしない）。
...
- **間欠的な赤 3 系統の隔離裁定**（除外するか更新するか。判定ロジックには触れない）。
...
| 3 | 着せ替えの表情 | `\![bind]` で本体側の表情が変わり、切替後も固着しない | 両層 | mayuna-compose／bindoption-exclusivity | 決定論＝既存の bind 系テスト。実機＝目視 |
...
| 10 | 二人立ち総合 | 相方の窓と相方用バルーンが表示され、話者が交替し、別名指定で相方の表情が変わり、両バルーンが追従し単独でも動かせる | 実機 | M-boot 充足＋本仕様（退役ユニット 2 本の吸収先） | 実機＝目視。**退役した 2 つのユニット名の検証を本項目が吸収した**（R4.5・`brief.md:109`）。旧名を復活させない |
...
| 15 | バルーンの表示ライフサイクル | 会話の始まりでバルーンが現れ、終わりから所定時間で消える。起動直後は不可視のまま確立される | 両層 | balloon-visibility | 決定論＝`balloon_visibility_lifecycle_e2e_tests.rs`。実機＝目視 |
...
| 19 | 再表示直後の重なり順 | 再表示の直後もバルーンがキャラの手前に居続ける | 実機 | ghost-window-zorder（申し送り B-1） | 実機のみ。再断行の要求を挿す箇所は確立時の 1 か所だけで再表示経路は挿さない（research §2.4）。**拡大率の切替は再表示を伴うので項目 16 の直後が確認の好機** |
| 20 | 子プロセスへの受け渡し | 実 32bit の脳が引数・環境変数・作業ディレクトリの 3 経路で立ち上がり、実際に応答する | 実機 | test-cage-determinism（申し送り B-3） | 実機のみ。「1 度通った」＝**3 経路の同時成立**。証跡は接続失敗が 0 行・実 SHIORI 由来の起動挨拶が出た・正規の解放完了が 1 行（research 決定 F） |
...
| 1.4 | 決定論で観測できない項目を明示 | 適合検証項目表 | 「観測の層」欄と理由（項目 7・12・17・19・20） |
...
| 2.4 | 表示指令の列（時刻と指令）を完全一致で判定 | D3 表示の台帳 | 〈段・指令〉列の等値＋採取時刻が段の宣言区間に入ること |
| 2.5 | 期待に無いものが 1 つでも出たら失敗 | D3 | 等値照合（部分一致・包含判定を使わない） |
| 2.6 | 新しい仕組みを発明しない | D1 | 既存 `SpineHarness`・既存注入口・既存取り出し口のみ |
| 2.7 | 全ての実行主体が有界時間で終わる | D1 | 既存の有界な畳み方（`spine.rs:871`） |
...
| 6.7 | 決まった書式の合否ブロック | D7 | 7 行の合否ブロック |
| 7.1 | 絵と窓の遅れを引受先なしの既知の未達として登記 | D8 既知症状の登記 | 拡大率遷移の見た目を合否条件に含めない |
...
| 9.1 | 3 系統に裁定を下し理由を記録 | D11 隔離裁定 | 裁定表 |
| 9.2 | 「待たずに数える」形を待つ形へ更新 | D11 | `spine_e2e_test_s3_helper_liveness_detected.rs` の更新 |
...
| 12.1 | 編集集合を限る | Boundary Commitments／Modified Files | 例外は R9.2・R9.8 と、`spine.rs` の記録追補（具体化） |
...
**節構造が実装の相の境界を表す（R12.4）**: 仕様は分割しない。実装段階は 2 相に切る。**相 1** ＝ 決定論層（D1・D2・D3・D6）と、間欠的な赤の更新（D11 の系統 ⑴）。**相 2** ＝ 実機層と判定（D5・D7・D8・D9・D10・D12）と、間欠的な赤の隔離（D11 の系統 ⑵⑶）。D4 適合検証項目表は両相が参照するため、相 1 の入口で確定させる。相 1 の成果物は相 2 の前提である（完成判定の ⑴ が相 1 のテストを含む全体テストの成功を要求するため）。

**層ごとに独立した合否（R1.2・R1.3）**: 決定論層の合否は常設テストの成否そのものであり、実機層の合否は 7 行の合否ブロックである。受入記録は両者を**別の欄**に書き、片方を他方で置き換えない。完成判定は 2 つをそれぞれ受け取る。
...
| 終了 | kanade へ終了指示 | 終了の照会 → 終了挨拶の再生 → 解放、が記録される |

- 判定は段の駆動をすべて終えた後にまとめて行う（段の途中で部分照合しない）。理由は、途中照合が「まだ届いていないだけ」を失敗と誤認するため。
- 後片付けは既存の有界な畳み方（`spine.rs:871`）を使う。同関数は終了指示の結果を捨てるため、既に止まっている運行に対しても畳み方が成立する。
...
| 表示の台帳 | `Emo2Wiring::drain_received()`（`frame/wiring.rs:280`） | 段名・表示指令 | 期待列と等値。加えて各要素の採取時の注入時刻が当該段の宣言区間に入る |
...
表示指令そのものは時刻を持たない（`crates/areka-emo-present/src/command.rs:39-73`）。時刻を持つのは演出の列だが、areka 側の 4 つの受け口はいずれも演出を記録せず、記録する受け口を足すと本番の受け口列と既存テストの分解へ波及する（research 決定 C）。よって時刻軸は**段の注入時刻の区間**とする。記録の単位は〈段名・表示指令〉であり、これを列として完全一致で判定したうえで、各要素について「採取した時点の注入時刻が当該段の宣言区間に入る」ことを不変条件として判定する。段の区間はテストが先に宣言する定数であり、走行のたびに変わらない。
...
6. **走行**——項目表の 20 項目を順に確かめる。項目 16 の直後に項目 19 を確かめる（拡大率の切替が再表示を伴うため）。
7. **判定**——機械判定の走行を回し、出力を全文残す。目視所見を段ごとに書く。
8. **合否**——D7 の 7 行のブロックで書く。
...
実機ログの判定は、決定論テストと**同一の純関数**を回す既存の明示実行の走行で行う（`crates/areka/src/placement/transition_signoff_tests.rs:59-61`）。環境変数の未設定・空・空白のみ、パスが読めない、観測行が 0 行——いずれも失敗として落ちる（同 `:35-56`）。判定を二重に書かない。
...
7 行の定型で書く。全ての欄が満たされたときにのみ合格とする。欄は先行仕様の逐語を用い、量の名前と上限値もそのまま引く。
...
| 系統 | 実体 | 裁定 | 理由 |
|---|---|---|---|
| ⑴ 記録が非空になるのを待つだけで、直後の呼出数を待たずに数える | `crates/areka-ghost/tests/ghost/spine_e2e_test_s3_helper_liveness_detected.rs:174-185` | **更新する**（隔離しない） | 同じ確認を「条件が満たされるまで待つ」形で書いた手本が既に在る（`crates/areka/src/emo2_boot/spine_boot_smoke_tests.rs:32-36`）。移植は機械的で、被覆を 1 つも失わない |
...
根治の引受先は `.kiro/specs/areka-P0-zorder-chain-residue/brief.md` の A-1（`:15`）と A-2（`:16`）。同 brief `:34` が「e2e が先に踏んだ場合は e2e が隔離裁定を行い根治は本 spec」という分担を明記している。
...
| 採取時の注入時刻が段の宣言区間の外 | 段名と時刻 | 段の切れ目が崩れている |
...
4. **終了握手**——終了の照会 → 終了挨拶の再生 → 解放、の順序と、解放がちょうど 1 度であること（R3.9・項目 13）。
...
1. **一周走行**——適合検証項目表 20 項目を順に確かめる。項目 16 の直後に項目 19 を確かめる。
2. **機械判定**——実機ログを既存の明示実行の走行へ食わせ、出力を全文残す。
3. **読み分け**——機械と目視が食い違ったら 3 問を上から当てて 1 行に定め、合否ブロックへ書く。
