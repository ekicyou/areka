...
- **採らなかった案 2**: 台本に待ちを入れて指令ごとの最小発火時刻を判定する。決定 B の根拠と同じ壊れ方（注入時刻が観測を追い越す）を新たに作る。\n- **設計討議 #2 の裁定（2026-09-02・検証報告 重大 3）**: 区間の検査は駆動器の不変条件により製品の退行では赤にならない（自己検査）。判定の本体は〈段名・指令〉の列の等値であることを設計に先に書き、判定が生きている対照（段の上限を 1 段ずらすと段名の照合が赤になる）を 1 本添える。R2.4 の文言は「段（注入時刻の区間）と指令」へ改訂した。却下: 区間検査を捨てる（自己検査の安全網を失う）／演出を記録する受け口を足す（採らなかった案 1）。
