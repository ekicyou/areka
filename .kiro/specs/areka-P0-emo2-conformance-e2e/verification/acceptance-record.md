...
| 3 | **生ログの日本語が化ける**（走行 A）。PowerShell が areka の UTF-8 標準出力を CP932 で受け、`Tee-Object` が化けたまま UTF-8 で書くため復元不能。ログの ASCII 欄（`event=`・`id=`・数値）は無傷 | **その場で直せる小さなもの**（道具の罠・製品外） | 起動スクリプト `lap.ps1` に `[Console]::OutputEncoding = UTF8` を追加して是正済み（2026-09-05） | 該当なし（製品コード非接触） |
...
| やり直しの予定（条件を整えて再走するか） | **再走する**。引受先 spec（emo-text の行送り正典化・`cursor-tag-canon` マージ後に起票）の完遂後、§2 から準備をやり直し（永続状態の消去を含む）、走行 A〜D を新品のログ置き場で採り直す。§6 の宣言は書き換えない（再走時に読み直す） |
