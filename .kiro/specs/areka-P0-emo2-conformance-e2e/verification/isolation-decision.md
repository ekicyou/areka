...
**2026-09-04 の開発者裁定と 2026-09-05 のタスク 5.5 で、上表の状況は変わった。** ⑸ は裁定が「更新する」へ改まり（design.md D11 ⑸・R9.2／R12.1 の例外を S1 へ広げた。**当てるのはタスク 5.6** で、本タスクの時点では未適用）、⑹ は**本仕様が直した**（タスク 5.5・§2.1 ⑹）。ゆえに ⑸ ⑹ そのものに外の引受先は要らない。**残るのは次の 2 件で、どちらも本仕様の編集集合の外である。**
...
| 完成判定（R10.7）へ渡せる状態か | **条件付きで渡せる（2026-09-05 に更新）。** ⑴〜⑷ は裁定済みで、間欠的な赤は既定の走行から外れた。**⑹ は本仕様が直した**（タスク 5.5・§2.1 ⑹）ので留保は不要になり、⑸ は「更新する」裁定をタスク 5.6 が当てる。残る留保は**族**が持つ——A-2 が名指ししていない兄弟群（`talk_close_tests`・`text_scale_tests`・`seriko_loop_tests`・`display_tests`）の壁時計期限の飢餓と、§2.1 ⑹「残る危険 ⑴」の製品側の欠陥。どちらも 1 本のテストではなく類型であり、引受先は未定（§3.3.1） |
