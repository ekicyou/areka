...
- [ ] 6. 実機一周走行の実施
- [ ] 6.1 走行の準備と期待の事前宣言を行う（**準備者の分は 2026-09-06 に済み**＝ビルド・32bit 橋渡し `014C`・記録置き場 `emo2-conformance-2026-09-06`・永続状態の消去・記録 §1／§3／§4／§5／§5.1。残るのは開発者の手元作業 2 件＝実拡大率の設定と §6 の確認）
...
- [ ] 6.12 症状 D／E を記録・手順書へ登記し、採り直しの準備をやり直す（2026-09-07 第 4 回改訂）
  ...
  - _Depends: 6.11, 6.14_
  - _Boundary: `verification/lap-procedure.md`・`verification/acceptance-record.md`・リポジトリ外の記録置き場_
- [ ] 6.13 行送りの後に上の行が描かれない症状を再現して直す（症状 F・その場で直す・2026-09-07 第 5 回改訂）
  - RED: `crates/areka-emo-text/src/viewbox_draw_frame_render_tests.rs` の隣に兄弟試験を新設し、相方側相当の描画範囲（高さ 93・行送り 30）で「2 行に折り返す文字列 → `LineBreak{1.5}` → 次の台詞を 1 字ずつ」を `render_frame` で駆動、送りのフレームと次のフレームの読み戻しで**先頭可視行のインクが在る**こと・最新行のインクが在ること・全域描画のオラクルとバイト等価であることを主張して HEAD で赤を確かめる。緑なら `actor_choice_contract_tests.rs` の隣で `present_actor` を 1 字ずつ進める同じ主張を立てる（design D17「進め方」）
  - GREEN: 赤になった層で直す。描画対象の決め方（各矩形に交差する行の集合・全域縮退時の描画対象）か描き直しの範囲に限り、`visible_window`・折返し・保留改行の式は変えない（R17.4）
  - `cargo test -p areka-emo-text` と `cargo test -p areka --bin areka` を exit 0 で通し、既存の golden・読み戻し検査（`live_diff`・`oracle_regression`・`line_pitch_readback`・`kero_menu_capacity`）が無改変で緑であることを示す。触った各ファイルが 1,000 行以下（`viewbox_draw.rs` 849・`viewbox.rs` 846・`draw.rs` 988 は分割を先に検討）
  - 完了状態: 再現の檻が直す前は赤・直した後は緑で、機序（どの層のどの分岐か）が Implementation Notes に file:line で書かれ、記録 §13.4 の下書きが在る
  - _Requirements: 17.1, 17.2, 17.3, 17.4, 12.2_
  - _Depends: 6.10_
  - _Boundary: `crates/areka-emo-text/src/`（`viewbox.rs`・`viewbox_draw.rs`・`actor.rs`・`layout.rs` と兄弟試験）・`crates/areka-emo-text/tests/`（fixture・例外表・`crates/areka*` の他 crate は触らない）_
- [ ] 6.14 症状 F を記録・手順書へ登記する（2026-09-07 第 5 回改訂）
