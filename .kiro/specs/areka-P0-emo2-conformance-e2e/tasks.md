...
- [ ] 6.15 `\w[n]` 括弧形の解釈を撤去する（症状 G の areka 側・その場で直す・2026-09-10 第 6 回改訂）
  ...
  - _Blocked: 取り下げ（2026-09-10・開発者裁定「`\w[xxx]` バグは pasta 側の修正で対応する・本 spec での対応は不要」）。実装係の差分（17 ファイル・`cargo test -p areka-parsers -p areka-sakura -p areka-ghost` と `-p areka --bin areka` は exit 0）は `parked-6.15-drop-bracket-wait.patch` に保存し作業木は元に戻した。8 ファイル一覧には `parse_tests.rs`・`validation_tests.rs`・`talk_lifecycle_tests.rs` が漏れていた（差分に含む）_
...
- **6.15 は取り下げ（2026-09-10）**: 開発者裁定「pasta 側で対応・本 spec では不要」。撤去の差分は `parked-6.15-drop-bracket-wait.patch`（`git apply` で再現可）。実装係の所見: `compile_sheet_tests.rs` のコメント `\w[50]`／`\w[100]` は実値 50／100 ms と食い違う（元から誤記）・間欠赤 1 件 `conformance_lap_walks_every_stage_to_its_completion`（負荷時 60 秒・単体再走は緑・§13.1 の系統か要確認）。
