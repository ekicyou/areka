...
### 9.6 設計ディスカッション（2026-09-03）の処置

検証（`design-validation.md`・条件付き GO・重大 3 件）を受けて、開発者へ問う議題は **0 件**、自明な修正として次を design.md へ当てた。

- **重大 1（版番号の境界事例）**: 上流 spec の設計がブランチ `claude/areka-p0-ukadoc-survey-1617ba`（未 main）に存在し、規則を「前後が数字でも小数点でもない」と決めていた。この規則ではゆるい規則との差が 0 件（版番号あり 144・複数版 7）。D6 を上流と同じ規則に改め、空にする逃げ道を撤去した。§9.2 の「上流の設計は未生成」は誤り（main に無いだけ）。
- **重大 2（`placement/`）**: `crates/areka/src/placement/config.rs:138-140`・`:227`・`source.rs:45`・`:48` を受理キー表と URL の置き場所に加え、`zorder_descript.rs:47` を誤りとして §12 訂正 11 に登記。
- **重大 3（`///` の警告）**: 上流の設計（行 761）が `//` も拾うので、式の途中は `//` と書き分ける D11 を新設。shiori・property の同日の裁定と揃う。
- **備考の行番号**: shiori 議題 1 の裁定（備考に行番号を書かず定義名で指す）に揃え、D4 の型と要件 2.8・2.9 を項目名・定義名へ改めた。
- そのほか: 要件 5.7 の `:89`（空行）を `:73`・`:75`・`:76`・`:92` へ、§8 の `windowposition-limit` をバルーン 3 件へ、V11 の行の形・V12 の比較条件・V14 の除外対象を明記。

## 10. 上流の道具の着地（2026-09-05・main のマージ）

origin/main を本作業ツリーへマージし（マージコミット・衝突なし）、上流 spec `areka-P0-ukadoc-survey-toolkit` の実装 PR#136 を取り込んだ。取り込んだもの: 道具 `crates/ukadoc-survey`、`doc/ukadoc-coverage/` の `catalog.toml`（1,749 件）・4 台帳の骨組み・報告 4 本・`summary.md`・`values.md`・`README.md`。`cargo test -p ukadoc-survey` は 43 本すべて緑（サブモジュール `vendors/pasta` の展開が要る）。

**設計の前提が 2 つ反転した**（design §3 に登記・D1／D6／D7／§4／§6.1／§7／§10／§11／§13／§14 を追随させた）。

1. **道具は着地済み** — 要件 9.3 の退避路は条件が成り立たない。要件 9.1／9.2 が発効し、報告の再生成と `cargo test -p ukadoc-survey` の通過が完了条件になった（D7 を反転）。
2. **骨組みは既にある** — `ledger/assets.toml` に 542 項目が全件 `unclassified`・CRLF で並んでいる。本 spec は値を書き換えるだけで、骨組み生成スクリプトは不要になった（D1 を改訂）。要件 1.7 の退避路も発動しない。

**照合が取れた実測**（2026-09-05）。担当 24 ページの `catalog.toml` 項目数 542・`versions` を持つ項目 144・2 つ以上持つ項目 7 は、設計時に自前の規則で数えた値と 3 つとも一致した。D6 は規則の再実装をやめ、`versions` から選ぶだけの形に改めた。

**是正候補を 1 件取り下げた**。設計 §9.8 ⑴「上流契約の『doc コメント `///`』は `//` を含む読み替えが要る」は、着地した実装が `///`・`//!`・`//` を等しく拾い（`crates/ukadoc-survey/src/evidence/extract.rs:37`）、`doc/ukadoc-coverage/README.md:358` が明記しているため、食い違いではなかった。

**検査計画の作り直し**。上流の 15 種の所見が本書の初版の自前検査の多くを代替する（id 集合・並び順・語彙・別名の連鎖・関連の相手・登場版・報告の陳腐化）。自前で要るのは上流が見ない 8 本だけになった（design §11 の W1〜W8）——未分類 0 件・優先度の形・担当欄の埋め漏れ・URL と台帳の両向き一致・世代表のバイト一致・行数上限・触ったファイルの範囲・報告の再生成。
