...
- **既定バルーン id ＝ `StayseeBalloon`。** バルーン定義（`descript.txt`）の `id` 行・`install.txt` の `directory` 行・リポジトリに置いた実フォルダ名の 3 つが、大小を含めてバイト一致していることを実測で確かめた（採り方と較正は `.kiro/specs/areka-P0-default-balloon-bundle/verification/provenance.md` の「既定バルーン id」の節）。配布物の中での置き場は根の下の `balloon/StayseeBalloon/`。
