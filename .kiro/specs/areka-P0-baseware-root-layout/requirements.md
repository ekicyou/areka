...
> 要件 9 の裁定候補 1〜5 は**要件ディスカッションで開発者が確定する**。確定するまで設計へ進まない。
...
- **In scope**:
  ...
  - 既定バルーン id `StayseeBalloon` の定数と、解決順の最後への配線。
...
4. When argv の第 1 引数が無い and 記憶が無いか、指すゴーストが根に無い, and 根に列挙されるゴーストが 2 体以上, the areka shall 列挙の並び（要件 2.5）の先頭を起動し、複数から選んだことを記録に残す（info）。
5. If 記憶 `areka.last.ghost` の指すゴーストが根に無い, then the areka shall その旨を `warn!` に残して 3・4 へ進む（黙って読み替えない）。
6. If argv の第 1 引数が無い and 根に列挙されるゴーストが 0 体, then the areka shall `error!` と利用者向けの告知（要件 6）を出し、0 以外の終了コードで終える。
7. If argv の第 1 引数の指すフォルダにゴーストが無い（`ghost/master/descript.txt` が無い）, then the areka shall `error!` と利用者向けの告知を出し、0 以外の終了コードで終える（`warn!` で起動を続けない）。
8. The areka shall 上の解決を「argv・記憶の値・列挙の結果」だけから決まる純粋な判断として持ち、argv／記憶／1 体／複数／0 体の 5 分岐と記憶の指す先が無い場合を決定論テストで踏む。
9. When ゴーストが決まる, the areka shall どの経路で決まったか（argv／記憶／唯一／複数の先頭）と決まったフォルダを記録に残す（info）。
...
1. When 根・ゴースト・バルーンのいずれかが解決できない（要件 1.4・4.6・4.7・5.8）, the areka shall 利用者向けのメッセージボックス（OS 標準の Win32 `MessageBoxW`・新規依存 0）に、何が無いかと、置くべき場所の絶対パス（`<根>/ghost/` または `<根>/balloon/`）と、そこに置くものの形（ゴーストなら `ghost/master/descript.txt` を持つフォルダ・バルーンなら `descript.txt` を持つフォルダ）を日本語で示す。
...
3. The 本仕様 shall 起動解決の分岐（要件 4.8・5.10）を、プロセスを起こさず純粋な判断として踏む決定論テストで確かめる。
...
3. The 本仕様 shall **裁定候補 3（複数から 1 つを選ぶ規則）**として、記憶が無く候補が 2 つ以上のとき「告知して終了」ではなく「列挙の並びの先頭を起動し info に残す」（要件 4.4・5.6）。第三者が切替メニュー（後続 spec）を持たない段階で止めてしまうと出口が無いため。
