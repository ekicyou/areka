...
| **DD-10** | 解決済みアンカーの表現/provenance | **`Anchored(Anchor)` Component を唯一の格納された真実源に確定（設計ディスカッション#2）**（全 char 窓・spawn で `config.alignment` を `Anchor::from_alignment` 解釈して付与）。cascade 解決（config.rs）は無改変＝**解釈のみ消費**（Req4.2/6.3）。実行時 `\![set,alignmenttodesktop]`（seriko・非所有）が `Anchored` を書換える interlock を契約として定義し、この spec は `Changed<Anchored>` で反応する（producer は将来 seriko）。**`ScopePlacement.bottom_snap: bool` フィールドと `BottomSnap` marker Component は格納状態として退役**（二つ目の格納表現を残さない＝Req1.6 の「二重化しない」を格納レベルで満たす）。実コード確認: 現 `bottom_snap`（resolver.rs:200）は `matches!(alignment, Bottom | Seam(_))` で非 Free を全 true へ畳む＝実体は「anchored か否か」の誤称ゆえ、5 アンカー `Anchored` への一本化が正しい。boolean 需要は使用点で `matches!(anchor, Anchor::Free)` を導出（述語関数 `Anchor::is_free()` は許容＝格納でなく導出）。**区別**: `BottomSnapPolicy`（follow.rs:76・Y 釘付け純ポリシー）は T の bottom 腕として再利用（R1.2）＝退役対象は marker `BottomSnap`（spawn.rs:90）のみ。既存 `bottom_snap`/`BottomSnap` 前提テスト（resolver.rs:977-1028・spawn.rs:580-679・follow.rs BottomSnap fixture 群）は `anchor`/`Anchored(Bottom)` 検証へ意味を保って差し替え（陳腐化 update・削除ではない）。 |
