...
>
> **⑷ 問題解決のループ設計（本 spec の要件フェーズが具体化する・開発者依頼 2026-08-11）**
> 1. **計測基盤の確立（最初のタスク）**: 再現可能な測定手順を檻にする——`AREKA_APP_SMOKE_EXIT_MS` 有界実走＋ログ走査で ⒜ `apply(ShowSurface)` の回数とコマ間隔、⒝ `cache_hit` 率、⒞ `loop ticker catch-up` 件数を機械判定（bindopt の `signoff-scan.py` 方式が実績あり・流用可）。＋ 15 秒刻み CPU サンプリング 20 分以上（(a) 消滅後の時間上昇の有無を初めて綺麗に測れる）。**ベースライン＝本日の保全ログ 4 本**
> 2. **第 1 段是正**（要件変更なし）: `compose_into` 切替＋`Target` へ再利用バッファ＋`AlphaMask` 再確保見直し（brief 本文 Approach どおり）
> 3. **再測**: 同一手順・同一 fixture で 1 と比較。判定軸＝コマ適用時間・ticker catch-up 0・アイドル CPU・スロー再生の目視解消
> 4. **判定分岐**: 足りる → アロケーション回数の決定論檻で固定して完了 ／ 足りない → キャッシュ容量裁定（Open Questions ①）へ進む
> 5. 檻は実時間でなく**アロケーション回数・呼出回数**で表現（brief 本文 Constraints どおり）
