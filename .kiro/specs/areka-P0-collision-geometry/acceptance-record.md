...
> **⚠️ 受け入れ不成立（2026-07-18 開発者判断・REJECTED）**
>
> 本記録は **k=1.0（固定物理px・非拡大）下**の座標契約を 2 DPI 水準で実測したものだが、**areka の基本設計は DPI追従**（画面 DPI に追従してマスコットが拡大縮小する・SSP とは異なる設計思想）である。現状 `TextSlotView::scale()` は常に 1.0 を返すため、**両 DPI 水準ともマスコットは同一物理寸（382×547）＝ヒットテストの座標経路は完全に同一**だった。よって「2 水準」は k=1.0 plumbing の DPI-clean 性（dpi/96 誤再スケールが無いこと）を確認できたにとどまり、**DPI追従の核心＝「マスコットが scale≠1.0 で拡大表示された状態で、拡大後の窓 client 点を k で縮約して正しく当たり判定できるか」は未実装かつ未検証**である。開発者判断「これでは受け入れできない」。
>
> **以下の実測値は k=1.0 plumbing 検証としては有効だが、受け入れ完了の証跡ではない。** DPI追従下の当たり判定（点÷k）を実装し再検証するまで本 Task 4.2 は未完了。射程（collision-geometry 拡張／emo-present DPI追従レンダリングと連動／別 spec）はスコープ判断待ち。
...
**判定: 受け入れ不成立（INSUFFICIENT / REJECTED・2026-07-18 開発者判断）**

- k=1.0 plumbing の DPI-clean 性は 120（125%）／192（200%）で実測確認した（座標配管が DPI で誤再スケールしない）。
- **しかし DPI追従（基本設計）下の当たり判定＝scale≠1.0（拡大表示）状態でのヒットテスト正しさは未実装・未検証**。両水準ともマスコットは同一物理寸（k=1.0）ゆえヒットテスト経路が同一で、scale 差の検証になっていない。
- 受け入れ完了には、DPI追従下の当たり判定対応（`EmoPresenter::hit_region` で点÷k・純関数の scale 対応・決定論 unit 檻・実機で拡大表示状態の目視解決一致）の実装と再検証が必須。
