...
└── areka/
    ...
    └── examples/
        └── collision-probe.rs # 【新規】実 DPI probe（#[path] で target_map.rs と hit_region.rs を私有 include）
```
...
- **Input / validation**: emo2 fixture（`crates/pilot/examples/shiori-host-32/fixtures/emo2/shell/master/`）。次の3点を**実値で固定する**（donor の既定値をそのまま流用すると probe は成立しない・下記「donor からの必須逸脱」）:
  ...
  3. **窓 client 寸法は `compose(1000, binds)` の extent（382×547）から与える**。donor は `surface_id: 0` 決め打ちの寸（434×687）を `WindowPos.size` へ渡す（`emo-present.rs:375-376,514-516`）ため、そのまま流用すると 3. の k=1.0 assert が**構成上必ず失敗する**（窓 434×687 vs `chain.size()` 382×547＝`chain.rs:173-193` の upload が composed 寸へ resize するため）。窓寸を `surface_size()` へ追随させる既存機構は `frame.rs:555-583` `resnap_shell_targets` だが、これは bin の frame 駆動であり example からは動かない（かつ C-7 で frame.rs は不触）。
...
- Integration: `#[path]` で `target_map.rs` と `hit_region.rs` を私有 include（File Structure Plan の2行）。`#[allow(dead_code)]` を include 側に付す（`window-placement.rs:99-113` 前例）。
