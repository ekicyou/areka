...
- [x] 4. Core: 意味層（decode）— 値正規化・集約・捕捉・写像・寛容吸収
- [x] 4.1 surface ブロックの枠組みとヘッダの寛容スキップを実装する
...
- [x] 4.4 surface.append ターゲット指定の捕捉（展開しない転記）を実装する
...
- [x] 4.5 kero.surface.alias 写像（不透明キー・順序付き ID・重複保持）を実装する
