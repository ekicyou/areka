...
### 8.6 値定数・テスト題材の確定（§4 #6・§8.2 #4・OPEN QUESTION 全消化）

- **値定数（ukadoc 一次確認・議題2）**: `\w[n]`/`\wN` = **n × 50ms**（`\w9`=450ms）、`\_w[ms]` = 絶対 ms。素の `\n` = **1.0**（100%）、`\n[percent]`=percent/100（`\n[150]`=1.5）、`\n[half]`=0.5、負値は戻り。型契約（`Wait(Duration)`/`NewLineRatio(f32)`）は元から不変。
- **テスト題材（議題3）**: ゴースト実体ファイルの**同梱はしない**。手書き／インラインの代表さくらスクリプトで構文・subset・寛容パススルーを網羅。作者提供の実 OnBoot 例 1 本をインライン代表フィクスチャに採用（`\![bind]`×6→GenericCommand、`\s[通常]` 日本語エイリアス不透明保持、`\_w` 絶対ウェイト、`\n`/`\n[150]` を実証）。emo2 は脳由来の特殊構文ゆえゴースト丸ごとは代表に不適。実物通し検証は下流 E2E（M-e2e）。
- **設計の実証**: 上記 OnBoot 例を design の Lexer→Decode で追跡し、全構文が設計 Instruction へ過不足なく落ちることを確認（`\q`/`\![move]`/`%username` は本例に不在＝個別手書きテストで網羅の必要性も裏付け）。
