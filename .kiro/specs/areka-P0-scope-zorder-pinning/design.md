...
> ⚠ **未決着 1 件（task 2.3 → 4.4 へ引き継ぎ・正本は `research.md` §13.7 の 6 件目）**:
> `a_settled_chain_holds_its_order_when_the_deepest_window_is_raised_to_the_front` の自己検査
> `control_before == Some(0)`（`zorder_chain_order_tests.rs:687`）が、全体走行の 3 プロセス同時 ×
> 120 走行で 1 件赤になる（同テスト単独では 0 件）。真因は未確定——初版 4.1 が置いた
> 「保証していない性質」への依存とも、2.3 が足した掃き出しの `SetWindowPos` ≒ 864 回という
> 新しい外乱とも読める。**決着には HEAD（2.3 抜き）での同じ全体走行 3 プロセス × 120 回との
> 比較が要る**。2.3 が試した是正（検体窓の始点を固定する形）は 1/120 → 3〜4/120 へ**悪化**したため
> 差し戻してある。
