...
- 鎖を成立させるための**後押し 1 回**（鎖の**根**を錨＝1 つ手前の窓の直後へ差し直す。§13 実測 10 で形を確定。§12.2 実測 9 の「先頭を 2 番目の直後へ」は**空振りする形だったので撤回**した）
...
| 後押しの形 | `SWP_NOZORDER` では効かない。Z を伴う指令なら効く。挿入位置に他プロセスの窓を渡す形（`GW_HWNDPREV`）は、その窓が消えると黙って失敗する | 後押しは**自分の窓 2 枚だけ**を参照する（→ §13 で形を採り直した） |
| **動かす窓**（§13 実測 10・2026-08-30） | **所有する窓を動かしたときだけ**鎖が並び直る。鎖の先頭は誰も所有していないので、先頭を動かしても並ばない（24 通り中 **0** 通り）。根を動かす形は 24 通り中 **24** | 後押しは**鎖の根を錨（1 つ手前の窓）の直後へ差し直す**。錨の直後が既に根の巡だけ、挿入位置を**先頭**へ切り替える（2 択・書く直前に 1 度だけ現況を読む） |
...
> しないと**檻が最初に作った窓＝鎖の先頭**が IME 窓の所有者になり、本番には無い性質
> （先頭も所有する窓である）の上で緑になる。受け皿は壊さない——スレッド終了時に OS が破棄する。

**既に着地済みの実測檻（本設計の土台の証跡・恒久保存）**

- `crates/wintf/src/api_owner_chain_probe_tests.rs`（`crates/wintf/src/api.rs:626-628` で登記）——
  §12 の実測をそのまま assert で固定した檻。Windows 側の性質（張り替えの非即時性・後押しの要否・
  最小化／破棄の連動・`clear_window_owner` の落とし穴）が変われば**ここが赤で教える**。

  > ⚠ **2026-08-30 訂正（task 2.3）**: この檻の**実測 7・9 の結論は `research.md` §13.2 で撤回**
  > された。当時この 2 本は「鎖の**先頭**を 2 番目の直後へ差し直せば鎖が収まる」と記録していたが、
  > それは**受け皿（IME 窓を鎖の外へ逃がす窓）を敷いていない汚染された測定**であり、受け皿を
  > 敷くと同じ形は 1 枚も動かさない。2 本は受け皿を敷いた形へ書き替え、実測 9 は
  > **撤回そのものを記録する**（撤回済みの形では動かないこと／採用形なら収まることを対で測る）。
  > よって「本 spec の設計はすべてこの檻の緑の上に立つ」とは**もはや言えない**——後押しの形に
  > ついては §13 の掃き出し（下記）が正であり、§12 の他の実測（張り替えの非即時性・副作用・
  > スプライス・`clear_window_owner`）は引き続きこの檻が支えている。
- `crates/wintf/src/api_owner_chain_nudge_probe_tests.rs`（同じく `api.rs` 末尾で登記・task 2.3 で新設）——
  §13 の実測（**動かすのが所有側でなければ鎖は並ばない**）を、始点の全順列 24 通り × 後押しの形 6 種の
  掃き出しで固定した檻。**スレッド既定の不可視 IME 窓を鎖から外す受け皿**を先に作るので、
  §12 の檻が乗っていた偶然の隣人には依存しない。
...
| `doc/COMPAT_ARCHITECTURE.md` §8 | 裁量 **14 件**・訂正 1 件・先送りの参照 1 件の登記（要件 12・13）。⚠ 件数は 2026-08-31 の着地で数え直した値（設計時の見積りは 11 件だった） |
...
| 12.2 | 列挙された裁量を記録 | COMPAT §8（**着地 14 件**・要件 12.2 の列挙は下限） | — | — |
...
/// 後押しの指令を組む（§13 実測 10）。
///
/// **鎖の根（`members` の末尾）を、錨＝その 1 つ手前の窓（`members[len-2]`）の直後へ
/// 差し直す**。動かすのが根であることが要点である——Windows が鎖を並べ直すのは
/// **所有する窓が動いたとき**であり、鎖の先頭は誰も所有していないので、先頭を動かしても
/// 並ばない（§13.2）。参照するのはどちらも自分のゴースト窓なので、位置・寸法は変わらず
/// （要件 11.1）、鎖の外どうしの相対順も変わらない（要件 6.1／6.2）。
///
/// **挿入位置は 2 択**（`anchor_next` ＝ 錨の生の 1 つ奥。適用系が書く直前に 1 度だけ読む）:
/// - `anchor_next != Some(root)` → 挿入位置は**錨**
/// - `anchor_next == Some(root)` → 挿入位置は**先頭**（`members[0]`）
///
/// 2 つの要求が同時に「現在位置と同じ」になることはない（根の生の 1 つ手前は高々 1 枚）ので、
/// **どちらか一方は必ず本物の位置変更**である。未文書の「隣接なら省略する」性質には依存しない。
/// 窓が 2 枚のときは錨と先頭が同じ窓になり 2 択は 1 つに畳まれる（そのとき空振りするのは
/// 既に望む並びである場合だけ）。
///
/// **他の形を選んではならない**:
/// - `SWP_NOZORDER`（触るだけ）は再整列を起こさない（実測 7）
/// - **先頭を 2 番目の直後へ差し直す形**（初版）は 1 通りも収まらない（§13.1）
...
pub(crate) fn nudge_command(
    members: &[HWND],
    anchor_next: Option<HWND>,
) -> Option<SetWindowPosCommand>;
```
...
- 手順（1 巡・鎖全体）:
  ...
  5. 何らかの操作が実際に走ったときのみ、鎖全体へ**後押しを 1 回**
     （挿入位置の 2 択のため錨の生の 1 つ奥を 1 度だけ読み、`nudge_command` →
     `SetWindowPos(root, insert_after, SWP_NOMOVE|SWP_NOSIZE|SWP_NOACTIVATE)`）
  6. 後押しの**直後に**重なりを実測し（既存の `measure_*` を流用・不可視の窓は読み飛ばす＝要件 9.3）、
     宣言と実測を 1 行に載せる（要件 9.2）
  7. `dirty` を false へ戻す。**追加の起床は要求しない**（要件 14.5）。
     ただし**1 本も書けず、かつハンドル未取得で見送った付与が在る巡は印を戻す**——計画は
     HWND 生成より前に公開されうるので（`resolve_member` は `WindowHandle` を要求しない）、
     ここで印を落とすと後からハンドルが現れても再公開されず鎖が永久に書かれない（§13.5）。
     重なりは 1 度も読まないので観測ではない。持ち越しが解けるのは
     **付与の両端の窓ハンドルが揃った edge が 1 本でも現れたとき**である
     （`handle_missing` は付与の**どちらか一方**の窓ハンドルが無いだけで立つので、
     ハンドル済みの窓を含む鎖でも 1 本も書けずに待つ配置が在る）。待っている間は実行環境を
     1 度も呼ばず、**見送りの記録も待ち始めの 1 回しか出さない**（`report_absent_elements` と
     同じ「前回と内容が違うときだけ出す」姿勢。毎巡出すと実機のログ判定が埋まる）。
     食い違い（`Diverged`）だけの巡は**持ち越さない**——待っても変わらないものを毎巡試すのは
     要件 14.2 が退役させた反復是正そのものだからである
...
- **挿入位置の 2 択のために現況を 1 度だけ読む**（`GetWindow(anchor, GW_HWNDNEXT)`）。読むのは
  書く直前の 1 度きりで、控えも持ち越しもしない——周期的な観測（要件 14.2 が禁じた形）ではなく、
  既存の撤去経路が外す前に `GWLP_HWNDPARENT` を読むのと同じ規律である
...
4. **`nudge_command`**——要素 2 枚以上なら**根**と挿入位置を参照した指令が 1 本／要素 1 枚以下なら `None`／
   挿入位置の 2 択（錨／先頭）の**両枝**／2 枚のときの畳み込み／
   **他プロセスの窓を挿入位置に選ばないこと**（自分の `members` 以外の HWND が指令に現れないことを固定する）
4b. **印の持ち越し**——1 本も書けずハンドル未取得が在る巡は `dirty` が残る／ハンドルが付いた次の巡で解ける／
   食い違いだけの巡では残らない（要件 14.2 が禁じた反復是正へ化けていないことの対照）／
   **同じ schedule で何巡回しても実行環境を 1 度も呼ばず、見送りの記録は 1 本ちょうど**
   （`5` なら記録の氾濫・`0` なら黙って諦めた形＝要件 8.3 違反。両側から挟む）
...
> ✅ **決着済み（task 4.4 の実測・正本は `research.md` §13.7 の 6・7 件目）**: 引き継がれていた
> 1/120 の不安定は、HEAD（2.3 抜き）と現行を**同じ全体走行 3 プロセス × 120 回**で測って
> **5/360 対 5/360 の同率**であり、2.3 の掃き出しは外乱ではなかった。真因は**檻の道具の欠陥 2 種**
> ——⑴ Z を読む走査が他プロセスの窓の破棄で途中で切れて**空を返す**、⑵ `HWND_TOP` の持ち上げが
> `Ok` を返しながら**1 つも動かさない**。どちらも「届いたことを観測してから進む」形（有界回数の
> やり直し）で閉じた。**是正後、本 spec の鎖の実窓の檻は 3 プロセス同時走行 3,300 走行で赤 0 件**
> である（3 × 120 を 3 回＝1,080 ＋ 隔離測定の 2,220）。`main`（本 spec の檻が 1 本も無いツリー）
> との**走行数を揃えた 2,220 対 2,220 の隔離測定でも、重なり順の檻の赤は両ツリーとも 0 本**。
> 本ブランチに残る赤 2 本は `tick_bridge` の vblank 500ms 期限だけで、重なり順とは無関係である
> ——レート・隔離測定・未担当項目の登記は `research.md` §13.7／§13.8 が正本。
>
> 実窓の檻が持ち上げに使う道具は `zorder_chain_order_tests.rs` の `raise_to_front_until` に集約して
> ある。**出し直すのは同じ持ち上げ 1 本だけ**で、検体窓を下げて条件を作りにいく逃げは打たない
> （それをやると「持ち上げの 1 行を消しても緑」に戻る）。2.3 が試して悪化させた
> 「**鎖**を塊ごと背面へ回して始点を固める」形は引き続き採らない。
...
| DD-3 | 後押しは**鎖の根を錨（1 つ手前の窓）の直後へ差し直す**（参照は自分の窓 2 枚だけ）。錨の直後が既に根の巡だけ挿入位置を**先頭**へ切り替える | §13 実測 10（**2026-08-30 に §12.2 実測 9 の形を撤回して差し替え**）。Windows が鎖を並べ直すのは所有側が動いたときだけで、先頭は誰も所有していない＝先頭を動かす形は 24 通り中 0 通りしか収まらない。位置・寸法を変えず（要件 11.1）、鎖の外どうしの相対順も変えない（要件 6.1／6.2）。2 択は「同時に空振りしない」ことが構造で言えるので、未文書の性質に依存しない | 先頭を 2 番目の直後へ（§12.2 実測 9・**空振りする**）／`SWP_NOZORDER`（効かない・実測 7）／`GW_HWNDPREV` の直後へ差し直す形（**挿入位置に他プロセスの窓を渡しうる**——消えると黙って失敗する。§12.9 の 2 件目で実測）／`HWND_TOP`・`HWND_BOTTOM`（グループの絶対位置を無用に動かす） |
...
**登記する裁量（要件 12.2 の列挙は下限＝「少なくとも」。⚠ 着地は 14 件——設計時の見積り 11 件に対し、維持方式・`reset` の余分トークン・出所欄の 3 件が加わった。2026-08-31 に数え直し）**
