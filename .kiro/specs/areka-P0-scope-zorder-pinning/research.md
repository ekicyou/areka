...
8. **要件 9.3 の「不可視」の定義**（G7）: Win32 の可視（現行の実測層）と合成層の可視のどちらを指すか。**非表示のバルーンが隣接判定に効く**という現実を、要件の文言のまま受けるか、設計で明示的に「Win32 の可視」と限定するか。実機サインオフの読み方がここで決まる。
9. **要件 12.3 の訂正の置き場**: 完了 spec 配下の `ghost-window-zorder/brief.md:10` を直接書き換えるか、COMPAT §8 に訂正行を足して上書きするか（先例＝scg が `window-placement` R2.9 を §8 で上書き）。**完了アーカイブを書き換える運用の是非**が絡む。
10. **要件 13 の sylphya への露出**（G7 の下・brief ⑧）: SET 有効群 21→22 へ `seriko.zorder` を**語彙だけ**足すか、語彙表には一切触れず COMPAT §8 と brief の記録のみで縮退シームを成立させるか。**bvc（`.vertical` を実追加）と行が隣接する**ので、足すなら後着が rebase で吸収する前提の合意が要る。
