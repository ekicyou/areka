...
| **鎖の先頭を 2 番目の直後へ差し直す**（実測 9） | ✅ | **参照はどちらも自分の窓**。主張する関係は鎖が既に強制しているものと同じ。位置・寸法は変えない |

**採用する後押しは最後の形**（実測 9）。要件 11.1（位置・寸法を変えない）を満たし、
かつ**外部の窓の生死に依存しない**。逐語値: 助走 `[3,2,1,0,4]`（`3`＝前の部外者・`4`＝後の部外者）→
後押し 1 回で鎖が `0,1,2` の順へ収まり、**部外者どうしの相対順（`3` が `4` より手前）は変わらない**。
...
- **後押しは「鎖の先頭を 2 番目の直後へ差し直す」1 形のみ**（§12.2 実測 9）。自分の窓 2 枚だけを参照するので、
  外部の窓が消えても黙って失敗しない。
...
| 2 | 後押しが効かず鎖が収まらない（全体走行でのみ・36 走行中 4〜8 回） | 後押しの挿入位置に `GW_HWNDPREV`＝**他プロセスの窓**を渡していた。読み取りと書き込みの間にその窓が消えると `SetWindowPos` が黙って失敗する | 後押しを「**鎖の先頭を 2 番目の直後へ**」＝自分の窓 2 枚だけを参照する形へ変更。**本番の設計もこちらを採る（DD-3）** |
