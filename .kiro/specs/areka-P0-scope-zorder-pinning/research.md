...
### 13.7 檻自身の非決定を 2 度潰した記録と、**未決着 1 件**（§12.9 の続き）
...
| 6 | `a_settled_chain_holds_its_order_when_the_deepest_window_is_raised_to_the_front` の自己検査 `control_before == Some(0)`（`zorder_chain_order_tests.rs:687`）が、**全体走行**の 3 プロセス同時 × 120 走行で 1 件赤。同テスト**単独**の 3 プロセス × 120 走行では 0 件 | ⚠ **未確定。** 有力なのは 4 件目と同じ形——「生成したての検体窓は鎖より手前に居るはず」は**こちらが保証していない性質**である（初版 4.1 が置いた自己検査で、assert とその入力は HEAD と byte 同一）。ただし**2.3 が足した §13.1 の掃き出しが新しい外乱である可能性を排除できていない**——窓の生成は 576→4 枚へ減らしたが、**`SetWindowPos` の回数は 24 通り × 6 形 ≒ 864 回のまま減っていない**。5 件目は同型の外乱が他の檻を赤にすることを実証済みであり、「単独走行では 0 件」という所見はむしろ外乱起因を支持する | ⚠ **未了。4.4 へ引き継ぐ**（下記） |

**⚠ 6 件目の引き継ぎ（task 4.4 へ）**

- **試みて差し戻した是正**: 検体窓の始点を成り行きに任せず、鎖の**根**を検体窓の直後へ差し直して
  鎖を塊ごと背面へ回す形に固めた。結果は**悪化**——1/120 → **3〜4/120**（今度は攪乱後に
  `[4, 0, 1, 2, 3]`＝鎖が検体窓を追い越さず `control_after` が落ちる）。劣化を出荷しないため
  **差し戻した**（現在は初版 4.1 の形のまま）。**同じ手を繰り返さないこと。**
- **決着に要る測定**: **HEAD（2.3 の変更を含まない状態）で同じ全体走行を 3 プロセス × 120 回**行い、
  同テストの赤のレートを 2.3 適用後（1/120）と比較する。これをやらない限り「既存の競合」は仮説の
  ままであり、**4.4 の完了状態（赤 0 件）に到達したかどうかを誰も判定できない**。
  - レートが HEAD でも同程度なら **2.3 とは無関係の既存の競合**＝自己検査の書き換え（4 件目と同じ
    「保証していないものを主張しない」方向）で閉じる
  - HEAD で 0 件なら **2.3 の掃き出しが外乱**＝掃き出しの `SetWindowPos` 回数を減らす（始点の順列を
    代表的な部分集合へ絞る等）方向で閉じる
