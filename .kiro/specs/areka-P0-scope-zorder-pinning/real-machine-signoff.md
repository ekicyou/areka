...
## §10 目視サインオフ（2026-08-31・**開発者承認済み**）

§7〜§9 の 8 走行はすべて**ログだけ**で判じたものである（手順書 §1「3 点とも目視を一切使わない」）。
本節は、開発者が**自分の目で**確かめたうえで承認した 1 走行を、別の証跡として登記する。
初版が実機で落ちたとき、ログは「書込 ok・窓は全部見つかった」と言い続けながら重なりが動かなかった。
**目で見たものと機械が測ったものを突き合わせる**ことには、それ自体の値打ちがある。

### 10.1 走行条件

| 項目 | 値 |
|---|---|
| 検体 | `fixtures/emo2-zsp-visual`（`emo2` の shell だけを差し替え・§3.1 と同じ作り。**派生検体なのでコミットしない**） |
| 指定 | `seriko.zorder,1,0`（**数値モード**＝初版が実機で落ちた構成） |
| 実行体 | HEAD `e87e4fcf` で再ビルド |
| 有界 auto-exit | `AREKA_APP_SMOKE_EXIT_MS=300000`（5 分・目視の時間を取るため長め） |
| ログ | `%LOCALAPPDATA%\areka-diag\zsp-visual-demo\visual.log`（628,295 bytes・md5 `78BCFD6A3FCCBB537FE51FC329EEBA6A`・非コミット） |

### 10.2 事前に宣言した並び（起動前に開発者へ説明した内容）

数値モードの `N` はスコープ番号で、そのスコープの窓 2 枚（バルーン → キャラ窓）へ展開される。
書いた順が手前から奥なので `1,0` は——

```
手前 ① エモのバルーン(b1) → ② エモ(s1) → ③ むらさきのバルーン(b0) → ④ むらさき(s0) 奥
```

emo2 は scope 0 が**むらさき**（さくら側）・scope 1 が**エモ**（けろ側）。
すなわち**相方が主人格より手前**へ来る＝指定が無いときと上下が入れ替わる。

### 10.3 三点が一致した

| 観測の系統 | 結果 |
|---|---|
| **①ログ（アプリ自身）** | `[zorder-group] applied … members=b1,s1,b0,s0 normalized=-`／`[zorder-chain] settled … declared=0x22370EFE,0x230108BC,0x21871092,0x1E200C30 measured=（同一） nudge_ok=true` |
| **②判定器** | `J1=PASS  J3=PASS` / **exit 0**。対照（同じログを `-Mode default` で判定）は **exit 1**＝道具は常時緑ではない |
| **③外部プロセスからの実測** | 別プロセスの P/Invoke で生の Z 順を走査。可視窓の 6〜9 番が `エモ / エモ / むらさき / むらさき`＝**`b1,s1,b0,s0` が生きた Z 順にそのまま出ている** |
| **④開発者の目視** | **承認**（2026-08-31） |

### 10.4 繋いだのは 1 本だけ——設計の中核が数字で見える

窓 4 枚の連続対は 3 つだが、`[zorder-chain] linked` は **1 件**、
`[zorder-pair] owner-established` は **2 件**で、**1 + 2 = 3** と合う。

```
b1 ── s1 ── b0 ── s0
   ↑     ↑     ↑
   │     │     └ スコープ 0 の対 → 既存ペア機構（owner-established）
   │     └────── スコープをまたぐ → 本 spec（linked）※これ 1 本だけ
   └──────────── スコープ 1 の対 → 既存ペア機構（owner-established）
```

**「ペアの繋ぎは鎖の部分列そのものである」**（design の Existing Architecture Analysis）が実走で確認できた。
既存機構を 1 行も書き換えずに済んだ理由が、この 1 対 2 の内訳である。

### 10.5 併せて確かめた（開発者の問い）

**Claude デスクトップが常時最前面なのは areka の影響か → 否。** 実測で切り分けた。

- ゴースト窓は 4 枚とも `WS_EX_TOPMOST` **無し**（外部プロセスからの実測）
- 本番コードがそもそも付けない（`crates/areka/src/placement/spawn.rs:572` DD13）
- `topmost` は先送り語彙の走査対象で、本番ファイルに現れたら檻が赤くなる
- ⚠ 近い罠は実在する（`crates/wintf/src/api.rs:117-131`＝常時最前面の窓を挿入位置に渡すと**動かされる側**が帯へ引き込まれる）が、これは自分の窓が巻き込まれる話であり、他プロセスの窓を帯へ移すものではない。かつ task 2.3 で「後押しが名指しするのは自分の窓 2 枚だけ」を檻で固定済み

Claude が上の帯に居るためゴーストは必ずその下に来るが、**鎖の中の 4 枚の相対順は帯の違いに関係なく保たれた**。
要件 6.1／6.2 が「鎖の外どうしの前後は主張しない・鎖の中の相対順だけを守る」と定めた形が、実機でそのとおりに現れている。
