...
- `PhysicalPoint` は文脈で2型ある: `crate::ecs::PhysicalPoint`（=`Point`・**i32**、cursor/registry/drag 用）と `crate::ecs::layout::hit_test::PhysicalPoint`（=`PointF`・**f32**、`hit_test_in_window` の引数型）。**座標変換は OS `ScreenToClient(hwnd, cursor)` へ委譲**し、結果を `PointF::new(pt.x as f32, pt.y as f32)` で `hit_test_in_window` へ渡す（4.2 訂正・下記参照）。~~旧: `client = cursor_screen - WindowPos.position` の手引き算~~ は高 DPI・マルチモニタで表示とズレるため撤回。
