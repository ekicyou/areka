...
走っているゴーストの中で、バルーンの絵と当たり判定の出し先を差し替える手段が 1 つも無い（2026-09-24 再測定＝`reload|ReplaceTarget|Rebuild|swap_sink|replace_sinks` は seriko・present・text で 0 件）。出し先は起動時に `GhostBootOptions.sinks`（`crates/areka-ghost/src/runtime.rs` の `GhostBootOptions` の定義）として値で渡されて固定される。本坑 `areka-P0-shell-balloon-switch`（台帳 #50）には「降ろして起こし直す（案 A）」と「present・seriko・text に差し替えの語を足す（案 B）」の 2 案があり、どちらが安いかはコードを書いてみないと分からない。案 B の鍵は `EmoPresenter::attach_target`（`crates/areka-emo-present/src/presenter/hub.rs`・同じ id を再登録すると表示コンテキストごと置き換える）だが、差し替えの途中の 1 フレームに古い絵と新しい当たり判定が混ざらないか・空の窓が点滅しないかはコードからは分からない。本先進坑は、SHIORI を生かしたまま present のバルーン資産だけを差し替えて 1 フレームも崩れずに表示が続くかを使い捨ての実験で確かめ、開発者が go／違う／直す を判定できる知見を README 3 幕に残す。
...
合否基準（brief より）:
- **go**: 差し替えの前後で、表示中の窓が消えない・古い絵が残らない・当たり判定が新しい絵と同じフレームで切り替わる（混在フレーム 0）。
- **直す**: 混在フレームが出るが、差し替えの順序（当たり判定と絵のどちらを先に替えるか）を変えれば消せる。
- **違う**: `attach_target` による置き換えでは崩れが消えず、案 A（降ろして起こし直す）へ倒す。
...
- **In scope**:
  ...
  - 混在が出た場合に、差し替えの順序を変えた版を試して数を比べること（「直す」の判定材料）。
  - 有界な自動終了。
...
3. The pilot example shall 各フレームで、表示されている絵がどちらのバルーンのもの（一方・他方・両方・どちらでもない）か、および効いている当たり判定がどちらのバルーンのものかを判別し、当たり判定も渡した値からではなく実際に効いている判定に問い合わせて判別する。
4. The pilot example shall 観測したフレームを次の 3 種の崩れで数え、1 つのフレームが複数の種類に当たるときはそれぞれに数える——「混在」（絵と当たり判定が別のバルーンのもの、または絵に両方のバルーンが同時に見えている）・「空」（窓が消えている、または何も描かれていない）・「古い絵の残り」（新しい資産の絵と当たり判定が揃った最初のフレームより後に、古いバルーンの絵が少しでも見えている）。
...
#### Acceptance Criteria
1. The pilot example shall 本番の差し替えの観測と同じ数え方を、わざと崩したと分かっているフレームに当てる較正を行う。
...
### Requirement 5: 差し替えの順序の試し分け

**Objective:** As a 開発者, I want 混在が出たときに、当たり判定と絵のどちらを先に替えるかを変えた版でも数を取ること, so that 「直す」（順序で消せる）と「違う」（消せない）を見分けられる

#### Acceptance Criteria
1. The pilot example shall `attach_target` による置き換えをそのまま使った差し替えを、基準の版として観測する。
2. If 基準の版で「混在」「空」「古い絵の残り」のいずれかが 1 以上数えられたとき, the pilot example shall 差し替えの順序を変えた版（当たり判定を先に替える版と、絵を先に替える版。いずれも同じフレームの中で揃える）でも同じ観測を行う。
3. The pilot example shall 各版の崩れの数を、版の名前とともに区別してログへ出す。
4. The pilot shall 1 フレーム遅らせて辻褄を合わせる版を、崩れを消した版として扱わない。
5. If 順序を変えた版が既存の crate を変えなければ組めないとき、またはどの版でも崩れが 0 にならず既存の crate を変えなければ消せないと分かったとき, the pilot shall その変更を先進坑の中で行わず、何を変える必要があるかを学びとして記録する。
