...
- **J2（5.4・飽和パターンの不在）**: 実走全体で |Changed 回数(まばたき) − Changed 回数(目)| ≤ 2、かつ最後のまばたき Changed が最後の目 Changed の 120 秒以内にあること（是正前観測: まばたき 3 回で恒久沈黙 vs 目は末尾まで継続——この形の不在を直接判定）。**この判定式は汎用則ではなく emo2 fixture 固有の較正値である**——emo2 ゴーストが毎回の表情変更で目とまばたきをペア送信することは 2026-08-11 実走で直接観測済み（この前提が成り立たない他ゴーストへ流用しない）。是正前保全ログで「必ず赤」となることの較正を維持する
