...
- [x] 7.2 emo2 実機実走とサインオフ受け入れ記録
  ...
  - _Depends: 7.1_
  - _解決済み（2026-08-11）: 一度 J3（目視）が FAIL となり Blocked にしたが、根因（bind から外れたbind 種アニメの最終コマ残留を掃除する経路の不在）を開発者裁定でスコープへ取り込み、タスク群 8 の3 段（state／looper／plan）で是正した。再実走で J1・J2 とも PASS・25 分長期実走で J3 も合格。経緯は real-machine-signoff.md §4（根因）／§経緯 と design.md D9 が正本_
...
- [x] 8. 【スコープ追加】bind から外れたパーツの残留コマの掃除（2026-08-11 裁定・Requirement 7）
- [x] 8.1 発行前の残留除去（状態側）
...
- **task 7.2／8.4（2026-08-11・完了）**: 実機サインオフ**合格**。第 3 回（7 分）＝J1 PASS（0/61）・J2 PASS（差 3・末尾差 2.19 秒）。第 4 回（25 分・J3 長期）＝J1 PASS（0/285）・**開発者目視でジト目固着ゼロ**（1402 が 13 回 bind され全て復帰・掃除発火は①11 回＋②3 回）。第 4 回の J2 条件 A（差 9 > 4）は 7 分較正の時間比例ドリフト（想定 10.7）であり飽和の形ではない——手順書 §7 の裁定手順で検分し、正準判定は同一較正条件の第 3 回 PASS を採った。`cargo test --workspace` exit 0。開発者宣言「この問題は解決とします」。
