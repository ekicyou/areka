...
EYE_CATEGORY = "目"
# J2 の許容値（emo2 固有較正）。
# 回数差は 2026-08-11 開発者裁定で 2 → 4 へ改訂した。初版の 2 は是正が正しく効いても
# 赤になる（タスク 7.1 の較正で、保全ログから排他置換を再現した想定形が まばたき 22 /
# 目 25＝差 3 になった）。emo2 は目「べそ/笑顔/静観」がいずれもまばたき「----」へ写る
# 多対一の対応を持つため、目だけ変わってまばたきが据え置きになる遷移が数回起きる。
# 想定値 3 に余裕 1 を足して 4。是正前の実測は差 22 ゆえ飽和との桁差は保たれる。
J2_COUNT_DIFF_MAX = 4
