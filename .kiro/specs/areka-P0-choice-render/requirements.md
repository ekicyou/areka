...
- **In scope**:
  ...
  - 実 emo2・実 pasta.dll・実 DPI（≠96）でメニュー選択肢行が見えること、および注入 hover でハイライトが光ることの人間サインオフ（**両方必達**）。そのために、実機上で hover 状態を注入できる**有界なデバッグ導線**（実ポインタ非依存・本番既定無効）を本仕様が提供する（実ポインタ配線の代替ではなく、注入駆動サインオフの実機到達手段）。
- **Out of scope**:
...
1. The 表示層 shall 短い縦並びメニュー（`\n` 区切り 2〜4 項目）＋fixture 指定 cursor.\* スタイル＋`\_l` 字下げ＋cursor.\* 未指定バルーン向けの矩形反転ハイライト縮退（Requirement 4.3）を M1 の実導出対象とする。
...
5. Where 正典で未確定・M1 非対象の形（`\_l` の負値/省略・emo2 未使用の cursor.\* サブキー）が与えられるとき、the 表示層 shall 当該入力を致命扱いせず語彙を保持したまま安全に縮退（ログ記録のうえ既定/スキップ）させる（cursor.\* 未指定時の矩形反転ハイライトは M1 実導出対象＝Requirement 4.3／6.1 であり、この縮退リストに含めない）。
...
6. The 表示層 shall 実機サインオフ用の hover 状態注入導線を、実ポインタ非依存かつ本番既定無効の有界な形（デバッグ導線）で提供し、当該導線が本番描画経路と既存資産の決定論挙動を変更しないことを保つ（R8.2 の実機到達手段。導線の具体形は設計フェーズで確定する）。
