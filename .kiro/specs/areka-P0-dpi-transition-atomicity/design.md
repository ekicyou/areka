...
- `presenter/show.rs` の予算域 :99-173（compose／resample／mask／insert）と定常アロケーション 0 の不変量＝`recompose-budget`。**本設計が触る範囲（着地後の実測で是正・task 7.5）**: ⑴ upload 直前の供給面寸の控え `prev_size`（:305）と upload 後の `resized` 算出（:317）、⑵ **手順 (3.5) から手順 (3b) へ前倒しした** `prev_physical`／`size_changed` の導出（:334-338）——下の遷移観測が `size_changed` を判定材料に要るためであり、`applied`／`native_size` はこの区間で 1 度も書かれないので**前倒しで値は変わらない**（同旨をコード内コメントが述べる）、⑶ 遷移観測の発行 2 か所（upload 直後 :340-359 と可視化後 :385-399・条件は `observe_surface`（:347）の札 1 つを共有）、⑷ perf 行への `frame:` 1 フィールド追加（:480）。`chain.rs` は無改変。**upload エラー分岐は移動していない**（`test-cage-determinism` ④の観測点）——merge-base の :297-301 が現ツリーでは :306-310 にあるが、これは上に 9 行が挿入されたぶん番号が下がっただけで、分岐の中身は 1 文字も変わっていない（当該ハンク `@@ -294,6 +297,12 @@` の追加は `let prev_size` 1 行とその説明コメント 5 行だけで、**分岐の 5 行は差分に 1 行も現れない**）。
- `windowposition-limit` の表示位置補正（本体 `apply_balloon_limit_gate`・`window_move.rs:986`）とキーワード基本位置の一度きり再導出（`keyword_base.rs:59-163`）＝本設計は呼ばない・変えない。**錨は関門の呼出（`window_move.rs:709`）ではなく本体に置く（task 7.5 で裁定）**——呼出は本設計が改造する `enqueue_window_set_pos`（:634-777）の内側にあり、そこを錨にすると「触らない」節が本設計の改造域を指すことになる。本設計が変えていないのは補正の**中身**なので、錨も中身の側に置く。なお `:822` は本変更より前から指す先が合っていなかった（HEAD の :822 は `log_window_move` の窓種別 marker 不在ガード）。
...
- **D11 flush 検査口＝`drain_window_pos_commands()`（実行せずに取り出す公開関数）＋合流の純関数**。決定論テストはキューの中身で回数・順序・合流を検証し、実 `SetWindowPos` は呼ばない。（**例外 1 件・task 7.2**: `command_batch_tests.rs` はバッチ投入の成否という**実 API の分岐**を測るので実窓を作り、実 `SetWindowPos`／`DeferWindowPos` を通す。実 DPI は要らずメッセージポンプも要らない〔`EndDeferWindowPos` は同一スレッドへ同期 send する〕。プロセス共有の自発カウンタを触るので `lock_self_initiated_for_test` をちょうど 1 層取る）
...
├── command_batch_tests.rs             # NEW(7.2): バッチ投入（DeferWindowPos）の対テスト（1 バッチ・引数不変・自発ガード・縮退・O9 の本数）
...
├── follow/window_move.rs              # MOD: enqueue_window_set_pos のタグ／既定位置追跡／ground レコード／move_window_with_route／deferral_covers_route(7.5)
├── follow_window_move_hold_watch_tests.rs # NEW(7.5): 監視が鳴る対象の分類（12 語＋route 無し）
...
| `crates/wintf/src/ecs/window/command.rs` | `tag` フィールド＋`with_tag`／`enqueue` の合流／`flush` の begin・write・end レコード／`drain_window_pos_commands`。MOD(5.3): 合流が着地（`COALESCIBLE_FLAGS`／`REQUIRED_FOR_COALESCE`／`is_coalescible`／`find_merge_target`／`merge_into`／`coalesce_geometry`）。`enqueue` は記録を**合流の後**に組む（合流先の通し番号は畳んでみるまで判らない）。**MOD(7.2)**: C8 の候補 **B-2b** を実装——`flush` の適用を `Begin`／`Defer`／`EndDeferWindowPos` の **1 バッチ**へ（`window_pos_args`＝引数を組む純関数／`apply_as_batch`／`apply_sequentially`＝縮退経路／`BatchDegrade`＝縮退の理由語 3 種／`begin_batch`＝確保（テストは失敗側へ倒せる））。ガードは**バッチ全体**に掛ける（`EndDeferWindowPos` が送るメッセージまで自発の内側・要件 10.2）。窓書込の実施ログは `log_window_write` へ括り出し、**字面は変えず** `via=`（`SetWindowPos`／`DeferWindowPos`）を足した | `new()` の 7 引数は不変。Z 専用指令は合流対象外。**同一窓の畳めない指令は仕切りとして働く**（C2 の合流規則を参照）。**バッチ化は引数も並びも変えない**（`window_pos_args` が 7 項目をそのまま運び、両経路が同じ順で回す）。書込レコードは**適用の後に**まとめて出す（`ax`〜`ah` の読み戻しが適用後の値になる）。実施ログの字面 `[guarded_set_window_pos] Calling SetWindowPos` は `dpi-window-vanish` の診断手順 O9 と `zorder_pair_*_tests.rs` の 16 本が数えている**固定トークン**ゆえ変えない |
...
| `crates/wintf/src/ecs/window_proc/window_pos.rs` | `WM_DPICHANGED`（:325）と `WM_WINDOWPOSCHANGED`（:41）に `msg` レコード（:58-66／:335-343）／採用時の同期書込（:460-490・`guarded_set_window_pos` は :464）に `write stage=sync` レコード（:468-489）。**MOD(7.2)**: 経路 A の `write stage=sync` は一括 flush のバッチを通らないので `in_batch: false` | 採否判定（`dpi_suggested_position_decision` の呼出 :406-409）不変（`dpi-window-vanish`） |
...
| `crates/areka/src/placement/follow/window_move.rs` | `enqueue_window_set_pos`（:634-777）: タグ付与・既定位置追跡・キャラ Bottom の `ground` レコード／`move_window_with_route`（:63）追加（`move_window_to`（:45）は委譲で不変）。MOD(5.5): **既定位置追跡が着地**——`track_default_char_pos`（非公開ヘルパ・:817-872）を新設し、`WindowPos` の bypass ミラーを書き換える**前**に呼ぶ（後に呼ぶと一致判定が常に成立し、明示操作で動かした窓まで追随する）。追随の条件は D9 の 3 つ（route が `is_system_reanchor`／対象が `CharWindowMarker` を持つ／書込前の現在位置が既定位置と一致）で、`None`（復元位置）は `None` のまま。台帳・位置が引けない形は `debug!` を残す（書込自体は成立ゆえ失敗ではないが、無音にすると追随の有無が事後に判らない）。連鎖確定の書込は `MoveCue`＝明示操作ゆえ本規則に掛からず、`drain_resnap` 側の明示的な `set_default_char_pos` と重ならない。**`move_window_with_route` は task 5.5 の時点では不要だったが、task 5.6 で新設した**——遷移後の解き直しは「寸を変えずに位置だけを書き、随伴バルーンを連れて動く」＝`move_window_to` と同一の手続きでありながら、route だけが `ChainRealign`（システム由来）である必要がある。`MoveCue` のまま書くと既定位置の追跡（D9／D16）が効かず、次の遷移で当該スコープが「明示的に動かされた」へ倒れる。`move_window_to` は新関数への委譲になり、幾何・随伴バルーンの route（`BalloonFollow`）は 1 bit も変わらない。MOD(5.4): 待ち札の適用範囲の**不変条件の監視**を入口へ（`warn!`＋`debug_assert!`）。**随伴バルーンの追従（`PlacementRoute::BalloonFollow`）だけは監視の対象外**——随伴の追従は 4 つの見送り点を通らないので止まりはせず（当初は 3 点と書いていた——4 点目は MOD(6.5) で加わった。本文 :668・:675 は 4 点で書かれている（task 7.5 で当該コメント塊を書き直したため 3 箇所から 2 箇所になった。見送りの 4 点そのものは不変））、外す理由は止まらない書込をこの監視が「漏れ」として鳴らすからである（偽の警報を防ぐ）。C5 の Risks を参照。2 窓の中心が別モニタに乗る形は実装中に決定論テストが実際に踏んだ。MOD(6.5): 監視の**説明**を 3 点から 4 点へ是正（作業領域変化を契機とする再スナップを加えた）——監視の条件も水準も不変であり、`WorkAreaResnap` が鳴らなくなるのは見送り側が塞がったからである。MOD(7.5): 監視の**条件**を是正——`route != Some(BalloonFollow)` の 1 語免除を、新設の純関数 `deferral_covers_route`（:508・`Option<PlacementRoute>` の網羅 match）へ置き換えた（監視の条件式は :678）。鳴るのは「見送りが覆うべき経路」7 語だけで、明示操作（`MoveCue`・`Restore`・`BalloonLimitRelease`・route 無しのドラッグ）と `SpawnInitial` では鳴らない。`warn!`（:682・メッセージ行 :687）と `debug_assert!`（:689）は両方そのまま残す。あわせて `move_window_to` の doc に「明示操作は監視の対象外」を明記 | 単一ライター維持。limit 関門・手順 5a 不変 |
...
| ファイル | 変更内容 | 起点タスク |
|---|---|---|
| `crates/wintf/src/ecs/window/transition_diag.rs` | NEW: target 定数・frame ミラー・TickStart・入れ子する flush epoch・`WriteTag`・レコード純関数・経路語 3 つ。**MOD(7.2)**: 書込レコードへ `in_batch`（`FIELD_IN_BATCH`）を足し、`write_line` の末尾へ載せた。**必須列 `WRITE_FIELDS` には入れず** `WRITE_OPTIONAL_FIELDS` を新設した——是正前に採取した実機ログにはこのフィールドが無く、必須にすると判定器が確定済みの逐語ログを軒並み「壊れた行」にしてしまうため（発行側が必ず載せることは逐語テストが固定する）。`call_us` の doc も追随（バッチ時は投入だけの所要） | 1.1・1.2・2.1・2.2 |
| `crates/wintf/src/ecs/window/transition_diag_tests.rs` | NEW: 語彙固定（正例／負例）・実濾過・刻印の権威。**MOD(7.2)**: `write_line` の逐語期待へ `in_batch=true` を足し、任意フィールドも発行側は必ず載せることと、縮退した書込が同じ位置に `in_batch=false` で載ること（フィールドごと消えないこと）を対で固定 | 1.1・1.2 |
...
| `crates/wintf/src/ecs/window/command_batch_tests.rs` | NEW(7.2): バッチ投入（B-2b）の対テスト 8 本——⑴ 引数の逐語運搬（Z 専用の挿入位置・表示フラグ・観測タグが引数へ漏れないこと）、⑵ キューの並びが写像で保たれること（Z 専用指令の位置を含む）、⑶ 縮退の理由語が 3 つとも別語であること、⑷ **実窓 3 指令が 1 区間 1 バッチで投入され `in_batch=true` を名乗ること**（同一本体の内側に**縮退した側**を対で置く＝札が定数で真でも緑にならない）、⑸ 偽ハンドルで `DeferWindowPos` が失敗し1 本ずつへ縮退して理由語が残ること、⑹ `EndDeferWindowPos` が送る `WM_WINDOWPOSCHANGED` の内側で`is_self_initiated()` が真であること（要件 10.2・1 本ずつの経路と対で見る＋ガード外で偽になる較正）、⑺ **Z 指令列の最終順序がバッチでも 1 本ずつでも一致すること**（要件 10.3・実窓 2 組の A/B）、⑻ **実施ログ（O9 の固定トークン）が経路によらず指令 1 件につき 1 行であること**（積み上げ時点で出すと破棄されたバッチが二重計上される＝レビューの実測で発覚。同一本体の内側に `via=` の絞り込みが恒真でないことの較正を置く）。実窓を作るテストは `lock_self_initiated_for_test` をちょうど 1 層取る | 7.2, 10.2, 10.3 |
...
| `crates/areka/src/placement/follow.rs` | MOD: ファサード再束縛 `use super::transition_diag;`（`window_move` が `super::` で辿る）＋兄弟テスト接続。MOD(5.1): `MonitorDpiEntry`／`MonitorDpiTable`／`MonitorSources`／`same_monitors` の再輸出。MOD(5.4): `DpiSyncHold` のファサード再束縛（単一の窓書込口が不変条件を見張るために読む）。MOD(5.5): 兄弟テスト `follow_default_pos_track_tests.rs` を接続。MOD(7.5): `DpiSyncHold` の再束縛コメントへ「見張る相手は見送りが覆うべき経路に限る」を明記＋兄弟テスト `follow_window_move_hold_watch_tests.rs` を接続 | 2.4, 5.1, 5.4, 5.5, 5.8 |
...
| `crates/areka/src/placement/transition_judge.rs` | NEW: 行パーサ（`judge-perf.py` と同一の辞書化・同名フィールドは欠陥）・遷移切り出し・判定量の集計。消費者がテストとサインオフランナーだけなので `#[cfg(test)]` のモジュールとして置く（bin crate ゆえ本番ビルドでは全項目 dead_code になり、項目ごとの許可属性が以後の真の dead code を隠すため）。**`visualize_to_write_us` は遷移区間内の全ての（可視化, 同一フレームの書込）組にわたる真の最大**（最後の 1 組ではない）＝task 3.2 は `Bounds::signoff` の上限をこの最大に対して置く。`frames_indeterminate` は「一様に 0」と「`frame` が 1 つも読めなかった」の両方で立つ。MOD(3.2)＋MOD(4.2)＋MOD(6.6): 意味の裁定 3 件を持つ——⑴ 随伴の同一フレーム性は**バルーンの `origin=BalloonFollow` の位置指令が書込へ着地したフレーム**で測る（要件 4.3 の義務は位置の追従であり、`KeepPositionResize` の遅れは要件 4.6 の見送りゆえ欠陥ではない）。**MOD(6.6)**: 着地の見つけ方を 2 通りへ広げた——⒜ `kind=write` が `origin=BalloonFollow` を名乗る（畳まれなかった指令）、⒝ `kind=enqueue` の `merged_into_seq` が指す先の書込（合流された指令）。task 5.3 の合流（C2）は**先着の経路語を残す**ので、寸の指令が先に積まれるバルーン窓では随伴の経路語が `kind=write` から消え、⒜ だけを見る当初の形は空振りしていた（2026-08-21 の実機再採取では 7 遷移中 5 遷移が `pairs=0`・残る 2 遷移も片側だけが測れており、キャラ窓ごとに数えて**計 12 件**が「量が欠けている」として落ちた。追従そのものは成立していた＝**製品ではなく測り方の欠陥**）。引き当ては**同じ窓の・その通し番号の・当該 `enqueue` より後に現れる最初の書込**に限る（窓を見ないと別窓の書込を掴み、観測位置を見ないと `seq` の振り直しで積まれる前の書込を掴む）。**書込へ届かなかった指令は数えない**（積まれただけの随伴を合格の根拠にしない）。**本番の観測語彙は 1 つも増やしていない**、⑵ `frames_to_last_write` は**見送り窓への書込を数えない**（4.6 で現状維持となった窓が後で可視化された際の書込は当該遷移の続きではない。抜け穴にならないのは、見送り窓の随伴位置は `balloon_same_frame` が引き続き見張るため）、⑶ **理由による除外の限定**（MOD(4.2)）＝除外を駆動するのは `reason=invisible` のみで `reason=k-unchanged` は使わない（定常状態の空振りゆえ・4.6 の裁定の注記 2026-08-20）。併せて `transition_judge_verdict.rs` へ `Violation::AllWrittenWindowsExcluded` を新設した（同ファイルの行に登記）。**MOD(7.2)**: `optional_fields`（任意フィールドの表）を新設した。`in_batch` を `required_fields` へ入れない理由（是正前の採取ログを後から否定しない）を doc に登記し、読む側は欠落を受け入れ、書く側の義務は発行側の逐語テストが持つ、という分担を明示した。**判定量・違反・Report は 1 つも変えていない** | 3.1, 3.2, 4.2 |
...
| `crates/areka/src/placement/transition_judge_test_support.rs` | NEW: 種別ごとの観測行組立（フィールド名は発行側の `pub const` を参照）＋テーマ間で共有する助走。MOD(6.6): `enqueue` を `enqueue_merged_into`（合流先の通し番号を字面で受け取る）への委譲へ替えた——従来の `enqueue` は `merged_into_seq` を番兵に固定していたため、**合流で書込が消えた形を檻が 1 度も作れなかった**（task 6.6 の欠陥が通り抜けた機序そのもの）。既存の呼出は 1 行も変わらない | 3.1, 6.6 |
...
| `crates/areka/src/placement/transition_judge_coalesced_follow_tests.rs` | NEW(6.6): 合流（C2）を通った随伴の追従を判定器が数に入れられることの檻。手組みの 4 通り（合流された／されない × 同一フレーム／次フレーム）を**同一のテスト本体の内側で対にし**、⑴ 合流で書込が消えた形が測れること ⑵ 本当に遅れた形が従来どおり違反として立つこと の両方を固定する（⑴ だけの檻は「何でも測れたことにする」退行で緑のまま通る）。加えて ⑶ 指令が積まれただけで書込に届かなかった形は**未測定**のまま ⑷ 合流先を通し番号だけで引き当てると掴んでしまう「積まれる前の書込」を掴まないこと。さらに **2026-08-21 の実機再採取の遷移 #3（全 56 レコード）を逐語で埋め込んだ回帰**を持ち、`balloon_pairs_checked=2`・決定論系統の全件合格・記録票の 9 量の逐語再現を固定したうえで、合流先の書込を 1 フレーム遅らせた変異で違反が立つことを同じモジュールの内側で対にする。**是正前は 3 本が赤**（`a_companion_follow_folded_into_another_command_is_measured_and_a_late_one_still_violates`・`the_recaptured_transition_measures_both_companions_and_passes_the_deterministic_bounds`・`the_recaptured_transition_still_flags_a_companion_that_lands_a_frame_late`）。残る 2 本（`a_companion_command_that_never_reached_a_write_is_still_unmeasured`・`a_merge_target_written_before_the_companion_command_is_not_its_landing`）は**是正が行き過ぎる向きを見張る側**なので是正前も緑である | 6.6, 4.3, 7.3, 8.3, 8.5 |
| `crates/areka/src/placement/transition_judge_verdict.rs` | NEW: 上限（`Bounds::deterministic`／`Bounds::signoff`）・違反（`Violation`／`Quantity`）・`judge`／`judge_transition_log`／`Report`。実機専用の上限は**暫定値**で置き task 4.3 が実測から差し替える。判定量の集計（`transition_judge.rs`）と分けるのは、量が語彙に追随し上限が要件に追随する＝変更の理由が異なるため（1,000 行の目安も満たす）。MOD(4.2): `Violation::AllWrittenWindowsExcluded` を新設し、書込のあった窓が 1 つ残らず除外された遷移を「合格」ではなく**未測定**として立てる（⑼ の被覆検査が `judged_windows` を回る形ゆえ、全窓除外では 1 度も回らず恒真になっていた）。**MOD(5.4)**: `HOLD_FRAME_ALLOWANCE` を本番の `dpi_sync::DPI_SYNC_HOLD_MAX_FRAMES` の参照へ替え、task 3.2 が暫定で置いた同値の二重定義を解消した。**MOD(4.3)**: ⑴ 実機専用の上限を確定値へ差し替え（`PROVISIONAL_*_US_MAX = 16_700` → `VISUALIZE_TO_WRITE_US_MAX`／`FLUSH_TOTAL_US_MAX` = `16_667`）。根拠は採取機に依らない形へ替えた＝「提示される 1 フレームは 60Hz を下回らない限り高々 1/60 秒」（全文は `mechanism-ledger.md` §4・L9）。名から `PROVISIONAL_` を落としたのは確定後も「暫定」と読ませないため。⑵ `Report` の `Display` が**合否によらず判定量を刷る**ようにした（`量:`／`量(参考):`／`量(窓):`／`量(見送り窓):`・欠けた量は番兵 `-`）——`PASS` の系統の量が消えると是正前後の比較が毎回生ログからの手起こしになる（task 4.2 で実際に起きた）。**MOD(6.6)**: `companion_expectations` の doc を裁定⑴ の是正へ追随させた（対を検査できるのは「`origin=BalloonFollow` の**位置指令が書込へ着地した**とき」であり、着地の引き当ては集計側 `summarize` が持つ）。**判定側のコードは 1 行も変えていない**——本関数は `balloon_follow_windows` を読むだけで、引き当ての規則を第二の写しとして持たない | 3.2, 4.2, 4.3, 6.6 |
...
| `crates/areka/src/placement/transition_signoff_procedure_tests.rs` | NEW: サインオフ手順書（C10 `signoff-procedure.md`）の判定語が発行側・判定器の単一定義元と一致することの檻。⑴ 手順書に載る観測行の例が発行側の語彙だけで書かれていること（種別・段階・per-kind の必須フィールド）、⑵ レコード種別 10 種が**すべて**手順書に現れること（片側だけだと「例を書かなければ緑」の恒真になる）、⑶ ランナーの入口の語（環境変数名・観測 target・行頭タグ・Report の 2 系統名・ランナーのテスト名）が**トークン境界つき**で載っていること、⑷ Report の出力例に並ぶ違反行が**その上限系統で実際に出得る**ものであること——⑷ の分類は手書きせず、最大違反の判定量へ `Bounds::deterministic()`／`Bounds::signoff()` を当てて**判定器が実際に積んだ違反**から起こす（`frame_bound` の門の内外が変われば分類も自動で追随する）。検査述語そのものの較正（壊した行が落ちる／後ろに字が付いた誤記が通らない／帰属を入れ替えた例が両方向とも捕まる／門の外の共有違反は両系統で許される）を同ファイルに置く。**MOD(7.2)**: 手順書の例に許す語彙へ判定器の `optional_fields` を足した（手順書が `in_batch=` を引用できないと、行に在る語を採取者が手順書で確かめられない）。**表は判定器から引くだけで語は自前で書かない**。結線が効いていることは変異で確認済み——`optional_fields` を空へ倒すと`the_procedure_only_quotes_record_lines_the_emitters_can_produce` が「語彙外のフィールド `in_batch=`」で赤になる | 4.1 |
...
| `crates/areka/src/placement/dpi_sync_tests.rs` | NEW(5.4): 純判定の全分岐（一致／表なし／不一致の上限未満・上限ちょうど・超過／周回境界）・判定語と観測点語が単一定義元を引くこと・World 越しの評価（中心が乗るモニタの値を引く／窓生成前の `CW_USEDEFAULT` は溢れずに `Proceed`／表なしは `Proceed`）・札の付け外しは拡大率の相だけ（起点は据え置き・一致で外れる・上限で外れる）・他 2 点は読むだけ・**待ち札のある窓への窓書込がその場で落ちること**（`#[should_panic]`）とその陽性の対（札が無ければ通る）。MOD(6.5): 4 つ目の観測点語が単一定義元を引くことと、`Resnap` と `WorkAreaResnap` が**別語**であること MOD(7.5): 監視が鳴る側の 1 本を `MoveCue` から `ChainRealign` へ差し替え（明示操作は鳴らないことが正しいと裁定したため）＋「明示操作は同じ札のある窓へ届いても鳴らない」対を同一土台で新設 | 5.4, 5.8 |
...
| `crates/areka/src/emo2_boot/frame_work_area_resnap_hold_tests.rs` | NEW(6.5): 整合ゲートの **4 点目**（`resnap_for_work_area_change`）の対テスト。到達条件 2 つ——⑴ 窓の拡大率と帰属モニタの表が食い違ったまま札が残る／⑵ その待ちのあいだに作業領域が動いて同期段が `Some` を返す——を `FrameHarness` で組み、**是正前は `enqueue_window_set_pos` の `debug_assert!` が落ちて赤**・是正後は当該窓への書込 0。零件の主張には同一本体の内側へ 2 つの陽性の対を置く: 札の無いキャラ窓は従来どおり `WorkAreaResnap` で書かれる（4 点目が再スナップを丸ごと止めていない）／札のあるバルーンへ随伴の追従（`BalloonFollow`）は届く（C5 の唯一の例外が生きている）。ほかに 4 語目の観測点語 `site=work-area-resnap` の発行（対＝札が無ければ 1 行も出ない）と、不変条件の監視が `WorkAreaResnap` では鳴らないこと（対＝見送り点を通らない経路なら同じ土台で今も `debug_assert!` が落ちる `#[should_panic]`）。要件 7.2 の 2 水準（120／192）で走る MOD(7.5): 陽性の対（`#[should_panic]`）の経路語を `MoveCue` から `ChainRealign` へ差し替え——監視が生きていることを示す役目は保ったまま、明示操作で鳴らす形を捨てた | 5.8, 7.2 |
| `crates/areka/src/placement/follow_window_move_hold_watch_tests.rs` | NEW(7.5): 待ち札の監視が**鳴る対象**の檻。⑴ 分類の網羅——`PlacementRoute::ALL` の 12 語すべてを表と突き合わせ（テスト側にも独立に書き下す二重記帳）、語数一致・重複無しも別に主張し、route 無しも 1 本で固定する ⑵ 本物の書込口を通した対——`\![move]`（`MoveCue`）と**本物のドラッグ経路**（`on_char_drag` の単一ライター腕＝route 無し）は札のある窓へ届いても `debug_assert!` が撃たれず `warn!` も 0 件（是正前はどちらも panic して赤）／同じ土台・同じ札で経路語 1 点だけを変えた `ChainRealign` と `KeepPositionResize` は従来どおり落ちる（`#[should_panic(expected = "DpiSyncHold")]`）／随伴の追従の免除が生きている。零件の主張には必ず「窓が実際に動いた」を同一本体で対にする。**ミューテーション検証**: `debug_assert!` の条件を反転（`false`→`true`）すると**鳴る側 4 本だけ**が赤（免除側は 1 本も動かない）／分類の使い方を反転（`if !deferral_covers_route`）すると免除側と鳴る側の 12 本が赤 | 5.8 |
...
| 5.8 | 一度書き | D5 配置＋C5 hold（D15）。監視が鳴るのは**見送りが覆うべき 7 経路**だけ（task 7.5・`deferral_covers_route`）。見送りは **4 点**——拡大率の相・報告寸の突合・**実表示寸の**再スナップ（`resnap_shell_targets`）・**作業領域変化を契機とする**再スナップ（`resnap_for_work_area_change`・4 点目は task 6.5 で追加。C5 当初の列挙は「再スナップ」の語が前者だけを指しており抜けていた） | 経路 (a)/(b) 両順序の決定論テスト＋`frame_work_area_resnap_hold_tests.rs`（4 点目・120／192）＋`follow_window_move_hold_watch_tests.rs`（監視の分類・12 語＋route 無し） |
...
**Dependencies**
- Inbound: areka 単一ライター（`window_move.rs:634`）、zorder 維持系（`zorder_pair_maintain.rs:483`）、`apply_window_pos_changes`（`graphics/systems/window_pos.rs:90-106`）（P0）
...
- Risks: `finalize_chain` はスコープ 0 を錨に左へ連ねる規則を維持する。X の再解決で `windowposition-limit` のバルーン補正（`window_move.rs:634` 関門）は元書込の route を保ったまま働く（変更なし）。
...
- 待ち札の**適用範囲の不変条件**: 「`DpiSyncHold` を持つ窓に対する窓書込（`enqueue_window_set_pos` 到達）は 0」。これを `enqueue_window_set_pos` の入口で `debug_assert!`＋`warn!` として置き、すり抜け経路が増えたときに実機ログで見えるようにする（ログ無し失敗経路の禁止）。**この監視は実際に穴を 1 つ捕まえた**——task 6.2 のレビューで 4 点目（`resnap_for_work_area_change`）が名指しされ、task 6.5 で見送り側へ加えた。監視自体は 4 点目を足した後もそのまま残す（将来の書込口のために）。**ただし不変条件の主語は「すべての窓書込」ではない（task 7.5 で確定）**——0 を作るのは見送りの側であり、その見送りは設計上**明示操作を止めない**。ゆえに監視が鳴る相手は「見送りが覆うべき経路」に限る（下の項）。
- **不変条件の例外 その 1 ＝随伴バルーンの追従（task 5.4 で確定。task 7.5 まで唯一の例外だった）**: `PlacementRoute::BalloonFollow` の書込は監視の対象外とする。バルーンの位置は独立した量ではなく**キャラ窓の従属量**であり、キャラ窓が動いた同一書込の一部として動く。**外れるのは監視であって見送りではない**——随伴の追従はそもそも 4 つの見送り点を通らないので、対象外にしなくても止まりはしない。外す理由は、止まらない書込をこの監視が「漏れ」として鳴らしてしまうからである（偽の警報を防ぐ）。2 窓の中心が別々のモニタに乗る配置（キャラ窓は表と揃い、バルーンだけまだ食い違う）では、待ち札の付いたバルーンへ随伴が**正当に**届く。しかも `dpi_sync_decision` は拡大率と表の値だけを見て作業領域を見ないので、表が古くても随伴の位置は壊れない。バルーン**自身**の書込（拡大率の相・報告寸の突合・**実表示寸の**再スナップ `resnap_shell_targets`・**作業領域変化を契機とする**再スナップ `resnap_for_work_area_change`）は 4 点の見送りが引き続き覆う（task 6.5 以前の本文はここを「再スナップ」の 1 語で書いており、`resnap_shell_targets` しか指していなかった＝4 点目が落ちた出所である）。この形は実装中に決定論テスト（要件 10.7）が実際に踏んで判明した。
- **監視が鳴る相手は「見送りが覆うべき経路」に限る（task 7.5 で確定・実装は `window_move::deferral_covers_route`）**: 免除が `BalloonFollow` の 1 語だけだったあいだ、この監視は**止まらないことが正しい書込**まで「漏れ」として鳴らしていた。`move_window_to` の doc（`window_move.rs:40-41`）が既に裁定を書いている——「**スクリプトの明示操作**ゆえ遷移ガードは適用しない（ドラッグ・`Restore` と同族・D13 帰結⑵）」。すなわち欠陥は見送りの側ではなく、**監視の免除がその裁定に追随していない**ことであった。debug ビルドでは利用者が窓を 1 回ドラッグしただけで `debug_assert!` が落ちる（`\![move]`・バルーンドラッグの解放も同じ）。是正したのは**監視だけ**であり、見送りの 4 点は 1 bit も変えていない。分類は `Option<PlacementRoute>` の**網羅 match** 1 本が持つ（既定の腕を置かない＝新設語が黙ってどちらかへ倒れない・task 6.5 の学び）。
  - **鳴る（見送りが覆う）7 語**: `DpiReproject`（拡大率の相）／`ReportedSizeReconcile`（報告寸の突合）／`Resnap`（実表示寸の再スナップ）／`WorkAreaResnap`（作業領域変化を契機とする再スナップ）＝4 点そのもの。加えて `KeepPositionResize`——唯一の発行元 `reconcile_window_size`（`frame/dpi.rs:159`）が上の 2 点の**内側**にしか無い。`ChainRealign`——4 点は通らないが**自前の見送り**を持つ（C4 の解決条件 2＝札を持つゴースト窓が 1 つも無いこと・`chain_realign.rs:144`）。`AnchorChange`——本番では未結線（`anchor_changed_system` を schedule へ登録している箇所は無く、呼ぶのは決定論テストだけ）ゆえ現に到達しないが、由来は基盤側の置き直しであり、結線した瞬間に「見送りを足すか」を必ず決めさせるため鳴る側に置く。
  - **鳴らない（見送らないことが正しい）5 語＋route 無し**: `MoveCue`（`\![move]` のスクリプト明示移動。起動時の連鎖確定 `frame/drain_resnap.rs:335` も同じ語で書く）／`Restore`（復元マージ・D13 が明示操作と定める）／`BalloonLimitRelease`（バルーンドラッグ**解放時**の補正＝ドラッグの一部。見送ると画面外のバルーンが引き戻されないまま残る）／`BalloonFollow`（上の例外 その 1）／`SpawnInitial`（位置そのものを作る書込＝保つべき「遷移前の値」が無い）／route 無し（＝ドラッグ経路のキャラ窓書込のみ）。
  - **`is_system_reanchor` を流用しない**——あちらが答えるのは「誰が動かしたか」（既定位置の追跡と可視性ガードが読む区分・D9／D16）であり、こちらは「見送りが覆うか」である。実際に `KeepPositionResize` の 1 語で割れる（あちらは `false`・こちらは `true`）。`diag.rs` の doc が求める「2 つの区分を意図的に割るなら理由を設計へ登記する」に応じてここへ登記する。
...
| B-2b `DeferWindowPos` 一括（**確定**・task 7.1 で採用） | flush を `Begin/Defer/EndDeferWindowPos` の 1 バッチへ（Z 専用は per-window flags で同居） | `crates/wintf/src/ecs/window/command.rs`（裁定時点で `:425-505` の `SetWindowPosCommand::flush` 1 関数。**task 7.2 の着地後は `:723-807` の `flush` と、その補助 `window_pos_args`〔`:282`〕・`apply_as_batch`〔`:386`〕・`apply_sequentially`〔`:453`〕・`BatchDegrade`〔`:299`〕・`begin_batch`〔`:349`〕・`log_window_write`〔`:188`〕**） | **満たした**（2026-08-21 再採取・7 遷移: 書込 **28/28** が `(WPC, DPICHANGED, WPC)` を内側に持ち中間形 0 本／`WM_DPICHANGED` 28/28 が `in_swp=true`／Σ`call_us`／`total_us` ＝ **99.82〜99.92%**。台帳 §10.3・§10.5） | 決定論: flush が 1 バッチ（`flush count` と `write` の `in_batch`）／時間効果は実機のみ（`flush_total_us`）。**task 7.2 で着地**——`command_batch_tests.rs` の 8 本（是正前は `a_flush_of_real_windows_is_submitted_as_one_batch_and_says_so`・`an_invalid_handle_discards_the_batch_and_degrades_to_one_write_per_command`・`the_write_log_line_is_emitted_once_per_command_on_both_paths` の 3 本が赤・後は 8 本とも緑）。**行の語は `in_batch=true`／`in_batch=false`** である（`ok=`／`in_swp=` と同じ真偽の綴り。候補表の初出は `in_batch=1` と書いていたが、観測語彙に真偽の綴りを 2 つ持たないため `true`／`false` に揃えた） | dlp（W8）＝brief 追記(74)。`ghost-window-zorder` は `completed/` ゆえ消化できず、受け先も dlp が兼ねる（同 brief 追記(71)⑷） |
...
> **B-2b の実装（task 7.2・2026-08-21）**: 採用した 1 案だけを実装した。**挙動を変える接触集合**は宣言どおり `command.rs` の `SetWindowPosCommand::flush` とその補助（`window_pos_args`／`apply_as_batch`／`apply_sequentially`／`BatchDegrade`／`begin_batch`／`log_window_write`）に留まり、B-4・B-3・自前 handler 是正には一切触れていない。**触ったファイルはこれだけではない**——観測語彙 1 フィールドの追加に伴い、発行側（`transition_diag.rs`）・同期書込の発行点（`window_pos.rs`）・判定器の任意フィールド表（`transition_judge.rs`）・行を組む既存テスト 3 本・新設の対テスト 1 本が追随している（全 11 ファイル・内訳は Modified Files）。
> - **要件 10.3 の守り方**: バッチ化は**投入の仕方だけ**を変える。引数は `window_pos_args`（純関数）が 7 項目をそのまま運び、バッチ経路と縮退経路の**両方**がそれを使ってキューの順に回す。Z 専用指令は `DeferWindowPos` の per-window 引数（`hwndInsertAfter`＋flags）として同じ位置に同居する。既存の合流の檻 21 本は 1 行も変えずに緑のままであり、加えて実窓 2 組の A/B（`a_batched_zorder_list_lands_in_the_same_order_as_one_write_per_command`）が**最終 Z 順序の一致**を実測で固定する。
> - **要件 10.2 の守り方**: `SetWindowPosGuard` は**バッチ全体**（`Begin` の直後から `End` の完了まで）に掛かる。`EndDeferWindowPos` が同期送達する `WM_WINDOWPOSCHANGED` の内側で `is_self_initiated()` が真であることは実窓の wndproc で直に確かめてある。
> - **縮退経路**: `Begin` 失敗・`Defer` 失敗・`End` 失敗の 3 つとも `warn!`（理由語つき）を出したうえで 1 本ずつの `guarded_set_window_pos` へ落ち、当該区間の書込レコードは `in_batch=false` になる。`Defer` が失敗するとバッチごと失われる（部分適用は起きない）ので全件を頭から書き直し、`End` の失敗は部分適用の可能性があるが同一引数の再適用は最終状態を変えない。
> - **観測の意味が 1 つ変わった（7.3 が読むときの注意）**: バッチでは `write` の `call_us` は**投入だけ**の所要になり、窓が実際に動く時間は 1 区間ぶんまとめて `flush stage=end` の `total_us` に入る。よって台帳 §3.7 ⑵⑶ が使った「各 `write` の呼出区間 `[t_us − call_us, t_us]` の内側に `msg` があるか」という読み方は、**是正後のログには当てはまらない**（`msg` は `End` の内側＝最後の `write` レコードより前に固まって並ぶ）。是正前後で比べられるのは `flush stage=end` の `total_us` と `in_batch` の件数である。
...
| kind | 発行 crate／点 | 必須フィールド | 任意フィールド |
|---|---|---|---|
...
| `write` | wintf flush／`window_pos.rs:468-489` | `stage=flush\|sync seq hwnd origin scope win_kind x y cx cy flags ax ay aw ah call_us ok`（`ax..ah` は書込後矩形・読み戻せなければ `-`） | `in_batch`（`WRITE_OPTIONAL_FIELDS`・task 7.2） |
| `msg` | wintf window_proc | `msg hwnd in_swp since_flush_us` | — |
| `hold` | areka C5 | `entity scope win_kind window_dpi table_dpi since_frame decision site=dpi\|reconcile\|resnap\|work-area-resnap` | — |
