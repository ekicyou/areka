...
**タスク数**: 本仕様 5〜6（#63 を切り出した後）。議題 ⑶ で emo2 を同梱できないとなれば既定ゴーストの差し替えと #12 の裁定の改訂で +1〜2。

**要件段階の議題**: ⑴ zip に `emo2-kakukaku` を入れるか／⑵ ライセンスは MIT 単独か MIT OR Apache-2.0 か／⑶ emo2 を zip に入れてよいか（入れられなければ既定ゴーストを `claudia` などへ替えるか、ゴースト無しで配って「入れてください」の告知から始めるか）——**3 件とも #63 の要件で決め、本仕様はその答えを受け取る**。
