...
5. While 永続化されたバルーン相対位置が存在するとき、the areka 窓配置 shall 永続値を優先し、`windowposition` の適用を初期既定位置の供給にとどめる（**保存値優先という順位**を変更しない）。
   - **実装時訂正（2026-07-31）**: 当初は「保存・復元の既存**規約**と優先順位を変更しない」としていたが、R3.8 の実機裁定でバルーン offset の**基準**（アンカー辺→char 左上）を変更したため、本条の射程を**優先順位のみ**へ限定する。**順位（永続値があれば永続値）は無改変**で `persist.rs::merge_scope` に変更はない。基準変更の内容と根拠は R3.8 を参照。なお旧基準で書かれた既存の永続値は同一キーのまま意味が変わる（開発段階の dev fixture のみ・移行策は設けない＝R7.4 により対応表へ記録）。
