...
- **`spine.rs` の純粋待機ループの有界性**（2026-07-31 に境界拡張・R7.9）——反復回数のみで区切られた待機ループを**壁時計 `Instant` deadline ＋ poll-backoff** へ改める。R7.8 とは**別クラス**（ラッチ型観測ゆえ時刻頭打ちでは直らない・予算枯渇が真因）。対象は同クラス 11 本。**台本・アサート・観測述語は非所有・無改変**。負検証の settle drain（「尽きるのが正常」）は `yield_now` のまま意図的に温存。
  - **着地形の訂正（2026-08-01・main マージ時）**: main が先に PR #96 で同じ欠陥クラスを是正しており（`choice-select-events` 着地に随伴）、**先着した main の形へ寄せた**（本プロジェクトの rebase 規律）。最終形は待機を **2 種**に分ける:
    - **純粋ポーリング**（各反復が系を進めない）→ main の `spin_wait_until`（密 yield `SPIN_YIELD_BUDGET`＝速い経路の検出遅延を犠牲にしない → 超過後 `BACKOFF_SLEEP` 1ms でコア占有をやめる）。main はここを 4 箇所適用済み。
    - **ハイブリッド**（毎反復 Tick 注入＋別スレッド結果の待機）→ 各呼出点の自前ループ＋`sleep(200µs)`。`spin_wait_until` は純粋ポーリング専用ゆえ流用しない。**本 spec の寄与はこの 6 箇所**（main は 1 箇所を反復上限のまま残していた）。期限定数は main の `SPIN_WAIT` へ統一。
    - main が「sleep 直行」を採らなかったのは実測根拠がある（小さい yield 予算では通常経路が 4.4 秒→10.3 秒へ悪化）。本 spec の当初形（deadline＋200µs sleep 直行）はこの知見を欠いていたため、**純粋ポーリング側は main の形が正**。
...
`spine_s2_talk_drives_surface_switch_and_typewriter_reveal` の駆動ループは、注入模擬時刻 `now` を毎反復 5ms ずつ**無条件に**進めながら、break 条件（テキストが runtime へ到達したか）は**実 async の進捗**に依存する。dispatcher は `elapsed=(now−base)/1000` で cue を解放するため、並列負荷で実 async が 280 反復（1.40s÷5ms）以内に間に合わないと **Clear が解放されてテキストバッファを消去**し、以後どれだけ回しても条件が成立しない（残り反復を空回りして 400〜550 秒を消費し失敗）。

**観測窓の導出**（当初 1.05s と誤記していた・レビューの頭打ち値掃引で 1.40s と実測確定＝1.395s は緑・1.405s は赤）: `areka-sakura/src/compile.rs` は `Instruction::Text` に対しても `offset += D` を進める（`D` ＝ `duration.rs::text_playback_duration` ＝ 文字数 × 50ms）。台本 `\s[2100]\w[1]アヒルやアヒル\w[20]\c\e` の実タイムラインは **Emote@0.0 → Wait@0.0(d=0.05) → Text@0.05(d=D) → Wait@0.05+D(d=1.00) → Clear@1.05+D**。7 文字ゆえ D=0.35s で **Clear@1.40**。台本のテキスト長を変えると Clear 時刻が `0.05 + 文字数×0.05 + 1.00` で動く。
