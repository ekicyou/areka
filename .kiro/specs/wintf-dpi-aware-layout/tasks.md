...
- [ ] 1. SetWindowPosGuard をネスト対応の save/restore 方式に修正する
- [ ] 1.1 `IS_SELF_INITIATED` フラグの保存・復元ロジックを実装する
...
- [ ] 2. `update_arrangements_system` に DPI スケール設定ロジックを追加する
- [ ] 2.1 Window エンティティの `Arrangement.scale` を DPI スケールで設定する
...
- [ ] 3. `WM_DPICHANGED` ハンドラをレイアウトシステム主導方式に書き換える
- [ ] 3.1 DPI コンポーネントをハンドラ内で直接更新する
...
- [ ] 3.2 `DpiChangeContext` を設定し `SWP_NOSIZE` を維持したまま SetWindowPos を呼ぶ
...
- [ ] 4. `WM_WINDOWPOSCHANGED` ハンドラの BoxStyle 更新ロジックを修正する
- [ ] 4.1 bypass 判定と BoxStyle skip 判定を分離する
...
- [ ] 4.2 DPI 変更時の DPI コンポーネント更新をハンドラから除去する
...
- [ ] 5. (P) `dump_all_windows_dpi` 関数のログ移行と情報追加を行う
- [ ] 5.1 (P) `println!` を `info!` マクロに移行する
...
- [ ] 5.2 (P) `BoxStyle.size` の論理 px サイズをログ出力に追加する
...
- [ ] 6. (P) `taffy_flex_demo` の `run_demo` タイムラインを短縮する
...
- [ ] 7. DPI スケーリング修正の統合検証を行う
- [ ] 7.1 `cargo test` で既存テストの回帰がないことを確認する
  - `cargo test` を実行してすべてのテストが PASS することを確認する
  - _Requirements: 1.1, 1.2, 1.3, 2.1, 2.2, 2.3_

- [ ] 7.2 デモ実行で DPI スケーリングの正しさをログ検証する
...
- [ ]* 7.3 DPI 変更ラウンドトリップをドラッグ操作で検証する
  - ウィンドウを 125% モニターから 200% モニターへドラッグし、`BoxStyle.size` が変化しないことをログで確認する
  - 逆方向（200% → 125%）でも同様に確認する
