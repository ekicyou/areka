...
- **ダミー窓は可視面（不透明矩形の子）を持つ**（task 2.2 追補・手動起動で確認）。WUC/DComp 合成窓は描画物が無いと完全透明＝画面に見えず手動ダブルクリック close も不能（HWND とタイトル「areka startup (dummy)」は生成される）。`spawn_dummy_window` に `Rectangle`＋`Brushes`（不透明グレー）＋`BoxStyle{flex_grow:1.0}`＋`ChildOf` の子を1枚足して可視化＝手動 close 経路（2.2 受入）を実用化。子は既定 Bounds ヒットゆえ双クリックが窓の `OnPointerPressed(on_dummy_pressed)` へバブル。**配置・座標・DPI は非主張のまま**（`WindowPos` 位置を足さず flex_grow の窓内レイアウトのみ・placement リジェクト再発なし）。env ゲート自動 close・smoke exit 0 は不変。
