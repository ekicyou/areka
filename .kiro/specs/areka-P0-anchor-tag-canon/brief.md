...
> **2026-09-03 追記（消化済み①・areka-P0-sakura-bare-tag-lexer・PR #（完了時に記入））**: 上記「直接修正」＝角括弧なし `\_` タグの消費是正は spec `areka-P0-sakura-bare-tag-lexer` で消化済み（規律＝`\_` ＋ `_` 0〜1 個 ＋ 1 文字の固定長・`\__X` 3 文字形も射程・意味付けなし＝`Instruction::Raw` 素通し・決定論テスト新設）。本 spec に lexer 修正は残らず規模は L→M。
