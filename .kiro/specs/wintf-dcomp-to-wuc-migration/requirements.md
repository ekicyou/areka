...
5. When 描画等価性を検証する, the 移行作業 shall 目視確認への依存を排し、決定論的なピクセル差分（レンダ結果のビット等価比較）を自動化した検証ハーネスを**主たる受け入れ手段**として用いる。
6. Where サーフェス（D2D 描画）層の出力が決定論的である, the 検証ハーネス shall 移行前後のサーフェス描画結果をピクセル等価（ハッシュ一致／差分ゼロ）で自動比較する。
7. If 合成層（ビジュアル木の配置・重なり順・不透明度・クリップ）の出力に決定論的なピクセルキャプチャを適用できない範囲が残る, then the 移行作業 shall その範囲を明示し、その範囲に限って目視確認を残差的フォールバックとして用いる（キャプチャ機構の選定は設計フェーズで確定する）。
