...
5. **バルーン本体画像参照の解決方式** — R2.8。descript/s0s/k0s に `balloons0.png` を指す明示行は無く、命名規約 `balloon{s|k}{ID}.png` から導出される。モデルが画像参照をどう表現するか（(a) 命名規約からファイル名を導出して保持 / (b) サーフェス種別＋ID のみ保持して解決は下流 / (c) fixture の実ファイル名を検証で突き合わせる）。※本 spec は host/I/O 非依存＝実ファイル存在チェックは領分外。**Research/確認要**（明示行が本当に無いか、SSP 命名規約が正本か）。
