...
- [x] 10.3 実機（emo2）サインオフ_※2026-07-24 開発者実機で完了（詳細は Implementation Notes 10.3-signoff）_
...
- 10.3: **MANUAL_VERIFY_REQUIRED（人間・実機サインオフ）**。仕様（`crates/areka/tests/emo2_real_run.rs` doc・R9.3）が「M-boot 完了は人間サインオフを経てのみ宣言・**AI 単独では宣言しない**」と明記。AI が自動化を試みた結果: `AREKA_APP_SMOKE_EXIT_MS=6000`＋emo2 fixture で実走→**exit 0・実表示 GPU 初期化・実 sink 結線（wire 成立）・attach planned=2 attached=2 まで到達**を確認。ただし **32bit pasta.dll の LOAD が `0x8007007E`（ERROR_MOD_NOT_FOUND＝依存 DLL 不在・VC ランタイム等の環境依存）で失敗**し実 SHIORI 接続が確立せず、prefetch（実 SHIORI 応答が前提）が発火しないため R9.3 の `shiori resource prefetch done`／`outcome="no_content"` ログ証跡＋バルーン `%username` 非露出目視は**この session では取得不能**。→ 実 32bit SHIORI が LOAD 可能な開発者実機で、`AREKA_EMO2_REAL_RUN` 実走＋RUST_LOG grep（prefetch 証跡）＋バルーン目視を人間が実施して初めて 10.3 完了宣言可。sylphya 側の結線（prefetch stage・ResourceSink・provider）は決定論檻（6.3/8.4/10.1）で全網羅済み＝残るは実 SHIORI 実表示の人間確認のみ。
- 10.3-signoff: **2026-07-24 開発者実機サインオフ完了（R9.3 必達・人間判断）**。前提修正＝emo2 の ghost/balloon root は**絶対パス**で起動すること（相対パスだと helper の pasta.dll LoadLibrary が `0x8007007E` で失敗＝前回不成立の根因）。手順＝`AREKA_APP_SMOKE_EXIT_MS=180000`＋`RUST_LOG=info,kanade=trace,areka_kanade=trace`・debug ビルド・helper を areka.exe 隣接配置。証跡: (1) **ログ grep**（run1=`target/emo2_signoff_10_3.log`・run2=`..._run2.log`）——両走とも `shiori resource prefetch done id="username" outcome="no_content"` **ちょうど 1 回**・run1 は `unload_clean`＋exit 0・両走 exit 0。(2) **目視（開発者・スクリーンショット確認）**——エモ頭部撫で→`Head1通常` パターン発火（エモ「な、なんだよう。」→むらさき「仕方ないなあ～、**ユーザーさん**ったらもう♪」）＝生 `%username` 非露出・既定値「ユーザーさん」展開を実表示バルーンで確認。run2 ログに mouse/touch 系 352 行＝撫で実経路の通過証跡。全 30 タスク完了・実装フェーズ完遂。
