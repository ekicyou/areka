...
4. **ukadoc `\v` 項の原文**。要件 3 の根拠文
   「スコープごとの重なり（本体側と相方側どちらが上にくるか）はユーザの操作次第」は
   brief 経由で引いているが、**ukadoc MCP の索引では `\v` 単体の項へ到達できなかった**
   （`\![set,windowstate,stayontop]` の項に当該文は無い）。design で URL 直参照により
   逐語を確定すること。なお `windowstate` 3 種と `OnWindowState*`／`OnFullScreenApp*`
...
1. **案 A（owner）／案 B（明示維持）のどちらを本線とするか、そして A の実機検証を
   どのタスク境界に置くか。** roadmap の W6 編成条件⑶は「案 A の WUC 共存実機検証が最初のタスク」と
   規定しており、design のタスク分割はこれに従う必要がある。
...
4. **要件 5.7（破棄の巻き込み禁止）と案 A の owner 破棄カスケードの裁定。**
   OS はキャラ窓の破棄時にバルーン窓を巻き込む。⑴要件 5.7 を「areka のコードが能動的に
   巻き込まないこと」と読むのか、⑵OS 由来の巻き込みも禁止と読むのか。⑵なら案 A は不可。
   併せて `WindowRegistry` drop による二重 `DestroyWindow` の扱いも決める。
