...
6. ~~**要件 1.3（バルーン活性化時にキャラを引き上げる）の具体形**~~
   → **2026-08-11 の要件ディスカッションで開発者が裁定（クローズ）: ⑴ペア浮上を採る。**
   要件 1.3 の現行表現のまま確定。**ただし「動く」の意味を分離すること**——
   **表示順はペアで動き、位置はバルーンのみが動く**（バルーンをドラッグしても
   キャラ窓の位置は追従しない）。この分離を要件 1.6 として新設した。
   design では「キャラをバルーンに位置追従させる」実装を採らないこと
   （位置の追従は既存の `BalloonFollow`＝キャラ→バルーンの一方向のみ）。
   案 A（owner）を採る場合、owned の活性化で owner まで手前へ来るかは実機未検証であり、
   **§6-1 の実機検証項目にこの判定を加えること**（来ないなら A でも 1.3 の明示実装が要る）。
...
9. ~~**要件 2.6（非表示→表示）の履行時期**~~
   → **2026-08-11 の要件ディスカッションで開発者が裁定（クローズ）: シームとして残す。**
   要件 2.6 は本 spec に保持し、`balloon-visibility`（W6 並走）が再表示時に呼び出す口として提供する。
   相互登記を requirements.md の Boundary Context へ明記済み。
   **受け入れ条件も明記した（要件 7.6 新設）**——本 spec の範囲では決定論的テストによる
   判断ロジックの検証のみで受け入れ、実際の重なりの実機確認は vis 着地後に vis 側の
   実機サインオフで行う（本 spec 単独では発火させる術がないため）。
   **design での宿題**: シームの具体形（関数か、コンポーネント付与か）を確定し、
   本 spec と vis の双方の設計文書に同じ形で記載すること（roadmap の因果 `zorder→vis`・
   W6 編成条件「vis は zorder の手前保証を前提にしてよい」に対応する）。
