...
- **D1（解決済・2026-07-03 要件ディスカッション #1）balloon は surface システムで描画**: 開発者判断により、**balloon は内部設計上 surface システムで描画する**（surface＝内部に element を持ち、element 合成で画像を生成する機構）。既存の surface 描画機構を流用して balloon 描画を実現する。→ 本層は balloon を shell surface と**区別せず**、同一の surface／element マニフェスト機構（要件 1.1）で扱う。要件 1.4 を「surface として表現された balloon の element を shell surface と同一機構で列挙」へ是正済（Boundary Context の In-scope／Adjacent expectations も更新）。要件 1.4 が前提にしていた「balloon モデルが balloon 画像を含む」という不整合は解消（本層は `BalloonModel` の画像パスを直接消費せず、surface 表現を受ける）。**残る設計判断（→design・カテゴリB）**: (i) balloon 画像（`balloons*.png`/`balloonk*.png`）を surface 表現（element を持つ surface）へ適合させる責務を**どの上流／隣接ユニットが担うか**（balloon parser 拡張／package-mount の balloon dir 解決／専用アダプタ）、(ii) その surface 表現の型（shell surface と共通か balloon 専用か）。本層は完成した surface 表現を入力として受け、元ファイルは読みに行かない（要件 3.6 維持）。透過設定（balloon descript の `use_self_alpha`・shell とは別定義）も surface 単位の設定として注入される。
