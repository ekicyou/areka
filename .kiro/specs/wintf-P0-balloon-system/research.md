...
- **背景**: グリフエンティティ方式の ULWモードパフォーマンス未検証
- **検証タイミング**: balloon03-content 実装後にベンチマーク実施
- **ベンチマーク基準**: 既存タイプライター実装（文字更新ごとに全レイアウト再計算）と比較
- **性能予測**: グリフ分割方式は初回レイアウト完了後、文字追加は `Visual.is_visible` 切替のみ。既存実装より高速なはず
- **ULWモードサポート方針**: **完全サポート**（文字数制限なし）
- **フォールバック戦略**: 実測でパフォーマンス問題が発生した場合、コンテンツ領域専用サーフェスを用意してグリフ単位 CommandList を焼き付ける最適化を実施
- **根拠**: 現時点では完全実装を優先し、実際の負荷を測定してから最適化判断を行う

### 決定: P3 — インライン要素の将来拡張方針

- **背景**: テキスト行内への画像・絵文字埋め込み需要の検討
- **決定**: **P0 範囲外**
- **根拠**: 
  - 本格的な組版エンジンが必要（taffy は `Display::Inline` 未サポート）
  - DirectWrite `IDWriteInlineObject` による実装は複雑度高
  - 現時点で明確な需要なし
- **将来の拡張アプローチ**:
  1. **グリフエンティティ延長方式**: `GlyphContainer` の子として画像エンティティを挿入（`Visual` + `Arrangement` + `BitmapSource`）。`IDWriteInlineObject` を経由せず ECS で直接制御
  2. **WebView コンテンツ領域**: BalloonContentArea を WebView2 に置き換え（HTML/CSS で本格的な組版を実現）
- **トレードオフ**: P0 の実装シンプル化 / インライン要素需要への将来対応
- **子仕様への影響**: balloon03-content ではグリフエンティティのみ実装。インライン要素は balloon04 以降で検討

---

## リスクと対策

- **R1: ULWモードでの毎フレーム再描画コスト** — 100文字時の N×DrawGlyphRun CPU 実行が 16ms 予算内に収まるか未検証。ただし既存タイプライター実装（文字更新ごとに全レイアウト再計算）より高速な設計（初回レイアウト + 以降はプロパティ切替のみ）。DCompモードでは CommandList 焼き付け後はプロパティ変更のみでGPU合成されるためリスク低い。対策: ULW完全サポート（文字数制限なし）、balloon03-content 実装後に既存実装と比較ベンチマーク、パフォーマンス問題発生時はコンテンツ領域専用サーフェス + CommandList焼き付け最適化を検討
