...
- **既存タイプライター比較**: 文字追加時のパフォーマンスを既存実装（文字更新ごとに全レイアウト再計算）と比較。グリフ分割方式（初回レイアウト + 以降は `Visual.is_visible` 切替のみ）の優位性を検証
...
- **ULW モード完全サポート**: 文字数制限なし。既存タイプライター実装（文字更新ごとに全レイアウト再計算）より高速な設計（初回レイアウト + 以降はプロパティ切替のみ）。実測でパフォーマンス問題が発生した場合はコンテンツ領域専用サーフェス + CommandList焼き付け最適化を実施
