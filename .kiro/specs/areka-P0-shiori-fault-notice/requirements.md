...
2. The 本仕様 shall **裁定 2（失敗の種類を告知に載せる・brief 議題 ⑵）**として、失敗の種類と理由を kanade の停止通知に載せて運ぶ（要件 2）。根拠: brief の Desired Outcome 1 が「失敗の種類が平易な言葉で載る」を完了条件に置く。kanade の 4 ファイル（`msg.rs`・`actor.rs`・`schedule/mod.rs`・要件 2.2 により `shiori/real.rs`）に手が入り #13 との共有が増えるが、直列（本仕様 → #58 → #13）なので並走の問題は生じない。**別案**（種類を載せず「SHIORI が動かなくなった」とだけ告げる・kanade 無改変）は採らない。
