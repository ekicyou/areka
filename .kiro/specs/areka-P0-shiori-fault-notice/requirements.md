...
- **In scope**:
  ...
  - #57「起動後の途中終了で記憶の確定が飛ぶ」の修正＝`app.run()` が失敗を返しても後始末を通してから終える（裁定 6・2026-09-24 開発者裁定で本仕様が拾う）。
  - 実 sink 結線が不成立のときの LogSink 側の起動でも停止通知の受け口を持ち、Fault で窓が残って固まらない（裁定 5・同上）。
- **Out of scope**:
...
- **Adjacent expectations**:
  ...
  - `ghost-restart-unit`（#58）は `crates/areka/src/main.rs`・`app_exit.rs`・`emo2_boot/frame.rs` を共有するので本仕様の後に着地する。#58 が `fn main` の終了順序を関数へ括り出すとき、本仕様が足す「`run()` の成否によらず後始末を通してから終了コードを決める」形（要件 3.2・6.3）と「起動の 2 経路とも停止通知の受け口を持つ」形（要件 6.4）をそのまま関数の中へ運ぶ。
...
1. When kanade が SHIORI の失敗で終了系列に入る, the areka shall 停止通知に失敗の種類（要件 1.4 の 5 語に写せる区分）と理由の一行（対応する `error!` の `error=`／`reason=` と同じ文言）を載せて運ぶ（今日の「Fault」だけの通知は中身を持たない）。
...
2. When SHIORI の失敗で終わる, the areka shall 終了の指示のあとの後始末（再生ループの停止 → ゴースト実行環境の終了統括 → SERIKO の合流 → 性能報告）を今日と同じ順序で最後まで通してから終了コードを決める（後始末を飛ばして早期に抜けない）。`app.run()` が失敗（`Err`）を返したときも同じ後始末を通す（要件 6.3＝#57 の修正）。
...
3. If `app.run()` が失敗（`Err`）を返す, then the areka shall 今日と同じ後始末（再生ループの停止 → ゴースト実行環境の終了統括 → SERIKO の合流 → 性能報告）を通してから、その失敗を 0 以外の終了コードで返す（#57「起動後の途中終了で記憶の確定が飛ぶ」の修正＝boot 直後に投函した `[last]` の記憶や窓位置の永続化が後始末で確定する）。main スレッドの panic は対象外（`catch_unwind` は置かない・`Drop` での自動の後始末も終了理由を持てないので置かない）。後始末の途中の失敗は今日どおり記録して先へ進めるか止めるかを設計で決め、記録なしで飛ばさない。
4. While 実 sink 結線（`wire_emo2_boot`）が不成立で LogSink 側の起動（`fn main` の `else` 腕）に倒れている, when kanade が SHIORI の失敗で止まる, the areka shall 実 sink 結線のときと同じ 1 本の停止通知の受け口を通して、全ゴースト窓を閉じ・告知を出し・0 以外の終了コードで終える（窓が残って固まらない）。起動の 2 経路が停止通知の受け口を別々に持つ形にはしない（受け口は 1 本を共有する）。
...
1. The 本仕様 shall 告知の文面（要件 1.3・1.4）を、失敗の種類 5 語 × ゴースト名の有無で純関数のテストにより固定する。
...
6. The 本仕様 shall 足すテストを判断の分岐に限り、既に確かめられている配線（マウント→SHIORI→sink・窓の配置・終了の指示・後始末の順序）を再テストしない。ただし要件 6.3「`run()` が `Err` でも後始末を通る」と要件 6.4「LogSink 側の起動でも停止通知が終了の指示に届く」は新しい判断の分岐なので、それぞれ決定論テストで固定する（前者は `run()` の結果を受けて後始末へ進む判断を関数に切り出して `Ok`／`Err` の両方で通ることを見る。後者は実 sink 結線を不成立にした起動＋`ShioriWiring::Custom` の接続失敗で停止通知が受け口に届くことを見る）。
...
5. The 本仕様 shall **裁定 5（LogSink 側の起動の穴・brief 議題 ⑸）**として、実 sink 結線が不成立のときの起動で Fault が起きると窓が残って止まる穴を本仕様で拾わず、ロードマップの台帳へ登記だけの行として起票する（要件 6.4）。根拠: 静的に読めただけで実走はしておらず、その起動はもともとゴーストが喋らない縮退の形（実 sink が無い）で、α の第三者が踏む経路ではない。規模 S を守る。引受先（#58 か新規）は要件ディスカッションで確定する。**2026-09-24 要件ディスカッションで暫定裁定を覆して確定**: 開発者「2 件とも本仕様で拾う」＝LogSink 側の起動でも停止通知の受け口を 1 本共有し、Fault で窓を閉じて告知して終える（要件 6.4・7.6）。規模は S を超えて 7〜9 タスクの見込み。
6. The 本仕様 shall **裁定 6（#57 の相乗り）**として、#57「起動後の途中終了で記憶の確定が飛ぶ」を本仕様で採らず `ghost-restart-unit` #58 へ送る（要件 6.3）。根拠: #58 が `fn main` の終了順序を関数に括り出すので、`Err` の腕でも同じ関数を通す直しはその関数の中に置くのが筋（brief と #58 の brief が一致）。本仕様は要件 3.2 で「後始末を通らない終わり方」を新しく足さないことだけを守る。**2026-09-24 要件ディスカッションで暫定裁定を覆して確定**: 開発者「2 件とも本仕様で拾う」＝`app.run()` が `Err` でも後始末を通してから 0 以外で終える（要件 6.3・3.2・7.6）。#58 は本仕様が入れた形を括り出しのときにそのまま関数の中へ運ぶ。台帳 #57 の行は完了時に「#55 で修正」と更新する。
