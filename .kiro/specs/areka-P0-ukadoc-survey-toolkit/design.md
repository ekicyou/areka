...
**書き出しの注入（2026-09-05・task 12.2）**: `cli::generate::ledger_init()` は 1 行の配線で、判断と順番は `ledger_init_with(read_existing, write)` が持つ。既存本文の取り寄せ（task 11 の `plan_ledger_init` の引数）に加えて書き手も引数で受けるので、在中テストは実 repo のカタログと実際の取り寄せ関数と「記録するだけの書き手」で呼び、書き出される `(パス, 本文)` 4 組がコミット済みの台帳とバイト一致すること（不動点＝`ledger-init` の再実行で台帳が変わらない・3.3a）、4 本目の取り寄せで失敗すると書き手が 1 度も呼ばれないこと（4 本決めてから書く・1.8 の趣旨）、書き手の失敗がそのまま返ることを、ファイルを 1 つも作らずに確かめる。常時テストが届かないのは `ledger_init()` の 1 行（どの取り寄せ・どの書き手を渡すか）だけである。
