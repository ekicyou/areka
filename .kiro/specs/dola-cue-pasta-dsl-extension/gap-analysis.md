...
#### 🟡 Warning: 暗黙キーフレームの「終了時点」が未定義

Req 2 は「各アクション行の終了時点で暗黙キーフレームを生成」とあるが、**アクション行の「所要時間」を何から算出するか未定義**:

- テキスト文字数 × 表示速度？
- 固定デュレーション？
- 明示指定必須（`!` コマンドで次の時刻を設定）？

現在の `Cue.start_time` は相対秒数を要求しており、パーサーだけでは時刻を決定できない。**本仕様は「設計のみ」のため、この未定義は設計フェーズでの解決が妥当**だが、要件レベルでの考慮も必要。
...
| ID | 項目 | 詳細 |
|----|------|------|
| **R-1** | 暗黙キーフレーム所要時間 | アクション行の「終了時刻」をどう決定するか。文字数ベース？固定値？パーサーでは算出不可（ランタイム依存）ならば IR 上は「次の行の start_time = 前の行の start_time + duration_placeholder」としてマーク |
| **R-2** | `@alias = Command(args)` の PEG 文法 | `=` / `＝` セパレータの新行種別。変数代入 `$var = expr` との構造的類似性を活用した設計が有力 |
| **R-3** | BarrierKind 命名修正 | 要件の `All / Any / Explicit` を実コードの `WaitForInput / WaitForChoice / Timeout` に整合させる |
...
| 暗黙キーフレーム所要時間の未定義 | 高 | 設計フェーズで R-1 を最優先リサーチ。placeholder 方式で段階的対応 |
| 外部リポジトリ（vendors/pasta）commit 二重管理 | 低 | 文法・AST 変更は PR 単位で管理。areka 側では submodule update |
| BarrierKind 要件不整合 | 中 | 要件 Req 3.3 の修正を設計フェーズ冒頭で実施 |
...
### 6.2 Req 2: 暗黙キーフレーム所要時間の明確化

**追加 AC 提案**:
> 5. The pasta DSL 拡張パーサー shall 暗黙キーフレームの「所要時間」算出方法（テキスト長ベース、固定値、または `!` コマンドによる明示指定）を設計フェーズで確定する。
...
2. **R-1（暗黙キーフレーム所要時間）を最優先リサーチ** — 設計全体の骨格に影響
3. **R-3（BarrierKind 名称）を設計開始前に要件修正** — 混乱防止
