...
本拡張は現行 pasta DSL の **行単位の役割分担** という設計原則を維持しつつ、キューシートモード専用の新しい行種別（`!` キューコマンド行）と、既存構文の再解釈（アクション行 → CueCommand マッピング、`@` エイリアス定義）を導入する。
...
- **文法設計原則**:
  ...
  - **明瞭なコマンド名**: 省略表記を避け、ソースコード級の明確な英単語を使用（例: `barrier`、`keyframe`）
  - **拡張性**: 将来的なコマンド追加（例: `timeout`、`wait`）に対応可能な構造
...
2. The pasta DSL 拡張パーサー shall `＆type：cuesheet` が存在しないシーンを現行 pasta 文法として扱い、キューシート専用構文を有効化しない。
...
**Objective:** スクリプト作者として、キーフレーム制御・Barrier 指定・バルーンクリアなどの演出制御コマンドを統一的な記法で記述したい。そうすることで、時系列制御とインタラクティブな演出フローを宣言的に表現できる。
...
> **設計注記**:
> - 具体的なコマンド文法（キーフレーム宣言・指定・Barrier・Clear の記法）は design.md で決定する
...
1. The pasta DSL 拡張パーサー shall キューシートモードでエイリアス定義行を認識し、`@alias_name = CueCommand(引数)` 形式の定義を許可する。
...
> **設計注記**: エイリアス定義行の具体的な文法（コマンド名の英語 / 日本語対応表、引数エンコード規則、複数引数の区切り文字）は design.md で決定する。

---

### 要件 5: Say / Emote / Clear のアクション行対応
...
4. When キューシートモードで継続行（`:content` 形式）が記述された場合、the pasta DSL 拡張パーサー shall そのコンテンツを直前のアクション行の `CueCommand::Text` に追加する。
5. The pasta DSL 拡張パーサー shall `CueCommand::Clear` の生成方法（要件 3 の `!` コマンドによる明示指定、またはシーン遷移時の自動生成）を設計フェーズで確定する。
6. The pasta DSL 拡張パーサー shall 1つのアクション行に複数の `@command` が含まれる場合の挙動を設計フェーズで確定する。
...
6. The pasta DSL 拡張パーサー shall `％`行が存在しない場合のデフォルトスロット割り当てルールを設計フェーズで確定する。

> **設計注記**: ルーティング状態の追跡、Add / Switch の判定ロジック、並列演出時の RouteAdd 優先適用ルール、RouteRemove の発行条件は design.md で詳細化する。
...
1. The pasta DSL 拡張仕様 shall 全機能（暗黙キーフレーム、キューコマンド行、エイリアス定義、Say / Emote / Clear、Routing 自動生成）を網羅したサンプルシーンを含む `cue.pasta` ファイルを成果物として提供する。
...
7. The pasta DSL 拡張仕様 shall エイリアス定義の CueCommand 記法（コマンド名の英語 / 日本語対応表、引数エンコード規則、フォールバック規則）を `design.md` に表形式で明示する。
...
- **設計フェーズへの引き継ぎ事項**:
    - キューコマンド行（`!` 行）の具体的文法（キーフレーム宣言・指定・Barrier・Clear の各コマンド記法）
