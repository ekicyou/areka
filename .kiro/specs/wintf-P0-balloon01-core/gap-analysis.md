...
### v2.0 レビュー議事の影響（2026-02-26）

v2.0 レビューでの設計議論により、以下の影響がギャップ分析に発生：

1. **ブロッカー追加**: `wintf-P0-cue-system`（演出キュー配送基盤）がブロッカーとして追加。cue-system の要件確定後に v3.0 の要件を最終化する
2. **ファサードパターン原則**: ルートエンティティ = 外部 API の原則が確定。Req 2 の拡張または新規要件として反映が必要
3. **TextDirection のコンポーネント化**: Balloon コンポーネントに TextDirection フィールドが追加される。Req 2 に影響
4. **BalloonStyleMap**: 感情値ベースのスタイルマップが新規 Missing アイテムとして追加が必要（帰属先が balloon01-core の場合）
5. **dola 内部利用**: バルーン内部サブシステムとして dola を利用可能。Constraint として追加が必要

これらはすべて v3.0 要件更新時にギャップ分析にも反映する。

---

*分析完了。v1.0 から配置制御（Req 5）と anchor 依存を削除し、純粋な描画ウィジェットとして責務を明確化。工数は S（1〜3日）に縮小、リスクは低を維持。cue-system ブロッカー解決後に v3.0 でギャップ分析も更新予定。*
