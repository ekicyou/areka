...
- **直す**: 届くが条件つき（例: 透過の付け外しの時機・受け入れの宣言の付け方・別の権限で動くプロセスからのメッセージを通す設定の要否）で、本坑の中で直せる。直し方を README に書く。絵の上では届くが絵の外でも窓が受け取ってしまう（背後の窓へ抜けない）場合もここに入れる（本坑は絵の外の落とし物を捨て、背後へ抜けないことを既知の制限とする）。
