...
| 6.3 | `\c[line]` が成立しうる行境界 | 配線・登記 | `Vec<PositionedLine>` の行境界＋拡張の口（DD-9） | — |
...
- 行構造（DD-9）: 実体化のフラッシュ (1) で現在行が閉じる＝`\_l` は行の分割点。`PositionedLine { rect, glyphs }` に「分割の由来」「内容の有無」を持たせる拡張は、`finish_line` に引数を 1 つ足すだけで付けられる（`\c[line]` の実装時に消費する口）。本仕様は付けない。
...
| `\c[line]`／`\c[line,数値,開始位置]` | 実装所有者**不在**（2026-09-02 実測）。「行」＝`\_l[x,y]`・`\n`・`\n[half]`・`\n[パーセント]` による分割、`\_l` 以外で分割された内容の無いものは行と見なさない。本仕様の行境界（`Vec<PositionedLine>`）と DD-9 の拡張の口が前提を供給する | 割当は `areka-P0-ukadoc-survey-sakura-script` の台帳 | 6.3・7.3 |
