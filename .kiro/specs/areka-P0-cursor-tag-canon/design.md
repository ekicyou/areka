...
| `\c[line]`／`\c[line,数値,開始位置]` | 実装所有者**不在**（2026-09-02 実測・**2026-09-04 再確認**＝`.kiro/`〔`completed/` を除く〕と `doc/` の Markdown・TOML 全域で `\c[line` の一致が本仕様の文書以外 0 件）。「行」＝`\_l[x,y]`・`\n`・`\n[half]`・`\n[パーセント]` による分割、`\_l` 以外で分割された内容の無いものは行と見なさない（正典逐語＝`requirements.md:205`・2026-09-04 に ukadoc 本文と一致を確認）。**配置層の行数を使ってはならない**（折返しを含み・内容の無い区間を持たない）。行は台本構造＝item 列（`TextItem::LineBreak`／`CursorMove`）を分割点で区切って数え、`\c` 自身を保留の確定点（文字と同じ発行）として扱う（DD-9・開発者裁定 2026-09-04） | 割当は `areka-P0-ukadoc-survey-sakura-script` の台帳。**同 spec の実在を 2026-09-04 に確認**——`brief.md:5`（所有範囲＝さくらスクリプト全 342 項目）・`:27`（全項目に「担当 spec〔既存 brief 名または『所有者なし』〕」を付ける）・`:43`（In に無所有一覧） | 6.3・7.3 |
