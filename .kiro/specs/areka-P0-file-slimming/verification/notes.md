...
## 33. 移設後スナップショットの採取と全体照合（タスク 7.1・要件 1.8 / 2.2 / 2.3 / 2.4 / 2.9）

- 実施日: 2026-08-09 / HEAD `2ef0311`（タスク 6.2 のコミット時点）/ **PowerShell（pwsh 7）** / 既定ターゲット `x86_64-pc-windows-msvc`
- **本 spec の 3 つの中心不変量——テスト総数不変・テストコード無改変・改名は全単射——を確定する節である。** 実装 27 サブタスク（群 1〜6）はすべてコミット済みで、ここが偽の緑になると spec 全体が無効になる
- 逐語のコマンドと全出力は `verification/task_7_1_results.txt`、本文一致の機械可読表は `verification/task_7_1_body_identity.csv`
- 本タスクは `.rs` ファイルを 1 本も変更していない
- **検証はすべて実装者とレビュアーが独立に実施した。** レビュアーは 49 本のサンプル抽出ではなく**全 49 本を再実行**し、スナップショット 2 本も自分で採取し直してバイト一致を確認している

### 33.1 3 本立ての採取（§10.2 と同一手順）

3 本とも exit 0。スコープも `--exclude areka` を含めてタスク 1.3 と同一である（スコープを黙って狭めれば一致は偽装できるため、レビュアーが照合済み）。

整列器の較正は §10.2 手順 3 の要求どおり「**未整列の生出力**を 2 通りに整列して digest が割れること」で行った（コミット済みファイルのハッシュ照合では整列器が動かず較正にならない）。

| 較正 | 序数（採用） | `Sort-Object`（カルチャ順） | 相違位置 |
|---|---|---|---:|
| `after_default` | `593395EB…F30D` | `CEC5ED48…6419` | 1,823 |
| `after_alltargets` | `77D2FED6…5284` | `3CA9B4EC…A4DA` | 1,726 |

**`[string[]]` 型注釈を省いた `[Array]::Sort` はカルチャ順の digest と完全に一致した**——Implementation Notes が警告する罠をこの場で再現したことになる。

| ファイル | 行数 | 相異なる名前 | SHA256 |
|---|---:|---:|---|
| `before_default.txt` | 4,790 | 4,787 | `77F03656…D2AD` |
| `after_default.txt` | 4,790 | 4,787 | `593395EB…F30D` |
| `before_alltargets.txt` | 4,105 | 4,097 | `3DB65799…B29F` |
| `after_alltargets.txt` | 4,105 | 4,097 | `77D2FED6…5284` |

**レビュアーが両リストを独自に採取し直した結果、コミット済みの成果物と 1 バイトの差も無かった。** 符号化規律（UTF-8 BOM 無し・CRLF・末尾改行 1 つ）も遵守。整列が真に序数であることは**位置単位**で確認した——`Compare-Object` は多重集合を比べるので順序の誤りを見抜けない（レビュアーが最初にこれで躓き、位置単位の照合に切り替えて確定させた）。

`after_build_warnings.txt` の 5 数値も基準値と同一（16/7/22/6/16）だが、**要件 2.6 の判定はタスク 7.2 の所管**であり本タスクは採取のみ。§10.5 が予告したとおり duplicates 3 件の帰属が `shiori-host32-testdll` の (lib) と (lib test) の間で移動しているが、これは比較対象外である。

### 33.2 全単射検証（要件 2.9・1.8）— 差分ゼロ

8 フラグメントを結合して **1,021 行**（78+84+239+20+88+75+79+358 とフラグメント総和が一致）。レビュアーが自分で結合し直した結果もコミット済み CSV とバイト一致。

- キー重複 **0** / 値重複 **0**
- 末尾セグメント（テスト関数識別子）の旧新相違 **0**（要件 2.9: 関数識別子は改名しない）
- 恒等行（旧＝新）**0**
- `reason` は `theme_split` のみ
- **`new_fqn` が別の行の `old_fqn` でもある件数 0**——連鎖写像が無いので適用順に依存しない

### 33.3 リスト照合（要件 2.2 / 2.3）— 差分ゼロ

`Compare-TestLists.ps1` は既定列挙・全ターゲット列挙とも **PASS・exit 0**。既定列挙は適用 1,021 / 未使用 0、全ターゲット列挙は適用 663 / 未使用 358。

未使用 358 行は `mapping/areka.csv`（358 行）と**集合として完全一致**し、`--exclude areka` の帰結である。**2 本のリストを跨いで未使用のまま残る行は 0**——どこでも使われない行があれば「変わっていない名前を表に載せた」＝最小性違反になるので、ここは明示的に確認した（レビュアーが再検算）。

**対応表を使わない独立検算も行った。** 多重集合の対称差で、消えた名前の集合が対応表の `old_fqn` 集合と、現れた名前の集合が `new_fqn` 集合と、両リストで完全一致した（既定列挙 REMOVED 1,021 / ADDED 1,021、全ターゲット列挙 663 / 663）。これは対応表が**完全**（登録外の変化が無い）かつ**最小**（変化していない行を持たない）であることの直接証跡であり、各タスクのレビュアーが自分の担当分について行ってきた検算を全域へ広げたものである。

**レビュアーの追加確認: doctest は 37 → 37 でバイト一致。** タスク 6.1 / 6.2 の本番分割が doctest を 1 本も動かしていないことの証跡で、これが崩れると要件 2.2 が黙って壊れる角度だった。

### 33.4 本文一致（要件 2.4）— 差分ゼロ

各元ファイルは固有の base コミットを持つ。**base の導出規則: 当該ファイルを最初に触れた移設コミットの親。** これを git から機械的に導出し、notes.md §13〜§30 に記録された `-Commit` 値と **47/49 が一致**した。

残る 2 ファイル——`crates/dola/src/cue/command.rs`（本表 `bea58b3`）と `crates/shiori-host32-helper/src/main.rs`（同 `fda59cf`）——は、§14 が 3 コミット共通の base として記載する `bfbdfb5` と **blob が同一**であることを `git rev-parse` で確認済みであり、読み出す内容は同じである。タスク 2.2 の 3 本目 `crates/areka-sylphya/src/actor.rs` は本表も `bfbdfb5` を採り notes と一致する（`task_7_1_results.txt` は当初「3 ファイルが乖離」と書いていたがレビュー指摘で 2 ファイルへ訂正した）。

対象 49 本は `target_inventory.csv`（必須対象 49 本）と**集合として完全一致**。移設先は **164 本**（タスク 5.3 が作り 5.4 で解消した `follow_tests.rs` を除く。6.1 / 6.2 が追加した本番サブモジュール 10 本は本番分割ゆえ対象外）。ブランチが追加した `.rs` は 174 本＝164＋10 で辻褄が合う。

**49 本すべて exit 0 の `MATCH`。不一致 0・引数不正 0。** レビュアーはサンプルではなく**全 49 本を再実行**し、`-Detail` の判定文字列が CSV と 1 件も食い違わないことを確認した。

主要ファイルの内訳（抜粋）:

| 元ファイル | base | 移設先 | 判定 |
|---|---|---:|---|
| `placement/follow.rs` | `f64fac1` | 12 | test fn 133=133 / helper 57=57 |
| `emo2_boot/frame.rs` | `2470ccb` | 9 | 56=56 / 56=56 |
| `emo-present/presenter.rs` | `91d59e0` | 8 | 47=47 / 39=39 |
| `tests/ghost/spine_e2e_test.rs` | `585cd5d` | 10 | 15=15 / 40=40 / mod block 10 |
| `areka/src/main.rs` | `425b914` | 7 | 32=32 / mod block 7 |

**49 個の 0 が「何もせず返った 0」でないことを対照実行で確かめた**（exit 2 を exit 1 と読み違える事故は本 spec で一度起きている）。実装者・レビュアーの双方が独立に:

| 対照 | 期待 | 実測 |
|---|---|---|
| 存在しない `-OriginalPath` | 2（引数不正） | **2** |
| 存在しない `-RelocatedPath` | 2 | **2** |
| 移設先を 1 本落とす | 1（不一致） | **1** |
| 別ファイルと組み合わせる | 1 | **1** |
| 移設先の 1 文字改変 | 1＋`[TEST-BODY]` 差分提示 | **1** |
| 無改変のコピー | 0 | **0** |

改変はすべてスクラッチパッドのコピーに対して行い、リポジトリのファイルは 1 本も書き換えていない。

### 33.5 §11.4 の盲点の全域再走査 — 該当 1 行・不一致 0

本文一致検証が構造的に検出できない唯一の差分（複数行文字列リテラル内部の行頭空白）を、49 本まとめて独立に再走査した。判定条件は「行の先頭が既に文字列リテラルの内部にあり、かつ直前行が `\` 継続でない」。実装は `RustParse.ps1` の `[RustItemScan]::Tokenize` を**全文脈と 1 行単体の 2 通り**で走らせ、結果が食い違う行＝前行から続く複数行構文の内部で始まる行として抽出する（新しい弱い解析器を作らずに済む）。

結果は **該当 1 行のみ**——`crates/wintf/src/ecs/window_proc/window_pos.rs:1137`。§11.4 が「リポジトリ全域で 1 箇所のみ」と記録した実測を、ツールとは独立な経路で再現した。当該行は移設後 `window_pos_tests.rs:691` に**行頭空白 17 個ごとバイト一致**で存在し、`\` 継続である前後 2 行（689-690）だけが 4 スペース de-indent されている。**タスク 2.1 の例外処理は正しく効いている。** 行頭空白込みで不一致となったファイルは 0/49。

### 33.6 `TOKENIZER-EQUIV` は**恒久的な想定内の赤**であり、その中身は要件 1.1 の全域証跡である

`Test-VerificationTools.ps1` は 24 ケース中 23 ケースが期待どおりで、`TOKENIZER-EQUIV` のみ赤。§11.6 の予告どおりで欠陥ではない。

**裁定: これは「タスク 7.3 への申し送り」ではなく、恒久的な想定内の赤として扱う。** 当該ケースは移設前ツリーを前提にしており、移設が完了した以上もう緑には戻らない。7.3 は steering の実装規律を書く任務であってこのケースを扱う先ではないし、扱うべきでもない。**判定は残る 23 ケースで行う**——第三者が総合 exit 1 だけを見て赤と読まないよう、ここに明記しておく。

**赤の中身を全数確認したところ、それ自体が要件 1.1 の全域証跡になっていた**（実装者・レビュアーの双方が確認）:

- 走査 619 ファイル / 不一致 **49** ファイル
- 不一致 49 の集合は `target_inventory.csv` の必須対象 49 本と**完全一致**（過不足なし）
- 49 件すべて `actual = ""` ＝ 本番ファイルからテストモジュールブロックが**完全に消えている**
- 残る 570 ファイルは移設前の `modules` 列を**そのまま再現** ＝ 想定外の巻き添え **0**

つまり「必須対象 49 本のテストモジュールが 1 つ残らず本番ファイルから出た」ことと「それ以外は 1 ファイルも動いていない」ことが、対応表とは独立な経路で同時に証明されている。

### 33.7 総括

リスト照合・全単射検証・本文一致の 3 つが**いずれも差分ゼロ**。要件 2.2（テスト総数不変）・2.3（列挙リスト一致）・2.4（テストコード無改変）・2.9（改名は全単射かつ関数識別子不変）・1.8（対応表による突合）を充足する。

### 33.8 本タスクの成果物

| ファイル | 内容 |
|---|---|
| `verification/after_default.txt` | 新規（4,790 行） |
| `verification/after_alltargets.txt` | 新規（4,105 行） |
| `verification/after_build_warnings.txt` | 新規（採取のみ・判定は 7.2） |
| `verification/test_name_mapping.csv` | 23 バイトの器 → **1,021 データ行**（8 フラグメントの結合） |
...
## 35. 実装規律の明文化（タスク 7.3・要件 1.5 / 6.1 / 6.2）

- 実施日: 2026-08-09 / HEAD `6c5cb70`（タスク 7.2 のコミット時点）
- 対象は `.kiro/steering/structure.md` **1 本のみ**。design.md:476-484 が指定する 3 領域（Integration Tests・Unit Tests・モジュール分割パターンの引用ブロック）に限定して改訂した
- **本節は「レビューが 3 件の実体ある誤りを見つけて差し戻した」記録でもある。** 初稿は 6 つのタスク項目と design 指定 4 点をすべて満たしていたが、**文書の中心的な主張そのものが本 spec 自身の成果物によって反証されていた**

### 35.1 改訂の内容

| 領域 | 内容 |
|---|---|
| Unit Tests | 新方式（フラット兄弟テストファイル＋パス属性）への全面書き換え。stem の導出規則 3 ケース・逆引きの一意性規則 2 本・1 モジュール 1 ファイル・1,000 行目安の両面適用・テーマ分割・共有ヘルパの集約・`#[cfg(test)]` 項目の残置・`///` doc の残置・E0428 回避・E0659 回避・`include_str!` 型構造テストの列挙義務・歴史的形式の維持と新規禁止・`benches/`／`build.rs` の 1 行 |
| Integration Tests | パス属性で読み込まれたファイルの子モジュール解決規則と、接続規約が `src/` と同一である旨を追記 |
| モジュール分割パターン | ファサード形式も可である旨と、その固有の注意点 6 点を補記 |

### 35.2 レビューが差し戻した 3 件（いずれも実体ある誤り）

**(1) 中心的主張が反証されていた——逆引きは一意ではなかった。** 初稿は「この規則は双方向に一意である」と断言していたが、**本 spec が自分で作った成果物に反例が 6 件あった**。`crates/areka-emo-text/src/` に `viewbox.rs` と `viewbox_draw.rs` の両方が実在するため、`viewbox_draw_test_support.rs` 等 6 本は「`viewbox` ＋ `draw_test_support`」とも「`viewbox_draw` ＋ `test_support`」とも読める。**タスク 7.3 の完了状態そのもの**（「文書を読んだだけで一意に導出できる」）が満たされていなかった。

是正は断言を 2 本の規則へ置き換えた——**最長 stem 優先**（逆引き）と**前向きの衝突禁止**（テーマ名の選び方）。後者は前者では塞げない穴を塞ぐ: 規則 1 だけでは、2 つの本番ファイルが同じファイル名を欲しがるディスク上の実衝突を防げない。

**再検証は目視でなく機械で行った**——レビュアーが `crates/` 全域の `#[path]` 宣言 **289 件**に新規則を適用し、**全件で真の宣言元を復元（不一致 0）**。6 件の反例はすべて `viewbox_draw.rs` へ正しく解決し、`layout_cursor_tests.rs` や `move_cue_move_sink_tests.rs` のような紛らわしい組でも誤爆しない（短い側の stem に本番ファイルが実在しないため）。

**(2) 行数が 2 つの機構をまたいで足し合わされていた。** 初稿はファサード形式の効果として `follow.rs`（8,472 → 122 行）と `frame.rs`（1,532 → 201 行）を並べていたが、**基準が違う**。`git show` の実測: `follow.rs` は `f05537e` で 8,472、**ファサード分割直前の `bddf620` で 2,032**、分割後 122。ファサードが消したのは 1,910 行で、残る 6,440 行はタスク 5.3／5.4 の**テスト分離**という別機構の成果である。約 98.5% の削減を謳いながら、実際にその技法が出した数字は `frame.rs` の約 87%・`follow.rs` の約 94%（2,032 起点）だった。

是正は両方をファサード分割の差分（2,032 → 122 ／ 1,532 → 201）へ揃え、テスト分離ぶんは別機構として分けて書き、**「2 つの機構の効果を足し合わせて 1 つの数字として語らないこと」**を明記した。

**(3) E0583 の規則が本 spec 自身の実測と矛盾していた。** 初稿は「`#[path]` で読み込まれたファイルの中で `mod X;` と素で書いても解決しない（E0583）」と無条件に書いていたが、`research.md:168` は**逆を実測している**——`tests/dom/a.rs` の中の素の `#[cfg(test)] mod a_tests;` は `tests/dom/a_tests.rs` へ解決して**成立する**。しかも初稿が挙げた理由（「子モジュールはそのファイル自身のディレクトリを基準に解決される」）は、まさに**素の `mod X;` が兄弟 `X.rs` に対して成功する**理由であって、失敗する理由にはなり得ない。読んだ人は「`#[path]` 読込ファイルは素の子モジュールを一切持てない」と誤解する。

是正は `research.md:167-168` §2.3.3 と §19.2 対照 B に合わせて書き直した——子は**そのファイル自身のディレクトリ**を基準に解決される（`<stem>/` サブディレクトリは探さない）ので、素の `mod X;` は兄弟 `X.rs` を探す。E0583 は**歴史的形式の `{module}/tests.rs` へ素で届こうとしたときに出るもの**であり、`#[path]` が必須になる真の理由は命名規約が `<stem>_<名前>.rs` を要求することである。

**(4) 参照方向の誤り**（「上の Unit Tests」が実際には下にあった）も是正。再検証では 3 本の方向つき相互参照をすべて突合した（1 つ直すと別が壊れるため）。

### 35.3 併せて整理した 2 件

- **600 行と 1,000 行の関係を明示**——「600行リファクタ」は当時の spec 名であって現行の閾値ではなく、今の目安は 1,000 行である旨を引用ブロックに 1 行入れた。読者がどちらの数字に従うべきか分かるようにした。
- **`benches/`／`build.rs` 0 件の主張にスコープを付けた**——`vendors/pasta/` には両方実在するため、「本ワークスペースの `crates/*`」と限定した（`vendors/` は §計測の除外対象）。

さらにレビューの非ブロッキング指摘を受け、最長 stem 規則の候補集合を「本番ファイル」から「**接続宣言を持ちうるファイル**（`src/` なら本番ファイル、`tests/` なら統合テストのファイル）」へ広げた——`tests/` ツリーには本番コードが無く、実際の所有者は統合テストのファイルだからである（`spine_e2e_test.rs` が 10 本の `spine_e2e_test_*.rs` を所有する）。

### 35.4 本タスクの成果物

| ファイル | 内容 |
|---|---|
| `.kiro/steering/structure.md` | 3 領域の改訂（45 挿入 / 3 削除・design.md:476-484 の指定範囲に限定） |
| `verification/notes.md` | 本節（§35）を追記 |

## 36. 実測表の更新（タスク 7.4・要件 5.3 / 6.3 / 6.4）

- 実施日: 2026-08-09 / 計測 HEAD `6c5cb70`（タスク 7.2 のコミット時点）/ **PowerShell（pwsh 7）**
- 書き換えたのは `.kiro/specs/areka-P0-file-slimming/brief.md` **1 本のみ**
- 表の全数値は**実装者（読み取り専用の計測エージェント）とレビュアーが独立に再導出**し、全域 7 指標・13 本の census・上位 16 ファイルの更新後 6 列のいずれも一致した

### 36.1 再計測の条件と、ベースラインを壊さないための注意

`verification/Measure-TestModules.ps1` を**無改変で** HEAD `6c5cb70` に対して実行した。スクリプトの除外正規表現に `^\.kiro/` が入っている版であることを実行前に確認済み——Implementation Notes の「本 spec のフィクスチャが全域実測へ混入する（619→627 ファイル）」への対処が入った版である。実際、`git ls-files '*.rs'` から `vendors/` を除いた 801 本と、スキャナの 793 本の差 8 本は `verification/fixtures/relocate/*.rs` ちょうど 8 本であり、除外が効いていることが独立に裏づけられる。

> **⚠ このスクリプトは `-OutDir` を省くと自分の置き場（＝`verification/`）へ書き出す。** つまり素で再実行すると**タスク 1.1 のコミット済みベースライン**（`scan_raw.csv` / `target_inventory.csv` / `excluded_inventory.csv` / `scan_summary.txt`）を上書きし、要件 6.4 が対比せよという「更新前」の証跡そのものを壊す。本タスクは実装者・レビュアーとも `-OutDir` をリポジトリ外へ逃がして実行し、`git diff HEAD -- verification/` が空であることで無傷を確認した。

`6c5cb70` 時点の計測値を採ったが、`git diff --name-only 6c5cb70 <現 HEAD> -- '*.rs'` は空なので、値は本コミット時点でもそのまま有効である。

### 36.2 更新前の値の出所

| 値 | 出所 |
|---|---|
| 619 ファイル・総行 257,134・テスト 99,012・本番本体 158,122 | `verification/scan_raw.csv`（タスク 1.1・コミット `3d418c8`）。`verification/scan_summary.txt` が同値を独立に持つ |
| 最大ファイル 8,472（`placement/follow.rs`） | `scan_raw.csv` の `total_lines` 最大 |
| 1,000 行超 54 本（src 45 ／ tests 6 ／ examples 3） | `scan_raw.csv` の `total_lines > 1000` |
| 必須対象 49 本・テストコード 68,921 行 | `verification/target_inventory.csv` ／ `scan_summary.txt` |
| 上位 16 ファイルの per-file 値 | `brief.md` の起票時実測表（2026-08-06・main `686ff10` 相当） |

### 36.3 更新後の計測——「テストコード」の定義を移設後も同じにするための導出

スキャナの「テストコード」は**ファイル内 `#[cfg(test)] mod` ブロックのみ**を数える定義なので、移設後はそのままでは前後比較にならない（移設したぶんが丸ごと落ちる）。そこで `#[cfg(test)]` ＋ `#[path = "…"] mod …;` の宣言対をリポジトリ全域から独立に列挙した:

- **164 宣言・重複を除いた実体 164 本・欠損 0 本・総行 69,591**
- この 164 本の内側に `#[cfg(test)] mod` ブロックは **0 行**（二重計上なし）
- スキャナ診断の `unresolved separated modules : 164` がこの 164 本と同数（解決器が `#[path]` を追えないための既知の性質であって欠陥ではない）
- 別経路の裏取り: マージベース以降に追加された `.rs` は 182 本、うち `.kiro/` の 8 本を除くと 174 本、そこから本番サブモジュール 10 本（`placement/follow/` 5＋`emo2_boot/frame/` 5）を引いて **164 本**

したがって **テストコード 99,682 行**（30,091＋69,591）・**本番本体 158,660 行**（258,342 − 99,682）。算術の突合: 793 − 619 = 174 = 164 + 10、258,342 − 257,134 = 1,208。

### 36.4 完了状態の 2 指標

| 指標 | 更新前 | 更新後 |
|---|---:|---:|
| 最大ファイル行数 | 8,472（`placement/follow.rs`） | **2,503**（`emo2_boot/spine.rs`＝要件 1.4 の除外） |
| 1,000 行超のファイル数 | 54 | **13** |

移設先テストファイルの最大は 960 行（`crates/areka-ghost/src/runtime_tests.rs`）で 1,000 行超は 0 本＝**要件 1.7 を全域で充足**。残る 13 本の内訳と理由は `brief.md` の更新後実測表に列挙した。

### 36.5 レビューが差し戻した 1 件——表の直下の要約文が自分の表と矛盾していた

初稿は census の直下に「**本番ファイルで 1,000 行を超えるのは `presenter.rs`（1,066）だけ**である」と書いていた。しかし**3 行上の census 自身が `crates/areka-ghost/src/shiori_inproc.rs` を 1,018 行で載せている**。これは本番コードである（`crates/areka-ghost/src/lib.rs:31` の `pub mod shiori_inproc;`・同 `:48` の `pub use shiori_inproc::inproc_connect;`）。

レビュアーは「本番ファイル＝本番本体の意味で書いたのでは」という救済読みを**明示的に棄却した**——同じ文書が per-file 表の列名で 本番本体 という語を使い分けており、当該文の対句は 移設先テストファイル 対 本番ファイル でファイル粒度の二分だからである。この誤りを放置すると、将来の棚卸が「`shiori_inproc.rs` は手当て不要」と読む。

是正は**両方の粒度を別々の文にして、それぞれに正しい語を付けた**:

- **本番ファイル**（ファイル総行）で 1,000 行超は **2 本**——`presenter.rs` 1,066（要件 4.5 で本体分割の範囲外）と `shiori_inproc.rs` 1,018（ファイル内テストモジュール 446 行で要件 1.5 の必須対象に届かない）
- **本番本体**（テストコードを除いた行）で 1,000 行超は **1 本**——`presenter.rs` のみ（`shiori_inproc.rs` の本番本体は 1,018 − 446 = **572**）

再検証でレビュアーは scan から両方を独立に再導出し、`{presenter.rs, shiori_inproc.rs}` と `{presenter.rs}` をそれぞれ確認した。

### 36.6 併せて訂正した陳腐化 1 件

`brief.md` の `## Current State` の箇条書き「本体が実際に太いのは `follow.rs`（1,997）と `frame.rs`（1,498）の 2 本のみ」は、タスク 6.1／6.2 の分割後は現在形として偽になっている。タスク 7.4 の範囲は実測表だが、**自分の作業で偽になった現在形の記述を残すのは登記の質を下げる**ので、原文を残したうえで角括弧の追記で現況を併記した（本番本体 1,000 行超は `presenter.rs` 1 本・`follow.rs` の厳密値は 1,996）。

### 36.7 要件 5.3 の遵守

書き換えたのは `brief.md` のみ。**他 spec の文書は 1 バイトも触れていない。** 置換範囲に含まれる唯一の `file:line` 形式の記述（起票時の注の `:299-345`）は `<details>` ブロック内に**逐語で保存**し、「interleaved は実測 0 件（要件 1.3）」という訂正は別行の注として併記した。差分は 53 挿入 / 1 削除で、削除された 1 行は旧セクション見出しだけである。
...
> **【追記・タスク 8.13】本節は当初 8 件で確定したが、群 8 のタスク 8.10（§50.7）が 9 件目を出したので本表へ追加した。** 見出しの「全 8 件」は「全 9 件」に改めてある。本文 #1〜#8 は 1 文字も書き換えていない（原記述は履歴として保全する規律・§37.3）。
...
| 9 | **ログ捕捉ハーネスが「並行安全」であるという判定が、実測で反証された**。`non_shell_broadcast_reception_is_benign_debug_no_warn_error` が**ワークスペース検証中に 1 度だけ失敗した——捕捉したログバッファが空**。隔離再実行は 177/177 緑、ワークスペース再実行も exit 0 で以後再現していない。**これは §16 の記録（`notes.md:1617`）の結論を経験的に反証する**——§16 は `capture_logs`／`capture_logs_flow` を「`with_default` はスレッドローカルだから並行干渉を受けない」として **問題なし** と判定していたが、空バッファはその前提が崩れた形である。ハーネス本体の doc 自身が `actor_test_support.rs:48-49` で「スレッドローカル `with_default` ゆえ並行テスト安全」と逐語で主張している。**#3・#5・#7 と同じ「ログ捕捉ハーネス」主題だが、あちらが重複の登記なのに対しこちらは捕捉機構そのものの決定論の欠陥**であり、#6・#8 と同じく偽の赤を生みうる。**再現率が低い（1/多数）ので、W6.9 は「再現しないから無い」と読まないこと** | 失敗した assert: `crates/areka-seriko/src/actor_dispatch_tests.rs:741`（`assert!(flow.0.contains("level=DEBUG"), …)`）／囲むテスト: 同 `:705`（`#[test]`）・`:706`（`fn`）／捕捉呼出: 同 `:713`（`capture_logs_flow`）／ハーネス実体: `crates/areka-seriko/src/actor_test_support.rs:39`（`fn capture_logs_flow`）・`:50`（`fn capture_logs`）・並行安全を主張する doc は `:48-49`／反証された判定: `verification/notes.md:1617`（§16.5 の記録のみ表 #1） | §50.7（タスク 8.10・**群 8 唯一の新規送付所見**）。**本行はタスク 8.13 が追加した** |

**9 件のいずれも要件 5.1 が本 spec に禁じている領分である**——#1 は注入シームの変更、#3・#5・#7 はハーネスの一本化・共有化、#6・#8 は待機機構の書き換え（かつテスト本文の変更＝要件 2.4 違反）、#2 と #4 は本 spec の対象外ファイルにあり #4 の是正はテストの入力値変更に当たる。ゆえに全件を所有 spec へ送る。

**#9 も同じである**（タスク 8.13 追記）——是正は捕捉ハーネスそのものの作り替えであり、要件 5.1 が禁じる「テストハーネスの一本化・共有化」および「テスト間の状態汚染の是正」の中心そのものに当たる。**本 spec は `areka-seriko` のテスト本文を 1 バイトも変えていない**（群 3.2 の移設で位置だけが動いた。§37.5(d) の実数表で `struct LevelCounter` 3・`struct WarnCounter` 5 が基準と HEAD で不動であることが裏づけ）。**したがって #9 は本 spec が持ち込んだ欠陥ではなく、本 spec の検証作業が既存の欠陥を顕在化させたものである。**
...
- **#9 の適合（タスク 8.13 追記）: 主題は Problem ③ と一致するが、問われている性質が違う。** ③ は「ログ捕捉ハーネスが 3 コピー、しかも競合する 2 設計が併存」という**重複**の問題で、#9 は**その捕捉機構そのものが並行実行下で取りこぼしうる**という決定論の問題である。`areka-seriko` が ③ の現行 scope に入っているかは brief の列挙で確認を要する（#3・#5・#7 と同様、列挙されている具体的なコピー集合とは一致しない可能性が高い）。**受理側への注意**: 症状は 1 度きりの空バッファで、隔離実行も再実行も緑である。**「再現しないから欠陥でない」と読まないこと**——「ログが出ないこと」を主張する assert（`matches("level=WARN").count() == 0` 等）は、捕捉が空になれば**空虚に真**になるので、同じ根が偽の赤ではなく**偽の緑**を出す側にも回る。§16 が「捕捉漏れで空虚に真になる経路は存在しない」と結論した根拠（`with_default` のスレッドローカル性）が反証された以上、その結論の射程は W6.9 で取り直す必要がある。
...
## 38. 実装全体の最終検証（`/kiro-validate-impl`・GO/NO-GO ゲート）

- 実施日: 2026-08-09 / HEAD `f4ef098`（タスク 7.5 のコミット時点）/ マージベース `247d48a`・46 コミット
- **判定: GO**
- 位置づけ: 各タスクの合否は実装時のレビュアーが既にゲートしている。本節が見るのは**タスクを横断して初めて見えるもの**——要件の取りこぼし、設計からの逸脱、境界のはみ出し、証跡の連鎖の整合である
- 独立した 2 系統の検証エージェントへ分担させ、いずれも**台帳の記述を読むのではなく自分のコマンドで再導出**した

### 38.1 機械的検査

| 検査 | 結果 |
|---|---|
| ワークスペース全テスト | `cargo test --workspace --no-fail-fast` **exit 0**・85 ユニット・**4,757 passed / 0 failed / 33 ignored / filtered out 0** |
| 実行時の起動確認（smoke） | 全テストに含まれる `smoke_boot_loop_exit`（骨格起動→ループ→exit 0／実ゴースト窓つき起動）と `emo2_real_run`（実ゴーストが起動し発話して exit 0）がいずれも緑——**ビルド成果物が実際に起動して最初の使用可能状態へ到達することを実測で確認** |
| 全ターゲットビルドの警告 | 16 / 7 / 22 / 6 / 16（基準値と完全一致） |
| `TODO`／`FIXME`／`HACK`／`TBD` | 本 spec が触れた `.rs` 49 本に **0 件** |
| ハードコードされた秘密情報 | 同 **0 件** |

### 38.2 要件被覆（要件 1〜7・全 41 受入基準）

**41 / 41 が COVERED。PARTIAL 0・GAP 0。**

タスクの `_Requirements:_` 行は「主張」であって証拠ではないので、全基準について**証拠の実体**（検証成果物・台帳の節・file:line）まで辿って確認した。うち **10 基準は検証側が独自に再導出**している。特筆すべき独立再導出:

- **2.2 の穴を 1 つ塞いだ**——`-- --list` は ignored の別を出力しないので、テストが黙って `#[ignore]` 化されても 4,790 という本数は保たれてしまう。そこで `#[test]` 属性 **4,800 = 4,800**・`#[ignore]` **7 = 7** を基準と HEAD で突合した。列挙スナップショットが構造的に見られない角度である
- **2.4** を 6 大ファイルについて、括弧の釣り合いで `#[test] fn` 本体を切り出し空白正規化して突合（`follow.rs` 133=133・`frame.rs` 56=56・`presenter.rs` 47=47・`main.rs` 32=32 ほか。本文差分 0・重複 0）
- **1.1／1.6** 全域再走査——ファイル内テストモジュールが 500 行超のファイルは **0 件**（最大 484）、非 `mod` の `#[cfg(test)]` 項目は **40 = 40**
- **2.9** 対応表 1,021 行のキー重複 0・値重複 0・恒等行 0・連鎖 0・関数識別子相違 0、および**位置単位**での前後リスト一致（`applied=1021 positional_mismatch=0`）

### 38.3 設計との整合と裁定 #1〜#13

**13 件すべて遵守。** File Structure Plan との差は**テストファイル数のみ**（design 概算 ≈110 → 実測 164）で、design がテーマ名・本数の 22/24 を実装時裁定に委ねていた結果である。**一貫して細かい側へ振れており**、最大の移設先が 960 行で 1,000 行の縁に張り付いたファイルが 1 本も無いことがその裏づけになる。事前に名前を決めていたファイル名は全て逐語で着地した。

本体分割の 2 木（`follow/` 5 本・`frame/` 5 本）は design の表と完全一致し、10 本すべて 701 行以下。

**裁定 #10（≤500 行の任意移設を行わない＝否定的コミット）**も確認済み——`#[cfg(test)] #[path]` 宣言を持つファイルは `target_inventory.csv` の **49 本ちょうど**で、余分 0・欠け 0。歴史的形式の分離済み 15 本には触れていない。

### 38.4 依存方向

- **配置木の `crate::` 禁止**: `follow.rs` ファサードと 5 サブモジュールの**本番コードに 0 件**。しかも `cargo build -p areka --examples` が **exit 0** で、examples → `placement/mod.rs` → `follow.rs` → `follow/anchor.rs` の **3 階層解決が実物で成立**していることを確認した（タスク 1.5 は仮置きでのスモークだったので、これが本番形での初の実証になる）
- **`areka-emo-text` の純粋層に `windows` 依存なし**: `PURE_SOURCES` の 9 モジュール**と、その兄弟テストファイル群**の双方で禁止パターン 0 件。**これは §37 の所見 #4（`include_str!` の被覆縮小）が実害ゼロであることの独立確認**でもある——縮んだ走査範囲に隠れていたものは無かった
- 新たな上位方向・層跨ぎの import は 0。移設した本番 49 本のうち **47 本は宣言以外の追加行が 0 行**

### 38.5 境界のはみ出し（本 spec の位置 ／ `test-cage-determinism` の中身）

**検出なし。** §37.5 の多重集合証明を信用せず、**別角度から独立に**組み立てた:

1. **本番側の差分監査**——49 本それぞれについて、`#[cfg(test)]`／`#[path]`／`mod X;`／空行**以外**の追加行を数えた。**49 本中 47 本が 0 行**。内容を足しているのは `follow.rs`（33）と `frame.rs`（63）＝認可された 2 本だけ。これ 1 つで「3 本目の本体分割」「紛れ込んだ挙動変更」「本番ファイル内のテストロジック改変」がまとめて否定される
2. **判定と属性の保存**——11 パターンを基準と HEAD で突合し**全て同数**（`#[test]` 4,826・`assert!` 4,191・`assert_eq!` 8,920・`assert_ne!` 279・`panic!` 484・`expect(` 2,988・`#[should_panic` 6・`#[ignore` 7・`debug_assert` 89・`unreachable!` 38 ほか）
3. **時刻シームの保存**——`thread::sleep` 50・`Instant::now` 141・`Duration::from_millis` 150・`Duration::from_secs` 253・`advance(` 38・`recv_timeout` 167・`yield_now` 43 がいずれも同数。文字列一致で増えて見えた 3 語（`spin` +10・`timeout` +2・`deadline` +1）は**すべて新モジュール名によるファイル名の一致**であってシームではないことを個別に追跡した
4. **ハーネス統合の探索**——`capture_logs` の定義は**基準 14・HEAD 14**。それぞれ自分の兄弟ファイルへ移っただけで、ファイルを跨ぐ集約は 1 件も無い

### 38.6 タスク横断の整合

- **対応表の合成**: 8 フラグメントの総和 **1,021 = 1,021**。孤児フラグメント 0。フラグメントを持たない 4 クレートは、タスク 2.1／2.2 がテーマ分割なしの純移設で処理したクレートと**ちょうど一致**する
- **宣言とファイルの一対一**: 全域 **290 件**の `#[path]` 対象がすべて実在（欠損 0）、命名規則違反 0、テストファイル 165 本すべてが**宣言元をちょうど 1 つ**持つ。`#[path]` 宣言を持たない追加 `.rs` 18 本は、本 spec の検証フィクスチャ 8 本（コンパイル対象外）と本体分割のサブモジュール 10 本（Rust 2018 の `foo.rs`＋`foo/` 形式で素の `mod` から引く）で、孤児ではない
- **タスク 6.1／6.2 が先行タスクを壊していない**: 両コミットが触れたのは `follow.rs`／`frame.rs` とその新サブモジュールのみで、**テストファイルの編集は 0 件**。凍結されたテスト群は今もファサード経由だけで私有項目へ届いている
- **タスク 7.3／7.4 の文書が実物と合っている**: steering の導出規則を実在 165 本へ適用して**不一致 0**。最長 stem のタイブレークは実際に必要（`viewbox.rs` 対 `viewbox_draw.rs`）で、新たな衝突も生まない。`structure.md` が引く行数も全数が実測と一致

### 38.7 GO 判定に添える申し送り（いずれも非ブロッキング）

1. **リポジトリ最大のファイルは今 `crates/areka/src/emo2_boot/spine.rs` の 2,503 行**であり、目安の 2.5 倍である。要件 1.4 の除外（親が既に `#[cfg(test)] mod spine;` で分離済み）に正しく該当し基準違反ではないが、**本 spec の動機が「1 ファイルの行数」そのものだった**以上、「8,472 → 2,503」の裏でこの 1 本が手つかずであることは**気づかれずに通すのではなく、意識して受け入れる**べき事項である。残る 12 本にもそれぞれ文書化された除外理由がある
2. **`cargo doc` の未解決 intra-doc リンク 95 件**（`follow` 21・`frame` 37・他）。純移動のバイト一致証明と両立しないため一律未修正としたのは正しい判断で steering にも記録済みだが、**どのゲートも見ていない恒久的な品質劣化**である
3. **`crates/areka` の examples は test モードで既存 E0433 のまま**——本 spec が持ち込んだものではなく登記・送付済みだが、**当該 examples のテストターゲットにはコンパイル被覆が一切無い**状態が続く
4. **所見 #4（`PURE_SOURCES` の被覆縮小）は、本 spec が生んだ唯一の被覆後退であり、自然な所有者がいない**。`areka-emo-text` 系の spec は全て `completed/` にある。§38.4 で実害ゼロを独立確認したので急がないが、**所有者不在のまま残る 1 件**である
5. **要件 7.2（クレート単位の緑を確認してからコミット）だけは再現可能な成果物を持たない**。27 の実装コミットを順に checkout し直せば検証できるが破壊的 git になるため本ゲートでは行わない。§13〜§30 の記録（16 件の明示的な緑）に依る。HEAD の全緑は再現したが、それは各コミット時点の状態を遡って証明するものではない
6. **`Test-VerificationTools.ps1` は恒久的に exit 1 を返す**（`TOKENIZER-EQUIV`・§33.6 の裁定）。判定は残る 23 ケースで行うこと

### 38.8 判定

```
DECISION: GO
MECHANICAL: Tests PASS（exit 0・4,757/0/33）／ smoke PASS ／ TODO CLEAN ／ secrets CLEAN ／ 警告 16/7/22/6/16
INTEGRATION: 対応表の合成・宣言とファイルの一対一・6.1/6.2 の遡及影響なし・文書と実物の一致——いずれも整合
COVERAGE: 41/41（GAP 0・PARTIAL 0）
DESIGN: 裁定 13/13 遵守・逸脱なし・依存方向クリーン・File Structure Plan の差はテストファイル数のみ（実装時裁定の範囲）
BOUNDARY: はみ出しなし（独立に組み直した 4 系統の検査で確認）
OWNERSHIP: LOCAL（送付済み 8 所見は登記された先送りであって本 spec の欠陥ではない）
BLOCKED_TASKS: なし（30/30 完了）
```

## 39. 【GO の撤回】1,000 行超 13 本は要件違反である（2026-08-09 開発者裁定）

- **§38 の GO 判定を撤回する。**
- 撤回の理由: §38.7 の申し送り (1) で「リポジトリ最大は `spine.rs` 2,503 行だが要件 1.4 の除外に該当し基準違反ではない」と報告したが、**開発者裁定によりこれは要件違反と確定した**。1,000 行を超えて残った **13 本すべて**が対象である。
- 本節は撤回の記録であり、消化は**群 8（タスク 8.1〜8.13）**で行う。

### 39.1 裁定の内容

**要件 1.7 の宣言が上位である**——「**この目安は本番ファイルとテストファイルの双方に等しく適用する。本 spec の基準は「1 ファイルの行数」であって「本番ファイルの行数」ではない**」。要件 1.1 の対象範囲も `src/`・`tests/`・`examples/`・`benches/`・`build.rs` を明記している。**全ファイルが対象である。**

これを受けて要件を 2 箇所改訂した（原文は取り消し線つきで保存）:

| 要件 | 改訂内容 |
|---|---|
| **4.5** | 「`follow.rs`・`frame.rs` 以外の本番本体を分割しない」を**取り消し**、「**1,000 行を超える全ファイル**の本番本体を分割する」へ改める。前提だった「本番本体 500〜1,000 行の水準は健全」は、1,000 行超の 4 本には最初から当てはまっていなかった |
| **1.4** | 「移設対象から除外」が除外するのは**移設だけで分割ではない**ことを明確化。既分離ファイルは移す先が無いだけであり、1,000 行目安は他のテストファイルと同様に適用される。**この読み方により 1.4 と 1.7 は矛盾せず両立する** |

### 39.2 見落としの出所は 3 つあり、性質が異なる

自己弁護のためでなく、同じ穴を再び踏まないために分けて記録する。

**① 検出条件が届いていなかった（5 本・6,447 行）——これが最も悪い。**
要件 1.1 の絞り込みは「`#[cfg(test)]` テストモジュールの合計が 500 行超」である。ところが `tests/` 配下のファイルは**ファイル全体がテストなので `#[cfg(test)]` を書かない**。実測すると `choice_test.rs`（1,563 行）・`mouse_test.rs`（1,116）・`close_test.rs`（1,009）・`dola/tests/cue/runtime_test.rs`（1,102）はいずれも **`#[cfg(test)]` の出現が 0 件**で、スキャナの視界に一度も入っていない。`common/mod.rs`（1,657）は 1 件あるが 256 行で閾値未満。

**要件 1.1 は対象範囲に `tests/` を明記していながら、絞り込み条件が `tests/` ではほぼ発火しない。** 唯一 `tests/` から必須対象になった `spine_e2e_test.rs` は `#[cfg(test)] mod` を使う例外的な書き方だったからである。これは判断の誤りではなく**条件の設計欠陥**であり、タスク 1.1 の全域再計測が design 値 49 と一致したことも、この欠陥を隠す方向に働いた——**フィルタと整合していることは、目的を被覆していることの証明にならない。**

**② 要件 1.4 の読み違え（3 本・5,254 行）。**
`spine.rs` 2,503・`decode_tests.rs` 1,395・`golden_tests.rs` 1,356。タスク 3.1 で「1,000 行超でも分割しない」と裁定し、その根拠 4 点（親から素の宣言済み／`excluded_inventory.csv` に登載／`#[cfg(test)] mod` ブロックを持たないので要件 1.1 の対象でない／要件 1.7 が縛るのは移設先のみ）を Implementation Notes に明文化して以後の全タスクへ配った。**根拠はいずれも「移設」の話であって「分割」の話ではなかった**のに、結論だけが分割へも及ぶものとして流通した。**誤った裁定を早期に明文化したことで、後続タスクが再検討する契機を奪った。**

**③ 要件 4.5 が 1.7 を上書きすると読んだ（4 本・4,731 行）。**
`presenter.rs` 1,066・examples 3 本。4.5 の文言は無条件の禁止だが、その根拠として括弧書きされた「本番本体 500〜1,000 行の水準は健全と判定済み」は**1,000 行を超えるこの 4 本には最初から当てはまらない**。根拠が届かない範囲まで禁止を及ぼしていた。

### 39.3 検証が見つけられなかったこと

タスク 7.1／7.2 の 3 不変量（テスト総数・本文一致・全単射）と §38 の 41 基準被覆は**すべて正しい**。それらが測っていたのは「**やったことが正しくやれているか**」であって、「**やるべきことを全部やったか**」ではない。

要件 1.7 に対する被覆判定を §38.2 では「移設先テストファイルの最大 960 行・1,000 行超 0 本」で COVERED としたが、これは 1.7 の**発動条件（移設先）**だけを見て**宣言（1 ファイルの行数が基準）**を見ていない。**基準の宣言文と、それを運ぶ条件節の射程が食い違っているとき、条件節だけを検証すると必ずこの取りこぼしが出る。**

なお §38.7 の申し送り (1) で「最大ファイルは 2,503 行・目安の 2.5 倍・気づかれずに通すのでなく意識して受け入れるべき」と提起はしていた。**提起しながら GO を出したのが誤りである**——要件との整合に疑いがある事項は、申し送りではなくゲートの判定材料にしなければならない。

### 39.4 対象 13 本と担当タスク

| 行 | ファイル | 出所 | 担当 |
|---:|---|---|---|
| 2,503 | `areka/src/emo2_boot/spine.rs` | ② | 8.1 |
| 1,657 | `areka-kanade/tests/kanade/common/mod.rs` | ① | 8.2 |
| 1,563 | `areka-kanade/tests/kanade/choice_test.rs` | ① | 8.3 |
| 1,434 | `areka-emo-text/examples/emo-text-layer.rs` | ③ | 8.10 |
| 1,395 | `areka-parsers/src/shell/decode_tests.rs` | ② | 8.5 |
| 1,356 | `areka-emo-compose/src/golden_tests.rs` | ② | 8.6 |
| 1,168 | `areka/examples/emo-present.rs` | ③ | 8.11 |
| 1,116 | `areka-kanade/tests/kanade/mouse_test.rs` | ① | 8.4 |
| 1,102 | `dola/tests/cue/runtime_test.rs` | ① | 8.7 |
| 1,066 | `areka-emo-present/src/presenter.rs` | ③ | 8.9 |
| 1,063 | `areka/examples/collision-probe.rs` | ③ | 8.12 |
| 1,018 | `areka-ghost/src/shiori_inproc.rs` | ①（閾値未満・要件 1.5 で併せ移設） | 8.8 |
| 1,009 | `areka-kanade/tests/kanade/close_test.rs` | ① | 8.4 |

合計 ~~**16,450 行**~~ → **17,450 行**（**【訂正・タスク 8.13】** 13 行の個別値はすべて正しく、総和のみ 1,000 を落としていた。レビュアーが分割前ブロブから全数実測: 2,503＋1,657＋1,563＋1,434＋1,395＋1,356＋1,168＋1,116＋1,102＋1,066＋1,063＋1,018＋1,009 ＝ 17,450）。群 8 の完了時に §38 を再判定する（タスク 8.13）。

## 40. 統合スパインのテーマ分割: `crates/areka/src/emo2_boot/spine.rs`（タスク 8.1・要件 1.4 / 1.7 / 2.2 / 2.4 / 2.9 / 3.1 / 3.2）

- 実施日: 2026-08-09 / 分割前コミット `09f3b85`（群 8 追加のコミット）/ **PowerShell（pwsh 7）**
- 対象は §39.4 の 13 本のうち**最大の 1 本**（2,503 行＝リポジトリ最大）。親 `emo2_boot/mod.rs:30-31` の素の `#[cfg(test)] mod spine;` で外部から gate された**既分離ファイル**であり、要件 1.4 の明確化どおり**移設ではなく分割**である。親の宣言（歴史的形式）は 1 文字も触っていない
- 触れたのは `spine.rs`＋新規テストファイル 7＋`verification/mapping/areka.csv`（追記）の 9 パス

### 40.1 成果ファイルと行数（すべて 1,000 行以下）

| ファイル | 行 | テスト |
|---|---:|---:|
| `spine.rs`（ハーネス本体 `:1-769` ＋接続宣言） | **794** | 0 |
| `spine_seriko_loop_tests.rs` | 594 | 5 |
| `spine_talk_close_tests.rs` | 355 | 2 |
| `spine_display_tests.rs` | 326 | 3 |
| `spine_text_scale_tests.rs` | 160 | 2 |
| `spine_move_cue_tests.rs` | 154 | 1 |
| `spine_boot_smoke_tests.rs` | 89 | 1 |
| `spine_test_support.rs` | 83 | 0 |

5＋2＋3＋2＋1＋1 = **14**（分割前と一致）。**リポジトリ最大 2,503 → 794 行。**

### 40.2 ハーネス本体は `spine.rs` に残した（`test_support` へ出さない）

`spine.rs` は「ファイル全体がモジュール本体」で `mod tests { … }` ブロックを持たない。`:1-769`（module doc・`use` 群・`ScriptedShioriBackend` 一式・GPU/fixture/有界待機ヘルパ・ログ捕捉・`SpineHarness`）は**テストが叩く本体**であり、タスク 6.1 の `follow.rs` が本番本体を残してテーマモジュールを宣言した形と同型である。親モジュールの私有項目は子から到達できるため**複製も可視性変更も不要**。`spine_test_support.rs` には兄弟テーマ間で実際に共有される 4 項目だけを集約した。

**`spine.rs:1-769` は分割前と逐行 diff で完全一致——バイトだけでなく行番号も動いていない**（分割前の `use` を 1 行も削らずに残せたため。子が `use super::…` で引くので `unused_imports` は出ないことを実測確認）。これにより `test-cage-determinism`（W6.9）が Problem ② で名指しする待機機構のアンカーが**そのまま解決する**——`SPIN_WAIT`（`:298`）・`SPIN_YIELD_BUDGET`（`:316`）・`BACKOFF_SLEEP`（`:319`）・`spin_wait_until`（`:327-344`）の 4 点をレビュアーが個別に再確認した。

一方、各テスト本体の中に在る `deadline`／poll-backoff は 6 本の兄弟ファイルへ移ったので、**テスト本文を指すアンカーだけが rebase 対象**である。同 brief の Boundary Candidate「待機機構（② — `spine.rs` **単独ファイル**）」の射程は、本タスクにより **8 ファイル**へ広がった。

### 40.3 ⚠ 既分離ファイルでは `Compare-RelocatedTests.ps1` が **exit 2** を返す（タスク 8.2〜8.7 への申し送り）

`spine.rs` は `#[cfg(test)] mod X { … }` ブロックを 1 個も持たないため、`RustItemScan::FindTestModules` が 0 件を返し `Compare-RelocatedTests.ps1:118-121` が「元ファイルに比較対象の `#[cfg(test)] mod` ブロックがありません」で **exit 2** を出す。**これは引数不正であって本文不一致ではない**（§11.2 の 0/1/2 契約）。レビュアーが独立に再現した。

**同じ穴を踏むタスク**: 8.5（`decode_tests.rs`）・8.6（`golden_tests.rs`）＝同型の既分離ファイル、および 8.2／8.3／8.4／8.7 ＝ `#[cfg(test)]` を書かない `tests/` 配下のファイル。**群 8 の 13 本中 7 本が該当する。**

**恒久対処**: スクリプトへ「ファイル全体を 1 モジュールとして扱う」スイッチを足す（§41 で実施）。

**本タスクで採った手当てと、その証跡としての限界**: 実装者は原本をバイト不変のまま `#[cfg(test)] mod spine_before { … }` で包んだ検証用入力を作り `-OriginalPath` に渡して `MATCH: test fn 14=14 / helper item 46=46 / mod block 1 / files 8`（exit 0）を得た。**判定の中身は正しい**——`RustParse.ps1` が全行を lstrip するので括弧階層の追加は比較に影響せず（§11.4 の第 1 行）、ヘルパとテストの区分は `#[test]` 属性で決まり入れ子の深さに依らず、ラッパ自身は項目を 1 つも足さない。レビュアーがこの 3 点を個別に検証した。

**しかし手作りの入力ファイルはリポジトリに無く、第三者が再実行できない**——Implementation Notes が「記録値が第三者に再現できない」として戒めている当の失敗形である。**再現可能な等価手順はこちら**（レビュアーが実行し同一結果を得た）:

```
# RustParse.ps1 を読み込み、git show で取り出した原本の全行を 1 項目集合として digest 化し、
# 現在の 8 ファイルの root digest と突合する（ラッパも一時ファイルも要らない）
Get-ItemDigest -Lines (git show 09f3b85:crates/areka/src/emo2_boot/spine.rs) -From 0 -To $lines.Length
  → test fn 14=14 / helper item 46=46 / issues 0
```

**以後の登記はこちらを正本の手順とする。**

### 40.4 本文一致の独立検算（要件 2.4）

行の多重集合（**トリムせず**）で分割前 2,503 行と成果物 8 本 2,555 行を突合した。レビュアーが独立に再導出し同数を得ている:

| 区分 | 件数 | 内訳 |
|---|---:|---|
| 置換 | **4** | `pub(super)` 付与前後のシグネチャ行のみ（`opaque_count`・`variant_name`・`bump_balloon_window_dpi`・`bump_char_window_dpi`） |
| 追加 | **51** | `#[cfg(test)]` 7・`#[path]` 7・`mod X;` 7・見出しコメント 3・`use` 27 |
| 削除 | **0** | — |
| **字下げの変更** | **0** | — |

**字下げ 0 件は単独で記録に値する。** 原本の項目はもともと列 0 に在ったので **de-indent が構造的に発生しない**——§11.4 の第 1 の盲点（複数行文字列リテラル内部の行頭空白）を「走査して該当なし」ではなく「**構造的に成立しない**」形で閉じたのは本 spec で初めてである。独自スキャナでの走査でも分割前・成果物とも該当 0 件（較正は既知ケース `window_pos_tests.rs` の `継続行 5 件: 382,429,614,690,691 / 盲点 1 件: 691` を再現して確認）。

### 40.5 テーマ境界の裁定

バナー 12 本はすべて thematic。区画をそのままテーマにしたのが 5 つ、ヘルパ参照関係で 3 区画を 1 つに束ねたのが `seriko_loop_tests`（`always_fire_rng`・`shell_pattern_frame`・`seriko_tick_expect_one` が 3 区画を跨ぐ。割ると共有ヘルパが 3 件増えるだけで境界の意味が出ない。594 行で目安内）。

**`S4` の配置**——物理的には「文字層 k 追従」区画の末尾に在るが `display_tests` へ寄せた。レビュアーの検証によれば**これは単に許容されるのでなく正しい**: 6.2 のバナー（`:860`）が自ら「S1／S3／**S4**」と列挙しており、S4 の直前に物理的に在るバナー（`:1144-1150`）は「以下 **2** ケース」と自己限定していて `:1216` と `:1287` しか指していない。物理位置で束ねると誤配置になっていた。

**バナー 2 本が `test_support` へ同伴した**（本文一致検証は先行コメント塊を後続項目へ束ねるため）:
- `:859-865`（task 6.2 観測ケース）→ `opaque_count` に付随。**これは強制**——当該ヘルパは 3 テーマから参照されるので集約が必須（複製は `ITEM-EXTRA`）、バナーだけ切り離せば `ITEM-MISSING`。合法な代替が無い
- `:1143-1151`（文字層 k 追従）→ `bump_balloon_window_dpi` に付随。**これは回避可能だった**（レビュー指摘）——当該ヘルパの消費者は `spine_text_scale_tests.rs` の 1 テーマだけで、真に共有なのは私有の `bump_window_dpi` のみ。テーマ側に残せばバナーが説明する 2 ケースと同居できた。**バイト検証済みのファイルを再度かき混ぜる価値は無いと判断して是正しない**が、**タスク 8.5／8.6 が同種の選択に直面したら、単一消費者のヘルパはテーマ側に残すこと**

### 40.6 検証結果

| 検証 | 基準（`09f3b85`） | 分割後 | 判定 |
|---|---|---|---|
| `cargo test -p areka` | 671+1+2 = **674 passed / 0 failed** | 同左 | 一致 |
| `cargo test -p areka -- --list` | 674 行 | 674 行・対称差は removed 14 / added 14 のみ | 一致 |
| `emo2_boot::spine::` 直下のテスト | 14 | **0** | 全数が子モジュールへ |
| 対応表 `areka.csv` | 358 行 | 372 行（先行 358 行はバイト不変・`numstat` = `14 0`） | 追記のみ |
| 対応表 8 フラグメント結合 | 1,021 行 | **1,035 行**・全単射 PASS | 一致 |
| ワークスペース警告 | 16/7/22/6/16 | 同左 | 一致 |
| 各ファイル行数 | — | 最大 794 | 全数 1,000 行以下 |

対応表 14 行はすべて `emo2_boot::spine::<name>` → `emo2_boot::spine::<テーマ>::<name>` で末尾セグメント不変。
...
## 42. 選択肢系統合テストのテーマ分割: `crates/areka-kanade/tests/kanade/choice_test.rs`（タスク 8.3・要件 1.7 / 2.2 / 2.4 / 2.9 / 3.1 / 3.2）

- 実施日: 2026-08-09 / 分割前コミット `5cf4866` / **PowerShell（pwsh 7）**
- §39.4 の出所 ①（`tests/` 配下で `#[cfg(test)]` を書かないファイル）の 1 本。**ファイル全体がテストコード**ゆえ移設ではなく分割
- 入口 `tests/kanade.rs` は 1 バイトも変更していない

### 42.1 成果ファイルと行数（すべて 1,000 行以下）

| ファイル | 行 | テスト |
|---|---:|---:|
| `choice_test.rs`（module doc＋`use`＋接続宣言 7 組） | **106** | 0 |
| `choice_test_timeout_tests.rs` | 334 | 2 |
| `choice_test_rejection_tests.rs` | 298 | 2 |
| `choice_test_canonical_cascade_tests.rs` | 246 | 2 |
| `choice_test_stage_failure_tests.rs` | 211 | 2 |
| `choice_test_choosing_status_tests.rs` | 183 | 1 |
| `choice_test_named_event_tests.rs` | 137 | 1 |
| `choice_test_test_support.rs` | 133 | 0 |

**1,563 → 最大 334 行**・合計 1,648・テスト 10 = 10。

### 42.2 `-WholeFile`（§41）の初適用——§40.3 の申し送りが解消した

本ファイルは `#[cfg(test)] mod` ブロックを 0 個しか持たないため従来経路は exit 2 で止まる種類である。`-WholeFile` を付けた 1 回の実行で `MATCH: test fn 10=10 / helper item 19=19 / mod block whole-file / files 8`（exit 0）を得た。**手作りのラッパ入力も一時ファイルも要らず、逐語のコマンドがそのまま第三者の再実行手順になる。**

レビュアーは緑を鵜呑みにせず**この入力で較正**した——移設先から `choice_test_rejection_tests.rs` を 1 本落とすと `MISMATCH: 3 件`（exit 1）で 2 テストが `TEST-MISSING` として名指しされる。**道具がこの入力で実際に弁別している**ことの確認である。

### 42.3 テーマ境界の裁定——境界を新設していない

module doc `:11-50` が自ら「群 1〜6」を宣言し、本文の `// ======` バナー 6 本が同じ 6 群を区切る。テーマ＝群を採用した。バナー 6 本はすべて各群先頭のテスト関数へ同伴（比較器が先行コメント塊を次項目へ束ねるため、切り離すと `ITEM-MISSING`）。`use` 直前バナー（§28.5）は 0 件。

### 42.4 ⚠ テーマモジュール名は `<テーマ>_tests` である（レビュー差し戻し 1 件・後続への申し送り）

初回実装はテーマモジュールを `named_event`／`timeout` 等と **`_tests` 接尾辞なし**で命名し、差し戻された。`design.md:134`（「新モジュール名は `<テーマ>_tests` 形式」）と `:239` が命じている。**レビュアーが対応表 1,045 行を機械走査したところ、リポジトリ全域 約 102 テーマモジュールのうち接尾辞を欠くのは当該 6 本だけだった**——数時間前にコミットされた同じ群のタスク 8.1 も `spine_<テーマ>_tests.rs` を採っている。**要件 3.1 は対象ごとに方式を変えることを禁じている。**

是正後、同じ機械走査で**接尾辞を欠くモジュールは 0 件**。旧名の残存参照も `.rs`／`.csv`／`.md`／`.ps1`／`.txt`／`.json` 横断で **0 件**。

**共有ヘルパだけは `<stem>_test_support.rs`＝モジュール名 `test_support`**（`design.md:151`）で接尾辞規則の対象外——ここを揃えようとしないこと。改名は本文証跡に触れない（`RustParse.ps1` は `use` 項目と `mod` 宣言を digest から落とす）ので、是正後も `10=10 / 19=19` exit 0 が再現した（レビュアーが実測で確認）。

### 42.5 §40.5 の申し送りの初適用（単一消費者のヘルパはテーマ側に残す）

ヘルパ 19 のうち **17 を集約**（複製 0）。**レビュアーが 19 件すべての消費者数を独自に数え直し**、集約した 17 件はいずれも 2 テーマ以上から参照されるか共有ヘルパの内部被呼出であって**強制**だったこと、単一消費者の 2 本——`talking_only_snapshot`（群 4 のみ）・`OUT_OF_CANDIDATE_CHOICE_ID`（群 6 のみ）——が正しくテーマ側に残ったこと、そして**バナーがテストから引き離された箇所が 0 件**であることを確認した。§40.5 が「回避可能だった」と記録した轍を踏んでいない。

### 42.6 親の `use` は残置が必須（§32.4 と同型・`tests/` ツリーでの初確認）

子は `use super::{…}` で親の `use` 束縛から外部クレート型と `super::common` の項目を引く。したがって親から `use` を落とせない。**`unused_imports` は 0**（ワークスペース警告の不変で実測）。共有ヘルパは §17.5 に従い**明示 import**で受けた。`test_support` の 14 項目に**行頭** `pub(super)` を付与（内部専用 3 項目は私有のまま）。

### 42.7 本文一致の独立検算（要件 2.4）

| 区分 | 件数 | 内訳 |
|---|---:|---|
| 置換 | **14** | `pub(super)` 付与前後のシグネチャ行のみ |
| 追加 | **85** | `use` 関連 59・`#[cfg(test)]` 7・`#[path]` 7・`mod X;` 7・見出しコメント 4・空行 1 |
| 削除 | **0** | — |
| 字下げの変更 | **0** | 全項目が元から列 0 |

検算 `1,563 − 14 + 14 + 85 = 1,648` = 実測。**初版は「追加 99」と書いて置換 14 行を両列で二重計上しており**（1,662 になって実測と合わない）、レビュー指摘で是正した。§11.4 の盲点は §40.4 と同じく**構造的に不成立**。

### 42.8 検証結果

`cargo test -p areka-kanade` 279+47 passed / 0 failed ／ `--list` 326 = 326（対称差 removed 10 / added 10・対応表適用後は完全一致）／ 対応表 `areka-kanade.csv` 88 → 98 行（先行 88 行バイト不変・`numstat` = `10 0`）／ 8 フラグメント結合 **1,045 行**で全単射 PASS ／ ワークスペース警告 16/7/22/6/16 ／ Cargo ターゲット 2 件のまま ／ `tests/kanade.rs` バイト不変。
...
## 44. 観測ハーネス基盤の責務分割: `crates/areka-kanade/tests/kanade/common/mod.rs`（タスク 8.2・要件 1.7 / 2.2 / 2.4 / 3.1 / 3.2）

- 実施日: 2026-08-09 / 分割前コミット `3b8856c` / **PowerShell（pwsh 7）**
- §39.4 の 13 本のうち **形が唯一違う 1 本**——`:1-1401` が兄弟テストファイル **9 本**から使われる共有ハーネス（項目 55・テスト 0）、`:1402-1657` が唯一のテストモジュール `mod smoke`（テスト 4）。**テーマ分割ではなく本体分割（設計判断 #5 の D1 ファサード再輸出）である**
- 入口 `tests/kanade.rs` および消費側 9 本は **1 バイトも変更していない**（レビュアーが 9 本すべての `git diff` 空を確認）

### 44.1 成果ファイルと行数（すべて 1,000 行以下）

| ファイル | 行 | テスト | 責務 |
|---|---:|---:|---|
| `mod.rs`（ファサード） | **91** | 0 | module doc＋接続宣言 7＋`pub use` 再輸出 35 |
| `common_mock_shiori.rs` | 373 | 0 | mock shiori 3 種 |
| `common_mock_sakura.rs` | 351 | 0 | mock sakura sink 2 種 |
| `common_harness.rs` | 267 | 0 | 駆動ハーネス組み立て 6 種＋`SinklessHarness` |
| `common_smoke.rs` | 266 | **4** | `mod smoke` を丸ごと 1 ファイルへ |
| `common_fixture.rs` | 230 | 0 | fixture 応答表＋`FixtureState` |
| `common_bounded.rs` | 108 | 0 | 期限付き待機／ハング検出 |
| `common_recording.rs` | 97 | 0 | `RecordedCall` 語彙と期待値導出 |

**1,657 → 最大 373 行**・合計 1,783。境界は原本の区画 10 本に一致させ、束ねたのは 2 箇所のみ（mock shiori 3 区画・mock sakura 2 区画。割ると共有が増えるだけ）。**新設した境界は 0。**

### 44.2 測定器の再改修: `-AllowNestedTestModules`（ガード① の明示オプトイン）

§41.5(a) が「タスク 8.2 は明示的なオプトインで `-WholeFile` を使ってよい」と裁定した当のスイッチである。**解除するのはガード① だけ**——レビュアーが逐行で確認し、ガード②（`-ModuleName` 併用）はガード① より**前**で無条件に評価され、ガード③（項目 0 件）も無条件であることを実行で裏づけた（いずれも exit 2）。

**非退行は 3 本立てで示した**: `task_7_1_body_identity.csv` の 49 行を ① HEAD 版 ② 改修版 ③ 改修版＋スイッチ の 3 通りで実行し、**49/49 が exit・verdict とも一致、しかも 3 つの結果集合が stdout+stderr+exit までバイト同一**。スイッチは `-WholeFile` を伴わなければ完全に不活性である。`Test-VerificationTools.ps1` は 23/24（`TOKENIZER-EQUIV` のみ赤）。

**解除が広すぎないか**もレビュアーが検証した。`src/` の本番ファイル（`areka-emo-compose/src/fold.rs @7599c90`）へ当てると `helper item 26=26`——通常経路の `17=17` に対し**本番項目 9 件を比較へ足す**、すなわち**判定が厳しくなる方向**であり、本番ファサードを移設先から外せば `MISMATCH: 9 件` で赤くなる。**誤用は赤に落ちるのであって偽の緑にはならない。** 残る性質として、docstring の前提（「`tests/` 配下・全体がテストコード」）は呼び出し側の主張であって機械的な検査ではない——これは受容する。

解除経路の失敗能力も個別確認: 移設先を落とす → exit 1（`TEST-MISSING` 4 件／別の 1 本で `ITEM-MISSING` 8 件）、テスト本文のリテラル改変 → exit 1 `TEST-BODY`、ヘルパのリテラル改変 → exit 1、不正パス → exit 2。

### 44.3 `mod smoke` は名前を保存した——対応表 0 行

design.md:134/:239 の `<テーマ>_tests` は**新設するテーマモジュール**の規則であり、本件の `mod smoke` は**元から名前を持つ単一のテストモジュール**なので設計判断 #2（1 テストモジュール＝1 テストファイル・完全修飾名不変）が適用される。改名すれば 4 本の完全修飾名を無用に壊していた。

結果、**対応表への追記は 0 行**、`cargo test -- --list` の対称差も **0**（レビュアーが `before_default.txt:605-608` / `after_default.txt:605-608` と突合して確認）。8 フラグメント結合は 1,063 行のまま全単射 PASS。

### 44.4 ⚠ 命名は「2 つの別々の規則」である（8.5／8.6 への申し送り）

初稿は「`#[test]` を含むファイルはテストモジュール命名を保つ／得る」と 1 つの規則のように書いたが、**これは 2 つの異なる規則を混ぜている**（レビュー指摘）。分けて書く:

1. **既存のテストモジュールを丸ごと移すとき**——名前を**保存**する（設計判断 #2）。完全修飾名が変わらないので対応表も 0 行。本件の `mod smoke` がこれ。
2. **`mod tests` をテーマへ解体するとき**——新設するモジュールは **`<テーマ>_tests`** を名乗る（design.md:134/:239・§42.4 の差し戻し）。**タスク 8.5／8.6 はこちらである。**

「保つ／得る」と一語で書くと、テーマ分割のときに元のモジュール名を残してよいと読めてしまう。**そう読んではならない。**

3. **`#[test]` を 1 つも含まないヘルパ／ハーネス専用ファイル**——設計判断 #5 の系統として**責務名そのまま**（`common_<責務>.rs` / `mod <責務>`）。`_tests` は内容に反する。タスク 6.1 の `follow.rs` → `mod anchor; mod drag_follow; mod visibility; mod window_move; mod work_area;` と同じ語彙である。判定式は単純——**そのファイルに `#[test]` があるか**。

### 44.5 ファサード再輸出の副作用——再輸出は `unused_imports` の対象になる

分割前は公開項目が `dead_code` 許容で通っていたが、ファサードへ移すと**再輸出は別の lint に掛かる**（§31.5 と同型だがこちらは lib target を持つクレートである）。実測で 6 名前が赤くなり、うち **5 つは「子同士も公開項目はファサード経由で引く」規則を採ることで消えた**。残る `run_bounded`（消費者ゼロ）だけを局所許容した——モジュール全体に置くと子 7 本の未使用 import を隠すからである。

レビュアーが `cargo rustc -p areka-kanade --test kanade -- --force-warn unused_imports` を実行し、**未使用は `bounded::run_bounded`（`mod.rs:75:9`）ちょうど 1 件**、属性は `:74` の当該 1 文に限定、コメントもその 1 名だけを名指ししている（§31.5 が咎めた「無実の隣人まで名指しする」誤りを回避）ことを確認した。

### 44.6 本文一致の独立検算（要件 2.4）

| 区分 | 件数 | 内訳 |
|---|---:|---|
| 置換 | **5** | 可視性付与前後のシグネチャ行のみ |
| 追加 | **133** | `use` 系 48・module doc 29・コメント 18・空行 17・パス属性 7・`mod X;` 7・テスト条件属性 6・許容指示 1 |
| 削除 | **7** | `mod smoke` の開き閉じ 2・`use` 3 文とその継続行 2 |
| **字下げの変更** | **216** | smoke ブロックの一律 4 スペース de-indent のみ |

検算 `1,657 − 7 + 133 = 1,783` ＝ 実測。レビュアーが独自スキャナで再導出し同値を得た（生の削除 228／追加 354 から字下げのみ 216 を差し引いた残差が 12／138、うち 5 対が置換）。

**本件は群 8 で初めて実際の de-indent が起きたタスク**であり、§40.4／§42.7／§43.5 のように「構造的に不成立」では §11.4 の盲点を閉じられない。**実際に走査した**——較正として `window_pos_tests.rs` の「継続行 5 件 [382,429,614,690,691]／盲点 1 件 [691]」を逐語再現したうえで、原本は継続行 **1 件（`:1139`）・盲点 0 件**、移設後は `common_bounded.rs:103` のみ・盲点 0 件。レビュアーはさらに原本 `:1134-1142` と `common_bounded.rs:98-106` をバイト単位で突合し、当該継続行が継続記号つきであること、および **216 行の de-indent はすべて `mod smoke`（文字列継続を 1 件も含まない）の中で起きている**ことを確認した。

### 44.7 消費側の到達性（本タスクの中心的な安全不変条件）

`super::common::…` で引く兄弟テストファイルは **9 本**（`boot_test` `choice_test` `close_test` `failure_test` `full_run_test` `mouse_test` `prefetch_test` `real_helper_test` `steady_test`）。すべて `git diff` 空。レビュアーが 9 本の import する **29 名前**を抽出し、ファサードの **35 再輸出**（＝原本 root の公開項目 35 と 1 対 1）と突合して**取りこぼし 0** を確認した。

> **訂正**: 初稿は `mod.rs:32` のコメントに「兄弟テストファイル **8 本**」と書いていた。**実測 9 本が正**（レビュー指摘・9 本すべての `use super::common::` を file:line で確認）。本タスクの中心的な安全不変条件そのものの数え違いであり、放置すれば本節へそのまま転記されていた。是正後に本文一致（`4=4 / 54=54` exit 0）とクレート緑を再実行して確認済み——当該コメントは接続宣言の直前にあり `RustParse.ps1` が digest から落とすため、証跡の取り直しは要らない。

### 44.8 検証結果

`cargo test -p areka-kanade` 279+47 passed / 0 failed ／ `cargo test --workspace` exit 0 ／ `--list` 326 = 326（**対称差 0**）／ 対応表追記 **0 行**・結合 1,063 行で全単射 PASS ／ ワークスペース警告 16/7/22/6/16（DIAG の多重集合も `task_7_2_warning_comparison.txt` と同一）／ Cargo ターゲット 2 件のまま ／ 8.3・8.4 の成果物は無変更。
...
記録のみ:
1. **`TOKENIZER-EQUIV` の不一致数が 49 → 50 になる**。50 本目は本ファイルで、`scan_raw.csv:116` の基準 `smoke:1402-1657(256)` がインラインに存在しなくなったためである。§33.6 の恒久赤の性質どおりで**退行ではない**が、明記しないと後の読み手が退行と誤読する。
2. **`cargo fmt --check` の増分が、群 8 で初めて `mod` 宣言の並び以外を含む**——`common_smoke.rs:141, :158, :204` は 4 スペース de-indent に伴う rustfmt の再折り返しである。直せば要件 2.4 に反するので放置が正解だが、§40.7(i)／§42.9(i)／§43.8 の「並びだけ」とは**種類が違う**。
3. intra-doc リンクの破断（§32.10 と同一判断・診断 0 件）。
4. **`test-cage-determinism`（W6.9）の待機機構アンカーが 3 ファイルへ分散した**（本文バイト不変・§40.2 と同型の rebase）: 旧 `common/mod.rs:1052-1144` → `common_bounded.rs:14-105`、旧 `:584-712`（`ShioriGate`）→ `common_mock_shiori.rs`、旧 `:873-1050`（`SakuraGate`）→ `common_mock_sakura.rs`。

## 45. シェル辞書デコードテストのテーマ分割: `crates/areka-parsers/src/shell/decode_tests.rs`（タスク 8.5・要件 1.4 / 1.7 / 2.2 / 2.4 / 2.9 / 3.1 / 3.2）

- 実施日: 2026-08-10 / 分割前コミット `7824ba7` / **PowerShell（pwsh 7）**
- 既分離ファイル（`shell/mod.rs:28-29` が `#[cfg(test)]` ＋ `mod decode_tests;` で**ゲート済み宣言**している）ゆえ移設ではなく**分割**。要件 1.4 は移設対象から除くだけで分割対象からは除かない（§39.2 の明確化）
- 形は群 8 で最も素直——`#[cfg(test)] mod` ブロック 0・入れ子 0・トップレベルは 59 個の `#[test] fn` **のみ**（`struct`／`const`／`impl`／`macro` いずれも 0）。ヘルパ項目 0 ゆえ `test_support` は作っていない
- `shell/mod.rs`・同名の `charset/decode_tests.rs`・`sakura/decode_tests.rs` は **1 バイトも変更していない**（レビュアーが `git diff --name-only 7824ba7` ＝ ハブ 1 本のみで確認）

### 45.1 成果ファイルと行数（すべて 1,000 行以下・最大 346）

| ファイル | 行 | テスト | テーマ（元ファイルの区画バナー） |
|---|---:|---:|---|
| `decode_tests.rs`（ハブ） | **49** | 0 | module doc＋`use` 3 文＋接続宣言 9 組 |
| `decode_tests_lenient_input_tests.rs` | 346 | 9 | `:788` subset 外・不正入力の振り分け |
| `decode_tests_element_collision_tests.rs` | 191 | 8 | `:153` element overlay ＋ collision 矩形 |
| `decode_tests_append_tests.rs` | 187 | 7 | `:504` surface.append ターゲット捕捉 |
| `decode_tests_animation_tests.rs` | 175 | 9 | `:335` animationN 集約 |
| `decode_tests_transcription_gap_tests.rs` | 142 | 4 | `:1125` 転記ギャップ 4 点 |
| `decode_tests_skeleton_tests.rs` | 140 | 8 | `:19-151` decode スケルトンとディスパッチ |
| `decode_tests_alias_tests.rs` | 111 | 7 | `:683` kero.surface.alias 写像 |
| `decode_tests_method_matrix_tests.rs` | 91 | 4 | `:1312` method 忠実転記の完全マトリクス |
| `decode_tests_pattern_method_tests.rs` | 57 | 3 | `:1261` pattern method 忠実転記 |

**1,395 → 最大 346 行**・合計 1,489・テスト 59。**新設した境界は 0**——元ファイルが持っていた区画バナー 8 本（`:153, :335, :504, :683, :788, :1125, :1261, :1312`）＋ 先頭区画で 9 テーマ。レビュアーがバナー位置を独立に列挙し、9 区画がバナー区切りの連続スライスに**厳密一致**すること、バナーを跨いだテストが 1 本も無いことを確認した。

命名は **§44.4 の規則 2**（テーマへの解体）——9 本すべて `<テーマ>_tests` ／ ファイル名 `decode_tests_<テーマ>_tests.rs`。§42.4 で差し戻した接尾辞欠落は 0 件。接続は設計判断 #1／#13 のとおり `#[cfg(test)]` ＋ `#[path]` ＋ `mod <テーマ>_tests;` の 3 行組（`decode_tests.rs:23-49`）。子はハブの `use` 束縛を `use super::{…}` で引く（§17.5／§32.4／§42.6 と同型）——`super::super::` も `crate::` 起点も 0 件で、ハブの `use` 3 文がすべて生き `unused_imports` は 0。

### 45.2 本文一致は道具の緑より強い証跡で示せた（要件 2.4）

比較器: `Compare-RelocatedTests.ps1 -Commit 7824ba7 -WholeFile -OriginalPath … -RelocatedPath <9 本> -Detail` → `MATCH: test fn 59=59 / helper item 0=0 / mod block whole-file / files 9`・**exit 0**。§41.4 が予告した digest（59/0）を実測で再現した。

**較正（§42.2）は 3 通りで実施**——`lenient_input` を落とす → `MISMATCH: 9 件` **exit 1**・`[TEST-MISSING]` 9 件を名指し／`method_matrix` を落とす → `MISMATCH: 4 件` exit 1／木の外の複製でリテラル 1 個を `2106`→`2107` に改変 → `[TEST-BODY]` 3 件（前後の行を表示）。**名前だけでなく内容も弁別している**ことをこの入力で示した。

さらにレビュアーが**道具を介さない再導出**を行った: `git show 7824ba7:…` の 9 区間 `[19-151] [153-333] [335-502] [504-681] [683-786] [788-1123] [1125-1259] [1261-1310] [1312-1395]` を切り出し、各テーマファイルのヘッダ以降と `cmp` → **9/9 バイト一致**（作業ツリーの CRLF と blob の LF 差は `core.autocrlf=true` に由来するリポジトリ全域の性質であり欠陥ではない・正規化して比較）。区間の合計 1,369 ＋ バナー区切りの空行 8 ＝ 1,377 が `git diff --stat` の削除行数と一致し、ハブ先頭 18 行が無傷であることも裏づけた。

### 45.3 ⚠ §11.4 の「リポジトリ全域で 1 箇所のみ」は**必須対象 49 本の範囲での話**だった

`notes.md:590` は「**必須対象 49 本を全走査した結果**、第 1 の盲点に該当する行は**リポジトリ全域で 1 箇所のみ**」と書いている。この 2 つの範囲は一致しない——本ファイルは `excluded_inventory.csv:30` に載る要件 1.4 の除外ファイルであり、**その走査に一度も入っていなかった**。群 8 が要件 1.4 の除外を分割対象へ引き戻した以上、この文は範囲を限って読まねばならない。

本ファイルを独自走査した結果（較正として `window_pos_tests.rs` の「継続行 5 件 `[382,429,614,690,691]`／盲点 1 件 `[691]`」を逐語再現したうえで）:

- **複数行リテラル 20 個・その内容行 152 行**（閉じの `";` 行を含めると 172 行）
- **そのうち行頭空白がリテラルの中身になっている行＝盲点は 0 件**。本ファイルの複数行入力はすべて内容行を列 0 から書く形式である

**語彙を取り違えないこと**——「盲点 152 件」ではなく「**継続行 152・盲点 0 件**」である（§40.4／§44.6 と同じ語彙）。実装報告は初稿でこれを盲点と書いていた（レビュー指摘）。加えて本タスクは**字下げの変更が 0 行**（1,369 本文行すべてバイト一致）なので、盲点があっても構造的に無害——列 0 の行からは削るべき空白が無い。

**タスク 8.6（`golden_tests.rs`）・8.7（`runtime_test.rs`）は要件 1.4 の同類ゆえ、§11.4 の 1 件に頼らず自分のファイルを走査すること。**

### 45.4 逆向き導出の注記（steering `structure.md:164` へ 8.13 で折り込む）

§35.3 が定めた最長 stem 優先の規範文は「同一ディレクトリに実在する**接続宣言を持ちうるファイル**」だが、`structure.md:164` はこれを `src/` について「本番ファイル」と言い換えている。`decode_tests.rs` は本番ファイルではないため、**言い換えのほうを字句どおり読むと** `decode_tests_alias_tests.rs` が「`decode.rs` ＋ `tests_alias_tests`」と読めてしまう。規範文（§35.3 がまさにこの形のために広げた表現）で読めば解決は一意である。

レビュアーが `crates/` の**同一ディレクトリ 208 件の `#[path]` 宣言**すべてに最長 stem 規則を当て、本タスクの 9 本が `decode_tests` へ一意解決すること、**新たな誤解決 0 件**（既存 20 件は `mod.rs` stem と `shiori-host-32` の歴史的形式で本タスク無関係）を確認した。前向きの衝突禁止も満たす——テーマを与えたのは**長いほうの stem** で、これは安全な向きである。

`src/` において要件 1.4 の分離済みテストファイルがテーマ子を持つのは**本件が初例**。8.6 の `golden_tests.rs` は同一ディレクトリに `golden.rs` が無いので該当しない。

### 45.5 行数会計（§42.7 の多重集合規約・置換は 1 回だけ計上）
...
検算 `1,395 − 0 + 94 = 1,489` ＝ 成果物 10 本の実測合計。**置換 0** はヘルパ項目が 0 で `pub(super)` 付与が 1 件も発生しないため。追加 94 の内訳は子 9 本のヘッダ（module doc 36・`use` 13・空行 18）＋ ハブ追記（見出しコメント 4・`#[cfg(test)]` 9・`#[path]` 9・`mod X;` 9・空行 1）。

> **訂正**: 実装報告は空行の寄与を「99 の生追加から −5」と書いていたが、**正しくは空行成分が純増 +10**（追加したヘッダ空行 18 − 落ちたバナー区切り 8）である（レビュー指摘・独立な多重集合差分で再導出）。合計 94 は変わらない。

### 45.6 検証結果

`cargo test -p areka-parsers` 359 passed / 0 failed ／ `cargo test --workspace --no-fail-fast` exit 0・**4,757 passed / 0 failed / 33 ignored**（＝ 4,790・`before_default.txt` と一致）／ `--list` は before 359 / after 359 で、対称差 59 removed / 59 added は**対応表 59 行を適用して 0**（他 300 本は無変更）／ ワークスペース警告 **16/7/22/6/16**（delta 0・DIAG 16 行の多重集合も `before_build_warnings.txt` と差分 0）／ `cargo rustc -p areka-parsers --lib --profile test -- --force-warn unused_imports` 0 件。

対応表 `verification/mapping/areka-parsers.csv` は**新規作成**（`README.txt` の 3 列 `old_fqn,new_fqn,reason`・`reason=theme_split`・UTF-8 BOM 無し）。**59 行**、全行の**末尾セグメント不変**。`Test-MappingBijection.ps1` PASS（59）／ディレクトリ結合 **1,122 行・9 フラグメント**で全単射 PASS（1,063 + 59）。レビュアーが 59 行を実配置から独立に再導出し**不一致 0**、かつ `git status --porcelain` に現れる（`.gitignore` されていない）ことを確認した——§42 の訂正が記録した 8.3 の取りこぼしを繰り返していない。

**1,000 行超のファイルは 8 本 → 7 本**（`crates/` 全域計測）。残る 7 本はタスク 8.6〜8.12 の対象と**過不足なく一致**する。

### 45.7 登記（要件 5.2）

**修正を要する壊れたテスト・不正なテスト・テスト間の状態汚染は 1 件も発見しなかった。新規送付所見 0 件。** 本ファイルは待機・スピン・時刻注入を 1 件も含まない。

記録のみ:
1. **`cargo fmt --check` の増分は 4 ハンク・すべて `mod` 宣言の並び**（rustfmt は辞書順を望むが、採った順は元ファイルの区画順＝タスク 4.1→4.2→4.3→4.4→4.5→4.6→1.4→1.2→1.3 の意味順）。§40.7(i)／§42.9(i)／§43.8 と同一判断で、§44.9(2) のような「並び以外」は含まない。子 9 本の rustfmt ハンクは **9 件で、分割前の元ファイル 9 件と同数**——移動した本文が持ち込んだ既存分だけで、**ヘッダが増やしたものは 0**（レビュアーが `rustfmt --check --edition 2024` 単体で実測）。
2. intra-doc リンクの破断は発生しない（移動した本文に intra-doc リンク 0）。
3. §45.4 の逆向き導出の注記は steering の言い換えの問題であり、**タスク 8.13 で `structure.md:164` へ 1 行折り込む**。

## 46. 合成ゴールデンテストのテーマ分割: `crates/areka-emo-compose/src/golden_tests.rs`（タスク 8.6・要件 1.4 / 1.7 / 2.2 / 2.4 / 2.9 / 3.1 / 3.2）

- 実施日: 2026-08-10 / 分割前コミット `84e73c5` / **PowerShell（pwsh 7）**
- 既分離ファイル（`lib.rs:169-170` の素の `#[cfg(test)] mod golden_tests;`）ゆえ移設ではなく**分割**。`lib.rs` は 1 バイトも変更していない
- 8.5 との違いは**ヘルパの濃さ**——`#[test]` 17 に対しヘルパ項目 **32**（自由関数・`struct BindPart`・定数 9 本・中途の `use` 2 文）。共有判定を目視でなく機械で行う必要があった

### 46.1 成果ファイルと行数（すべて 1,000 行以下・最大 294）

| ファイル | 行 | テスト | 由来（元ファイルの区画） |
|---|---:|---:|---|
| `golden_tests.rs`（ハブ） | **53** | 0 | 原本 `:1-30` をバイト不変で保持＋接続宣言 7 組 |
| `golden_tests_determinism_budget_tests.rs` | 294 | 5 | `:703-988` 決定性・確保予算 |
| `golden_tests_frame_extent_tests.rs` | 227 | 2 | `:1137-1356` フレーム範囲 |
| `golden_tests_trim_equivalence_tests.rs` | 217 | 2 | `:493-701` トリム等価 |
| `golden_tests_surface1000_bind_tests.rs` | 178 | 4 | `:319-324` ＋ `:330-491` bind 合成 |
| `golden_tests_surface0_base_tests.rs` | 167 | 2 | `:32-221` 基本合成 |
| `golden_tests_blink_static_tests.rs` | 156 | 2 | `:990-1135` 瞬き bind の静的非活性 |
| `golden_tests_test_support.rs` | 144 | 0 | 共有ヘルパ 10 項目 |

**1,356 → 最大 294 行**・合計 1,436・テスト 17。**新設した境界は 0**——`// ====` 行は `:223,238 / :493,517 / :703,725 / :990,1008 / :1137,1161` の 10 本＝**箱 5 つ**で、これに先頭区画を足した 6 区画がそのままテーマになる。レビュアーがフラグメント境界（`222 / 318-319 / 324-325 / 329 / 491-492 / 701-702 / 988-989 / 1135-1136`）を独立に測り、**すべて項目間の空行上にあり区画を跨いだテストは 0** であることを確認した。

命名は **§44.4 規則 2**——テーマ 6 本すべて `<テーマ>_tests` ／ 共有ヘルパのみ `test_support`（design.md:151・接尾辞の対象外）。このディレクトリは `plan_test_support.rs`・`scale_test_support.rs`・`composer_tests.rs` 等が同居してヘルパ名が密なので §35 の逆向き導出を全域で当てた——**`crates/` の同一ディレクトリ `#[path]` 宣言 217 件へ最長 stem 規則を機械適用し誤解決 0**（既存例外 22 件は `mod.rs` stem と `shiori-host-32` の歴史的形式で本件無関係）。同ディレクトリに `golden.rs` は無いので §45.4 の曖昧さは生じない。

### 46.2 共有ヘルパの判定は機械的に行った（§40.5／§42.5）

`test_support` へ集約したのは **10 項目**で、いずれも 2 テーマ以上から参照されるか共有ヘルパの内部被呼出。**単一消費者の 22 項目はテーマ側に残した**（`ORIG_W`〜`CORE_R` の 9 定数と `margined_core_image` はトリム等価テーマへ、`png_size`／`build_atlas_real_sizes`／`recorded_frames`／`assert_frames_fit_base` はフレーム範囲テーマへ、等）。10＋22＝32＝ヘルパ項目数。

**可視性付与は 10 件**（要件 2.4 が許す唯一の署名編集）。うち 1 件は**構造体フィールド単位**（`BindPart.anim_id`——`p.anim_id` を 2 テーマが読むため）。レビュアーが正規化器の挙動を実測で確定した: `ConvertTo-NormalizedLine`（`RustParse.ps1:491-495`）は `Trim()` 後に `^pub(\s*\([^)]*\))?\s+` を除くので**独立行にある名前付きフィールド**の修飾は吸収される（`pub(super) anim_id` → `pub anim_id` は **exit 0＝盲**）。ただし `u32`→`u64` を伴えば **exit 1** で捕まる——**吸収されるのは 2.4 が明示的に許す可視性修飾だけで、型・名前・本文は捕捉する**。§22.2 のタプル構造体問題（`Name(pub(super) T)` が**行内**に来る形）は本件に当たらない。

### 46.3 区画バナー 1 本が `test_support` へ同伴した——これは強制であり回避策が無い

`// ==== task 8.2: surface1000 … ====`（原本 `:223-238`）は、比較器の先行コメント塊規則（`RustParse.ps1:314-331`）により `solid_opaque`（原本 `:246`）の項目本文 `:223-253` の一部である。`solid_opaque` の消費者は `build_atlas_for_surface1000`（原本 `:299`）と `build_atlas_for_blink_cage`（`:1018,:1023`＝瞬きテーマ）の 2 つで、**前者自身が bind 合成・決定性の両テーマから呼ばれる**ため design.md:151 により `test_support` 行きが確定する。したがって `solid_opaque` も 2 モジュールから参照される共有ヘルパである。

複製すれば `ITEM-EXTRA`、バナーだけ切り離せば `ITEM-MISSING`（`test_support` 除外の較正出力がこの行範囲を逐語で示した）。**§40.5 の「強制」側であって「回避可能だった」側ではない**——レビュアーが独立に再導出して確認した。結果 `surface1000_bind_tests` は自分の区画バナーを持たないので、module doc にその旨を明記してある。

なお `blink_static_tests` には **原本のその位置に無かった `use crate::plan::build_plan;` を 1 行新設した**。原本の module-root `use`（`:727`）は決定性テーマの区画に属しており、瞬きテーマは `:1051,:1057` でそれに依存していたためである。`RustParse.ps1:536` が `use` 項目を digest から落とすので本文証跡は汚染されない。中途の `use`（`:727` と `:1163`）は**バナー直後の位置ごと**それぞれのテーマへ同伴させた（§28.5 の罠を構造的に回避）。

### 46.4 本文一致（要件 2.4）と較正

比較器: `-Commit 84e73c5 -WholeFile -RelocatedPath <7 本> -Detail` → `MATCH: test fn 17=17 / helper item 32=32 / mod block whole-file / files 7`・**exit 0**。§41.4 が予告した digest（17/32）を実測再現。

**較正（§42.2）は 3 通り**——決定性テーマを落とす → `MISMATCH: 7 件` **exit 1**（`[TEST-MISSING]` 5＋`[ITEM-MISSING]` 2）／`test_support` を落とす → `MISMATCH: 10 件` **exit 1**（行範囲つきで名指し）／木の外の複製でリテラルを改変 → `[TEST-BODY]` **exit 1**。レビュアーが 3 通りとも再実行して再現した。

**道具を介さない再導出**（レビュアー実測・実装報告より再現性の高い形）: 原本 1,356 行と成果物 8 本を最長一致で突合すると **11 個の連続フラグメント**に分解でき、**原本 1,354 行を重複 0 で被覆**。未被覆は `:222` と `:329` の**空行 2 行のみ**。成果物側は **バイト一致 1,344 ／ 可視性のみ差分 10 ／ 真に新規 82**。**字下げの変更 0 行**（行頭空白を含む生バイトで比較）。

> **訂正**: 実装報告の「blob 被覆 1,313・未被覆 43（すべて空行）・バイト一致 1,303」および「空行 43-in / 43-out が相殺」は、報告者独自のフラグメント分解に依存した中間値で第三者が再現できない（レビュー指摘）。**再現するのは上記の 11 フラグメント形である。** 結論（重複 0・未被覆はすべて空行・置換 10／追加 80／削除 0・空行純増 +9）は変わらない。同じく「T1 の出所は `:223-491`」も不正確で、正しくは `:319-324` ＋ `:330-491`（§46.3 の帰結）。

### 46.5 行数会計（§42.7 の多重集合規約・置換は 1 回だけ計上）
...
検算 `1,356 − 0 + 80 = 1,436` ＝ 成果物 8 本の実測合計。**空行は 125 → 134 の純増 +9**（最大フラグメント分解では「ヘッダ空行 11 追加 − 原本の空行 2 脱落」）。追加 80 の非空行 71 は module doc 15・`use` 34・見出しコメント 1・接続宣言 21（7×3）。

### 46.6 検証結果

`cargo test -p areka-emo-compose` 216 passed / 0 failed ／ `cargo test --workspace --no-fail-fast` exit 0・**4,757 passed / 0 failed / 33 ignored**（＝4,790）／ 警告 **16/7/22/6/16**（delta 0・DIAG 16 行の多重集合差分 0）／ `cargo rustc -p areka-emo-compose --lib --profile test -- --force-warn unused_imports` **0 件**（レビュアーは mtime を更新して実リコンパイルさせたうえで再確認）。

`--list` は before 216 / after 216・対称差 17/17 が**対応表適用で 0**。対応表 `mapping/areka-emo-compose.csv` は **17 行追記**（78 → 95 データ行・`numstat` が `17 0` で先行 78 行のバイト不変を裏づけ）。フラグメント全単射 PASS（95）／ディレクトリ結合 **1,139 行・9 フラグメント** PASS（1,122 + 17）。

**1,000 行超のファイルは 7 本 → 6 本**。残る 6 本はタスク 8.7〜8.12 の対象と過不足なく一致する。

### 46.7 登記（要件 5.2）

**修正を要する壊れたテスト・不正なテスト・テスト間の状態汚染は 1 件も発見しなかった。新規送付所見 0 件。** 本ファイルは待機・スピン・時刻注入を含まない。
...
## 48. プロセス内 SHIORI のテストモジュール移設: `crates/areka-ghost/src/shiori_inproc.rs`（タスク 8.8・要件 1.3 / 1.5 / 1.7 / 2.2 / 2.4 / 3.1 / 3.2）

- 実施日: 2026-08-10 / 移設前コミット `f90fe8f` / **PowerShell（pwsh 7）**
- **群 8 で唯一の「素直な移設」**——8.1・8.3〜8.7 はテーマ分割、8.2 と 8.9〜8.12 は本体分割だが、本件は §13／§14 と同型の 1 モジュール＝1 ファイル移設である。したがって **§44.4 は規則 2 ではなく規則 1**（名前を保存・完全修飾名不変・対応表 0 行）が適用される
- 要件 1.1 の必須対象ではない（テストコード 446 行 < 500）。**要件 1.5 の併せ移設**として実施し、要件 1.7 の 1,000 行制約を本番本体の側で満たした

### 48.1 成果ファイルと行数

| ファイル | 行 | テスト | モジュール |
|---|---:|---:|---|
| `shiori_inproc.rs`（本番本体） | **581** | 0 | 接続宣言 3 組 |
| `shiori_inproc_adapter_tests.rs` | 260 | 11 | `adapter_tests`（原本 `:758-1017`） |
| `shiori_inproc_tests.rs` | 107 | 4 | `tests`（原本 `:573-679`） |
| `shiori_inproc_host_tests.rs` | 70 | 4 | `host_tests`（原本 `:684-753`） |

**1,018 → 581**・合計 1,018（純移設ゆえ総行不変）・テスト 19。

**`581 = 1,018 − 437`**（出て行くのは本体行だけ）。タスク本文の概算 572 との差 **9 = 残置した `#[cfg(test)]` 3 行 ＋ 接続宣言 6 行**（`#[path]` 3 ＋ `mod X;` 3）。乖離ではない。

紛らわしい 3 つの数の関係もレビュアーが突合した——**446** = 3 ブロック（`#[cfg(test)]` 〜 `}`）＝110+73+263 ／ **448** = `:571-1018` の連続領域（446 ＋ 区切り空行 2）／ **443** = `git diff --numstat` の削除数（本体 437 ＋ `mod X {` 3 ＋ `}` 3。`#[cfg(test)]` は context として残る）。追加は 6。三者は矛盾しない。

命名の逆引きは一意——`crates/areka-ghost/src/` を全走査し、3 本それぞれについて前置一致する同一ディレクトリの stem は **`shiori_inproc` ただ 1 つ**（`shiori_wiring` は `shiori_w` で分岐するので当たらない）。前向き衝突 0。同一ディレクトリ `#[path]` 宣言は全域 **222 → 225** で新たな例外 0。

### 48.2 本文一致（要件 2.4）——道具と道具なしの二本立て

道具は**モジュールごとに 1 回ずつ**回した（`-WholeFile` は使えない。本ファイルは実ブロックを持つのでガード① が発火し **exit 2＝引数不正**。実装者が意図的に踏んで確認した）。

| 実行 | 結果 | exit |
|---|---|---:|
| `-ModuleName tests` | `MATCH: test fn 4=4 / helper item 0=0 / mod block 1 / files 1` | **0** |
| `-ModuleName host_tests` | 同上 `4=4 / 0=0` | **0** |
| `-ModuleName adapter_tests` | `MATCH: test fn 11=11 / helper item 4=4 / mod block 1 / files 1` | **0** |
| 併合（`-ModuleName` 無し・3 本指定） | `MATCH: test fn 19=19 / helper item 4=4 / mod block 3 / files 3` | **0** |

**較正（§42.2）**: `adapter_tests` の移設先を落とす → `MISMATCH: 15 件` **exit 1**（`[TEST-MISSING]` 11 ＋ `[ITEM-MISSING]` 4）／木の外の複製で `400`→`401` → `[TEST-BODY]` **exit 1**（before/after 提示）。レビュアーが両方とも再現した。

**道具を介さない再導出**（再現規則を逐語で書く）: 原本の本体 3 区間 `:573-679 ∥ :684-753 ∥ :758-1017`（計 437 行）の各行に「**行頭が 4 スペースならちょうど 1 回だけ剥ぐ**（空行は触らない）」を当てると、3 つの子ファイルが**行数・内容とも 0 不一致で再現**する。断片 **1 個**・被覆 437・重複 0・未被覆 0・**可視性のみ差分 0・真に新規 0**。本番 `:1-570` は base とバイト一致。

8.6（11 断片）・8.7（20 断片）より綺麗なのは疑わしさではなく形の違いである——**可視性付与も `use` 追加も 1 件も無い純移設**だから。子の `use` は原本にあった 5 文のみ、`pub(super)` は 0 件。各モジュールの `use super::*;` が `#[path]` 兄弟経由でも同一に解決するため要件 2.8 の手当てが 1 件も要らなかった（§13／§14.1 と同じ）。

de-indent 規則の取りこぼしも検算済み——**本体 437 行のうち「空でないのに 4 スペースで始まらない行」は 0 行**。字下げの変更 **386**（437 − 空行 51）。

### 48.3 §11.4 の盲点——実際の de-indent が起きた 2 例目、5 行を個別に裏取りした

群 8 で実 de-indent が起きたのは §44（タスク 8.2）以来 2 例目である。8.5〜8.7 のように「元から列 0 だから構造的に不成立」では閉じられない。

較正として `window_pos_tests.rs` の「継続行 5 件 `[382,429,614,690,691]`／盲点 1 件 `[691]`」を逐語再現したうえで走査:

| 対象 | 継続行 | 盲点 |
|---|---:|---:|
| 移設前 `shiori_inproc.rs @f90fe8f` | **5**（`:661,662,663,890,891`） | **0** |
| 移設後 4 本合計 | **5**（`_tests.rs:89,90,91` ／ `_adapter_tests.rs:133,134`） | **0** |

5 行すべてが移設本体の内側にあるため、レビュアーが**生バイトで 1 行ずつ**確認した——`:660` が `\n\` 終端 → `:661` が継続行、`:661` が `\` 終端 → `:662`、`:662` が `\` 終端 → `:663`、`:663` は `",` で閉じる。`:889` が `\n\` 終端 → `:890`、`:890` が `\` 終端 → `:891`、`:891` は `",` で閉じる。**直前行が `\` 終端である以上、Rust の文字列継続エスケープが改行と後続行の行頭空白をすべて捨てる**ので、この 13 個の空白はリテラルの中身ではない。行頭空白は 5 行とも 13 → 9 になったが**リテラルの実行時値は不変**。

**盲点 0 ゆえ手で守るべき行は 1 行も無かった。** 本文同一性の証明は閉じている。

### 48.4 行数会計（§42.7 の多重集合規約）
...
| 追加 | **6**（`#[path]` 3 ＋ `mod X;` 3） |
| 削除 | **6**（`mod X {` 3 ＋ 閉じ `}` 3） |
| **字下げの変更** | **386**（追加でも削除でもない・別建て） |

検算 `1,018 − 6 + 6 = 1,018` ＝ 成果物 4 本の実測合計（581+107+70+260）。**空行成分は 88 → 88 の ±0**（追加・削除いずれの行も空行でない）。

### 48.5 検証結果

`cargo test -p areka-ghost` **147 passed / 0 failed**（lib 107 ＋ 統合 40 ＋ doctest 0）／ `cargo test --workspace --no-fail-fast` exit 0・**85 ユニット・4,757 passed / 0 failed / 33 ignored**（＝4,790）。i686 成果物は健在（§9.3 とバイト数一致）・`cargo clean` は実行していない。

**テスト名は対応表を当てずに一致した**——`--list` は前後とも **147**。さらにレビュアーが spec 着手前スナップショット `verification/before_default.txt` の `shiori_inproc::` **19 行**と現在の 19 行を直接 `diff` して**差分 0**。モジュール名保存（§44.4 規則 1・設計判断 #2）が完全修飾名レベルで裏づけられた。**対応表への追記 0 行**・`git status --porcelain .kiro` 0 行・結合再検証 **1,163 行 / 10 フラグメント** PASS。

`cargo rustc -p areka-ghost --profile test --lib -- --force-warn unused_imports` **0 件**。`areka-ghost` は lib target を持つので §31.5 の bin 論法は使えないが、**再輸出も import 追加も 0 件の純移設**なので新規 lint 面が構造的に生じない。`rustfmt --check` は base 単体 13 ハンク／移設後（`#[path]` 追従で 4 ファイル横断）13 ハンクで **delta 0**。4 ファイルを個別に走らせると親の追従分と二重計上して 25 になる——**測り方を書かないと再現しない**。

**1,000 行超のファイルは 5 本 → 4 本**。残る 4 本はタスク 8.9〜8.12 の対象と過不足なく一致する。

### 48.6 §47.10 の追認——警告の 5 数値は 3 者 3 様だった

実装者 **15/7/21/6/15**・レビュアー **14/4/14/0/14**・基準 **16/7/22/6/16**。§47.10 が予告したとおり逐語一致しない。

DIAG の**多重集合**を基準と突合した結果は次のとおりで、**新規行 0・欠落は MSVC の import library 生成通知のみ**:

| DIAG 行 | 基準 | レビュアー実測 |
|---|---:|---:|
| `"cdecl" is not a supported ABI …` | 6 | 6 |
| `unused std::ops::ControlFlow …` | 4 | 4 |
| デッドコード系 4 種 | 4 | 4 |
| `linker stdout: ライブラリ … を作成中` | 2 | **0** |

欠落 2 行は本文で特定でき、残る 14 行は基準と一字一句同じ多重集合である——**別の退行が都合よく隠されている余地は無い**。減少方向ゆえ要件 2.6 は充足。**タスク 8.13 は 5 数値の逐語一致で判定してはならない**（判定条件は「DIAG 多重集合が基準の部分集合かつ新規行 0」）。

### 48.7 登記（要件 5.2）

**修正を要する壊れたテスト・不正なテスト・テスト間の状態汚染は 1 件も発見しなかった。新規送付所見 0 件。**

レビュアーが独自に妥当性を確認した 4 件（いずれも既存・本タスクで不変）: `invalid_image_returns_err` は pid＋nanos で一時 `.dll` を一意化しアサート前に除去（`shiori_inproc_tests.rs:29,43`）／`inproc_connect_missing_file_fails_immediately` は pid＋thread id で一時 fixture 木を一意化しアサート前に除去（`shiori_inproc_adapter_tests.rs:240,248`）／`happy_path_loads_and_resolves_factory` と `build_real_backend` の二重 `LoadLibraryW` は Windows の HMODULE 参照計数と Drop の `FreeLibrary` で釣り合う。`set_var`・sleep・共有可変状態はいずれも 0 件。

記録のみ:
1. `rustfmt --check` の増分 **0**。ただし測り方に条件がある（上記 §48.5）。
2. intra-doc リンクの破断は発生しない（移動した本文の `//!` 内部 doc は本体ごと同行しており、外部を指す `[`…`]` リンク 0 件）。
3. 設計判断 #3（テストモジュール以外に付いた `#[cfg(test)]` 項目の残置）は**空振り**——base に非 `mod` の `#[cfg(test)]` 項目は 0 件だった。

## 49. 表示合成プレゼンタの本番本体をファサード分割: `crates/areka-emo-present/src/presenter.rs`（タスク 8.9・要件 2.5 / 2.6 / 2.7 / 4.2 / 4.3 / 4.4 / 4.5 改訂後）

- 実施日: 2026-08-10 / 分割前コミット `1ffebce` / **PowerShell（pwsh 7）**
- **タスク 6.1（`follow.rs`）／6.2（`frame.rs`）と同じ D1 ファサード再輸出**（設計判断 #5）。子は `presenter/` サブディレクトリへ置き素の `mod` で宣言する（`#[path]` フラット兄弟はテストファイル専用の規約）
- 移設済みテストファイル **8 本は 1 バイトも変更していない**。8 組の `#[cfg(test)] #[path] mod` 宣言もファサードへ逐語のまま残置した（`#[path]` は宣言ファイルのディレクトリ基準で解決するため、`presenter/` へ移すと壊れる）

### 49.1 成果ファイルと行数（すべて 1,000 行以下・最大 278）

| ファイル | 行 | 責務 |
|---|---:|---|
| `presenter.rs`（ファサード） | **109** | module doc＋`use` 13 文＋`mod` 6＋テスト接続宣言 8 組 |
| `presenter/show.rs` | 278 | `apply_show`（巨大 impl の核） |
| `presenter/read.rs` | 202 | `TextSlotView`＋照会 6 本 |
| `presenter/hub.rs` | 160 | `EmoPresenter` の器・target 登録・指令ディスパッチ・`Hide`／`InvalidateCache`・`reply`・`Default` |
| `presenter/refresh.rs` | 142 | `refresh_scale`＋`take_pending_resize` |
| `presenter/hit.rs` | 138 | `ClientHit`＋当たり判定 2 本 |
| `presenter/target.rs` | 106 | target ごとの私有状態 `PresentTarget` |

**1,066 → 最大 278 行**・合計 1,135。境界は元ファイルの doc が宣言する役割区分（表示成立点・照会契約・判定入口）に沿わせ、**関数の中は 1 つも割っていない**。

`impl EmoPresenter {` は **1 → 5 ブロック**へ分配した（同一型への別 impl は同一クレート内なら合法）。レビュアーが独立実装の分解器で「余剰 4 は素の impl ヘッダ複製のみ・属性もジェネリクスも `where` 節も持たない」ことを確認している。

### 49.2 純移動の機械証明（§31.2 の方法）——独立実装で二重に取った

実装者・レビュアーがそれぞれ別の分解器を書いて突合した。レビュアー側は字句状態追跡つき（行コメント／入れ子ブロックコメント／raw・byte 文字列／文字リテラル対ライフタイム／エスケープ）で `impl` ブロックをメンバまで 1 段展開している。

| 指標 | 実装者（項目単位） | レビュアー（impl 展開後） |
|---|---:|---:|
| 単位数 | 27 ↔ 27 | 30 ↔ 34（差 4 ＝ impl ヘッダ複製） |
| 欠落 / 重複 / 新設 | 0 / 0 / 0 | 0 / 0 / — |
| 変更行 | **19** | **19** |
| 区分 | 可視性付与 19・rustfmt 折り返し 0・その他 0 | 同左 |
| 字下げの変更 | **0** | **0** |

**19 行はすべて行頭に `pub(super) ` を挿入しただけ**で、引数の順序・型・本体は完全一致（機械的に剥がすと全単位がバイト一致）。§31.2 が戒める「空白差分だから無害」で済ませず、1 行ずつ提示して確認した。

内訳: `struct PresentTarget` ＋その名前付きフィールド 15 ／ `EmoPresenter.targets` ／ `fn apply_show` ／ `fn reply`。**`pub(super)` ＝ `pub(in crate::presenter)` で、分割前の「`presenter` 私有」と可視集合が完全同一**——広がっていない。タプル構造体は 0 件なので §22.2 の行内 `pub(...)` 罠は不成立。

再現可能なフラグメント分解（§46.4／§47.5／§48.2 と同一規則）: 断片 **46**・被覆 **1,047**・重複 **0**・未被覆 **19**（空行 0・すべて `pub(super)` を付けた署名／フィールド行）・バイト一致 1,047・可視性のみ 19・真に新規 69。1,047+19+69 = **1,135** ＝ 実測合計。

> **分解の副産物 1 件**: `window: Entity,` は原本に 2 回現れる（`PresentTarget:82` と `TextSlotView:176`）ため、貪欲分解では前者が `read.rs:20` に、後者が未被覆側に落ちる。**多重集合としては「同一 1・可視性付与 1」で釣り合っており欠落ではない**（レビュアーが独立に確認）。

**較正（§42.2）**: 連結から子を落とすと検査器が赤くなり欠落項目を原本行範囲つきで名指しする——`hit.rs` を落とす → 3 件（`ClientHit :238-253` ／ `hit_region :895-913` ／ `hit_region_client :915-1008`）、`read.rs` → 12 件、`target.rs` → 1 件。レビュアーも 2 通り再現した。

### 49.3 公開面不変（要件 2.5）——手法の異なる 2 通りで取った

- **rustdoc JSON**（nightly・`-Zunstable-options --output-format json`）を分割前後で採取し、root モジュールから辿った「公開パス＋種別＋正準化した署名」を序数整列して `diff` → **734 行 ↔ 734 行・バイト単位で同一**。
- **レビュアーが別手法で再導出**——`lib.rs` から到達可能な `pub` 項目を機械列挙して **21 = 21 で名前集合一致**、および自前生成の rustdoc JSON で `presenter::{ClientHit, EmoPresenter, TextSlotView}` と 18 の `pub fn` が同一パスに在ることを確認。
- **`presenter::hit` 等の新しい公開モジュールパスは 0 件**（子はすべて素の私有 `mod`・`pub mod` 0 件）。
- **クレート外の呼び出し側は 0 行変更**——`areka_emo_present::` の参照は 36 箇所・24 ファイル（`crates/areka` 17・`crates/areka-emo-text` 5 ほか）で、`git diff --name-only` はリポジトリ全体で `presenter.rs` 1 本のみだった。

### 49.4 `#[allow(unused_imports)]` が 0 文で済んだ——真因は lib/bin の別ではない

タスク文は「当該クレートが lib target を持つため 6.1／6.2 とは様相が異なる。実測して判断する」としていた。実測の答えは **抑止指示 0 文**（6.1 は 3 文・6.2 は 24 文）。

**ただし真因は lib/bin の違いではない。子が `use super::{…}` でファサードの束縛を引く形を採ったから**である（§44.5 が 8.2 で確立した「子同士も公開項目はファサード経由で引く」規則の再適用）。レビュアーがフィンガープリントを崩して**実際に再コンパイルさせた**うえで `--force-warn unused_imports` を lib／test 両プロファイルで実行し**両方 0 件**、さらに**ファサードの 13 `use` 文が供給する 29 名前すべてに子側の消費者がある**ことを機械照合した（未消費 0）——**死んだ再輸出は無い**。

**タスク 8.10〜8.12 が同じ形を採れるかは対象ごとに実測が要る**（サンプルは lib target を持たない）。

### 49.5 §32.4 の危険は現実化しなかった

8 本のテストファイルはいずれも `use super::*;` で始まり、外部クレート型（`areka_emo_atlas::*`・`wintf::ecs::*`・`areka_parsers::shell::*` 等）を親の `use` 束縛経由で引く。`use` を子へ配分してファサードから落とせば 6.2 と同じ大量の E0433／E0425 が出るところだった。**ファサードが `use` 一式を保持したことで回避**している。module doc `:1-44`・`use` ブロック `:46-65`・テスト接続宣言 `:1043-1066` はいずれも base とバイト一致（レビュアー確認）。

テスト名も改名 0——`presenter::` 接頭辞は前後とも **47 名で集合完全一致**。

### 49.6 行数会計（§42.7 の多重集合規約）
...
検算 `1,066 − 0 + 69 = 1,135`。**空行成分は 56 → 65 の純増 +9**。追加 69 の非空行 60 は `use` 文とその継続行 21・module doc 19・`mod` 宣言 6・説明コメント 6・`impl` ヘッダ 4・`impl` 閉じ括弧 4。

### 49.7 検証結果

`cargo test -p areka-emo-present` exit 0——**lib 127 ＋ 統合テスト 1**（`tests/swapchain_spike.rs`）・**doctest 0**（実装報告の「127 + doctest 1」は不正確。レビュアー実測が正）。`cargo test --workspace --no-fail-fast` exit 0・**85 ユニット・4,757 passed / 0 failed / 33 ignored**（＝4,790）。`cargo clean` は実行していない（i686 成果物は §9.3 とバイト数一致で健在）。

警告は分割前後とも **15/7/21/6/15**（同一の増分ビルド状態で採取・delta 0）。§47.10／§48.6 が定めた**正しい判定条件**である多重集合突合では、基準 16 行に対し実測 15 行・**新規行 0・部分集合 → PASS**。欠落 1 行は MSVC の import library 生成通知で、既に 2 度特定済みの減少方向。

`rustfmt --check` は**親エントリ 1 本だけに掛けてモジュール木を辿らせる測り方**（§48.5 と同じ。子を個別に走らせると親の追随分と二重計上する）で **51 → 51・delta 0**。

**§11.4**: 較正（`window_pos_tests.rs` 継続行 5 件 `[382,429,614,690,691]`／盲点 1 件 `[691]`）を逐語再現したうえで走査し、前後とも **継続行 0・盲点 0**（本ファイルは複数行文字列リテラルを 1 つも持たない）。de-indent 0 と併せ構造的にも無害。

対応表は無変更（本番本体の分割はテストを改名しない）・結合 **1,163 行 / 10 フラグメント**で全単射 PASS。

**1,000 行超のファイルは 4 本 → 3 本**。残る 3 本はタスク 8.10〜8.12 の対象と過不足なく一致する。

### 49.8 登記（要件 5.2）

**修正を要する壊れたテスト・不正なテスト・テスト間の状態汚染は 1 件も発見しなかった。新規送付所見 0 件。** 本タスクはテストコードを 1 バイトも触っていない。

記録のみ:
1. **intra-doc リンク**（§32.10 と同一判断・一律未修正）: lib doc 警告 **13 → 16（+3）**。分割サブツリー由来の未解決は 5 箇所（`presenter/hit.rs:54,55` ／ `read.rs:140` ／ `refresh.rs:127,130`）で、うち**真に新規に壊れたのは 3 件**（`derive_scale` 1・`PresentOutcome` 2）。`PresentTarget::applied` の 2 件は分割前も警告済で種別が変わっただけ——レビュアーが base のリンク文言を file:line で裏取りした。修正しない理由は**本文バイト一致の純移動証明と両立しないから**であり、解決用の `use` を足せば `unused_imports` が増えるだけである。
2. **要件 2.7（tracing target）**: 既定 target が `areka_emo_present::presenter::{hub,show,refresh,read,hit}` の子パスへ変わる。リポジトリ全域で **target リテラルの完全一致判定は 0 件**、実行時フィルタ `areka_emo_present=debug`（`hit_region_client` の実機サインオフ手順）は**クレート名レベルの前置一致**なので子 target にも当たる。当該クレートの捕捉ハーネスは `enabled()` が metadata を無視し `CapturedEvent` が target を保持しない（レビュアー確認）。観測互換は保たれる。
3. ⚠ **手順上の指摘（レビュー）**——実装者は「移設前」の rustdoc スナップショットを、**作業ツリーの `presenter.rs` を base blob で一時上書きして**採取した。読み取り専用の git のみを使う規律に反する形であり、過去に作業を破壊した事故と同種のリスクを持つ。復元が正確であったことはレビュアーが確認した（`//!` `:1-44`・`use` `:46-65`・テスト接続宣言 `:1043-1066` が base とバイト一致・`git status -uall` に残骸 0・項目単位の再構成が base と完全一致・ワークスペース全緑）。**今後この種の「移設前スナップショット」は `git archive` かスクラッチ複製で採ること。**

## 50. テキスト層サンプルの本体分割: `crates/areka-emo-text/examples/emo-text-layer.rs`（タスク 8.10・要件 4.2 / 4.3 / 4.4 / 4.5 改訂後）

- 実施日: 2026-08-10 / 分割前コミット `c827124` / **PowerShell（pwsh 7）**
- **群 8 で 4 つ目の形——サンプルバイナリの本体分割。** 純移動の規律は 6.1／6.2／8.9 と同じだが、接続の形が違う。**サンプルのルートファイルは自分のバイナリターゲットのクレートルート**なので、素の `mod drive;` は `examples/drive.rs` を探しに行き**新しいターゲットを生んでしまう**。ゆえに `#[path]` が必須である（8.9 の `presenter/` 素 `mod` 形との実質的な差異・レビュアーが確認）
- `#[test]` 0・`#[cfg(test)]` 0 の純サンプルコード。テストは 1 行も触れていない

### 50.1 成果ファイルと行数（すべて 1,000 行以下・最大 783）

| ファイル | 行 | 由来（原本の区画バナー） |
|---|---:|---|
| `emo-text-layer.rs`（ファサード） | **210** | module doc＋`use` 27 文（`:1-99` バイト不変）＋`#[path] mod` 6＋中継 `use self::…` 6＋`fn main`（原本 `:464-543` とバイト一致） |
| `emo-text-layer/drive.rs` | **783** | `:662-1434` 駆動 system |
| `emo-text-layer/verdict.rs` | 147 | `:269-407` 判定と readback 述語＋観測記録（箱 2 つを束ねた） |
| `emo-text-layer/setup.rs` | 123 | `:545-660` セットアップ |
| `emo-text-layer/scenario.rs` | 120 | `:150-267` シナリオ定義 |
| `emo-text-layer/demo.rs` | 59 | `:409-462` `Demo` |
| `emo-text-layer/fixture.rs` | 50 | `:101-148` 定数・fixture パス解決 |

**1,434 → 最大 783 行**・合計 1,492。**バナー箱 8 個 → 7 destination で新設した境界は 0**。レビュアーが各 destination が単一の連続区間であること、ファイル内の原本順が保たれていること、**バナーを跨いだ項目が 0 件**であることを確認した。

`drive.rs` の 783 行は群 8 で最大の子だが、**原本が宣言する単一のバナー箱（`:662-1434`）にちょうど対応し、その範囲内に下位バナーが 1 つも無い**。これ以上割れば §42.3／§47.2 が禁じる「原本に無い境界の新設」になる。レビュアーも同意見である。

### 50.2 純移動の機械証明——レビュアーがより細かい粒度で再導出した

| 指標 | 実装者 | レビュアー（独立実装・impl を 1 段展開） |
|---|---:|---:|
| 単位数 | 55 ↔ 55 | **57 ↔ 57** |
| 欠落 / 重複 / 新設 | 0 / 0 / 0 | 0 / 0 / 0 |
| バイト一致 | 31 | **33** |
| 可視性のみ | 23 | 23 |
| rustfmt 強制の折り返し | 1 | 1 |
| **説明のつかない差異** | **0** | **0** |
...
**行会計は両者一致**: **置換 52 ／ 削除 1 ／ 追加 59 ／ 字下げ 0**。検算 `1,434 − 1 + 59 = 1,492`。空行成分は **97 → 99 の純増 +2**。

**可視性付与 52 行はすべて `None → pub(super)`——広がったものは 0、キーワードの格下げも 0**（レビュアー実測）。原本の項目はサンプルのクレートルートに在り「私有＝クレート全体から見える」状態だったので、直下の子からの `pub(super)` ＝ `pub(in crate)` は**可視集合が完全に同一**である。加えて **26 項目は逆に狭まった**（`drive` 14・`scenario` 7・`fixture` 2・`setup` 2・`verdict` 1——§31.6 の型）。

**較正（§42.2）**: 連結から子を落とすと欠落項目を原本行範囲つきで名指しする——`verdict.rs` → 10 件（`struct Verdict :269-282` 〜 `struct Observations :391-407`）、`fixture.rs` → 7 件、`demo.rs` → 3 件、`setup.rs` → **3 件**。

### 50.3 唯一の rustfmt 強制折り返しは「強制」を実証した

`inline_extent_first_band`（原本 `:328-362` → `verdict.rs:62-102`）。引数の順序・型・本体はバイト一致で、差は `pub(super) ` 前置と改行位置のみ。

**レビュアーが「強制」を実証した**——折り返さない 1 行形をスクラッチで再構成し（**108 桁**。実装報告の 110 桁は誤り）、`rustfmt --check --edition 2024` が**実装が採ったのと一字一句同じ 7 行形**を要求することを確認。出荷した `verdict.rs` は単体で rustfmt clean（exit 0）である。

> **訂正**: 実装報告は「空白を潰せば両者は同一」と書いたが、**rustfmt は `pitch: u32` の後ろに末尾カンマも挿入している**ので純粋な空白潰しでは一致しない（レビュー指摘）。一致するのは末尾カンマを無視するトークン正規化のもとだけである。意味は変わらない。

### 50.4 フラグメント台帳の内訳に取りこぼしがあった

実装報告の「未被覆 102 ＝ 可視性のみ 52 ＋ 真に新規 50」は**追加を過小に書いている**。網羅的な多重集合では **111 ＝ 52 ＋ 59** で、貪欲なフラグメント分解が 9 件の追加（空行・`};`・繰り返し現れる `use super::{`）を被覆側へ吸収してしまうためである（§49.2 と同じ副産物）。

**1,390 と 102 はどちらも成果物側の数**（和が 1,492）なので、§46 で問題になった「原本側と成果物側の取り違え」は起きていない——レビュアーが確認した。**再現する数字は 置換 52 ／ 削除 1 ／ 追加 59** であり、本節はそれを正本とする。

削除 1 は原本の 1 行署名 `fn inline_extent_first_band(...) -> u32 {` そのもので、7 行形に置き換わった。正当である。

### 50.5 Cargo ターゲット不変（要件 4.3）

`cargo metadata --no-deps` は前後とも **example ターゲットちょうど 2 個**（`emo-text-layer`・`emo-text-typewriter-demo`）で src_path も不変。子ディレクトリに `main.rs` は無い（`find` で 0 件）。両方の `.exe` が `target/debug/examples/` に生成される。

**子ディレクトリ名をサンプル stem と同じ `emo-text-layer/` にした**判断について、レビュアーが隔離したスクラッチクレートで実証した——`examples/foo.rs` の隣に `examples/foo/main.rs` を置くと Cargo は**マニフェスト解析で失敗する**（`found duplicate example name foo`）。**静かに 2 つ目のターゲットを生むのではなく大声で落ちる**ので、この命名は安全側である。なお同ディレクトリ＋`main.rs` の形はこのリポジトリで現に使われている（`crates/pilot/examples/shiori-host-32/main.rs`）ので、`emo-text-layer.rs:101-104` の警告コメントは無駄ではない。

§35 の最長 stem／前向き衝突の規則は `<stem>_<テーマ>.rs` という**兄弟ファイル**の形を対象とするもので、サブディレクトリ形には当たらない。逆引きはディレクトリ名で一意である。

**`cargo run --example emo-text-layer` は実行していない**（GPU 合成窓を開くため実表示が要る）。タスクの完了状態は「サンプルビルドが緑」なのでビルドで充足する。`fn main` が原本 `:464-543` とバイト一致であることから、実行時の意味も構造的に不変である。

### 50.6 検証結果

`cargo build -p areka-emo-text --examples` exit 0・**警告 0** ／ `cargo build --workspace --all-targets` exit 0 ／ `cargo test --workspace --no-fail-fast` exit 0・**85 ユニット・4,757 passed / 0 failed / 33 ignored**（＝4,790）。`cargo clean` は実行していない。

警告は前後とも **15/7/21/6/15**。§47.10／§48.6 の判定条件（DIAG 多重集合が基準の**真部分集合**かつ新規行 0）で **PASS**——欠落 1 行は既知の MSVC import library 生成通知。

**`#[allow(unused_imports)]` は 0 文**（base も 0）。`--force-warn unused_imports` は強制リコンパイル後 **0 件**。**§49.4 の形（子が `use super::{…}` でルートの束縛を引く）は lib target が無くても働いた**——サンプルは lib を持たないので §31.5 の bin 挙動を懸念していたが、ルートの 27 `use` 文がすべて子に消費されるため未使用が生じない。

`rustfmt --check` は親エントリ単体（モジュール木を辿らせる測り方）で **420 → 420・35 ハンク → 35 ハンク・delta 0**。到達には rustfmt 主導の編集 3 件（`mod` 宣言の辞書順整列・中継 `use` の折り返し・§50.3 の署名折り返し）を要した。

> **訂正**: 実装報告の「違反の多重集合がバイト同一」は**言い過ぎ**で、ハンク数と総行は同一（delta 0）だがコンテキスト行が 2 行異なる（レビュー指摘）。

**§11.4**: 較正（`window_pos_tests.rs` 継続行 5 件 `[382,429,614,690,691]`／盲点 1 件 `[691]`）を逐語再現したうえで、前後とも **継続行 0・盲点 0**（複数行文字列リテラルが 1 つも無い）。**de-indent は起きていない**（最上位項目を別ファイルの最上位へ移しただけ）。

**intra-doc リンクは 1 本だけ**（`[`window_probe`]`・原本 `:1225` → `drive.rs:574`）で、参照先は同一ファイル内（`drive.rs:760`）ゆえ **破断 0**。`cargo rustdoc` が挙げる 9 件の未解決はすべて `src/` の既存分である。

対応表は無変更（テストが 0 件）・結合 **1,163 行 / 10 フラグメント** PASS。**1,000 行超のファイルは 3 本 → 2 本**（残り＝`areka/examples/emo-present.rs` 1,168・`areka/examples/collision-probe.rs` 1,063 ＝ タスク 8.11／8.12）。

### 50.7 ⚠ 登記（要件 5.2）——群 8 で初めての新規送付所見 1 件

**修正を要する壊れたテスト・不正なテストは発見しなかった。** ただし**テスト間の状態汚染を疑わせる観測が 1 件出た**。

**新規送付所見（送り先: `test-cage-determinism` / W6.9・ログ捕捉ハーネス項）**

> `crates/areka-seriko/src/actor_dispatch_tests.rs:741` の `non_shell_broadcast_reception_is_benign_debug_no_warn_error` が、ワークスペース検証中に **1 度だけ失敗した**——**捕捉したログバッファが空**。隔離して再実行すると 177/177 緑、ワークスペース再実行でも exit 0 で、以後再現していない。
>
> **これは `notes.md:1617`（§16）の結論を経験的に反証する。** §16 は `capture_logs`／`capture_logs_flow` を「`with_default` はスレッドローカルだから並行干渉を受けない」として並行安全と判定していた。空バッファはまさにその前提が崩れた形である。
>
> `areka-seriko` は本タスクで 1 バイトも触れておらず `areka-emo-text` のサンプルに依存もしないので、**本タスクが原因である可能性は構造的に無い**。既存の欠陥の顕在化である。

この所見は **§37 の送付所見一覧（8 件）へ 9 件目として加える**——**タスク 8.13 の作業とする**。

記録のみ:
1. 実装報告の 3 つの計数誤り（レビュー実測が正）: `struct Demo` の名前つきフィールドは **21** 個（`demo.rs:12`–`:50`）で 20 ではない（合計 52 は不変——Demo 22 ＋ Observations 6 ＋ Verdict 4 ＋ 単行項目 20）／`setup.rs` の較正は `ITEM-MISSING` **3 件**で 4 ではない／EVIDENCE の `verdict.rs:9/:20/:37?no/:130` は文字化けで、正しくは可視性付与が **`:9, 11, 13, 15, 20, 32, 45, 105, 136, 138, 140, 142, 144, 146`（14 件）**＋折り返した署名 `:64`。`fn is_opaque`（`:37`）は**意図的に付与していない**（26 件の狭まった項目のひとつ）。
2. 分割途中で 43 件（E0603×21・E0425×15・E0432×4 ほか）→ 132 件（E0616 私有フィールド 131・E0624）→ 0 とコンパイルエラーが遷移した。**E0616 群は可視性付与だけで解消しており本文の編集ではない**——項目単位の証明で「説明のつかない本文差異 0」が裏づける。

## 51. 表示合成サンプルの本体分割: `crates/areka/examples/emo-present.rs`（タスク 8.11・要件 4.2 / 4.3 / 4.4 / 4.5 改訂後）

- 実施日: 2026-08-10 / 分割前コミット `c587f55` / **PowerShell（pwsh 7）**
- **§50（タスク 8.10）の方式をそのまま適用**——`#[path]` で stem 名のサブディレクトリへ接続・`main.rs` を作らない・子は `use super::{…}` でルートの束縛を引く（§49.4 の形）
- `#[test]` 0・`#[cfg(test)]` 0。本ファイルは本番木の `#[path]` include を持たない（それは 8.12 の課題）

### 51.1 成果ファイルと行数（すべて 1,000 行以下・最大 374）

| ファイル | 行 | 由来（原本の区画） |
|---|---:|---|
| `emo-present.rs`（ファサード） | **223** | `:1-138`（バイト不変）＋接続 8 組＋`:255-303`（Entry Point 箱と `fn main`・バイト不変） |
| `emo-present/reconcile.rs` | **374** | `:758-1121` |
| `emo-present/setup.rs` | 163 | `:309-460` |
| `emo-present/systems.rs` | 159 | `:605-752` |
| `emo-present/state.rs` | 91 | `:172-253`（箱 3 つを束ねた） |
| `emo-present/window.rs` | 73 | `:537-599` |
| `emo-present/balloon.rs` | 72 | `:466-531` |
| `emo-present/input.rs` | 51 | `:1127-1168` |
| `emo-present/fixture.rs` | 29 | `:144-166` |

**1,168 → 最大 374 行**・合計 1,235。バナー箱 **11 個**に対し 8 destination ＋ ファサードで、**各 destination は単一の連続区間・原本順を保持・バナーを跨いだ項目 0 件**（レビュアーが独立に確認）。**新設した境界は 0。** 子に `crate::` は 0 件。

`reconcile.rs` の 374 行は原本 `:757-1122` の**単一バナー箱に対応し、その内部に下位バナーが 1 本も無い**。これ以上割れば §42.3／§47.2 が禁じる境界の新設になる——レビュアーも同意見。

### 51.2 純移動の機械証明——単位の粒度が流儀で割れることが分かった
...
| 欠落 / 重複 / 新設 | 0 / 0 / 0 | 0 / 0 / 0 |
| バイト一致 | 10 | **11** |
| 可視性のみ | 19（28 行） | 19（28 行） |
| 折り返し | 0 | 0 |
| **説明のつかない差異** | **0** | **0** |
| 字下げの変更 | 0 | 0 |

**差 1 は流儀の違い**——`impl` の閉じ括弧（原本 `:226`）を単位に数えるかどうか。実装者は数えず行会計に委ね、レビュアーは単位として数えて `state.rs` に**ちょうど 1 度だけ**存在することを直接証明した。§50.2 と同型の粒度差である。**第三者が再現できるよう両方を記録する。**

行会計（多重集合・§42.7）は両者一致: **置換 28 ／ 削除 0 ／ 追加 67 ／ 字下げ 0**。検算 `1,168 − 0 + 67 = 1,235`。空行 **84 → 86（+2）**。追加 67 の内訳（レビュアー分類）は `use` 文 16・`use` 継続行 18・コメント 9・`#[path]` 8・`mod` 宣言 8・閉じ括弧 6・空行 2。新規コメント 9 行はすべてファサードの配線箱で、**子には 1 行も足していない**。

**可視性付与 28 行はすべて `None → pub(super)`・広がり 0・格下げ 0**（レビュアーが 1 行ずつ提示）。直下の子からの `pub(super)` ＝ `pub(in crate)` で、サンプルのクレートルート私有と可視集合が同一。**逆に狭まったのは 6 件**（`ReconcileKind`・`build_and_spawn`・`build_shell_target`・`build_balloon_assets`・`read_balloon_offset`・`reconcile_window_size`——§31.6 の型）。

ファサードのバイト同一性は `rb` 読みのバイト比較で確認: `:1-138` ＝ base `:1-138`、`:175-223` ＝ base `:255-303`。

**較正（§42.2）は 8 通り**——子を 1 本ずつ落とすと毎回赤くなり欠落項目を原本行範囲つきで名指しする（fixture 5・balloon 2・window 2・state 8・systems 2・input 1・setup 4・reconcile 5）。

> **記録**: 実装者の字句解析器は初版が誤っていた——`assert_startup_golden`（原本 `:923-1082`）を `:1075` で誤終端していた。**文字列リテラル内の `\` 継続を改行ごと飲む**典型的な実装バグで、ファイルを 1 本も書く前に修正している。レビュアーが自前の器で同じ 7 行を正しく処理できることを確認した。**§11.4 の継続行が字句器そのものを壊す**という新しい形の注意点である。

### 51.3 Cargo ターゲット不変とビルド（要件 4.3・完了状態）

`cargo metadata --no-deps` の example ターゲットは前後とも **23 個**で名前・`src_path` とも不変。子ディレクトリに `main.rs` は 0 件。`#[path]` 属性は子の配線 8 本だけ（`emo-present.rs:149,151,153,155,157,159,161,163`）。

`cargo build -p areka --examples` **exit 0・警告 0**（タスクの完了状態）。`cargo build --workspace --all-targets` exit 0。`cargo test --workspace --no-fail-fast` exit 0・**85 ユニット・4,757 passed / 0 failed / 33 ignored**（＝4,790）。`cargo clean` は実行していない。

### 51.4 ⚠ Implementation Notes の E0433 アンカーが陳腐化していた——本 spec 自身が動かした

Implementation Notes は「`crates/areka` の examples は test モードで既存 E0433（`crates/areka/src/placement/spawn.rs:879`）」と書いている。**現在の逐語は `crates\areka\examples\..\src\placement\spawn_assembly_tests.rs:183:12` の `cannot find 'input_events' in 'crate'` である。**

レビュアーが移設元を特定した——**本 spec のタスク 5.2（commit `f64fac1`）の純テスト移設**であり、`f64fac1^:crates/areka/src/placement/spawn.rs` の同一コードが確かに **879 行目**にある。**同じ欠陥が別の file:line へ動いただけ**で、本 spec がテストを移したことの正当な帰結である（8.11 起因ではない）。

test モードで落ちるのは **`window-placement` と `collision-probe` の 2 本のみ**（いずれも本タスク無変更）。`cargo test -p areka --example emo-present --no-run` は **exit 0**、`--examples` の exit 101 は本タスクに 1 件も帰属しない。

**タスク 8.13 は Implementation Notes のこの記載を訂正すること。**

### 51.5 検証結果

警告は前後とも **15/7/21/6/15**。§47.10／§48.6／§50.6 の判定条件（DIAG 多重集合が基準 16 行の真部分集合かつ新規行 0）で **PASS**——欠落 1 行は既知の MSVC import library 生成通知。`--force-warn unused_imports --force-warn dead_code` は **0 件**、`allow(unused` は前後とも **0 件**。**§49.4 の形はサンプル（lib target なし）でも 2 度目の成功**である。

`rustfmt --check` は親エントリ単体で **前後とも 6 ハンク / 98 行・delta 0**。ハンク本文の多重集合は「コンテキスト行 1 本から `pub(super) ` を剥がせば一致」で、**順序は変わっている**（rustfmt はモジュールを宣言順に辿るため base `:1090` → `reconcile.rs:343` が先頭へ）。実装者はこれを**あえて狭く言った**——§50.6 が戒めた「バイト同一」の言い過ぎを回避している。

**§11.4**: 較正（`window_pos_tests.rs` 継続行 5 件 `[382,429,614,690,691]`／盲点 1 件 `[691]`）を逐語再現したうえで、base 継続行 **7 件** `[1022,1029,1030,1050,1051,1061,1062]` → after **7 件**（すべて `reconcile.rs:[275,282,283,303,304,314,315]`・オフセット 747 で完全対応）・**盲点 0 → 0**・de-indent 0。

対応表は無変更・結合 **1,163 行 / 10 フラグメント** PASS。**1,000 行超のファイルは 2 本 → 1 本**（残＝`collision-probe.rs` 1,063 ＝ タスク 8.12）。

### 51.6 intra-doc リンクが 3 本壊れた——そして素の `cargo rustdoc` は偽の緑を出す

§32.10 の一律未修正判断に従い放置する。件数は **未解決 0 → 3**（`reconcile.rs:340` の `[CycleState::next]` と `[PresentCommand]`・原本 `:1087` ／ `state.rs:22` の `[CYCLE_INTERVAL_SECS]`・原本 `:184`）。既存の `private_intra_doc_links` 2 件（ファサード `:35`／`:58`——module doc でバイト不変）は種別も件数も不変。

**⚠ 測り方の注意（8.12／8.13 への申し送り）**: 子は私有モジュールなので、**素の `cargo rustdoc` は未解決リンクを 0 と報告する——偽の緑である**。`--document-private-items` が要る。なお「0 を報告する」は**未解決リンクが 0** の意で、warning 総数は既存 2 件である（レビュー指摘・語を精密にすること）。

before の採取は、`-v` で捕捉した rustdoc 起動をスクラッチの base blob に対して入力パスを差し替え `CARGO_MANIFEST_DIR` を設定して再生する方法で行った——**作業ツリーには一切書いていない**（§49.8(3) の規律。レビュアーも同じ方法で再現し `git status` で無接触を確認）。
...
記録のみ:
1. **`cargo run --example emo-present` は実行していない**（GPU 合成窓に実表示が要る）。タスクの完了状態はビルドの緑であり、`fn main` が原本 `:255-303` とバイト一致なので実行時の意味も構造的に不変である（§50.5 と同一判断）。
2. tracing target のリテラル一致判定は 0 件。
3. レビュアーが 8.6〜8.10 の再発 3 モード——(α) 私的アルゴリズム依存の中間値の引用、(β) 項目・フィールドの計数誤り、(γ) 成り立たない等式の言い過ぎ——を**すべて明示的に検査し、いずれも不成立**と判定した。本タスクは §50.4 を受けて貪欲台帳を主図から外し、網羅的多重集合を正本にしている。

## 52. 当たり判定サンプルの本体分割: `crates/areka/examples/collision-probe.rs`（タスク 8.12・要件 4.2 / 4.3 / 4.4 / 4.5 改訂後）

- 実施日: 2026-08-10 / 分割前コミット `21f3800` / **PowerShell（pwsh 7）**
- **群 8 最後の分割であり、リポジトリの 1,000 行超ファイルを 0 本にしたタスク**
- 方式は §50（8.10）／§51（8.11）と同一。本タスク固有の制約は**本番木の `#[path]` 私有 include をサンプル本体に残すこと**

### 52.1 成果ファイルと行数（すべて 1,000 行以下・最大 330）

| ファイル | 行 | 由来（原本の区画） |
|---|---:|---|
| `collision-probe/probe.rs` | **330** | `:523-845` |
| `collision-probe.rs`（ファサード） | **327** | `:1-236`（バイト不変）＋配線 32 行＋`:323-381`（バイト不変） |
| `collision-probe/setup.rs` | 149 | `:382-522` |
| `collision-probe/pointer.rs` | 90 | `:932-1016` |
| `collision-probe/ratio.rs` | 88 | `:846-931` |
| `collision-probe/fixture.rs` | 53 | `:237-288` |
| `collision-probe/smoke.rs` | 49 | `:1017-1063` |
| `collision-probe/state.rs` | 35 | `:289-322` |

**1,063 → 最大 330 行**・合計 1,121。原本のバナー箱 **8 個**が 7 子＋ファサードへ 1 対 1 で落ち、**新設した境界は 0**（ファサードに配線用の箱を 1 つ足しただけ。原本 8 箱の見出しはすべて逐語で 1 度ずつ存在する）。**バナーを跨いだ項目は 0 件**——レビュアーが全項目の原本行範囲が単一の箱に収まることを確認した。

ファサードのバイト同一性はバイナリ比較で確認: `:1-236` ≡ base `:1-236`、`:269-327` ≡ base `:323-381`（両ファイルとも LF・末尾改行あり）。

### 52.2 本タスクの中心的な不変条件——本番木 include はサンプル本体に残った

三つの `#[path = "../src/..."]` 宣言は `collision-probe.rs:218`（`emo2_boot/target_map.rs`）・`:224`（`emo2_boot/hit_region.rs`）・`:231`（`placement/mod.rs`）に、doc コメントごと**バイト不変で**残っている（上記のとおり `:1-236` はバイト同一区間の内側）。相対パスは宣言ファイルのディレクトリ基準で解決するので、子（`examples/collision-probe/`）へ移せば `../../src/...` が要る——移さないのが正解である。

レビュアーが機械的に確認した:
- **子は `mod` を 0 件・`#[path]` を 0 件しか宣言していない**（配線はすべてファサード）
- **`../src/` は子に 1 件も現れない**
- `crate::` の grep ヒットは `collision-probe/setup.rs:79` の 1 件だけで、**コメント内の散文**である。その行は原本 `:452` に同一の形で存在し、移設した項目はバイト一致
- **`crates/areka/src/` はバイト不変**（`git diff --name-only 21f3800 -- crates/areka/src` が 0 ファイル）
- `crates/areka/src/emo2_boot/hit_region.rs:118` の `super::target_map::shell_target(scope)` は**クレートルートの `mod target_map` へ従来どおり解決する**（両宣言がサンプル本体に留まったため）

### 52.3 純移動の機械証明——道具が 2 度壊れていたので独立実装で取り直した

| 指標 | 実装者 | レビュアー（独立実装の字句器） |
|---|---:|---:|
| 単位数 | 29 ↔ 29 | **29 ↔ 29** |
| 欠落 / 重複 / 新設 | 0 / 0 / 0 | 0 / 0 / 0 |
| バイト一致 | 11 | 11 |
...
**実装者は自分の字句器のバグを 2 件見つけて出荷前に直したと申告している**——(i) 閉じ括弧は `depth` ではなく `depth+1` に在る、(ii) コメント・文字列の中の `;`／`}` が項目を終端させていた。§51.2 の「文字列内 `\` 継続で誤終端」に続き、**分解器そのものが壊れる形は 2 タスク連続**である。レビュアーが独立実装で全数を再導出して一致を確認した。

単位の流儀は宣言済み——`use`／`mod` 宣言は単位集合から除く（`RustParse.ps1:536` と同じ）。`impl` 再帰は実装したが本ファイルに `impl` ブロックが 0 個なので空振り。同着は署名キーの出現順で解決。

**「置換 24」と「可視性のみ 18 単位」の関係**もレビュアーが突合した——**18 単位が変わり、その合計が 24 行**である。`ProbeBoot` だけが 7 行（構造体行＋フィールド 6・`state.rs:22,24,26,28,30,32,34`）を持ち、残る 17 単位は 1 行ずつ。17 + 7 = 24。

行会計（多重集合・§42.7）: **置換 24 ／ 削除 0 ／ 追加 58 ／ 字下げ 0**。検算 `1,063 − 0 + 58 = 1,121`。空行 **84 → 87（+3）**。追加 58 の内訳はレビュアーが逐語再現した——`use` 先頭行 20・コメント 11・`#[path]` 7・`mod` 7・`use` 継続行 7・`};` 3・空行 3。

**可視性付与 24 行はすべて `None → pub(super)`・広がり 0・格下げ 0**。base には**どの字下げにも `pub` トークンが 1 つも無く**、after の 8 ファイルの `pub` はすべて `pub(super)` で総数がちょうど 24（レビュアー実測）。直下の子からの `pub(super)` ＝ `pub(in crate)` で可視集合は同一。バイト一致の 11 単位は逆に**狭まった**（§31.6 の型・サンプル bin はクレート外公開面を持たない）。

**較正（§42.2）は 7 通り**——子を 1 本ずつ落とすと毎回赤くなり欠落項目を原本行範囲つきで名指しする（fixture 10・probe 6・ratio 4・setup 3・smoke 2・state 2・pointer 1）。

### 52.4 🎯 リポジトリの 1,000 行超ファイルが **0 本**になった

**レビュアーが 2 通りの独立計測で確認した:**

| 方法 | 走査範囲 | 1,000 行超 |
|---|---|---:|
| (a) `git ls-files --cached --others --exclude-standard` の `.rs` 880 本 | `target/`・`.git/` を除外（gitignore／内部）・**`.kiro/` を含む**（fixture 8 本・いずれも小さい） | **0** |
| (b) spec の `Measure-TestModules.ps1`（872 本） | `target｜vendors｜.git｜node_modules｜.claude/worktrees｜.kiro` を除外 | **0** |

同じ (a) の方法で base `21f3800` を測ると**ちょうど 1 本**（`collision-probe.rs` 1,063）——**1 → 0** が同一の物差しで裏づけられている。**`vendors/pasta` サブモジュールは本ワークツリーで populate されており**（`.rs` 160 本・最大 961）、これを含めても 0 は崩れない。

`Measure-TestModules.ps1` は併せて **テストコード 500 行超のファイルも 0 本**と報告する。

**残る最大ファイル**: 986 `crates/pilot/examples/pilot-clickthrough-alpha-toggle/main.rs` ／ 974 `areka-emo-text/src/draw.rs` ／ 962 `areka/src/main.rs` ／ 960 `areka-ghost/src/runtime_tests.rs` ／ 950 ／ 945 ／ 942 ／ 937。

### 52.5 検証結果

`cargo build -p areka --examples`（タスクの完了状態）**exit 0・警告 0**（8 ファイルすべてを touch して強制再コンパイルしたうえで）。`cargo build --workspace --all-targets` exit 0。`cargo test --workspace --no-fail-fast` exit 0・**85 ユニット・4,757 passed / 0 failed / 33 ignored**（＝4,790）。`cargo clean` は実行していない（i686 成果物は健在）。

Cargo の example ターゲットは前後とも **23 個**（`areka` は 5 本）で名前・`src_path` とも不変。子ディレクトリに `main.rs` は 0 件で、`examples/` 直下に新しいファイルは 1 本も無い。

`cargo test -p areka --example collision-probe --no-run` は **exit 101**、エラーはちょうど 1 件で逐語 `error[E0433]: cannot find 'input_events' in 'crate'` at `crates\areka\examples\..\src\placement\spawn_assembly_tests.rs:183:12`——**§51.4 と文言も位置も同一**であり、**本タスクが 1 バイトも触っていない `window-placement` が同じものを再現する**ので本タスクに帰属しない。`emo-present` は exit 0。

警告は **15/7/21/6/15**（文脈情報）。判定条件である DIAG 多重集合は基準 16 行の**真部分集合・新規行 0** で **PASS**（欠落 1 ＝ 既知の MSVC import library 生成通知）。**`collision` を含む警告行は 0 件。** `cargo rustc -p areka --example collision-probe -- --force-warn unused_imports --force-warn dead_code` は **22 件**を出すが、レビュアーが位置を全数読んで**すべて `#[path]` include した本番木（`..\src\...`）の中で、ファサードにも子にも 0 件**であることを確認した。`#[allow(unused` は 0 件——**§49.4 の形（子が `use super::{…}` で引く）は 3 度目の成功**である。

**§11.4**: 較正（`window_pos_tests.rs` 継続行 5 件 `[382,429,614,690,691]`／盲点 1 件 `[691]`）を逐語再現したうえで、base・after 8 本とも **継続行 0・盲点 0**（複数行文字列リテラルが無い）。de-indent 0。

対応表は無変更・結合 **1,163 行 / 10 フラグメント** PASS・追記 0 行。

`rustfmt --check --edition 2024` は親エントリ単体（`#[path]` include 経由で本番木まで辿る）で **前後とも 144 ハンク / 1,653 行・delta 0**。`src/` 木の 138 ハンク本文はバイト同一。

> **訂正（レビュー指摘・失敗モード γ の軽度な形）**: 実装報告は「サンプル側で本文が異なるハンクは `:233` の 1 つ」と書いたが、**実際は 2 つ**——`collision-probe.rs:233`（コンテキスト行がバナー文言）と `collision-probe/fixture.rs:43`（コンテキスト行が `fn emo2_root` → `pub(super) fn emo2_root`）。いずれも既に列挙済みの変更に由来し、ハンク数・総行は delta 0 のままなので実質的な影響は無い。

### 52.6 intra-doc リンク——警告種別の**減少**が損失を隠していないことを確かめた

`--document-private-items` つきで測ると見出し **34 → 35**（未解決 **26 → 30**、`private_intra_doc_links` **6 → 3**、未閉じ HTML タグ 2 → 2）。

**`private_intra_doc_links` が 6 → 3 へ減っている**——警告種別の減少はしばしば損失を隠す。レビュアーが独立に木の外へ base の複製（base blob ＋ `crates/areka/src` のコピー・**作業ツリーには無接触**）を作って rustdoc 起動を再生し、確認した: **private から抜けた 3 件（`on_probe_pointer_moved`・`emo2_root`・`build_shell_target`）が、未解決へ入った 4 件のうちのちょうど 3 件**である。**真に新規の破断は 1 件だけ**——`fixture.rs:36` の `[ratio_parts]`（doc は `RATIO_PARTS_MAX_DEN` に付いて `fixture` に在り、参照先の関数は `ratio` へ移った）。**差引 +1 で、減少は何も隠していない。**

§32.10 の一律未修正判断に従い放置する（直せば `:1-236` のバイト同一性を失う）。

**§51.6 の偽の緑が別の形で再現した**——素の `cargo rustdoc` は見出しを **6 件**しか報告しない（`--document-private-items` つきは 35 件）。**29 件の過小報告**である。8.11 の「未解決 0 を報告する」形と違い、こちらは 0 ではなく 6 を報告するので**一見もっともらしく見える**ぶん危険が大きい。**タスク 8.13 は必ず `--document-private-items` で測ること。**
...
記録のみ:
1. **`cargo run --example collision-probe` は実行していない**（GPU 合成窓に実表示が要る）。完了状態はビルドの緑であり充足する。**本件の実行時不変の論拠は 8.10／8.11 より強い**——`fn main` が原本 `:324-381` とバイト一致、移設した全項目が `pub(super) ` 前置を除いてバイト一致、本番木もバイト不変。唯一の実行時差分は tracing の既定 target が子モジュールパス（`collision_probe::probe` 等）へ移ることだが、移設したコードに `target = "…"` リテラルは **0 件**、リポジトリ全域に `collision_probe` を target 文字列として参照する箇所も **0 件**で、文書化された受け入れ手順は**メッセージ本文**（`collision-probe: k=`）を grep するので契約は保たれる（レビュアー確認）。
2. **既存の rustfmt 違反 `collision-probe.rs:236`**（`use placement::spawn::{…}` が 100 桁超）は放置した。直せば保存区間 `:1-236` のバイト同一性を失う。
3. レビュアーが再発 3 モードを明示的に検査し、**(α) 私的アルゴリズム依存の中間値の引用と (β) 計数誤りは 0 件**、**(γ) は §52.5 の 1 件（軽度）**と判定した。

## 53. 対応漏れ解消後の全体再検証と GO 再判定（タスク 8.13・要件 1.7 / 2.1 / 2.2 / 2.3 / 2.4 / 2.6 / 2.9 / 6.3 / 6.4）

- 実施日: 2026-08-10 / HEAD `5e36218`（タスク 8.12 のコミット時点・着手時 `git status --porcelain` 空）/ **PowerShell（pwsh 7）** / 既定ターゲット `x86_64-pc-windows-msvc`
- ツールチェーン: `cargo 1.97.1 (c980f4866 2026-06-30)` / `rustc 1.97.1 (8bab26f4f 2026-07-14)` / `stable-x86_64-pc-windows-msvc (default)`
- マージベース `247d48a`（`git merge-base origin/main HEAD`）から **62 コミット**
- **位置づけ**: §38 の GO は §39 で撤回された。群 8（タスク 8.1〜8.12）が撤回理由を消化したので、本節は**タスク 7.1／7.2／7.4／7.5 の全域検証を群 8 のぶんまで含めてやり直し**、そのうえで撤回された GO を再判定する
- 本タスクは `.rs` を 1 本も変更していない。`cargo clean` は実行していない（着手前に i686 host-32 成果物の実在を確認——`shiori-host32-helper.exe` **271,872 B** ／ `shiori.dll` **155,648 B**・§9.3 の記録とバイト数一致）

### 53.1 移設後スナップショット 3 本の再採取（要件 2.2 / 2.3 / 2.6）

タスク 7.1 が採った 3 本は群 8 の 13 サブタスクぶん陳腐化していたので、**§10.2 と同一手順で採り直して上書きした**。3 本とも exit 0。スコープは `--exclude areka` を含めてタスク 1.3／7.1 と同一である。

| # | コマンド（逐語） | exit | 保存先 | 行数 |
|---:|---|---:|---|---:|
| 1 | `cargo test --workspace --no-fail-fast -- --list` | 0 | `verification/after_default.txt` | **4,790** |
| 2 | `cargo test --workspace --exclude areka --all-targets --no-fail-fast -- --list` | 0 | `verification/after_alltargets.txt` | **4,105** |
| 3 | `cargo build --workspace --all-targets` | 0 | `verification/after_build_warnings.txt` | 260 |

整列器の較正は §10.2 手順 3 が要求する形——**未整列の生出力**を序数と `Sort-Object` の 2 通りに整列して digest が割れること——で行った（コミット済みファイルのハッシュ照合では整列器が 1 度も動かないので較正にならない）。**較正 digest は「整列済み配列を LF 1 文字で連結し、UTF-8（BOM 無し・末尾改行なし）でバイト化した SHA256」** であり、コミットされるファイル（CRLF・末尾改行 1 つ）のハッシュとは**別物である**——両者を取り違えないこと。下の較正表と、その次の表（コミット済みファイルの SHA256）は物差しが違う。

| 較正 | 序数（採用） | `Sort-Object`（カルチャ順） | 相違位置 |
|---|---|---|---:|
| `after_default` | `D2F88E82…CA64` | `95C441D9…DE58` | **1,823** |
| `after_alltargets` | `230C7861…B2D3` | `2082705B…6562` | **1,726** |

相違位置 1,823／1,726 は §33.1 が記録した値と一致する（群 8 は 142 本のモジュールパスを変えたが、序数とカルチャの分岐点の分布は変わっていない）。

**コミットされたファイル自体の SHA256**（UTF-8 BOM 無し・CRLF・末尾改行 1 つ・§10.2 手順 5）:

| ファイル | 行数 | バイト | SHA256 |
|---|---:|---:|---|
| `before_default.txt` | 4,790 | 352,588 | `77F03656…D2AD` |
| `after_default.txt` | 4,790 | 365,134 | `75F13A78…A34AB` |
| `before_alltargets.txt` | 4,105 | 295,410 | `3DB65799…B29F` |
| `after_alltargets.txt` | 4,105 | 303,864 | `75EC64C8…5FD2` |

`before_default.txt` のハッシュが §10.2 の記録 `77F03656…D2AD` と**逐語一致**する——本節のハッシュ手順が過去の記録と同じ物差しであることの較正である（基準ファイルは本 spec で 1 バイトも変えていない）。

`after_default.txt` の差分は `git diff --numstat` で **142 / 142**、`after_alltargets.txt` は **128 / 128**。行数は前後とも不変で、変わったのは名前だけである。

### 53.2 対応表の結合と全単射（要件 2.9 / 1.8）— 差分ゼロ

群 8 は 4 つのフラグメントへ追記・新設した（8.3／8.4 が `areka-kanade.csv`、8.5 が `areka-parsers.csv`（新規）、8.6 が `areka-emo-compose.csv`、8.7 が `dola.csv`（新規）。8.1 は `areka.csv`、8.2／8.8 は名前保存ゆえ 0 行）。

```powershell
pwsh -File $V/Test-MappingBijection.ps1 -Path $V/mapping -Out $V/test_name_mapping.csv
```

→ `PASS: 全単射 OK / 行数 1163 / 相異なる old_fqn 1163 / 相異なる new_fqn 1163 / フラグメント 10`・exit 0。

| フラグメント | 行 | | フラグメント | 行 |
|---|---:|---|---|---:|
| `areka.csv` | 372 | | `areka-parsers.csv` | 59 |
| `areka-emo-text.csv` | 239 | | `areka-sakura.csv` | 75 |
| `areka-kanade.csv` | 116 | | `dola.csv` | 24 |
| `areka-emo-compose.csv` | 95 | | `areka-ghost.csv` | 20 |
| `areka-emo-present.csv` | 84 | | `areka-seriko.csv` | 79 |

総和 **1,163**（372+239+116+95+84+59+75+24+20+79）で結合結果と一致。**10 フラグメント**——タスク 7.1 の 8 に対し `areka-parsers` と `dola` が増えている。

道具を信用せず、**対応表の性質を独立に再導出した**（`Import-Csv` で読んで数えるだけの手順）:
...
| `new_fqn` の重複 | **0** |
| 恒等行（旧＝新） | **0** |
| `reason` が `theme_split` でない行 | **0** |
| 末尾セグメント（テスト関数識別子）の旧新相違 | **0** |
| `new_fqn` が別の行の `old_fqn` でもある件数（連鎖写像） | **0** |

### 53.3 リスト照合の三方突合（要件 2.2 / 2.3）— 対称差 0

**(a) 対応表を適用した照合**（`Compare-TestLists.ps1`）:

| 対象 | before | after | 適用 | 未使用 | 判定 |
|---|---:|---:|---:|---:|---|
| 既定列挙 | 4,790 | 4,790 | 1,163 | **0** | **PASS**（exit 0） |
| 全ターゲット列挙 | 4,105 | 4,105 | 791 | 372 | **PASS**（exit 0） |

未使用 372 行は `mapping/areka.csv`（372 行）と**集合として完全一致**（`Compare-Object` の差分 0 を独立に確認）。`--exclude areka` の帰結であり、**2 本のリストを跨いで未使用のまま残る行は 0**（既定列挙の未使用が 0 だから）。ここが 0 でなければ「変わっていない名前を表に載せた」＝最小性違反になる。

**(b) 対応表を使わない独立検算**（多重集合の対称差・`: test` 接尾辞を落として突合）:

| 対象 | REMOVED | ADDED | REMOVED 集合 vs `old_fqn` 集合 | ADDED 集合 vs `new_fqn` 集合 |
|---|---:|---:|---|---|
| 既定列挙 | 1,163 | 1,163 | **完全一致**（両側とも差 0） | **完全一致**（両側とも差 0） |
| 全ターゲット列挙 | 791 | 791 | REMOVED ⊂ `old_fqn`・差は `old_fqn` 側 372 のみ | ADDED ⊂ `new_fqn`・差は `new_fqn` 側 372 のみ |

既定列挙で両集合が**完全一致**することは、対応表が**完全**（登録外の変化が無い）かつ**最小**（変化していない行を持たない）であることの直接証跡である。全ターゲット側の 372 は (a) の未使用行とちょうど同じ集合である。

**(c) doctest**: 既定列挙の doctest 行（`<path>.rs - <item> (line N): test` 形式）は **37 → 37**。群 8 は doctest を 1 本も動かしていない。

### 53.4 本文一致（要件 2.4）— 群 8 の全対象 12 実行と、タスク 7.1 の 49 本再実行

**群 8 の全対象を、各タスク自身の分割前コミットを `-Commit` に与えて再実行した。12 実行すべて exit 0** で、判定文字列も各節の記録を**逐語で再現**した。`$LASTEXITCODE` の生値を載せる（**0 = 一致 / 1 = 不一致 / 2 = 引数不正**。2 を 1 と読み違えないこと）。

| タスク | 元ファイル | base | オプション | files | 判定 | exit |
|---|---|---|---|---:|---|---:|
| 8.1 | `crates/areka/src/emo2_boot/spine.rs` | `09f3b85` | `-WholeFile` | 8 | `test fn 14=14 / helper item 46=46` | **0** |
| 8.2 | `crates/areka-kanade/tests/kanade/common/mod.rs` | `3b8856c` | `-WholeFile -AllowNestedTestModules` | 7 | `4=4 / 54=54` | **0** |
| 8.3 | `crates/areka-kanade/tests/kanade/choice_test.rs` | `5cf4866` | `-WholeFile` | 8 | `10=10 / 19=19` | **0** |
| 8.4 | `crates/areka-kanade/tests/kanade/mouse_test.rs` | `3c34122` | `-WholeFile` | 5 | `11=11 / 8=8` | **0** |
| 8.4 | `crates/areka-kanade/tests/kanade/close_test.rs` | `3c34122` | `-WholeFile` | 4 | `7=7 / 4=4` | **0** |
| 8.5 | `crates/areka-parsers/src/shell/decode_tests.rs` | `7824ba7` | `-WholeFile` | 9 | `59=59 / 0=0` | **0** |
| 8.6 | `crates/areka-emo-compose/src/golden_tests.rs` | `84e73c5` | `-WholeFile` | 7 | `17=17 / 32=32` | **0** |
| 8.7 | `crates/dola/tests/cue/runtime_test.rs` | `35d3f62` | `-WholeFile` | 5 | `24=24 / 10=10` | **0** |
| 8.8 | `crates/areka-ghost/src/shiori_inproc.rs` | `f90fe8f` | `-ModuleName tests` | 1 | `4=4 / 0=0 / mod block 1` | **0** |
| 8.8 | 同上 | `f90fe8f` | `-ModuleName host_tests` | 1 | `4=4 / 0=0 / mod block 1` | **0** |
| 8.8 | 同上 | `f90fe8f` | `-ModuleName adapter_tests` | 1 | `11=11 / 4=4 / mod block 1` | **0** |
| 8.8 | 同上 | `f90fe8f` | （併合・`-ModuleName` 無し） | 3 | `19=19 / 4=4 / mod block 3` | **0** |

**8.8 に `-WholeFile` を使ってはならない**——本ファイルは実在の `#[cfg(test)] mod X { … }` ブロックを持つのでガード① が発火して **exit 2（引数不正）** になる。§48.2 が意図的に踏んで確認済みである。

**タスク 7.1 の 49 本も再実行した。** `verification/task_7_1_body_identity.csv` の 49 行を、各行固有の base コミット・移設先集合・期待 exit・期待 verdict 文字列つきで機械照合——**49/49 が exit・verdict とも一致、不一致 0**。群 8 の 12 実行と合わせて **61 実行がすべて exit 0** である。

**61 個の 0 が「何もせず返った 0」でないことを対照実行で確かめた**（すべてスクラッチパッドの複製に対して行い、リポジトリのファイルは 1 本も書き換えていない。実行後の `git status --porcelain` にリポジトリ側の残骸 0）:

| 対照 | 期待 | 実測 |
|---|---:|---:|
| 移設先を 1 本落とす（`runtime_test_test_support.rs`） | 1 | **1** |
| 存在しない `-OriginalPath` | 2 | **2** |
| 存在しない `-RelocatedPath` | 2 | **2** |
| 木の外の複製でリテラル `102.5` → `102.6` | 1 | **1** |

**8.9〜8.12 は本文一致検証の対象外である。** 4 本とも `#[test]` を 1 つも含まない本番／サンプル本体の分割で、`Compare-RelocatedTests.ps1` は「元ファイルから項目が 1 つも採れないとき exit 2」（§41.1 ガード③）に当たるか、そもそもテスト本文を持たない。純移動の証跡は各タスク自身が独立実装の分解器で取っている——**8.9 §49.2**（27↔27 ／ レビュアー 30↔34・変更行 19・すべて `pub(super)` 付与・字下げ 0）、**8.10 §50.2**（55↔55 ／ レビュアー 57↔57・置換 52／削除 1／追加 59・説明のつかない差異 0）、**8.11 §51.2**（29↔29 ／ レビュアー 30↔30・置換 28／削除 0／追加 67）、**8.12 §52.3**（29↔29・置換 24／削除 0／追加 58）。本節はこれらを再導出していない——**新しい主張を足さず、出所を明示して引き継ぐ**。

### 53.5 全域行数計測（要件 1.7 / 6.3）— **1,000 行超 0 本**

**独立な 2 通りの物差しで測った。走査範囲を明記する**（範囲を書かない計数は再現しない）。

| 方法 | 走査範囲 | `.rs` 本数 | 総行 | **>1,000 行** |
|---|---|---:|---:|---:|
| (a) `git ls-files --cached --others --exclude-standard -- '*.rs'` ＋ `[IO.File]::ReadAllLines().Length` | `target/` は `.gitignore`・`.git/` は git 内部で除外。`vendors/pasta` は**サブモジュールなので superproject の `ls-files` には現れない**。`.claude/worktrees/` は本ワークツリー自身が当該パス配下なので相対パスには現れない。**`.kiro/` を含む**（検証フィクスチャ 8 本） | 880 | 259,329 | **0** |
| (b) `verification/Measure-TestModules.ps1`（無改変・`-OutDir` をリポジトリ外へ逃がして実行） | `target｜vendors｜.git｜node_modules｜.claude/worktrees｜.kiro` を除外 | 872 | 259,154 | **0** |

**2 つの差はちょうど説明がつく**——880 − 872 = **8** は `.kiro/specs/areka-P0-file-slimming/verification/fixtures/relocate/*.rs` の 8 本、259,329 − 259,154 = **175** はその 8 本の総行である。

**`vendors/pasta` サブモジュールは本ワークツリーで populate されている**ので別途測った——`.rs` **160 本・最大 961 行・>1,000 行は 0 本**（`target/` 配下は除外）。これを足しても 0 は崩れない。

> ⚠ `Measure-TestModules.ps1` は `-OutDir` を省くと**自分の置き場（`verification/`）へ書き出し、タスク 1.1 のコミット済みベースライン**（`scan_raw.csv` ／ `target_inventory.csv` ／ `excluded_inventory.csv` ／ `scan_summary.txt`）を上書きする。本節はスクラッチパッドへ逃がして実行し、実行後の `git status --porcelain` にこれら 4 本が現れないことで無傷を確認した（§36.1 と同じ手当て）。

**残る最大ファイル 8 本**（(a)(b) とも同一の並び）:

| 行 | ファイル |
|---:|---|
| 986 | `crates/pilot/examples/pilot-clickthrough-alpha-toggle/main.rs` |
| 974 | `crates/areka-emo-text/src/draw.rs` |
| 962 | `crates/areka/src/main.rs` |
| 960 | `crates/areka-ghost/src/runtime_tests.rs` |
| 950 | `crates/areka/src/placement/follow_visibility_balloon_wiring_tests.rs` |
| 945 | `crates/areka-kanade/src/actor_tests.rs` |
| 942 | `crates/areka-emo-compose/src/plan_ops_tests.rs` |
| 937 | `crates/areka/src/placement/follow_resize_tests.rs` |

**ファイル内テストコードが 500 行を超えるファイルも 0 本**（スキャナの `targets (test_lines > 500) count : 0`・全域の最大は **485 行**）。要件 1.1 の必須対象は消尽している。

**行数の会計**（brief.md の実測表と同じ数）: ファイル数 619 → 793 → **872**、総行 257,134 → 258,342 → **259,154**。群 8 が足した 79 本は A エントリ 79 と一致し（`git diff --name-status 09f3b85 HEAD -- 'crates/**/*.rs'` は **A=79 / M=13 / D=0**）、M の 13 本は §39.4 の 13 本と**集合として完全一致**する。

「テストコード」の計上は 3 時点とも同一定義（ファイル内 `#[cfg(test)] mod` ブロック ＋ `#[cfg(test)]` ＋ パス属性で接続された分離済みファイルの総行）で **29,389 ＋ 81,052 ＝ 110,441**、本番本体は 259,154 − 110,441 ＝ **148,713**。パス接続は **216 宣言・216 実体・欠損 0**（うち `#[cfg(test)] mod` ブロックを内側に持つものは **0 本**＝二重計上なし）。群 7 着地時点（164 本 69,591 行）からの **+11,461** は群 8 が新設した 52 本の総行にちょうど一致し、ファイル内ブロックの **−702** は `shiori_inproc.rs` の 446 ＋ `common/mod.rs` の `mod smoke` 256 に一致する。**テストコードが +10,759 に見えるのは増えたからではなく、旧定義が本番本体に数えていたテストコードが可視化されたからである**（§39.2 の見落とし出所 ①②）。

### 53.6 ワークスペース全緑（要件 2.1）— exit 0・失敗 0

`cargo test --workspace --no-fail-fast` は **exit 0**。**85 ユニット・4,757 passed / 0 failed / 33 ignored / 0 measured / 0 filtered out**。`test result: FAILED` 行 **0**・`failures:` 行 **0**。**1 回目の実行で全緑であり、再実行による flake 救済は行っていない（再試行 0 回）。**

「速すぎる緑」でないことを §34.2 と同じ物差しで確認した:

- stderr の `Running`／`Doc-tests` 行 **85** と stdout の `test result:` 行 **85** が 1 対 1
- **85 ユニットすべてで `filtered out` が 0**
- `--exclude` を使っていない（あれは全ターゲット列挙専用）

**列挙との独立な突合**: `passed + ignored = 4,757 + 33 = 4,790` が `before_default.txt` ／ `after_default.txt` の行数 4,790 と一致する。タスク 8.13 が**列挙**で示した総数不変（要件 2.2）を、**実行**の側から独立に裏づけたことになる。

### 53.7 警告非増加（要件 2.6）— **判定は 5 数値の逐語一致では行わない**

`cargo build --workspace --all-targets` は exit 0。§10.5 の集計手順（正規表現・単複両対応の `warnings?` を含む）を再現した結果は **15 / 7 / 21 / 6 / 15**（`NET == DIAG_COUNT` の健全性チェックは成立）。

**この 5 数値は基準の 16/7/22/6/16 と逐語一致しない。それは退行ではない。** §47.10（8.7 のレビュー・`cargo clean` からの完全再ビルドで 14/6/20/6/14）・§48.6（実装者 15/7/21/6/15・レビュアー 14/4/14/0/14・基準 16/7/22/6/16 の 3 者 3 様）・§49.7・§50.6 の **4 度の観測**が示すとおり、`linker stdout: ライブラリ … を作成中` は **MSVC が import library を新規作成したときだけ出る**ので、5 数値は `target/` の増分ビルド状態に依存する。**`before_build_warnings.txt` を採取したときの `target/` 状態は再現できない**（基準は 2026-08-07 の状態で、以後 62 コミットぶんのビルドが走っている）。

**したがって判定条件は「DIAG 行の多重集合が基準の部分集合であり、新規行が 0 件」である**（§47.10 が定め §48.6 が明文化した規則）。`linker stdout` 行はワークツリー絶対パスと日本語ロケールの MSVC 出力を含むため、多重集合の突合ではこの 1 種へ正規化する（§10.5 の「比較は本文一致ではなく件数」と同じ扱い）。

| DIAG 行（正規化後） | 基準 `before_build_warnings.txt` | 本節の実測 |
|---|---:|---:|
| `"cdecl" is not a supported ABI for the current target` | 6 | **6** |
| `unused std::ops::ControlFlow that must be used` | 4 | **4** |
| `enum CommandConsumer is never used` | 1 | **1** |
| `enum LedgerError is never used` | 1 | **1** |
| `struct ConsumerLedger is never constructed` | 1 | **1** |
| `associated items new, try_register, consumer_of, and canonical are never used` | 1 | **1** |
| `linker stdout: ライブラリ … を作成中`（import library 生成通知） | 2 | **1** |
| **合計** | **16** | **15** |

**新規行 0 ／ 欠落 1（減少方向）／ 部分集合 → 判定 PASS（増加ゼロ）。** 欠落した 1 行は `shiori.dll` の import library 生成通知で、ユニット別内訳の唯一の差（`shiori-host32-testdll` (lib) が generated 4 → 3）と同一の出所である。残る 6 ユニットの `generated` 件数は基準と**差分 0**。**別の退行が都合よく隠されている余地は無い**——欠落は 1 行だけで、その本文が特定できているからである。

### 53.8 命名規約の逆向き導出（要件 3.2）と steering の是正

`crates/` 全域の**同一ディレクトリ** `#[path]` 宣言（属性値にディレクトリ区切りを含まないもの）**223 件**へ、`structure.md:164` の最長 stem 規則を機械適用した:

- **真の宣言元を復元 203 件・誤解決 0 件**
- 残る **20 件**は前置一致する候補 stem がそもそも存在しない形（`mod.rs` stem 14 ＋ `shiori-host-32` の歴史的形式 6）で、規則の適用対象外である

**機械証跡の分母は 223 ではなく 203 である。** 除外した 20 件のうち **14 件は `mod.rs` stem のケース**（`crates/areka/src/input_events/mod.rs` 1・`crates/areka/src/placement/mod.rs` 4・`crates/areka-kanade/src/schedule/mod.rs` 2・`crates/areka-kanade/tests/kanade/common/mod.rs` 7。例: `common/mod.rs` → `common_harness.rs`）で、**これらは `structure.md:159` の読み替え規則（`bar/mod.rs` の `<stem>` は親ディレクトリ名 `bar`）が現に解決する**——本節のスキャナが「前置一致する候補 stem が存在しない」と分類したのは、宣言元の basename `mod` をそのまま stem に採った保守的な扱いによる。独立に親ディレクトリ名を stem として当て直すと **14/14 が解決する**（残る 6 件は `crates/pilot/examples/shiori-host-32/` の歴史的形式で、これは規則の対象外のままである）。したがって結論（誤解決 0）は変わらないが、**要件 3.2 に対して機械適用が実際に走った母数は 203/223 であり、223/223 ではない。**

> 件数は §47.10（222 件・例外 23 件）・§48.1（225 件）と一致しない。**分類の定義が違うためである**——本節は「同一ディレクトリ宣言」を属性値にパス区切りを含まないものと定め、「例外」を「前置一致する候補 stem が 1 つも無いもの」と定めている。§47.10 自身が「§46.1 が 22 件と記録したのとの差 1 は当時の分類差」と記しているとおり、この系列の数は定義に敏感である。**判定に効くのは誤解決 0 のほうであり、そこは 3 者とも一致している。**

**§45.4 の申し送りを消化した。** `structure.md:164` は最長 stem 規則の候補集合を「同一ディレクトリに実在する**接続宣言を持ちうるファイル**」と規範化したうえで、括弧書きで「`src/` なら本番ファイル、`tests/` なら統合テストのファイル」と言い換えていた。**この言い換えを字句どおり読むと誤解決する**ことを実測で示した:

| `decode_tests_alias_tests.rs` の逆引き | 候補集合 | 最長 stem | 結果 |
|---|---|---|---|
| 規範文どおり（接続宣言を持ちうる全ファイル） | `decode_tests`, `decode` | `decode_tests` | **正しい**（`decode_tests` ＋ `alias_tests`） |
| 言い換えを字句どおり（本番ファイルのみ） | `decode` | `decode` | **誤り**（`decode` ＋ `tests_alias_tests`） |

是正は言い換えを**候補集合の列挙**へ置き換えた——`src/` は本番ファイル**および歴史的形式で既に分離されているテストファイル**（`decode_tests.rs`・`golden_tests.rs`・`spine.rs` を実例として明記）、`tests/` は統合テストのファイル、`examples/` はサンプルのルートファイル。**「本番ファイル」と読み替えないこと**を明示し、上表の誤解決を反例として書き込んだ。

**§32.10 の intra-doc の 1 行はタスク 7.3 で既に着地済みだった**（本節の編集後で `structure.md:252`・編集前は `:245`——「`super::`／相対の intra-doc リンクは指す先が変わる。doc コメントは項目本文の内側にあるため、直すと『本文がバイト単位で不変』という純移動の証明が崩れる。`cargo doc` はゲートではないので**一律未修正が正解**」）。**追加は不要。** 確認したうえで何もしていない。

### 53.9 Implementation Notes の E0433 アンカーの訂正（§51.4 の申し送り）

Implementation Notes は出所を `crates/areka/src/placement/spawn.rs:879` と書いていたが、**そのアンカーは本 spec 自身のタスク 5.2（`f64fac1`）の純テスト移設で動いている**。HEAD で実機再確認した:

```
> cargo test -p areka --examples --no-run          # exit 101
error[E0433]: cannot find `input_events` in `crate`
   --> crates\areka\examples\..\src\placement\spawn_assembly_tests.rs:183:12
error: could not compile `areka` (example "window-placement" test) due to 1 previous error
error: could not compile `areka` (example "collision-probe" test) due to 1 previous error
```

**エラーは 1 種のみ・落ちるのは `window-placement` と `collision-probe` の 2 本のみ**（`emo-present` は exit 0）。**同じ欠陥が別の file:line へ動いただけ**であり、群 8 の本体分割（8.11／8.12）に帰属しない。`tasks.md` の当該項目へ、原文を取り消し線で残したうえで訂正を折り込んだ。§37.1 #2 は既に訂正後のアンカーを持っている（§37.3 #2 で是正済み）ので触っていない。

### 53.10 境界のはみ出しと登記（要件 5.1 / 5.2 / 5.3）

- **他 spec の文書は 1 バイトも触っていない。** `git diff --name-only 247d48a HEAD -- '.kiro/specs/'` のうち `.kiro/specs/areka-P0-file-slimming/` の外にあるものは **0 件**。`.kiro/steering/` で触れたのは `structure.md` **1 本のみ**（タスク 7.3 と本タスク 8.13）。要件 5.3 充足。
- 群 8 が触れた `crates/**/*.rs` は **A=79 / M=13 / D=0**。M の 13 本は §39.4 の 13 本と集合として完全一致し、**それ以外の本番・テストファイルには手が届いていない**。
- **ブランチ全体（`git diff --name-status -M 247d48a HEAD -- 'crates/**/*.rs'`）では A=253 / M=61 / D=0 / R=0。** 3 つとも辻褄が合う: 追加 253 ＝ 群 1〜6 のテストファイル 164 ＋ 本体分割サブモジュール 10 ＋ 群 8 の 79。変更 61 ＝ 群 1〜6 の必須対象 49 ＋ 群 8 の 13 − **重複 1**（`crates/areka-emo-present/src/presenter.rs` はタスク 4.3 のテスト分離と 8.9 の本体分割の両方で触れた唯一のファイル）。**削除 0・リネーム 0 は要件 3.5 が問う「本番ファイル自体のパス変更」が全期間で 1 件も無かったことの直接証跡である。**
- 群 8 が触れた 92 本に `TODO` ／ `FIXME` ／ `HACK` ／ `TBD` は **0 件**。
- **送付所見を 8 件 → 9 件へ拡張した**（§37.1 に 9 行目を追加・§37.6 に適合の評価を追記）。9 件目は §50.7（タスク 8.10）が出した**群 8 唯一の新規送付所見**で、送り先は他の 8 件と同じ `areka-P0-test-cage-determinism`（W6.9）である。全アンカーを HEAD で再解決した——`crates/areka-seriko/src/actor_dispatch_tests.rs:705`（`#[test]`）・`:706`（`fn non_shell_broadcast_reception_is_benign_debug_no_warn_error`）・`:713`（`capture_logs_flow` 呼出）・`:741`（失敗した `assert!`）、`crates/areka-seriko/src/actor_test_support.rs:39`／`:50`（ハーネス実体）・`:48-49`（「スレッドローカル `with_default` ゆえ並行テスト安全」と逐語で主張する doc）、反証された判定は `notes.md:1617`（§16.5 #1）。**いずれも HEAD で逐語に解決する。**
- **群 8 が出した新規送付所見はこの 1 件だけである。** 8.1（§40.7）・8.3（§42.9）・8.4（§43.8）・8.5（§45.7）・8.6（§46.7）・8.7（§47.9）・8.8（§48.7）・8.9（§49.8）・8.11（§51.7）・8.12（§52.7）は全て「新規送付所見 0 件」と登記している。**水増ししない。**

### 53.11 要件 41 基準の再判定

§38.2 は 41/41 COVERED と判定したが、**§39.3 が指摘したとおり要件 1.7 の判定が発動条件（移設先）だけを見て宣言（1 ファイルの行数）を見ていなかった**。本節は 1.7 を**宣言のほうで**判定し直す。

| 要件 | 判定 | 証跡 |
|---|---|---|
| 1.1 テストモジュール 500 行超の全ファイルを外へ | **充足** | 全域再走査でファイル内テストコード 500 行超は **0 本**（最大 485）。§53.5 |
| 1.2 必須対象を設計時に全数再計測 | 充足 | タスク 1.1（49 本）。**ただし絞り込み条件そのものに欠陥があった**——§39.2 ① |
| 1.3 複数テストモジュールの集約 | 充足 | §13〜§30・8.8 の 3 モジュール（§48） |
| 1.4 既分離ファイルは**移設**対象外（**分割は対象**） | **充足** | 明確化後の読みで `spine.rs`（8.1）・`decode_tests.rs`（8.5）・`golden_tests.rs`（8.6）を分割。§53.5 の 0 本 |
| 1.5 ≤500 行の任意移設 | 充足 | 裁定 #10 は「行わない」。例外は 8.8（要件 1.7 のために併せ移設・§48）。**この例外は `design.md` §裁定一覧 #10 の行と §Non-Goals の第 2 改訂注記（旧「500 行以下のファイルの移設」）・§Requirements Traceability 1.5 行へ本タスクで折り込み済み** |
| 1.6 非 `mod` の `#[cfg(test)]` 項目の一貫処理 | 充足 | 残置で一貫。全域 **40 件**（スキャナ診断・§38.2 の 40=40 と同数） |
| **1.7 1 ファイル 1,000 行以下（本番・テスト双方）** | **充足** | **独立 2 方式で >1,000 行 0 本**（880 本／872 本）。`vendors/pasta` を足しても 0。§53.5 |
| 1.8 対応表による突合 | 充足 | §53.2・§53.3 |
| 2.1 `cargo test --workspace` 全緑 | **充足** | exit 0・85 ユニット・4,757/0/33・`FAILED` 0 行・再試行 0 回。§53.6 |
| 2.2 テスト総数一致 | **充足** | 4,790 = 4,790 ／ 4,105 = 4,105 ／ 実行側 `passed+ignored` = 4,790。§53.1・§53.6 |
| 2.3 前後の証跡採取 | **充足** | `before_*` は無変更・`after_*` 3 本を再採取（SHA256 付き）。§53.1 |
| 2.4 テストコード本文不変 | **充足** | 群 8 の 12 実行 ＋ タスク 7.1 の 49 本再実行 ＝ **61/61 exit 0**・4 通りの対照で弁別能力を確認。8.9〜8.12 は各節の純移動証明。§53.4 |
| 2.5 公開 API 不変 | 充足 | 群 8 で lib クレートの公開面に触れうるのは 8.9 と 8.8。8.9 は rustdoc JSON **734 行 ↔ 734 行バイト同一**＋到達可能 `pub` **21 = 21**（§49.3）、子は全て私有 `mod`（`presenter.rs:48-53`・子に `pub mod` **0 件**——本節で再確認）。8.8 は可視性付与 **0 件**の純移設で `areka-ghost/src/lib.rs` は**バイト不変**（`pub mod shiori_inproc;` `:31`・`pub use shiori_inproc::inproc_connect;` `:48` が健在）。サンプル 3 本はクレート外公開面を持たない。加えて `cargo build --workspace --all-targets` **exit 0** が全下流の呼出を通している |
| 2.6 警告非増加 | **充足** | DIAG 多重集合が基準 16 行の**真部分集合・新規行 0**。§53.7 |
| 2.7 本番挙動不変 | 充足 | 8.9〜8.12 の純移動証明（説明のつかない差異 0）＋全緑。`fn main` は 8.10／8.11／8.12 とも原本とバイト一致（§50.5・§51.2・§52.1） |
| 2.8 コンパイル不能は可視性・import で解決 | 充足 | 群 8 の可視性付与はすべて `None → pub(super)`・広がり 0・格下げ 0（§49.2・§50.2・§51.2・§52.3）。テストロジックの書き換え 0 |
| 2.9 改名は全単射・関数識別子不変 | **充足** | 1,163 行・重複 0/0・恒等 0・連鎖 0・末尾セグメント相違 **0**。§53.2 |
| 3.1 移設方式は単一裁定 | 充足 | 群 8 も同一方式。§42.4 の差し戻し（`_tests` 接尾辞欠落）は是正済みで再発 0（§43.3） |
| 3.2 テストファイルのパスを規則から一意に導出 | **充足** | 同一ディレクトリ `#[path]` 223 件へ最長 stem 規則を機械適用し**誤解決 0**。§45.4 の言い換えの穴は本節で塞いだ。§53.8 |
| 3.3 in-crate 配置の維持 | 充足 | 外部 `tests/` への移動 0。群 8 の分割はすべて同一クレート内 |
| 3.4 候補方式の対比と裁定根拠 | 充足 | design §裁定 #1 |
| 3.5 本番ファイル自体のパス変更の一覧 | **充足（空振り）** | 裁定した方式は本番ファイルのパスを変えない。群 8 の 8.9〜8.12 はサブディレクトリを**新設**したが、`presenter.rs`・`emo-text-layer.rs`・`emo-present.rs`・`collision-probe.rs` は元の場所に残っている（M エントリであって R エントリではない）。**パスが変わった本番ファイルは全期間で 0 本** |
| 4.1 `follow.rs`／`frame.rs` の本体分割 | 充足 | タスク 6.1／6.2（§31・§32） |
| 4.2 分割後 1,000 行以下の目安 | **充足** | 群 8 の本体分割は最大 783（`emo-text-layer/drive.rs`）。全域 0 本 |
| 4.3 公開面と可視性の維持・呼出側はパス追随のみ | 充足 | Cargo ターゲット数は前後不変（examples 23・emo-text 2）・`src_path` 不変（§50.5・§51.3・§52.5）。クレート外の呼出側 0 行変更（§49.3） |
| 4.4 関数分割・責務移動・ロジック書き換えの禁止 | 充足 | 純移動証明で「説明のつかない差異 0」（4 タスクとも実装者・レビュアーの独立実装で二重取り） |
| **4.5（改訂後）1,000 行超の全ファイルを分割** | **充足** | 13 本すべて消化。>1,000 行 **0 本**。§53.5。**`design.md` 側も本タスクで追随させた**——§Overview の「2 本のみ」・§Non-Goals の 3 項（`follow`／`frame` 以外の本体分割・500 行以下の移設・テストモジュールを持たない巨大テストファイルの分割）・§Requirements Traceability 4.5 行を、`requirements.md` 4.5 と同じ取り消し線＋改訂注記の形で改訂 |
| 4.6 テーマ単位で複数ファイルへ | 充足 | `follow` 12 本・`frame` 9 本（§27〜§29） |
| 5.1 ハーネス一本化・汚染是正・時刻シーム変更をしない | 充足 | §37.5 の多重集合証明（群 1〜6）。群 8 は 8.1〜8.8 がテスト本文バイト不変、8.9〜8.12 はテストコード非接触 |
| 5.2 所見を file:line 付きで登記し所有 spec へ送る | **充足** | **9 件**（§37.1・本節で 9 件目を追加）。全アンカーを HEAD で再解決 |
| 5.3 他 spec の brief を書き換えない | **充足** | `.kiro/specs/` のうち本 spec 外は **0 件**。§53.10 |
| 5.4 着手時に他 spec の実装ブランチが 0 本 | 充足 | §6（2026-08-07 実測） |
| 5.5 衝突時は対象から外す | **充足（空振り）** | 着手後に他 spec の実装が同一ファイルへ着地した事実は無い |
| 5.6 正典ブランチ | 充足 | `claude/areka-p0-file-slimming-64d065`（本 HEAD） |
| 6.1 steering に新規テストモジュールの置き場 | 充足 | `structure.md:146`（「新規のテストモジュールは本番ファイルの中に本体を書かない」） |
| 6.2 steering に命名規約 | **充足** | `structure.md:144-183`（Unit Tests 節・本節の編集後の行番号）。導出規則・逆引き 2 規則・1 モジュール 1 ファイル・1,000 行目安・共有ヘルパ集約・E0428／E0659 回避まで含む。本節で `:164` の候補集合の言い換えを是正（§53.8） |
| 6.3 brief の実測表を更新 | **充足** | 本節の数値で全面更新。§53.5 の値と 1 対 1 |
| 6.4 更新前と更新後を対比可能な形で残す | **充足** | 全域表を **更新前／群 7 着地／更新後の 3 列**へ拡張し、起票時の原表は `<details>` に逐語保存のまま。旧「残る理由」表は削除せず**「却下された理由」＋出所（§39.2 の ①②③）＋担当タスク＋分割後の最大**を付けて書き直した |
| 7.1 クレート単位の論理コミット | 充足 | マージベースから **62 コミット**（群 8 で +15） |
| 7.2 各コミット前にクレート緑を確認 | **証跡は記録のみ** | §38.7 (5) と同じ——27＋13 の実装コミットを順に checkout し直せば検証できるが破壊的 git になるため行わない。§13〜§30・§40〜§52 の記録に依る。**HEAD の全緑は再現したが、それは各コミット時点の状態を遡って証明するものではない** |
| 7.3 テスト分離と本体分割のコミットを分ける | 充足 | 6.1／6.2／8.9〜8.12 はいずれも独立コミット |

**41 / 41 が充足。GAP 0。** 完全な証跡を持たないのは 7.2 の 1 件だけで、これは §38 の時点から性質が変わっていない**既知の記録依存**である（新たな未達ではない）。

### 53.12 §38 の GO を再判定する

§39 で撤回された GO を、上表と §53.1〜§53.10 の実測に基づいて再判定する。**撤回の唯一の理由は「1,000 行を超えて残った 13 本が要件違反である」であり、その 13 本は群 8 で 0 本になった。**

```
DECISION: GO

WITHDRAWAL_CAUSE_CLEARED:
  §39 の撤回理由（1,000 行超 13 本＝17,450 行）は消尽した。
  独立な 2 方式（git ls-files 880 本 / スキャナ 872 本）でいずれも >1,000 行 0 本。
  vendors/pasta（160 本・最大 961）を足しても 0。残る最大は 986 行。
  ファイル内テストコード 500 行超も 0 本（最大 485）。

MECHANICAL:
  Tests   PASS  cargo test --workspace --no-fail-fast exit 0 /
                85 ユニット / 4,757 passed / 0 failed / 33 ignored / filtered out 0 /
                再試行 0 回
  Lists   PASS  4,790 = 4,790 / 4,105 = 4,105 / doctest 37 = 37 /
                passed+ignored = 4,790 が列挙と一致
  Mapping PASS  1,163 行・10 フラグメント・全単射 /
                重複 0/0・恒等 0・連鎖 0・末尾セグメント相違 0
  Bodies  PASS  61/61 exit 0（群 8 の 12 ＋ タスク 7.1 の 49 再実行）/
                対照 4 通りで 0・1・2 を弁別
  Warnings PASS DIAG 多重集合が基準 16 行の真部分集合・新規行 0・欠落 1（減少方向）
                ※ 5 数値は 15/7/21/6/15。逐語一致は判定条件ではない（§47.10・§48.6）
  Cleanliness   TODO/FIXME/HACK/TBD 0 件（群 8 が触れた 92 本）

COVERAGE: 41/41 充足・GAP 0
          （要件 1.7 は今回**宣言のほう**で判定した——§39.3 が指摘した検証の穴を塞いだ）

DESIGN:   **逸脱あり・すべて 2026-08-09 の要件改訂（§39.1）で authorised 済み。**
          §38 の「逸脱なし」は誤りだった——`requirements.md` は §39 で改訂されたのに
          `design.md` は design フェーズの `362f72e` のまま放置され、群 8 の成果と
          文面が矛盾していた（第 3 の死角）。本タスクで `design.md` を改訂した。

          Non-Goals からの逸脱 3 件（いずれも要件 4.5 改訂・要件 1.4 明確化が授権）:
            (a) 「`follow.rs`・`frame.rs` 以外の本番本体分割」        → 8.9／8.10／8.11／8.12
            (b) 「テストモジュールが 500 行以下のファイルの移設」      → 8.8（設計判断 #10 の唯一の例外）
            (c) 「テストモジュールを持たない巨大テストファイルの分割」 → 8.3／8.4／8.7
          Requirements Traceability からの逸脱 1 行:
            4.5 「2 本以外の本体分割禁止／§Non-Goals（presenter.rs 1,042 行等は分割しない）」
                → 改訂後の 4.5（1,000 行超の全ファイルを分割）へ差し替え
          §Overview の「2 本のみ」も同裁定で取り消した。

          遵守した裁定: #1（移設方式＝案 C）・#2（1 テストモジュール＝1 テストファイル）・
          #5（D1 ファサード再輸出）・#13（冗長 `#[cfg(test)]` を残す）・
          §44.4 の命名 3 規則。**#10 のみ例外 1 件（8.8）**。
          本番ファイルのパスを変えた実績は全期間で 0 本（要件 3.5 は空振り）。

BOUNDARY: はみ出しなし。他 spec の文書 0 件・steering は structure.md のみ・
          群 8 の crates 変更は A=79 / M=13 / D=0 で M は §39.4 の 13 本と完全一致。
          **この判定は `design.md` §This Spec Owns の【改訂後】の所有一覧に対して行っている**
          ——テストモジュールの位置＝必須対象 49 ＋ 併せ移設 1（8.8）＝ 50 ファイル、
          本番本体のファイル内訳＝`follow.rs`・`frame.rs` ＋ 群 8 の 13 本。
          是正 2 巡目まで §This Spec Owns は「49 ファイル」「`follow.rs`・`frame.rs`」の
          ままで、群 8 の 13 本を所有と宣言していなかった。**古い一覧に対する
          「はみ出しなし」は空虚な真であり、`DESIGN: 逸脱なし` と同型の欠陥だった。**
          §Out of Boundary（5 項）は改訂していない——群 8 はその 5 項のいずれにも触れていない。

OWNERSHIP: LOCAL。送付所見は 9 件（8 → 9・9 件目は §50.7）。
           いずれも登記された先送りであって本 spec の欠陥ではない。

BLOCKED_TASKS: なし（30/30 ＋ 群 8 の 13/13 完了）
```

**GO に添える申し送り（いずれも非ブロッキング・§38.7 からの更新）:**

1. **§38.7 (1)（`spine.rs` 2,503 行）は解消した。** リポジトリ最大は **986 行**（`crates/pilot/examples/pilot-clickthrough-alpha-toggle/main.rs`）で目安の内側である。
2. **`cargo doc` の未解決 intra-doc リンクは群 8 でも増えた。** 各節が記録した実測は、**8.6 が 3 本**（§46.7(2)）・**8.9 が真に新規 3 件**（§49.8(1)・lib doc 警告 13 → 16）・**8.11 が 3 件**（§51.6・未解決 0 → 3）・**8.12 が真に新規 1 件**（§52.6・差引 +1）であり、**8.5／8.7／8.8／8.10 は 0**、8.1／8.2／8.3 は「別モジュールを指すようになった」と記録するのみで診断は 0 件である。**本節は総数を再計測していない**——群 1〜6 ぶんの 95 件（§37.4 注）と単純に足せる数ではなく、足した数を載せれば §46〜§52 が繰り返し戒めた「再現しない中間値」になるからである。純移動のバイト一致証明と両立しないため一律未修正としたのは正しく steering にも記録済みだが、**どのゲートも見ていない恒久的な品質劣化**であることは変わらない。**測るときは必ず `--document-private-items` を付けること**——素の `cargo rustdoc` は 8.11 で「未解決 0」（真値 3）、8.12 で「見出し 6 件」（真値 35・**29 件の過小報告**）という偽の緑を出した（§51.6・§52.6）。
3. **`crates/areka` の examples は test モードで既存 E0433 のまま**（§53.9 で HEAD 再確認・`window-placement` と `collision-probe` の 2 本）。本 spec が持ち込んだものではなく登記・送付済みだが、**当該 examples のテストターゲットにはコンパイル被覆が一切無い**状態が続く。
4. **所見 #4（`PURE_SOURCES` の被覆縮小）は依然として所有者不在**である。`areka-emo-text` 系の spec は全て `completed/` にある。§38.4 で実害ゼロを独立確認済みなので急がない。
5. **要件 7.2 と要件 2.5 の 2 件は、ブランチ全体を機械 diff した成果物を持たない**（7.2 は §38.7 (5) から変化なし）。7.2（各コミット前のクレート緑）は §53.11 の表のとおり記録依存である。**2.5（公開 API 不変）を 7.2 と並べて挙げるのは本節が初めてで、これまで 7.2 だけを挙げていたのは非対称だった**——2.5 が持つのはタスク単位の証跡（8.9 の rustdoc JSON 734↔734 バイト同一・8.8 の `areka-ghost/src/lib.rs` バイト不変・`cargo build --workspace --all-targets` exit 0）であって、**マージベース `247d48a` と HEAD の公開面をワークスペース全体で機械 diff した成果物ではない**。8.9 の rustdoc JSON も分割の前後（群 8 の 1 タスク内）で採ったものであり、ブランチ全体を覆っていない。強い根拠ではあるが、7.2 と同じ「記録と部分証跡に依る」区分に属する。
6. **`Test-VerificationTools.ps1` は恒久的に exit 1 を返す**（`TOKENIZER-EQUIV`・§33.6 の裁定）。本節で再実行し **23/24**（`TOKENIZER-EQUIV` のみ赤）を確認した。判定は残る 23 ケースで行うこと。**不一致数は 49 → 50（§44.9(1)）→ 51 になっている。** その中身を独立に再導出したところ、**この赤はそれ自体が要件 1.1 の全域証跡だった**（§33.6 と同じ性質）——基準 `scan_raw.csv`（619 ファイル）の `modules` 列を現行スキャナの出力と突合すると、**変化したのはちょうど 51 ファイル**で、その内訳は `target_inventory.csv` の必須対象 **49 本** ＋ `crates/areka-ghost/src/shiori_inproc.rs`（8.8）＋ `crates/areka-kanade/tests/kanade/common/mod.rs`（8.2）である。**51 件すべて現在値が空文字＝ファイル内テストモジュールが完全に消えており**、残る **568 ファイルは基準の `modules` 列をそのまま再現する（想定外の巻き添え 0）**。退行ではない。
7. **9 件目の送付所見（§37.1 #9）の再現率は低い**（多数回の実行のうち 1 度）。**「再現しないから欠陥でない」と読まないこと**——同じ根は「ログが出ないこと」を主張する assert を空虚に真にする側にも回るので、偽の赤だけでなく**偽の緑**を出しうる。§37.6 に受理側への注意として明記した。

8. **要件 2.2 の計測器には構造的な死角がある——`crates/areka` の examples の中のテストは、2 本のスナップショットのどちらにも現れない。** 台帳はこれまで**原因**（examples の test モードが既存 E0433 で落ちるので全ターゲット列挙を `--exclude areka` で運用している——§37.1 #2・§53.9）だけを記録し、**不変量にとっての帰結**を書いてこなかった。ここで書く。

   - 既定列挙 `cargo test --workspace -- --list` は **example のテストターゲットをビルドしない**（`--all-targets` を付けたときにだけ examples が列挙対象になる）。
   - 全ターゲット列挙 `cargo test --workspace --exclude areka --all-targets -- --list` は **`crates/areka` を丸ごと除外している**（§10.2 / §53.1 の手順 2）。
   - **したがって `crates/areka` の examples に書かれたテストは、既定列挙にも全ターゲット列挙にも入らない。** 具体的には `crates/areka/examples/mock-shell.rs` の **`#[test]` 22 個**（実測。`grep -c '#\[test\]'` ＝ 22。`tokens_single_line_is_one_text_token`・`shell_image_asset_exists`・`drag_at_extreme_virtual_screen_coords_does_not_overflow` を代表に取って `before_default.txt` / `after_default.txt` / `before_alltargets.txt` / `after_alltargets.txt` の 4 本すべてで **hit 0** を確認した）が、**どのリストにも載らず、どの全緑にも走っていない**。要件 2.2 の「前後で総数が完全一致」は、この 22 本については**そもそも何も主張していない**。

   **実害は無い。** 群 8 が `crates/areka` の examples で触れたのは `emo-present.rs`（8.11）と `collision-probe.rs`（8.12）の 2 本で、**両者とも `#[test]` は 0 個**（実測: ルートファイル 2 本＋分割後の子ファイル 8＋7 本の計 17 パスすべてで `#[test]` 0。`crates/areka/examples/` 配下で `#[test]` を含むファイルは `mock-shell.rs` ただ 1 本）。**`mock-shell.rs` は本 spec が全期間で 1 バイトも触っていない**。そして 8.11／8.12 の無変更性は列挙ではなく各節の**純移動証明**（§51.2・§52.3 のバイト一致・フラグメント分解）が担っている。**したがって死角があること自体は正しく、死角の中で何かを失った事実は無い。**

   受け入れ側への申し送りとしては次の 2 点である——(i) 将来 `crates/areka` の examples にテストを足す spec は、**このリストが自分のテストを数えていない**ことを知ったうえで別の証跡を用意すること。(ii) 死角を塞ぐ唯一の道は E0433 の是正（送付先 `areka-P0-test-cage-determinism` W6.9・§37.1 #2）であり、それが済むまで `--exclude areka` を外せない。

### 53.12.1 是正 1〜2 巡目のあとの再言明（2026-08-10）

上の判定ブロックは、レビューで 2 度差し戻された指摘（1 巡目 Critical 1・その他 3／2 巡目 Critical 1・Important 2・任意 4）を織り込んで書き直したものである。**何が変わり、何が変わらなかったかを混ぜずに書く。**

**変わったこと（文書の側だけ）:**

1. `design.md` が改訂された。§39 の要件改訂に**誰も追随させていなかった**——`requirements.md` は 4.5 を取り消し 1.4 を明確化したのに、`design.md` は design フェーズの `362f72e` のままで、Non-Goals の 3 項と Requirements Traceability の 4.5 行が群 8 の成果と正面から矛盾していた。**これは §38 → §39 で撤回された GO と同じ形の失敗である**（文書の宣言を額面どおり受け取った）。`/kiro-complete` の DoD ゲートはこれらの文書を読むので、放置は次の偽の緑を用意することに等しかった。
2. 判定ブロックの `DESIGN:` 行が「逸脱なし」から「**逸脱あり・すべて §39.1 の要件改訂で authorised**」へ変わった。逸脱は Non-Goals 3 件＋対応表 1 行で、遵守した裁定は #1／#2／#5／#13、**#10 だけが例外 1 件（8.8）**である。旧文は #10 を黙って落としていた。
3. 申し送りに 2 件足し、1 件を対称化した——(8) 要件 2.2 の計測器が `crates/areka` の examples を数えない死角（`mock-shell.rs` の 22 本。**実害 0** の根拠つき）、(5) 要件 2.5 を 7.2 と並べて「ブランチ全体の機械 diff を持たない」区分へ移した。§53.8 には要件 3.2 の機械適用の母数が **203/223** であって 223/223 ではないことを明記した。
4. **是正 2 巡目: `requirements.md` の `## Boundary Context` を改訂した。** 1 巡目で `design.md` を直したが、**同じ取り消し済み文言が 1 つ上流の `requirements.md:38-39` に残っていた**——In scope が本番本体分割の対象を `follow.rs`・`frame.rs` の 2 本に限り、Out of scope が「`follow.rs`・`frame.rs` 以外の本番本体分割」を除外していた。§39 は 4.5 と 1.4 の本文だけを改訂し、**同じ命題を写していた Boundary Context には触れていなかった**。`/kiro-complete` の DoD ゲートはこの節を読む。同節の「テストモジュールが 500 行以下のファイルの**必須**移設」は**取り消していない**——除外しているのは必須移設だけで、要件 1.5 が許容する任意の併せ移設（8.8）までは禁じていないからである。この読みを注記に明記した。
5. **是正 2 巡目: 「移設対象の外延は 49 本」の 4 つ目の出現（`design.md` §移設方式の裁定の末尾）を改訂し、§Overview の注記に移設側の例外を 1 文足した。** 1 巡目は §Non-Goals・§裁定一覧 #10・§対応表 1.5 行の 3 箇所を注記したが、§移設方式の裁定の括弧書きを取りこぼしていた。実績の外延は **50 本**（49 ＋ 8.8）。
6. **是正 2 巡目: `design.md` §This Spec Owns を改訂した。** 「必須対象 49 ファイル」「`follow.rs`・`frame.rs` の本番本体」は群 8 が所有した 13 本を含んでおらず、**`kiro-review` の Boundary Audit と上の `BOUNDARY:` 判定はこの一覧に対して下される**。古い一覧に対する「はみ出しなし」は空虚な真であり、`DESIGN: 逸脱なし` と同型だった。所有表と 13 本の全数を注記に入れ、`BOUNDARY:` にも「改訂後の一覧に対する判定である」旨を明記した。§Out of Boundary は改訂不要（群 8 はその 5 項に触れていない）。
7. **是正 2 巡目・任意指摘の消化**: §Modified Files に群 8 の 13 本を追記（計画の記録として原文は保存）／§設計時点で確定している登記・送付所見の E0433 行へ HEAD アンカー（`spawn_assembly_tests.rs:183`・§53.9）の相互参照を追記／§Non-Goals 第 2 注記の要件 1.5 の引き方を「一貫性のために」から「1.5 は必須としないと定めるだけで禁じておらず、併せ移設という**手段**を授権する条項。**動機**は要件 1.7 が供給した」へ書き改め、§裁定一覧 #10 の「一貫性のための任意移設は依然 0 件」と両立させた／本節の日付を 2026-08-10 に是正。

**変わらなかったこと（機械証跡は 1 ビットも動いていない）:** テスト実行 4,757/0/33・列挙 4,790 = 4,790 ／ 4,105 = 4,105・対応表 1,163 行の全単射・本文一致 61/61・>1,000 行 0 本・警告の真部分集合。**再採取も再判定もしていない**（レビュアーが独立に再現済みで、文書の是正は成果物のバイトに触れない）。

```
DECISION (再言明): GO

GO が依って立つもの（この 4 つ以外に無い）:
  1. 三不変量の機械証跡——テスト総数一致（列挙 2 方式＋実行側 passed+ignored の 3 通り）・
     本文一致 61/61・全単射対応表 1,163 行。いずれも独立に再現済み。
  2. 要件 41 基準 41/41 充足・GAP 0（要件 1.7 は**宣言のほう**で判定し直した——§53.11）。
     完全な証跡を持たないのは 7.2 と 2.5 の 2 件で、いずれも既知の記録依存である。
  3. 設計からの逸脱は**すべて 2026-08-09 の開発者裁定（§39.1）が明示的に授権したもの**であり、
     その授権が `requirements.md` と `design.md` の**両方で、本文・境界節・対応表まで
     一貫して**書かれている状態になった。**是正 1 巡目の時点ではこの項はまだ偽だった**
     ——`requirements.md` の 4.5 は改訂済みでも同書の `## Boundary Context` が
     取り消し済みの禁止を保持しており、同一文書の中で矛盾していた（2 巡目で消化）。
     現在、取り消された命題「本番本体分割は `follow.rs`・`frame.rs` の 2 本のみ」は
     両文書のどこにも無注記では残っていない。
  4. 境界のはみ出し 0（他 spec の文書 0 件・steering は structure.md のみ）。
     **この判定は `design.md` §This Spec Owns の改訂後の所有一覧に対するものである**
     （50 ファイル＋`follow`・`frame`＋群 8 の 13 本）。2 巡目までは古い一覧に対する
     空虚な真だった。

GO が依っていないもの（誤読を防ぐため明示する）:
  - 警告 5 数値の逐語一致（判定条件は DIAG 多重集合の部分集合性——§47.10・§48.6）
  - `cargo doc` の未解決 intra-doc リンク（申し送り 2・どのゲートも見ていない）
  - `crates/areka` examples のテスト被覆（申し送り 3・8・E0433 のため列挙にも全緑にも入らない）
  - 各コミット時点のクレート緑（要件 7.2・記録依存）

是正 2 巡分で見つかったものは**すべて文書の欠陥**であり、成果物のバイトに触れる
指摘は 1 件も出ていない（A〜E の機械証跡はレビュアーが独立に 2 度再現している）。
判定を変えるべき事由は見つからなかった。**GO を維持する。**
```
...
| ファイル | 内容 |
|---|---|
| `verification/after_default.txt` | 再採取（4,790 行・142/142 差替） |
| `verification/after_alltargets.txt` | 再採取（4,105 行・128/128 差替） |
| `verification/after_build_warnings.txt` | 再採取。**[JUDGEMENT RULE] と [SUBSET CHECK] の 2 節を新設**——5 数値の逐語一致を判定条件と誤読させないため |
| `verification/test_name_mapping.csv` | 1,021 → **1,163 データ行**（10 フラグメントの結合） |
| `.kiro/specs/areka-P0-file-slimming/brief.md` | 全域実測表を 3 列（更新前／群 7 着地／更新後）へ拡張。旧「1,000 行超 13 本の残る理由」表を「却下された理由＋§39.2 の出所＋担当＋分割後の最大」へ書き直し。上位ファイル表の `presenter.rs`／`spine.rs` を更新。`## Current State` の現在形を是正 |
| `.kiro/steering/structure.md` | `:164` の最長 stem 規則の候補集合を、言い換え（「本番ファイル」）から**列挙**へ置き換え、`decode_tests_alias_tests.rs` の誤解決を反例として明記（§45.4 の消化） |
| `.kiro/specs/areka-P0-file-slimming/tasks.md` | Implementation Notes の E0433 アンカーを訂正（原文は取り消し線で保存・§51.4 の消化）。**チェックボックスには触れていない** |
| `verification/notes.md` | 本節（§53）を追記。あわせて **§37.1 へ 9 行目を追加**（見出しを「全 8 件」→「全 9 件」・#1〜#8 の本文は無変更）、**§37.1 直後の総括文と §37.6 へ #9 のぶんを追記** |
| `.kiro/specs/areka-P0-file-slimming/design.md` | **是正 1 巡目で追加・2 巡目で拡張。** §39 の要件改訂に追随していなかった箇所を `requirements.md` 4.5 と同形（取り消し線＋改訂注記）で改訂。1 巡目 6 箇所＝§Overview の「2 本のみ」／§Non-Goals の 3 項／§裁定一覧 #10 の行／§Requirements Traceability の 1.5・4.5 行。2 巡目 5 箇所＝§Overview 注記へ移設側の例外 1 文／§This Spec Owns の所有一覧（49→50・`follow`/`frame`→＋群 8 の 13 本）／§移設方式の裁定の「外延は 49 本のみ」／§Modified Files への 13 本追記／§送付所見の E0433 行への HEAD アンカー相互参照。あわせて §Non-Goals 第 2 注記の要件 1.5 の引き方を是正 |
| `.kiro/specs/areka-P0-file-slimming/requirements.md` | **是正 2 巡目で追加。** `## Boundary Context`（`:38-39`）が §39 の要件改訂に追随しておらず、取り消し済みの「`follow.rs`・`frame.rs` 以外の本番本体分割」を In scope／Out of scope の両方に保持していた。4.5 と同形の取り消し線＋改訂注記で是正。**Requirement 本文（1.4・4.5 を含む）は 1 バイトも触っていない**——本タスクが触れたのは `## Boundary Context` の 2 行とその直後に足した注記のみ |

**是正 1 巡目（レビュー差し戻しの消化）で `verification/notes.md` に加えた変更:** §53.8 へ「機械適用の母数は 203/223」の 1 段落、§53.11 の 1.5／4.5 行へ `design.md` の改訂箇所への参照、§53.12 の `DESIGN:` 行の全面書き直し、申し送り (5) の対称化（2.5 を 7.2 と並置）と (8) の新設（`crates/areka` examples の計測死角）、**§53.12.1 の新設**（再言明）。

**是正 2 巡目で `verification/notes.md` に加えた変更:** §53.12 の `BOUNDARY:` へ「判定は改訂後の所有一覧に対するもの」の 6 行、§53.12.1 の見出し日付の是正（2026-08-09 → 2026-08-10）・「変わったこと」へ 4〜7 の追加・GO 依拠 3／4 の書き直し（3 は**是正 1 巡目の時点ではまだ偽だった**ことを明記）・末尾に「2 巡分の指摘はすべて文書の欠陥」の 2 行、本表への `requirements.md` 行の追加。

**2 巡を通じて、証跡ファイル（`after_*`・`before_*`・`test_name_mapping.csv`）・`crates/**/*.rs`・`brief.md`・`.kiro/steering/structure.md`・`tasks.md` のチェックボックスには触れていない。** A〜E の機械証跡は再採取していない。

**是正 3 巡目（親が直接実施・2026-08-10）で加えた変更:** レビュー 3 巡目が挙げた 2 件はいずれも再測定を要さない誤記だったため、親が直接是正した。

1. **「合計 16,450 行」を「17,450 行」へ訂正（5 箇所）**——`verification/notes.md:6858`（§39.4・原典。取り消し線＋訂正注記の形で保存）・同 `:8327`（§53.12 `WITHDRAWAL_CAUSE_CLEARED`）・`brief.md:60`・`design.md:57`・`design.md:414`。**13 本の個別行数はすべて正しく、総和のみ 1,000 を落としていた**（レビュアーが分割前ブロブから全数実測: 2,503＋1,657＋1,563＋1,434＋1,395＋1,356＋1,168＋1,116＋1,102＋1,066＋1,063＋1,018＋1,009 ＝ **17,450**。親も独立に検算した）。判定には影響しない——1,000 行超 0 本は総和に依存しない——が、**開発者向けの `brief.md` と GO ブロックに載る数字**であり、本 spec が繰り返し戒めてきた計数誤りそのものである。`brief.md` の実測表本体（872／259,154／110,441／148,713／986／0／0）と 13 行の「分割後の最大」列・本数列はレビュアーが全数再測定して**すべて正しい**——誤っていたのはこの 1 つの総和だけだった。
2. **`requirements.md:11`（`## Project Description (Input)`）へ注記を追加**——同段落には取り消し済みの「**2 本のみ**」と「`follow.rs`／`frame.rs` **以外の本番本体分割は範囲外**」が無注記で残っており、§53.12.1 の GO 依拠 3 の締めの一文（「両文書のどこにも無注記では残っていない」）を偽にしていた。`(Input)` 節は起票時の逐語記録ゆえ**本文は 1 文字も書き換えず**、同節が `:9` で既に確立している作法（`> 注（2026-08-07 実測による是正）`）に倣って注記だけを足した。これにより当該一文は限定を要さずそのまま真になる。`git diff -U0` はハンク 2 つ（`@@ -12,0 +13,2 @@` と `@@ -38,2 +40,8 @@`）で、**Requirement 本文（1.x〜7.x）には 3 巡を通じて 1 行も届いていない。**

**3 巡を通じて、証跡ファイル（`after_*`・`before_*`・`test_name_mapping.csv`）・`crates/**/*.rs`・`.kiro/steering/structure.md`・`tasks.md` のチェックボックスは非接触である。** レビュー 3 巡が挙げた指摘は Critical（`design.md` の失効した Non-Goals）→ Critical（`requirements.md` Boundary Context の同型残存）→ 誤記 2 件、と単調に縮小し、**成果物のバイトに触れる指摘は 1 件も出なかった**。機械証跡 A〜E はレビュアーが全数を独立に再現しており、3 巡のいずれでも再採取していない。

> **アンカーずれの申し送り（レビュー 3 巡目の指摘・是正作業自体が生んだもの）**: 是正 1〜3 巡目で `design.md` は **559 → 583 行（+24）**になり、**本 spec 内の `design.md:NNN` 引用 11 アンカー／10 箇所が全数ずれた**（`tasks.md:375` → `:123` ／ `tasks.md:397`・`notes.md:6036` → `:177` ／ `notes.md:725` → `:532` ／ `:6007` → `:190-196` ／ `:6198` → `:211` ／ `:6460`・`:6500` → `:476-484` ／ `:7044` → `:151` ／ `:7181` → `:134`・`:239`）。**各引用文が併記する節名のほうを正として読むこと。** 手で直さない理由は 4 つ——いずれも判定を担わない文脈ポインタで三不変量にも 41 基準にも効かない／引用文が例外なく節名を併記している／**他 spec から本 spec の `design.md` を行番号で指す文書はリポジトリ全域に 0 件**（レビュアーが走査確認）でずれは本 spec 内に閉じ完了時に `completed/` へアーカイブされる／10 箇所の手直しは新たな誤りを持ち込むリスクのほうが大きい。

## 54. 最終ゲート `/kiro-validate-impl` の結果（2026-08-10・HEAD `3c0adae`）

タスク 8.13 の自己判定とは**独立に**、3 軸を並列のサブエージェントで検証した。**3 軸とも GO 差し止めなし。**

### 54.1 機械チェック（A〜D＋警告）——全 PASS

`cargo test --workspace --no-fail-fast` exit 0・**85 ユニット・4,757 passed / 0 failed / 33 ignored**（5 数値とも基準に逐語一致・**1 回目で緑・再試行 0**。§50.7 に登記した `areka-seriko` の揺らぎは再現しなかった）。TODO 類は `.rs` 全域で **0 件**（14 件の一致はすべて `notes.md` 内の「0 件である」と述べる散文）。秘密情報 **0 件**。警告は DIAG 多重集合が基準 16 行の**真部分集合・新規行 0**（欠落 1 ＝ 既知の MSVC import library 通知）——§47.10 の判定条件の **5 度目**の確認。

**実行時生存性は MANUAL に落ちなかった**——リポジトリに既に自動ブート検査がある。`crates/areka/tests/smoke_boot_loop_exit.rs` が `env!("CARGO_BIN_EXE_areka")` で**ビルド済みバイナリを子プロセスとして起動**し、`AREKA_APP_SMOKE_EXIT_MS=500` で有界終了させ、`status.success()` と tracing 出力の経路マーカーを検査する。2 本とも 1.63 秒で緑（`skeleton_boots_loops_and_exits_zero_within_watchdog` ／ `skeleton_boots_with_real_ghost_windows_and_exits_zero`）。**コンパイル検査の代用ではなく本物の起動証跡である。**

> **記録**: `crates/areka/tests/emo2_real_run.rs:131-137` の `emo2_real_run_boots_talks_and_exits_zero` は `AREKA_EMO2_REAL_RUN=1` が無ければ即 `return` する既定 OFF のテストで、**何も起動しないまま 4,757 の緑に 1 行を数えられている**。本 spec 以前からの設計で本 spec は触れていないが、この `ok` を起動の証跡と読んではならない。

### 54.2 要件被覆——**第 4 の盲点は無い。むしろ自己申告の穴が 1 つ埋まった**

§39.3 が記録した失敗の形（「やったことが正しいか」だけを測り「やるべきことが全部done か」を測らない）を、スコープ境界・列挙の死角・Cargo ターゲット探索・doctest 命名・ignore 状態・マニフェスト差分の 6 方向から探した。**すべて台帳を再現し、新たな穴は出なかった。**

独立に書いたスキャナで単位まで一致した値: 1,000 行超 **0 本**（880 / 872 の 2 通り・差 8 ＝ `.kiro` フィクスチャ・`vendors/pasta` 160 本最大 961 を足しても 0）／ファイル内テスト 500 行超 **0 本**（最大 485）／非 `mod` `#[cfg(test)]` **40 件**／対応表 **1,163 行**（重複・恒等・連鎖・末尾セグメント相違すべて 0）／三方向照合の対称差 **0**（4,790=4,790・4,105=4,105）／Cargo ターゲット **23=23**（`Cargo.toml` の変更が全域 **0** であることも確認）／`git diff --name-status -M` が **R=0・D=0**（本番ファイルのパスは一度も変わっていない）。

**新たに得た証跡 4 件**（台帳が持っていなかったもの）:
1. **要件 2.5 のブランチ全体機械 diff**——`crates/` 全域の素の `pub` 項目宣言の多重集合が **2,428 → 2,449 で削除 0**、追加 21 はすべてファサードの `pub use`。`pub mod` は **176 = 176**（新しい公開パス 0）。分割した 6 つの親の module-level `pub` 項目は HEAD で全数が親から到達可能（follow 15/15・frame 11/11・presenter 3/3）。**`lib.rs` はブランチ全域で 1 本も変更されていない。** §53.12 の申し送り 5 が「存在しない」と書いた当の diff であり、**要件 2.5 はこれで充足と再分類してよい**。
2. **要件 2.7／4.4 のブランチ全体の本番残余 diff**——意図的な分割親でない 55 本のうち、`#[cfg(test)] mod` ブロックと接続宣言を剥がすと **44 本がバイト一致**、9 本は群 8 のテーマ分割ファサード（`-WholeFile` 実行が被覆済み）、残る 2 本（`areka-emo-text/src/choice.rs`・`areka/src/emo2_boot/move_cue.rs`）の差は**区画バナーのコメント 65 行だけで、全数が兄弟テストファイルへ移動したことを追跡済み**（喪失 0・新設 0）。§46.3 のバナー移動が群 8 の外でも無損失であることの確認。
3. **要件 2.2 の死角を肯定的に閉じた**——`#[test]` の総数がブランチ両端で **4,826 = 4,826**、`#[ignore]` が **7 = 7**。§53.12 の申し送り 8 は「実害は見つからない」という否定形だったが、静的計数で**テストは 1 本も増えても減っても無効化されてもいない**と積極的に言える。死角の境界も確定した——`--all-targets` は **`areka` 以外**の examples のテストは列挙している（`crates/pilot` の 6 件が `after_alltargets.txt` に在り `after_default.txt` に無いことで確認）。
4. **doctest は隠れた改名経路ではない**——doctest 名はファイルパスと行番号を埋め込むため、分割が対応表の外でこっそり名前を変えうる。`before_default.txt` と `after_default.txt` の doctest 37 行は**双方向で差集合が空＝バイト一致**。

残る記録依存は要件 **7.2** の 1 件のみ（各コミット時点のクレート緑。62 コミットの逐次チェックアウトが要り破壊的 git になる）。

### 54.3 設計整合と境界——PASS。ただし文書の衛生 4 件を是正した

`#[path]` は `crates/` 全域 **367 件中 366 件が解決**（唯一の未解決は doc コメント内の偽陽性 `hit_region.rs:51`）。同一ディレクトリ 223 件へ最長 stem 規則を当てると **217 解決・誤解決 0・候補なし 6**（`shiori-host-32` の歴史的形式）。`decode_tests_alias_tests.rs` は **8.13 改訂後の候補集合でのみ**正しく解ける（改訂前の「本番ファイルのみ」では `decode` ＋ `tests_alias_tests` と誤読する）——§45.4 の是正が実効であることの独立確認。

ファサードの再輸出は 4 つとも公開項目集合が完全一致（follow 19/19・frame 10+1/11・presenter 3/3・`common/mod.rs` 35/35）で**名前の落ちも可視性の広がりも 0**。裁定 #1／#2／#5／#10／#13 はすべて遵守（テスト関数識別子の多重集合が **4,797 = 4,797**・#10 の例外はちょうど 1 件で 8.2 の `mod smoke` は `7824ba7^` の時点で既に存在した名前の保存＝正しく本体分割に分類）。§44.4 の 3 規則も群 8 の 52 ファイルで機械確認（`#[test]` を含むのに `_tests` でないのは `common_smoke.rs` の 1 本だけ＝規則 1 の名前保存）。

**§This Spec Owns の改訂後の一覧は空虚な真ではなくなった**——`target_inventory.csv` の 49 行 ∪ 群 8 の 13 本 ＝ **宣言 61**、`git diff --diff-filter=M` の実測 **61**、**対称差は両方向とも 0**。`§Out of Boundary` の 5 項も全数独立確認（とくに項目 1: 全域の非空行多重集合で削除 610／追加 2,429 のうち **416 の削除が可視性前置つきの HEAD 行と 1 対 1 で対応**し、残る 194 は `mod X {`／`}` の枠と `use` の再編で、**テスト本文の内容変更は 0**）。

**是正した 4 件**（いずれも文書のみ・`.rs` 非接触）:
1. ⚠ **steering `structure.md:251` の因果の記述が誤っていた——これが最も価値がある。** 「lib ターゲットを持たない bin クレートでは `pub use` 再輸出が `unused_imports` を出す」と書いてあったが、**決めるのは lib の有無ではなく「その再輸出に非 test ビルドの消費者が居るか」だけ**である。タスク 8.9（§49.4）が「子が `use super::{…}` で引けば 0 件」を確立し、8.10〜8.12 が lib を持たないサンプルバイナリで 3 度再現したのに、steering はその前の理解のまま**恒久ルールとして固定していた**。しかも 8.9 の理解も片面で、**`follow.rs:60,62,68` には子が全員 `use super::{…}` を使っているのに 3 文の `#[allow(unused_imports)]` が残っている**——テスト・examples のための**外向き**再輸出は内向きの形では消えないからである。内向き／外向きを分けて書き直した。**spec より長生きする記述なので完了前に直す価値があった。**
2. `tasks.md:371` の §11.4 アンカー `window_pos.rs:1137` は**本 spec のタスク 2.1 自身が移設して存在しなくなっていた**（同ファイルは HEAD で 447 行）。現在の所在 `window_pos_tests.rs:691` と較正用の既知ケースを追記。§51.4 で訂正した E0433 アンカーと**構造的に同型**の陳腐化である。
3. `tasks.md:397` の `design.md:177` 参照が 8.13 自身の改訂注記でずれていた（現 `:199`）。訂正。
4. `tasks.md:375` の「本文の是正は完了時にまとめて判断する」が**未決のまま完了しようとしていた**。裁定を下した——**起票時文書は原文のまま残す**（`research.md`・`requirements.md:27,36,94`・`design.md:143`）。訂正値は §2.2／§2.3 と当該注記に file:line つきで登記済みで、必須対象 49 本に無影響。起票時の判断根拠を後から書き換えると、なぜその設計になったかが追えなくなる。

**非阻害の申し送り 3 件**（`/kiro-complete` 後の読み手向け）: (i) `design.md §File Structure Plan`（`:257-337`）と `§Revalidation Triggers`（`:80`）には 8.13 の改訂注記が付いておらず、前者は `follow/`・`frame/` の 2 サブツリーしか示していない（授権は `§Modified Files`・`§This Spec Owns` の注記が持つ）。(ii) `common/mod.rs` は 7 つの接続宣言のうち `mod smoke` だけが `#[cfg(test)]` を持ち、6 つの責務モジュールは持たない——裁定 #5（8.2 は本体分割）と §44.4 規則 3 の下では妥当だが、`structure.md:142` を `tests/` 木の分割へ当てる読み手は別の形を作る。(iii) `spec.json` の `phase` は `tasks-generated` のまま（`/kiro-complete` が進める）。

### 54.4 判定

**DECISION: GO。** 撤回された §38 の GO と違い、今回は**三不変量・境界・要件被覆のすべてが、spec 自身の道具ではなく独立に書かれたスキャナで再現されている**。§39.3 が指摘した「やったことが正しいか」しか測っていない状態は解消され、「やるべきことが全部 done か」の側——全域行数計測・`#[test]` 総数・`pub` 宣言の多重集合・Cargo ターゲット探索・doctest 集合——も機械証跡を持つ。
