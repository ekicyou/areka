...
> 注（2026-08-09 開発者裁定による改訂・`verification/notes.md` §39）: 本段落の「**2 本のみ**」および「`follow.rs`／`frame.rs` **以外の本番本体分割は範囲外**」は取り消されている。Requirement 1.7 の宣言——「本 spec の基準は『1 ファイルの行数』であって『本番ファイルの行数』ではない」——が上位であることが確認され、Requirement 4.5 が改訂・Requirement 1.4 が明確化された（`## Boundary Context` および 4.5 の改訂注記を参照）。**現行の運用規則は「出自を問わず 1,000 行を超える全ファイルの本番本体を分割する」**であり、`follow.rs`・`frame.rs` は Requirement 4.1 が個別に指定する 2 本である。実作業はタスク 8.1〜8.12。本節は起票時の逐語記録ゆえ本文は書き換えず、`:9` の前例に倣い注記で訂正する。
...
- **In scope**: areka リポジトリの全 Rust ソースファイル（全クレートの `src/`・`tests/`・`examples/`・`benches/`・`build.rs`）のうちテストモジュールが 500 行を超えるもののテスト分離／1,000 行を超えるテストファイルのテーマ単位分割／~~`crates/areka/src/placement/follow.rs` と `crates/areka/src/emo2_boot/frame.rs` の本番本体分割~~ **→ 1,000 行を超える全ファイルの本番本体分割**（`follow.rs`・`frame.rs` は Requirement 4.1 が個別に指定する 2 本）／移設方式の単一裁定と一貫適用／新規テストモジュールの配置規律の steering 明文化／移設後の行数実測の brief 更新／移設前後のテスト総数一致の証跡採取。
- **Out of scope**: テストコードの内容変更・追加・削除／テストハーネスの一本化・共有化・テスト間の状態汚染の是正・時刻注入シームの変更（`test-cage-determinism` W6.9 の領分）／~~`follow.rs`・`frame.rs` 以外の本番本体分割~~／関数分割・責務移動・ロジック書き換えを伴うリファクタ／他 spec の brief に記載された file:line アンカーの更新／テストモジュールが 500 行以下のファイルの**必須**移設。

  **【改訂・2026-08-09 開発者裁定】上記 2 行のうち、本番本体分割の範囲を定めた 2 箇所を改める。** 本節は Requirement 4.5 の改訂（本書 4.5 の改訂注記）と Requirement 1.4 の明確化（同 1.4）に**追随していなかった**。裁定の内容は同一である——Requirement 1.7 の宣言「**本 spec の基準は「1 ファイルの行数」であって「本番ファイルの行数」ではない**」が上位であることが確認され、4.5 の「`follow.rs`・`frame.rs` 以外の本番本体を分割しない」は取り消され、1.4 の除外は**移設**のみに掛かり**分割**には掛からないことが明確化された。したがって:

  - **In scope の本番本体分割は「1,000 行を超える全ファイル」である**（出自——`src/`／`tests/`／`examples/`、既分離か同居か——を問わない）。1,000 行以下の本番本体を分割しない点は変わらない。
  - **Out of scope の「`follow.rs`・`frame.rs` 以外の本番本体分割」は取り消す。** この一文は改訂前の 4.5 を写したものであり、4.5 の取り消しと同時に効力を失う。
  - **Out of scope の「テストモジュールが 500 行以下のファイルの**必須**移設」は取り消さない。** この一文が除外しているのは**必須**移設だけであり、Requirement 1.5 が明示的に許容する**任意の併せ移設**までは禁じていない。実際にタスク 8.8 が `crates/areka-ghost/src/shiori_inproc.rs`（1,018 行）のテストコード 446 行（閾値未満）を、Requirement 1.7 の 1,000 行目安を本番本体の側で満たすために 1.5 の併せ移設として外へ出しており、これは本行と矛盾しない（`design.md` §裁定一覧 #10 の改訂注記に同旨）。
...
4. Where テストモジュールが既に本番ファイルの外に在る（例: `crates/areka/src/emo2_boot/spine.rs`＝親の `#[cfg(test)] mod spine;` で外部から gate されている）, the file-slimming 実装 shall 当該ファイルを**移設**対象から除外し、そのまま維持する。

   **【明確化・2026-08-09 開発者裁定】本項が除外するのは「移設」だけであって「分割」ではない。** 実装時に本項を「1,000 行超でも手を触れない」と読んだが、それは誤読である（`spine.rs` 2,503 行・`decode_tests.rs` 1,395 行・`golden_tests.rs` 1,356 行が手つかずで残った）。既に分離済みのファイルは**移す先が無いだけ**であり、要件 1.7 の 1,000 行目安は他のテストファイルと同様に適用される。この読み方により本項と 1.7 は矛盾せず両立する。
...
5. ~~The file-slimming 実装 shall `follow.rs`・`frame.rs` 以外のファイルの本番本体を分割しない（本番本体 500〜1,000 行の水準は健全と判定済み）。~~
   ...
   **【改訂・2026-08-09 開発者裁定】上記は取り消す。** 実装完了後の検証で、**1,000 行を超えて残ったファイルが 13 本**あることが判明した。本項の「500〜1,000 行の水準は健全」という前提は、**1,000 行を超える本番本体には最初から当てはまらない**（`areka-emo-present/src/presenter.rs` 1,066 行・examples 3 本 1,434／1,168／1,063 行）。要件 1.7 が「**本 spec の基準は「1 ファイルの行数」であって「本番ファイルの行数」ではない**」と宣言している以上、本項が 1.7 を上書きするのは誤りである。

   **改訂後**: The file-slimming 実装 shall **1,000 行を超える全ファイル**の本番本体を、責務単位のサブモジュールへ分割する。`follow.rs`・`frame.rs` は Requirement 4.1 が個別に指定する 2 本であり、本項はそれ以外の該当ファイルにも同一の分割規律（4.2〜4.4 の D1 ファサード形式・公開面不変・純移動）を適用する。1,000 行以下の本番本体は引き続き分割しない。
