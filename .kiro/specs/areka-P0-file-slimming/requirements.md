...
**檻が 500 行を超えるファイルは全域で 49 本**（檻合計 68,921 行）。内訳は `src/` の 48 本（檻合計 66,830 行・brief 実測表の 16 本はその部分集合）＋ `crates/areka-ghost/tests/ghost/spine_e2e_test.rs` 1 本（2,574 行・檻 2,091 行・檻 10 個）である。
...
2. The file-slimming 実装 shall 檻分離の必須対象一覧を設計時にリポジトリ全域で全数再計測して確定する。2026-08-07 の全域実測では **49 ファイル**（檻合計 68,921 行）が該当し、`src/` の 48 本＋`crates/areka-ghost/tests/ghost/spine_e2e_test.rs` である。設計時の再計測がこれと乖離する場合は差分の理由を示す。
