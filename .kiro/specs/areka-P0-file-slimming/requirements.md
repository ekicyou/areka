...
**現状**: areka のソースファイルが肥大している（最大 8,472 行＝`crates/areka/src/placement/follow.rs`）。実測の結果、肥大の 7〜8 割は in-file の檻（`#[cfg(test)]` テストモジュール）であり、本番ロジック単体は大半が 500〜1,000 行で健全。問題は檻と本体が同一ファイルに同居する構造そのものにあり、(1) spec が檻を本体の途中に挿入すると後続行の全アンカーがずれる（実例＝`collision-dpi-hittest` PR#100 のマージコミット `ce86995` が `crates/areka/src/input_events/balloon.rs` を +183/-27 で改変し、`bindoption-exclusivity` brief の監視アンカーが +155 ドリフト）、(2) 干渉台帳の「同一ファイル異ハンク」衝突を増幅する、(3) 4,000〜8,000 行のファイルはエディタ・diff・レビューの全てで扱いづらい、という 3 つの税を構造的に発生させている。
...
対象規模は 2026-08-07 の全数実測で確定している: `crates/*/src/**/*.rs` 387 ファイル・総 189,190 行のうち、`#[cfg(test)] mod {...}` ブロックが占めるのは 92,591 行（48.9%）。うち**檻が 500 行を超えるファイルは 48 本**（当該 48 本の檻合計 66,830 行・檻 1,000 行超は 26 本）。この 48 本が檻分離の必須対象であり、brief 実測表の 16 本はその部分集合である。

移設先の形式には既存前例がある。`crates/*/src/` 配下には `#[path = "..."]` 形式は 1 件も無く、素の `#[cfg(test)] mod <name>;`（宣言のみ・実体は別ファイル）が **26 宣言ファイル・52 箇所**で既に使われている（`areka-parsers`・`dola/runtime`・`wintf/ecs`・`crates/areka/src/emo2_boot/mod.rs` の `spine` 等）。未採用なのが檻の重いクレート群（`areka`・`areka-emo-*`・`areka-kanade`・`areka-sakura`・`areka-seriko`）である。
...
3. When 1 つのファイルが複数の `#[cfg(test)]` 檻を持つとき（本体の途中に挿入された interleaved 檻を含む）, the file-slimming 実装 shall それらを当該ファイル専用の同一のテストファイルへ集約する。
...
4. The file-slimming 実装 shall 檻の内容——テスト名・アサーション・入力値・期待値・属性・コメント——を変更しない。移設に伴い機械的に必要となる `use`／可視性／モジュール接続の調整のみを許容する。
...
4. When 移設方式を裁定するとき, the file-slimming 実装 shall 候補方式を対比したうえで裁定根拠を記録する。対比には少なくとも、`crates/*/src/` に既に 52 箇所（26 宣言ファイル）存在する素の `#[cfg(test)] mod <name>;` 前例と、`crates/*/src/` に既存前例が 1 件も無い `#[path]` 指定形式の双方を含める。
