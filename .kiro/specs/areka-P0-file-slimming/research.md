...
## 8. 設計裁定記録（design フェーズ・2026-08-07）

> 本節は design.md 生成時の最終裁定を記録する。§7 の設計判断 #1〜#13 はすべて裁定済み。前提となる開発者裁定（要件ディスカッション 2026-08-07）: 動機は「1 ファイルの行数が大きすぎることそのもの」（#9 再裁定なし）／対象はリポジトリ全 Rust ソース／**1,000 行目安はテストファイルにも等しく適用**（#12 は選択肢 (i) 系＝テーマ分割必須で確定）／検証はテスト名一致でなく本数一致＋本文一致＋全単射対応表／正典ブランチは `claude/areka-p0-file-slimming-64d065`（重複ワークツリー e4f098 は開発者指示で破棄済み＝R-5 解消）。

### Decision: 移設方式は案 C（`#[path]` フラット兄弟・常時明示）— #1

- **代替案**: A（素の `mod tests;`＋`foo/tests.rs`）は E0583 実測により `tests/` ツリーで単独不成立＝要件 3.1 違反で脱落。B（`foo/mod.rs` 化）は本番パス変更 44 本＋入口 1 編集＋`mod.rs` 増殖。
- **選択**: C。接続規約 `#[cfg(test)] #[path = "<stem>_<モジュール名>.rs"] mod <モジュール名>;`（同一ディレクトリ）。stem＝basename／`mod.rs`→親ディレクトリ名／`main.rs`・`lib.rs`→そのまま。モジュール root でも `#[path]` を常に明示（単一文言）。
- **根拠**: 3.1 を文言レベルで満たす唯一の案・本番パス変更 0 本（3.5 の一覧は空）・`#[path]` は `tests/` 115 箇所＋steering L135 の既定形式。
- **Trade-offs**: `src/` への `#[path]` 新規導入と steering L142-145 の書き換え（6.2 の作業に吸収）。既存分離済み 55 宣言は 1.4 により維持し揃え直さない（R-1 解消）。

### Decision: 集約単位・テーマ分割・残置 — #2/#3/#4/#12/#13

- **#2**: 複数テストモジュール 8 本は 1 テストモジュール＝1 テストファイル（同一ディレクトリの `<stem>_*` 名前空間）。入れ子集約はしない＝この 8 本は FQN 完全保存。
- **#3**: 非 `mod` `#[cfg(test)]` 40 件は全数残置（移設は `mod` ブロックのみ）。`fail_next_render` は `test-cage-determinism` へ所見送付。
- **#4/#12**: 1,000 行超のテストファイルはテーマ単位分割（対象＝再計測で **24 テストモジュール**。集約ファイル基準 27 本のうち choice.rs・schedule/mod.rs・spine_e2e_test.rs の 3 本はモジュール個別化のみで解消）。新モジュール名 `<テーマ>_tests`・共有ヘルパは `<stem>_test_support.rs`・関数識別子は不改名・旧→新 FQN 全単射対応表を証跡化。follow/frame のテーマは本体シームに対応（4.6）。僅少超過で自然な境界が無い場合のみ単一維持を許容し理由記録。
- **#13**: 統合テストツリー内の冗長 `#[cfg(test)]` は残す（属性不変・計測一貫性）。R-6 の `wintf/src/ecs/world/mod.rs` は対象外につき是正しない（記録のみ）。

### Decision: 本体分割は D1 ファサード再輸出 — #5

- follow.rs → ファサード＋5 サブモジュール（anchor/drag_follow/window_move/work_area/visibility）、frame.rs → ファサード＋5 サブモジュール（attach/wiring/dpi/scale_text/drain_resnap）。呼び出し側 26＋10 箇所は 0 変更・公開 API 完全不変。
- 制約: placement 木は `crate::` 不使用（examples `#[path]` include）／`Emo2Wiring` は impl（`#[cfg(test)]` メソッド 5 件）ごと wiring.rs へ一体移動／`#[allow(dead_code)]` は項目に同伴。
- **R-3 解消（design 時 grep 実測）**: 旧 target `areka::placement::follow`／`areka::emo2_boot::frame` の完全一致判定はコード中に 0 件（完全一致は分割対象外の `areka::placement::diag` のみ・`diag.rs:371`）。実行時フィルタと完了済 spec 手順書の directive はすべて前置一致で子モジュール target にもマッチ＝挙動・観測互換は保たれる。

### Decision: 証跡と検証 — #6/#7、R-4/R-7/R-8/R-9/R-10

- **#6**: 3 本立て採取（既定 `--list`／`--exclude areka --all-targets --list`／`build --workspace --all-targets`）＋ソート済み名前リストの機械比較。テーマ分割分は対応表適用後に一致判定（行数一致が 2.2、名前一致が 2.9 を担保）。R-4 は解消済（doctest 名不変）。
- **#7（R-7）**: 空白非依存（lstrip 正規化）比較スクリプト `Compare-RelocatedTests.ps1` を成果物に含める。置き場は spec 配下 `.kiro/specs/areka-P0-file-slimming/verification/`（`scripts/` は不在・spec ライフサイクルに属するため）。
- **R-8**: 警告基準は移設前スナップショットの `cargo build --workspace --all-targets` 警告集計（before/after 比較・増加ゼロ）。
- **R-9**: areka examples の既存 E0433 は既存状態として記録し、証跡運用は `--exclude areka`。所見は `test-cage-determinism` へ送付。
- **R-10（#11）**: `benches/`・`build.rs` は 0 件と design に明記し、steering に将来追加時の同一規律 1 行を置く。

### Decision: steering 改訂範囲と任意移設 — #8/#10、R-2

- **#8**: `structure.md` L142-145 書き換え（新規テストモジュールは兄弟テストファイルへ・命名規約・1,000 行目安の双方適用・既存 `{module}/tests.rs` は歴史的形式として維持）＋L133-140 追記（`#[path]` 子解決規則）＋L204 補記（ファサード形式）＋将来カテゴリ 1 行。
- **#10（R-2）**: 任意移設（テストモジュール 500 行以下）は**行わない（0 件）**。差分最小化を優先。ディレクトリ内混在は steering の新規運用規律で将来収束。

### Risks & Mitigations（design 確定版）

- 本体分割 2 本が最大の非機械的リスク → D1 ファサードで呼び出し側 0 変更・examples ビルド緑をコミットゲートに固定。
- テーマ分割による名前変化 → 全単射対応表＋関数識別子不改名で証跡強度を維持。
- コールドビルド・i686 前提 → 移設前スナップショットを実装開始時に 1 回で採取・最終全緑は i686 ビルド後。
- 他 spec 実装との衝突 → 着手時 W5.95 空白確認（5.4）＋着地時は除外・登記（5.5）。
