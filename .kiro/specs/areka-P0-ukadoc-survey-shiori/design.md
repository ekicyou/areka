...
- 小群と件数（実測）: 本体更新 12・他ゴースト更新 8・点検 2・結果 5。鎖の本数は 11＋7＋1＋4＝**23**。
- これに加えて、本体更新と他ゴースト更新の**対応する 8 組**（`OnUpdateBegin` と `OnUpdateOtherBegin` など）を `same-feature` で結ぶ。合計 **31 本**。
...
### DD-11（§3-12）優先度の仮置き — **案 ⒜ を採用（テーマ群ごとに 1 つの値）**

優先度は項目ごとに考えず、**（状態・テーマ）の組から 1 つの値が決まる**表で置く。上流 要件 4.7 の 4 つの根拠の序列（壊れ方 ＞ 伺からしさ ＞ 影響する既存資産の広さ ＞ 基盤の共有度）は入れ替えない。

| 状態 | テーマ | 段階 | 値 | 根拠（4 つの軸の当てはめ） |
|---|---|---|---|---|
| `implemented`／`alias`／`not-applicable` | — | — | `""` | 作業が残っていない |
| `absent`／`vocabulary-only`／`degraded` | 触れ合い | A | `A1` | 黙って壊れる × 触れれば返るという最も基本的な手応え |
| 同上 | 掛け合い | A | `A2` | 黙って壊れる × 会話の往復そのもの |
| 同上 | 気配 | A | `A3` | 黙って壊れる × 何もしなくても生きている感じ |
| 同上 | 装い | B | `B1` | 黙って壊れる × 見た目の変化 |
| 同上 | 記憶 | B | `B2` | 黙って壊れる × 前を覚えている感じ |
| 同上 | 交わり | C | `C1` | 黙って壊れる × 相手が要る（単体では現れない） |
| 同上 | 気配り | C | `C2` | 黙って壊れる × 外の出来事に依る |
| 同上 | 更新 | D | `D1` | 黙って壊れる × 更新はテーマとして独立している |
| 同上 | （テーマなし・配管や値の照会） | D | `D2` | 単体では利用者に見えない |
| 同上 | （受け口そのものが無い外部連携） | E | `E1` | 受け口を建てるところからで、他の段階の前提にならない |

数値は段階の中の通し番号であり、段階と数値のどちらも**仮置き**である。最終決定は `ukadoc-coverage-roadmap` が行う（要件 7.5）。
...
| 2 | 送出していないイベント（`On` 始まり） | 251 | `absent` | 項目ごと（8 語彙） | テーマから決まる |
...
| 14 | 外部連携（SSTP 2・FMO 6・WEB 3・DLL 1・PLUGIN 1・HEADLINE 1） | 14 | `absent` | `[]` | `E1` |
| 15 | イベント一覧の補足（`memo_shiorievent`） | 1 | `not-applicable` | `[]` | `""` |

内訳の検算: 11＋251＋3＋25 ＝ 290（`list_shiori_event`）／168（`list_shiori_event_ex`）／1＋131＋27 ＝ 159（`list_shiori_resource`）／5＋2＋15＋4 ＝ 26（`spec_shiori3`）／19（`list_plugin_event`）／14（外部連携の 6 ページ）／1（`memo_shiorievent`）＝ **677**。

群 2 の 251 件は「290 − 実装済み 11 − `On` 以外 25 − 別名 3」である。仕分けの途中で新たな別名が見つかれば、その行は群 2 から群 3 へ移る（要件 6.1 の順序で決める）。件数が固定なのは 677 の合計とページ別の内訳だけで、状態ごとの件数は判定の結果として決まる。
...
| `OnUpdate` の掲載順の鎖（DD-4） | `same-feature` | 23 | 先に来る項目 |
...
要件 3.6 の「縮退の記録の転記元」は `doc/COMPAT_ARCHITECTURE.md` §8（沈黙ルール対応表）と `doc/emo2-conformance-scope.md` §6（見直し表）である。実測では、§8 の行のうち SHIORI ドメインの項目を名指ししているのは **2 行**だけである。
...
実装時に §8 の全行を 1 行ずつ読み、SHIORI ドメインの項目を名指ししている行が他に無いことを確かめる。増えていれば同じ形で対応する行の `note` に書く。
...
| 3.6 | 縮退の転記元を `note` に書く | 縮退の転記元の表（COMPAT §8 の 2 行） |
...
| 7.6 | 壊れ方の段とログの根拠を `note` に | DD-10（群の共通 `note` に壊れ方の段とログの有無を含める） |
...
| 12.3 | 既存 spec が所有する項目は `owner` を書くだけ | 群 10（`Charset` の `owner`）・縮退の転記元（`Status` の `owner`） |
...
**赤を先に出す**。台帳を書き始める前に、上の 7 通りの壊し方を当てた見本で台本が実際に赤になることを 1 件ずつ確かめる。緑は台本が壊れていても出るため、緑だけでは検証にならない。
...
| 使い捨ての台本が壊れていて緑を出す | 中 | 7 通りの壊し方で赤が出ることを書き始める前に確かめる |
