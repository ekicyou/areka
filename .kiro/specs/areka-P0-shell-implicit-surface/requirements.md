...
2. The areka shall 「同じ色」を色の**完全な一致**（復号後の 32bit の値が等しい＝B・G・R・A の 4 バイトが等しい。α を比較に入れるのは 4.8 の `tRNS` の裁定と同じ帰結で、3 成分だけで比べると左上が完全透明のときに黒寄りの不透明な画素まで消える。2026-09-20 にレビューで裁定）と定め、近い色を同じとみなす許容幅を持たない（許容幅 0）。正典は「同色」としか書かず許容幅に触れていない＝沈黙であり、C13（色がずれる形式は抜き色と相性が悪い）は完全一致を前提にした記述である。
...
8. The areka shall 「`element0` が在れば画像を使わない」が壊れたらテストが赤になることを、面 10 の形（`element0` の画像 336×400 と、面の画像 427×463 が**別の絵**）で示す。面 0 の形（`element0` と面の画像が同じ絵）では、誤って二重に重ねても**外形**が 434×687 のまま変わらず、外形だけを見る検査にはならないので、面 0 の外形に頼らない。画素は面 0 でも変わる（絵の縁が半透明ゆえ、重なると色が変わる。摂動の実測で先頭の差は 827 バイト目）。
