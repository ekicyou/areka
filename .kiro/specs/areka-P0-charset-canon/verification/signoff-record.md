...
この記録は実バイナリを実際に起動して得たものである。表の数はすべて走らせた結果で、
見込みや引き写しではない。**目視の欄だけは埋まっていない**——理由と、開発者が自分で
...
**未実施（開発者の担当）。** 7 節の手順で行うこと。確認すべきは「挨拶が文字化けせずに
バルーンへ出ること」と「表示が本仕様の適用前と同一であること」の 2 点。
...
| ⑵ emo2 | 挨拶が文字化けせずバルーンに出ること／表示が本仕様の適用前と同一であること | **未実施（開発者の担当）** |

画面を見る確認は人の目が要るので、この記録では埋めない。emo2 の分は次の 1 行をそのまま
