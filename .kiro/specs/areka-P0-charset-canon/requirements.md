...
8. The SHIORI 通信層 shall ステータス行・ヘッダ名・ステータスコードの解析、および 200／204／311／312／400／500 の扱いを本仕様の前後で変えない（UTF-8 の応答に対する既存テストは、期待値を変えずに緑。呼び出し形の機械的な追随は可）。
