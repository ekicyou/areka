...
| 49 | `app-lifetime-separation`（**09-20 起票**・#13 から切り出し） | **α** | 基盤（アプリの寿命を窓の数から切り離す。**「窓 0 で終了」の所在は `main.rs` ではなく wintf の `wire_shutdown_hook`**＝#13 の brief と旧 A2 行が見落としていた。wintf の既定と example 7 本は変えず、areka だけが切り替える。今日の終了操作 6 種はどれも同じ見え方で終わる） | S（5〜7 タスク） | **A1** | なし（**#12 を待たない**＝列挙も記憶も使わない） | ○（終了経路の形を決める・後ろのゴースト切替がこの上に建つ） | ⚪ |
