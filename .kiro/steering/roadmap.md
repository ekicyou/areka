...
> **M1 は 2026-09-11 に完成宣言済み**（下記「M1 ゴール」）。**M2 のゴールは 2026-09-18 の棚卸⑭で「α 版」に決めた**（下記「M2 ゴール（α）」）——開発者の指示「α 版として第三者に使い始めてもらうことができるだけの機能セット。大事なのはゴースト・シェル・バルーンのファイル管理、インストール、ネットワーク更新、つまりアプリとしてのデスクトップマスコット管理。オーナードローは不要だが最低限のメニューは要る。表現力増強は emo2 が普通に動いている水準で一旦よい」。本ファイルは **spec 台帳 56 行（完了 16・α 8・バグ 5・α 後 26・保留 1＝2026-09-24 の実数え。表の段列を機械で数えた）の着手順（ウェーブ）と干渉条件だけ**を持つ。**着手の優先度は「バグ修正 → α に要る機能」で、どちらにも属さない spec は当面着手しない**（2026-09-20 開発者指示・下の「棚卸⑮の裁定」）。
...
| 48 | `shiori-loadu`（**09-20 起票**・開発者指示） | **バグ** | バグ（実害＝**ログ無しの失敗**。`load` へ渡すパスを既定コードページへ変換するとき、表せない字の置換を検出していない＝`WideCharToMultiByte` の `lpUsedDefaultChar` が `None`。日本語以外のロケールや特殊な字を含むフォルダに置かれたゴーストが、ログ 0 行で辞書を見失う）＋正典（ukadoc DLL 共通仕様の 4 つ目の入口 `loadu`＝置き場所のパスを UTF-8 で渡す。`load` と両方在れば `loadu` を優先。手元の検体で両方の枝が踏める＝YAYA は `loadu` 持ち・里々と pasta は `load` のみ。#30 の切れ端 ⓐ を SHIORI を主語にして切り出した） | S（6〜7 タスク） | **A1** | なし（完了 `host32-shiori-load`・#3 ✅・#5 ✅ の上に建つ。触るのは `crates/shiori-host32-helper/src/{shiori_proxy.rs,main.rs}` と新しい i686 の偽 DLL クレート＝09-20 実測で A1 の他 spec と共有するのは `doc/COMPAT_ARCHITECTURE.md` §8 の末尾追記と生成物 `THIRD-PARTY-NOTICES.md` だけ） | −（裁定 3 件は 09-20 に承認済み） | ⚪ |
...
- **brief の体制**: 台帳 56 行のうち **50 本に brief あり**・登記だけの行 6 本（#38〜#41・#54・#56）。**完了 16・α 8・バグ 5・α 後 26・保留 1**（2026-09-24 の実数え）＝着手は該当 brief を読んで `/kiro-start <unit>` へ直行。brief の file:line は起票時値＝**着手時に必ず再検証**（棚卸⑮では A1〜A5 の全 brief とバグの性質を持つ brief を main `fe157df1` で再測定し、結果を各 brief の「2026-09-20 棚卸⑮の再測定」の節へ書いた。α 後の残りの brief は再測定していない）。
