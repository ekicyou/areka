...
- `areka-P0-window-placement` — サーフェス窓の生成＋既定位置＋ドラッグ（既定位置＝ukadoc `seriko.alignmenttodesktop` カスケード準拠・窓数は構成入力・二人立ちの本格結線は M-dual）。**順序ゲート `areka-P0-emo-present` は 2026-07-09 解消 ✅**（本番ゴースト実表示に対して実装・検証する。2026-07-05 に demo 前提の着手が実 DPI 座標破綻＝論理/物理混在でワークツリーごとリジェクト→brief 改稿済み・wintf 座標契約の確定を design 必須先行に）。✔ 本番 emo2 表示＋**実 DPI（≠96）**で既定位置・ドラッグ・バルーン追従が正しい（dpi=96 のみの緑は不合格）（**brief 済・07-05 改稿・07-09 実シンボル再調整**＝ghost-setup✅ でシーム単独所有・`EmoPresenter::attach_target(window: Entity)` 実形確定・donor に `examples/emo-present.rs` 追加・wintf `DPI` 変換ヘルパ実在確認）
