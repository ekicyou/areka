...
> **M1 は 2026-09-11 に完成宣言済み**（下記「M1 ゴール」）。**M2 のゴールは 2026-09-18 の棚卸⑭で「α 版」に決めた**（下記「M2 ゴール（α）」）——開発者の指示「α 版として第三者に使い始めてもらうことができるだけの機能セット。大事なのはゴースト・シェル・バルーンのファイル管理、インストール、ネットワーク更新、つまりアプリとしてのデスクトップマスコット管理。オーナードローは不要だが最低限のメニューは要る。表現力増強は emo2 が普通に動いている水準で一旦よい」。本ファイルは **spec 台帳 56 行（完了 18・α 7・バグ 4・α 後 26・保留 1＝2026-09-24 の実数え。表の段列を機械で数えた）の着手順（ウェーブ）と干渉条件だけ**を持つ。**着手の優先度は「バグ修正 → α に要る機能」で、どちらにも属さない spec は当面着手しない**（2026-09-20 開発者指示・下の「棚卸⑮の裁定」）。
...
## spec 台帳（**表は 56 行**。うち **brief 済み 50 本**＝2026-09-13 の 30 ＋ 09-18 起票 8 ＋ 09-19 起票 3 ＋ 09-20 起票 8（#47 `balloon-break`・#48 `shiori-loadu`・棚卸⑮の 6 本＝#42 `default-balloon-nar-fold`・#49〜#53）＋ 09-24 起票 1（#55 `shiori-fault-notice`）、**#38〜#41・#54・#56 の 6 行は brief を持たない「登記だけの行」**＝下の節を見よ。**実数え（2026-09-24・表の段列を機械で数えた＝✅ 18・α 7・バグ 4・α 後 26・保留 1 ＝ 56**。09-20 の見出しの「完了 15・α 9」は #49 の完了〔09-23〕が数に反映されていなかった）。着手は brief 持ちが `/kiro-start <名>`・登記だけの行は `/kiro-discovery` から）
...
| 12 | `baseware-root-layout`（**09-18 起票・09-20 再測定**） | **α** | 基盤（根 `ghost/`・`balloon/`・列挙と素性・最後の選択の記憶＝**鍵の定義と書き込みまで。起動時にシェル名を効かせる口は #50**・起動解決・「無い」告知。**メニューへの登記は範囲外**＝#13・#50 が持つ。09-20 の再測定で崩れた前提 3 件＝`MessageBoxW` の告知が常設の smoke テストを止める／`resolve()` は 4 か所から独立に呼ばれシェルの記憶を起動時に効かせる口が無い／列挙に `resolve()` は流用できない） | S〜M（13〜17 タスク） | **A1 後段**（要件と設計は A1 で先行可・**実装は #49 の着地後**） | #11 ✅。実装は **#49**（`crates/areka/src/main.rs` の別の関数と `tests/smoke_boot_loop_exit.rs` を共有） | ○（09-20 に − から変更＝議題が 1 件から 3 件へ増えた・根の形は後ろの α の spec 全部が乗る契約） | ⚪ |
...
| 13 | `ghost-shell-balloon-switch`（**09-18 起票・09-20 に 3 分割の真ん中として存続**＝名前は完了 spec 7 本・`crates/areka/src/menu/mod.rs` のコメント・網羅台帳 `shiori.toml` が引受先として指すので変えない） | **α** | 正典（切替要求の入口の型 `SwitchRequest`・`\![change,ghost]`・名前解決 `random`／`sequential`／`lastinstalled`・**ゴースト切替**＝`OnGhostChanging/Changed`・降ろして起こし直す仕組み・メニューの「ゴースト」枠の登記。**寿命の分離は #49・シェルとバルーンは #50 へ切り出した**） | M〜L（15〜18 タスク。分割前は 30〜40） | **A2**（単独） | #12・**#49**・`host32-window-thread-pump` ✅・`kanade-boot-talkdone-drop` ✅ | ○（α 最大の構造変更は分割後も本行に残る＝`main.rs` の終了順序と `wire_emo2_boot` を繰り返し入れる形へ括り出す） | ⚪ |
...
| 15 | `ghost-install`（**09-18 起票・09-20 再測定**） | **α** | 製品（D&D `WM_DROPFILES`・ファイル選択・`\![execute,install,path]`・`OnInstall*`・`terms.txt`・`OnFileDrop2`・網羅台帳 `descript_install` の 11 行を実装済みへ。**`areka-nar` の堅牢化 3 件は #52 へ移した**。`InstallRequest`／`InstallOutcome` は `areka_nar` の同名の型と衝突＝要件で改名） | M（17〜20 タスク。移す前は 20〜25） | **A4-①** | #11 ✅・#12・#13（`lastinstalled`）・**#50**（シェルとバルーンの `.nar` を入れた直後の切替）・#14 ✅（登記）・**#52** | − | ⚪ |
| 16 | `network-update`（**09-18 起票・09-20 に結線の側として存続**） | **α** | 製品（kanade の「更新」の相・`OnUpdate*`／`OnUpdateOther*` の全列と Ref・台本の入口 3 つ・メニュー登記・`OnUpdateProcessExec`・`useorigin1`・`\![execute,install,url]`・更新後の読み直し・網羅台帳の更新。**エンジン（定義ファイル・取得・照合・確定・`delete.txt`）は #51 へ切り出した**） | M（12〜15 タスク。分割前は 24〜30） | **A4-②**（#15 の後・直列） | **#51**・#12・#13（読み直しは同じゴーストへの切替で代用）・#15（`execute,install,url` は #15 の受け口ファイルを改変する）・#14 ✅ | ○（イベント列 51 件の Ref） | ⚪ |
| 17 | `alpha-release-signoff`（**09-18 起票・09-20 再測定**） | **α** | 完成の器（配布 zip・第三者の手順 12 項目・実機サインオフ・宣言。09-20 追記＝`LICENSE-MIT`・既定バルーンの出どころが #42 の着地で `.nar` に変わる・`shell-implicit-surface` の目視 5 項目を実機一周へ受け取った） | S〜M（8〜12 タスク） | **A5** | #12・#13・#15・#16・#42・#49・#50・#51 全部 | − | ⚪ |
| 18 | `dpi-transition-two-tick-bounce` | α 後 | バグ（見た目・判定器・開発者裁定「拡大率の切替は頻繁に起こらないため許容」。**09-20: 判定器が見送りの窓を除いていない穴だけは「直接修正候補」へ切り出した**） | M | 旧 W14 | #1 ✅ | ○ | ⚪ |
...
| 22 | `emo-text-canon-residue`（⑶・項目 12 は実バグ） | α 後 | 台帳（emo-text 残件 4。**09-20: 上流の待ちは両方解けた・項目 12 は「直接修正候補」で先に直して本行から外す**） | M | 旧 W14 | #7・#9 ✅ | − | ⚪ |
...
| 43 | `wintf-drag-state-rest-contract`（**09-19 起票・09-20 に相乗り 2 件**） | **バグ** | バグ（構造・潜在＝wintf のドラッグ状態は `JustEnded` で休み続けるのに説明は「1 フレームのみ」・`reset_to_idle` の呼び手が製品に無い。#14 の実機確認でメニューが出なくなる実害 1 件・利用側は是正済み）＋**#44 の残件 1・2 を相乗り**（別の窓の預かりの誤配＝`trigger.rs` の 1 条件／台本の `\![open,readme]` が説明書の無いゴーストで `error!` を出す）＝同じ `crates/areka/src/menu/trigger.rs` を触るので 1 本にまとめた | S（5〜7 タスク） | **A1** | #14 ✅ | −（裁定 2 件＝直し方 2 案・記録の水準） | ⚪ |
...
| 50 | `shell-balloon-switch`（**09-20 起票**・#13 から切り出し・名前は `roadmap-draft.md` の束「切替」の候補名） | **α** | 正典（`\![change,shell\|balloon]`・`OnShellChanging/Changed`・`OnBalloonChange`・メニューの「シェル」「バルーン」枠の登記・起動時の「最後のシェル」の適用。**#13 の brief の前提「資産を作り直すだけ・バルーンが最軽量」は 09-20 の実測で崩れた**＝出し先は起動時に値で渡されて固定され、走っている最中に差し替える語彙が seriko・present・text・dispatcher のどこにも 0 件。シェル名は `resolve()` の 4 呼び出し点へ運ぶ必要がある） | M（12〜15 タスク） | **A3**（単独・旧「空き」の段を埋めた） | #12・#13（`SwitchRequest`・降ろして起こし直す仕組みの一般形）・#42 | ○（今は存在しない仕組みの設計＝**要件の前に先進坑を 1 本推す**：SHIORI を生かしたまま present のバルーン資産だけを差し替えて 1 フレームも崩れないか） | ⚪ |
| 51 | `update-engine`（**09-20 起票**・#16 から切り出し・前例は #11 と #15 の分け方） | **α** | 基盤（新規クレート＝`updates2.dau`／`updates.txt` の読み手・MD5 の差分・`HttpFetch` の境界と偽実装と WinHTTP 実装・一時フォルダからの**全か無かの確定**・`delete.txt`。イベントは送らない・網羅台帳に触らない。入力は `homeurl` の文字列と対象フォルダのパスだけ） | M（12〜14 タスク） | **A1**（α の直列経路の外で最も長い仕事を先に走らせる） | なし | ○（第三者のフォルダを書き換える・失敗時の保全が主題。裁定候補＝MD5 は OS の CNG を推す＝依存の承認が要らなくなる） | ⚪ |
| 52 | `nar-install-hardening`（**09-20 起票**・#11 の最終検証の先送り 3 件を #15 から切り出し） | **バグ** | バグ（堅牢性＝第三者の `.nar` を受ける口。1 要素あたりの名前の長さに上限が無く 30 万文字が受理される／巻き戻せなかったとき元の木の在りかを利用者へ伝える手段が無い／失敗の記録の `work` の欄が確定の段の失敗で一度も検査されていない。`areka-nar` の呼び手は今 `sample-ghost-kit` だけ＝いま直せば追随 0） | XS〜S（3〜5 タスク） | **A1** | #11 ✅ | −（裁定 1 件＝上限の値と数え方） | ⚪ |
| 53 | `keycolor-clickthrough-coverage`（**09-20 起票**・#10 が「直さずに残したもの」・引受先がどこにも無かった） | **バグ** | テストの穴（実害に直結。#10 の要件 4 の 10 番＝抜き色で透明になった場所のクリックが背後の窓へ抜ける、を赤にできるテストが 0 本。抜き色 → α と、α → 当たりのマスクの両端は在るが**繋ぐテストが無い**。壊れると絵の四角い外枠の全面がクリックを食い、周りのデスクトップが操作できない。09-20 に目視では確認済み。相乗りの候補 3 件＝同じ完了 spec のテストの穴） | XS〜S（2〜4 タスク） | **A1** | #10 ✅・#11 ✅ | − | ⚪ |
...
| **A1**（**バグ 4 本 ∥ α 3 本・並走**＋ #12 の要件と設計の先行。2026-09-20 棚卸⑮） | **バグ**: ① ~~`shiori-loadu`~~ ✅（09-24 完了） ∥ ② `wintf-drag-state-rest-contract` ∥ ③ `nar-install-hardening` ∥ ④ `keycolor-clickthrough-coverage` ／ **α**: ⑤ ~~`app-lifetime-separation`~~ ✅（09-23 完了） ∥ ⑥ ~~`default-balloon-nar-fold`~~ ✅（09-24 完了） ∥ ⑦ `update-engine` ／ **文書のみ**: ⑧ `baseware-root-layout`（~~`shell-implicit-surface`~~ ✅ は 09-20 に完了） | `/kiro-start areka-P0-shiori-loadu` ／ `/kiro-start areka-P0-wintf-drag-state-rest-contract` ／ `/kiro-start areka-P0-nar-install-hardening` ／ `/kiro-start areka-P0-keycolor-clickthrough-coverage` ／ `/kiro-start areka-P0-app-lifetime-separation` ／ `/kiro-start areka-P0-update-engine` ／ `/kiro-start areka-P0-baseware-root-layout`（⑧ は `/kiro-design` まで。`/kiro-impl` は ⑤ の着地後） | **着手中の spec は 0 本・open PR 0 本**（09-20 実測）＝白紙から組んだ。①〜⑦ は触るソースファイルの重なりが **0**（下の「干渉台帳（A1）」に全数）。⑧ は ⑤ と `crates/areka/src/main.rs`（958 行）の別の関数と `tests/smoke_boot_loop_exit.rs` を共有するので**実装だけ後段へ送る**——`/kiro-start` と `/kiro-design` は文書しか書かないので並走してよい（⑤ の着地後に settled main へ再突合）。**手が足りないときの削る順**: ⑥ → ⑦ → ④ → ③（①②はバグの実害が大きい順に先・⑤ は α の直列経路の先頭なので削らない）。⑦ は直列経路の外で最も長い仕事（M・12〜14 タスク）なので、早く始めるほど A4 が待たされない |
| **A1 後段**（単独） | `baseware-root-layout` の実装 | `/kiro-spec-tasks areka-P0-baseware-root-layout` → `/kiro-impl areka-P0-baseware-root-layout` | ⑤ `app-lifetime-separation` が main へ入ってから（09-23 着地済み）。`boot_config.rs`／`main.rs`（`main()` の根の解決と代替経路）／`main_config_input_tests.rs`／`crates/areka-sylphya/src/persist/{mod,format}.rs`／`crates/areka-ghost/src/lib.rs` と新規ファイル／`tests/smoke_boot_loop_exit.rs`。`crates/areka-ghost/src/runtime.rs` と `crates/areka/src/menu/` には触らない見込み（09-20 実測） |
| **A1 後段 ②**（単独・2026-09-24 追加） | `shiori-fault-notice`（バグ） | `/kiro-start areka-P0-shiori-fault-notice` | #12 の実装が main へ入ってから（#12 が作る利用者向け告知の仕組みに乗る・`main.rs` と `tests/smoke_boot_loop_exit.rs` を共有）。A2（#13）も `main.rs` の終了順序を触るので並走させない。要件と設計は #12 の実装と並走してよい |
| **A2**（単独） | `ghost-shell-balloon-switch`（ゴースト切替と切替要求の入口） | `/kiro-start areka-P0-ghost-shell-balloon-switch` | #12 と #49 の上に建つ。`main.rs`（`app.run()` の後ろの終了順序を繰り返し入れる単位へ）／`boot_config.rs`／`emo2_boot/{mod,frame,consumer_ledger}.rs`／kanade の `schedule/close.rs` と 5 ファイル／`menu/mod.rs`（最初の登記者）／網羅台帳。**後ろの 3 本を軽くする提案**＝汎用の通知の入口を 1 本作れば、後続の kanade への接触は `schedule/events.rs` の数行に縮む。実機は emo2 ⇄ R_POST_and_KOMAINU の往復。`zorder-chain-residue` A-2 の spine の族が本ウェーブの切替テストで赤を出したら、その時点で A 群だけを単独枠に挟む（先回りしない） |
| **A3**（単独） | `shell-balloon-switch` | （推奨）先進坑 `crates/pilot/examples/areka-P0-shell-balloon-switch/` を 1 本 → `/kiro-start areka-P0-shell-balloon-switch` | 旧「空き」の段を埋めた。A2 の「降ろして起こし直す仕組み」の上に「SHIORI を残す版」として建つ。`emo2_boot/{assets,mod,consumer_ledger}.rs`・`frame/attach.rs`・`placement/{spawn,source,persist}.rs`・`resolve.rs`・`areka-ghost/src/runtime.rs`・各アクターの語彙・kanade の 5 ファイル・網羅台帳。2 つ目のシェルを持つ検体を登記するなら `sample-ghost-kit` の直書きの数を数え直す |
| **A4**（直列 2 本） | ① `ghost-install` → ② `network-update` | `/kiro-start areka-P0-ghost-install` → `/kiro-start areka-P0-network-update` | 両方が `emo2_boot/{consumer_ledger,mod}.rs`・kanade の 5 ファイル・`main.rs`（登記の呼び出し）・網羅台帳に触る＝**同時に走らせず直列**。② は ① の受け口ファイルを改変する（`\![execute,install,url]`）。① は #52 の着地で申し送りが 1 件に減る・`Win32_UI_Controls_Dialogs` の機能 1 行・`WM_DROPFILES` は `crates/wintf/src/ecs/window_proc/mod.rs` に 1 分岐。② は #51 のエンジンを呼ぶだけ。裁定 ⑸ |
| **A5** | `alpha-release-signoff` | `/kiro-start areka-P0-alpha-release-signoff` | 配布 zip（PowerShell スクリプト 1 本・既定バルーンは `.nar` を展開して入れる）＋第三者の手順 12 項目の実機一周（`shell-implicit-surface` の目視 5 項目を含む）＋開発者の署名。**α 完成宣言の器** |
...
- **条件付きの重なり 1 件**: ⑦ が `areka-nar` の確定の部品を公開して使うと決めた場合だけ、③と `crates/areka-nar/src/` を共有する＝**③を先に着地させる**（XS〜S で早く閉じる）。⑦ の要件段階で決まる。
- **直接修正候補の 2 件**（下の節）は `crates/areka-emo-text/src/writing.rs` と `crates/areka/src/placement/transition_judge_verdict*.rs` だけで、①〜⑧ のどれとも重ならない。
...
- **2026-09-24 追加（バグ・テストの穴）**: `crates/shiori-host32-helper/src/shiori_proxy.rs` の既存 `mod tests` の `ansi_encode_mixed_japanese_is_multibyte` は、機械の既定コードページが 65001（UTF-8）だと本来の欠陥と無関係に赤になる（既定コードページのバイト列が UTF-8 と一致するため）。機械の設定で結果が変わるテストは決定論ではない。`areka-P0-shiori-loadu` が足した `encode_with_codepage` にコードページ 932 を渡して確かめる形へ直せば、機械に依存しない。同じ `mod tests` に残る古い注記（「3 解決の先頭」＝入口は今 4 つ）と、モジュール冒頭の古い節（「consume 点 / Task 6」）も一緒に直す。**`shiori-loadu` が main へ着地してから**直す（同 spec の要件 6.8 がこのブロックの無改変を課している）。
- **2026-09-24 追加（バグ・記録の肥大・XS）**: 失敗の記録の 1 行に長い名前が丸ごと載る穴が `crates/areka-nar` に 2 か所残る（完了 `nar-install-hardening` の設計 Open Questions の 1 項目目。同 spec は範囲外として登記だけした）。⑴ `RefuseReason::NameUndecodable.raw_hex` は生バイト全体の 16 進を載せる——長さの検査は復号の**後**なので上限 200 が効かず、復号できない 65,535 バイトの名前は記録の 1 行が 131,070 文字になる。⑵ `container.rs` の `unsupported`（`UnsupportedEntry.name`）は `String::from_utf8_lossy(name_raw)` で名前の全体を載せ、`read_central_directory` の中＝長さの検査より**前**に起きる（暗号化・zip64・非対応の圧縮方式を持つ長い名前）。直し方は同 spec の `PathTooLong` と同じ規則（先頭 32 単位の有界の一部＋測った長さ）へ揃える各 1 行＋テスト各 1 本。⑵ は `container` → `names` の依存の向きを逆にできないので、切り詰めの助手を `error.rs` 側へ置く（`names.rs` の `head_utf16` をそちらへ移して両方から使う）。既存の拒否理由の中身を変えるが、呼び手は `sample-ghost-kit` だけで理由の文面を読む箇所は 0。
...
- **brief の体制**: 台帳 56 行のうち **50 本に brief あり**・登記だけの行 6 本（#38〜#41・#54・#56）。**完了 18・α 7・バグ 4・α 後 26・保留 1**（2026-09-24 の実数え）＝着手は該当 brief を読んで `/kiro-start <unit>` へ直行。brief の file:line は起票時値＝**着手時に必ず再検証**（棚卸⑮では A1〜A5 の全 brief とバグの性質を持つ brief を main `fe157df1` で再測定し、結果を各 brief の「2026-09-20 棚卸⑮の再測定」の節へ書いた。α 後の残りの brief は再測定していない）。
