...
## spec 台帳（**表は 46 行**。うち **brief 済み 41 本**＝2026-09-13 の 30 ＋ 09-18 起票 8 ＋ 09-19 起票 3、**#38〜#42 の 5 行は brief を持たない「登記だけの行」**＝下の節を見よ。**状態列の実数え（2026-09-19・`areka-P0-balloon-origin-outside-validrect` の完了時に数え直し）＝完了 13・α 7・α 後 25・保留 1 ＝ 46**。着手は brief 持ちが `/kiro-start <名>`・登記だけの行は `/kiro-discovery` から）
...
| 10 | `shell-implicit-surface`（09-13 起票） | **α** | バグ（**里々標準テンプレートが 1 枚も絵を出せない**・第三者の最初の 1 体が里々である前提で α の入口）＋**抜き色の透過**（2026-09-19 開発者裁定で範囲に追加＝両テンプレートの絵が全て α チャンネルを持たず、抜かなければ絵が四角のままで第一の成果に届かない。触るクレートに `areka-emo-atlas` が加わる）＋まばたきの間隔の語 `sometimes`・`rarely`（同日の裁定「テンプレートゴーストを動かすのに要る実装は本仕様が巻き込む」） | M〜L | **A1-①** | `charset-canon` ✅・`present-gpu-transform-scale` ✅・#11（検体は共有ヘルパ経由で受ける＝09-18 に順序を反転） | ○（裁定 2 件＝「element0 より下」の層表現と `surface.append` の順序は、**どちらも正典の逐語で決まった**＝前者は「element0が定義されている場合…破棄され」（C4・C5）ゆえ「下に敷く」ではなく「画像を使わない」、後者は「既に在る面へ追記する」（C6）ゆえ画像だけで存在する面にも効く。裁量が残ったのは正典が沈黙する 6 点で、`doc/COMPAT_ARCHITECTURE.md` §8 に登記済み） | ⚪ |
...
> **なぜここに書くのか**: 下の 7 件はどれも、正典に書かれていて areka が引き受けていない振る舞いだが、**本ファイルの spec 台帳（46 行）にも `.kiro/specs/` にも、これを引き受ける仕様が 1 本も無い**（2026-09-20 に数え直した実測）。網羅台帳 `doc/ukadoc-coverage/ledger/assets.toml` の側でも、該当する項目の担当欄は空か、名指しの仕様が完了アーカイブに在って先送りを消化できない。先送りを完了アーカイブへ入れると誰も読まなくなるので、行だけ立てて生きた本ファイルから見えるようにしておく（開発規律 deferral-requires-verified-owner）。**着手の判断は α 完成宣言のあとの棚卸で行う。引受先が決まっていないので、ここに実在しない仕様の名前は書かない。**
...
**開発者の目と手が要る確認 5 件（`shell-implicit-surface` の実機確認で自動走行では確かめられなかったもの）**: ⑴ 絵の外のクリックが背後の窓へ抜けること ⑵ 右クリックメニューの 1 項目目の表示 ⑶ `konnoyayame` の目の周りに四角い地色が出ないこと ⑷ 起動挨拶の**字形**が文字化けしないこと ⑸ `emo2` の撫で・メニュー・終了が適用前と同じに見えること。いずれも走っている窓に対する操作か目視が要り、有界の自動終了＋ログの絞り込みでは判定できない。**どれも本仕様が新しく作った経路ではない**（クリック透過・右クリックメニュー・字形の描画・バルーンの左右反転は既存の機能で、本仕様はそこへ絵を届けられるようにしただけである）ので新しい仕様は起票しない。α の受け入れの一周で開発者の目が通る場所＝`areka-P0-alpha-release-signoff`（#17・第三者の手順 11 項目の実機サインオフ）の実機一周でまとめて見るのが最も安い。再現の手順は `.kiro/specs/areka-P0-shell-implicit-surface/tasks.md`「実機確認の記録」の「共通の条件」にある。
