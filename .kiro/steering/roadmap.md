...
> **M1 は 2026-09-11 に完成宣言済み**（下記「M1 ゴール」）。**M2 のゴールは 2026-09-18 の棚卸⑭で「α 版」に決めた**（下記「M2 ゴール（α）」）——開発者の指示「α 版として第三者に使い始めてもらうことができるだけの機能セット。大事なのはゴースト・シェル・バルーンのファイル管理、インストール、ネットワーク更新、つまりアプリとしてのデスクトップマスコット管理。オーナードローは不要だが最低限のメニューは要る。表現力増強は emo2 が普通に動いている水準で一旦よい」。本ファイルは **spec 台帳 54 行（完了 15・α 9・バグ 4・α 後 25・保留 1＝2026-09-20 棚卸⑮の実数え）の着手順（ウェーブ）と干渉条件だけ**を持つ。**着手の優先度は「バグ修正 → α に要る機能」で、どちらにも属さない spec は当面着手しない**（2026-09-20 開発者指示・下の「棚卸⑮の裁定」）。
...
## spec 台帳（**表は 54 行**。うち **brief 済み 49 本**＝2026-09-13 の 30 ＋ 09-18 起票 8 ＋ 09-19 起票 3 ＋ 09-20 起票 8（#47 `balloon-break`・#48 `shiori-loadu`・棚卸⑮の 6 本＝#42 `default-balloon-nar-fold`・#49〜#53）、**#38〜#41・#54 の 5 行は brief を持たない「登記だけの行」**＝下の節を見よ。**実数え（2026-09-20 棚卸⑮・状態列が ✅ の行＝完了 15／状態列が ⚪ の行を段列で分けて α 9・バグ 4・α 後 25・保留 1 ＝ 54**）。着手は brief 持ちが `/kiro-start <名>`・登記だけの行は `/kiro-discovery` から）
...
- **brief の体制**: 台帳 54 行のうち **49 本に brief あり**・登記だけの行 5 本（#38〜#41・#54）。**完了 15・α 9・バグ 4・α 後 25・保留 1**（2026-09-20 棚卸⑮の実数え）＝着手は該当 brief を読んで `/kiro-start <unit>` へ直行。brief の file:line は起票時値＝**着手時に必ず再検証**（棚卸⑮では A1〜A5 の全 brief とバグの性質を持つ brief を main `fe157df1` で再測定し、結果を各 brief の「2026-09-20 棚卸⑮の再測定」の節へ書いた。α 後の残りの brief は再測定していない）。
