...
| **W12** 裁定枠 C | `ukadoc-survey-toolkit`（別セッション起票・追記(89)・調査道具＝新規 crate＋doc/ 台帳のみ） | （追記(89) 参照） | 既存 crate 非接触＝**共有ファイルは root `Cargo.toml` の members 1 行のみ**（W12 の他 spec は Cargo.toml 非接触）。survey 3 本→coverage-roadmap は toolkit 完了後（W13〜W14）。M2 機能 spec は起票しない（材料作り） |
...
- **cursor-tag ⇄ toolkit／channels ⇄ toolkit**〔ファイル素（toolkit は新規 crate＋doc/ 台帳＋root `Cargo.toml` members 1 行）〕
