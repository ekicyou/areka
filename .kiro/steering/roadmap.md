...
| 挙動バグ | 常駐アイドルの CPU 消費＝毎フレーム全再合成のアロケーション予算是正 | `recompose-budget` | **W6.75** |
| 挙動バグ | 拡大率切替時の跳ね＝遷移の原子性＋work area 追随（+36px/24px 浮き） | `dpi-transition-atomicity` | **W6.75**（**要件フェーズのみ W6.5 併走可**・再観測は要件の research） |
...
| **W6.5**（2本＋文書併走1本） | `scale-exact-rational` ∥ `windowposition-limit`（＋`dpi-transition-atomicity` は**要件フェーズのみ**併走） | `/kiro-start areka-P0-scale-exact-rational`・`/kiro-start areka-P0-windowposition-limit`・`/kiro-start areka-P0-dpi-transition-atomicity`（文書のみ） | 実測で素。**wpl は scg の実形へ rebase 必達**——干渉面は resolver.rs P2/P5（現 :154／:184）に留まらず **`placement/chain_finalize.rs`（第 2 の位置ライター・P4 クランプ非経由）／`frame/drain_resnap.rs` `finalize_chain_once`／`spawn.rs` `ScopeWindows.default_char_pos`** へ拡大（**wpl brief の scg 申し送りブロックが正本**）。逆順不可の理由（limit クランプ後の値へ scg 式が二重補正）は解消済みだが、クランプと連鎖確定の**適用順序**は wpl の要件裁定事項。**exact** は `presenter/read.rs`（f32 汚染点 **:109**・2026-08-14 現物一致確認済み）＝show.rs 系（budget/atom/cage④）と別ファイル。**exact は bod の前提**（丸め権威）ゆえ W6.75 より先。**atom の第 1 段再観測は W6 中に実施されなかった**＝atom 自身の requirements フェーズ（コード非接触）として本ウェーブに併走し、観測結果で W6.75 の形を確定する。**⛔ atom の design 以降は W6.75 で**（`presenter/show.rs` `apply_show`・frame 系で budget と衝突） |
| **W6.75** | `recompose-budget` ∥ `dpi-transition-atomicity`（＋`balloon-offset-dpi`） | `/kiro-start areka-P0-recompose-budget`・（atom は W6.5 併走の要件から継続） | **分岐は atom 再観測（W6.5 併走の要件フェーズ）で確定**: ⑴**縮退時（859ms/8 回が van S1 是正で消滅）**＝atom は「+36px work-area 追随＋檻」へ縮退し **bod と統合して 1 spec 化**（follow 系）→ budget（show.rs／cache.rs 系）と**並走可**。⑵**残存時**＝atom は show.rs `apply_show` 域へ広がるため **budget 先行→atom 直列**（budget 着地後に 859ms を再測すると合成コスト ≒143ms の帰着切り分けが最良）・bod は atom に従う。budget の着手ゲートは開放済み（bind 着地で CPU 単調上昇の (a) bind 同根説は構造的に消滅・第 0 段計時ログ設計は budget brief の 2026-08-11 追記が正本） |
...
- **budget⇄atom**〔**同ハンク＋因果・同居継続**: `presenter/show.rs` の `apply_show`（:32 起点）内——budget=compose/resample **:68-88**／atom=スワップ〜upload 域 **:220-270 帯**（vis 着地で旧 :55-70／:186-215 から +13〜+40 ドリフト・2026-08-14 実測）。859ms が合成コスト帰着なら budget へ差し戻し＝budget 先着後の再測が切り分け最良。atom 縮退時は解消〕
