...
| **W6.5**（3本＋文書併走1本） | `recompose-budget` ∥ `scale-exact-rational` ∥ `windowposition-limit`（＋`dpi-transition-atomicity` は**要件フェーズのみ**併走） | `/kiro-start areka-P0-recompose-budget`・`/kiro-start areka-P0-scale-exact-rational`・`/kiro-start areka-P0-windowposition-limit`・`/kiro-start areka-P0-dpi-transition-atomicity`（文書のみ） | 実測で素。**budget は性能最優先の開発者裁定（2026-08-14）で W6.75 から前倒し**——干渉は⑴exact⇄budget=slimming 緩和済み（`read.rs` ∥ `show.rs`）⑵wpl とはドメインごと素⑶atom は本ウェーブでは文書のみ＝非接触、かつ台帳の推奨順序（**budget 先着→atom が 859ms を再測**）が自然成立し編成が強化される。着手ゲート開放済み（bind 着地で (a) 同根説消滅・第 0 段計時ログ設計は budget brief の 2026-08-11 追記が正本）・キャッシュ容量 1 は emo-present R4.1 の**承認済み要件**ゆえ変更には要件段階の裁定が要る。**wpl は scg の実形へ rebase 必達**——干渉面は resolver.rs P2/P5（現 :154／:184）に留まらず **`placement/chain_finalize.rs`（第 2 の位置ライター・P4 クランプ非経由）／`frame/drain_resnap.rs` `finalize_chain_once`／`spawn.rs` `ScopeWindows.default_char_pos`** へ拡大（**wpl brief の scg 申し送りブロックが正本**）。逆順不可の理由（limit クランプ後の値へ scg 式が二重補正）は解消済みだが、クランプと連鎖確定の**適用順序**は wpl の要件裁定事項。**exact** は `presenter/read.rs`（f32 汚染点 **:109**・2026-08-14 現物一致確認済み）＝show.rs 系（budget/atom/cage④）と別ファイル。**exact は bod の前提**（丸め権威）ゆえ W6.75 より先。**atom の第 1 段再観測は W6 中に実施されなかった**＝atom 自身の requirements フェーズ（コード非接触）として本ウェーブに併走し、観測結果で W6.75 の形を確定する。**⛔ atom の design 以降は W6.75 で**（`presenter/show.rs` `apply_show`・frame 系で budget と衝突） |
| **W6.75** | `dpi-transition-atomicity`（＋`balloon-offset-dpi`） | （atom は W6.5 併走の要件から継続・design 解禁） | **budget は W6.5 へ前倒し済み（2026-08-14 裁定）**＝atom design は budget の実形（show.rs）へ rebase してから。**分岐は atom 再観測（W6.5 併走の要件フェーズ）で確定**: ⑴**縮退時（859ms/8 回が van S1 是正で消滅）**＝atom は「+36px work-area 追随＋檻」へ縮退し **bod と統合して 1 spec 化**（follow 系）。⑵**残存時**＝atom は show.rs `apply_show` 域へ広がる——budget が W6.5 で先着済みゆえ **着手時に 859ms を再測**すると合成コスト ≒143ms の帰着切り分けが最良・bod は atom に従う |
