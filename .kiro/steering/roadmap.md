...
> **M1 は 2026-09-11 に完成宣言済み**（下記「M1 ゴール」）。**M2 のゴールは 2026-09-18 の棚卸⑭で「α 版」に決めた**（下記「M2 ゴール（α）」）——開発者の指示「α 版として第三者に使い始めてもらうことができるだけの機能セット。大事なのはゴースト・シェル・バルーンのファイル管理、インストール、ネットワーク更新、つまりアプリとしてのデスクトップマスコット管理。オーナードローは不要だが最低限のメニューは要る。表現力増強は emo2 が普通に動いている水準で一旦よい」。本ファイルは **spec 台帳 54 行（完了 17・α 7・バグ 4・α 後 25・保留 1＝2026-09-24 の実数え）の着手順（ウェーブ）と干渉条件だけ**を持つ。**着手の優先度は「バグ修正 → α に要る機能」で、どちらにも属さない spec は当面着手しない**（2026-09-20 開発者指示・下の「棚卸⑮の裁定」）。
...
## spec 台帳（**表は 54 行**。うち **brief 済み 49 本**＝2026-09-13 の 30 ＋ 09-18 起票 8 ＋ 09-19 起票 3 ＋ 09-20 起票 8（#47 `balloon-break`・#48 `shiori-loadu`・棚卸⑮の 6 本＝#42 `default-balloon-nar-fold`・#49〜#53）、**#38〜#41・#54 の 5 行は brief を持たない「登記だけの行」**＝下の節を見よ。**実数え（2026-09-24・状態列が ✅ の行＝完了 17／状態列が ⚪ の行を段列で分けて α 7・バグ 4・α 後 25・保留 1 ＝ 54。棚卸⑮の 09-20 は完了 15・α 9）**。着手は brief 持ちが `/kiro-start <名>`・登記だけの行は `/kiro-discovery` から）
