...
- **⑥ emo（render-engine）**: `areka-P0-collision-geometry`（collision→region/actor 写像）／ `areka-P0-choice-render`（選択肢表示）／ `areka-P0-dual-window`（kero 2nd 窓）／ `areka-P0-emo-text-viewbox`（**スクロールの viewbox 化**＝クリップ視窓＋内容オフセット・再描画レス・pixel 等価 golden が受け入れ基準・emo-text-layer の分離シーム消費・wintf `visual-clip` ✅ 流用のみ。**brief 済 2026-07-09**・依存: emo-text-layer のみ＝並走安全）
...
- emo:`emo-text-viewbox`（emo-text-layer の分離シーム消費のみ・他ユニット非干渉）
