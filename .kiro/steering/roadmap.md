...
- `areka-P0-emo2-boot` — **M-boot 統合（マイルストーン完成ユニット・2026-07-09 新設）**: 実 sink 結線（`LogSink`×2→`SerikoSink`＋text-layer sink）＋**seriko `SurfaceOutput`→emo-present `PresentCommand` アダプタ**（scope→`TargetId` 写像・`UiSender` 配送）＋窓写像→`attach_target` 装着＋balloon target 結線＋実 pasta で boot→talk→close 実走。新規機構なし＝アダプタ1個＋結線＋観測に徹する。**⚠️ 順序ゲート: window-placement＋emo-text-layer 完了後**（並走フロントではない）。✔ (a) 決定論 spine（ScriptedShioriBackend＋実 sink 経路・CI 常設）＋(b) env-gate 実走（実 pasta・実 DPI・人間サインオフ）（**brief 済 2026-07-09**）
