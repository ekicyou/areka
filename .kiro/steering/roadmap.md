...
| 11 | `nar-install`（09-12 起票） | **α** | 基盤（`.nar` の読取・`install.txt`・安全な展開＝`areka-nar`・検体を `.nar` 保管・**検体名 → 根の共有ヘルパ＝サンプルゴースト試験の仕組みの種**） | M | **A0**（単独・**即時着手可**） | なし。38 ファイル・9 クレートに触るため全 spec と共有ファイル発生＝並走不可 | ○ | ⚪ |
...
| 37 | `default-balloon-bundle`（**09-18 起票・2 度目の再入**） | **α** | 資産（既定バルーンの選定と `.nar` 保管・候補 CC0 `Balloon for Staysee Syncfield`・第三者告知・`use_self_alpha` 常時 1 の裁量登記） | S | **A1-③** | #11（`.nar` 保管慣行）。#10・#12 とは共有ファイル 0 の見込み（id 定数 1 行は後着が #12 側へ足す・`COMPAT_ARCHITECTURE.md` §8 は #10 と別節） | −（見た目の採否は開発者が実機で） | ⚪ |
...
| 14 | `popup-menu-minimal`（**09-18 起票**） | **α** | 製品（Win32 標準メニュー・項目 7 種・`*button.caption`・登記式） | M | **A3** | #12・#13（`SwitchRequest`） | −（裁定候補 2 件） | ⚪ |
...
| **A0**（単独・**即時着手可**） | `nar-install` | `/kiro-start areka-P0-nar-install` | **α の先頭。** 38 ファイル・9 クレートの参照を 1 つの共有ヘルパへ寄せる段を含む＝全 spec と共有ファイル発生・**並走不可**（走行中 0 本の今が入れ時）。展開先の形はベースウェアの根（`<根>/ghost/<dir>/`・`<根>/balloon/<dir>/`）に揃える（brief に追記済み）。共有ヘルパは「検体名 → 根」＝**サンプルゴーストを増やして試験する仕組みの種**（検体を足す作業は `.nar` を 1 つ `vendors/sample_ghost/` に置いて名前を 1 行登記するだけになる）。`zip` の依存承認。**2026-09-18 に `shell-implicit-surface` との順序を反転**（implicit-surface は共有ヘルパを最初から使う＝nar-install 側に付け替え作業が生まれない） |
| **A1**（3 本・並走） | ① `shell-implicit-surface` ∥ ② `baseware-root-layout` ∥ ③ `default-balloon-bundle` | `/kiro-start areka-P0-shell-implicit-surface` ／ `/kiro-start areka-P0-baseware-root-layout` ／ `/kiro-start areka-P0-default-balloon-bundle` | いずれも A0 の共有ヘルパを前提。① は `areka-emo-compose`／`emo2_boot/assets.rs`／`placement/measure.rs`／`areka-parsers/src/shell/*`。② は `boot_config.rs`／`main.rs`／`areka-ghost/src/runtime.rs`／`areka-sylphya/src/persist/*`。③ は `vendors/sample_ghost/`・バルーン fixture の差し替えテスト・`THIRD-PARTY-NOTICES.md`・`COMPAT_ARCHITECTURE.md` §8。**共有ファイル 0 の見込み**（⚠ 同 crate 別ファイル: ①⇄②＝`crates/areka/src/`・①⇄③＝`COMPAT_ARCHITECTURE.md` §8 の別節＝後着 rebase）。着手時に実測で確かめ、1 つでも共有が出れば直列へ。② の裁定 ⑵・③ の採否（見た目）は要件段階で開発者が実機確認 |
...
| **A3** | `popup-menu-minimal` | `/kiro-start areka-P0-popup-menu-minimal` | `input_events/`＋新規 `menu.rs`。A2 の `SwitchRequest` を呼ぶ。裁定 ⑶⑷。**`SwitchRequest` の型が A2 の成果物**なので文書フェーズだけ A2 中に先行させてよい |
