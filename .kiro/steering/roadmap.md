...
- 互換拡充時: `surfaces-basepos`（S・ドリフト 0）／`sakura-time-directives`（L・段階 A/B/D→C）／**`charset-canon`**（M・2026-09-02 起票・SHIORI/3 の `Charset` 交渉＋`shiori.encoding`／`forceencoding`＋surfaces.txt の decode 迂回 2 箇所＝**里々ゴーストが動くための前提**・surfaces.txt 片は XS で前倒し可）。
