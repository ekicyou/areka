...
- `areka-P0-host32-lifecycle` — 常駐運転の健全性証明＋死活監視: 周期 request 連打耐性（**イベント意味論なし＝ダミー ID・OnSecondChange の意味論は kanade 領分**）・`HelperHandle::poll_exit`/`ExitKind` seam の監視常設化・**異常語彙の統一報告型（本ユニットが正本・kanade が消費）**・shutdown 全経路。自動再起動は持たない（M2）。✔ N秒運転（連打）→clean unload＋強制 kill→Abnormal/Terminated 検出（**brief 済 2026-07-05**）
