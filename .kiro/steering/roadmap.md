...
> **M1 は 2026-09-11 に完成宣言済み**（下記「M1 ゴール」）。**M2 のゴールは 2026-09-18 の棚卸⑭で「α 版」に決めた**（下記「M2 ゴール（α）」）——開発者の指示「α 版として第三者に使い始めてもらうことができるだけの機能セット。大事なのはゴースト・シェル・バルーンのファイル管理、インストール、ネットワーク更新、つまりアプリとしてのデスクトップマスコット管理。オーナードローは不要だが最低限のメニューは要る。表現力増強は emo2 が普通に動いている水準で一旦よい」。本ファイルは **spec 台帳 56 行（完了 17・α 8・バグ 4・α 後 26・保留 1＝2026-09-24 の実数え。表の段列を機械で数えた）の着手順（ウェーブ）と干渉条件だけ**を持つ。**着手の優先度は「バグ修正 → α に要る機能」で、どちらにも属さない spec は当面着手しない**（2026-09-20 開発者指示・下の「棚卸⑮の裁定」）。
...
| 42 | `default-balloon-nar-fold`（**09-19 登記・09-20 起票**） | **α** | 資産（`vendors/sample_ghost/StayseeBalloon/` が展開フォルダのまま残っている＝#37 と #11 が互いを指したまま両方着地し、誰も畳まなかった。畳む・登記表 `SAMPLES` に 1 行・直書きの検体数 2 か所と名前の一覧 2 か所と関数名を 5→6 に・追跡から外す。**旧手順に欠けていた 1 点**＝`crates/areka-emo-text/tests/staysee_balloon_fixture_test.rs` が展開フォルダを直接指しており、窓口 `SampleRoot` へ付け替えないと結合テスト一式が赤になる） | XS（2〜3 タスク） | **A1** | #11 ✅・#37 ✅（両方着地済み＝いつでも着手可） | − | ⚪ |
...
**#42 は 2026-09-20 に brief を得た**（`.kiro/specs/areka-P0-default-balloon-nar-fold/brief.md`）。旧節の手順は同 brief へ移し、欠けていた 1 点（`areka-emo-text` の結合テストが展開フォルダを直接指している）を足した。旧節の全文は history「2026-09-20 棚卸⑮退避」。
...
- **brief の体制**: 台帳 56 行のうち **50 本に brief あり**・登記だけの行 6 本（#38〜#41・#54・#56）。**完了 17・α 8・バグ 4・α 後 26・保留 1**（2026-09-24 の実数え）＝着手は該当 brief を読んで `/kiro-start <unit>` へ直行。brief の file:line は起票時値＝**着手時に必ず再検証**（棚卸⑮では A1〜A5 の全 brief とバグの性質を持つ brief を main `fe157df1` で再測定し、結果を各 brief の「2026-09-20 棚卸⑮の再測定」の節へ書いた。α 後の残りの brief は再測定していない）。
