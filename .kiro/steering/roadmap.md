...
| 47 | `balloon-break`（**09-20 起票**・開発者指示） | α 後 | 互換（バルーンの左ダブルクリックで再生中の台本を止め、全バルーンを即時に隠す。ukadoc に操作の定義は無いが `\![enter,nouserbreakmode]`・`\t`・`OnBalloonBreak` が存在を前提にしている。`nouserbreakmode` の無効化を含む。`OnBalloonBreak` の発火は #35 に残す） | S〜M | **開発者指示で 09-20 着手**（α のゴールには要らないが、指示が段の規則に優先する） | なし（先行必須の未完 spec は無い。`kanade/schedule/*` を触る #35・`translate-pipeline`・`sakura-time-directives` とは並走させない） | −（裁定 6 件・いずれも利用者から見える結果で語れる） | ⚪ |
