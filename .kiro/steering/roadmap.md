...
- 完了 spec = **155**（`.kiro/specs/completed/` の**直下エントリ数**＝ディレクトリ 154 ＋ 単体ファイル `graphics-rendering-stability.md` 1）。**ディレクトリ数を採ると 1 ずれる**ので計数時は直下エントリで数えること（旧値 151 は `file-slimming` マージ PR#103 が本行の更新を落とした stale であり、実数は 152 だった）。
...
| ~~挙動バグ~~ | ~~バルーンが他アプリ窓の背後へ埋もれない＝各バルーンを自シェルの直前へ維持~~ | `completed/ghost-window-zorder` | ✅ **完了**（2026-08-13・W6 で着地。案 A＝Win32 owner で構造保証・実機ゲート G1〜G8 全 PASS・サインオフ S1〜S4 全 PASS） |
| 挙動バグ | P2 連鎖の幅差隙間（実機 123px）解消＝SSP 実測で正典確定 | `scope-chain-gap` | **W6**（確定） |
...
- ~~**atom⇄zorder**~~ **✅ 解消**（2026-08-13 zorder 着地）〔zorder は flush 経路（`tick_bridge`／`command.rs`）を**意図的に一切触らずに**完了した——多ペア干渉の是正で flush 改造案を検討したが、本トリガに触れるという理由で却下し「1 巡 1 ペア・次巡へ持ち越し」を採った（completed/ghost-window-zorder の tasks.md に裁定記録）。よって atom 着手時の再突合は不要。ただし **atom が flush 経路を改造する場合は、zorder の維持系が同経路へ指令を積む点を確認すること**〕
...
**2026-08-13 追記(64)（`ghost-window-zorder` 完了）**: **案 A（Win32 owner）で確定**——実機ゲート G1〜G8 全 PASS ゆえ案 B・補助浮上の分岐は全て不成立（5.2〜5.7 は未実施として語彙のみ記録）。実機サインオフ S1〜S4 全 PASS（拡大率の異なる 2 画面で**同一キャラ窓が 764×1094@192dpi ／ 573×821@144dpi と異なる物理寸で往復**した証跡つき）。**検証フェーズで本物の欠陥を 3 件掘り当てた**——①残存窓の表示状態を誰も測っていない ②検証段階の `HandleMissing` 未踏 ③**挿入位置が他アプリの常時最前面窓だとバルーンが帯へ引き込まれる**（隣接は成立し検証も通るので静かに壊れる・要件 8.1 毀損）。③は開発者裁定で本仕様にて是正（タスク 6.4 新設）——**当初案の `HWND_NOTOPMOST` は実測で「既に通常窓の対象を 1 枚も動かさない」と判明し `HWND_TOP` へ**。持ち上がり幅は保証されないが隣接は **owner 関係**が保証する（一組で動く・間に窓を挟めない・owner 無しの対照では隣接しない）。**vis への申し送りは brief 追記(61) に登記済み**（呼ぶ口は `ReassertZOrder` を 1 つ insert・不可視バルーンは隣として数えられない・show と要求の順序は未検証）。**atom⇄zorder の干渉は解消**（上記台帳）。
