...
ワークスペース全体の合否は `pwsh -NoProfile -File tools/test-all.ps1 [-License]` 1 本で判定する（`kiro-complete` の Test Gate・移動後テストゲートが呼ぶ）。素の `cargo test --workspace` では足りない理由と、スクリプトが守っていること:
