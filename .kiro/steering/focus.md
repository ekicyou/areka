...
- 棚卸しの基準実数（2026-07-02 更新）: 完了 **110**（07-01 比 +7＝`areka-P0-host32-ipc`・`areka-P0-parser-foundation`・`areka-P0-shell-parse`・`areka-P0-balloon-parse`・`areka-P0-package-mount`・`areka-P0-sakura-parse` 整理・`wintf-dcomp-to-wuc-migration`） / **active = 0** / **brief-only = 2**（`wintf-clickthrough-alpha-toggle`＝着手可・`wintf-ulw-removal`＝clickthrough 完了待ちゲート下）。**M1 M-boot 進捗 6/16**（②parsers 全完了・①shiori 2/5）・**次フロント＝`areka-P0-host32-shiori-load`**（brief は着手時 just-in-time）
