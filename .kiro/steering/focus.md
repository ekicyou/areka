...
- 棚卸しの基準実数（2026-07-03 更新）: 完了 **112**（`areka-P0-host32-ipc`／`-parser-foundation`／`-shell-parse`／`-balloon-parse`／`-package-mount`／`-sakura-parse`／`-host32-shiori-load`・`wintf-dcomp-to-wuc-migration`・`wintf-clickthrough-alpha-toggle` 等） / **active = 0** / **brief-only = 4**（07-03 深掘り discovery で生成: `areka-P0-host32-request`・`areka-P0-emo-surface`・`areka-P0-window-placement`＋既存更新 `wintf-ulw-removal`＝全て**着手可**）。**M1 M-boot 進捗 約 7/16**（②parsers 全完了・①shiori: pilot✅/ipc✅/shiori-load✅）。着手順注意: emo-surface⇄window-placement は main.rs 起点衝突＝順次推奨（正本 roadmap.md ポートフォリオ節）
