...
- 棚卸しの基準実数（2026-07-09 更新⑥・`ghost-setup` 完了後）: 完了 **122**（`areka-P0-ghost-setup`＝⓪ ghost エンジン起動〜終了統括を新規完了アーカイブ。WS-A talk 契約統一〔新設 `areka-talk`〕＋WS-B 結線の背骨〔新設 `areka-ghost`〕・決定論 spine e2e S1〜S6・env ゲート実 pasta 追験） / **active = 0** / **brief-only = 3**（`areka-P0-emo-present`・**`areka-P0-seriko-engine`**・`areka-P0-window-placement`〔emo-present ゲート下〕）。**M-boot 17/21（残4）**。**即並走可能フロント2本**: emo-present／seriko-engine（`ghost-setup` は消化）。正本 roadmap.md 追記⑩
