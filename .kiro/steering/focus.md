...
- 棚卸しの基準実数（2026-07-17 更新⑯・`cue-playback-duration` 完了＝実機 #3/#4/#6＋`\s` 同期の解決）: 完了 **129**（`areka-P0-cue-playback-duration`＝**テキスト再生 duration の権威台本化**〔dola が envelope 一律 duration＋`CueSheet.absolute_start_time` 自己完結台本／sakura が純関数 `text_playback_duration` 唯一定義＋第一級 `Wait` cue＋冒頭 `ClearAll` 前置／emo-text が配送 duration へ服従・`char_wait` 撤去〕＋**cue 再生ランタイムを dola へ統合**〔`CuePlayer`／`CueSink` 1 本／変換 1 本／broadcast／占有 horizon 完了〕＋**旧世代 wintf `ecs/cue`＋dola `compile_sheet` 撤去**〔25 ファイル・4153 行減〕を新規完了アーカイブ・`cargo test --workspace` exit 0〔77 suite ok〕・**2026-07-17 実機人間サインオフ済**〔開発者判定「タイミング関係は治っている」〕） / **active = 0** / **brief-only = 2**（`areka-P0-surface-resize-resnap`〔#1〕・`areka-P0-mayuna-compose`〔#2〕）。**即並走フロント2本**（交差面ゼロ＝即並走可・mayuna は settled cue モデル〔10 variant・envelope duration・単一 `CueSink`〕へ additive に載る）。実機残件: #1/#2 は上記2本・**#7 は pasta 上流（areka スコープ外）**。実走の罠: workspace ビルドが x64 helper を `target/debug/` へ置くため実走前に **i686 helper の上書きコピー必須**。正本 roadmap.md 追記㉔
