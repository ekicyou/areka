...
- 棚卸しの基準実数（2026-07-09 更新⑥・`emo-present` 完了後＝⑥emo トラック全完了）: 完了 **122**（`areka-P0-emo-present` 新規完了アーカイブ＝emo 直列3/3・表示結線。実装完了後の実機まばたきデモ検証で `ComposeCache` のキャッシュ仕様バグ〔surface id 単独キーが bind 差分に衝突〕を発見・是正＝合成入力（surface_id＋BindSet）完全一致キーの容量1メモ化スロットへ再設計） / **active = 0** / **brief-only = 3**（`areka-P0-seriko-engine`・`areka-P0-ghost-setup`・`areka-P0-window-placement`〔**emo-present 完了でゲート解除＝新フロント**〕）。**M-boot 17/21（残4）**。**即並走可能フロント3本**: seriko-engine／ghost-setup／window-placement（`emo-present` は消化・下流 window-placement/emo-text-layer が実 DPI 表示済みの本番ゴースト example を観測土台として消費可）。正本 roadmap.md 追記⑩
