...
- 棚卸しの基準実数（2026-07-16 更新⑯・再入精査⑧＝体裁フェーズ棚卸・**M1 残工程の brief 全数完備**）: 完了 **130** / **active = 0** / **brief-only = 11**（実装中 `areka-P0-cue-playback-duration`〔別坑〕・ゲート下 `areka-P0-mayuna-compose`・並走中フロント4本 `position-persist`/`idle-talk`/`collision-geometry`/`input-events` ＋ **新規5本: `areka-P0-sakura-dialogue-tags`**〔④・\q/\_l/\![move]/%username の compile 貫通・choice cue 形の正本・cue-playback ゲート下〕・**`areka-P0-choice-render`**〔⑥・選択肢 UI・ChoiceSelection 契約の正本〕・**`areka-P0-choice-select-events`**〔③・新設名＝input-events 分離増分の宛先・任意名イベントカスケード〕・**`areka-P0-seriko-loop`**〔⑤・Tick 注入＋pattern 状態・ゲート下〕・**`areka-P0-emo2-conformance-e2e`**〔⓪統合・**M1 完成宣言ユニット**・適合検証14項目・M-dual 吸収の正本〕）。**roadmap-only（brief 無し）ユニット＝ゼロ**・**M-dual（dual-surface/dual-window）は退役**（e2e #10 へ吸収）。**M1 残工程ゴール表**を roadmap 統合点直下に新設（マイルストーン別の単一文ゴール）。並走フロントは不変（最大5本＝cue-playback 走行中 ∥ position-persist ∥ idle-talk ∥ collision-geometry ∥ input-events）・cue-playback 完了後 Wave＝mayuna ∥ sakura-dialogue-tags → seriko-loop・choice-render ∥ choice-select-events → e2e（最終）。**フェーズ別ゲート精密化（2026-07-16 追補）**: 時限ゲートの適用は tasks/実装のみ＝**ゲート下 spec の要件・設計は先行着手可**（cue-playback マージ後の `/kiro-validate-design` 再突合が義務・Fable 早期投入順=①seriko-loop 設計→②sakura-dialogue-tags 設計〔choice cue 形確定〕→③choice-render 設計。正本 roadmap 時限ゲート注記）。正本 roadmap.md 追記㉖＋M1 残工程ゴール表
- 棚卸しの基準実数（2026-07-17 更新⑰・`cue-playback-duration` 完了＝実機 #3/#4/#6＋`\s` 同期の解決・**時限ゲート解除**）: 完了 **131**（`areka-P0-cue-playback-duration`＝**テキスト再生 duration の権威台本化**〔dola が `Cue` envelope 一律 duration＋`CueSheet.absolute_start_time` 自己完結絶対時刻台本／sakura が純関数 `text_playback_duration`＋`CHAR_NOMINAL_MS` 唯一定義＋第一級 `Wait` cue＋冒頭 `ClearAll` 前置／emo-text が配送 duration へ服従・`char_wait` 撤去〕＋**cue 再生ランタイムを dola へ統合**〔`CuePlayer`〔pending_choices/`resolve_choice` 含む〕／`CueSink` 単一トレイト／`to_talk_schedule` 変換1本／`cue_target_of` 単一権威／broadcast／**占有 horizon 完了**〕＋**旧世代 wintf `ecs/cue`＋dola `compile_sheet` 撤去**〔25 ファイル・4153 行減〕を新規完了アーカイブ・`cargo test --workspace` exit 0〔77 suite ok〕・**2026-07-17 実機人間サインオフ済**〔実 emo2＋実 pasta.dll＋実 DPI＋絶対パス・6 talk 観測・開発者判定「タイミング関係は治っている」〕） / **active = 0** / **brief-only = 10**（`mayuna-compose`・`position-persist`・`idle-talk`・`collision-geometry`・`input-events`・`sakura-dialogue-tags`・`choice-render`・`choice-select-events`・`seriko-loop`・`emo2-conformance-e2e`）。**⏰ 時限ゲート解除**: 追記㉖の Wave 計画どおり **`mayuna-compose` ∥ `sakura-dialogue-tags`**（compile.rs 近接・別アーム＝マージ可）→ **`seriko-loop`**（mayuna 先行推奨＝bind 読み口・CueSink 集約確定で seriko actor 受信面の不確実性は解消）・**`choice-render` ∥ `choice-select-events`**（`CuePlayer` の `pending_choices`/`resolve_choice` が settled シームとして着地）→ **`emo2-conformance-e2e`**（最終）へ進める。**ゲート下 spec の設計は先行着手可だったため、着手前に `/kiro-validate-design` で settled cue モデル**（10 variant・envelope 一律 duration・単一 `CueSink`・`cue_target_of` 単一権威・broadcast・占有 horizon）**との再突合が義務**（追記㉖ フェーズ別ゲート精密化の履行）。実機残件: #2 は `mayuna-compose`・**#7 は pasta 上流（areka スコープ外）**（#1 は追記㉔で解決済み）。開発者所見「タイミング以外の改善が別途必要」＝本 spec の Non-Goals（描画/レイアウト・bind 合成等）ゆえ隣接 spec の領分。実走の罠: workspace ビルドが **x64 helper** を `target/debug/` へ置くため実走前に **i686 helper の上書きコピー必須**（x64 では 32bit pasta.dll を LoadLibrary 不可）。正本 roadmap.md 追記㉗
- 棚卸しの基準実数（2026-07-17 更新⑱・**ポートフォリオ合流セッション**＝並走4spec 要件マージ後の一括衝突裁定）: 完了 **130**（配置フォルダ基準の実測ディレクトリ数——更新⑰の「131」はルーズファイル `graphics-rendering-stability.md` 込み計数＝規則どおりディレクトリ基準へ是正） / **active = 4**（並走4spec が PR#61-64 で**要件フェーズ完了・spec.json 有り requirements-generated へ遷移**: `position-persist`・`idle-talk`・`collision-geometry`・`input-events`） / **brief-only = 7**（`mayuna-compose`・`sakura-dialogue-tags`・`choice-render`・`choice-select-events`・`seriko-loop`・`emo2-conformance-e2e`・`status-execution-states`〔更新⑰列挙から漏れ〕）。**合流裁定6件（正本＝roadmap 追記㉘）**: (A) 撫でクラスタ合流ゲート決着＝collision-geometry R7 二段観測改稿（実 DPI≠96 probe 自前必須・client 座標経路の点のみ証跡）＋input-events Req8.3＝合流サインオフ1回（実 resolver 前提・完了のみ合流待ち） (B) `Status`/`ShioriCall` 契約正本=idle-talk・実装推奨順 idle-talk→input-events（非ハード） (C) idle-talk 要件の実質欠陥4件を本文適用（fail-open ただし書き・OnTalk 訂正・ログ観測化・全構築点檻） (D) 実装依存順確定（**mayuna→seriko-loop は完了ゲート**〔bindgroup1400-1402 既定 OFF〕・position-persist 独立・select-events 実質最後尾・e2e 最終） (E) `\![move]` は永続値を書かない（R1.9 二層分離・2回目起動の位置蒸発は許容仕様＝e2e 実機判断へ） (F) ゴール表/時限ゲート/並走根拠/集計の文書是正。**次フロント**: 4本の設計フェーズ（idle-talk 先鋒）∥ mayuna-compose ∥ sakura-dialogue-tags（choice cue 形先鋒）・Fable 討議は seriko-loop→choice-render 直列2発。
