...
- 棚卸しの基準実数（2026-07-05 更新・マージ後の実地確認）: 完了 **116**（`areka-P0-actor-foundation`・`wintf-ulw-removal`・`areka-P0-host32-request`・`areka-P0-emo-atlas` が新規完了アーカイブ） / **active = 0** / **brief-only = 4**（`areka-P0-app-shell`〔新設=アプリ組み上げ三段の第一段・main.rs 骨格化＋モックデモを examples/mock-shell.rs へ別名保全〕・`areka-P0-emo-compose`・`areka-P0-emo-present`・`areka-P0-window-placement`〔**emo-present 完了後**にゲート解除＝本番ゴースト先行の原則〕）。**M1 M-boot 進捗 約 9/20**（②parsers 全完了・⓪actor-foundation 完了・①shiori: pilot✅/ipc✅/shiori-load✅/request✅・lifecycle 残・⑥emo: atlas✅・compose/present 残）。アプリ組み上げ＝app-shell（骨格）→ghost-setup（結線）→emo2-conformance-e2e（適合）の三段・boot/close 発火順序=kanade 運行表。正本 roadmap.md ポートフォリオ節＋所有マップ
