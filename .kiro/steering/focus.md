...
- 棚卸しの基準実数（2026-07-03 更新）: 完了 **112**（`areka-P0-host32-ipc`／`-parser-foundation`／`-shell-parse`／`-balloon-parse`／`-package-mount`／`-sakura-parse`／`-host32-shiori-load`・`wintf-dcomp-to-wuc-migration`・`wintf-clickthrough-alpha-toggle` 等） / **active = 0** / **brief-only = 6**（07-03 深掘り discovery で生成: `areka-P0-host32-request`・**emo 直列3分割** `areka-P0-emo-atlas`→`-emo-compose`→`-emo-present`（旧 emo-surface を粒度分割・αトリミングアトラス・packing クレート `rectangle-pack` 要承認）・`areka-P0-window-placement`＋既存更新 `wintf-ulw-removal`＝全て**着手可**）。**M1 M-boot 進捗 約 7/18**（②parsers 全完了・①shiori: pilot✅/ipc✅/shiori-load✅）。着手順注意: emo-present⇄window-placement は `crates/areka` 起点衝突＝順次推奨（emo-atlas/-compose は純粋層＝非衝突。正本 roadmap.md ポートフォリオ節）
