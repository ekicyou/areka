...
- 棚卸しの基準実数（2026-07-05 更新④・`emo-compose` 完了後）: 完了 **120**（追記⑦ `areka-P0-emo-compose`＝emo 直列2・合成コア〔fold→plan→blit→Composer facade・実 emo2 fixture golden・ログ発火/エラー写像も決定論的回帰檻化〕を新規完了アーカイブ） / **active = 0** / **brief-only = 3**（`areka-P0-emo-present`〔**emo-compose 完了でゲート解除＝新フロント**〕・`areka-P0-window-placement`〔emo-present ゲート下〕・`areka-P0-sakura-engine`）。**即並走可能フロント2本**: emo-present／sakura-engine（`emo-compose` は消化・下流 emo-present が `ComposedSurface`／正規化 Surface 公開形を正本消費）。正本 roadmap.md 追記⑦
