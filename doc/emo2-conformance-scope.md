...
- **M1 不要**（emo2 未使用）: raise/open/exec/anchor/timerraise/bind 等 `\!` 系、`\b \_b \i \j \& \f[] \_a \_q \_n \x`。
...
| wintf-P0-surface-hierarchy | 汎用シーングラフ（循環検出/多重同一性）は **emo2 には YAGNI**。overlay z-order＋0,0 合成で足りる。汎用エンジンは M2（ベクトル/縦書き）へ後ろ倒し |
