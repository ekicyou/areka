...
**この 2 件は 5-3 の整理の後に足したものである。** 5-3 が書く「整理の後の非空の宛先の件数」と
「空の件数」は、この 2 件を含んでいない。**この 2 件を含めて数え直すと、非空の宛先は 415 件、
空は 1,334 件で、和は 1,749 である**（数え方は 5-3 と同じ手順——4 台帳の行頭の `owner = ` の行
を全部拾い、値が空文字列のものとそうでないものに分ける）。5-3 が書く規則 ⑴〜⑶ で変えた件数は
変わらない——この 2 件は規則 ⑴〜⑶ ではなく要件 8.6 による書き足しだからである。
