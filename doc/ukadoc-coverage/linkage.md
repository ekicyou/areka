...
見出しの丸括弧に書いた段階は、読みやすさのための写しである。段階の正本は `briefing.md` の
順位表であり、食い違ったときは順位表を採る。
...
束に入らない項目は、名前付き束と同じ `[bundle."…"]` の表に `single = true` を付けて置く。表の鍵は
束の名前ではなく項目の id そのものである——要件 4.4 が「⑹ を書けない束を名付けない」と定めており、
`briefing.md` の順位の行も単独項目を id で指すからである（設計 D-2）。単独項目の囲みは `machine`・
`hand`・`foundation` を持たず、代わりに `reason`（束へ入れられない理由）を持ち、`members` は id を
1 つだけ持つ。

件数は「合計」の節の `tally.singles` に、ドメインごとの内訳は `tally.singles_by_domain` の 4 欄に
ある（0 のドメインも欄を省かない）。数え方: この節の `single = true` の囲みを数え、各 id を台帳の
`[ledger] domain` で引いて振り分けた。property・sakura-script・shiori が 0 なのは、3 台帳の全項目を
引いてこの節に置いた id が 1 つも無いことを確かめた 0 である。

出どころは 2 か所である。⑴ タスク 3.5 の「単独項目に落とす候補」の箇条書きから 1 件、⑵ タスク 3.1 の
「13 のページ単位の id の行き先」の表が行き先を「単独項目」と書いた 3 件。落とす理由はどちらも同じ
形——ページ 1 枚をまとめて指す粗い粒度なので、ページ全体が欠けたときに壊れる振る舞いを 1 つに
絞って書けない（要件 4.4）。4 件とも状態は `absent` で、壊れ方の最悪値は台帳の備考の冒頭の
「壊れ方: 黙って壊れる。」から採った。

### `ukadoc:manual_balloon`（バルーン）

```toml
[bundle."ukadoc:manual_balloon"]
single = true
members = ["ukadoc:manual_balloon"]
reason = "台帳の備考が「この項目はページ 1 枚をまとめて指す粗い粒度で、ページの中の記述を 1 つずつ分けて持っていない」と書き、担当の段も「担当が決まるのは、このページが並べるファイルのうち、descript.txt の中の個々の欄であって、ページ全体ではない」と書いている。バルーンフォルダに置くファイル一式を並べるだけのページで、ページ全体が欠けたときに壊れる振る舞いを 1 つに絞って書けない（要件 4.4）。個々の欄の帰属は同じ台帳の descript_balloon の各項目が名前付き束の側で持つ。"
domains = ["assets"]
breakage = "黙って壊れる"
themes = ["装い"]
```

出どころはタスク 3.1 の表である。13 件のうちテーマを持つ 3 件の 1 つで、`themes` の「装い」は台帳の
`values` から来ている（単独項目でも `themes` は構成 id の `values` の和集合として数え直される）。

### `ukadoc:manual_directory`（全体の構成）

```toml
[bundle."ukadoc:manual_directory"]
single = true
members = ["ukadoc:manual_directory"]
reason = "台帳の備考が「この項目はページ 1 枚をまとめて指す粗い粒度で、ページの中の記述を 1 つずつ分けて持っていない」と書き、担当の段も「担当が決まるのは、このページが並べるファイルのうち、それぞれの定義ファイルの中の個々の欄であって、ページ全体ではない」と書いている。ゴーストとシェルのフォルダ構成の全体像を示すページで、ページ全体が欠けたときに壊れる振る舞いを 1 つに絞って書けない（要件 4.4）。"
domains = ["assets"]
breakage = "黙って壊れる"
themes = []
```

出どころはタスク 3.1 の表である。このページが名指しで指す 5 つのページのうち 3 つは名前付き束へ
落ち、2 つはこの節の別の単独項目へ落ちた。その 5 本の関連の行き先は「過剰だった関連」の表に
1 行ずつ書いてある。

### `ukadoc:manual_ghost`（ゴースト）

```toml
[bundle."ukadoc:manual_ghost"]
single = true
members = ["ukadoc:manual_ghost"]
reason = "台帳の備考が「この項目はページ 1 枚をまとめて指す粗い粒度で、ページの中の記述を 1 つずつ分けて持っていない」と書き、担当の段も「担当が決まるのは、このページが並べるファイルのうち、descript.txt の中の個々の欄であって、ページ全体ではない」と書いている。ghost/master に置くファイル一式を並べるだけのページで、ページ全体が欠けたときに壊れる振る舞いを 1 つに絞って書けない（要件 4.4）。"
domains = ["assets"]
breakage = "黙って壊れる"
themes = []
```
...
single = true
members = ["ukadoc:memo"]
reason = "台帳の備考が「この項目はページ 1 枚をまとめて指す粗い粒度で、ページの中の記述を 1 つずつ分けて持っていない」と書いており、同じ備考が中身を「ベースウェアの機能比較表と、おすすめサイトのバナーなどの雑多な覚え書き」と述べている。1 つの機構を指していないので、要件 4.1 ⑹ の「欠けると壊れる既存ゴーストの振る舞い」を 1 つに絞って書けない（要件 4.4）。"
domains = ["assets"]
breakage = "黙って壊れる"
themes = []
```

出どころはタスク 3.5 の「単独項目に落とす候補」の箇条書きである。台帳の `links` が空で、関連を
1 本も持たない。

## 合計

対象は、状態が `implemented`・`vocabulary-only`・`degraded`・`absent` のいずれかである項目である。
下の囲みがその全数と分割の内訳を持つ。数はここに 1 度だけ置き、常時の検査が数え直して突き合わせる
（判定 ⑶-f）。

数え方（すべてこの節を書いた時点で数え、引き算や引用では導かなかった）。

- `target`: 台帳 4 本の全項目のうち、状態が上の 4 語のいずれかであるものを数えた。
- `from_machine`: `single = true` でない束の `members` から `hand` の id を引いた残りを、全束で合算した。
- `by_hand`: 同じ束の `hand` を全束で合算した。
- `singles`: `single = true` の囲みを数えた（単独項目の `members` は id 1 つなので、囲みの数と id の数は同じ）。
...
実体は指し先の項目が持つ——同じ実体を 2 度数えないために束へ入れない。`not-applicable` は areka が
扱わないと決めた項目（他ベースウェア専用の記述など）で、順位を付ける相手そのものが無い。除いた
件数は下の `alias_excluded`・`not_applicable_excluded` にある。この 2 つの状態の id が名前付き束にも
単独項目にも 1 つも現れないことは判定 ⑶-d が見張る。
...
SAORI は台帳に行を 1 つも持たない。**カタログ側 0 件・台帳側 0 件**である（数え方: `catalog.toml` の
項目 id 全数と台帳 4 本の項目 id 全数をそれぞれ小文字にして、部分文字列 `saori` を含むものを
数えた。どちらも 0 件）。理由は `ukadoc:spec_dll` の備考が書いている——「ukadoc に SAORI の
独立したページは無いので、SAORI 用の行は作らない」。

成立条件は 3 つで、段階 A の検証項目として扱う（要件 4.5）。`ukadoc:spec_dll` の備考の逐語は
「成立の条件は 32bit の同じプロセスに同居すること・作業ディレクトリ・DLL の探索パスの 3 つ。」で、
要件 4.5 はこの 3 つを次の綴りで名指す。

1. **32bit 同一プロセスの同居** — 備考は「同じ決まりを共有する SAORI・MAKOTO・PLUGIN の DLL は、
   読み込む呼び出しそのものが無いので同居できない」と書く。今 32bit の助け手が読み込むのは
   SHIORI の DLL だけで、SAORI の DLL を同じプロセスに載せる呼び出しは 1 か所も無い。
2. **作業ディレクトリ `ghost/master`** — SAORI の DLL は相対パスで置き場を語るので、呼ぶ側の
   作業ディレクトリが `ghost/master` に定まっていることが要る。
3. **DLL 探索パス** — 同じ理由で、`ghost/master` の下に置かれた DLL を探索パスが拾えることが要る。

**実装項目としてどの束にも入れない。** 台帳に行が無いので束へ入れる id が無く、単独項目にも
置かない（単独項目は台帳の項目 1 件を指す表であり、行の無い SAORI は指せない）。したがって
`tally` の 3 つの数のどれにも SAORI 由来の 1 件は入らない（**0 件**）。SAORI に触れる唯一の台帳の
行は `ukadoc:spec_dll` だが、これは DLL 共通の入口の決まりを述べる SHIORI 側の項目であり、
名前付き束「SHIORI の要求と応答」の構成 id に入っている——SAORI の実装項目としてではない。
備考も「areka はプロトコルを実装せず、実装の主体は SHIORI 側である」と書いている。

## 別軸

M2 の技術選定 4 つ——pasta の native x64・`IShiori` の in-proc 化・ベクトル描画・AI——は束として
名付けず、段階にもウェーブにも並べず、順位も付けない（要件 4.6・10.6）。この 1 節だけに置く。

理由は 2 つある。⑴ この 4 つは正典の記述ではなく areka の内側の実装方式の選択で、台帳 4 本にも
カタログにも対応する項目が **0 件**である（数え方: 4 つが指す機構を台帳の全項目の題と id で
引いたが、ukadoc の項目として立っているものは 1 つも無い。SAORI と違ってページ全体も存在しない）。
構成 id が列挙できないので、要件 4.1 の 8 つのうち ⑵⑶⑷⑺⑻ が書けず、束として成立しない。
⑵ 順位の 4 つの根拠（壊れ方の段・テーマの数・影響する既存資産の広さ・依存する基盤の共有度）は
すべて構成 id から導く値なので、この 4 つを同じ物差しに載せられない。

進退も網羅とは独立である。どの技術選定を採っても、正典の網羅の側の段階と束の順序は変わらない
——変わるのは 1 つの束を実装するときの中身の作り方だけである。
