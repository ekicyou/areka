...
| 一部だけが効かない | 4 | 主要な用法は動いていて、書いた引数の一部だけが落ちる |
...
| 設計の調べものが、複数の選択肢 id を取る形の縮退を拾い落としている | 設計とタスクを改訂済み。縮退は 4 件・実装済みは 22 件で確定している |
...
常設の検査をこの作業ツリーで回した結果は、**所見が 1 種類・22 件だけ**である。
その 1 種類は「実装済みとした項目のソースに正典の URL が置かれていない」で、22 件の項目は台帳で実装済みとした 22 件とちょうど一致する。URL を置くのは後の段の仕事なので、ここで出るのは想定どおりである。
**それ以外の 14 種類の所見は 1 件も出ていない。** 上の表で 0 件と書いた 2 つ——どの台帳にも無いカタログの項目・2 つ以上の台帳に現れる項目——も、この 14 種類に含まれる。

> **この結果は暫定である。** 報告を作り直したあとにもう一度回して確かめるところまでが次の段の仕事なので、最終の結果は下の枠に入る。
...
| `C40` | 4 | 主要な用法は動いていて、書いた一部だけが効かない |
...
**この表は次の段が埋める。** 検査を回して結果を書き残すのは検査の段（6.2 と 6.3）の仕事なので、この段では枠と担当だけを置く。
検査した件数も一緒に書く決まりである（0 件で緑になったのか、検査が働いて緑になったのかを読者が区別できるようにするため）。

| 番号 | 検査 | 検査した件数 | 合否 | 書く段 |
| --- | --- | ---: | --- | --- |
| V1 | 項目の塊がちょうど 342 個 | — | — | 6.2 |
| V2 | 台帳の項目の集合が骨組みの手順で得た集合と一致し、4 つの節への割り振りが合う | — | — | 6.2 |
| V3 | 項目が文字順に並び、重複が無い | — | — | 6.2 |
| V4 | 未分類が 0 件 | — | — | 6.2 |
| V5 | 必須の 7 欄がそろい、状態が空文字でなく、凍結された語彙のいずれかである | — | — | 6.2 |
| V6 | 別名の行に写像先があり、その先が台帳に実在し、その状態が別名でない | — | — | 6.2 |
| V7 | 関連の種別が 6 種のいずれかで、相手がスナップショットに実在する | — | — | 6.2 |
| V8 | テーマ名が 8 つのいずれかで、テーマが付いた行に「失うもの」の 1 文がある | — | — | 6.2 |
| V9 | 優先度の形と、空にしてよい条件 | — | — | 6.2 |
| V10 | 登場版が本文から取れる版のいずれかで、複数取れる項目は最小が採られている | — | — | 6.2 |
| V11 | ソースの正典 URL が実装済みの集合とちょうど対応する | — | — | 6.2 |
| V12 | この文書の表が、作り直した結果と 1 バイトも違わない | — | — | 6.2 |
| V13 | URL を足したファイルの行数と、組み立ての警告が 0 件であること | — | — | 6.2 |
| V14 | 変更したファイルが境界の内側だけであること | — | — | 6.3 |
| V15 | URL を置く前後でテストの結果が同じであること | — | — | 6.3 |

上の「4 つの台帳がそろった状態で確かめたこと」に書いた数も、報告を作り直したあとに 6.3 が取り直して確かめる。
