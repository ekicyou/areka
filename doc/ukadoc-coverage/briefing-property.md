...
### 「未対応」に落ちる項目は 0 件——調べたうえで 1 件も無かった

報告の状態の内訳では「未対応」の行が 0 件で並ぶ。これは書き落としではない。
**188 件を 1 件ずつ当てたうえで、1 件も当たらなかった。**

理由は 1 つである。台帳の状態は、areka が実行時に「正典の語彙かどうか」を判定する規則そのもの
（`crates/areka-sylphya/src/actor.rs:150-166` の `is_canonical_vocab`）で決めている。
その規則は、**id の根が areka 側のルート枝 10 のいずれかであるか、葉が汎用プロパティ名 17 の
いずれかであれば、正典の語彙とみなす。**
このページの 188 件は、見出しが葉だけの 35 件（汎用の葉 17・サウンドの要素葉 18）を
規則 0 で完全な名前へ戻したうえで根を見ると、**1 件残らずルート枝 10——`system`・`baseware`・
`ghostlist`・`activeghostlist`・`currentghost`・`balloonlist`・`headlinelist`・`pluginlist`・
`history`・`rateofuselist`——のどれかに属する**（葉だけの 35 件がどの根に付くかは、台帳の
各項目の「前置き」の行に書いてある）。
つまり areka はこの 188 件の名前をすべて認識していて、書き込みには「受理＋警告＋非反映」で
応答する。「areka がその名前をまったく知らない」＝「未対応」に当たる項目は 1 件も生まれない。

ここも ⑵ 節の「同じ値へ到達する名前の群 0 件」・⑶ 節の「`候補:` の行 0 件」と同じで、
**調べたが無かった**のであって調べていないのではない。

### 台帳の備考は「語彙表に文字どおり載っているか」を 2 通りに言い分ける

要件 2.3 は、葉の名前が areka 側の語彙表に**文字どおり**載っているかどうかを備考で区別せよと定める。
台帳ではこれを `語彙表:` で始まる 1 行で書く。当たり方が 2 通りあるので、書き出しを分けている。

| 当たり方 | 備考の書き出し | 件数 |
|---|---|---:|
| 見出しそのものが汎用プロパティ名 17 の 1 つと綴りまで一致する | 「見出しそのものが…」 | **19 id** |
| 見出しは載っていないが、末尾のひと区切り（葉）が 17 の 1 つと一致する | 「葉（見出しの末尾のひと区切り）が…」 | **14 id** |
| **合計** | | **33 id** |

上の **19 は id の数**で、**異なり見出しでは 17 種**である。見出し `name` と `path` はそれぞれ 2 つの id を
持ち（`:1` が汎用の葉・`:2` が 2.8.72 で加わったサウンドの要素葉）、載っているか否かは綴りの一致で
決まるので、どちらの id にもこの行が付く。**⑴ 節の L6 と ⑹ 節の是正 6 が扱う「17」は
areka 側の名前の数**（と、正典側での置かれ方が 13 ＋ 4 に分かれること）であって、
ここの 19・14・33 とは数えているものが違う。食い違いではない。

下の 14 件は、`rateofuselist` の下の 6 件・`currentghost.seriko` の当たり判定とツールチップの 6 件・
`currentghost.scope(ID).name`・`baseware.name` である。この 14 件はどれも根がルート枝 10 に属するので、
葉が一致しなくても状態は変わらない。それでも区別するのは、**areka の判定が根と葉のどちらで
当たっているかが項目ごとに違う**からで、語彙表の 17 名を動かすときに影響が及ぶ範囲がこの 33 件だからである。
...
- 「brief の書き方」の列は、brief に書いてある件数をそのまま写す。言い回しも brief のままに
  ...
  「areka 側の語彙表」と書く。
- その語彙表の中に「書き込みが有効な名前の一覧」（`crates/areka-sylphya/src/vocab/dotted.rs:72` の
  `SET_EFFECTIVE`）が入っている。この文書では最後までこれを **「areka 側の書き込み有効一覧」**と呼び、
  その中の 1 項目を指すときは**「書き込み有効一覧の 1 行」**と書く。語彙表の全体と、
  その中に入っている 1 つの一覧は別のものなので、呼び分ける。
...
（重なっている側は「書き込み有効一覧の 1 行だけを触る」ので、担当欄は値を導出する側が取る）。
...
| Z2 | brief:22 | sylphya の書き込み有効一覧に `seriko.zorder` は入れない・この brief が語彙の正本 | 同じ 1 件 | **食い違い**。C2（21 → 26 に含める）と正面からぶつかる。書き込み有効一覧の 1 行の持ち主が 2 本いる |
...
語彙表の側を直すべきかどうかは ⑸ 節で扱う。**結論を先に書くと、直すのは brief の書き方だけで、
areka 側の 17 名そのものは 1 つも変えない**（⑸ の是正 6）。正典の側で置かれ方が 2 通りに分かれるのは
事実だが、それは areka が 17 名を持っていることの誤りにはならない。
...
2. **188 件すべての本文を言い回しで走査した** — 古い呼び名・廃止・非推奨・改名・統合・置き換え・移行などを
   表す 27 語で全項目の本文を通した。当たったのは 11 件で、そのほとんどは
   「設定するとこのタグを打ったのと同じ働きになる」「書き込みは追加ではなく丸ごとの入れ替えになる」
   といった**動きの説明**だった。1 件（`currentghost.scope(ID).name`）だけは動きの説明ではなく、
   スコープの番号ごとにゴーストの定義ファイルのどのキーと同じ値になるかを述べた**値の源の説明**である。
   どちらにしても、**ある名前を別の名前へ読み替えよ、と述べたものは 1 件も無い。**

見出しが重複する `name` と `path`（各 2 件）も、同じ値への別の名前ではない。
汎用の葉の側とサウンドの要素葉の側で**指す値そのものが違う**別々の項目である。書き分けの根拠は ⑹ 節に置く。

### brief の `balloon.汎用`・`balloon.count` は頭を省いた書き方で、別の一族ではない
...
- 上流の道具には、検査ごとに「その検査が今いくつの対象を実際に調べているか」を数え、
  0 件のものを名指しで固定しているテストがある。別名の行を数える面は「対象 0 件」の主張のままでよい
...
- **`currentghost.seriko.zorder`** — `areka-P0-zorder-property` が値の導出を単独で持つと主張し、
  ...
  争点は値の導出の担い手と、書き込み有効一覧の 1 行の持ち主。主張しているのは **3 本**。
...
- **1 つ目・値を導き出す仕事を誰がやるか** — `areka-P0-zorder-property` と `areka-P0-currentghost-property-tree` が
  どちらも自分だと書いている。
- **2 つ目・areka 側の書き込み有効一覧の 1 行を誰が触るか** — `areka-P0-zorder-property` は
  「入れない・この brief が正本」と書き、
...
- areka 側の書き込み有効一覧の 1 行は `areka-P0-property-query-channels` が持つ。
  `areka-P0-zorder-property` は書き込み有効一覧に触れない。

**この案を選んだ理由**: この 3 つの組み合わせは、既に `.kiro/steering/roadmap.md` の三重所有の裁定（棚卸⑫の干渉台帳・2026-09-11 棚卸⑬で「棚卸⑬の仮裁定」節として採択）に推奨として
...
| 誰の | 採ったとき | 採らなかったとき |
|---|---|---|
| `areka-P0-zorder-property` | 範囲は今のまま（値の導出だけ）。ただし brief:22 の「書き込み有効一覧に入れない・この brief が語彙の正本」は書き直しが要る——書き込み有効一覧の 1 行は `areka-P0-property-query-channels` が入れることになるため。⑸ 節の是正候補に載せる | 主張が宙に浮いたままになる。完了済み spec `areka-P0-scope-zorder-pinning` は、先送りにした語彙の追跡先をこの spec 単独と記録している（`.kiro/specs/completed/areka-P0-scope-zorder-pinning/requirements.md:230-231`）ので、その追跡先を誰も受け取らない状態が続く |
...
| `areka-P0-property-query-channels` | 範囲は今のまま。21 → 26 の追随に `seriko.zorder`・`seriko.sticky-window` を含めてよいことが確定する | 「21 → 26」で増える 5 件のうち 2 件が誰の持ち物か決まらないまま、書き込み有効一覧を触ることになる |
...
（同 brief が扱うのは `zorder` の 1 項目だけ）。残る 2 本の争いは上の 2 つ目（書き込み有効一覧の 1 行）だけで、
1 つ目（値を導き出す仕事）のほうは初めから争いになっていない。

案 甲を採ると、この項目はこう片付く。

- 値を導き出す仕事は `areka-P0-currentghost-property-tree` が持つ（`seriko.*` の一括所有に残る。外れるのは `zorder` だけ）。
- areka 側の書き込み有効一覧の 1 行は `areka-P0-property-query-channels` が持つ（`zorder` と同じ扱い）。
...
**ここに並べるのはすべて提案で、書き換えは 1 つも行っていない。** 直すかどうかを決めるのは
各 brief の持ち主と、4 ドメインをまとめる担当（`areka-P0-ukadoc-coverage-roadmap`）である。
本調査は既存の brief の本文に 1 文字も触れていない。

書き方は「どの brief のどの記述を、どの id の一覧へ書き換えるか」で揃える。
差し替える先の id は、この文書の前の節に既に表として置いてあるので、そこを指す。

### 件数を直す候補（4 件）

いずれもカタログの側を正とした結果である。数の突き合わせの中身は ⑹ 節にある。

| # | どの brief のどこ | 今の記述 | どう直すか | 差し替える先の id の表 |
|---|---|---|---|---|
| 是正 1 | `areka-P0-property-catalog-lists` brief:17 | 「`ghostlist` ×5・`activeghostlist` ×5＋`.ext` ＝ 12」 | 合計は **10**。`.ext` の 2 件は `activeghostlist` の 5 件の中に既に入っているので、別に足すと二重になる | ⑴ 節「L2」の 10 行 |
| 是正 2 | 同 brief:18 | 「`balloonlist` ×3・`headlinelist` ×2・`pluginlist` ×4＋`.ext` ＝ 11」 | 合計 11 は動かないが内訳が違う。`headlinelist` は **3**、`pluginlist` は **5**（どちらも `.count` の葉を数え落としている）。`.ext` の 2 件は `pluginlist` の 5 件の中にある | ⑴ 節「L3」の 11 行 |
| 是正 3 | 同 brief:19 | 「`history.*` 8」 | **12**。4 つの枝それぞれに `.count` の葉がもう 1 つある | ⑴ 節「L4」の 12 行 |
| 是正 4 | `areka-P0-currentghost-property-tree` brief:4・brief:22 | 「`currentghost.*` 枝（≈65 項目）」 | この spec が実際に主張している範囲は **66 件**（枝ごとに書いてある数の合計 62 ＋ brief:23 が括弧書きで挙げている `mousecursor` 系 4 件）。`currentghost` で始まる項目はカタログでは 69 件で、残る 3 件は `currentghost.sound.*` ＝ `areka-P0-property-catalog-lists` へ切り出し済み | ⑴ 節「T1〜T7」と「T2」の表。突き合わせの全体は ⑹ 節の① |

### 名前の書き方を直す候補（2 件）

| # | どの brief のどこ | 今の記述 | どう直すか | 理由 |
|---|---|---|---|---|
| 是正 5 | `areka-P0-currentghost-property-tree` brief:23 | 「`balloon.汎用` ＋ `balloon.count`」 | `currentghost.balloon.汎用プロパティ名` と `currentghost.balloon.count` の完全な名前に書き直す（⑵ 節の 2 行の表がその 2 件） | 短く書いただけで、指しているものは正しい（⑵ 節で確かめた）。ただし頭の `currentghost.` を落とすと「`balloon.*` という別の一族がある」と読めてしまう。実際にそう読み違えた記録がある——本調査の要件 3.8 は、この箇条書きを「古い名前の例示」と読んで「カタログに存在しない」と書いていた |
| 是正 6 | `areka-P0-property-catalog-lists` brief:21 | 「汎用プロパティ名（共有葉）17 ／ `GENERIC_PROP_NAMES`（sylphya 登記済み・17 で一致）が各カタログ根の下で乗算される」 | 17 を **13 ＋ 4** に分け、「各カタログ根の下で乗算される」という説明は 13 件の側にだけ掛ける | 下の「是正 6 の中身」 |

#### 是正 6 の中身

`menu`・`sakura.bind.menu`・`kero.bind.menu`・`char*.bind.menu` の 4 件は、確かに areka 側の
汎用プロパティ名 17 名（`crates/areka-sylphya/src/vocab/dotted.rs:37`）に文字どおり載っている。
だから brief がこの 4 件を自分の範囲に数えたのは憶測ではなく、台帳の担当欄も
`areka-P0-property-catalog-lists` のままでよい（この 4 件を主張しているのはこの brief だけである）。

成り立たないのは**理由付けのほう**である。正典はこの 4 件の使える場所を 1 つに絞っていて、
本文の頭に括弧書きでそう断っている——`menu` は `currentghost.shelllist` の下だけ、
残る 3 件は `currentghost` の直下だけである。「どのカタログ根の下でも乗算される」が当てはまるのは、
正典の側では残る **13 件**だけになる。

| 群 | 件数 | 正典での置かれ方 | 差し替える先の id の表 |
|---|---:|---|---|
| どのルート枝の下でも使える葉 | 13 | 前置きを付けて使う。カタログには葉を 1 件だけ置く | ⑴ 節「L6」の 13 行の表 |
| `menu` の群 | 4 | 使える前置きが 1 つに限られる。いずれも書き込みが有効で、優先度は 4 件とも `C10` | ⑴ 節「L6」の 4 行の表 |

### 裁定が下りた場合にだけ要る候補（1 件）

| # | どの brief のどこ | 今の記述 | どう直すか |
|---|---|---|---|
| 是正 7 | `areka-P0-zorder-property` brief:22 | 「sylphya の SET 有効プロパティ一覧（`crates/areka-sylphya/src/vocab/dotted.rs`・21 項）に `seriko.zorder` は**入れない**（… 本 brief が語彙の正本）」 | ⑷ 節の案 甲を採る場合にかぎり、「一覧への登録は `areka-P0-property-query-channels` が行う。本 brief が正本として持つのは読み書きの書式（brief:13-17）であって、一覧の 1 行ではない」の形に直す |

**この 1 件だけは裁定に従属する。** 案 甲が採られなければ書き直しは要らない。対象の id は
`currentghost.seriko.zorder:1` の 1 件で、⑷ 節の「案 甲を採ったとき／採らなかったときに動くもの」の表が
ここを指している。今のままだと、同じ 1 行について「入れない」と「21 → 26 で入れる」という
正反対の指示が 2 本の brief に残る。

### 上流の README への提案（1 件）

台帳の形式の正本は `doc/ukadoc-coverage/README.md` である。本調査から提案するのは 1 件だけで、
**優先度の後ろに付く数値の刻み方**である。

- **今の README**: 優先度は「段階 1 文字（A〜E）＋数値」で、例として `"C1"`・`"C2"` を挙げ、
  「後ろの数値は同じ段階の中での並び」と説明している（`README.md:115`・`:127`・`:306`）。
- **提案**: 数値を **10 刻み**にし、段階の末尾を `C90` と定める。本台帳は既にこの形で書いてある
  （使った値は `C10` 26 件・`C20` 2 件・`C30` 7 件・`C90` 151 件・空文字 2 件）。
- **なぜ 10 刻みか**: 通し連番にすると、あとから 1 件を前へ挿すたびに同じ段階の全部を振り直すことになる。
  本ドメインは 188 件あるので、裁定が 1 つ下りて 1 件が前へ出るだけで大量の行の数値が動き、
  差分が読めなくなる。10 刻みなら既存の値を 1 つも動かさずに間へ 9 件まで挿せる。
  台帳は人が手で書く文書なので、書き換える行が少ないほうがよい。
- **決めるのは統合担当**: 4 本の台帳で揃っていないと意味が無い形なので、本調査は提案までにとどめる
  （段階そのものの最終決定も統合担当の仕事である）。

もう 1 件、ソースに置く正典 URL のコメントの形についても提案する予定があったが、**取り下げた。**
上流の着地した実装が `///`・`//!`・`//` の 3 つを拾うことを確かめたためである
（`crates/ukadoc-survey/src/evidence/extract.rs:37`）。提案するまでもなく既に動く。

### 判断の記録（2 件・是正候補ではない）

どちらも担当欄の中身は変わらないが、あとから「なぜそう決めたのか」を問われる形なので残しておく。

- **`menu` の群 4 件を `areka-P0-property-catalog-lists` に付けた根拠** — この 4 件を自分の範囲だと
  書いている brief はこの 1 本だけである（主張者が 1 本ならそこを担当にする、という規則が当たった）。
  brief が「17 名」を areka 側の名前の一覧と名指しで同じものだと書いており、その一覧に 4 件が
  実在するので、対応付けは憶測ではない。上の是正 6 が直すのは件数の内訳と理由付けだけで、担当は動かない。
- **`currentghost.sound.*` の頭 3 件に「語彙表だけを触る spec」の行を付けなかった理由** —
  `areka-P0-property-query-channels` brief:29 が挙げているのは「サウンド語彙 ≈18 葉」で、
  これは要素の葉 18 件を指している。頭の 3 件
  （`currentghost.sound.count:1`／`currentghost.sound.index_28ID_29._30b5_30a6_30f3_30c9_30d7_30ed_30d1_30c6_30a3_540d:1`／
  `currentghost.sound_28_8981_7d20_540d_29._30b5_30a6_30f3_30c9_30d7_30ed_30d1_30c6_30a3_540d:1`）まで
  広げるのは、書いていないことを補うことになる。担当欄はどちらの読みでも
  `areka-P0-property-catalog-lists` で変わらない。

## ⑹ カタログとの件数の差

### 数え方が食い違ったときはカタログを正とする

この節の数はすべて `doc/ukadoc-coverage/catalog.toml`（正典のページから機械で写した一覧）を数えた結果で、
brief の数と合わないときは**カタログの側を正としている。** brief の数は 2026-08-27 の手作業の調べに
由来していて、id を 1 つも列挙していないため、後から数え直す手がかりが無い。

差は全部で **6 つ**ある。本調査の要件が名前を挙げているのは上の 4 つで、下の 2 つは実際に数えて出てきた。

| # | どの brief のどこ | brief の数 | カタログ | 差 | 原因 |
|---|---|---:|---:|---:|---|
| ① | tree brief:4・brief:22 の `currentghost.*` | ≈65 | 69 | ＋4 | 下の「①の突き合わせ」 |
| ② | lists brief:19 の `history.*` | 8 | 12 | ＋4 | 4 つの枝それぞれにある `.count` の葉を数えていない |
| ③ | lists brief:18 の `headlinelist` | 2 | 3 | ＋1 | `.count` の葉 1 件を数えていない |
| ④ | lists brief:18 の `pluginlist` | 4 | 5 | ＋1 | `.count` の葉 1 件を数えていない |
| ⑤ | lists brief:17 の `ghostlist`＋`activeghostlist` | 12 | 10 | −2 | `.ext` の 2 件を `activeghostlist` の 5 件と別に足している。5 件の中に既に入っているので二重 |
| ⑥ | lists brief:21 の汎用プロパティ名 | 17 | 13 | −4 | 17 は areka 側の名前の一覧の件数。正典で「どの根の下でも使える葉」は 13 件で、残る 4 件は `menu` の群（是正 6） |

① は是正 4 に、②③④ は是正 2・是正 3 に、⑤ は是正 1 に、⑥ は是正 6 に、それぞれ
「どの id の一覧へ書き換えるか」の形で並べ直してある。

### ①の突き合わせ — `currentghost` 69 と「≈65」

**この差は「`mousecursor` 系 4 件」だけでは説明が付かない。** 4 を足しただけでは 65 に 1 足りない。
順に足していくと次のようになる。

| 積み上げ | 件数 | 何を足したか |
|---|---:|---|
| tree brief が枝ごとに書いている数の合計 | 62 | 19（`balloon.scope(ID)` 系＋`balloon` 直下 2）＋17（`scope(ID)` 系）＋6（`mousecursor`）＋14（`seriko`）＋4（`shelllist`）＋2（`status`・汎用） |
| ＋ brief:23 が括弧書きで名前だけ挙げている `currentghost.balloon.mousecursor` 系 | 66 | どの束にも足していない 4 件（⑴ 節 T2 の表） |
| ＋ `currentghost.sound.*` | **69** | tree の範囲外。lists brief:22 が「tree spec から明示的に切り出し」と書いて引き取っている 3 件 |

**確かなのはここまでである。** 66 が tree の今の主張の範囲、69 がカタログの総数で、
差の 3 件は所有が別の spec へ移っているだけである。抜けでも重複でもない。

**「≈65」という数だけは、brief 自身のどの足し算とも一致しない。** 62 でもなく 66 でもない。
brief の中の数で 65 にちょうど届く組み合わせは 62 ＋ サウンド 3 の 1 通りだけで、
lists brief:22 が切り出しを明記していることを踏まえると、**≈65 は切り出しの前に数えた値が
そのまま残ったもの**と読むのが最も自然である。ただし brief は「≈」を付けて概数として書いており、
この読みを裏づける記述はどの文書にも無い。**この 1 点だけは推測で、確定していない。**

是正 4 では 65 の出どころを詮索せず、切り出し後の範囲である **66** に置き換えることを提案している。
どちらの読みを採っても 66 になるので、この推測の当否を待たずに直せる。

### 見出しが重複する `name`／`path` の 4 行

見出しの名前だけでは id が決まらない項目が 2 つある。`name` と `path` で、それぞれ 2 つの id を持つ。
どちらの群に属するかは、本文と、正典のページの中での並び順の 2 つで決まる。並び順の番号は
このページの 188 項目を上から数えたときの位置である。

| 見出し | id | どちらの群か | 判別の根拠 |
|---|---|---|---|
| `name` | `name:1` | 汎用プロパティ名の葉（13 件） | 本文は対象そのものの呼び名を返すとだけ述べ、版番号を持たない。並びは 154 番で、`sakuraname`・`keroname` と続く群の先頭 |
| `path` | `path:1` | 汎用プロパティ名の葉（13 件） | 本文は対象そのものの置き場所を返すとだけ述べ、版番号を持たない。並びは 159 番で同じ群の中 |
| `name` | `name:2` | サウンドの要素葉（18 件） | 本文は再生の対象をどう書いて指定したかを返すと述べ、2.8.72 で加わったと記す。並びは 171 番で、`char*.bind.menu`（170 番）の直後＝サウンドの葉の群の先頭 |
| `path` | `path:2` | サウンドの要素葉（18 件） | 本文は再生の対象の解決後の置き場所を返すと述べ、2.8.72 で加わったと記す。並びは 172 番でサウンドの群の 2 番目 |

同じ判別は台帳の該当 4 項目の備考にも 1 行ずつ入れてある。**この 4 件は同じ値への別の名前ではない**
（⑵ 節でその判定をしている）。指している値そのものが違う、別々の項目である。

### サウンドの要素葉 18 件の全列挙

この 18 件だけは、他の行のように「接頭辞＋件数」で書けない。共通の頭を持たないからである
（`duration`・`error`・`id`・`loop`・`meta.*` 8・`name`・`path`・`pause`・`playing`・`position`・`preload`）。
「サウンドの要素葉 18」とだけ書いても brief の持ち主が id の一覧へ書き換えられないので、ここに全部並べる。

この 18 件を指しているのは `areka-P0-property-catalog-lists` brief:22 の「サウンド語彙族 ≈21」の後半と、
`areka-P0-property-query-channels` brief:29 の「サウンド語彙 ≈18 葉」である。
どちらも件数は合っていて、直す必要は無い。
...
| `meta.artist` | `meta.artist:1` |
| `meta.albumartist` | `meta.albumartist:1` |
| `meta.album` | `meta.album:1` |
| `meta.genre` | `meta.genre:1` |
| `meta.year` | `meta.year:1` |
| `meta.track` | `meta.track:1` |
| `meta.artwork` | `meta.artwork:1` |

`currentghost.sound.*` の頭 3 件はこの 18 件に含まれない。頭のほうは前置きを与える側で、
合わせた 21 件（3 ＋ 18）が lists brief:22 の「≈21」に当たる（⑴ 節の L7 の行）。

## 末尾 — 検証の結果

この節は、ここまでに書いたことが本当に確かめられたのかを、走らせた手順の出力のまま残すためにある。
合否だけでなく **「その検査が実際に何件を見たのか」** を並べる。
1 件も見ずに緑になった検査と、働いて緑になった検査とでは、読む人にとって意味がまるで違うからである。

### 道具の検査 — 食い違い 0 件・証拠のある項目 2 件

上流の道具（`cargo run -p ukadoc-survey -- check`）は 2 つの数を出す。
...
証拠のある 2 件は `baseware.name:1` と `baseware.version:1` で、どちらも根拠のある場所として
`crates/areka-ghost/src/sylphya_wiring.rs` が挙がる。台帳で「実装済み」にした 2 件と同じ 2 件である。
道具はこの 1 回の走行で、台帳が読めるか・件数が 188 か・カタログと逐語で一致するか・文字順と重複・
他ページの混入・必須の欄・別名の連鎖・関連の相手の実在・版番号・テーマ名・実装済みの証拠・報告の陳腐化を
まとめて見ている。**0 件はそのどれにも引っかからなかったという意味である。**

### 自前の 4 検査 — すべて緑・いずれも 188 件を実際に見た

道具が持っていない 4 つだけを、使い捨ての短い手順で見た（手順は repo に置いていない）。

| 検査 | 見たこと | 合否 | 検査した件数 | 内訳 |
|---|---|:--:|---:|---|
| 6b | 分類の済んでいない項目が 1 件も無い | 緑 | 188 | 分類済み 188・未分類 0 |
| 6c | 別名・対象外を除く全項目で、備考の**中身**が空でない | 緑 | 188 | 免除して飛ばした件数 **0** |
| 7 | 担当欄が、`.kiro/specs/`（`completed/` を含む）に実在する spec 名か空文字 | 緑 | 188 | 非空 **186**。実在を照合できる spec は 192 件ある |
| 8 | 優先度が空文字・`C10`・`C20`・`C30`・`C90` のいずれか | 緑 | 188 | 非空 **186**。値の内訳は `""` 2・`C10` 26・`C20` 2・`C30` 7・`C90` 151 |

**検査 7 と 8 は、途中まで「見るものが無いまま緑」だった。** 担当欄も優先度も、書き始める前は 188 件すべてが
空文字で、空文字はどちらの検査でも合格である。この 2 つが意味のある緑になったのは、
担当欄を埋めたあと（⑴ 節と ⑶ 節のもとになった作業）と、優先度を置いたあとで、
それぞれ非空が 0 件から 186 件へ増えた時点である。上の表の「非空」の列はそのための数で、
**この数が 0 のあいだの緑には何の意味も無かった。**

**4 つとも赤にできることを別に確かめてある。** 台帳そのものには触れずに 20 通りの作り物を通したところ、
赤になるはずの 11 例が赤・緑のままでよい 9 例が緑になり、20 例すべてが期待どおりだった。
備考を半角空白 3 つにしても、全角空白にしても、改行とタブだけにしても検査 6c は赤になる。
担当欄に実在しない spec 名を入れても、`completed`（spec ではなく入れ物の名前）を入れても検査 7 は赤になる。
優先度を小文字にしても、90 より大きい数値にしても検査 8 は赤になる。
**この較正があるので、上の 4 つの緑は「検査が甘いから緑」ではない。**

### ワークスペースの標準のテスト

**`cargo test --workspace` の数だけは、この節を書く前の段階で測ったものを持ち越している**（この節を書く時点で測り直してはいない）。

- `cargo test --workspace` — 成功（終了状態 0）。**101 スイート・7,007 本成功・0 本失敗・40 本 ignored・0 本除外**。
- ignored の 40 本は**すべて合流点（`2350e68a`）の時点から在った**もので、
  本調査が新しく `#[ignore]` を付けた箇所は **0 件**である。緑にするために飛ばした試験は 1 つも無い。

この節を書く時点で測り直したのは、上流の道具そのものの試験である。

- `cargo test -p ukadoc-survey` — **596 本成功・0 本失敗**。
  台帳・カタログ・報告の整合を見る側が、本調査の書き込みを入れたうえで全部通る。

### 対象が 0 件で緑になった検査

**まるごと対象 0 件で緑になったのは、検査 9（別名の行き先がまた別名になっていないか）ただ 1 つである。**
このドメインには「同じ値へ 2 通りの名前で行き着く組み合わせ」が 1 件も無いので（⑵ 節）、
別名の行き先を書いた欄も、置き換えを書いた欄も、状態が「別名」の行も、いずれも 0 件。
検査 9 は 1 件も見ずに緑になった。**働いて緑になったのではない。**

一部だけ空振りしたものが 3 つある。原因はどれも同じで、このドメインの別名が 0 件だからである。

| 検査 | 空振りした部分 | 実際に働いた部分 |
|---|---|---|
| 6a | 別名の行き先の欄が「別名」の行にだけ在るか、を見る枝 | 必須の欄がそろっているか・状態が定めた語彙のどれかか、は 188 件で働いた |
| 10 | 相手の実在を見るうち、別名の行き先と置き換えを見る枝 | 「関連」の相手は 73 本ぶんを実際に照合した |
| 6c | 別名・対象外を免除する枝（免除 0 件） | 検査そのものは 188 件すべてを見ている |

**痩せてはいるが空振りではない**検査も、主語の件数を添えて残す。同じ緑でも重さが違うからである。

| 検査 | 本台帳が与えた主語の件数 |
|---|---|
| 7 | 186 回の照合。ただし通った spec 名は **3 種類**だけ（`areka-P0-property-catalog-lists`・`areka-P0-currentghost-property-tree`・`areka-P0-sylphya`） |
| 10 | 関連 **73 本**・関連を持つ項目 **40 件**・相手の id は **39 種類** |
| 11 | 版番号を持つ項目 **99 件**（番号の種類は 21 種） |
| 12 | テーマを持つ項目 **3 件**。凍結された 8 語のうち、本台帳のデータが通したのは **「装い」1 語だけ** |
| 13 | 証拠のある項目 **2 件** |

### 検査 15（ソースの差分の形）は手順だけでは足りなかった

検査 15 を回した使い捨ての手順には、**確かめていないことが 3 つある。**
緑という結果だけを見て「ソースは全部見た」と読まないでほしい。3 つとも人が差分を目で読んで別に確かめた。

1. **コメント 2 行が定義行の直前に在ることを、手順は見ていない。** 手順が見たのは
   「足された行がちょうど 2 行で、どちらも id 付きの正典 URL の形をしている」ことだけである。
   同じ 2 行をファイルの先頭へ移しても手順は緑になってしまう。
   「値を定義している各要素の直前の行」という置き場所は、差分を読んで確かめた。
2. **手順は `crates/ukadoc-survey/tests/consistency/checks.rs` をまるごと免除している。**
   このファイルは上流の道具が持つ「どの検査が今いくつの対象を見ているか」の一覧で、
   本調査が台帳に中身を入れたぶん、主張を「対象 0 件」から「対象あり」へ書き換える必要があった。
   手順はこのファイルの差分の中身を一切見ないので、**逆に主張を弱めて「対象 0 件」へ戻しても緑になる。**
   弱めていないことは、このファイルが件数をどう突き合わせているかを読んで確かめた。
3. **手順は `crates/` の外を 1 つも見ない。** 全体の道筋の文書や他 3 ドメインの台帳が動いていても素通りする。
   次の節の確認は、この手順に頼らず別に取った。

### 触っていないことの最終確認

`main` との合流点（`2350e68a`）と今の先端を比べた。**比べる前に落とし穴を 1 つ塞いである** ——
綴りを間違えたパスを指すと差分は空になり、しかも成功として返る。
「差分が無い」と「そんなパスは無い」が同じ見た目になるので、
**先に両方の時点でそのパスが実在することを確かめ、実在しないものは「差分なし」と数えない**手順にした。
わざと綴りを崩した 3 つのパスを通したところ、3 つとも「差分なし」ではなく「実在しない」として弾かれた。

次に挙げるものは、**すべて両方の時点で実在し、すべて差分が無い。**

| 触っていないもの | パス |
|---|---|
| テーマの定義書 | `doc/ukadoc-coverage/values.md` |
| 台帳の形式の正本 | `doc/ukadoc-coverage/README.md` |
| 正典から機械で写した一覧 | `doc/ukadoc-coverage/catalog.toml` |
| 4 ドメインをまとめる報告 | `doc/ukadoc-coverage/report/summary.md` |
| 他 3 ドメインの台帳 | `ledger/assets.toml`・`ledger/sakura-script.toml`・`ledger/shiori.toml` |
| 他 3 ドメインの報告 | `report/assets.md`・`report/sakura-script.md`・`report/shiori.md` |
| 既存 4 本の brief の本文 | lists・tree・channels・zorder の各 `brief.md` |
| 全体の道筋 | `.kiro/steering/roadmap.md` |
| 上流の道具の本番のコード | `crates/ukadoc-survey/src/`（71 ファイル） |

そのうえで、合流点から動いたファイルを 1 つ残らず並べると **11 本**だった。

| ファイル | 動き |
|---|---|
| `doc/ukadoc-coverage/ledger/property.toml` | ＋1,913 ／ −941 |
| `doc/ukadoc-coverage/report/property.md` | ＋12 ／ −7（道具で作り直したもの・手では触っていない） |
...
そこから 1 ファイルも持ち込んでいない。

### 設計時に書く予定だった注記が要らなくなった経緯（1 行）

設計の時点では「上流の 2 つの検査（台帳とカタログの照合・関連の相手の実在）は他 3 ドメインの台帳が揃うまで
本当には確かめられない」と断る予定だったが、上流の実装が 4 ドメインの台帳ごと着地したため 2 つとも成立し、
その断り書きは不要になった。
