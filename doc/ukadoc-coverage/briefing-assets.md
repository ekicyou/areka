...
> **⚠ 2026-09-19 の追記——この節の数はもう現状ではない。** 仕様 `areka-P0-nar-install` が着地して、書庫を開いて配置するところまでの実装（クレート `areka-nar`）と、伸長のための依存 1 本が入った。したがって下の表の「`.nar` の綴り 0 行」「`install.txt` の綴り 3 行」「書庫を開く依存 0 件」と、それに続く 6 段の「areka のいま」のうち **② 展開と ③ 配置**の記述は、いずれも**着地前の写真**である。この追記で直したのは、検体の置き場が 32bit 実証例の展開済みフォルダから配布形の `vendors/sample_ghost/*.nar` へ移ったことによる**道の綴り**だけで、数と結論の測り直しはしていない。測り直しは網羅台帳の統合担当（`ukadoc-coverage-roadmap`）の仕事である。なお網羅台帳の `descript_install` の 11 項目の状態は、同じ日にこの仕様を担当として登記したうえで **`absent` のまま**にしてある（理由は台帳の備考にある）。
...
| `OnUpdate` の綴り | 同上 | **0 行**。2026-09-06 の時点では `crates/` 全体まで広げると 5 行あったが、いずれも試験用ゴーストの辞書の中で areka のコードではなかった。その辞書は 2026-09-19 に配布形の検体へ畳まれ、いまは `vendors/sample_ghost/emo2.nar` の中の `ghost/master/dic/update.pasta` に在る（展開した実物の道は `cargo run -p sample-ghost-kit --bin nar-sample-path -- emo2` が出す `folder=` の行の下）。**`crates/` 全体で数え直しても 0 行**である |
...
読む側の実装は無いが、**正典の書式で書かれた実物は試験用ゴーストの中に 7 本ある**。置き場を数え直したところ、うち 2 本はゴースト本体の中ではなくその兄弟にあった。7 本という数のほうは合っていた。各段の実例にはこの 7 本を使う。

**2026-09-19 に置き場が変わった。** 7 本はいずれも配布形の検体 `vendors/sample_ghost/*.nar` の中にあり（`emo2.nar` に 5 本、`emo2-kakukaku-offsetdpi.nar` と `emo2-kakukaku-wplimit.nar` に 1 本ずつ）、下の表には**どの検体の中の、どの道か**を書く。7 本という数は変わっていない。展開した実物の絶対パスは `cargo run -p sample-ghost-kit --bin nar-sample-path -- <検体名>` が出す `folder=` の行が教える。なお 5 本目の検体 `R_POST_and_KOMAINU.nar` にも `install.txt` が 1 本あるが、これは試験用ゴーストの外なので上の 7 本には数えていない。

| # | 置き場（検体と、その中の道） | 何のファイルか | 中身 | 使う段 |
|---|---|---|---|---|
| 1 | `emo2.nar` の `install.txt` | 配布物を入れる指示（ゴースト本体） | 6 行。文字コードの宣言・種別・名前・入れ先の名前・同梱バルーンの入れ先と取り出し元 | ③ |
| 2 | `emo2.nar` の `emo2-kakukaku/install.txt` | 同（同梱バルーン） | 4 行。文字コードの宣言・種別・名前・入れ先の名前 | ③ |
| 3 | `emo2-kakukaku-offsetdpi.nar` の `install.txt` | 同（検証用に増やしたバルーン） | 4 行。同じ 4 つの欄 | ③ |
| 4 | `emo2-kakukaku-wplimit.nar` の `install.txt` | 同（検証用に増やしたバルーン） | 4 行。同じ 4 つの欄 | ③ |
| 5 | `emo2.nar` の `updates.txt` | 更新のときに突き合わせる一覧 | 109 行。文字コードの宣言 1 行＋ファイル 1 つにつき 1 行の 108 行。各行は道・取り違えを見つけるための値・大きさ・日付を区切り文字でつないでいる | ⑤ |
| 6 | `emo2.nar` の `ghost/master/updates.txt` | 同（ゴースト側の置き場に置いた写し） | #5 とバイト単位で同一である（`cmp` で確認） | ⑤ |
| 7 | `emo2.nar` の `delete.txt` | 更新のときに消す道の指示 | 2 行。文字コードの宣言と、消す道 1 つ | ⑥ |
