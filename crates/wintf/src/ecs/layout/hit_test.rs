...
// ============================================================================
// is_in_click_through_area - HitTest::None エリア判定
// ============================================================================

/// 指定座標が HitTest::None エンティティの bounds 内にあるかを判定
///
/// クリックスルー（WS_EX_TRANSPARENT）のタイマー復帰判定で使用。
/// 通常の `hit_test` は `HitTest::None` をスキップするため、
/// 背後の親エンティティが返されてしまう。この関数は逆に
/// `HitTest::None` のエンティティのみを対象として bounds チェックする。
///
/// # Arguments
/// - `world`: ECS World 参照
/// - `root`: 検索ルートエンティティ
...
///
/// # Returns
/// - `true`: 座標が HitTest::None エンティティの bounds 内にある
/// - `false`: そのようなエンティティがない
pub fn is_in_click_through_area(world: &World, root: Entity, screen_point: PhysicalPoint) -> bool {
    let mut traversal = DepthFirstReversePostOrder::new(root);
    ...
    while let Some(entity) = traversal.next(world) {
        ...
        if mode == HitTestMode::None {
            if let Some(global) = world.get::<GlobalArrangement>(entity) {
                if global.bounds.contains(screen_point.x, screen_point.y) {
                    return true;
                }
            }
        }
...
/// 指定Window内でクライアント座標が HitTest::None エリア内かを判定
///
/// # Arguments
/// - `world`: ECS World 参照
/// - `window`: Window エンティティ（WindowPos を持つ）
...
/// # Returns
/// - `true`: 座標が HitTest::None エンティティの bounds 内にある
/// - `false`: そのようなエンティティがないか WindowPos が存在しない
pub fn is_in_click_through_area_in_window(
    world: &World,
    window: Entity,
    client_point: PhysicalPoint,
) -> bool {
    let Some(window_pos) = world.get::<WindowPos>(window) else {
        return false;
    };
    ...
    is_in_click_through_area(world, window, screen_point)
}
