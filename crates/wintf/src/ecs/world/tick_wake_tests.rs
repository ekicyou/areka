...
//! 触る検査（本ファイル・`world_tick_gate_tests.rs`・`runtime/tick_bridge.rs`・
//! `ecs/drag/systems.rs`）は、全部この同じ錠を取ってから触る。ここに自前の錠を作っては
//! ならない。
//!
//! なお錠は**他の検査**からしか守らない。旗を立てるのは本番の経路（窓書込指令の積み上げ
//! ほか）で、そちらは錠を知らないから、「旗が立っていないはず」を共有の旗の上で主張して
//! はならない。主張してよいのは「立てた旗が含まれる」側だけである。
//!
