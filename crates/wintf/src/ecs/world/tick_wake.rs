...
/// Z 順の要求（スコープ内ペアの維持・タグ入口）。
///
/// ⚠ 2026-08-31 訂正: 以前は「利用者の操作への追随」も挙げていたが、その引き金は
/// `areka-P0-scope-zorder-pinning` の退役（外部由来の重なり変化を捉える経路の撤去）で
/// 消えている。所有の鎖は構造で順を保つので、追随のための起床を必要としない。
///
