...
//! # 錠を取らないことが要点
//!
//! かつては、カウンタを触る／読むテストを直列化する錠（`command.rs` の
//! `lock_self_initiated_for_test()`）があった。本ファイルはそれを**意図的に取らなかった**。
//! 錠なしで、しかも自分で別スレッドを起こして並列に走らせたうえで緑になる——これがスレッド
//! 局所化の成立そのものだからである。錠を足せばこのテストは何も測らなくなる。
//!
//! 錠はもう存在しない（呼出は spec `areka-P0-test-cage-determinism` のタスク 7.2 で 21 箇所
//! すべてが外れ、定義も同 9.1 で削除された）。ここに錠を足し直さないこと。
//!
//! # 待ち合わせは channel だけ
//!
