...
// ============================================================================
// 後押しの選定——自分の窓 2 枚しか参照しない（動かすのは鎖の根）
// ============================================================================

/// 後押しの指令を組む（純関数・§12.2 実測 9／§13 実測 10）。
///
/// 表示中の窓の owner を張り替えても、それだけでは重なりが動かない（§12.2 実測 6）。
/// 収めるには Z を伴う指令を 1 回だけ出す必要がある。その形を
/// **鎖の根（`members` の末尾）を、その 1 つ手前の窓（`members[len-2]`・以下「錨」）の
/// 直後へ差し直す**に固定する。
///
/// # なぜ動かすのは根なのか（§13 実測 10）
///
/// Windows が鎖全体を並べ直すのは、**所有する窓が動いたとき**である——所有される窓は
/// 所有者より手前という不変条件を保つため、所有側を動かすと被所有側が引き連れられ、
/// それが鎖の奥から手前へ再帰的に伝わる。鎖の中で「所有していて、かつ誰にも所有されて
/// いない」のは根だけであり、逆に**先頭は誰も所有していない**。
///
/// 初版が採っていた「先頭を 2 番目の直後へ」は、先頭が何も所有していないため
/// **その 1 枚が動くだけ**で鎖は並び直らない。しかも指令の意味は「先頭を 2 番目の**奥**へ」
/// であり、望む並びとは逆である。24 通りの始点すべてで実測した結果は
/// **0/24**（＝1 度も収まらない）であった。収まって見えていたのは、スレッド既定の
/// 不可視 IME 窓がたまたま先頭に所有されていた配置に限る（§13.1）。
///
/// # 挿入位置は 2 択（現況を 1 度だけ読む）
///
/// `SetWindowPos` は「要求した位置が現在位置と同じ」なら重なりを 1 ミリも動かさない。
/// よって錨の直後が既に根であるとき、素直な形は**完全な空振り**になる。そこで
/// **その 1 度きりの判定にだけ**、錨の生の 1 つ奥（`anchor_next`）を使う:
///
/// - `anchor_next != Some(root)` → 挿入位置は**錨**（望む関係そのものを主張する形）
/// - `anchor_next == Some(root)` → 挿入位置は**先頭**（`members[0]`）
///
/// 2 つの要求が同時に空振りすることはない——根の生の 1 つ手前は高々 1 枚であり、
/// 鎖が 3 枚以上なら錨と先頭は別の窓だからである。よって
/// **どちらか一方は必ず本物の位置変更になる**。これは「Windows が隣接時に省略する」という
/// 未文書の性質に賭ける形ではなく、「要求位置が現在位置と違えば Z は動く」という
/// `SetWindowPos` の基本の性質だけに乗る形である。
///
/// 窓が 2 枚のときは錨と先頭が同じ窓になり、2 択は 1 つに畳まれる。そのとき空振りする
/// のは「根が先頭の直後に居る」＝**既に望む並びである**場合だけなので、収めるものが無い。
///
/// # 現況を読むこと自体は要件 14.2 に触れない
///
/// 禁じられているのは**周期的な観測と是正**である。ここで読むのは、書く直前に 1 度だけ、
/// 出す指令の形を選ぶためであり、既存の撤去経路（`detach_one` が外す前に
/// `GWLP_HWNDPARENT` を読む）と同じ規律である。読んだ値を控えもしないし、次の巡へ
/// 持ち越しもしない。
///
/// # 他の形を選んではならない
///
...
pub(crate) fn nudge_command(
    ...
    let anchor_at = members.len().checked_sub(2)?;
    let root = *members.last()?;
    let anchor = members[anchor_at];
    // 錨の直後が既に根なら、同じ要求は Z を 1 ミリも動かさない。先頭を挿入位置に採る
    // （鎖が 3 枚以上なら錨と先頭は別の窓なので、こちらは必ず本物の位置変更になる）。
    let insert_after = if anchor_next == Some(root) {
        members[0]
    } else {
        anchor
    };
    ...
    let pos = WindowPos {
        zorder: ZOrder::InsertAfter(insert_after),
        position: None,
        size: None,
        no_activate: true,
        ..WindowPos::new()
