...
//!
//! 本フィーチャーが共有ファイルへ足した継ぎ目（`window_proc` の非活性化の枝、`api.rs` の
//! 走査ラッパ、areka の `main.rs` の結線 1 行）は、いずれも重なり以外の語を 1 語も持ち込んで
//! いない。ただしそれらのファイルは他フィーチャーの持ち物でもあるため、全文走査の対象には
//! 入れない——他所の追記で赤くなると、本フィーチャーへの誤った告発になる。
//!
//! # `\v` を語として走査しない理由
//!
