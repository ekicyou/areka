...
//! 項目 2・付録 A′ #8・要件 3.3／6.2／6.3／6.4）。
//!
//! # 何を突き合わせるのか
//!
//! 本ファイルは実 `apply_show` が画面へ載せた**表示バイトと当たり判定マスク**を、同一入力を
//! テスト側で独立に辿った**便宜経路**——`Composer::compose`（**原寸**）＋
//! `AlphaMask::from_pbgra32`——と 1 バイト単位で突き合わせる
//! （`test_support::native_golden_with` がその合成そのものである）。
//!
//! # 期待値は拡大率に依らない（裁定 D・要件 5.1）
//!
//! presenter が抱えるのは原寸バイトそのもので、拡大は wintf の描画経路（`Arrangement.scale`
//! ＝D2D 変換行列）が掛ける。ゆえに期待値は窓 DPI がいくつでも**同一の 1 組**であり、CPU で
//! 画素を拡大する段は経路のどこにも無い。窓 DPI を 3 通り（96／192／120＝k 恒等・整数倍・端数）
//! 踏むのは、**k が表示バイトにもマスクにも 1 ビットも影響しない**ことを固定するためである。
//!
//! # 2 つの経路の唯一の差＝**出力先が使い回しかどうか**（ここが本檻の検出力の源）
//!
//! 便宜経路は毎回まっさらな `ComposedSurface` へ合成する。本番経路は
//! **キャッシュ追い出しエントリから回収したバッファ**へ書く——そこには**直前のコマの絵が
//! 丸ごと残っている**（`ComposeCache::take_recycled` はバイト列を保ったままエントリを渡し、
//! `FrameBudget::display_buffer` がそれをそのまま返し、交代で合成先席に入る）。ゆえに
//! 「直前の内容を持つ器へ合成しても結果が変わらない」（`compose_into` の全面上書き）が崩れた
//! 瞬間、両者は食い違う。
//!
...
//! そこで止まる。
//!
//! # 本檻が捕まえられない残留の形（実測で判明した限界）
//!
//! 「出力の一部の領域を**毎回同じように**書き落とす」変異は、外形が変わらない定常状態では
//! **本檻を素通りする**。書き落とされた領域は誰にも書かれないまま 0 であり続けるため、
//! 使い回しの器の側もまっさらな器の側も同じ 0 になるからである。合成そのものの網羅は
//! `areka-emo-compose` の合成 golden が持つ。本檻が固有に捕まえるのは **presenter 層の配線**
//! ——回収したバッファを合成先席へ渡し損ねる・交代を落とす・マスクの出所を取り違える——である。
//!
//! # 空の実装で偽の合格を出さない
//!
...
use super::test_support::{
    elem, make_world_with_gpu, native_golden_with, shell_of, show_ok, spawn_window_with_dpi,
    surface,
};

// ── fixture ────────────────────────────────────────────────────────────────────────────

/// native 外形（表示に載るのはこの外形そのものである・k は掛からない）。
...
///
/// 容量 3 では 2 面が**両方とも表に収まり、3 適用目以降はすべてヒットする**——「使い回し
/// バッファの上を走る本番経路」という本檻の検出力の源（直前の別の面の絵が残っている器へ書く）が
/// 丸ごと消える。巡回長を容量より 1 大きくすると、入ってくる面は必ず 1 手前で追い出されており
/// 毎適用がミスになる。
const ROTATING_FACES: usize = CACHE_CAPACITY + 1;

/// 暖機の適用回数（回る実体が立ち上がりきる回数）。
...
/// 窓 DPI（author_dpi は 96 固定ゆえ k は恒等・2/1・5/4 になる）。**期待値はこの 3 通りで同一**
/// でなければならない——それが「拡大は描画側の係数だけ」（裁定 D）の観測形である。
const WINDOW_DPIS: [u16; 3] = [96, 192, 120];
...
///   （＝表示バッファの外形が面をまたいで動かず、回収がそのまま成立する）。
...
fn expected_of(emo_world: &EmoWorld, atlas: &AtlasTable, id: u32) -> Expected {
    let (bytes, extent) = native_golden_with(
        emo_world,
        atlas,
        id,
        &BindSet::default(),
    ...
    let mask = AlphaMask::from_pbgra32(&bytes, extent.0, extent.1, extent.0 * 4);
    Expected {
        bytes,
        extent,
        mask,
...
/// 巡回する 4 面ぶんの期待値を、presenter が握るのと**別の実体**で作る。
fn expected_faces() -> Vec<Expected> {
    let (probe_world, probe_atlas) = build_alpha_varying_faces(NATIVE_W, NATIVE_H);
    FACES
        .iter()
        .map(|id| expected_of(&probe_world, &probe_atlas, *id))
        .collect()
}
...
/// まっさらな出力先を使う便宜経路と 1 バイトも違わない。**窓 DPI 3 水準で同じ 1 組の期待値**に
/// 一致する（k は表示バイトにもマスクにも現れない・要件 5.1）。
///
/// 4 面を巡回適用する。合成キャッシュは容量 3（要件 7.1）ゆえ**巡回長 4 > 容量 3** で毎回ミスし、
/// 最も古い引き当てのエントリが追い出されてその表示バッファが回収され、交代で合成先席に入る
/// ——**いま合成している器には別の面の絵が入っている**。便宜経路との差はこの一点だけである。
///
...
fn the_budget_path_produces_the_same_display_bytes_and_mask_as_a_fresh_buffer_path() {
    // 便宜経路は presenter が握るのと別の実体で辿る（内部値の追認にしない）。**窓 DPI に依らず
    // 1 組で足りる**ことが裁定 D の観測形そのものである。
    let want = expected_faces();

    // fixture 前提: 各面が別の絵・別のマスクであること（同じなら残留を弁別できない）。
    // 期待値が窓 DPI に依らない 1 組なので、この前提も 1 度だけ確かめれば足りる。
    for (i, id) in FACES.iter().enumerate() {
        assert_eq!(
            want[i].extent, want[0].extent,
            "面の外形が揃っていない（表示バッファの回収経路を踏めない）"
        );
        assert_expected_is_not_empty(&want[i], &format!("面 {id}"));
        for j in 0..i {
            assert_ne!(
                want[i].bytes, want[j].bytes,
                "面 {id} と {} の表示バイトが同一＝残留が混ざっても気付けない",
                FACES[j]
            );
        }
    }
    ...
    assert!(
        distinct_masks > 1,
        "巡回する面のマスクが全て同一＝マスクへの残留が混ざっても気付けない"
    );

    for window_dpi in WINDOW_DPIS {
        let at_dpi = format!("窓 DPI {window_dpi}");
        ...
        for i in 0..WARMUP + STEADY {
            ...
            let at = format!("{at_dpi} の {i} 回目（面 {id}）");
        ...
        // （合否はあくまで上のバイト比較である）。
        //
        // 回る器は **k に依らず** キャッシュの 3 本＋合成先席の 1 本＝4 本である（交代は拡大率で
        // 分岐しない・要件 1.3／7.3・`show.rs` の `swap_native_scratch` は無条件）。
        let circulating = CACHE_CAPACITY + 1;
        let mut distinct = steady_display_ptrs.clone();
        distinct.sort_unstable();
        distinct.dedup();
        assert!(
            distinct.len() <= circulating,
            "{at_dpi}: 定常状態の表示バッファが {} 種類の器に散っている（回収が成立しておらず、\
             使い回しの上での等価を観測できていない）",
            distinct.len()
        );
        assert!(
            steady_display_ptrs.len() > distinct.len(),
            "{at_dpi}: 定常状態で同じ器が一度も再登場しない（別の面の絵が残った器へ書く経路を\
             踏めていない）"
        );

        // ⑺ ミス経路であることの陽性対照: 定常の各適用でマスクの実体が動く。
        //
        // ヒットなら合成もマスク再生成も走らず、スロットのマスクは**同じ実体のまま**である。
        // 巡回長がキャッシュ容量以下へ縮むと本檻は静かにヒット経路の檻へ化けるので、ここで
        // 止める（本 spec は「ミスしか踏まない／ヒットしか踏まない」取り違えを 2 度踏んだ）。
        for w in steady_mask_ptrs.windows(2) {
            assert_ne!(
                w[0], w[1],
                "{at_dpi}: 定常の連続する適用でマスクの実体が動かない＝引き当てがヒットしている\
...
///
/// 上の檻は設計上すべての適用がミスする（面を毎回変える）。`show.rs` 手順 (3)——表示記録の反映
/// ＋マスク供給——は合成の有無に依らず走るため、**ヒット経路にも乗っている**。ミスしか踏まない
/// 檻だけでは、そこが 1 度も動かないまま「毎コマ経路は全て覆った」と言えてしまう。
///
...
fn repeating_the_same_surface_keeps_the_display_bytes_and_mask_equivalent() {
    for window_dpi in [192_u16, 120] {
        ...
        let (emo_world, atlas) = build_alpha_varying_faces(NATIVE_W, NATIVE_H);
        let (probe_world, probe_atlas) = build_alpha_varying_faces(NATIVE_W, NATIVE_H);
        let want = expected_of(&probe_world, &probe_atlas, FACE_A);
        assert_expected_is_not_empty(&want, &format!("{at_dpi} 面 {FACE_A}"));
        ...
        let missed_mask = {
            ...
            let entry = target
                .cache
                .get(FACE_A, &BindSet::default(), &PatternState::default())
                .expect("直前の適用がスロットを埋めている");
            std::sync::Arc::as_ptr(&entry.mask)
        };
        assert_ne!(
            missed_mask, mask_before_miss,
            "{at_dpi}: 1 回目の適用がミスになっていない（ヒットとの対照が空虚になる）"
        );
        ...
        assert_eq!(
            std::sync::Arc::as_ptr(&entry.mask),
            missed_mask,
            "{at_dpi}: マスクの実体が動いた＝引き当てがミスしている（ヒット経路を観測できていない）"
        );
        assert_eq!(
            (entry.composed.width(), entry.composed.height()),
            want.extent,
            "{at_dpi}: ヒット経路の表示外形が便宜経路と違う"
        );
        assert_eq!(
            entry.composed.bytes(),
            want.bytes.as_slice(),
            "{at_dpi}: ヒット経路の表示バイトが便宜経路と違う"
        );
        assert_eq!(
            *entry.mask, want.mask,
            "{at_dpi}: ヒット経路のマスクが便宜経路と違う"
        );
        assert_eq!(
            presenter.read_back(TargetId(0)).expect("read_back 失敗"),
            want.bytes,
            "{at_dpi}: ヒット経路の読み戻しが便宜経路と違う"
        );
    }
}

/// fixture 自身の健全性: 巡回する 4 面が **同じ原寸外形・互いに別のバイト**であり、マスクが
/// 一様でなく、**面の本数がキャッシュ容量より多い**こと。
///
...
/// 事故をここで止めるためである。
///
/// 拡大率で場合分けしないのは、期待値が原寸だけで決まるからである（k は fixture に現れない）。
#[test]
fn the_rotating_face_fixture_differs_in_bytes_and_outnumbers_the_cache() {
    ...
    for (i, id) in FACES.iter().enumerate() {
        assert_eq!(
            want[i].extent,
            (NATIVE_W, NATIVE_H),
            "α≠0 ゆえトリムは全域を残し、外形は原寸と厳密一致するはず"
        );
        assert_expected_is_not_empty(&want[i], &format!("面 {id}"));
        for j in 0..i {
            assert_ne!(
