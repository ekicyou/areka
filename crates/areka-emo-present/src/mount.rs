...
mod tests {
    ...
    use wintf::ecs::{HitTestMode, VisualGraphics};
    ...
    /// attach 時・`set_layout` 時とも surface entity の `Arrangement` が**論理寸＝原寸・`scale`＝k・
    /// 原点 0**（[`logical_arrangement`]）へ直接設定されること（`BoxStyle`/taffy 非経由）。
    #[test]
    fn arrangement_is_logical_native_with_scale_k() {
        let (mut world, _window, mount) = attach_fixture(3, 2);
        ...
        assert_eq!((arr.scale.x, arr.scale.y), (1.0, 1.0), "装着時の k は 1");
        assert_eq!(
