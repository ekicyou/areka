...
///
/// 改訂第 2 版ではこの並びが**そのまま鎖の連結順になる**（要件 15・DD-11＝先に登記された
/// グループほど手前・基底が最前）。初版の註「台帳の並びは前後関係へ何も含意しない」は
/// 誤りになったので撤回した。要件 3.6 が言う非強制は、いまも**指定が一つも無い既定状態**
/// についての規定であって、グループどうしが並んだ後の話ではない。
#[test]
