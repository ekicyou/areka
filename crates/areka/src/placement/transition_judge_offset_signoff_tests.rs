...
//!
//! ```text
//! $AREKA   = "<絶対パス>\areka.exe"
//! $GHOST   = "<絶対パス>\ghost"
//! $BALLOON = "<絶対パス>\balloon"
...
//!
//! # 3. 手順（必須の 6 手・順序も必須・1 手でも欠くと判定が立たない）
//!
...
//! 3. **キーワード指定のバルーンを、素材未消費のまま遷移へ最低 1 度通す**（**必須**・手 4 より
//!    先に行う）。起動してキーワード指定のバルーンを出したら、素材を消費させる**前に**
//!    1 度だけ拡大率をまたがせる——門の行（`verdict=keyword-pending`）を**遷移の内側に
//!    1 行**残すためである。
//!    *理由*: 判定 ⑷ が母数に採るキーワード指定スコープの集合は、**遷移の内側に現れた
//!    `keyword-pending` の行からしか作られない**（`check_alignment_residual`）。素材を先に
//!    消費させると門は 2 度と出ず、集合が空のまま測れた行が 0 件になって「揃えを 1 度も
//!    測れていない」の**偽の赤**が出る——正しい実装のままで、である。手 4 の消費だけを
//!    行うと必ずこの形になるので、順序そのものが手順の一部である。
//! 4. **キーワード指定のバルーンについて、素材を消費させてから低い拡大率側へ遷移させる**
...
//! 5. **ゴーストをモニタ間で往復させる**（125% 側 → 200% 側 → 125% 側 …）。同じ拡大率へ
...
//! 6. **先行仕様の残所見を目で確かめる**（要件 8.4 の目視側）: 低い拡大率の側で、バルーンが
...
//! 5. **最初の遷移起点より前に出た記録は 1 件も判定されない。** 判定器が起点と認めるのは
//!    `kind=monitor` かつ `old_dpi` と `new_dpi` が異なる行だけであり（`transition_judge.rs`
//!    の `is_transition_origin`）、最初の起点より前の行は**どの遷移にも属さないので捨てる**
//!    （同 `split_transitions`）。一方で起動直後の 1 巡目は全窓へマッチする
//!    （`emo2_boot/frame/dpi.rs` の `dpi_phase_with`）ため、**起動時にまとめて出る追随の行は
//!    必ず最初の起点より前に落ちる**。起動直後の値の正しさは判定器の視野の外であり、
//!    目で確かめる。
