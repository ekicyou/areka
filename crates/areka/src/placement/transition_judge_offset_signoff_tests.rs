...
//! キーワード指定スコープ（手 3・手 4）の**両方**を要求する。片側しか持たない検体では手順を
//! 最後まで回せない——
//!
...
//! 判定は成立し、表示設定を触る必要は無い**。
//!
//! **拡大率をまたがせる操作は、手 3・手 4・手 5 のいずれも手 5 の作法で行う**——人の手で
...
//! 5. **ゴーストを、人の手でドラッグしてモニタ間を往復させる**（125% 側 → 200% 側 → 125% 側 …）。
...
//! 5. **最初の遷移起点より前に出た記録は 1 件も判定されない。** 最初の起点より前の行は
//!    **どの遷移にも属さないので捨てる**（`transition_judge.rs` の `split_transitions`）。
...
//!    **残るのは起動直後の 1 巡目である**——1 巡目は全窓へマッチして追随の行を出す
//!    （`emo2_boot/frame/dpi.rs` の `dpi_phase_with`）が、その時点でまだ起点が 1 行も出て
//!    いなければ、それらの行はどの遷移にも属さず捨てられる。起動直後の値の正しさは判定器の
//!    視野の外であり、目で確かめる。
