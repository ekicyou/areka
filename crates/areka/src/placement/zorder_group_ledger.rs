...
impl ZOrderGroupLedger {
    ...
    /// # 本番の消費点が無い（意図した状態）
    ///
    /// design は変化の検出を「台帳の版か `GhostWindows` の構成」と書いているが、指令消化の相
    /// （`zorder_drain.rs`）は**組み上げた鎖の計画そのものの突き合わせ**を選んだ。
    /// 結果の比較は版の判定を厳密に包含し、さらに「計画の変わらない版の上昇で印が立ち、
    /// 適用系が空振りする」要件 6.1 の穴も塞ぐ。ゆえにこの読み口は本番から呼ばれない。
    ///
