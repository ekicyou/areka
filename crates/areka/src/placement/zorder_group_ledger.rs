...
impl ZOrderGroupLedger {
    ...
    /// # 本番の消費点が無い（意図した状態）
    ///
    /// design は変化の検出を「台帳の版か `GhostWindows` の構成」と書いているが、射影の相
    /// （`zorder_drain.rs`）は**射影結果そのものの突き合わせ**を選んだ（task 3.3 の裁量）。
    /// 結果の比較は版の判定を厳密に包含し、さらに「射影の変わらない版の上昇で印が立ち、
    /// 維持系が空振りする」要件 6.1 の穴も塞ぐ。ゆえにこの読み口は本番から呼ばれない。
    ///
    /// 版そのものは design が定めた台帳の一部であり、状態遷移の檻
    /// （`zorder_group_ledger_state_tests.rs`）が「いつ進み、いつ進まないか」を全数で
    /// 押さえている。読み口を消すとその不変条件を主張する手段が無くなるので、
    /// **この 1 項目だけ**に許可を与える（モジュール全体の許可は結線の着地とともに撤去済み）。
    #[allow(dead_code)]
