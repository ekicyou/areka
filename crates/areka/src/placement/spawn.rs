...
pub fn spawn_ghost_windows(
    ...
    for p in placements {
        ...
        // や shell 設定で明示した列は、グループ機構（areka-P0-scope-zorder-pinning）が
        // **別系統**で宣言する（areka 側の台帳 → 鎖の合成 → wintf の `ZOrderChainPlan` →
        // 鎖の適用系）。あちらが張るのは**スコープをまたぐ**繋ぎだけであり、本宣言
        // （`KeepDirectlyAbove`）が張るスコープ内ペアには 1 本も触れない。両者は語彙も
        // 書込先も交わらず、それを消費するペア機構の 5 ファイルも一切変わっていない
        // （要件 9.5 の凍結）。
        // 変わったのは「作者が明示した列だけは、別の宣言としてスコープ間にも存在しうる」
        // という一点であり、明示が無い間はグループが 1 本も無い＝従来どおりである。
        world
...
/// （[`ReassertZOrder`](wintf::ecs::ReassertZOrder)）を挿し、それをペア維持系
