...
/// 一周の全 10 段（design D1「段・注入・完了条件」の表と同順・同名）。
///
/// 区間はすべて 1 段 10 秒幅で、段の境目で不連続にしてある（前段の上限＝次段の下限−1 秒）。
/// 幅は「観測が成立するまで注入と観測を交互に回す」余地であって、注入本数の宣言ではない。
pub(super) const LAP_STAGES: &[LapStage] = &[
    // 起動と装着は毎秒の変化通知を投函しない（起動系列は組立が、装着は frame 相の直接駆動が起こす）。
    LapStage {
        name: "起動",
        begin_ms: 0,
        limit_ms: 0,
...
/// `crates/areka-kanade/src/schedule/events.rs:173`）。80,000 ms は 1 時間の 45 分の 1 に満たない。
pub(super) const LAP_INJECT_LIMIT_MS: u64 = 80_000;
