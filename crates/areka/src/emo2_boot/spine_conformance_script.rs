...
//
// # 走行 A（2026-09-05）の症状 #1 はこの層では観測できない
//
// 実機の症状「相方側バルーンでメニューの先頭の選択肢が描かれない」は、行送り
// `ceil(font.height × 1.25)`＝35px と相方側 validrect の高さ 93px の関係で 3 行が収まらない
// という**字の配置**の欠陥である（`crates/areka-emo-text/src/layout.rs:532-546` の
// `visible_window` が 1 行スクロールし、`draw.rs:766-774` が先頭行を skip する）。
// 決定論一周が突き合わせる 3 つの列は交信・面の表示指令・進行状態だけを台帳にするので、
// 字がどこへ置かれたかは 1 ビットも運ばない。ゆえに本改訂は**台本を実物へ忠実に戻すこと**
// であって、症状 #1 の檻ではない。その檻は引受先の別仕様（emo-text の行送り正典化）が持つ。
