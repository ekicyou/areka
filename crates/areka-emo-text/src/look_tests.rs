...
//!
//! **「定義が無い側」も独立に断言する**——差し込みの列が空のとき（バルーン定義に宣言が無いとき）
//! 太字・斜体・下線・打ち消し線・白抜き・上下付き ほかは「零のまま」であることが
//! 要件（4.3 の口に何も渡されなければ値は既定）なので、零を明示的に述べる（§3）。
...
/// 差し込みの列が空なら（バルーン定義に宣言が無ければ）飾りの項目は、両層とも正典の既定（＝零）のままである
