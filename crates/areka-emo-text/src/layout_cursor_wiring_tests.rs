...
/// `0/0/400/224`・折返し閾値 400。font 10（全角 'あ' の advance 10・pitch 12＝10 + 行間 2）。
...
///
/// 手計算（`start = (40, 20)`・font 10・pitch 12。式は design.md 解決表
/// `位置 = 基点 + 値 × 係数`）:
/// - `\_l[0,0]` → `(40 + 0, 20 + 0) = (40, 20)`
/// - `\_l[100,50]` → `(40 + 100, 20 + 50) = (140, 70)`（正典「数値＝ピクセル単位座標」）
/// - `\_l[2em,3lh]` → `(40 + 2×10, 20 + 3×12) = (60, 56)`（正典「1em＝文字高さ」「1lh＝1em＋行間」）
///
...
/// `TextRegion::start()`（範囲内の宣言は宣言どおり・範囲外の宣言と未宣言は角へ縮退）から
/// 来ていることが確かめられる。
///
...
/// これに乗りうる（Requirement 6.3。`\c[line]` 自体は開発者裁定 2026-09-05 で実装しない）。
///
/// 手計算（前提 A・font 10）:
