...
// advance 10・pitch 13）。origin(0,0)・wordwrappoint 未宣言ゆえ文字描画開始点は (0, 0)・
// 折返し閾値＝画像右辺 400。
...
///
/// 手計算（font 10・pitch 13・文字描画開始点 (0, 0)）:
...
/// - `あ`: フラッシュで改行が効き、行送り 0 + 13 = 13・行内は行頭 0 へ戻る。
///   ゆえに 2 個目は**次行の先頭 (0, 13)**。
///
/// 書き換え前の現行値は **(20, 13)**——改行の到着時にカーソルを保留したままにしていたので、
...
/// 仮適用した (0, 13) なので `@10` は 10 になり、着地は **(10, 13)**。順方向だけを固定すると
...
fn written_order_decides_absolute_cursor_against_newline() {
    ...
    assert_eq!(
        lines[0].rect.top, 113.0,
        "正典値（4.3 の書かれた順による・DD-11）: 先に効いた `\\_l` の 100 へ、後に書かれた\
         `\\n` の送り 13 が乗る。書き換え前の現行値は 100（カーソルが改行送りを上書きしていた）"
    );
    assert_eq!(inline_positions(&lines[0]), vec![0.0]);
    ...
    assert_eq!(
        ...
        "逆順は不変: 改行送り 13 にカーソルの絶対 Y=100 が後勝ちする"
    );
}
...
///    (1) 現在行 [あ\0] を確定 →(2) Σ=1 を消費して行送り 0 + 13 = 13・行内は 0 へ →
///    (3) カーソルの Y=100 がそれに後勝ちして行送り 100。そのうえで Σ=1 を積み直す。
/// 5. `あ`: フラッシュで (2) が効き、行送り 100 + 13 = **113**。
///
/// **(2) を走らせないと 126.0 になる**——`\_l` より前に書かれた Σ=1 が (3) を追い越して
/// 保留に残り、4 段目で積み直した改行と合流して Σ=2 になる（100 + 2×13 = 126）。
/// 126 は書かれた順（113）にも旧正典（100）にも一致しない値で、前に書かれた改行が
/// カーソルの**後**に効いてしまっている。
///
/// **4.3 前は 100.0 だった**（先行実体化そのものが無く、フラッシュで (2) 改行送り 26 に
...
fn written_order_applies_newlines_before_and_after_the_cursor() {
    ...
    assert_eq!(
        lines[1].rect.top, 113.0,
        "正典値（4.3 の書かれた順による・DD-11）: 前に書かれた `\\n` が (2) で先に効き、         `\\_l` の 100 が (3) で後勝ちし、後に書かれた `\\n` の送り 13 が最後に乗る。         (2) を走らせないと Σ が漏れて 126.0・4.3 前は 100.0 だった"
    );
    assert_eq!(
        inline_positions(&lines[1]),
        vec![0.0],
