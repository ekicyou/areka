...
impl Spike {
    fn new() -> Spike {
        ...
        let dwrite = core
            .dwrite_factory()
            .expect("GraphicsCore::dwrite_factory")
            .clone();
    ...
    fn target_bitmap(&self, tex: &ID3D11Texture2D) -> ID2D1Bitmap1 {
        ...
        unsafe {
            self.dc
                .CreateBitmapFromDxgiSurface(&dxgi_surface, Some(&props as *const _))
        }
...
fn compare_region(a: &[u8], b: &[u8], rx0: u32, ry0: u32, rx1: u32, ry1: u32) -> (usize, u32) {
    ...
    for y in ry0..ry1 {
        for x in rx0..rx1 {
            ...
            if a[idx..idx + 4] != b[idx..idx + 4] {
                ...
                let d = (x - rx0).min(y - ry0).min(rx1 - 1 - x).min(ry1 - 1 - y);
...
fn run_mode(mode: WritingMode, label: &str) {
    ...
    let layouts: Vec<IDWriteTextLayout> = LINES
        .iter()
        .map(|t| spike.line_layout(t, &font, mode))
        .collect();
    ...
    println!(
        "  保持域インク数(B / blit)   : {core_ink_b} / {core_ink_blit}  (中心核 shrink={CORE_SHRINK})"
    );
