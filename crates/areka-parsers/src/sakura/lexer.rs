...
//! - 正準タグ（`\` ＋ word ＋ `[args]`）— タグ名は `bare_tag_len` の固定長規律で終端し、
//!   その確定位置に `[` があれば角括弧経路へ入る（`]` が引数終端・要件 13.2）。
//!   確定位置より後ろの `[` はタグ名を伸ばさない（`\iテキスト[x]` の `[` は本文）。
//! - bare タグ（`\e` `\c` `\-` `\n`・`\_a` `\__q` 等）— 角括弧を伴わないタグの綴り
//!   （1〜3 文字・要件 13.1）。タグ名の長さは綴りの一覧によらず `bare_tag_len` の規則で
//!   決まり、角括弧の有無はその確定位置で判定する（例外は `scan_tag` 参照）。
//! - `\wN`／`\bN`／`\pN` 短縮（N は 1 桁）— 角括弧を伴わない短縮形
//!   （`\wN` の待ちは要件 3.2 の構文側）。
...
pub(crate) enum Token {
    /// 正準形 `\word[args]`（`word` は確定済みのタグ名で直後が `[`、args は `]` までをカンマ分割）。
    ///
    /// `word` は本文を含まない 1〜3 文字（`\_X`／`\__X` 形を含む）。例外で 2 文字になる
    /// のは短縮対象語＋1 桁（`w2` 等）と旧仕様の選択肢 `q*` の 2 形。
    Tag { word: String, args: Vec<String> },
    /// 角括弧を伴わないタグの綴り（先頭の `\` を除く）。
    ///
    /// 既知 1 文字タグ `e` `c` `-` `n` `0` `1` `h` `u` のほか、未知の 1 文字語や
    /// `\_` 始まりの 2〜3 文字形（`_a` `_q` `__c` `__q` 等）も同じ載荷に載る。
    /// 入力末尾の裸 `\` は綴り `"\\"` として載せる。
    Bare(String),
...
///
/// 形態:
/// - 確定したタグ名の直後が `[` → `Token::Tag`（`]` まで引数）。
/// - `\<word>N`（短縮対象語＋1 桁・`\wN`/`\bN`/`\pN`）→ `Token::Shorthand`。
/// - `\X`／`\_X`／`\__X`（角括弧なし・綴り 1〜3 文字）→ `Token::Bare`。
fn scan_tag(chars: &[(usize, char)], i: usize) -> (Token, usize) {
    ...
    // ワード（コマンド語）を読む: 最大 3 文字で頭打ちにし（`[`／`\`／`%` でも止まる）、
    // 長さは `bare_tag_len` の規則で確定してから `[` を判定する。例外は短縮対象語＋1 桁と
    // 旧仕様の選択肢 `q*[` の 2 形で、読み取り結果をそのまま使う。
    // 短縮対象語（`\w`/`\b`/`\p`）は 1 文字読んだ時点で次が数字なら短縮形を優先する。
    let word_start = i + 1;
    ...
    // それ以外: ワードを `[`／`\`／`%` の手前まで、ただし 3 文字で頭打ちにして読み進める
    // （`bare_tag_len` の入力として十分。この時点の読み取り結果は本文を含み得る）。
    ...
    if let Some(&(_, '[')) = chars.get(j) {
        ...
        // 角括弧なし。`word` は上で長さを確定済みの綴りそのもの（本文を含まない）なので
        // そのまま返す（`bare_tag_len` を再び呼ばない）。残りはテキスト／後続走査へ委ねる。
        (Token::Bare(word), j)
    }
}
...
///
/// `word` は `\` の直後から `[`／`\`／`%` の手前まで、最大 3 文字で頭打ちした走査結果
/// （空でない）。`scan_tag` はこの返り値でタグ名を確定してから `[` を判定する。
///
/// **ワード走査の結果の長さを消費長に使ってはならない**（要件 2.4）。頭打ち前の走査は
/// 本文でも空白でも止まらず、長さをそのまま消費すると台詞を飲み込んだ（旧欠陥:
/// `\_aをクリックする。` の走査結果が `_aをクリックする。` になった）。頭打ち後も
/// 走査結果は本文を含み得る（`\_aを`）。判断は綴りの先頭 1〜2 文字だけを見て決め、
/// 走査済みの実長は**頭打ちにのみ**使う。
///
/// 決定表（`n` ＝ `word` の文字数）:
///
