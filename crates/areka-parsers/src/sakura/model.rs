...
pub enum Instruction {
    ...
    /// 文字装飾 `\f[key,args...]`（転記のみ・text-decoration-canon 要件 2.1/2.2）。
    ///
    /// `args` は角括弧の中を `,` で割った列そのもの（`args[0]` がキー・以降が値の列）。
    /// 意味は一切読まず、記述順のまま保持し、空のトークンも潰さない。引数の形は 4 通り:
    /// `\f[]` と裸の `\f` はどちらも `[]`、`\f[""]` だけが `[""]`、`\f[bold,]` は
