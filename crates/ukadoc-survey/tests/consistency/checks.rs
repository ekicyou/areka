...
///
/// **このテストは「緑だから守られている」を否定するために置く。** 6.5・6.6・6.7 の 2 面
/// （関連・別名・後継の相手／状態が alias の行）・6.8・6.11 は対象が 0 件で、判定が
/// 丸ごと消えても実データは緑のままである。だから 0 件であること自体を主張に変え、
/// 対象が 1 件でも生まれたら赤にする——そのときは「空振り」の但し書きを外し、
/// 非空の主張へ書き換えること。
///
/// 対象があると分かっている側（6.3・6.4・6.7 の 2 面（カタログに版番号のある項目／
/// 登場版の記入がある行）・6.10・7.4/7.5）は 0 件になったら赤にする。
