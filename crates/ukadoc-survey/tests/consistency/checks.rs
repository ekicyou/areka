...
//!    ——要件ごとに今日の対象を数え、対象の有無を**名指しで**固定する。0 件のはずの
//!    行に対象が生まれたら赤になり、書き換えを促す。
...
//!
//! ⑶ が要る理由は ⑵ が明かしていた——かつての実データでは 6.5・6.6・6.7・6.8・6.11 の
//! 対象が **0 件**だった（正典 URL がソースのどこにも置かれておらず、台帳が全行未分類
//! だったから）。台帳の記入が進んで対象は生まれたが、対象があることと判定が働いている
//! ことは別である。写しを壊す ⑶ だけが、その判定が**実データの上で**生きていることを
//! 示す。
//!
...
enum Subjects {
    ...
    ///
    /// いまは 12 行すべてが [`Subjects::NonEmpty`] なので誰も作らないが、下の照合と
    /// ともに残す——母数が 0 へ戻った要件が出たときに、また名指しで固定するためである。
    #[allow(dead_code)]
    Zero,
}
...
/// 10 の要件それぞれについて、今日の実データに対象があることを名指しで固定する
/// （要件 6.13）。
///
/// **このテストは「緑だから守られている」を否定するために置く。** かつては 6.5・6.6・
/// 6.7 の 3 面・6.8・6.11 の対象が 0 件で、判定が丸ごと消えても実データは緑のまま
/// だった。台帳の記入が進んで対象が生まれたので、このテスト自身の求めるとおり、その
/// 7 行を `Subjects::NonEmpty` へ移してある（開発者裁定 2026-09-06）。
///
/// いまは 12 行すべてが「0 件になったら赤」を言う。読み込みが空を返した場合も、判定の
/// 母数が 0 へ戻った場合も、ここで捕まえる。
///
