...
//!
//! ⑶ が要る理由は ⑵ が明かす——今日の実データでは 6.7 の 1 面（状態が alias の行）と
//! 6.8 の対象が **0 件**である（4 台帳のどこにも別名の行が無く、テーマ名がまだ 1 つも
//! 書かれていないから）。対象 0 件の判定は緑でも「壊れていない」ことを 1 つも言わない。
//! 写しを壊す ⑶ だけが、その判定が**実データの上で**生きていることを示す。
//!
...
///
/// **このテストは「緑だから守られている」を否定するために置く。** 6.7 の 1 面
/// （状態が alias の行）・6.8 は対象が 0 件で、判定が
/// 丸ごと消えても実データは緑のままである。だから 0 件であること自体を主張に変え、
/// 対象が 1 件でも生まれたら赤にする——そのときは「空振り」の但し書きを外し、
/// 非空の主張へ書き換えること。
///
/// 対象があると分かっている側（6.3・6.4・6.5・6.6・6.7 の 3 面（関連・別名・後継の
/// 相手／カタログに版番号のある項目／登場版の記入がある行）・6.10・6.11・7.4/7.5）は
/// 0 件になったら赤にする。
