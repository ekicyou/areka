...
/// # なぜこの 1 本が要るか
///
/// [`crate::blit::execute`] が呼ぶ `resize_and_clear` はゼロ埋めを**除去してはならない**——
/// 合成は画布へ重ねる意味論で、どの命令にも覆われない領域は「書かれずに 0 のまま」であることに
/// 依存している。ところが実測では、`blit` 側へこの除去を誤って適用してもこのクレートの
/// 既存テストは**全て緑のまま**通り、別クレート（emo-present）の等価檻でしか赤にならなかった。
