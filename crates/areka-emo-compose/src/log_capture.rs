...
//! # 設計（本番非侵襲・スレッドローカル）
//!
//! - `crates/areka/src/shiori_demo.rs` の実証済みパターンに倣い、`fmt`＋`MakeWriter` ではなく
//!   最小の [`tracing_subscriber::Layer`] 実装（[`Capture`]）でイベントの **level／target／
//!   フィールド**を文字列化して共有 `Arc<Mutex<Vec<String>>>` へ push する。フィールド値
...
///
/// 出力形式（1 イベント 1 行）: `level=WARN target=areka_emo_compose surface_id=1000 …`。
/// `record_debug` で全フィールドを `name=value` として載せるため、テストは level（WARN/ERROR）と
/// discriminating field（surface_id/method/key/path）を substring で検証できる。
    ...
    fn on_event(&self, ev: &tracing::Event<'_>, _: tracing_subscriber::layer::Context<'_, S>) {
        ...
        impl Visit for V<'_> {
            fn record_debug(&mut self, f: &Field, v: &dyn std::fmt::Debug) {
                use std::fmt::Write;
                // 文字列フィールドは `key="value"` の Debug 表記になる（例 path="bogus.png"）。
