...
//!
//! 窓口は純 Rust なので、i686 の helper ターゲットでも組める。依存は `thiserror`・`areka-nar`・
//! `miniz_oxide` の 3 つで、`areka-nar` がさらに `areka-parsers` と `encoding_rs` を引く——
//! いずれも純 Rust で、C のビルドもプラットフォーム固有の道具も要らない。
