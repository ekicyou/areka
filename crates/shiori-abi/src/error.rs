...
//! - カスタム HRESULT 定数 [`SHIORI_S_PENDING`] / [`SHIORI_E_NOT_LOADED`] /
//!   [`SHIORI_E_UNKNOWN_TOKEN`]: 成功の 2 値分岐（即時 `S_OK` / 遅延 `SHIORI_S_PENDING`）と
//!   失敗カテゴリを COM 規約（HRESULT）で表現する（design.md §Error Strategy/§Error Categories,
//!   要件 2.3/2.4/3.5/3.6/7.1/7.2/7.3）。
...
/// ergonomic 層はこのコードを受けて `RequestOutcome::Deferred` を構築する（要件 3.3/3.5）。
pub const SHIORI_S_PENDING: HRESULT = make_shiori_success(0x0001);

/// 未ロード状態での request 拒否を示す失敗 HRESULT（要件 2.4）。値 `0xA0A1_0002`。
pub const SHIORI_E_NOT_LOADED: HRESULT = make_shiori_failure(0x0002);
...
pub enum ShioriError {
    /// 未ロード状態で request を受理しなかった（要件 2.4・[`SHIORI_E_NOT_LOADED`]）。
    #[error("brain not loaded")]
    NotLoaded,
    /// load 操作が失敗した（要件 2.3）。内包 HRESULT が失敗コードを保持する。
    #[error("load failed: {0}")]
    LoadFailed(HRESULT),
    /// request 操作が失敗した（要件 3.6）。内包 HRESULT が失敗コードを保持する。
...
/// - その他の失敗 HRESULT → 呼び出し文脈に応じ呼び出し側が [`ShioriError::LoadFailed`] /
///   [`ShioriError::RequestFailed`] を選ぶ。文脈が無い汎用変換としては
...
pub fn hresult_to_shiori_error(hr: HRESULT) -> ShioriError {
    ...
    match hr {
        SHIORI_E_NOT_LOADED => ShioriError::NotLoaded,
...
mod tests {
    ...
    #[test]
    fn shiori_e_not_loaded_is_failure_with_customer_bit() {
        assert!(SHIORI_E_NOT_LOADED.is_err(), "SHIORI_E_NOT_LOADED は失敗コードであること");
        assert_eq!(SHIORI_E_NOT_LOADED.0 & CUSTOMER_BIT, CUSTOMER_BIT);
        assert_eq!(
            SHIORI_E_NOT_LOADED.0 & SEVERITY_FAILURE,
            SEVERITY_FAILURE,
            "severity（失敗）ビットがセットされていること"
        );
        assert_eq!(SHIORI_E_NOT_LOADED.0, 0xA0A1_0002u32 as i32);
    }
    ...
    #[test]
    fn not_loaded_maps_to_not_loaded_variant() {
        let err = hresult_to_shiori_error(SHIORI_E_NOT_LOADED);
        assert!(
            matches!(err, ShioriError::NotLoaded),
            "SHIORI_E_NOT_LOADED は ShioriError::NotLoaded へマップされること, got {err:?}"
        );
    }

    #[test]
    fn unknown_token_maps_to_com_variant_preserving_hresult() {
        // 専用変種を持たない失敗 HRESULT は Com へ落ち、元の HRESULT を保持する。
        let err = hresult_to_shiori_error(SHIORI_E_UNKNOWN_TOKEN);
        match err {
            ShioriError::Com(e) => assert_eq!(e.code(), SHIORI_E_UNKNOWN_TOKEN),
    ...
    #[test]
    fn load_failed_wraps_hresult() {
        // 文脈依存変種（LoadFailed/RequestFailed）は HRESULT を内包し、保持できること。
        let hr = HRESULT(0x8000_4005u32 as i32);
        let err = ShioriError::LoadFailed(hr);
        match err {
            ShioriError::LoadFailed(got) => assert_eq!(got, hr),
            other => panic!("expected LoadFailed, got {other:?}"),
        }
    }

    #[test]
    fn request_failed_wraps_hresult() {
        let hr = HRESULT(0x8000_4005u32 as i32);
        let err = ShioriError::RequestFailed(hr);
        match err {
            ShioriError::RequestFailed(got) => assert_eq!(got, hr),
