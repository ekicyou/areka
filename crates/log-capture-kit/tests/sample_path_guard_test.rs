...
//!
//! 設計の較正は「除外領域に ⑴〜⑷ の実体があること」を求めているが、**段 ① の時点で
//! 実体があるのは ⑴ だけ**である。残る 3 形を「いずれ入るから」と黙って落とすと、
//! 除外が飾りになっても気付けない。そこで [`EXCLUSION_FORMS`] が 4 形それぞれに
//! 「今日あるか・無いならなぜか」を明示し、[`the_exclusion_declares_which_forms_it_holds`]
//! が**両方向**で判定する——あると宣言した形が消えたら赤、無いと宣言した形が現れたら赤
//! （宣言を書き換えさせる）。段 ③ で ⑶ の実体が入った瞬間に宣言の更新を強制されるので、
//! 表は自動的に締まる。
//!
//! # 走査語を逐語で書かない約束
//!
...
/// 4 形それぞれの、除外領域における実体の有無（2026-09-18 実測・段 ①）。
///
/// 設計の較正は「除外領域に ⑴〜⑷ の実体があること」を求めているが、段 ① の実測では
/// ⑴ しか無い。無い 3 形を黙って落とすと較正が恒真になるので、理由つきで宣言し、
/// 現れたら赤になるようにしてある（module doc を参照）。
const EXCLUSION_FORMS: &[(Form, Presence)] = &[
    (Form::OldLocation, Presence::Present),
    (
        Form::CheckedInTree,
    ...
    (
        Form::Namespace,
        Presence::ArrivesIn("2.5（展開先の根を作る）／5.2（窓口を展開する形へ切り替える）"),
    ),
    (
        Form::BundledBalloonPath,
        Presence::ComposedAtRuntime(
...
fn the_checked_in_sample_tree_is_detected_but_the_nar_file_name_is_not() {
    let name = SAMPLES
        .iter()
        .find(|s| s.checked_in_parent == VENDOR_PARENT)
        .map(|s| s.name)
        .expect("展開形が vendors 配下に置かれた検体が登記されている");
