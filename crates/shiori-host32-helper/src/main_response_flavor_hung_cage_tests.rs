...
//! # 何を固定するか
//!
//! ホスト役の窓を持つスレッドはメッセージを取り出さない（本番のホストも往復の間は自分の
//! `SendMessageTimeoutW` の中で待ち、取り出さない）。helper はその窓へ応答を**再入で**送り返す。
//! 応答方向に `SMTO_ABORTIFHUNG` が付いていると、OS がホストのスレッドを「応答なし」と見なした後は、この
...
//!   （往復中の本番ホストと同じ「取り出さない」姿。手空き時の周期的な保守は `areka-kanade` の
//!   決定論テストが固定する）。往復は本番の
